...
import { formatdiagnostics, lintplanfile, parseplanfile, planlintsummary, rulesetcachekey, plansteprisk } from "./plan.js";
import { dryflowdriver, flowrunreport, parseflowrunrequest, parsegrantsfile, renderflowevents, renderflowtimeline, runflow } from "./flow.js";
import type { flowdriver } from "./flow.js";
import { exportchain, exportdescriptorof, exportextractions, exportnotes, exportrecords, secretfieldshapes } from "./export.js";
import { openheadlesssession, openlibraryrun, remoteattachframes } from "./headless.js";
import { dryrunworkflow, newworkflowrun, runworkflow, composeworkflow, workflowblockof, workflowstepof } from "./workflow.js";
import { platformtargets } from "./runtime.js";
import { adapterdeclarationsof, capabilityprobeof, filesystemstorageadapter, portablecapabilityset } from "./runtime.js";
import { actionkindcatalog, actionrisk, dryrunprojection, portablerulesetof, mcpmodegate, nativeinstallconsentgate, cssselectorvalid, fixtureconsentgate, kindoptionfields, validatetargetref } from "./policy.js";
import { clicommands, surfacepalette } from "./views.js";
import { apifreezedate, apifreezerelease, capmanifestof, deprecationwindow, protocolsupported, sharedprotocolversion, type capmanifestsurface } from "./apifreeze.js";
...
import { createservesession, routeserveframe, shutdowndrain, startserve, transportendpoints, defaultmcpport, localhostbind } from "./mcp.js";
import { createlinepump, httpanswer, httpendpoint, parsepost, routepath } from "./http.js";
import { hostmanifestdestination, installnativehost, nativedefaultstate, nativediagnostics, nativehostmanifesttemplate, uninstallnativehost } from "./bridge.js";
import type { auditevent, cliconfiguration, clientrecord, consentprovider, headlessfixture, immutablelogentry, mcpserverconfig, planfile, planfilestep, planlintdiagnostic, portableruleset, runlogentry, sitenote, stepoutcome, storedrunlog, toolmock, transportkind, variablescope, workflowstep, exportdescriptor, exportresult, headlessfixtureoutcome, manifestcheckfinding, planprogress, planrisksummary, runworkflowoutcome, workflowdocument, workflowrecord, workflowrun } from "./types.js";

const forbidden = new Set(["debugger", "cookies", "webRequest", "history", "bookmarks", "nativeMessaging", "proxy", "management"]);
const allowedoptional = new Set(["tabs", "downloads", "clipboardRead", "clipboardWrite", "offscreen", "nativeMessaging"]);
...
function decodeidentitykey(key: string): Buffer {
  ...
  if (!der.subarray(0, 2).equals(Buffer.from([0x30, 0x82]))) throw new Error("The manifest key does not decode to a DER subject public key sequence.");
...
async function recordedidentitydigest(): Promise<string | undefined> {
  ...
  for (const candidate of ["checksums.txt", "dist/checksums.txt"]) {
    ...
    } catch { /* an unbuilt tree carries no artifact record: the structural identity checks alone stand, and the build regenerates the record on every run */ }
  }
  return undefined;
}
...
async function globalstate(args: string[]): Promise<{ config: cliconfiguration; verbosity: "quiet" | "normal" | "verbose"; json: boolean; flags: Map<string, string>; positional: string[] }> {
  const { positional, flags } = parseflags(args);
  const configpath = flags.get("config") ?? defaultconfigfile;
  let raw: unknown = {};
  try { raw = JSON.parse(await readFile(configpath, "utf8")); } catch { /* an absent or unreadable configuration keeps every default off; a present but malformed one refuses below */ }
  const config = parsecliconfig(raw);
  const verbosityflag = flags.get("verbose") === "true" ? "verbose" : flags.get("quiet") === "true" ? "quiet" : undefined;
...
async function cachedruleset(cachedir: string): Promise<portableruleset> {
  ...
  } catch { /* a missing or unreadable cache recompiles below */ }
...
function readonlykinds(): string[] {
  return actionkindcatalog().filter(kind => { try { return actionrisk(kind as never) === "read"; } catch { return false; } });
}
...
async function cmdplanlint(args: string[], state: { config: cliconfiguration; verbosity: "quiet" | "normal" | "verbose"; json: boolean }): Promise<void> {
  const { positional, flags } = parseflags(args);
  const target = positional[0] ?? state.config.defaultplan;
  if (target === undefined || target.trim() === "") throw new Error("The planlint command needs the plan file or directory it lints, or the default plan location in the configuration.");
  ...
  const capabilities = flags.has("capabilities")
    ...
    : portablecapabilityset({ vocabulary: actionkindcatalog(), probes: capabilityprobeof({ dom: flags.get("domless") !== "true", storage: true, network: true, worker: true }), domlesskinds: readonlykinds() });
  const allowlist = flags.has("allowlist")
    ? (flags.get("allowlist") as string).split(",").map(origin => origin.trim()).filter(origin => origin !== "")
  ...
  const report: Array<{ file: string; diagnostics: planlintdiagnostic[]; risk: ReturnType<typeof planrisksummaryof>; exitcode: number }> = [];
  let exitcode = 0;
  ...
  if (state.verbosity !== "quiet" && state.verbosity === "verbose" && format === "human") console.log(`The planlint ran ${ruleset.rules.length} portable rules over ${files.length} plan file${files.length === 1 ? "" : "s"} with ${capabilities.length} declared capabilities${allowlist !== undefined ? ` and the consent allowlist of ${allowlist.length} origin${allowlist.length === 1 ? "" : "s"}` : ""}.`);
...
async function cmdmigrateplan(args: string[], state: { config: cliconfiguration; verbosity: "quiet" | "normal" | "verbose"; json: boolean }): Promise<void> {
  const { positional, flags } = parseflags(args);
  const source = positional[0];
  if (source === undefined || source.trim() === "") throw new Error("The migrateplan command needs the source file it converts.");
  ...
  const parsed = parseplanfile(JSON.parse(serialized)); /* the frozen grammar verifies the exact bytes the command writes */
  const ruleset = await cachedruleset(flags.get("cache") ?? ".devthink");
  const capabilities = flags.has("capabilities")
    ? JSON.parse(await readFile(flags.get("capabilities") as string, "utf8")) as string[]
    : portablecapabilityset({ vocabulary: actionkindcatalog(), probes: capabilityprobeof({ dom: flags.get("domless") !== "true", storage: true, network: true, worker: true }), domlesskinds: readonlykinds() });
...
async function cmdrecipes(args: string[], state: { config: cliconfiguration; verbosity: "quiet" | "normal" | "verbose"; json: boolean }): Promise<void> {
  const { positional, flags } = parseflags(args);
  const candidates = ["gallery.json", "dist/gallery.json", "tests/code/recipes/gallery.json"];
  let gallery: { entries: Array<{ id: string; category: string; difficulty: string; description: string; fixture: string; origin: string; expecteddurationms: number }>; fixturepages: Array<{ name: string }> } | undefined;
  let gallerypath = "";
  for (const candidate of candidates) {
    try { gallery = JSON.parse(await readFile(candidate, "utf8")); gallerypath = candidate; break; } catch { /* the built index first, the repository source second, and neither found refuses below */ }
  }
  if (gallery === undefined) throw new Error("The recipes command needs the gallery index the installed package ships (gallery.json at the package root), the build copy (dist/gallery.json) or the repository source (tests/code/recipes/gallery.json); run from the package root or run pnpm build first.");
  const id = positional[0];
  if (id === undefined) {
    if (state.json || flags.get("format") === "json") { console.log(JSON.stringify(gallery, null, 2)); return; }
    for (const entry of gallery.entries) console.log(`${entry.id.padEnd(24)} ${entry.category.padEnd(11)} ${entry.difficulty.padEnd(12)} ${entry.fixture.padEnd(17)} ${entry.description}`);
  ...
  for (const candidate of recipecandidates) {
    try { recipetext = await readFile(candidate, "utf8"); break; } catch { /* the index sibling first, the built copy second */ }
  }
  if (recipetext === undefined) throw new Error(`The recipe file of ${id} sits beside neither the gallery index nor the shipped fixture set; run from the package root or run pnpm build first.`);
  ...
  if (flags.get("dryrun") === "true") {
    ...
    const provider: consentprovider = { async resolvegate(gate) { resolvedgates.push(gate.id); return "approve"; } };
...
async function attachremotedriver(endpoint: string, origin: string): Promise<flowdriver> {
  ...
  if (attached.error !== undefined || attached.result?.attached !== true) throw new Error(`The remote session at ${endpoint} refused the attach: ${attached.error?.message ?? "the endpoint returned no attached result."}`);
  let sequence = 1;
  return {
    async execute(step) {
      sequence += 1;
      const started = Date.now();
      const frame = toolcallframe({ id: sequence, name: `browser.${step.kind}`, params: { id: step.id, kind: step.kind, label: step.label, ...(step.target !== undefined ? { target: step.target } : {}), ...(step.value !== undefined ? { value: step.value } : {}), ...(step.options !== undefined ? { options: step.options } : {}) } });
      ...
      if (body.error !== undefined) return { state: "failed", summary: `The remote session refused the step ${step.id}: ${body.error.message ?? "the endpoint returned an error."}`, duration };
...
async function cmdflowrun(args: string[]): Promise<void> {
  ...
  const grants = request.options.grantspath !== undefined ? parsegrantsfile(JSON.parse(await readFile(request.options.grantspath, "utf8"))) : [];
  ...
  const driver = endpoint !== undefined ? await attachremotedriver(endpoint, file.origin) : request.options.dryrun ? dryflowdriver() : undefined;
  if (driver === undefined) throw new Error("The flowrun needs a remote endpoint (--endpoint) or the dry run flag (--dryrun); the cli drives no local browser itself.");
  const profiledir = flags.get("profile") ?? ".devthink/profile";
  const store = new sessionmemory(filesystemstorageadapter({ readfile: path => readFile(path, "utf8"), writefile: (path, data) => writeFile(path, data, "utf8"), mkdir: path => mkdir(path, { recursive: true }), join: (...parts: string[]) => join(...parts) }, profiledir));
  ...
  try {
    const run = await runflow({ request, file, grants, ...(provider !== undefined ? { provider } : {}), driver, now: Date.now(), clock: () => Date.now(), emit: event => { if (request.options.format === "human") console.log(renderflowevents([event], "human")); } });
...
async function cmdrunworkflow(args: string[], state: { config: cliconfiguration; verbosity: "quiet" | "normal" | "verbose"; json: boolean }): Promise<void> {
  const { positional, flags } = parseflags(args);
  const workflowpath = positional[0];
  if (workflowpath === undefined || workflowpath.trim() === "") throw new Error("The runworkflow command needs the workflow file it runs.");
  ...
  if (resumeid !== undefined) {
    const checkpoints = JSON.parse(await readFile(checkpointfile, "utf8")) as Array<{ runid: string; cursor: number; scopes: variablescope[]; log: runlogentry[]; outputs: Record<string, stepoutcome> }>;
    const checkpoint = [...checkpoints].reverse().find(entry => entry.runid === resumeid);
    if (checkpoint === undefined) throw new Error(`No checkpoint of the run ${resumeid} sits beside ${workflowpath}; the resume needs its checkpoint id.`);
  ...
  if (state.verbosity !== "quiet") {
    const approval = await ask(`Run the workflow ${record.name} of ${record.steps.length} step${record.steps.length === 1 ? "" : "s"} on ${record.origins.join(", ")}${resumed ? ` resuming from the checkpoint ${run.cursor}` : ""}${dryrun ? " as a dry run" : ""}? [y/N] `);
  ...
  if (dryrun) {
    const dried = dryrunworkflow({ record, run, ...(scopes !== undefined ? { scopes } : {}), ...(log !== undefined ? { log } : {}), now: Date.now(), projection: dryrunprojection });
    summary = runworkflowsummaryof({ runid: dried.run.id, run: dried.run, log: dried.log });
  } else {
    const endpoint = flags.get("endpoint");
    if (endpoint === undefined) throw new Error("The runworkflow command needs a remote endpoint (--endpoint) or the dry run flag (--dryrun); the cli drives no local browser itself.");
    const driver = await attachremotedriver(endpoint, record.origins[0] as string);
    const checkpoints = [] as Array<{ runid: string; cursor: number; scopes: variablescope[]; log: runlogentry[]; outputs: Record<string, stepoutcome> }>;
    const executed = await runworkflow({
      ...
      execute: async (step: workflowstep, context) => {
        ...
        try { risk = actionrisk(step.kind as never); } catch { /* an unknown kind already refused at composition */ }
        if (risk === "sensitive" && state.verbosity !== "quiet") {
          const answer = await ask(`The step ${step.id} of kind ${step.kind} grades sensitive on ${record.origins.join(", ")}.\nApprove the step before it runs? [y/N] `);
        ...
        return { ok: result.state === "done", summary: result.summary, ...(context.block !== undefined ? { details: { block: context.block } } : {}) };
      },
      now: Date.now(),
      gates: { sessionactive: true, planapproved: true, origingranted: origin => record.origins.includes(origin) },
      oncheckpoint: async checkpoint => {
        if (state.verbosity !== "quiet" && format === "human") console.log(`step ${checkpoint.run.cursor}/${record.steps.length}: ${(checkpoint.log.at(-1)?.summary) ?? ""}`);
  ...
  await writeFile(`${workflowpath}.audit.jsonl`, workflowauditlines({ runid: summary.runid, workflow: record.name, log: summary.steps.map(entry => ({ stepid: entry.stepid, label: entry.label, state: entry.state as "done", startedat: 0, duration: entry.duration, summary: entry.summary })), now: Date.now() }) + "\n", "utf8");
  if (format === "json") console.log(JSON.stringify(summary, null, 2));
    ...
    console.log(`The workflow run ${summary.runid} ended ${summary.state} at checkpoint ${summary.checkpoint} with the exit class ${summary.exitclass} (${summary.exitcode}).${summary.reason !== undefined ? ` ${summary.reason}` : ""}`);
  }
...
async function cmdexportdata(args: string[], state: { config: cliconfiguration; verbosity: "quiet" | "normal" | "verbose"; json: boolean }): Promise<void> {
  const { positional, flags } = parseflags(args);
  ...
  if (input !== undefined) {
    ...
    const store = new sessionmemory(filesystemstorageadapter({ readfile: path => readFile(path, "utf8"), writefile: (path, data) => writeFile(path, data, "utf8"), mkdir: path => mkdir(path, { recursive: true }), join: (...parts: string[]) => join(...parts) }, storedir));
  ...
  const shapes = flags.has("shapes") ? (flags.get("shapes") as string).split(",").map(shape => shape.trim()).filter(shape => shape !== "") : [...defaultmaskshapes];
  const out = flags.get("out");
  const exported = exportdatacontent({ scope, format, records, shapes, ...(from !== undefined ? { from } : {}), ...(to !== undefined ? { to } : {}), ...(out !== undefined ? { path: out } : {}) });
  ...
  if (out !== undefined) { await mkdir(resolve(out, ".."), { recursive: true }); await writeFile(out, exported.content + "\n", "utf8"); if (state.verbosity !== "quiet") console.log(`The exportdata wrote ${out}.`); }
...
async function cmdheadless(args: string[], state: { config: cliconfiguration; verbosity: "quiet" | "normal" | "verbose"; json: boolean }): Promise<void> {
  const { positional, flags } = parseflags(args);
  const planpath = positional[0] ?? state.config.defaultplan;
  if (planpath === undefined || planpath.trim() === "") throw new Error("The headless command needs the plan file it replays, or the default plan location in the configuration.");
  const fixturesdir = flags.get("fixtures") ?? state.config.fixturesdir;
  if (fixturesdir === undefined || fixturesdir.trim() === "") throw new Error("The headless command needs the fixtures directory (--fixtures) or the fixturesdir location in the configuration.");
  ...
  const handle = openlibraryrun({ plan, statesource: origin => fixtures.find(candidate => candidate.origin === origin), ...(flags.has("grants") ? { policy: { grants: (flags.get("grants") as string).split(",").map(grant => grant.trim()).filter(grant => grant !== "") } } : {}), now: Date.now() });
  const outcomes: Array<{ stepid: string; kind: string; state: "done" | "refused" | "unsupported"; summary: string; details?: Record<string, unknown> }> = [];
...
async function cmdserve(args: string[]): Promise<void> {
  ...
  const transports: transportkind[] = transportflag === "stdio" ? ["stdio"] : transportflag === "http" ? ["http"] : ["stdio", "http"];
  const portflag = flags.get("port");
  const port = portflag !== undefined && Number.isFinite(Number(portflag)) && Number(portflag) > 0 && Number(portflag) <= 65535 ? Math.floor(Number(portflag)) : defaultmcpport;
  ...
  const config: mcpserverconfig = {
    ...
    ...(framesizeflag !== undefined && Number.isFinite(Number(framesizeflag)) ? { framesize: Math.floor(Number(framesizeflag)) } : {}),
    ...(queuedepthflag !== undefined && Number.isFinite(Number(queuedepthflag)) ? { queuedepth: Math.floor(Number(queuedepthflag)) } : {}),
    ...(drainwindowflag !== undefined && Number.isFinite(Number(drainwindowflag)) ? { drainwindow: Math.floor(Number(drainwindowflag)) } : {})
  };
  ...
  const mocks: toolmock[] = mocknames.map(tool => ({ tool, result: { content: `The ${tool} mock answered from the serve test context; no browser was touched.`, iserror: false }, testcontext: true, createdat: startedat }));
  const client: clientrecord = { id: "serve-stdio", transport: "stdio", paired: flags.get("pair-client") === "true", connectedat: startedat, ...(flags.get("pair-client") === "true" ? { pairedat: startedat } : {}) };
  const holder: { session: ReturnType<typeof createservesession> } = { session: createservesession({ config, client, origin: "stdio", now: startedat }) };
  ...
  const handle = async (frame: import("./types.js").jsonrpcframe): Promise<import("./types.js").jsonrpcframe | undefined> => {
  ...
  const pump = createlinepump({ write: line => { process.stdout.write(line); }, handle, now: startedat });
  ...
  const stop = (): void => {
    if (stopping) return;
    stopping = true;
    const drain = shutdowndrain({ inflight: holder.session.contexts, now: Date.now(), ...(config.drainwindow !== undefined ? { window: config.drainwindow, drainstart: startedat } : {}) });
  ...
  if (transports.includes("stdio")) {
    ...
    process.stdin.on("data", chunk => { void pump.feed(typeof chunk === "string" ? chunk : String(chunk)).catch(error => { process.stderr.write(`${error instanceof Error ? error.message : String(error)}\n`); }); });
  ...
  if (transports.includes("http")) {
    ...
    const handler = async (request: import("node:http").IncomingMessage, response: import("node:http").ServerResponse): Promise<void> => {
      ...
      response.end(answers.length === 1 ? httpanswer(answers[0] as import("./types.js").jsonrpcframe) : httpanswer({ jsonrpc: "2.0", id: null, error: { code: "internal", message: "The posted message produced no answer." } }));
    };
    ...
    if (certpath !== undefined && keypath !== undefined) {
      ...
      server = createsecure({ cert: await readFile(certpath, "utf8"), key: await readFile(keypath, "utf8") }, handler as never);
    ...
    await new Promise<void>(resolve => { server.listen(endpoint.port, endpoint.bind, () => { resolve(); }); });
  ...
  await new Promise<void>(() => { /* the serve runs until the stdin closes or the process receives a stop signal */ });
}
...
async function cmdnative(args: string[]): Promise<void> {
  ...
  if (subcommand === "install") {
    ...
    if (outcome.refused !== undefined) { console.error(outcome.refused); process.exitCode = exitcodeof("schemaerror"); return; }
  ...
  if (subcommand === "uninstall") {
    const outcome = await uninstallnativehost({
      ...
      io: {
        ...
        removefile: async path => { await (await import("node:fs/promises")).rm(path, { force: true }); }
      }
    });
    if (outcome.refused !== undefined) { console.error(outcome.refused); process.exitCode = exitcodeof("schemaerror"); return; }
  ...
  if (subcommand === "diagnostics") {
    ...
    const template = hostname !== "" && companionpath !== "" ? nativehostmanifesttemplate({ hostname, companionpath }) : undefined;
    const destination = hostname !== "" ? hostmanifestdestination({ hostname, ...(profiledir !== "" ? { profiledir } : {}), ...(systemwide ? { systemwide: true } : {}), platform }) : undefined;
    const report = nativediagnostics({ state: nativedefaultstate(), now: Date.now() });
    console.log(JSON.stringify({ ...report, ...(destination !== undefined ? { manifestdestination: destination } : {}), ...(template !== undefined ? { manifesttemplate: template.manifest } : {}) }, null, 2));
    return;
  }
...
async function cmdexport(args: string[]): Promise<void> {
  ...
  const exported = scope === "runs"
    ? await exportchain({ descriptor, log: { runid: typeof (payload as { runid?: string }).runid === "string" ? (payload as { runid: string }).runid : "run", sessionid: "flowrun", entries: (payload as { log: immutablelogentry[] }).log, updatedat: Date.now() } as storedrunlog, shapes, ...(out !== undefined ? { path: out } : {}) })
    : scope === "extractions"
      ? exportextractions({ descriptor, rows: payload as Array<Record<string, unknown>>, shapes, ...(out !== undefined ? { path: out } : {}) })
      : exportnotes({ descriptor, notes: payload as sitenote[], shapes, ...(out !== undefined ? { path: out } : {}) });
...
async function cmdinit(args: string[]): Promise<void> {
  ...
  if (target === undefined || target.trim() === "") throw new Error("The init command needs the plan file path it scaffolds.");
  const scaffold = {
...
async function cmdcommands(args: string[]): Promise<void> {
  ...
  if (flags.get("format") === "json") { console.log(JSON.stringify(registry, null, 2)); return; }
...
async function main(): Promise<void> {
  ...
  if (command === "manifest") {
    ...
    const manifest = JSON.parse(manifesttext) as { version: string; key?: string; permissions?: string[]; optional_permissions?: string[]; host_permissions?: string[]; sandbox?: { pages?: string[] }; offscreen?: { document?: string; reasons?: string[]; justification?: string }; content_scripts?: Array<{ matches?: string[]; world?: string; js?: string[] }> };
    ...
    if (recorded !== undefined && digest !== recorded) throw new Error(`The manifest identity key digest ${digest} does not match the recorded build artifact digest ${recorded}; changing the extension identity must be a reviewed decision through a rebuilt artifact.`);
    ...
    } catch { /* a manifest check before a build carries no bundled scripts; the deep checks report the absent bytes */ }
    ...
    if (deep.exitcode !== 0) {
      for (const finding of deep.findings.filter(entry => entry.severity === "error")) console.error(`${finding.severity.toUpperCase()} ${finding.rule} at ${finding.path}${finding.source !== undefined ? ` (${finding.source})` : ""}: ${finding.message}`);
    ...
    const capsurfaces: capmanifestsurface[] = ["background", "pagebridge", "sidepanel", "popup", "cli", "library", "mcp"];
    const capmanifestfiles: Array<{ surface: string; release: string; protocolmajor: number; messages: number; permissions: number }> = [];
    for (const surface of capsurfaces) {
      const capstext = await readFile(new URL(`./caps/${surface}.json`, import.meta.url), "utf8");
      const stored = JSON.parse(capstext) as { surface: string; release: string; protocolmajor: number; messages: string[]; kinds: string[]; permissions: string[] };
    ...
    console.log(JSON.stringify({
      ...
      ...(manifest.offscreen !== undefined ? { offscreendocument: manifest.offscreen.document, offscreenreasons: manifest.offscreen.reasons } : {}),
      deepchecks: { rules: deep.findings.length, findings: deep.findings.map(finding => ({ rule: finding.rule, path: finding.path, severity: finding.severity, ...(finding.source !== undefined ? { source: finding.source } : {}), message: finding.message })) }
    }, null, 2));
    return;
  }
  const state = await globalstate(args);
  if (command === "planlint") { await cmdplanlint(args, state); return; }
  if (command === "migrateplan") { await cmdmigrateplan(args, state); return; }
  if (command === "recipes") { await cmdrecipes(args, state); return; }
  if (command === "flowrun") { await cmdflowrun(args); return; }
  if (command === "runworkflow") { await cmdrunworkflow(args, state); return; }
  if (command === "exportdata") { await cmdexportdata(args, state); return; }
  if (command === "headless") { await cmdheadless(args, state); return; }
  if (command === "serve") { await cmdserve(args); return; }
  if (command === "native") { await cmdnative(args); return; }
  if (command === "export") { await cmdexport(args); return; }
  if (command === "init") { await cmdinit(args); return; }
  if (command === "doctor") { await cmddoctor(); return; }
  if (command === "commands") { await cmdcommands(args); return; }
  if (command === "describe") { await cmddescribe(args); return; }
  if (command === "help" || command === "--help") { await cmdhelp(); return; }
...
if (cliDirectEntry || cliBunEntry) main().catch(error => { console.error(error instanceof Error ? error.message : String(error)); process.exitCode = exitcodeof("schemaerror"); });
...
export const cliexitclasses: readonly string[] = ["ok", "consentrefused", "stepfailed", "schemaerror", "unsupported", "cancelled"];
...
export function exitcodeof(exitclass: string): number {
  const codes: Record<string, number> = { ok: 0, consentrefused: 1, stepfailed: 2, schemaerror: 3, unsupported: 4, cancelled: 5 };
  const code = codes[exitclass];
  if (code === undefined) throw new Error(`The exit class ${exitclass} stays outside the documented classes ${cliexitclasses.join(", ")}.`);
  return code;
}
...
export const manifestkeyallowlist: readonly string[] = [
  "manifest_version", "key", "name", "version", "description", "default_locale",
  "permissions", "optional_permissions", "optional_host_permissions", "host_permissions",
  "content_scripts", "content_security_policy", "web_accessible_resources",
  "sandbox", "offscreen", "background", "action", "side_panel",
  "chrome_url_overrides", "options_ui", "icons", "minimum_chrome_version",
  "browsers", "vsix"
];
...
export function capabilityapireport(permissions: string[]): Array<{ api: string; minchrome: number }> {
  const floors: Record<string, number> = { activeTab: 88, storage: 88, scripting: 88, sidePanel: 114, tabs: 88, downloads: 88, clipboardRead: 88, clipboardWrite: 88, offscreen: 109, nativeMessaging: 88 };
  return permissions.flatMap(permission => floors[permission] !== undefined ? [{ api: permission, minchrome: floors[permission] as number }] : []);
}
...
export function deepmanifestchecks(input: { manifest: unknown; manifesttext: string; manifestfile: string; filebytes: Record<string, Uint8Array>; capabilities: Array<{ api: string; minchrome: number }>; digestof: (bytes: Uint8Array) => string; bundlepresent?: boolean }): { findings: manifestcheckfinding[]; exitcode: number } {
  ...
  for (const key of Object.keys(manifest)) {
    if (!manifestkeyallowlist.includes(key)) findings.push({ rule: "manifest.key.allowlist", path: key, ...(manifestsourceline(input.manifesttext, input.manifestfile, `"${key}"`) !== undefined ? { source: manifestsourceline(input.manifesttext, input.manifestfile, `"${key}"`) as string } : {}), severity: "error", message: `The manifest key ${key} stays outside the runtime policy allowlist; an undeclared key never ships unreviewed.` });
  }
  const permissions = Array.isArray(manifest.permissions) ? manifest.permissions.filter((permission): permission is string => typeof permission === "string") : [];
  const optional = Array.isArray(manifest["optional_permissions"]) ? manifest["optional_permissions"].filter((permission): permission is string => typeof permission === "string") : [];
  for (const permission of permissions) {
    const source = manifestsourceline(input.manifesttext, input.manifestfile, `"${permission}"`);
    findings.push({ rule: "manifest.permission.source", path: `permissions.${permission}`, ...(source !== undefined ? { source } : {}), severity: "info", message: `The required permission ${permission} declares at ${source ?? "an unknown line"}.` });
  }
  for (const permission of optional) {
    const source = manifestsourceline(input.manifesttext, input.manifestfile, `"${permission}"`);
    findings.push({ rule: "manifest.permission.source", path: `optional_permissions.${permission}`, ...(source !== undefined ? { source } : {}), severity: "info", message: `The optional permission ${permission} declares at ${source ?? "an unknown line"}.` });
  }
  ...
  if (csp === undefined) {
    ...
    const policy = (csp as Record<string, unknown>)["extension_pages"] ?? (csp as Record<string, unknown>)["extension_scripts"];
    if (typeof policy !== "string") findings.push({ rule: "manifest.csp.hashes", path: "content_security_policy.extension_pages", severity: "error", message: "The extension pages policy needs its string form." });
      ...
      for (const hash of hashes) {
        ...
        findings.push(matches.length > 0
          ? { rule: "manifest.csp.hashes", path: "content_security_policy.extension_pages", severity: "info", message: `The script hash ${hash} verifies against ${matches.map(([name]) => name).join(", ")}.` }
          : { rule: "manifest.csp.hashes", path: "content_security_policy.extension_pages", severity: "error", message: `The script hash ${hash} verifies against no bundled script; every declared hash pins a shipped file.` });
      }
    }
  }
  ...
  if (resources === undefined) {
    ...
    const declared = Array.isArray(resources) ? resources.flatMap(entry => typeof entry === "string" ? [entry] : entry && typeof entry === "object" && Array.isArray((entry as Record<string, unknown>).resources) ? ((entry as Record<string, unknown>).resources as unknown[]).filter((resource): resource is string => typeof resource === "string") : []) : [];
    ...
    for (const resource of declared) {
      ...
      if (!reviewedwebresources.includes(resource)) findings.push({ rule: "manifest.war.reviewed", path: "web_accessible_resources", ...(source !== undefined ? { source } : {}), severity: "error", message: `The web accessible resource ${resource} stays outside the reviewed resource set ${reviewedwebresources.join(", ")}.` });
    }
  }
  ...
  if (icons === undefined) {
    ...
    for (const [size, path] of Object.entries(icons as Record<string, unknown>)) {
      ...
      if (typeof path !== "string" || path.trim() === "") { findings.push({ rule: "manifest.icons", path: `icons.${size}`, severity: "error", message: `The icon of size ${size} needs its file path.` }); continue; }
      const bytes = input.filebytes[path];
      if (bytes === undefined) {
        if (input.bundlepresent === false) { findings.push({ rule: "manifest.icons", path: `icons.${size}`, severity: "info", message: `The icon ${path} of size ${size} rides the browser bundle of the release zip channel; the running tree carries no bundle to verify against — a repository checkout verifies it after pnpm build and an installed library package carries the manifest structure alone.` }); continue; }
        const source = manifestsourceline(input.manifesttext, input.manifestfile, path); findings.push({ rule: "manifest.icons", path: `icons.${size}`, ...(source !== undefined ? { source } : {}), severity: "error", message: `The icon ${path} of size ${size} ships no file in the bundle.` }); continue;
      }
      try {
...
export function planlintfindings(input: { file: planfile; capabilities?: string[]; allowlist?: string[] }): planlintdiagnostic[] {
  ...
  input.file.steps.forEach((step, index) => {
    ...
    if (capabilities !== undefined && !capabilities.has(step.kind)) diagnostics.push({ code: "plan.capability.catalog", path: `${path}.kind`, severity: "error", message: `The step ${step.id} carries the kind ${step.kind} outside the declared capability set; the target runtime never executes it.` });
  });
  if (input.allowlist !== undefined && input.allowlist.length > 0 && !input.allowlist.includes(input.file.origin)) diagnostics.push({ code: "plan.origin.allowlist", path: "origin", severity: "error", message: `The plan origin ${input.file.origin} stays outside the consent allowlist of ${input.allowlist.length} origin${input.allowlist.length === 1 ? "" : "s"}; the extension demands the same grant before any step runs.` });
  return diagnostics;
}
...
export function parseworkflowdocument(value: unknown): workflowdocument {
  ...
  const blocks = candidate.blocks === undefined ? undefined : (candidate.blocks as unknown[]).map((block, index) => {
    if (!block || typeof block !== "object") throw new Error(`The workflow block at index ${index} must be an object.`);
    return block as Record<string, unknown>;
  });
  return { version: root.version, workflow: { name: candidate.name, version: candidate.version, origins: candidate.origins as string[], steps, ...(blocks !== undefined ? { blocks } : {}) } };
}
...
export function composeworkflowdocument(document: workflowdocument, now: number): workflowrecord {
  ...
  return composeworkflow({
    ...
    blocks,
    now,
    kindallowed: kind => { try { actionrisk(kind as never); return true; } catch { return false; } },
    riskof: kind => { try { return actionrisk(kind as never); } catch { return "sensitive" as const; } }
  });
}
...
export function runworkflowsummaryof(input: { runid: string; run: workflowrun; log: runlogentry[] }): runworkflowoutcome {
...
export function workflowauditlines(input: { runid: string; workflow: string; log: runlogentry[]; now: number }): string {
...
export function exportdatawindow(records: Array<Record<string, unknown>>, from?: number, to?: number, timefield = "at"): Array<Record<string, unknown>> {
...
export function secretstorerefusal(records: Array<Record<string, unknown>>): { refused: boolean; fields: string[]; reason?: string } {
...
export function exportdatacontent(input: { scope: "session" | "audit" | "extraction"; format: string; records: Array<Record<string, unknown>>; shapes: string[]; from?: number; to?: number; path?: string }): { result: exportresult; content: string } {
  ...
  if (refusal.refused && refusal.reason !== undefined) return { result: { descriptor, bytes: 0, rows: 0, ...(input.path !== undefined ? { path: input.path } : {}), reason: refusal.reason }, content: "" };
  const windowed = exportdatawindow(input.records, input.from, input.to);
  return exportrecords({ descriptor, records: windowed, shapes: input.shapes, ...(input.path !== undefined ? { path: input.path } : {}) });
}
...
export function parseheadlessfixture(value: unknown): headlessfixture {
  ...
  for (const field of ["schemaversion", "url", "title", "textpreview", "textlength", "forms", "interactive", "capturedat"]) {
    if (observation[field] === undefined) throw new Error(`The fixture observation misses its ${field} field of the live snapshot schema.`);
  }
  ...
  return { id: root.id, origin: root.origin, observation: root.observation as headlessfixture["observation"], grants: root.grants.filter((grant): grant is string => typeof grant === "string"), recordedat: root.recordedat };
}
...
export function resolvefixture(fixtures: headlessfixture[], origin: string): headlessfixture {
  ...
  if (resolved === undefined) throw new Error(`No recorded fixture covers the origin ${origin}; headlessmode replays only against recorded page state.`);
  return resolved;
}
...
function convertedstep(input: { entry: string; id: string; kind: string; label: string; target?: string; value?: string; options?: string }): planfilestep {
  const catalog = new Set(actionkindcatalog());
  ...
  let risk: "read" | "interaction" | "sensitive" = "sensitive";
  try { risk = actionrisk(input.kind as never); } catch { /* the catalog check above already refused the kind */ }
...
export function importv1plan(input: { source: unknown; now: number }): conversionrecord {
  try {
    ...
    const passthrough: conversionrecord = { file: current, format: "v1", sourceversion: current.version, mapped: current.steps.length, notes: [
    ...
    return passthrough;
  } catch { /* the source is not the current grammar: the legacy grammar below reads it */ }
...
export function importautoma(input: { source: unknown; now: number }): conversionrecord {
  ...
  for (const [key, rawblock] of Object.entries(blocks)) {
    ...
    const data: Record<string, unknown> = typeof block.data === "object" && block.data !== null && !Array.isArray(block.data) ? block.data as Record<string, unknown> : {};
    if (type === "trigger") {
      pausedtriggers += 1;
  ...
  if (origin === undefined) throw new Error("The automa workflow carries no newtab block; the plan origin derives from the first newtab url, so a workflow without one refuses instead of guessing an origin.");
...
export function importselenium(input: { source: unknown; now: number }): conversionrecord {
  ...
  commands.forEach((rawcommand, index) => {
    ...
    if (name === "open") {
      if (target === undefined) throw new Error(`The selenium command ${id} of open carries no target; the open command names the url it resolves against the side file base.`);
      let resolved: string;
    ...
    if (name === "click" || name === "type" || name === "sendKeys" || name === "select") {
      if (target === undefined) throw new Error(`The selenium command ${id} of ${name} carries no target; a step that addresses the page names its selector.`);
    ...
    if (name === "pause") {
      if (target === undefined || !/^\d+$/.test(target.trim()) || Number(target.trim()) < 1) throw new Error(`The selenium command ${id} of pause carries its target ${target ?? ""} as the milliseconds the reviewed delay grammar refuses; the pause needs a positive integer of milliseconds.`);
...
export function importuivision(input: { source: unknown; now: number }): conversionrecord {
  ...
  commands.forEach((rawcommand, index) => {
    ...
    if (name === "open") {
      if (target === undefined) throw new Error(`The ui vision command ${id} of open carries no Target; the open command names the absolute url it opens.`);
    ...
    if (name === "click" || name === "type" || name === "verifyText") {
      if (target === undefined) throw new Error(`The ui vision command ${id} of ${name} carries no Target; a step that addresses the page names its selector.`);
  ...
  if (origin === undefined) throw new Error("The ui vision macro carries no open command; the plan origin derives from the first open target, so a macro without one refuses instead of guessing an origin.");
  const gated = steps.filter(step => step.gate === true).map(step => step.id);
  notes.push(
...
export function parsecsvrows(text: string): string[][] {
  ...
  const pushcell = (): void => { row.push(cell); cell = ""; cellstarted = false; };
  const pushrow = (): void => { if (row.length > 0 || cellstarted || cell !== "") { pushcell(); rows.push(row); row = []; } };
  for (let index = 0; index < text.length; index += 1) {
    ...
    if (char === "\"") { inquote = true; cellstarted = true; continue; }
...
export function importtabular(input: { text: string; now: number }): conversionrecord {
  ...
  if (goal === undefined || goal === "") throw new Error("The tabular source carries no `# goal:` preface line; a csv row set holds no metadata of its own, so the goal must ride the preface the conversion reads.");
  if (origin === undefined || origin === "") throw new Error("The tabular source carries no `# origin:` preface line; a csv row set holds no metadata of its own, so the HTTPS origin must ride the preface the conversion reads.");
  ...
  if (header === undefined) throw new Error("The tabular source carries no header row; the header names the step, target and value columns.");
...
export function migrateplanconversion(input: { format: string; text: string; now: number }): conversionrecord {
  ...
  let source: unknown;
  try { source = JSON.parse(input.text); } catch (error) {
...
export function bundlesizeaccounting(input: Array<{ target: string; bytes: number }>): { ok: boolean; over: Array<{ target: string; bytes: number; budget: number }>; reason: string } {
  ...
  const budgets: Record<string, number> = { index: 1_800_000, indexcjs: 1_900_000, neutral: 1_800_000, umd: 1_900_000, node: 1_900_000, bun: 1_800_000, deno: 1_800_000, cli: 600_000, headless: 700_000, policy: 600_000, protocol: 380_000, memory: 330_000, progress: 60_000 };
