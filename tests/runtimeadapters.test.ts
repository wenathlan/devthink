...
import { adapterdeclarationsof, capabilityprobeof, chromestorageadapter, denokvadapter, dommapof, downgradesof, fetchshellof, filesystemstorageadapter, memorystorageadapter, portablecapabilityset, runtimeadapterdeclarationof, runtimeadapterof, runtimecapabilitiestable, timershellof, workermapof } from "../runtime.js";
...
describe("capability probes", () => {
  it("probes the dom, storage, network and worker capabilities of one runtime without failing anything", () => {
    expect(capabilityprobeof({ dom: true, storage: true, network: true, worker: true })).toEqual({ dom: true, storage: true, network: true, worker: true });
  ...
  it("declares the adapter mapping of every platform target", () => {
    expect(runtimeadapterdeclarationof("browser")).toMatchObject({ runtime: "browser", storage: "chrome", worker: "webworker", dom: "livepage" });
    expect(runtimeadapterdeclarationof("browser", true).dom).toBe("remote");
    expect(runtimeadapterdeclarationof("node")).toMatchObject({ storage: "filesystem", worker: "workerthreads", dom: "remote" });
    expect(runtimeadapterdeclarationof("bun").storage).toBe("filesystem");
    expect(runtimeadapterdeclarationof("deno")).toMatchObject({ storage: "denokv", worker: "webworker", dom: "remote" });
  ...
  it("gates the adapter mapping against its platform target", () => {
    for (const runtime of ["browser", "node", "bun", "deno"] as const) {
      ...
      expect(adaptermappinggate({ adapter: runtimeadapterdeclarationof(runtime, runtime === "browser"), headless: runtime === "browser" }).allowed).toBe(true);
    }
    expect(adaptermappinggate({ adapter: { ...runtimeadapterdeclarationof("node"), storage: "chrome" } }).allowed).toBe(false);
    expect(adaptermappinggate({ adapter: { ...runtimeadapterdeclarationof("deno"), storage: "filesystem" } }).allowed).toBe(false);
    expect(adaptermappinggate({ adapter: { ...runtimeadapterdeclarationof("bun"), worker: "webworker" } }).allowed).toBe(false);
    expect(adaptermappinggate({ adapter: { ...runtimeadapterdeclarationof("browser"), dom: "remote" }, headless: false }).allowed).toBe(false);
    expect(adaptermappinggate({ adapter: { ...runtimeadapterdeclarationof("node"), fetch: "shim" as never } }).allowed).toBe(false);
  });
  ...
  it("downgrades features instead of failing the runtime", () => {
    ...
    const downgrades = downgradesof({ features: [{ feature: "page capture", capability: "dom" }, { feature: "profile store", capability: "storage" }], probes });
  ...
  it("derives the portable capability set from the probes and the reviewed vocabulary", () => {
    ...
    const domless = portablecapabilityset({ vocabulary, probes: capabilityprobeof({ dom: false, storage: true, network: true, worker: true }), domlesskinds: vocabulary.filter(kind => { try { return actionrisk(kind as never) === "read"; } catch { return false; } }) });
    expect(domless).not.toContain("click");
    expect(domless).toContain("readtext");
    expect(portablecapabilityset({ vocabulary, probes: { dom: false, storage: true, network: true, worker: true } as capabilityprobe })).toEqual([]);
  });
});
...
describe("runtime shells", () => {
  ...
  it("maps chrome storage and deno kv onto the same seam", async () => {
    ...
    const chrome = chromestorageadapter({ get: async key => area.get(key), set: async (key, value) => { area.set(key, value); } });
    await chrome.set("session", { id: "s1" });
    expect(await chrome.get("session")).toMatchObject({ id: "s1" });
    const kv = new Map<string, unknown>();
    const deno = denokvadapter({ get: async <T>(key: string) => kv.get(key) as T | undefined, set: async <T>(key: string, value: T) => { kv.set(key, value); } });
  ...
  it("drives the same session memory through every adapter so the profile store format stays one", async () => {
    ...
    const memory = new sessionmemory(chromestorageadapter({ get: async key => store.get(key), set: async (key, value) => { store.set(key, value); } }));
  ...
  it("provides the timer and fetch shells from the injected platform primitives", async () => {
    ...
    const timer = timershellof({ settimeout: callback => { scheduled.push(callback); return scheduled.length; }, now: () => now });
    const handle = timer.schedule(() => "ran", 50);
    expect(handle).toBe(1);
    expect(timer.now()).toBe(now);
    (scheduled[0] as () => void)();
    const fetches: string[] = [];
    const fetchshell = fetchshellof({ fetch: async input => { fetches.push(input); return { ok: true }; } });
  ...
  it("composes one runtime adapter from its declaration, probes and injected primitives", () => {
    ...
    const adapter = runtimeadapterof({
      ...
      timer: { settimeout: callback => { callback(); return 0; }, now: () => now },
