...
import { aggregatereport, arbitrationcaseof, costentryof, depthlimitof, depthoflineage, interleave, lessondecay, lessonmatches, lessonrecordof, lessonreuse, loadreportof, prioritylaneof, releasecase, scalesuggestion, sharedcostsplit, spawnsubagent } from "../agent.js";
import { aggregateconflictescalationgate, aggregatemergegrade, arbitrationverdictgate, depthgate, lanechangegate, lessonsecretgate, lessonsanitizestep, scaleconsentgate, spawngate, subsetscopegate } from "../policy.js";
...
function parentscope(over: Partial<agentscope> = {}): agentscope {
  return { agentid: "a1", origins: ["https://example.com", "https://shop.example"], toolnamespaces: ["browser", "memory"], actionkinds: ["observe", "readtext", "click"], ...over };
}
...
describe("agentwork spawning and depth limits", () => {
  it("spawns a child under its parent with the copied scope and refuses paused parents and lineage depth jumps", () => {
    ...
    const spawned = spawnsubagent({ records: [parent], spawns: [], spec: { parentid: "p1", objective: "Read the pricing table", depth: 1 }, parentscope: parentscope(), id: "c1", now });
    ...
    const narrowed = spawnsubagent({ records: [parent], spawns: [], spec: { parentid: "p1", objective: "Read the shop only", depth: 1, narrowscope: { origins: ["https://shop.example"], actionkinds: ["readtext"] } }, parentscope: parentscope(), id: "c2", now });
    expect(narrowed.scope.origins).toEqual(["https://shop.example"]);
    expect(narrowed.scope.actionkinds).toEqual(["readtext"]);
    expect(() => spawnsubagent({ records: [record({ id: "p1", name: "planner", state: "paused" })], spawns: [], spec: { parentid: "p1", objective: "No spawn from a paused parent", depth: 1 }, parentscope: parentscope(), id: "c3", now })).toThrow(/paused/i);
    expect(() => spawnsubagent({ records: [parent], spawns: [], spec: { parentid: "p1", objective: "Depth must stay one level under the parent", depth: 3 }, parentscope: parentscope(), id: "c4", now })).toThrow(/exactly one level under/i);
    expect(spawngate({ parentscope: parentscope(), objectiveorigin: "https://example.com", parentstate: "active" }).allowed).toBe(true);
    expect(spawngate({ parentscope: parentscope({ origins: ["https://example.com"] }), objectiveorigin: "https://outside.example", parentstate: "active" }).allowed).toBe(false);
  });
...
describe("agentwork aggregation and interleaving", () => {
  it("merges parallel outputs with per agent provenance, resolves conflicts by policy order and escalates the unresolved", () => {
    ...
    const verdict = aggregateconflictescalationgate({ unresolved: open.conflicts.filter(conflict => conflict.resolvedby === undefined).map(conflict => conflict.key) });
    expect(verdict.allowed).toBe(false);
    const ordered = aggregatereport({ id: "agg2", subject: "The pricing table read", cells, conflictorder: ["a2", "a1"], now });
  ...
  it("orders the actions of concurrent agents into one timeline while every event keeps its lane visible", () => {
    const lanes = [{ name: "interactive", priority: 10, interactive: true, agentids: ["a1"] }, { name: "background", priority: 1, agentids: ["a2"] }];
    const events = [
...
describe("agentwork lessonshare", () => {
  it("records sanitized lessons, serves the matching ones, counts reuse and decays the stale", () => {
    const first = lessonrecordof({ id: "l1", agentid: "a1", finding: "The pricing table loads only after the hero settles.", origin: "https://example.com", now });
    const second = lessonrecordof({ id: "l2", agentid: "a2", finding: "The login form refuses pasted passwords on the shop.", origin: "https://shop.example", now });
    ...
    const matches = lessonmatches({ lessons, task: "Read the pricing table of the hero again", origin: "https://example.com" });
...
describe("agentwork arbitration and lanes", () => {
  it("grants the first requester by default, honors the priority lane, and releases when the holder finishes", () => {
    const lanes = [{ name: "background", priority: 1, agentids: ["a1"] }, { name: "fastlane", priority: 9, agentids: ["a2"] }];
    const byfirst = arbitrationcaseof({ id: "case1", resource: "https://example.com/pricing", origin: "https://example.com", requesters: ["a1", "a3"], lanes: [], now });
    expect(byfirst.verdict?.holderagentid).toBe("a1");
    expect(byfirst.verdict?.reason).toContain("first requester");
    const bylane = arbitrationcaseof({ id: "case2", resource: "https://example.com/pricing", origin: "https://example.com", requesters: ["a1", "a2"], lanes, now });
    expect(bylane.verdict?.holderagentid).toBe("a2");
    expect(bylane.verdict?.lane).toBe("fastlane");
    expect(() => arbitrationcaseof({ id: "case3", resource: "https://example.com/pricing", origin: "https://example.com", requesters: ["a1"], lanes, now })).toThrow(/at least two requesting agents/i);
    expect(arbitrationverdictgate({ holderagentid: "a1", origin: "https://example.com", origingrants: ["https://example.com"] }).allowed).toBe(true);
    expect(arbitrationverdictgate({ holderagentid: "a1", origin: "https://outside.example", origingrants: ["https://example.com"] }).allowed).toBe(false);
    expect(arbitrationverdictgate({ holderagentid: "a1", origin: "https://example.com", sessionlockholder: "another-run" }).allowed).toBe(false);
    ...
    expect(() => releasecase({ cases: [bylane], id: "case2", holderfinished: false, now })).toThrow(/holder.*still works/i);
  });
  ...
  it("ranks the queue by lane priority, keeps sensitive steps in the interactive lane and prevents lane starvation through the round robin fallback", () => {
    ...
    expect(lanechangegate({ lane: { name: "low", priority: 1 }, sensitive: true, confirmed: false }).allowed).toBe(false);
    expect(lanechangegate({ lane: { name: "low", priority: 1 }, sensitive: true, confirmed: true }).allowed).toBe(true);
    expect(lanechangegate({ lane: { name: "low", priority: 1 }, sensitive: false, confirmed: false }).allowed).toBe(true);
  });
});
...
describe("agentwork scaleworkers and costshare", () => {
  it("suggests spawning on low load, pausing on throttle, never acting without consent and splitting the shared costs", () => {
    const reports = [loadreportof({ origin: "https://example.com", concurrency: 1, latency: 80, now }), loadreportof({ origin: "https://shop.example", concurrency: 3, latency: 4500, now })];
    ...
    expect(scalesuggestion({ reports: [loadreportof({ origin: "https://example.com", concurrency: 2, latency: 900, now })], lowthreshold: 200, throttlethreshold: 4000 }).map(suggestion => suggestion.suggestion)).toEqual(["hold"]);
    const entries = [
    ...
    expect(() => costentryof({ agentid: "a1", units: 0, description: "A zero cost never enters the ledger.", now })).toThrow(/positive/i);
  });
});
