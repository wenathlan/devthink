...
describe("extension manifest", () => {
  it("keeps privileged browser capabilities optional and out of the default package", async () => {
    const manifest = JSON.parse(await readFile("web/extension/manifest.json", "utf8")) as { manifest_version: number; version: string; key?: string; permissions: string[]; optional_permissions?: string[]; host_permissions?: string[]; content_scripts?: unknown[]; sandbox?: { pages?: string[] }; offscreen?: { document?: string; reasons?: string[]; justification?: string } };
    const packagejson = JSON.parse(await readFile("package.json", "utf8")) as { version: string };
    expect(manifest.manifest_version).toBe(3);
    expect(manifest.version).toBe(packagejson.version);
    expect(manifest.permissions).toEqual(["activeTab", "storage", "scripting", "sidePanel"]);
    expect(manifest.optional_permissions ?? []).toEqual(["tabs", "downloads", "clipboardRead", "clipboardWrite", "offscreen", "nativeMessaging"]);
    ...
    expect((manifest.offscreen as { justification?: string }).justification ?? "").toMatch(/offscreen document worker pool/);
  });

  it("keeps the native messaging permission optional only so the native bridge stays a per install user choice", async () => {
    const manifest = JSON.parse(await readFile("web/extension/manifest.json", "utf8")) as { permissions: string[]; optional_permissions?: string[] };
  ...
  it("keeps a stable, strictly decodable identity key", async () => {
    ...
    if (existsSync("dist/checksums.txt")) {
      const recorded = (await readFile("dist/checksums.txt", "utf8")).split("\n").find(line => line.trim().endsWith("  manifest.json key"));
  ...
  it("declares the optionspage as the options surface and the dashboardpage chrome url without new permissions", async () => {
    const manifest = JSON.parse(await readFile("web/extension/manifest.json", "utf8")) as { options_ui?: { page?: string; open_in_tab?: boolean }; chrome_url_overrides?: Record<string, string> };
  ...
  it("declares the icon family of the final polish at every required size with payloads that decode to the declared pixels", async () => {
    const manifest = JSON.parse(await readFile("web/extension/manifest.json", "utf8")) as { icons?: Record<string, string>; action?: { default_icon?: Record<string, string> } };
    const expected = { "16": "icons/16.png", "19": "icons/19.png", "32": "icons/32.png", "38": "icons/38.png", "48": "icons/48.png", "128": "icons/128.png" };
...
describe("the frozen capability manifest declarations", () => {
  it("matches the manifest permissions with the capmanifest declarations of every surface", async () => {
    ...
    const manifest = JSON.parse(await readFile("web/extension/manifest.json", "utf8")) as { version: string; permissions: string[]; optional_permissions?: string[]; optional_host_permissions?: string[] };
    const declared = new Set<string>();
    for (const surface of ["background", "sidepanel", "popup", "mcp"] as const) {
      const stored = JSON.parse(await readFile(`dist/caps/${surface}.json`, "utf8")) as { surface: string; release: string; protocolmajor: number; permissions: string[] };
    ...
    const requested = [...manifest.permissions, ...(manifest.optional_permissions ?? []), ...(manifest.optional_host_permissions ?? [])];
...
describe("the strict content security policy", () => {
  it("pins the extension pages of the root manifest and both overlays to local scripts and own framing", async () => {
    const manifest = JSON.parse(await readFile("web/extension/manifest.json", "utf8")) as { content_security_policy?: { extension_pages?: string }; browsers?: Record<string, { content_security_policy?: { extension_pages?: string } }> };
  ...
  it("keeps every declared source free of wildcards, remote hosts, eval and inline", async () => {
    const manifest = JSON.parse(await readFile("web/extension/manifest.json", "utf8")) as { content_security_policy?: { extension_pages?: string }; browsers?: Record<string, { content_security_policy?: { extension_pages?: string } }> };
    const policies = [manifest.content_security_policy?.extension_pages, manifest.browsers?.firefox?.content_security_policy?.extension_pages, manifest.browsers?.safari?.content_security_policy?.extension_pages].filter((entry): entry is string => typeof entry === "string");
    expect(policies).toHaveLength(3);
    for (const policy of policies) for (const source of policy.split(";").flatMap(directive => directive.trim().split(/\s+/).slice(1))) {
...
describe("the multi agent dashboard surface", () => {
  it("carries every multi agent panel of the dashdone completion inside the declared dashboardpage surface without new permissions", async () => {
    const manifest = JSON.parse(await readFile("web/extension/manifest.json", "utf8")) as { permissions: string[]; optional_permissions?: string[]; chrome_url_overrides?: Record<string, string> };
    ...
    for (const reference of [...surface.matchAll(/(?:src|href)\s*=\s*["']([^"']+)["']/g)].map(match => match[1] ?? "")) {
