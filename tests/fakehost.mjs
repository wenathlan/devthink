...
const protocol = args.has("--protocol")
  ? Number.parseInt(process.argv[process.argv.indexOf("--protocol") + 1] ?? "1", 10)
  : 1;
const wsbridgeport = args.has("--port")
  ? Number.parseInt(process.argv[process.argv.indexOf("--port") + 1] ?? "0", 10)
  : 0;
const silent = args.has("--silent");
const crashafter = args.has("--crashafter")
  ? Number.parseInt(process.argv[process.argv.indexOf("--crashafter") + 1] ?? "1", 10)
  ...
  for (;;) {
    ...
    if (frame.kind === "handshake") {
      writeframe({
        ...
        body: { build, protocol, surfaces: ["osdialog", "notification"], wsbridgeport },
      });
      if (wsbridgeport > 0)
        writeframe({
          kind: "advertisement",
          correlationid: frame.correlationid,
          body: { port: wsbridgeport, token: "fixture-token" },
        });
      continue;
    }
    ...
    if (frame.kind === "call") {
      ...
      if (body.classconsent !== true || body.surfaceconsent !== true) {
        writeframe({
          ...
          body: {
            family: "surface",
            message: `The ${body.surface} surface refuses execution without an active consent grant.`,
            retry: "none",
          },
        });
        continue;
      ...
      writeframe({
        kind: "call",
        correlationid: frame.correlationid,
        body: { surface: body.surface, delivered: true, details: { fixture: true } },
      });
      continue;
    }
    writeframe({
      kind: "error",
      correlationid: frame.correlationid,
      body: { family: "port", message: `The fake host knows no ${frame.kind} kind.`, retry: "none" },
    });
  }
});
