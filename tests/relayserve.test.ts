...
import { composeenvelope, contractcapabilities, eventpostbody, eventstreambody, parsewireframe, sessioncreatebody, sessionjoinbody, stableopid } from "../bridge.js";
import { newrelayserverstate, relayconnectionclose, relayconnectionopen, relayframeof, relayidleconnections, relaylivetokens, relaypendingpairings, relayserverframe } from "../http.js";
...
function send(input: { state: ReturnType<typeof newrelayserverstate>; connectionid: string; frame: string; now?: number }) {
  const outcome = relayserverframe({ state: input.state, connectionid: input.connectionid, frame: input.frame, now: input.now ?? now });
  return { state: outcome.state, replies: outcome.replies.map(reply => parsewireframe(reply)), routed: outcome.routed };
}

describe("relayserve", () => {
  it("opens a session for the extension member and issues its token and pairing code", () => {
    let state = statewith("conn-1", "conn-2");
    const create = composeenvelope({ op: "sessioncreate", opid: stableopid("ext", 1), at: now, body: sessioncreatebody({ role: "extension", memberid: "ext", capabilities, pairingcode: "PAIR-CODE" }) });
  ...
  it("refuses a sessioncreate frame that does not come from the extension member", () => {
    ...
    const create = composeenvelope({ op: "sessioncreate", opid: stableopid("site", 1), at: now, body: sessioncreatebody({ role: "site", memberid: "site", capabilities }) });
    const outcome = send({ state, connectionid: "conn-1", frame: JSON.stringify(create) });
    expect(outcome.replies[0]?.body).toEqual({ error: "The sessioncreate operation serves the extension member only." });
  ...
  it("redeems the pairing code for the site member and issues its own token", () => {
    ...
    const join = composeenvelope({ op: "sessionjoin", opid: stableopid("site", 1), at: now, body: sessionjoinbody({ sessionid, role: "site", memberid: "site", capabilities, pairingcode: "PAIR-CODE" }) });
  ...
  it("routes an event post to the other member of the session and keeps the event log", () => {
    ...
    const joined = send({ state, connectionid: "conn-2", frame: JSON.stringify(join) });
    state = joined.state;
    const post = composeenvelope({ op: "eventpost", opid: stableopid("ext", 2), at: now, sessionid, token: extensiontoken, body: eventpostbody({ event: { kind: "chat", stream: "chat", payload: { message: "collect the pricing table" } } }) });
  ...
  it("answers an event stream subscription with the ack over the subscribed streams", () => {
    ...
    const create = composeenvelope({ op: "sessioncreate", opid: stableopid("ext", 1), at: now, body: sessioncreatebody({ role: "extension", memberid: "ext", capabilities }) });
    const created = send({ state, connectionid: "conn-1", frame: JSON.stringify(create) });
    state = created.state;
    const sessionid = (created.replies[0]?.body as { sessionid?: string }).sessionid ?? "";
    const token = (created.replies[0]?.body as { token?: string }).token ?? "";
    const stream = composeenvelope({ op: "eventstream", opid: stableopid("ext", 2), at: now, sessionid, token, body: eventstreambody({ streams: ["chat", "review"], since: now - 500 }) });
  ...
  it("refuses every frame that fails the token authentication without touching the session state", () => {
    ...
    state = created.state;
    const sessionid = (created.replies[0]?.body as { sessionid?: string }).sessionid ?? "";
    const post = composeenvelope({ op: "eventpost", opid: stableopid("ext", 2), at: now, sessionid, token: "forged-token", body: eventpostbody({ event: { kind: "chat", stream: "chat", payload: { message: "no" } } }) });
    ...
    const ghost = composeenvelope({ op: "eventpost", opid: stableopid("ext", 3), at: now, sessionid: "session-ghost", token: "any", body: eventpostbody({ event: { kind: "chat", stream: "chat", payload: { message: "no" } } }) });
  ...
  it("rotates the token of a returning member and revokes the previous one", () => {
    ...
    const rejoin = composeenvelope({ op: "sessionjoin", opid: stableopid("ext", 2), at: now, sessionid, token: firsttoken, body: sessionjoinbody({ sessionid, role: "extension", memberid: "ext", capabilities }) });
    ...
    const session: relayserversession = state.sessions[0] ?? { id: "missing", extension: undefined, site: undefined, events: [], tokenhashes: [], revoked: [] };
    expect(relaylivetokens(session)).toEqual([secondtoken]);
    expect(session.revoked).toEqual([firsttoken]);
    /* the revoked token no longer authenticates */
    const post = composeenvelope({ op: "eventpost", opid: stableopid("ext", 3), at: now, sessionid, token: firsttoken, body: eventpostbody({ event: { kind: "chat", stream: "chat", payload: { message: "no" } } }) });
    const refused = send({ state, connectionid: "conn-1", frame: JSON.stringify(post) });
    expect((refused.replies[0]?.body as { error?: string }).error).toContain("token authentication");
  });
