...
describe("cli deep manifest checks", () => {
  it("verifies every manifest key against the allowlist, the permission sources, the csp hashes, the reviewed web resources, the duplicate permissions, the chrome floor and the icon dimensions", () => {
    const manifesttext = JSON.stringify(
      {
        ...
        name: "devthink",
        version: "1.1.80",
        description: "The reviewed browser agent.",
    ...
    const checked = deepmanifestchecks({
      manifest,
      manifesttext,
      manifestfile: "manifest.json",
      filebytes: bytes,
      capabilities: capabilityapireport(["storage", "tabs", "downloads"]),
      digestof,
    });
    ...
    expect(
      checked.findings.find(
        (finding) => finding.rule === "manifest.permission.source" && finding.path === "permissions.storage",
      )?.source,
    ...
    const nativerequired = deepmanifestchecks({
      manifest: {
        manifest_version: 3,
        name: "devthink",
        version: "1.1.85",
      ...
      manifesttext: JSON.stringify({ permissions: ["storage", "nativeMessaging"] }),
      manifestfile: "manifest.json",
      filebytes: {},
      capabilities: capabilityapireport(["storage", "nativeMessaging"]),
      digestof: () => "x",
    });
    expect(
      nativerequired.findings.some(
        (finding) =>
          finding.rule === "manifest.permission.nativemessaging" &&
          finding.severity === "error" &&
          finding.path === "permissions.nativeMessaging",
      ),
    ).toBe(true);
    expect(nativerequired.exitcode).toBe(1);
    const nativeoptional = deepmanifestchecks({
      manifest: {
        manifest_version: 3,
        name: "devthink",
        version: "1.1.85",
      ...
      manifesttext: JSON.stringify({ permissions: ["storage"], optional_permissions: ["nativeMessaging"] }),
      manifestfile: "manifest.json",
      filebytes: {},
      capabilities: capabilityapireport(["storage", "nativeMessaging"]),
      digestof: () => "x",
    ...
    expect(
      nativeoptional.findings.some(
        (finding) =>
          finding.rule === "manifest.permission.nativemessaging" &&
          finding.severity === "info" &&
          finding.path === "optional_permissions.nativeMessaging",
      ),
    ).toBe(true);
    ...
    expect(
      checked.findings.some(
        (finding) =>
          finding.rule === "manifest.csp.hashes" &&
          finding.severity === "info" &&
          finding.message.includes("sha256-KNOWNHASH= verifies against worker.js"),
      ),
    ).toBe(true);
    ...
    expect(
      checked.findings.some(
        (finding) => finding.rule === "manifest.icons" && finding.message.includes("decodes to 16x16"),
      ),
    ).toBe(true);
    expect(rules("manifest.chromeminimum")).toBe(0);
    expect(checked.exitcode).toBe(1);
    const floored = deepmanifestchecks({
      manifest: {
        manifest_version: 3,
        name: "devthink",
        version: "1.1.80",
    ...
    expect(
      floored.findings.some(
        (finding) =>
          finding.rule === "manifest.chromeminimum" &&
          finding.severity === "error" &&
          finding.message.includes("capability floor 114"),
      ),
    ).toBe(true);
    const badicon = deepmanifestchecks({
      manifest: { icons: { "16": "icon16.png" } },
      manifesttext: "{}",
      manifestfile: "m.json",
      filebytes: { "icon16.png": pngof(32, 32) },
      capabilities: [],
      digestof: () => "x",
    });
    expect(
      badicon.findings.some(
        (finding) =>
          finding.rule === "manifest.icons" &&
          finding.message.includes("decodes to 32x32 while its size key declares 16"),
      ),
    ).toBe(true);
    ...
    const unbundled = deepmanifestchecks({
      manifest: { icons: { "16": "icons/16.png" } },
      manifesttext: "{}",
      manifestfile: "m.json",
      filebytes: {},
      capabilities: [],
    ...
    expect(
      unbundled.findings.some(
        (finding) =>
          finding.rule === "manifest.icons" &&
          finding.severity === "info" &&
          finding.message.includes("rides the browser bundle of the release zip channel"),
      ),
    ).toBe(true);
    expect(unbundled.exitcode).toBe(0);
    const bundledmissing = deepmanifestchecks({
      manifest: { icons: { "16": "icons/16.png" } },
      manifesttext: "{}",
      manifestfile: "m.json",
      filebytes: {},
    ...
    expect(
      bundledmissing.findings.some(
        (finding) =>
          finding.rule === "manifest.icons" &&
          finding.severity === "error" &&
          finding.message.includes("ships no file in the bundle"),
      ),
    ).toBe(true);
    expect(bundledmissing.exitcode).toBe(1);
    const unreviewedresource = deepmanifestchecks({
      manifest: { web_accessible_resources: [{ resources: ["panel.html"] }] },
      manifesttext: "{}",
      manifestfile: "m.json",
      filebytes: {},
      capabilities: [],
    ...
    expect(
      unreviewedresource.findings.some(
        (finding) =>
          finding.rule === "manifest.war.reviewed" && finding.message.includes("outside the reviewed resource set"),
      ),
    ).toBe(true);
    ...
    expect(capabilityapireport(["sidePanel", "offscreen"])).toEqual([
      { api: "sidePanel", minchrome: 114 },
      { api: "offscreen", minchrome: 109 },
    ]);
...
describe("cli planlint findings and risk summary", () => {
  it("accepts the reviewed plan while refusing forbidden kinds, broken selectors, missing option fields, capability misses and origins outside the consent allowlist", () => {
    const clean = planlintfindings({
      file: planfixture(),
      capabilities: ["observe", "readtext", "countelements"],
      allowlist: ["https://example.org"],
    });
    expect(clean).toHaveLength(0);
    const broken = planlintfindings({
      file: planfixture({
        steps: [
          { id: "observe", kind: "observe", label: "Observe the release page" },
    ...
    expect(broken.find((diagnostic) => diagnostic.code === "plan.options.required")?.message).toContain(
      "waitelement misses the required option field wait",
    );
    const bounded = planlintfindings({
      file: planfixture({
        steps: [
          { id: "wait", kind: "waitelement", label: "Wait for the list", target: "main", options: '{"wait":5000}' },
        ],
      }),
    });
...
describe("cli runworkflow document, summary and audit trail", () => {
  it("parses the workflow document under schemastrict, composes it through the shared engine and maps the run states onto the documented exit codes", () => {
    ...
    expect(() =>
      parseworkflowdocument({
        ...
        workflow: {
          name: "x",
          version: 1,
          origins: ["http://insecure.example"],
          steps: [{ id: "s", kind: "observe", label: "l" }],
        },
      }),
    ...
    const log: runlogentry[] = [
      {
        stepid: "observe",
        label: "Observe the changelog page",
        state: "done",
        startedat: now,
        duration: 100,
        summary: "The recorded page carried 512 text characters.",
      },
      {
        stepid: "read",
        label: "Read the changelog text",
        state: "done",
        startedat: now + 100,
        duration: 400,
        summary: "The text read the changelog.",
      },
    ];
    ...
    const failed = runworkflowsummaryof({
      runid: run.id,
      run: { ...run, state: "failed", failreason: "The changelog list never appeared." },
      log,
    });
    expect(failed.exitclass).toBe("stepfailed");
    expect(failed.exitcode).toBe(2);
    expect(failed.reason).toContain("never appeared");
    const cancelled = runworkflowsummaryof({
      runid: run.id,
      run: { ...run, state: "cancelled", cancelreason: "The operator cancelled the replay." },
      log,
    });
...
describe("cli exportdata windows, formats and secret refusal", () => {
  it("filters the records through the from and to window while the shared serializers emit json, csv and markdown and the secret store material refuses the export in full", () => {
    ...
    expect(exportdatawindow(records, undefined, now - 1000).map((record) => record.title)).toEqual([
      "alpha",
      "undated",
    ]);
    const jsonexport = exportdatacontent({
      scope: "session",
      format: "json",
    ...
    const csvexport = exportdatacontent({
      scope: "extraction",
      format: "csv",
      records: [{ title: "alpha", at: now }],
      shapes: [],
      path: "out.csv",
    });
    expect(csvexport.content.split("\n")[0]).toBe("at,title");
    expect(csvexport.result.path).toBe("out.csv");
    const markdown = exportdatacontent({
      scope: "extraction",
      format: "markdown",
      records: [{ title: "alpha", at: now }],
      shapes: [],
    ...
    const refusedexport = exportdatacontent({
      scope: "session",
      format: "json",
      records: [{ vaultid: "v1" }],
      shapes: [],
...
describe("cli headlessmode fixture replay", () => {
  it("parses the recorded page state under schemastrict while the consent gate serves the granted read only vocabulary, refuses the wider kinds and reports the kinds a fixture cannot satisfy", () => {
    ...
    const parsed = parseheadlessfixture({
      id: fixture.id,
    ...
    const observed = headlessfixturestep(fixture, {
      id: "observe",
      kind: "observe",
      label: "Observe the release page",
    });
    expect(observed.state).toBe("done");
    expect(observed.summary).toContain("512 text characters");
    const counted = headlessfixturestep(fixture, {
      id: "count",
      kind: "countelements",
      label: "Count the release sections",
      target: "main section",
    });
    ...
    const wider = headlessfixturestep(
      { ...fixture, grants: ["hover"] },
      { id: "hover", kind: "hover", label: "Hover the release link" },
    );
    expect(wider.state).toBe("refused");
    expect(wider.summary).toContain("grades interaction");
    const unprojected = headlessfixturestep(
      { ...fixture, grants: [...fixture.grants, "wait"] },
      { id: "wait", kind: "wait", label: "Wait for the quiet page" },
    );
    expect(unprojected.state).toBe("unsupported");
    expect(unprojected.summary).toContain("no projection for it");
    const progress = headlessprogress({
      progress: undefined,
      planid: "plan1",
      step: { id: "observe", kind: "observe", label: "Observe the release page" },
      outcome: observed,
      now,
    });
...
describe("cli bundle size accounting and library entries", () => {
  it("grades the dist targets against their size budgets so a bundle that outgrows its budget fails the build before it ships", () => {
    const ok = bundlesizeaccounting([
      { target: "cli", bytes: 480_000 },
      { target: "headless", bytes: 690_000 },
    ]);
    expect(ok.ok).toBe(true);
    expect(ok.over).toHaveLength(0);
    const over = bundlesizeaccounting([
      { target: "cli", bytes: 700_000 },
      { target: "headless", bytes: 480_000 },
      { target: "worker", bytes: 9_000_000 },
    ]);
