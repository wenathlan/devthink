...
describe("runbudget tracking", () => {
  ...
  it("fires budgetalerts at the user thresholds with severity levels and pauses only at the critical level", () => {
    ...
    const warning = budgetalertstate({
      budget: runbudgetof({ runid: "run1", steps: 5, stepbudget: 10, pressure: 0.5, memorybudget: 0.8, now }),
      thresholds: { warning: 0.8, critical: 0.9 },
    });
    expect(warning).toHaveLength(0);
    const mixed = budgetalertstate({
      budget: runbudgetof({ runid: "run1", steps: 5, stepbudget: 6, pressure: 0.5, memorybudget: 0.8, now }),
      thresholds: { warning: 0.8, critical: 0.9 },
    });
  ...
  it("stores the runbudget records and the budgetalerts of the profile workspace", async () => {
    ...
    await store.addrunbudget(
      runbudgetof({ runid: "run1", steps: 6, stepbudget: 10, pressure: 0.5, memorybudget: 0.8, now }),
    );
...
describe("timeoutcancel", () => {
  ...
  it("records the cancel event beside the step outcome and requires its immutable log entry", () => {
    ...
    expect(() => timeoutcanceleventof({ runid: " ", stepid: "s1", bound: 1, elapsed: 1, now, logged: true })).toThrow(
      /run id/i,
    );
    expect(() => timeoutcanceleventof({ runid: "run1", stepid: " ", bound: 1, elapsed: 1, now, logged: true })).toThrow(
      /step id/i,
    );
  ...
  it("stores the timeoutcancel events of the profile workspace", async () => {
    const store = new sessionmemory(new fakeadapter());
    await store.addtimeoutevent(
      timeoutcanceleventof({ runid: "run1", stepid: "s1", bound: 500, elapsed: 700, now, logged: true }),
    );
    const events = await store.gettimeoutevents();
    expect(events).toHaveLength(1);
    expect(events[0]?.stepid).toBe("s1");
    expect(timeoutrecordeventgate({ event: { logged: events[0]!.logged, stepid: events[0]!.stepid } }).allowed).toBe(
      true,
    );
  });
});
