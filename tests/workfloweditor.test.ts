...
import { applyoverride, buildsteplibrary, diffversions, expandtemplate, exportworkflow, groupselect, importworkflow, loadworkflow, markbreakpoint, minimapfocus, palettenodes, reordersteps, renderminimap, runtobreakpoint, saveworkflow, searchsteps, snapnode, undoedit, redoedit, addedge, removeedge, removenode, addnode, editstep, bindparam, zoomcanvas } from "../workflow.js";
import { actionrisk, dryrunprojection, editorsavegate, exportcontentreview, runreviewgranted, validatesiteoverride, watchdogconfigvalid } from "../policy.js";
import { composeworkflow, expandblocks, newworkflowrun, runworkflow, steptemplateof, workflowblockof, workflowstepof, watchdogpass } from "../workflow.js";
...
function composedworkflow(): workflowrecord {
  const inner = workflowblockof({ name: "inner", label: "Inner block", steps: [
  ...
  return composeworkflow({
    ...
    steps: [
      ...
      { id: "s2", kind: "wait", label: "Close wait", value: "10", bindings: [{ variable: "heading", kind: "string", stepid: "i1" }] },
    ],
    blocks: [inner],
    now,
    kindallowed,
...
describe("workflow editor palette and library", () => {
  ...
  it("builds the step library over every reviewed kind grouped by category", () => {
    ...
    expect(library.find(entry => entry.kind === "delay")?.optionschema[0]).toEqual({ name: "base", kind: "number", required: true });
  });
});
...
describe("editor load and save round trips", () => {
  ...
  it("gives every repeated invocation of one block a unique canvas node id", () => {
    ...
    if (!inner) throw new Error("The inner block did not normalize.");
    const record = composeworkflow({ id: "wf2", name: "twice", version: 1, origins: ["https://example.com"], steps: [{ block: "inner", label: "First" }, { id: "s1", kind: "wait", label: "Between", value: "1" }, { block: "inner", label: "Second" }], blocks: [inner], now, kindallowed, riskof });
    const model = loadworkflow(record);
  ...
  it("refuses saves with duplicate node ids, unknown blocks, backwards edges and unknown edge endpoints", () => {
    ...
    const duplicated: editormodel = { ...model, nodes: [...model.nodes, { step: { ...(first.step as object), id: "s1" } as never, x: 40, y: 500 }] };
    ...
    const unknownblock: editormodel = { ...model, nodes: [...model.nodes, { step: { ...attached, block: "ghost" }, x: 40, y: 500 }] };
    expect(() => saveworkflow(unknownblock, { now, kindallowed, riskof })).toThrow("unknown block");
    const backwards: editormodel = { ...model, edges: [...model.edges, { from: "s2", to: "s1", variable: "late", kind: "string" }] };
    expect(() => saveworkflow(backwards, { now, kindallowed, riskof })).toThrow("cycle");
    const dangling: editormodel = { ...model, edges: [...model.edges, { from: "ghost", to: "s2", variable: "x", kind: "string" }] };
...
describe("editor canvas edits", () => {
  ...
  it("binds nested params into a block invocation of the canvas", () => {
    ...
    expect(bound.nodes.find(node => node.invocation?.block === "inner")?.invocation?.params).toEqual([{ name: "base", kind: "number", default: 7 }]);
    const replaced = bindparam(bound, "inner", { name: "base", kind: "number", default: 9 });
    expect(replaced.nodes.find(node => node.invocation?.block === "inner")?.invocation?.params).toEqual([{ name: "base", kind: "number", default: 9 }]);
    ...
    expect(saved.steps.find(step => step.id === "i1")?.params).toEqual([{ name: "base", kind: "number", default: 7 }]);
  });

  it("inserts shared step templates with nested params onto the canvas", () => {
    const template: steptemplate = { id: "t1", name: "shared wait", origin: "https://example.com", step: { id: "wait", kind: "wait", label: "Shared wait", value: "3" }, sharedat: now };
    const model = loadworkflow(composedworkflow());
    const inserted = expandtemplate(model, steptemplateof(template) as steptemplate, [{ name: "base", kind: "number", default: 2 }]);
    expect(inserted.nodes.map(node => node.step?.id)).toContain("wait");
    expect(inserted.nodes.find(node => node.step?.id === "wait")?.step?.params).toEqual([{ name: "base", kind: "number", default: 2 }]);
    ...
    expect(() => expandtemplate(model, { id: "t2", name: "broken", origin: "https://example.com", step: { id: "x", kind: "wait" } as never, sharedat: now })).toThrow("reviewed workflow step");
  });
  ...
  it("searches steps by label, kind and variable name", () => {
    ...
    expect(searchsteps(withedge, "OPEN")).toEqual([{ id: "s1", label: "Open read", kind: "readtext", matched: ["label"] }]);
...
describe("editor mini map and zoom", () => {
  it("projects the full canvas into the mini map and answers the viewport rectangle", () => {
    ...
    expect(zoomedprojection.minimap.viewport.width).toBeCloseTo((model.layout.width / 2) * zoomedprojection.minimap.scale, 5);
...
describe("editor version diffs", () => {
  it("highlights added, removed and changed steps between two versions", () => {
    ...
    const edited = composeworkflow({
      ...
      blocks: [workflowblockof({ name: "inner", label: "Inner block", steps: [{ id: "i1", kind: "readtext", label: "Read the heading", target: "h1" }, { id: "i2", kind: "wait", label: "Inner wait", value: "5" }] })!],
      now,
      kindallowed,
      riskof,
    });
...
describe("editor export, import and sharing", () => {
  ...
  it("packs templates into one share file and restores them on import", () => {
    const record = composedworkflow();
    const template: steptemplate = { id: "t1", name: "shared wait", origin: "https://example.com", step: { id: "wait", kind: "wait", label: "Shared wait", value: "3" }, sharedat: now };
    const bundle = importworkflow({ contents: JSON.stringify({ format: workflowfileversion, exportedat: now, workflow: record, templates: [template] }), now, kindallowed, riskof });
  ...
  it("refuses imports that break the file contract", () => {
    ...
    expect(() => importworkflow({ contents: JSON.stringify({ format: 1, exportedat: now, workflow: { ...record, steps: [] }, templates: [] }), now, kindallowed, riskof })).toThrow("at least one step");
    expect(() => importworkflow({ contents: JSON.stringify({ format: 1, exportedat: now, workflow: { ...record, steps: [{ id: "x", kind: "explode", label: "Unknown kind" }] }, templates: [] }), now, kindallowed, riskof })).toThrow("reviewed action kind");
    expect(() => importworkflow({ contents: JSON.stringify({ format: 1, exportedat: now, workflow: record, templates: [{ id: "t", name: "broken" }] }), now, kindallowed, riskof })).toThrow("reviewed step");
    expect(() => importworkflow({ contents: JSON.stringify({ format: 1, exportedat: now, workflow: record, templates: "no" }), now, kindallowed, riskof })).toThrow("list");
    expect(() => importworkflow({ contents: "format: 1\nworkflow: {}", format: "yaml", now, kindallowed, riskof })).toThrow();
  });

  it("parses the documented yaml subset of scalars, sequences and mappings", () => {
    const parsed = importworkflow({ contents: "format: 1\nexportedat: 1800000000000\nworkflow:\n  id: \"wfy\"\n  name: \"yamlflow\"\n  version: 1\n  origins:\n    - \"https://example.com\"\n  steps:\n    -\n      id: \"s1\"\n      kind: \"wait\"\n      label: \"Wait a beat\"\n      value: \"10\"\n  blocks: []\n  risk: \"read\"\n  createdat: 1800000000000\ntemplates: []\n", format: "yaml", now, kindallowed, riskof });
...
describe("editor breakpoints and debug runs", () => {
  it("plans debug segments that pause before marked steps and report the remainder", () => {
    ...
    const surfaces = [...marked.nodes.flatMap(node => node.step !== undefined ? [node.step] : []), ...marked.blocks.flatMap(block => block.steps.flatMap(entry => "kind" in entry && "label" in entry && !("block" in entry) ? [entry as { id: string; breakpoint?: boolean }] : []))];
  ...
  it("runs the segment before a breakpoint, pauses and resumes exactly there", async () => {
    ...
    const executor = { execute: async (step: { id: string }) => { executed.push(step.id); return { ok: true, summary: `did ${step.id}` }; } };
    ...
    const partial = await runworkflow({ record: segmentrecord, run, ...executor, now, gates: { sessionactive: true, planapproved: true, origingranted: () => true } });
    ...
    const resumed = await runworkflow({ record, run: paused, log: partial.log, outputs: partial.outputs, ...executor, now, gates: { sessionactive: true, planapproved: true, origingranted: () => true } });
...
describe("editor policy gates", () => {
  it("gates editor saves on the session, the approved plan and the canvas shape", () => {
    const model = loadworkflow(composedworkflow());
    const session = { id: "sess", tabid: 4, origin: "https://example.com", startedat: now - 1000, expiresat: now + 600_000, grants: ["https://example.com"] };
    const plan = { id: "run", objective: "Edit the workflow", origin: "https://example.com", steps: [], createdat: now - 2000, expiresat: now + 600_000, state: "approved" as const };
  ...
  it("validates per site overrides against the reviewed knobs and export contents against secrets", () => {
    ...
    const secretstep: workflowrecord = { ...clean, steps: [...clean.steps, { ...plainstep, options: "{\"apikey\":\"abc\"}" }] };
  ...
  it("bounds the watchdog configuration as user configured values", () => {
    ...
    expect(watchdogconfigvalid({ enabled: true, stallthreshold: 30_000, action: "pause", zombiewindow: 60_000 }).allowed).toBe(true);
...
describe("per site override application", () => {
  it("applies reviewed knob deltas to the matching steps of a workflow", () => {
    const record = composeworkflow({ id: "wf3", name: "knobs", version: 1, origins: ["https://example.com"], steps: [
    ...
    const overridden = applyoverride(record, { id: "o1", workflowid: record.id, pattern: "https://example.com", deltas: { delaybase: 250, waitms: 900 }, createdat: now });
  ...
  it("refuses patterns that match no workflow origin and knobs outside the reviewed set", () => {
    const record = composedworkflow();
    expect(() => applyoverride(record, { id: "o1", workflowid: record.id, pattern: "https://other.example", deltas: { delaybase: 250 }, createdat: now })).toThrow("matches none");
    expect(() => applyoverride(record, { id: "o1", workflowid: record.id, pattern: "https://example.com", deltas: { speed: 2 }, createdat: now })).toThrow("reviewed knobs");
    expect(() => applyoverride(record, { id: "o1", workflowid: record.id, pattern: "https://example.com", deltas: { delaybase: -5 }, createdat: now })).toThrow("positive number");
    const delayrecord = composeworkflow({ id: "wf4", name: "delays", version: 1, origins: ["https://sub.example.com"], steps: [{ id: "d1", kind: "delay", label: "Delay", options: "{\"base\":100}" }], now, kindallowed, riskof });
    const overridden = applyoverride(delayrecord, { id: "o2", workflowid: delayrecord.id, pattern: "https://*.example.com", deltas: { delaybase: 400 }, createdat: now });
