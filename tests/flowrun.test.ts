...
import { dryflowdriver, flowgateof, flowproposalvalue, flowrunexitcodeof, flowstepsensitive, flowtimelinerowsof, parseflowrunrequest, parsegrantsfile, renderflowevents, renderflowtimeline, runflow, type flowdriver } from "../flow.js";
...
function fixturedriver(behavior: Record<string, "done" | "failed"> = {}): flowdriver & { executed: string[] } {
  ...
  return {
    ...
    async execute(step: planfilestep) {
      ...
      return { state: outcome, summary: outcome === "done" ? `The fixture browser executed the step ${step.id} of kind ${step.kind}.` : `The fixture browser failed the step ${step.id}.`, duration: 5 };
    }
  };
}
...
describe("flowrun requests", () => {
  it("parses terminal arguments into one request with its options", () => {
    const request = parseflowrunrequest(["plan.json", "--format", "json", "--output", "runs", "--grants", "grants.json"]);
    expect(request.planpath).toBe("plan.json");
    expect(request.options).toMatchObject({ format: "json", outputdir: "runs", grantspath: "grants.json", interactive: false, dryrun: false });
  ...
  it("derives the sensitivity of one step the same way the runtime does", () => {
    ...
    expect(flowgateof({ id: "s", kind: "type", label: "l" } as never, "https://example.org", "waits").id).toBe("gate:s");
  });
...
describe("flowrun execution", () => {
  it("runs one plan through the fixture browser, seals the chain and exits zero", async () => {
    ...
    const run = await runflow({ request: { planpath: "plan.json", options: { format: "human", outputdir: "out", grantspath: "grants.json", interactive: false, dryrun: false } }, file: plan(), grants: ["https://example.org"], driver, now, clock: () => now });
  ...
  it("passes the proposal validation of the extension before any step runs", async () => {
    ...
    await expect(runflow({ request: { planpath: "plan.json", options: { format: "human", outputdir: "out", grantspath: "grants.json", interactive: false, dryrun: false } }, file: unknownkind, grants: ["https://example.org"], driver: fixturedriver(), now })).rejects.toThrow(/reviewed action kind|outside the grants/);
    const ungranted = plan({ origin: "https://elsewhere.org" });
    await expect(runflow({ request: { planpath: "plan.json", options: { format: "human", outputdir: "out", grantspath: "grants.json", interactive: false, dryrun: false } }, file: ungranted, grants: ["https://example.org"], driver: fixturedriver(), now })).rejects.toThrow(/outside the grants/);
  });

  it("never starts without an origin grant file or an interactive prompt", async () => {
    await expect(runflow({ request: { planpath: "plan.json", options: { format: "human", outputdir: "out", interactive: false, dryrun: true } }, file: plan(), grants: [], driver: dryflowdriver(), now })).rejects.toThrow(/never starts/);
  });

  it("routes every sensitive gate wait to the consent provider and counts the approval", async () => {
    const gates: string[] = [];
    const provider = { resolvegate: async (gate: flowrungate) => { gates.push(gate.id); return "approve" as const; } };
    const typing = plan({ steps: [{ id: "type", kind: "type", label: "Type the query", target: "input" }] });
    const run = await runflow({ request: { planpath: "plan.json", options: { format: "human", outputdir: "out", grantspath: "grants.json", interactive: true, dryrun: false } }, file: typing, grants: ["https://example.org"], provider, driver: fixturedriver(), now, clock: () => now });
  ...
  it("revokes the run when the consent provider refuses and when no provider resolves the gate", async () => {
    const typing = plan({ steps: [{ id: "type", kind: "type", label: "Type the query", target: "input" }] });
    const request = { planpath: "plan.json", options: { format: "human" as const, outputdir: "out", grantspath: "grants.json", interactive: true, dryrun: false } };
    const refused = await runflow({ request, file: typing, grants: ["https://example.org"], provider: { resolvegate: async () => "refuse" as const }, driver: fixturedriver(), now, clock: () => now });
    ...
    const denydefault = await runflow({ request, file: typing, grants: ["https://example.org"], driver: fixturedriver(), now, clock: () => now });
  ...
  it("fails the run with exit code one when the fixture browser fails a step", async () => {
    const failing = plan({ steps: [{ id: "observe", kind: "observe", label: "Observe" }, { id: "read", kind: "readtext", label: "Read", target: "main" }] });
    const run = await runflow({ request: { planpath: "plan.json", options: { format: "human", outputdir: "out", grantspath: "grants.json", interactive: false, dryrun: false } }, file: failing, grants: ["https://example.org"], driver: fixturedriver({ read: "failed" }), now, clock: () => now });
  ...
  it("walks the plan in dry run mode with no page effects", async () => {
    const run = await runflow({ request: { planpath: "plan.json", options: { format: "human", outputdir: "out", grantspath: "grants.json", interactive: false, dryrun: true } }, file: plan(), grants: ["https://example.org"], now, clock: () => now });
    expect(run.outcome.state).toBe("done");
    expect(run.events.some(event => event.summary.includes("without page effects"))).toBe(true);
    await expect(runflow({ request: { planpath: "plan.json", options: { format: "human", outputdir: "out", grantspath: "grants.json", interactive: false, dryrun: false } }, file: plan(), grants: ["https://example.org"], now })).rejects.toThrow(/remote endpoint.*dry run flag/);
  });
});
