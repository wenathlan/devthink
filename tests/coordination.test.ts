...
import { acquirelock, compareoutputs, expirelocks, interleavetimeline, lockkey, mergeresults, preparehandoff, releaselock, replayagentrun, resumehandoff, scanconflicts, sharelesson, swarmcosts, swarmreport, transferhandoff } from "../agent.js";
...
describe("coordination tab handoffs", () => {
  ...
  it("prepares the handoff with the packaged task state and the bound tab", () => {
    const record = preparehandoff({ agents: swarm, id: "h1", fromagentid: "a1", toagentid: "a2", taskstate: "Halfway through the footer.", reason: "The user moved the work.", now });
    ...
    expect(() => preparehandoff({ agents: swarm, id: "h2", fromagentid: "a1", toagentid: "a2", taskstate: " ", now })).toThrow(/packaged task state/i);
    expect(() => preparehandoff({ agents: swarm, id: "h3", fromagentid: "a1", toagentid: "a1", taskstate: "State.", now })).toThrow(/never hands off to itself/i);
    expect(() => preparehandoff({ agents: swarm, id: "h4", fromagentid: "missing", toagentid: "a2", taskstate: "State.", now })).toThrow(/not registered/i);
  });

  it("transfers the tab binding between the agents under the one agent per tab rule and resumes from the packaged state", () => {
    const prepared = preparehandoff({ agents: swarm, id: "h1", fromagentid: "a1", toagentid: "a2", taskstate: "Halfway through the footer.", now });
    ...
    expect(() => resumehandoff({ handoffs: [resumed], id: "h1", now: now + 3 })).toThrow(/only a transferred handoff resumes/i);
  });

  it("refuses the transfer onto a tab another live agent already holds", () => {
    const third = [agent({ id: "a3", name: "Probe", tabid: 7 })];
    const prepared = preparehandoff({ agents: [swarm[0]!, ...third], id: "h1", fromagentid: "a1", toagentid: "a3", taskstate: "State.", now });
    expect(() => transferhandoff({ agents: [swarm[0]!, ...third], handoffs: [prepared], id: "h1", now })).toThrow(/one tab binds one agent/i);
...
describe("coordination resource locks", () => {
  ...
  it("acquires one exclusive lock keyed by the origin and the selector and refuses a second holder", () => {
    ...
    expect(first.locks[0]).toMatchObject({ key: `${origin}|${selector}`, holder: "a1", kind: "exclusive", origin, selector });
  ...
  it("releases one lock with its holder named and returns abandoned locks to the pool past the expiry", () => {
    ...
    const stale: resourcelock = { key: lockkey(origin, selector), holder: "a1", kind: "exclusive", origin, selector, acquiredat: now, expiresat: now + 1000 };
    const eternal: resourcelock = { key: lockkey("https://other.example", "body"), holder: "a2", kind: "shared", origin: "https://other.example", selector: "body", acquiredat: now };
...
describe("coordination conflict scans", () => {
  it("detects the overlapping writes with their writers and the suggested ordering", () => {
    const scan = scanconflicts({ id: "s1", writers: [
  ...
  it("marks a scan clean when no writes overlap", () => {
    const scan = scanconflicts({ id: "s2", writers: [
      { agentid: "a1", origin: "https://example.com", selector: "#form" },
...
describe("coordination result merging with conflict rules", () => {
  ...
  it("keeps the latest value under the last rule and the named agent under preferagent", () => {
    ...
    expect(last.entries.find(candidate => candidate.key === "rowcount")).toMatchObject({ value: "41", agentid: "a2" });
    const preferred = mergeresults({ entries: parallel, rule: "preferagent", preferagent: "a2", now });
    expect(preferred.entries.find(candidate => candidate.key === "rowcount")).toMatchObject({ value: "41", agentid: "a2" });
    const fallback = mergeresults({ entries: parallel, rule: "preferagent", preferagent: "a3", now });
    expect(fallback.entries.find(candidate => candidate.key === "rowcount")).toMatchObject({ value: "41", agentid: "a2" });
  ...
  it("builds the aggregate report across agents with sections, sources and confidence", () => {
    const built = swarmreport({ id: "rep1", title: "The pricing extraction", outputs: parallel, rule: "first", confidence: "Every row was re-read once.", now });
    ...
    const refused = swarmreport({ id: "rep2", title: "The conflicting extraction", outputs: parallel, rule: "fail", now });
...
describe("coordination comparison, timeline, lessons and costs", () => {
  it("contrasts the competing agent outputs for the user", () => {
    const comparison = compareoutputs({ id: "c1", subject: "The row count", outputs: [{ agentid: "a1", value: "42" }, { agentid: "a2", value: "41" }], now });
    ...
    expect(() => compareoutputs({ id: "c2", subject: "s", outputs: [{ agentid: "a1", value: "42" }], now })).toThrow(/at least two/i);
  });
  ...
  it("interleaves the actions of every agent into one ordered stream and replays one agent run", () => {
    ...
    const replay = replayagentrun({ events: [
  ...
  it("writes one verified lesson to the blackboard findings section", () => {
    const board = sharelesson({ board: emptyboard(), id: "l1", agentid: "a1", statement: "The footer renders only after the scroll.", verifiedby: "a3", now });
    const lesson = board.entries[0];
    expect(lesson).toMatchObject({ key: expect.stringContaining("lesson:"), author: "a1", section: "findings", consentclass: "read" });
    expect(lesson?.value).toContain("verified by a3");
    expect(() => sharelesson({ board: emptyboard(), id: "l2", agentid: "a1", statement: "Unverified.", verifiedby: " ", now })).toThrow(/verifier/i);
    expect(() => sharelesson({ board: emptyboard(), id: "l3", agentid: "a1", statement: " ", verifiedby: "a3", now })).toThrow(/statement/i);
  });
