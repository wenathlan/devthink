...
const requestedlimit = Number.parseInt(process.argv.find(argument => argument.startsWith("--limit="))?.split("=")[1] ?? String(identifiers.length), 10);
...
async function inspectscripts(zippath, files) {
  ...
  return { scriptcount: scripts.length, apiuses: [...apiuses].sort(), featureuses: [...featureuses].sort(), transports: [...transports].sort() };
}
...
function boundary(record) {
  ...
  if (permissions.includes("nativeMessaging") || transports.includes("native host bridge")) return transports.includes("network transport signal") ? "hybrid local host and network hint" : "local native-host bridge indicated";
  if (transports.includes("network transport signal")) return "network-capable signal; endpoint and data path unverified";
  if (transports.includes("extension runtime message") || transports.includes("tab or content bridge")) return "extension-local bridge indicated";
  return "not established from manifest and static signals";
}

function sourcestatus(record) {
  if (record.identifier === "infppggnoaenmfagbfknfkancpbljcca") return "public source known; license is mixed AGPL/commercial";
  if (record.identifier === "hkcbjmdeheceggmlbhbiljejkkpjljjp") return "listing label says open source; repository/license not yet verified";
...
async function inspect(identifier) {
  ...
  try {
    const files = (await execute("unzip", ["-Z1", zippath], { maxBuffer: 8 * 1024 * 1024 })).stdout.trim().split("\n").filter(Boolean);
...
for (const identifier of selected) {
  try {
    ...
    records.push({ identifier, status: `inspection error: ${error instanceof Error ? error.message : String(error)}`, source: "Chrome update service" });
  }
}
await rm(artifacts, { recursive: true, force: true });
...
const details = records.filter(record => record.manifest).map(record => {
...
const permissionrows = aggregate(records.filter(record => record.manifest).flatMap(record => record.manifest.permissions ?? [])).map(([name, count]) => `| ${name} | ${count} |`).join("\n");
const apirows = aggregate(records.filter(record => record.staticanalysis).flatMap(record => record.staticanalysis.apiuses)).map(([name, count]) => `| ${name} | ${count} |`).join("\n");
const featurerows = aggregate(records.filter(record => record.staticanalysis).flatMap(record => record.staticanalysis.featureuses)).map(([name, count]) => `| ${name} | ${count} |`).join("\n");
...
const transportrows = records.map(record => `| ${record.identifier} | ${record.status} | ${record.staticanalysis ? compactvalues(record.staticanalysis.transports) : "none observed"} | ${boundary(record)} | ${sourcestatus(record)} |`).join("\n");
...
console.log(JSON.stringify({ requested: selected.length, verified: records.filter(record => record.status === "manifest verified").length, failed: records.filter(record => record.status !== "manifest verified").length }, null, 2));
