...
import { agentbudgetcheck, agenteventof, agentheartbeat, agentruncontext, assignrole, killall, opentabagent, pauseone, recordagentusage, registeragent, resumeone, roledefaults, scopegate, spawn, stopone, swarmoverview, swarmstateof, disarmkillswitch } from "../agent.js";
import { sendmessage, receivemessages, mailboxof, roleaddress, unreadcount, broadcastrecipient, resolverecipients } from "../agent.js";
...
describe("multiagent sub agents and depth limits", () => {
  it("spawns a sub agent under a spawn request with the parent and the depth", () => {
    const spawned = spawn({ agents: [agent()], request: { parentid: "a1", role: "planner", task: "Draft the extraction plan", depth: 1 }, limit: { maxdepth: 2 }, id: "a2", now });
  ...
  it("refuses the recursion past the configured depth limit", () => {
    const child = spawn({ agents: [agent()], request: { parentid: "a1", role: "worker", task: "First", depth: 1 }, limit: { maxdepth: 3 }, id: "a2", now });
    expect(() => spawn({ agents: child, request: { parentid: "a2", role: "worker", task: "Too deep", depth: 2 }, limit: { maxdepth: 1 }, id: "a3", now })).toThrow(/depth limit/i);
    const grandchild = spawn({ agents: child, request: { parentid: "a2", role: "worker", task: "Second", depth: 2 }, limit: { maxdepth: 3 }, id: "a3", now });
    expect(() => spawn({ agents: grandchild, request: { parentid: "a3", role: "worker", task: "Too deep", depth: 3 }, limit: { maxdepth: 2 }, id: "a4", now })).toThrow(/depth limit/i);
  });

  it("refuses a spawn whose depth does not sit exactly one level under the parent", () => {
    expect(() => spawn({ agents: [agent()], request: { parentid: "a1", role: "worker", task: "Skipped level", depth: 2 }, limit: {}, id: "a2", now })).toThrow(/one level under/i);
    expect(() => spawn({ agents: [agent({ id: "other" })], request: { parentid: "missing", role: "worker", task: "No parent", depth: 1 }, limit: {}, id: "a2", now })).toThrow(/not registered/i);
  });

  it("keeps spawning unbounded when no depth limit is configured", () => {
    const child = spawn({ agents: [agent()], request: { parentid: "a1", role: "worker", task: "First", depth: 1 }, limit: {}, id: "a2", now });
    const grandchild = spawn({ agents: child, request: { parentid: "a2", role: "worker", task: "Second", depth: 2 }, limit: {}, id: "a3", now });
...
describe("multiagent budgets and scopes", () => {
  ...
  it("refuses tools and origins outside the granted scope while an absent scope stays unbounded", () => {
    ...
    expect(scopegate({ scope: undefined, origin: "https://any.example", namespace: "browser" })).toMatchObject({ allowed: true });
    const open: agentscope = { agentid: "a2", origins: [], toolnamespaces: [] };
    expect(scopegate({ scope: open, origin: "https://any.example", namespace: "browser" })).toMatchObject({ allowed: true });
  });
  ...
  it("builds the per agent run context on the workflow engine records", () => {
    const context = agentruncontext({ agent: agent(), task: { id: "t1", lane: "extraction", priority: 2, payload: "Read the table", state: "queued", enqueuedat: now }, now });
...
describe("multiagent mailboxes", () => {
  ...
  it("delivers a direct message to one recipient inbox with the unread counter", () => {
    const delivered = sendmessage({ mailboxes: [mailbox("a1"), mailbox("a2"), mailbox("a3")], agents, id: "m1", senderid: "a1", recipient: "a2", routing: "direct", payload: "The table is ready.", now });
  ...
  it("delivers a broadcast to every agent but the sender", () => {
    const delivered = sendmessage({ mailboxes: [mailbox("a1"), mailbox("a2"), mailbox("a3")], agents, id: "m2", senderid: "a1", recipient: broadcastrecipient, routing: "broadcast", payload: "Status check.", now });
  ...
  it("routes a role addressed message to every live agent of the role", () => {
    ...
    const delivered = sendmessage({ mailboxes: [mailbox("a1"), mailbox("a2"), mailbox("a3")], agents, id: "m3", senderid: "a1", recipient: "observer", routing: "role", payload: "Watch the banner.", now });
    expect(mailboxof(delivered, "a3").inbox).toHaveLength(1);
  ...
  it("refuses a direct message to an unknown recipient or to the sender itself", () => {
    expect(() => sendmessage({ mailboxes: [], agents, id: "m4", senderid: "a1", recipient: "missing", routing: "direct", payload: "Hello?", now })).toThrow(/not registered/i);
    expect(() => sendmessage({ mailboxes: [], agents, id: "m5", senderid: "a1", recipient: "a1", routing: "direct", payload: "Note to self.", now })).toThrow(/own sender/i);
    expect(() => sendmessage({ mailboxes: [], agents, id: "m6", senderid: "a1", recipient: "a2", routing: "direct", payload: "  ", now })).toThrow(/payload/i);
  });

  it("drains one inbox with ack tracking and resets the unread counter", () => {
    const delivered = sendmessage({ mailboxes: [mailbox("a2")], agents, id: "m7", senderid: "a1", recipient: "a2", routing: "direct", payload: "First.", now });
    const second = sendmessage({ mailboxes: delivered, agents, id: "m8", senderid: "a1", recipient: "a2", routing: "direct", payload: "Second.", now: now + 1 });
...
describe("multiagent overview and events", () => {
  it("returns the swarm at a glance with agents, tasks and messages", () => {
    ...
    const queue = enqueue({ queue: emptyqueue({ lanes: ["extraction"] }), id: "t1", lane: "extraction", priority: 1, payload: "Read the table", now });
    const mailboxes = sendmessage({ mailboxes: [mailbox("a1"), mailbox("a2")], agents, id: "m1", senderid: "a1", recipient: "a2", routing: "direct", payload: "Hi.", now });
    const overview = swarmoverview({ agents, queue, mailboxes });
    expect(overview).toMatchObject({ agents: 2, active: 1, paused: 1, tasks: 1, queued: 1, claimed: 0, messages: 1, unread: 1 });
  });

  it("builds the agent lifecycle events and the swarm state snapshot", () => {
    const event = agenteventof({ id: "e1", kind: "claimed", agentid: "a1", taskid: "t1", summary: "The agent claimed the task.", now });
    expect(event).toMatchObject({ kind: "claimed", agentid: "a1", taskid: "t1" });
    const snapshot = swarmstateof({ agents: [agent()], queue: emptyqueue(), mailboxes: [], killswitch: { engaged: false } });
