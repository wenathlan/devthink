...
import {
  blockinvocationof, composeworkflow, expandblocks, expressionof, expressionoperators, expressioneval, regexruleof, regexextract, steptemplateof, workflowblockof, workflowstepof, workflowkinds,
  bindvariables, cancelrun, delayjitter, dryrunworkflow, newworkflowrun, pauserun, popscope, pushscope, resolvevariable, runstep, runworkflow, seededrandom, setvariable, validateworkflow, waitelementplan, watchdogpass,
  backoffdelay, branchof, choosebranch, conditionof, controlflowkinds, controlsteps, controlsummary, defaultloopbound, evaluatecondition, foreachof, iscontrolflowkind, joinbranches, loopof, parallelof, repeatuntilof, runcontrolstep, runforeach, runloop, runparallel, runrepeatuntil, runtry, runwhile, tryof, validatecontrolpayload, visitmatch, whileof, applyretry, applyruntimeout, applytimeout, cancellederror,
  armrule, applycooldown, cronnext, cronparse, drainqueue, evaluatetrigger, eventrulematches, istriggerkind, listdue, matchurl, observeevents, pauseall, queuefire, resumeall, runurllist, schedulecron, scheduleinterval, triggerfamilyof, triggerfamilies, triggerkinds, triggerpayloadof, triggersummary, verifywebhook, webhooksecretok, timezonevalid, manualpreview, confirmmanualrun, ruleorigins, ruleoriginsgranted, updaterule, defaulttriggercooldown,
  buildsteplibrary, loadworkflow, saveworkflow, snapnode, reordersteps, groupselect, expandtemplate, addnode, editstep, renderminimap, minimapfocus, zoomcanvas, searchsteps, markbreakpoint, runtobreakpoint, diffversions, exportworkflow, shareworkflow, importworkflow, bindparam, applyoverride, addedge, removeedge, removenode, undoedit, redoedit, palettecategories,
} from "../workflow.js";
import type { branchstep, conditionstep, editormodel, expressiontype, foreachstep, loopstep, parallelstep, repeatuntilstep, retrypolicy, siteoverride, steptemplate, taskqueue as _unusedq, trystep, variablescope, variablevalue, whilestep, workflowblock, workflowrecord, workflowrun, workflowstep, triggerule, manualrun } from "../types.js";
...
function scope(variables: Array<{ name: string; kind: "string" | "number" | "boolean" | "list" | "element"; value: string | number | boolean | string[] }>): variablescope[] {
  return [{ name: "root", variables: variables.map(variable => ({ name: variable.name, kind: variable.kind, value: variable.value, setat: now })) }];
}

/** The plain executor stub that always succeeds. */
const execute = async (one: workflowstep): Promise<{ ok: boolean; summary: string }> => ({ ok: true, summary: `done ${one.id}` });
...
describe("torture: step, block and template normalizers", () => {
  ...
  it("refuses wrong typed fields and validates the optional markers", () => {
    ...
    expect(workflowstepof({ id: "s1", kind: "wait", label: "l", breakpoint: true })).toMatchObject({ id: "s1", breakpoint: true });
  ...
  it("keeps unicode and injection payloads verbatim inside the normalized step", () => {
    const parsed = workflowstepof({ id: "\u00e9", kind: "wait", label: injection, target: injection, value: injection });
  ...
  it("normalizes block invocations and refuses the blank names", () => {
    expect(blockinvocationof({ block: "login", label: "the login block" })).toEqual({ block: "login", label: "the login block" });
    expect(blockinvocationof({ block: "  ", label: "l" })).toBeUndefined();
    expect(blockinvocationof({ block: "login", label: "" })).toBeUndefined();
    expect(blockinvocationof({ block: "login", label: "l", params: [{ name: "bad name", kind: "string" }] })).toBeUndefined();
    expect(blockinvocationof(null)).toBeUndefined();
  });

  it("normalizes workflow blocks with mixed steps and refuses malformed children", () => {
    const block = workflowblockof({ name: "login", label: "The login block", steps: [step("s1"), { block: "inner", label: "inner" }] });
  ...
  it("normalizes step templates with the shared time and refuses the broken ones", () => {
    ...
    expect(steptemplateof({ id: "t1", name: "n", origin, step: { id: "s", kind: "WAIT", label: "l" }, sharedat: now })).toBeUndefined();
  ...
  it("normalizes expressions with the reviewed operator set and refuses the rest", () => {
    ...
    expect(expressionof({ left: { ref: "a" }, right: { literal: 1 }, operator: "add", result: "sum", resultkind: "number" })).toBeDefined();
    expect(expressionof({ left: { literal: 1 }, operator: "not", result: "flag", resultkind: "boolean" })).toBeDefined();
    ...
    expect(expressionof({ left: { ref: "Bad Name" }, operator: "not", result: "x", resultkind: "boolean" })).toBeUndefined();
    expect(expressionof({ left: { literal: 1 }, right: { literal: "x" }, operator: "add", result: "x", resultkind: "number" })).toBeDefined();
    expect(expressionof({ left: { literal: 1 }, operator: "add", result: "X", resultkind: "number" })).toBeUndefined();
    expect(expressionof({ left: { literal: 1 }, operator: "add", result: "x", resultkind: "bigint" as never })).toBeUndefined();
  ...
  it("expands the blocks, refuses the cycles and the unknown references", () => {
    ...
    const blocks: workflowblock[] = [{ name: "outer", label: "outer", steps: [{ block: "inner", label: "inner" }] }, { name: "inner", label: "inner", steps: [step("i1")] }];
    ...
    expect(() => expandblocks([{ block: "outer", label: "outer" }], [{ name: "outer", label: "o", steps: [{ block: "outer", label: "outer" }] }])).toThrow(/recurs inside itself/i);
...
describe("torture: composition, validation and risk grading", () => {
  it("refuses blank names, broken versions and non https origins", () => {
    expect(() => composeworkflow({ name: " ", version: 1, origins: [origin], steps: [step("s1")], now })).toThrow(/name/i);
    expect(() => composeworkflow({ name: "n", version: 0, origins: [origin], steps: [step("s1")], now })).toThrow(/version/i);
    expect(() => composeworkflow({ name: "n", version: 1.5, origins: [origin], steps: [step("s1")], now })).toThrow(/version/i);
    expect(() => composeworkflow({ name: "n", version: NaN, origins: [origin], steps: [step("s1")], now })).toThrow(/version/i);
    expect(() => composeworkflow({ name: "n", version: 1, origins: [], steps: [step("s1")], now })).toThrow(/at least one granted HTTPS origin/i);
    expect(() => composeworkflow({ name: "n", version: 1, origins: ["http://insecure.example"], steps: [step("s1")], now })).toThrow(/HTTPS/i);
    expect(() => composeworkflow({ name: "n", version: 1, origins: ["not a url"], steps: [step("s1")], now })).toThrow(/not a valid url/i);
  });

  it("refuses duplicate block names and unknown binding sources", () => {
    const blocks: workflowblock[] = [{ name: "b", label: "b", steps: [step("s1")] }, { name: "b", label: "b", steps: [step("s2")] }];
    expect(() => composeworkflow({ name: "n", version: 1, origins: [origin], steps: [step("s1")], blocks, now })).toThrow(/unique/i);
    const binding = [{ variable: "rows", kind: "number" as const, stepid: "ghost" }];
    expect(() => composeworkflow({ name: "n", version: 1, origins: [origin], steps: [step("s1", { bindings: binding })], now })).toThrow(/references the unknown step ghost/i);
  });

  it("refuses the unreviewed kinds at the top level and inside the control payloads", () => {
    expect(() => composeworkflow({ name: "n", version: 1, origins: [origin], steps: [{ ...step("s1"), kind: "dimensionhop" as never }], now, kindallowed: kind => kind === "wait" })).toThrow(/not a reviewed action kind/i);
    const loop = controlstep("l1", "loop", { loop: { list: "items", item: "it", index: "ix", steps: [{ ...step("c1"), kind: "dimensionhop" as never }] } });
    expect(() => composeworkflow({ name: "n", version: 1, origins: [origin], steps: [loop], now, kindallowed: kind => kind === "wait" || kind === "loop" })).toThrow(/inside the control payload of l1/i);
  });

  it("grades the workflow risk from the most sensitive member", () => {
    expect(compose([step("s1")]).risk).toBe("read");
    expect(composeworkflow({ name: "n", version: 1, origins: [origin], steps: [step("s1")], now, riskof: kind => kind === "wait" ? "interaction" : "read" }).risk).toBe("interaction");
    expect(composeworkflow({ name: "n", version: 1, origins: [origin], steps: [step("s1")], now, riskof: () => "sensitive" }).risk).toBe("sensitive");
    const loop = controlstep("l1", "loop", { loop: { list: "items", item: "it", index: "ix", steps: [step("c1")] } });
    expect(composeworkflow({ name: "n", version: 1, origins: [origin], steps: [loop], now, riskof: kind => kind === "wait" ? "sensitive" : "read" }).risk).toBe("sensitive");
  });
  ...
  it("freezes the composed record so no later mutation rewrites the review", () => {
    ...
    expect(() => { (record.steps as { push: (one: unknown) => number }).push(step("s2")); }).toThrow();
  });

  it("validates the bindings against earlier steps and the variables against the definitions", () => {
    const record = compose([step("s1"), step("s2", { bindings: [{ variable: "rows", kind: "number", stepid: "s1" }] }), workflowstepof({ id: "s3", kind: "wait", label: "l", expression: { left: { ref: "rows" }, right: { literal: 1 }, operator: "add", result: "sum", resultkind: "number" } })!]);
    ...
    const undefinedref = compose([workflowstepof({ id: "s1", kind: "wait", label: "l", expression: { left: { ref: "ghost" }, right: { literal: 1 }, operator: "add", result: "sum", resultkind: "number" } })!]);
  ...
  it("carries the unicode origins and deduplicates them", () => {
    const record = composeworkflow({ name: "n", version: 1, origins: [`${origin}/path?x=1`, `${origin}/other`, origin], steps: [step("s1")], now, riskof: () => "read" });
...
describe("torture: typed scope stack, bindings and expressions", () => {
  ...
  it("binds the outcome values with the coercion rules and refuses the mismatches", () => {
    const outputs = { s1: { stepid: "s1", ok: true, summary: "rows: 12", details: { count: 12, flag: true, list: ["a", "b"], text: "t" }, at: now } };
    const bound = bindvariables(scope([]), [
    ...
    expect(() => bindvariables(scope([]), [{ variable: "bad", kind: "number", stepid: "s1", path: "text" }], outputs, now)).toThrow(/not a finite number/i);
    expect(() => bindvariables(scope([]), [{ variable: "bad", kind: "boolean", stepid: "s1", path: "text" }], outputs, now)).toThrow(/not a boolean/i);
    expect(() => bindvariables(scope([]), [{ variable: "bad", kind: "list", stepid: "s1", path: "flag" }], outputs, now)).toThrow(/not a list/i);
    expect(() => bindvariables(scope([]), [{ variable: "bad", kind: "element", stepid: "s1", path: "count" }], outputs, now)).toThrow(/not an element reference/i);
    expect(() => bindvariables(scope([]), [{ variable: "bad", kind: "string", stepid: "s1", path: "missing.deep" }], outputs, now)).toThrow(/found no value at missing.deep/i);
  });

  it("coerces the numeric strings and splits comma lists on binding", () => {
    const outputs = { s1: { stepid: "s1", ok: true, summary: "s", details: { numeric: "42", joined: "a,b,c" }, at: now } };
    const bound = bindvariables(scope([]), [{ variable: "n", kind: "number", stepid: "s1", path: "numeric" }, { variable: "l", kind: "list", stepid: "s1", path: "joined" }], outputs, now);
  ...
  it("evaluates the arithmetic with the division and modulo guards", () => {
    const scopes = scope([{ name: "a", kind: "number", value: 10 }, { name: "b", kind: "number", value: 3 }]);
    expect(expressioneval({ left: { ref: "a" }, right: { ref: "b" }, operator: "add", result: "x", resultkind: "number" }, scopes)).toBe(13);
    expect(expressioneval({ left: { ref: "a" }, right: { ref: "b" }, operator: "subtract", result: "x", resultkind: "number" }, scopes)).toBe(7);
    expect(expressioneval({ left: { ref: "a" }, right: { ref: "b" }, operator: "multiply", result: "x", resultkind: "number" }, scopes)).toBe(30);
    expect(expressioneval({ left: { ref: "a" }, right: { ref: "b" }, operator: "divide", result: "x", resultkind: "number" }, scopes)).toBeCloseTo(10 / 3);
    expect(expressioneval({ left: { ref: "a" }, right: { ref: "b" }, operator: "modulo", result: "x", resultkind: "number" }, scopes)).toBe(1);
    expect(() => expressioneval({ left: { ref: "a" }, right: { literal: 0 }, operator: "divide", result: "x", resultkind: "number" }, scopes)).toThrow(/divides by zero/i);
    expect(() => expressioneval({ left: { ref: "a" }, right: { literal: 0 }, operator: "modulo", result: "x", resultkind: "number" }, scopes)).toThrow(/divides by zero/i);
  });

  it("evaluates the comparisons, logic and text operators with coercion refusals", () => {
    const scopes = scope([{ name: "s", kind: "string", value: "abc" }, { name: "t", kind: "boolean", value: true }, { name: "f", kind: "boolean", value: false }, { name: "n", kind: "number", value: 5 }]);
    expect(expressioneval({ left: { literal: "5" }, right: { ref: "n" }, operator: "less", result: "x", resultkind: "boolean" }, scopes)).toBe(false);
    expect(expressioneval({ left: { literal: 5 }, right: { ref: "n" }, operator: "equal", result: "x", resultkind: "boolean" }, scopes)).toBe(true);
    expect(expressioneval({ left: { literal: "5" }, right: { ref: "n" }, operator: "equal", result: "x", resultkind: "boolean" }, scopes)).toBe(false);
    expect(expressioneval({ left: { ref: "t" }, right: { ref: "f" }, operator: "and", result: "x", resultkind: "boolean" }, scopes)).toBe(false);
    expect(expressioneval({ left: { ref: "t" }, right: { ref: "f" }, operator: "or", result: "x", resultkind: "boolean" }, scopes)).toBe(true);
    expect(expressioneval({ left: { ref: "t" }, operator: "not", result: "x", resultkind: "boolean" }, scopes)).toBe(false);
    expect(() => expressioneval({ left: { ref: "s" }, right: { ref: "t" }, operator: "and", result: "x", resultkind: "boolean" }, scopes)).toThrow(/not a boolean/i);
    expect(expressioneval({ left: { ref: "s" }, right: { literal: "b" }, operator: "concat", result: "x", resultkind: "string" }, scopes)).toBe("abcb");
    expect(expressioneval({ left: { ref: "s" }, right: { literal: "b" }, operator: "contains", result: "x", resultkind: "boolean" }, scopes)).toBe(true);
    expect(expressioneval({ left: { ref: "s" }, operator: "length", result: "x", resultkind: "number" }, scopes)).toBe(3);
  });

  it("refuses the list operands outside the contains and length operators", () => {
    const scopes = scope([{ name: "list", kind: "list", value: ["a", "b"] }]);
    expect(expressioneval({ left: { ref: "list" }, right: { literal: "a" }, operator: "contains", result: "x", resultkind: "boolean" }, scopes)).toBe(true);
    expect(expressioneval({ left: { ref: "list" }, operator: "length", result: "x", resultkind: "number" }, scopes)).toBe(2);
    expect(() => expressioneval({ left: { ref: "list" }, right: { literal: 1 }, operator: "add", result: "x", resultkind: "number" }, scopes)).toThrow(/needs the contains or length operator/i);
    expect(() => expressioneval({ left: { ref: "ghost" }, right: { literal: 1 }, operator: "add", result: "x", resultkind: "number" }, scopes)).toThrow(/undefined variable ghost/i);
  });

  it("extracts the named groups and answers the honest no match", () => {
    const rule = { pattern: "price:\\s*(?<amount>\\d+)", flags: "", groups: ["amount"] };
    expect(regexextract(rule, "price: 42", now)).toEqual({ matched: true, variables: [{ name: "amount", kind: "string", value: "42", setat: now }] });
...
describe("torture: run lifecycle, gates and the dry run projection", () => {
  ...
  it("refuses the run behind the session, plan and origin gates", async () => {
    ...
    await expect(runworkflow({ record, run, execute, now, gates: { sessionactive: false, planapproved: true, origingranted: () => true } })).rejects.toThrow(/outside an approved session/i);
    await expect(runworkflow({ record, run, execute, now, gates: { sessionactive: true, planapproved: false, origingranted: () => true } })).rejects.toThrow(/approved plan review/i);
    await expect(runworkflow({ record, run, execute, now, gates: { sessionactive: true, planapproved: true, origingranted: () => false } })).rejects.toThrow(/falls outside the session grants/i);
  });
  ...
  it("refuses the run of a terminal state and resumes the paused run from its checkpoint", async () => {
    ...
    for (const state of ["done", "failed", "cancelled"] as const) {
      await expect(runworkflow({ record, run: { ...newworkflowrun({ workflowid: record.id, now }), state }, execute, now })).rejects.toThrow(new RegExp(`already ${state}`, "i"));
    }
    const paused = { ...newworkflowrun({ id: "r1", workflowid: record.id, now }), state: "paused" as const, cursor: 1, pausedat: now };
  ...
  it("runs the steps in order, checkpoints and fails honestly on the executor failure", async () => {
    ...
    const failing = async (one: workflowstep): Promise<{ ok: boolean; summary: string }> => one.id === "s2" ? { ok: false, summary: "the wait timed out" } : { ok: true, summary: `done ${one.id}` };
    const failed = await runworkflow({ record, run: newworkflowrun({ workflowid: record.id, now }), execute: failing, now });
    ...
    let checkpoints = 0;
    await runworkflow({ record, run: newworkflowrun({ workflowid: record.id, now }), execute, now, oncheckpoint: () => { checkpoints += 1; } });
  ...
  it("interpolates the step fields from the scopes and records the consumed names", async () => {
    ...
    const executed = await runstep({ step: record.steps[0]!, scopes: scope([{ name: "name", kind: "string", value: "reviewed" }]), outputs: {}, execute, now });
    expect(executed.output.summary).toBe("done s1");
    expect(executed.log.consumed).toEqual(["name"]);
    await expect(runstep({ step: record.steps[0]!, scopes: scope([]), outputs: {}, execute, now })).resolves.toMatchObject({ output: { ok: false } });
  });
  ...
  it("answers the dry run with the read only projection and the refusal entries", () => {
    ...
    const projected = dryrunworkflow({ record, run: newworkflowrun({ workflowid: record.id, dryrun: true, now }), now, projection: one => one.kind === "wait" ? `would wait ${one.value}` : undefined });
  ...
  it("grades the watchdog verdicts at the stall and zombie boundaries", () => {
    ...
    const verdicts = watchdogpass({ runs, lastcompletedat: { r1: now - 500, r2: now - 1000 }, liveexecutors: ["r1"], config: { enabled: true, stallthreshold: 500, zombiewindow: 1000, action: "pause" as never }, now });
    ...
    const healthy = watchdogpass({ runs: [runs[0]!], lastcompletedat: { r1: now - 100 }, liveexecutors: ["r1"], config: { enabled: true, stallthreshold: 500, zombiewindow: 1000, action: "pause" as never }, now });
...
describe("torture: control flow payloads, bounds and runners", () => {
  it("normalizes the control payloads and refuses the malformed twins", () => {
    expect(conditionof({ expression: { left: { ref: "ready" }, operator: "not", result: "stop", resultkind: "boolean" } })).toBeDefined();
    expect(conditionof({ expression: { left: { ref: "ready" }, operator: "add", result: "sum", resultkind: "number" } })).toBeUndefined();
    ...
    expect(loopof({ list: "items", item: "it", index: "ix", bound: 3, steps: [step("c1")] })).toMatchObject({ bound: 3 });
    expect(whileof({ while: { left: { ref: "ready" }, operator: "not", result: "stop", resultkind: "boolean" }, steps: [step("c1")] })).toBeUndefined();
    expect(whileof({ while: { left: { ref: "ready" }, operator: "not", result: "stop", resultkind: "boolean" }, bound: 2, steps: [step("c1")] })).toMatchObject({ bound: 2 });
    expect(whileof({ while: { left: { ref: "ready" }, operator: "not", result: "stop", resultkind: "boolean" }, bound: 0, steps: [step("c1")] })).toBeUndefined();
    expect(repeatuntilof({ until: { left: { ref: "ready" }, operator: "not", result: "stop", resultkind: "boolean" }, steps: [step("c1")] })).toBeDefined();
    ...
    expect(parallelof({ branches: [{ id: "b1", steps: [step("c1")] }], join: { strategy: "first", onfail: "cancel" } })).toBeDefined();
    expect(parallelof({ branches: [{ id: "b1", steps: [step("c1")] }, { id: "b1", steps: [step("c2")] }], join: { strategy: "first", onfail: "cancel" } })).toBeUndefined();
    expect(parallelof({ branches: [{ id: "b1", steps: [step("c1")] }], join: { strategy: "never", onfail: "cancel" } })).toBeUndefined();
    expect(parallelof({ branches: [{ id: "b1", steps: [step("c1")] }], join: { strategy: "first", onfail: "ignore" } })).toBeUndefined();
  ...
  it("validates the control payload of every kind with the kind specific error", () => {
    expect(() => validatecontrolpayload(controlstep("c1", "condition", {}))).toThrow(/needs a reviewed boolean expression/i);
    ...
    expect(() => validatecontrolpayload(controlstep("c1", "whileloop", {}))).toThrow(/mandatory positive safety bound/i);
    ...
    expect(() => validatecontrolpayload(workflowstepof({ id: "c1", kind: "wait", label: "l", options: "{bad json" })!)).not.toThrow();
    expect(() => validatecontrolpayload(workflowstepof({ id: "c1", kind: "loop", label: "l", options: "{bad json" })!)).toThrow(/JSON object/i);
  });

  it("collects the child steps recursively and summarizes the constructs for review", () => {
    const nested = controlstep("l1", "loop", { loop: { list: "items", item: "it", index: "ix", steps: [controlstep("l2", "foreach", { foreach: { selector: ".row", item: "e", index: "i", steps: [step("c1")] } })] } });
    expect(controlsteps(nested).map(child => child.id)).toEqual(["l2", "c1"]);
    expect(controlsteps(step("s1"))).toEqual([]);
    expect(controlsteps(controlstep("c1", "loop", { loop: { list: "items", item: "it", index: "ix", steps: [step("x")] } }))).toHaveLength(1);
    const summary = controlsummary(nested);
    expect(summary).toMatchObject({ kind: "loop", list: "items", item: "it", index: "ix" });
    expect(controlsummary(controlstep("r1", "repeatuntil", { repeatuntil: { until: { left: { ref: "ready" }, operator: "not", result: "stop", resultkind: "boolean" }, steps: [step("c1")] } }))).toMatchObject({ kind: "repeatuntil", bound: defaultloopbound });
    expect(controlsummary(controlstep("w1", "whileloop", { while: { while: { left: { ref: "ready" }, operator: "not", result: "stop", resultkind: "boolean" }, bound: 7, steps: [step("c1")] } }))).toMatchObject({ kind: "whileloop", bound: 7 });
    expect(controlsummary(controlstep("b1", "branch", { branch: { paths: [{ name: "one", steps: [step("c1")] }], else: { name: "other", steps: [] } } }))).toMatchObject({ kind: "branch", paths: ["one"], elsepath: "other" });
    expect(controlsummary(step("s1"))).toBeUndefined();
    expect(controlsummary(controlstep("c1", "loop", { loop: { list: "l", item: "i", index: "x", steps: [step("c1")] } }))).toMatchObject({ bound: defaultloopbound });
  });

  it("evaluates the conditions and refuses the non boolean result", () => {
    const scopes = scope([{ name: "flag", kind: "boolean", value: true }]);
    expect(evaluatecondition({ expression: { left: { ref: "flag" }, operator: "not", result: "x", resultkind: "boolean" } }, scopes)).toBe(false);
    expect(() => evaluatecondition({ expression: { left: { literal: 1 }, right: { literal: 1 }, operator: "add", result: "x", resultkind: "number" } }, scopes)).toThrow(/must resolve to a boolean/i);
  });

  it("chooses the branch path by page state with the unconditional and else fallbacks", () => {
    const readywhen: expressiontype = { left: { ref: "pageready" }, right: { literal: true }, operator: "equal", result: "x", resultkind: "boolean" };
    ...
    const negated: branchstep = { paths: [{ name: "ready", when: { left: { ref: "pageready" }, operator: "not", result: "x", resultkind: "boolean" }, steps: [step("r1")] }], else: branch.else };
    const elsepath = choosebranch({ stepid: "b1", branch: negated, scopes: scope([]), pagestate: { ready: true }, now });
  ...
  it("runs the loop inside the bound and refuses the list that exceeds it", async () => {
    ...
    const within = await runloop({ step: controlstep("l1", "loop", { loop }), loop, scopes: scope([{ name: "items", kind: "list", value: ["a", "b"] }]), outputs: {}, execute, now });
    expect(within.ok).toBe(true);
    expect(within.summary).toMatch(/ran 2 iterations/i);
    const overflow = await runloop({ step: controlstep("l1", "loop", { loop }), loop, scopes: scope([{ name: "items", kind: "list", value: ["a", "b", "c"] }]), outputs: {}, execute, now });
    expect(overflow.ok).toBe(false);
    expect(overflow.summary).toMatch(/exceeds the reviewed safety bound of 2/i);
    await expect(runloop({ step: controlstep("l1", "loop", { loop }), loop, scopes: scope([]), outputs: {}, execute, now })).rejects.toThrow(/undefined list variable items/i);
    await expect(runloop({ step: controlstep("l1", "loop", { loop }), loop, scopes: scope([{ name: "items", kind: "string", value: "a" }]), outputs: {}, execute, now })).rejects.toThrow(/not a list/i);
    const emptyloop: loopstep = { ...loop, bound: 0 };
    const empty = await runloop({ step: controlstep("l1", "loop", { loop: emptyloop }), loop: emptyloop, scopes: scope([{ name: "items", kind: "list", value: [] }]), outputs: {}, execute, now });
  ...
  it("runs the loop body per iteration and rebinds the item and index", async () => {
    ...
    const runner = async (one: workflowstep, context: { scopes: variablescope[] }): Promise<{ ok: boolean; summary: string }> => {
    ...
    const result = await runloop({ step: controlstep("l1", "loop", { loop }), loop, scopes: scope([{ name: "items", kind: "list", value: ["a", "b"] }]), outputs: {}, execute: runner, now });
    expect(result.ok).toBe(true);
    expect(seen).toEqual(["c1:a:0", "c1:b:1"]);
  });

  it("runs the repeat until convergence and refuses the endless block", async () => {
    const until: expressiontype = { left: { ref: "done" }, right: { literal: true }, operator: "equal", result: "x", resultkind: "boolean" };
    let scopes = scope([{ name: "done", kind: "boolean", value: false }]);
    const converging = async (one: workflowstep, context: { scopes: variablescope[] }): Promise<{ ok: boolean; summary: string; scopes?: variablescope[] }> => {
      if (one.id === "c1") return { ok: true, summary: "flips", scopes: setvariable(context.scopes, "done", "boolean", true, now) };
      return { ok: true, summary: "done" };
    };
    const converged = await runrepeatuntil({ step: controlstep("r1", "repeatuntil", { repeatuntil: { until, bound: 3, steps: [step("c1")] } }), repeat: { until, bound: 3, steps: [step("c1")] }, scopes, outputs: {}, execute: converging, now });
    expect(converged.ok).toBe(true);
    expect(converged.summary).toMatch(/converged after 1 iteration/i);
    const endless = await runrepeatuntil({ step: controlstep("r2", "repeatuntil", { repeatuntil: { until, bound: 2, steps: [step("c2")] } }), repeat: { until, bound: 2, steps: [step("c2")] }, scopes, outputs: {}, execute, now });
  ...
  it("runs the while loop until the condition stops and reports the overflow", async () => {
    const condition = { while: { left: { ref: "count" }, right: { literal: 2 }, operator: "less", result: "x", resultkind: "boolean" }, bound: 5, steps: [{ ...step("c1") }] } as whilestep;
    let scopes = scope([{ name: "count", kind: "number", value: 0 }]);
    const counting = async (one: workflowstep, context: { scopes: variablescope[] }): Promise<{ ok: boolean; summary: string; scopes?: variablescope[] }> => {
      if (one.id === "c1") {
    ...
    const ended = await runwhile({ step: controlstep("w1", "whileloop", { while: condition }), condition, scopes: scope([{ name: "count", kind: "number", value: 0 }]), outputs: {}, execute: counting, now });
    expect(ended.ok).toBe(true);
    expect(ended.summary).toMatch(/ended after 2 iterations/i);
    const overflowing = await runwhile({ step: controlstep("w2", "whileloop", { while: { ...condition, bound: 1 } }), condition: { ...condition, bound: 1 }, scopes: scope([{ name: "count", kind: "number", value: 0 }]), outputs: {}, execute: counting, now });
    expect(overflowing.ok).toBe(false);
    expect(overflowing.summary).toMatch(/hit its reviewed safety bound of 1 iterations while its condition still held/i);
    const never = await runwhile({ step: controlstep("w3", "whileloop", { while: condition }), condition, scopes: scope([{ name: "count", kind: "number", value: 10 }]), outputs: {}, execute, now });
  ...
  it("runs the foreach over the resolved elements with the honest zero match", async () => {
    ...
    const resolved = await runforeach({ step: controlstep("f1", "foreach", { foreach }), foreach, scopes: scope([]), outputs: {}, execute, now, resolveelements: async () => ["r1", "r2", "r3"] });
    expect(resolved.ok).toBe(true);
    expect(resolved.summary).toMatch(/ran 3 iterations/i);
    const empty = await runforeach({ step: controlstep("f2", "foreach", { foreach }), foreach, scopes: scope([]), outputs: {}, execute, now, resolveelements: async () => [] });
    expect(empty.ok).toBe(true);
    expect(empty.summary).toMatch(/matched no element and the foreach ran zero iterations/i);
    await expect(runforeach({ step: controlstep("f3", "foreach", { foreach }), foreach, scopes: scope([]), outputs: {}, execute, now })).rejects.toThrow(/needs the element resolver/i);
  });
  ...
  it("merges the parallel writes under the first, last and fail strategies", () => {
    const branches: Array<{ id: string; order: number; ok: boolean; cancelled: boolean; variables: variablevalue[] }> = [
      { id: "b1", order: 0, ok: true, cancelled: false, variables: [{ name: "rows", kind: "number", value: 12, setat: now }] },
      { id: "b2", order: 1, ok: true, cancelled: false, variables: [{ name: "rows", kind: "number", value: 14, setat: now }, { name: "solo", kind: "string", value: "only", setat: now }] },
      { id: "b3", order: 2, ok: false, cancelled: true, variables: [{ name: "rows", kind: "number", value: 99, setat: now }] },
    ];
  ...
  it("runs the parallel block with the join and the cancel on fail policy", async () => {
    const parallel: parallelstep = { branches: [{ id: "ok", steps: [step("c1")] }, { id: "bad", steps: [step("c2")] }], join: { strategy: "first", onfail: "cancel" } };
    const failing = async (one: workflowstep): Promise<{ ok: boolean; summary: string }> => one.id === "c2" ? { ok: false, summary: "the branch failed" } : { ok: true, summary: "done" };
    const cancelled = await runparallel({ step: controlstep("p1", "parallel", { parallel }), parallel, scopes: scope([]), outputs: {}, execute: failing, now });
    ...
    const continuing = await runparallel({ step: controlstep("p2", "parallel", { parallel: continuingparallel }), parallel: continuingparallel, scopes: scope([]), outputs: {}, execute: failing, now });
  ...
  it("computes the retry backoff under the fixed and exponential shapes", () => {
    const fixed: retrypolicy = { attempts: 3, backoff: { shape: "fixed", base: 100, jitter: 0 }, retryable: ["timeout"] };
    expect(backoffdelay(fixed, 1, 1)).toBe(100);
    expect(backoffdelay(fixed, 3, 1)).toBe(100);
    const exponential: retrypolicy = { attempts: 3, backoff: { shape: "exponential", base: 100, jitter: 0 }, retryable: ["timeout"] };
    ...
    const jittered: retrypolicy = { attempts: 3, backoff: { shape: "fixed", base: 100, jitter: 50 }, retryable: ["timeout"] };
  ...
  it("retries only the reviewed error classes and reports the exhaustion", async () => {
    const policy: retrypolicy = { attempts: 3, backoff: { shape: "fixed", base: 0, jitter: 0 }, retryable: ["timeout"] };
    let calls = 0;
    const retried = await applyretry({ stepid: "s1", policy, run: async () => { calls += 1; return calls < 3 ? { ok: false } : { ok: true }; }, errorclass: () => "timeout", now });
    expect(retried.value.ok).toBe(true);
    expect(retried.attempts).toHaveLength(2);
    expect(retried.exhausted).toBe(false);
    let nonretryable = 0;
    const refused = await applyretry({ stepid: "s1", policy, run: async () => { nonretryable += 1; return { ok: false }; }, errorclass: () => "consent", now });
    expect(nonretryable).toBe(1);
    expect(refused.exhausted).toBe(false);
    let exhausted = 0;
    const drained = await applyretry({ stepid: "s1", policy, run: async () => { exhausted += 1; return { ok: false }; }, errorclass: () => "timeout", now });
  ...
  it("aborts the step and the run against their reviewed budgets", async () => {
    ...
    if (!runaborted.cancelled || runaborted.error === undefined) throw new Error("The run budget abort carries its cancellation error.");
  ...
  it("runs the try block through the catch handler and the rerun option", async () => {
    const failing = async (one: workflowstep): Promise<{ ok: boolean; summary: string; details?: Record<string, unknown> }> => one.id === "c1" ? { ok: false, summary: "the fragile step failed", details: { errorclass: "page" } } : { ok: true, summary: `done ${one.id}` };
    const handledfragile = { steps: [step("c1")], catch: { steps: [step("h1")] } };
    const handled = await runtry({ step: controlstep("t1", "trycatch", { try: handledfragile }), fragile: handledfragile, scopes: scope([]), outputs: {}, execute: failing, now });
    ...
    const rerun = await runtry({ step: controlstep("t2", "trycatch", { try: rerunfragile }), fragile: rerunfragile, scopes: scope([]), outputs: {}, execute: async (one: workflowstep) => {
      if (one.id === "c1") {
        c1runs += 1;
    ...
    const failinghandler = await runtry({ step: controlstep("t3", "trycatch", { try: failingfragile }), fragile: failingfragile, scopes: scope([]), outputs: {}, execute: async () => ({ ok: false, summary: "always fails" }), now });
  ...
  it("dispatches the control step through runcontrolstep with the merged scopes and log", async () => {
    const dispatched = await runcontrolstep({ step: controlstep("c1", "condition", { condition: { expression: { left: { literal: true }, operator: "not", result: "flag", resultkind: "boolean" } } }), scopes: scope([]), outputs: {}, execute, now });
    ...
    await expect(runcontrolstep({ step: malformed, scopes: scope([]), outputs: {}, execute, now })).rejects.toThrow(/loop step needs a reviewed list variable/i);
    const passthrough = controlstep("p1", "parallel", { parallel: { branches: [{ id: "b1", steps: [step("c1")] }], join: { strategy: "first", onfail: "cancel" } } });
...
describe("torture: trigger payloads, cron grammar and cooldowns", () => {
  ...
  it("normalizes every family payload and refuses the malformed twins", () => {
    ...
    expect(triggerpayloadof("menu", { title: "Run the pricing extraction" })).toEqual({ title: "Run the pricing extraction" });
    ...
    expect(triggerpayloadof("webhook", { secret, schema: [{ name: "rows", kind: "number", required: true }] })).toEqual({ secret, schema: [{ name: "rows", kind: "number", required: true }] });
    expect(triggerpayloadof("webhook", { secret: "short", schema: [{ name: "rows", kind: "number" }] })).toBeUndefined();
    expect(triggerpayloadof("webhook", { secret, schema: [{ name: "rows", kind: "number" }, { name: "rows", kind: "string" }] })).toBeUndefined();
  ...
  it("arms the rules with the reviewed payloads and the default cooldowns", () => {
    ...
    expect(rule).toMatchObject({ kind: "cron", workflowid: "w1", cron: "*/5 * * * *", cooldown: 0, state: { enabled: true }, stats: { fires: 0, launches: 0, suppressions: 0 }, createdat: now });
    expect(rule?.label).toBe("The cron rule of w1");
    const webhook = armrule({ family: "webhook", workflowid: "w1", payload: { secret: "abcdefgh12345678abcdefgh12345678", schema: [{ name: "rows", kind: "number" }] }, now });
    expect(webhook?.cooldown).toBe(defaulttriggercooldown);
    expect(armrule({ family: "event", workflowid: "w1", payload: { events: ["mutate"] }, now })?.cooldown).toBe(defaulttriggercooldown);
    ...
    expect(armrule({ family: "cron", workflowid: "w1", payload: { cron: "* * * * *" }, cooldown: 0, now })).toBeUndefined();
    expect(armrule({ family: "cron", workflowid: "w1", payload: { cron: "* * * * *" }, cooldown: -1, now })).toBeUndefined();
    expect(armrule({ family: "cron", workflowid: "w1", payload: { cron: "* * * * *" }, cooldown: 5, label: "  every minute  ", now })?.label).toBe("  every minute  ");
  });
  ...
  it("parses the cron grammar with names, steps, ranges and the sunday normalization", () => {
    ...
    for (const bad of ["", "* * * *", "* * * * * *", "60 * * * *", "* 24 * * *", "* * 0 * *", "* * 32 * *", "* * * 0 *", "* * * * 8", "* * * * tue-mon", "*/0 * * * *", "a * * * *", "1--2 * * * *"]) {
  ...
  it("schedules the interval fires with the jitter spread and lists the due rules", () => {
    ...
    const rule = { ...armrule({ family: "interval", workflowid: "w1", payload: { period: 1000 }, now })!, state: { enabled: true, cooldown: 0, nextfireat: now - 1 } };
  ...
  it("applies the cooldown window with the exact boundary kept", () => {
    const rule = { ...armrule({ family: "interval", workflowid: "w1", payload: { period: 1000 }, now })!, state: { enabled: true, cooldown: 100, lastfireat: now } };
  ...
  it("evaluates the triggers through the suppression gates and queues the fires", () => {
    ...
    expect(evaluatetrigger({ rule, now, cause: "interval", runactive: false, workflowreviewed: true })).toMatchObject({ fired: true });
    expect(evaluatetrigger({ rule: updaterule(rule, { state: { enabled: false } }), now, cause: "c", runactive: false, workflowreviewed: true }).suppressed).toBe("disabled");
    expect(evaluatetrigger({ rule: updaterule(rule, { state: { pausedat: now } }), now, cause: "c", runactive: false, workflowreviewed: true }).suppressed).toBe("paused");
    expect(evaluatetrigger({ rule, now, cause: "c", runactive: false, workflowreviewed: false }).suppressed).toBe("unreviewed");
    expect(evaluatetrigger({ rule, now, cause: "c", runactive: true, workflowreviewed: true }).suppressed).toBe("dedupe");
    const cooled = updaterule(rule, { state: { lastfireat: now, cooldown: 100 } });
    expect(evaluatetrigger({ rule: cooled, now: now + 50, cause: "c", runactive: false, workflowreviewed: true })).toMatchObject({ fired: false, suppressed: "cooldown", remaining: 50 });
    const fired = evaluatetrigger({ rule, now: now + 1, cause: "visit", url: `${origin}/pricing`, title: "Pricing", payload: { rows: 12 }, runactive: false, workflowreviewed: true });
    expect(fired.fire).toMatchObject({ ruleid: rule.id, at: now + 1, cause: "visit", url: `${origin}/pricing`, title: "Pricing" });
  ...
  it("drains the queued fires and keeps the failed one for the next wake", async () => {
    ...
    let launched = 0;
    const drained = await drainqueue(fires, async () => { launched += 1; if (launched === 1) throw new Error("the launch failed"); });
    expect(launched).toBe(1);
    expect(drained.remaining.map(fire => fire.id)).toEqual(["f1", "f2"]);
    const done = await drainqueue(fires, async () => { launched += 1; });
  ...
  it("verifies the webhook deliveries against the secret and the schema", () => {
    const secret = "abcdefgh12345678abcdefgh12345678";
    const rule = armrule({ family: "webhook", workflowid: "w1", payload: { secret, schema: [{ name: "rows", kind: "number", required: true }, { name: "note", kind: "string" }] }, now })!;
    expect(verifywebhook({ rule, secret, payload: { rows: 12 } })).toEqual({ verified: true });
    expect(verifywebhook({ rule, secret: "wrong secret 1234567890123456", payload: { rows: 12 } }).reason).toMatch(/does not match the reviewed secret/i);
    expect(verifywebhook({ rule, secret, payload: { note: "x" } }).reason).toMatch(/required webhook field rows is missing/i);
    ...
    expect(verifywebhook({ rule: rulewithoutsecret, secret, payload: {} }).reason).toMatch(/carries no reviewed secret/i);
  });
  ...
  it("observes the event rules, suspends and resumes the fleet", () => {
    ...
    expect(observeevents([rule, armrule({ family: "cron", workflowid: "w2", payload: { cron: "* * * * *" }, now })!], "mutate")).toEqual([rule]);
  ...
  it("previews the manual run and records the confirmation", () => {
    const record = compose([step("s1"), controlstep("l1", "loop", { loop: { list: "items", item: "it", index: "ix", bound: 4, steps: [step("c1")] } })]);
  ...
  it("summarizes the rules and checks the origins against the workflow grants", () => {
    const rule = armrule({ family: "visit", workflowid: "w1", payload: { origins: [origin, "https://other.example"] }, now })!;
    expect(triggersummary(rule)).toMatchObject({ kind: "visit", workflowid: "w1", origins: [origin, "https://other.example"], cooldown: 0, enabled: true });
    ...
    expect(ruleoriginsgranted(armrule({ family: "menu", workflowid: "w1", payload: { title: "t" }, now })!, [])).toBe(true);
  });

  it("plans one fire per url of the url list rules", () => {
    const rule = armrule({ family: "urllist", workflowid: "w1", payload: { urls: [origin, "https://other.example"] }, now })!;
    const fires = runurllist(rule, now + 1);
    expect(fires.map(fire => fire.url)).toEqual([origin, "https://other.example"]);
    expect(fires.every(fire => fire.cause === "urllist" && fire.ruleid === rule.id && fire.at === now + 1)).toBe(true);
...
describe("torture: workflow editor parsing, files and canvas rules", () => {
  const record = (): workflowrecord => compose([step("s1"), step("s2"), controlstep("l1", "loop", { loop: { list: "items", item: "it", index: "ix", bound: 3, steps: [step("c1")] } })]);
  ...
  it("builds the step library over the kinds grouped by category", () => {
    ...
    expect(library.find(entry => entry.kind === "delay")?.optionschema[0]).toMatchObject({ name: "base", kind: "number", required: true });
  ...
  it("refuses the node that is both a step and an invocation and the duplicate ids", () => {
    ...
    expect(() => saveworkflow({ ...model, nodes: [both] }, { now })).toThrow(/exactly one workflow step or one block invocation/i);
  ...
  it("refuses the edges that run backwards or reference unknown steps", () => {
    const model = loadworkflow(record());
    expect(() => saveworkflow({ ...model, edges: [{ from: "s2", to: "s1", variable: "rows", kind: "number" }] }, { now })).toThrow(/would form a cycle/i);
    expect(() => saveworkflow({ ...model, edges: [{ from: "ghost", to: "s1", variable: "rows", kind: "number" }] }, { now })).toThrow(/unknown source step ghost/i);
    expect(() => saveworkflow({ ...model, edges: [{ from: "s1", to: "ghost", variable: "rows", kind: "number" }] }, { now })).toThrow(/unknown target step ghost/i);
  });
  ...
  it("snaps nodes to the grid, refuses the broken grids and the invocation nodes", () => {
    ...
    const invocationmodel = loadworkflow(composeworkflow({ name: "n", version: 1, origins: [origin], steps: [{ block: "b", label: "b" }], blocks: [{ name: "b", label: "b", steps: [step("i1")] }], now }));
  ...
  it("inserts templates and steps with id deduplication and refuses the malformed ones", () => {
    ...
    expect(() => expandtemplate(model, { ...template, step: { ...step("s1"), kind: "WAIT" as never } })).toThrow(/does not carry one reviewed workflow step/i);
    expect(() => expandtemplate(model, template, [{ name: "Notvalid", kind: "string" } as never])).toThrow(/malformed nested parameter/i);
  ...
  it("edits the step payloads through the inspector and refuses the unknown steps", () => {
    ...
    expect(() => editstep(model, { ...step("s1"), kind: "WAIT" as never })).toThrow(/needs one reviewed workflow step/i);
  });
  ...
  it("marks the breakpoints on the canvas and in the blocks and refuses the unknown", () => {
    ...
    const blockmodel = loadworkflow(composeworkflow({ name: "n", version: 1, origins: [origin], steps: [{ block: "b", label: "b" }], blocks: [{ name: "b", label: "b", steps: [step("i1")] }], now }));
  ...
  it("segments the debug runs at the breakpoints and reports the remaining steps", () => {
    ...
    expect(runtobreakpoint({ record: withbreakpoint, breakpoints: [] })).toMatchObject({ until: 1, pausat: "s2", remaining: 2 });
    expect(runtobreakpoint({ record: withbreakpoint, breakpoints: ["s3"] })).toMatchObject({ until: 1, pausat: "s2", remaining: 2 });
    expect(runtobreakpoint({ record: record(), breakpoints: ["s9"] })).toMatchObject({ until: 3, pausat: undefined, remaining: 0 });
  ...
  it("diffs the workflow versions with the added, removed and changed steps", () => {
    const from = record();
    const to = compose([step("s1", { value: "20" }), step("s3"), controlstep("l1", "loop", { loop: { list: "items", item: "it", index: "ix", bound: 9, steps: [step("c1")] } })]);
  ...
  it("roundtrips the export and import in json and yaml with the format version gate", () => {
    ...
    for (const format of ["json", "yaml"] as const) {
      ...
      const imported = importworkflow({ contents: exported.contents, format, now, kindallowed: () => true, riskof: () => "read" });
    ...
    const auto = importworkflow({ contents: exportworkflow(source, "json").contents, now, kindallowed: () => true, riskof: () => "read" });
  ...
  it("refuses the malformed import documents of every shape", () => {
    ...
    expect(() => importworkflow({ contents: JSON.stringify({ format: 99, exportedat: now, workflow: { steps: [] }, templates: [] }), format: "json", now })).toThrow(/not the reviewed format 1/i);
    expect(() => importworkflow({ contents: JSON.stringify({ format: 1, exportedat: now, workflow: { steps: [] }, templates: [] }), format: "json", now })).toThrow(/at least one step/i);
    const steps = [step("s1")];
    const minimal = (workflow: Record<string, unknown>): string => JSON.stringify({ format: 1, exportedat: now, workflow, templates: [] });
    expect(() => importworkflow({ contents: minimal({ name: "n", version: 1, origins: [origin], steps: [{ ...steps[0]!, kind: "WAIT" }] }), format: "json", now })).toThrow(/reviewed step/i);
    expect(() => importworkflow({ contents: JSON.stringify({ format: 1, exportedat: now, workflow: { name: "n", version: 1, origins: [origin], steps }, templates: "nope" }), format: "json", now })).toThrow(/templates .* must be a list/i);
    expect(() => importworkflow({ contents: JSON.stringify({ format: 1, exportedat: now, workflow: { name: "n", version: 1, origins: [origin], steps }, templates: [{ id: "t", name: "n", origin, step: { ...steps[0]!, kind: "WAIT" }, sharedat: now }] }), format: "json", now })).toThrow(/does not carry one reviewed step/i);
  ...
  it("shares the workflow with the packed templates and imports them back", () => {
    ...
    const imported = importworkflow({ contents: shared.contents, format: "yaml", now, kindallowed: () => true, riskof: () => "read" });
  ...
  it("wires the nested parameters into the block invocations", () => {
    const model = loadworkflow(composeworkflow({ name: "n", version: 1, origins: [origin], steps: [{ block: "b", label: "b", params: [{ name: "depth", kind: "number" }] }], blocks: [{ name: "b", label: "b", steps: [step("i1")] }], now }));
  ...
  it("applies the site overrides to the matching knobs only", () => {
    const override: siteoverride = { id: "o1", workflowid: record().id, pattern: origin, deltas: { loopbound: 9 }, createdat: now };
    const withloop = compose([step("s1"), controlstep("l1", "loop", { loop: { list: "items", item: "it", index: "ix", bound: 3, steps: [step("c1")] } })]);
    ...
    expect(() => applyoverride(withloop, { ...override, pattern: "https://denied.example" })).toThrow(/matches none of the workflow origins/i);
    expect(() => applyoverride(withloop, { ...override, deltas: { unknown: 1 } })).toThrow(/not one of the reviewed knobs/i);
    expect(() => applyoverride(withloop, { ...override, deltas: { loopbound: 0 } })).toThrow(/must be a positive number/i);
    expect(() => applyoverride(withloop, { ...override, deltas: { loopbound: -1 } })).toThrow(/must be a positive number/i);
    expect(() => applyoverride(withloop, { ...override, deltas: { loopbound: NaN } })).toThrow(/must be a positive number/i);
  ...
  it("adds, removes and refuses the binding edges of the canvas", () => {
    ...
    expect(() => addedge(model, { from: "s2", to: "s1", variable: "rows", kind: "number" })).toThrow(/would run backwards/i);
    expect(() => addedge(model, { from: "ghost", to: "s2", variable: "rows", kind: "number" })).toThrow(/unknown source step ghost/i);
    expect(() => addedge(model, { from: "s1", to: "s2", variable: "Bad Name", kind: "number" })).toThrow(/lowercase word/i);
