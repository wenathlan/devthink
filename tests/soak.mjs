...
Date.now = () => clocknow;
function resetclock() { clocknow = epoch; }
function tick() { clocknow += steptick; }
...
function rowof(index) { return `row-${String(index + 1).padStart(2, "0")}`; }

/** Reads the deterministic cell count the fixture page holds for one row id. */
function cellsof(rowid) { return Number(rowid.slice(4)); }
...
function passstep(library, id, list) {
  const parsed = library.workflowstepof({
    ...
    options: JSON.stringify({ loop: { list, item: "row", index: "rowindex", bound: rowsperpass, steps: [
      { id: "readrow", kind: "readtext", label: "Read the pricing row", target: `tr[data-row="\${row}"]`, value: "The pricing row \${row} holds 7 cells at 9 usd.", extract: { pattern: "row (?<rowid>row-\\d\\d) holds (?<cells>\\d+) cells", flags: "", groups: ["rowid", "cells"] } },
    ] } }),
  });
...
function countstep(library, id, result, leftref) {
  const parsed = library.workflowstepof({ id, kind: "compute", label: `Count the rows of the pass`, expression: { left: { ref: leftref }, right: { literal: 1 }, operator: "add", result, resultkind: "number" } });
...
function soakworkflow(library, policy) {
  ...
  return library.composeworkflow({
    ...
    steps: [
      ...
      { id: "subtotal", kind: "compute", label: "Sum the first two passes", expression: { left: { ref: "passonecount" }, right: { ref: "passtwocount" }, operator: "add", result: "subtotal", resultkind: "number" } },
      { id: "totalrows", kind: "compute", label: "Sum every pass", expression: { left: { ref: "subtotal" }, right: { ref: "passthreecount" }, operator: "add", result: "totalrows", resultkind: "number" } },
    ],
    now: epoch,
    kindallowed,
    riskof,
...
function soakscopes(library) {
  return [{ name: "root", variables: [0, 1, 2].map(pass => ({ name: `${["passone", "passtwo", "passthree"][pass]}rows`, kind: "list", value: Array.from({ length: rowsperpass }, (_, index) => rowof(pass * rowsperpass + index)), setat: epoch })) }];
}
...
function soakexecutor(library, memorymodule, context) {
  return async step => {
    ...
    if (step.kind === "readtext") {
      ...
      await context.store.addaudi({ id: `soak-audit-${context.rowcounter}`, kind: "scrape", at: clocknow, summary: `The pricing row ${rowid} read its ${cellsof(rowid)} cells from the fixture page.`, stepid: "readrow" });
      context.log = await library.appendlogentry({ log: context.log, id: `soak-entry-${context.rowcounter}`, kind: "step", summary: `The pricing row ${rowid} of the fixture page read ${cellsof(rowid)} cells at 9 usd.`, origin: fixtureorigin, stepid: "readrow", at: clocknow });
      await context.store.setmemoryitem(library.memoryitemof({ key: `pricingrow-${rowid}`, value: { cells: cellsof(rowid), price: 9 }, provenance: library.provenanceof({ origin: fixtureorigin, runid: "soakrun", stepid: "readrow", now: clocknow }) }));
    }
    return { ok: true, summary: step.kind === "readtext" ? "The pricing row read its cells from the fixture page." : `The ${step.id} step computed its rows.` };
  };
}
...
async function opensoakrun(library, memorymodule) {
  ...
  const context = {
    ...
    state: library.openrun({ runid: "soakrun", sessionid: "soak-session", planid: "soak-plan", profileid: "soak-profile", interval: steptick, now: clocknow }),
    heartbeat: undefined,
    dispatched: [],
    rowcounter: 0,
  };
...
export async function runsoaksuite() {
  if (!existsSync("dist/index.js") || !existsSync("dist/policy.js") || !existsSync("dist/memory.js")) {
    console.error("SOAK The gate reads the compiled library, policy and memory bundles; run node tests/build.mjs before the soak run.");
    return { release, checklist: "docs/soak.md", entries: [{ id: 1, title: "the compiled bundles exist", family: "build", module: "dist", outcome: "fail", detail: "the dist bundles are absent" }], summary: { total: 1, passed: 0, failed: 1 } };
  }
  ...
  try {
    await entry({ id: 1, title: "the retention window loads from the library settings and bounds the stored counts", family: "retention window", module: "memory.js and run.js" }, async () => {
      const store = new memorymodule.sessionmemory(new fakeadapter());
      await store.setsettings({ auditretention: retentionwindow });
      const settings = await store.getsettings();
      check(settings?.auditretention === retentionwindow, `the settings read the audit retention window ${settings?.auditretention} instead of ${retentionwindow}`);
      for (let index = 1; index <= retentionwindow + 2; index += 1) await store.addaudi({ id: `probe-${index}`, kind: "workflow", at: epoch + index * steptick, summary: `The probe event ${index} of the retention window check.` });
      const audit = await store.getaudit();
      check(audit.length === retentionwindow, `the audit trail kept ${audit.length} events instead of the retention window of ${retentionwindow}`);
      check(audit[0]?.id === `probe-${retentionwindow + 2}`, `the retention window dropped the newest event: ${audit[0]?.id}`);
      const openstore = new memorymodule.sessionmemory(new fakeadapter());
      await openstore.addaudi({ id: "probe-open", kind: "workflow", at: epoch, summary: "The probe event of the absent retention window." });
      ...
      check(window === library.roadmapheartbeatwindow && window === 60_000, `the absent heartbeat setting answered ${window} instead of the documented roadmap window of 60000`);
      check(library.heartbeatwindowof({ heartbeatwindow: 30_000 }) === 30_000, "the configured heartbeat window never overrode the documented default");
    ...
    await entry({ id: 2, title: "the long workflow composes and validates against the frozen kind catalog", family: "composition", module: "workflow.js and policy.js" }, async () => {
      const record = full.record;
      check(record.steps.length === 2 + passes * 2, `the soak workflow carries ${record.steps.length} steps instead of ${2 + passes * 2}`);
      check(record.risk === "interaction", `the soak workflow graded ${record.risk} instead of the interaction risk of the loop passes`);
      const verdict = library.validateworkflow(record, { kindallowed: kind => policy.actionkindcatalog().includes(kind), inputs: ["rowindex", "passonecount", "passtwocount", "passthreecount", "subtotal"] });
      ...
      check(summary?.kind === "loop" && summary?.bound === rowsperpass && summary?.list === "passonerows", `the first pass carried ${JSON.stringify(summary)} instead of the loop summary with its bound`);
      const catalog = policy.actionkindcatalog();
      for (const step of record.steps) {
        check(catalog.includes(step.kind), `the step ${step.id} carries the kind ${step.kind} outside the frozen catalog`);
        for (const child of library.controlsteps(step)) check(catalog.includes(child.kind), `the child step ${child.id} carries the kind ${child.kind} outside the frozen catalog`);
      }
    ...
    await entry({ id: 3, title: "the soak run stays alive across the full retention window", family: "liveness", module: "run.js and workflow.js" }, async () => {
      const { result, context } = full;
      check(result.run.state === "done", `the soak run ended ${result.run.state} instead of done`);
      check(result.run.cursor === full.record.steps.length, `the soak run cursor stopped at ${result.run.cursor} of ${full.record.steps.length} steps`);
      check(library.resolvevariable(result.scopes, "totalrows")?.value === totalrows, `the total rows variable answered ${library.resolvevariable(result.scopes, "totalrows")?.value} instead of ${totalrows}`);
      check(context.dispatched.length === totalrows + 5, `the executor dispatched ${context.dispatched.length} steps instead of ${totalrows + 5}`);
      check(context.rowcounter === totalrows, `the run read ${context.rowcounter} pricing rows instead of ${totalrows}`);
      check(context.state.keepalive.state === "active" && context.state.state === "active", `the run state ended ${context.state.state} with its keepalive ${context.state.keepalive.state} instead of active`);
      check(context.state.keepalive.beats === context.dispatched.length, `the keepalive emitted ${context.state.keepalive.beats} beats instead of one per dispatched step`);
      const window = library.heartbeatwindowof(undefined);
      check(library.heartbeatisstale({ heartbeat: context.heartbeat, now: clocknow + window - steptick, window }) === false, "the last heartbeat already sat stale inside the documented window");
      check(library.heartbeatisstale({ heartbeat: context.heartbeat, now: clocknow + window + steptick, window }) === true, "the heartbeat never turned stale past the documented window");
      const swept = library.zombiesweep({ states: [context.state], now: clocknow, interval: steptick, missedlimit: 3 });
      check(swept.reaped.length === 0, `the zombie reaper took the live soak run: ${swept.reaped.join(", ")}`);
      const silent = library.openrun({ runid: "soak-silent", sessionid: "soak-session", planid: "soak-plan", profileid: "soak-profile", interval: steptick, now: epoch });
      const discriminantnow = clocknow + steptick * 5;
      const discriminant = library.zombiesweep({ states: [library.beatrun(context.state, discriminantnow), silent], now: discriminantnow, interval: steptick, missedlimit: 3 });
      check(discriminant.reaped.join(",") === "soak-silent", `the zombie reaper answered ${discriminant.reaped.join(", ")} instead of the silent run alone`);
      const frozen = library.closerun(library.beatrun(context.state, clocknow), clocknow);
      check(frozen.state === "completed" && frozen.keepalive.state === "stopped" && frozen.keepalive.portopen === false, "the completed run never froze its keepalive port");
    ...
    await entry({ id: 4, title: "the checkpoint mid-run resumes and completes identically", family: "resume", module: "run.js and workflow.js" }, async () => {
      const segmented = await runsoaksegmented(library, policy, memorymodule);
      const { record, partial, resumed, checkpoint } = segmented;
      check(partial.run.cursor === checkpoint.boundary, `the segment stopped at cursor ${partial.run.cursor} instead of the boundary ${checkpoint.boundary}`);
      check(checkpoint.recovery.recoverable === true && checkpoint.recovery.pendingstepid === record.steps[checkpoint.boundary]?.id, `the recovery plan answered ${JSON.stringify(checkpoint.recovery)} instead of the pending step of the boundary`);
      const digest = library.runcachedigest({ runid: "soakrun", resource: fixturepage });
      const recordcheckpoint = library.makecheckpoint({ runid: "soakrun", stepid: record.steps[checkpoint.boundary - 1]?.id ?? "", completed: record.steps.slice(0, checkpoint.boundary).map(step => step.id), digest, now: clocknow });
      check(library.checkpointisvalid({ checkpoint: recordcheckpoint, runid: "soakrun", digest }) === true, "the checkpoint of the boundary never validated against its run and page digest");
      check(library.checkpointisvalid({ checkpoint: recordcheckpoint, runid: "soakrun", digest: "0".repeat(64) }) === false, "the checkpoint validated against a changed page digest");
      const remaining = library.resumecheckpoint({ checkpoint: recordcheckpoint, steps: record.steps.map(step => step.id) });
      check(remaining.join(",") === record.steps.slice(checkpoint.boundary).map(step => step.id).join(","), `the resume answered ${remaining.join(", ")} instead of the steps past the checkpoint`);
      check(resumed.run.state === "done" && resumed.run.cursor === record.steps.length, `the resumed run ended ${resumed.run.state} at cursor ${resumed.run.cursor}`);
      check(library.resolvevariable(resumed.scopes, "totalrows")?.value === totalrows, "the resumed run never recomputed the total rows");
      const uninterrupted = JSON.stringify({ run: full.result.run, log: full.result.log, outputs: full.result.outputs });
      const resumedoutcome = JSON.stringify({ run: resumed.run, log: resumed.log, outputs: resumed.outputs });
      check(uninterrupted === resumedoutcome, `the resumed outcome drifted from the uninterrupted outcome: the first difference sits at ${firstdifference(uninterrupted, resumedoutcome)}`);
    ...
    await entry({ id: 5, title: "the audit trail hash chain verifies end to end", family: "audit trail", module: "security.js" }, async () => {
      const { sealed } = full;
      check(sealed.log.entries.length === totalrows, `the immutable log carries ${sealed.log.entries.length} entries instead of the ${totalrows} rows`);
      ...
      check(read.ok === true && read.entries.length === totalrows, `the verified read refused the sealed log: ${read.reason}`);
      const exportbundle = await library.exportlogchain(sealed.log);
      check(exportbundle.chainvalid === true && exportbundle.entries === totalrows, `the chain export refused the sealed log: ${exportbundle.reason}`);
      check(exportbundle.sealhash === sealed.seal.sealhash.current, "the chain export lost the seal hash of the completion");
      ...
      check(tamperverification.valid === false && tamperverification.brokenat === 3, "the tampered entry survived the chain verification");
      let refused = false;
      try { await library.appendlogentry({ log: sealed.log, kind: "step", summary: "The late append.", origin: fixtureorigin, at: clocknow }); } catch { refused = true; }
    ...
    await entry({ id: 6, title: "repeated soak runs seal the identical audit hash", family: "no drift", module: "workflow.js and security.js" }, async () => {
      ...
      const second = JSON.stringify({ run: repeat.result.run, log: repeat.result.log, outputs: repeat.result.outputs });
      check(first === second, `the repeated soak run drifted: the first difference sits at ${firstdifference(first, second)}`);
      check(full.sealed.seal.sealhash.current === repeat.sealed.seal.sealhash.current, "the repeated soak run sealed a different audit hash");
      check(library.resolvevariable(repeat.result.scopes, "totalrows")?.value === totalrows, "the repeated soak run never recomputed the total rows");
      const firstentries = full.sealed.log.entries;
      const secondentries = repeat.sealed.log.entries;
      for (let index = 0; index < totalrows; index += 1) check(firstentries[index]?.hash.current === secondentries[index]?.hash.current, `the log entry ${index} drifted between the two runs`);
    ...
    await entry({ id: 7, title: "the memory items keep their provenance inside the retention bounds", family: "memory", module: "memory.js" }, async () => {
      const { context } = full;
      const items = await context.store.getmemoryitems();
      check(items.length === totalrows, `the memory store holds ${items.length} items instead of the ${totalrows} rows`);
      for (const item of items) {
        check(item.provenance.origin === fixtureorigin && item.provenance.runid === "soakrun" && item.provenance.stepid === "readrow", `the item ${item.key} drifted from the soak provenance: ${JSON.stringify(item.provenance)}`);
      }
      const audit = await context.store.getaudit();
      check(audit.length === retentionwindow, `the audit trail of the soak run kept ${audit.length} events instead of the retention window of ${retentionwindow}`);
      const plansteps = [{ id: "readrow", kind: "readtext", summary: "Read the pricing row of the fixture page.", risk: "read" }];
      ...
      check(library.matchingrule(rules, "pricingrow-row-01")?.lifetime === expirywindowms, "the expiry rule never matched the pricing row keys");
      const expired = library.expireditems(items, rules, clocknow);
      check(expired.length > 0 && expired.length < items.length, `the expiry pass answered ${expired.length} expired items of ${items.length}`);
      const purge = library.purgeitems({ items, rules, confirmed: true, now: clocknow });
      check(purge.kept.length <= retentionwindow, `the purge kept ${purge.kept.length} items outside the retention window of ${retentionwindow}`);
      check(purge.purged.length === expired.length, `the purge answered ${purge.purged.length} outcomes instead of the ${expired.length} expired items`);
      for (const outcome of purge.purged) {
        check(outcome.provenance.origin === fixtureorigin && outcome.provenance.runid === "soakrun" && outcome.provenance.stepid === "readrow", `the purge outcome of ${outcome.key} lost the soak provenance`);
        await context.store.addpurgesummary({ key: outcome.key, summary: outcome.summary, provenance: outcome.provenance, at: outcome.at });
      }
      await context.store.removememoryitems(purge.purged.map(outcome => outcome.key));
      const kept = await context.store.getmemoryitems();
      check(kept.length === purge.kept.length, `the store holds ${kept.length} items after the purge instead of ${purge.kept.length}`);
      const summaries = await context.store.listpurgesummaries();
      check(summaries.length === purge.purged.length, `the store holds ${summaries.length} purge summaries instead of ${purge.purged.length}`);
      const bundle = library.auditexportof({ runs: [{ runid: "soakrun", planid: "soak-plan", sessionid: "soak-session", state: "completed", createdat: epoch, updatedat: clocknow }], items: kept, rules, timeline: [], locks: [], now: clocknow });
      const chunks = library.exportchunks(bundle, 400);
      const reassembled = JSON.parse(chunks.map(chunk => chunk.payload).join(""));
      check(JSON.stringify(reassembled) === JSON.stringify(bundle), "the audit export never round tripped through its chunks");
    ...
    await entry({ id: 8, title: "every browser kind keeps the run alive on its fake tab", family: "browser matrix", module: "run.js" }, async () => {
      const window = library.heartbeatwindowof(undefined);
      for (const browser of faketabprovider.kinds) {
        const tabid = faketabprovider.tabof(browser, 1);
        let state = library.openrun({ runid: `soak-${browser}`, sessionid: "soak-session", planid: "soak-plan", profileid: "soak-profile", interval: steptick, now: epoch });
        for (let beat = 0; beat < retentionwindow; beat += 1) { tick(); state = library.beatrun(state, clocknow); }
        const isolated = library.tabisolate({ tabid, config: {}, runid: `soak-${browser}`, now: clocknow });
        check(library.tabnamespace(tabid) === `tab:${tabid}`, `the tab namespace of ${browser} answered ${library.tabnamespace(tabid)}`);
        ...
        check(live.reaped.length === 0, `the zombie reaper took the beating ${browser} run: ${live.reaped.join(", ")}`);
        const silent = library.openrun({ runid: `soak-${browser}-silent`, sessionid: "soak-session", planid: "soak-plan", profileid: "soak-profile", interval: steptick, now: epoch });
        tick(); tick(); tick(); tick(); tick();
        state = library.beatrun(state, clocknow);
        const discriminant = library.zombiesweep({ states: [state, silent], now: clocknow, interval: steptick, missedlimit: 3 });
        check(discriminant.reaped.join(",") === `soak-${browser}-silent`, `the zombie reaper answered ${discriminant.reaped.join(", ")} on ${browser} instead of the silent run alone`);
        const heartbeat = library.runheartbeat({ runid: `soak-${browser}`, now: clocknow });
        check(library.heartbeatisstale({ heartbeat, now: clocknow + window - steptick, window }) === false, `the ${browser} heartbeat sat stale inside the window`);
      }
      const exportbundle = library.exportrunstate([full.context.state], clocknow);
      check(exportbundle.runs === 1 && exportbundle.beats === full.context.state.keepalive.beats, `the run state export answered ${JSON.stringify(exportbundle)}`);
    ...
    await entry({ id: 9, title: "the soak report records every check and the exit answers the failures", family: "report", module: "soak.mjs" }, async () => {
      ...
      check(summary.failed === 0, `${summary.failed} soak check${summary.failed === 1 ? "" : "s"} failed before the report landed`);
  ...
  return {
    ...
    soak: { retentionwindow, passes, rows: totalrows, steps: 2 + passes * 2, dispatched: full.context.dispatched.length, logentries: full.sealed.log.entries.length, heartbeatwindow: library.heartbeatwindowof(undefined) },
    entries: executed,
...
function firstdifference(one, two) {
  for (let index = 0; index < Math.max(one.length, two.length); index += 1) if (one[index] !== two[index]) return String(index);
  return "the end";
}
