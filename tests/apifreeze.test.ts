...
describe("the protocolv2 api freeze of 1.1.91", () => {
  ...
  it("gates the negotiation and the unknown fields by the negotiated major", () => {
    expect(protocolnegotiationgate({})).toEqual({
      allowed: true,
      major: 2,
      reason: expect.stringContaining("protocolv2 default"),
    });
    ...
    expect(protocolnegotiationgate({ client: "3.0.0" }).reason).toContain(
      "supported protocol versions are 2 through 2",
    );
  ...
  it("freezes the response envelope, the error code table and the framing rules", () => {
    ...
    expect(errorcodetable.map((entry) => entry.code)).toEqual([
      "parse",
      "method",
      "params",
      "internal",
  ...
  it("accepts the versioned memory item envelope and refuses a foreign version", () => {
    ...
    expect(() => memoryitemframe({ version: "0.0.1", key: "k", value: 1, provenance })).toThrow(
  ...
  it("matches the manifest permissions with the capmanifest declarations", async () => {
    ...
    const declared = new Set([
      ...capmanifestof("background").permissions,
      ...capmanifestof("sidepanel").permissions,
      ...capmanifestof("popup").permissions,
      ...capmanifestof("mcp").permissions,
    ]);
  ...
  it("flags capability drift between two capability manifests", () => {
    ...
    const widened = {
      ...background,
      messages: [...background.messages, "newmessage"],
      permissions: background.permissions.slice(0, -1),
    };
  ...
  it("verifies the schema hash artifact stays stable", async () => {
    ...
    for (const file of schemafiles) {
      const digest = createHash("sha256")
        .update(await readFile(`dist/schemas/${file}`))
        .digest("hex");
