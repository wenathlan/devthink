...
describe("policy", () => {
  ...
  it("blocks action before plan approval and before page bridge invocation", () => {
    expect(
      canexecute({ session, plan: { ...plan, state: "pending" }, step, tabid: 4, origin: "https://example.com", now }),
    ).toMatchObject({ allowed: false });
  });

  it("blocks cross-origin navigation and expired sessions", () => {
    const navigation: toolstep = {
      id: "next",
      kind: "navigate",
      value: "https://other.example",
      summary: "Navigate away",
      risk: "sensitive",
    };
    expect(canexecute({ session, plan, step: navigation, tabid: 4, origin: "https://example.com", now })).toMatchObject(
      { allowed: false },
    );
    expect(
      canexecute({
  ...
  it("allows a target preview only while the matching session and reviewed plan remain current", () => {
    expect(
      canpreview({ session, plan: { ...plan, state: "pending" }, step, tabid: 4, origin: "https://example.com", now }),
    ).toEqual({ allowed: true });
    expect(
      canpreview({
        session,
        plan,
        step: { id: "missing", kind: "click", summary: "Preview an absent target.", risk: "sensitive" },
        tabid: 4,
        origin: "https://example.com",
        now,
      }),
  ...
  it("blocks every execution and preview while the session is paused", () => {
    ...
    expect(canexecute({ session: paused, plan, step, tabid: 4, origin: "https://example.com", now })).toMatchObject({
      allowed: false,
  ...
  it("validates the target-based actions and keeps wait durations unbounded", () => {
    expect(
      canexecute({
        ...
        step: {
          ...
          target: "#country",
          value: "br",
          summary: "Select the reviewed country.",
          risk: "sensitive",
        },
        tabid: 4,
        origin: "https://example.com",
    ...
    expect(
      canexecute({
        session,
        plan,
        step: { id: "s2", kind: "select", target: "#country", summary: "Select without a value.", risk: "sensitive" },
        tabid: 4,
        origin: "https://example.com",
        now,
      }),
    ).toMatchObject({ allowed: false, reason: "A reviewed option value is required." });
    expect(
      canexecute({
        session,
        plan,
        step: { id: "s3", kind: "scroll", summary: "Scroll without a target.", risk: "interaction" },
        tabid: 4,
        origin: "https://example.com",
        now,
      }),
    ).toMatchObject({ allowed: false, reason: "A page target is required." });
    expect(waitduration({ id: "w1", kind: "wait", value: "999999", summary: "Wait a long time.", risk: "read" })).toBe(
      999_999,
    );
    ...
    expect(
      canexecute({
        session,
        plan,
        step: { id: "w3", kind: "wait", value: "abc", summary: "Wait an invalid duration.", risk: "read" },
        tabid: 4,
        origin: "https://example.com",
        now,
      }),
    ).toMatchObject({ allowed: false, reason: expect.stringContaining("Wait duration") });
  });
  ...
  it("validates the reviewed options grammar of the new kinds", () => {
    expect(
      parseoptions({
        ...
        kind: "setattribute",
        target: "#box",
        options: '{"name":"data-x","value":"1"}',
        summary: "Set an attribute.",
        risk: "sensitive",
      }),
    ).toEqual({ name: "data-x", value: "1" });
    expect(() =>
      parseoptions({
        id: "o2",
        kind: "setattribute",
    ...
    expect(
      canexecute({
        session,
        plan,
        step: { id: "o3", kind: "setattribute", target: "#box", summary: "Set without options.", risk: "sensitive" },
        tabid: 4,
        origin: "https://example.com",
        now,
      }),
    ).toMatchObject({ allowed: false, reason: "A reviewed name is required in options." });
    expect(
      canexecute({
        session,
        plan,
        step: { id: "o4", kind: "presskey", summary: "Press without a key.", risk: "sensitive" },
        tabid: 4,
        origin: "https://example.com",
        now,
      }),
    ).toMatchObject({ allowed: false, reason: "A reviewed value is required." });
    expect(
      canexecute({
        session,
        plan,
        step: { id: "o5", kind: "zoomset", value: "0", summary: "Zoom to zero.", risk: "interaction" },
        tabid: 4,
        origin: "https://example.com",
        now,
      }),
    ).toMatchObject({ allowed: false, reason: "The reviewed zoom must be a positive number." });
    expect(
      canexecute({
        session,
        plan,
        step: { id: "o6", kind: "tabclose", value: "seven", summary: "Close by word.", risk: "sensitive" },
        tabid: 4,
        origin: "https://example.com",
        now,
      }),
    ).toMatchObject({ allowed: false, reason: "A numeric browser id is required." });
    expect(
      canexecute({
        session,
        plan,
    ...
    ).toMatchObject({ allowed: false, reason: "Reviewed width and height numbers are required in options." });
    expect(
      canexecute({
        session,
        plan,
  ...
  it("keeps previews restricted to target-based actions", () => {
    expect(
      canpreview({
        session,
        plan: { ...plan, state: "pending" },
        step: { id: "h1", kind: "hover", target: "#menu", summary: "Preview the reviewed menu.", risk: "interaction" },
        tabid: 4,
        origin: "https://example.com",
        now,
      }),
    ...
    expect(
      canpreview({
        session,
        plan: { ...plan, state: "pending" },
        step: { id: "e1", kind: "extract", summary: "Preview an extraction.", risk: "read" },
        tabid: 4,
        origin: "https://example.com",
        now,
      }),
    ).toMatchObject({ allowed: false, reason: "Only a target-based action can be previewed." });
    expect(
      canpreview({
        session,
        plan: { ...plan, state: "pending" },
        step: { id: "r1", kind: "readgeometry", target: "#card", summary: "Preview the reviewed card.", risk: "read" },
        tabid: 4,
        origin: "https://example.com",
        now,
      }),
...
describe("interaction universe policy", () => {
  ...
  it("validates the reviewed pointpath and speedprofile grammar of movepointer", () => {
    const path: toolstep = {
      id: "m1",
      kind: "movepointer",
      summary: "Travel the reviewed pointer path.",
      risk: "interaction",
      options:
        '{"pointpath":{"start":{"x":10,"y":20},"end":{"x":400,"y":300},"waypoints":[{"x":100,"y":40}],"duration":900},"speedprofile":{"easing":"easeinout","peak":1200,"jitter":24}}',
    };
    expect(gate(path)).toEqual({ allowed: true });
    expect(
      gate({ id: "m2", kind: "movepointer", summary: "Travel without a path.", risk: "interaction" }),
    ).toMatchObject({
      allowed: false,
      reason: "A reviewed pointpath with start and end points is required in options.",
    });
    expect(
      gate({
        id: "m3",
        kind: "movepointer",
        summary: "Travel with a broken end point.",
        risk: "interaction",
        options: '{"pointpath":{"start":{"x":1,"y":2},"end":{"x":"far"}}}',
      }),
    ).toMatchObject({ allowed: false, reason: "The reviewed pointpath needs numeric start and end points." });
    expect(
      gate({
        id: "m4",
        kind: "movepointer",
        summary: "Travel with broken waypoints.",
        risk: "interaction",
        options: '{"pointpath":{"start":{"x":1,"y":2},"end":{"x":3,"y":4},"waypoints":[{"x":5}]}}',
      }),
    ).toMatchObject({ allowed: false, reason: "The reviewed pointpath waypoints must be numeric points." });
    expect(
      gate({
        id: "m5",
        kind: "movepointer",
        summary: "Travel with a negative duration.",
        risk: "interaction",
        options: '{"pointpath":{"start":{"x":1,"y":2},"end":{"x":3,"y":4},"duration":-5}}',
      }),
    ).toMatchObject({
      allowed: false,
    ...
    expect(
      gate({
        id: "m6",
        kind: "movepointer",
        summary: "Travel with an unknown easing.",
        risk: "interaction",
        options: '{"pointpath":{"start":{"x":1,"y":2},"end":{"x":3,"y":4}},"speedprofile":{"easing":"snap"}}',
      }),
    ).toMatchObject({ allowed: false, reason: "The reviewed easing must be linear or easeinout." });
    expect(
      gate({
        id: "m7",
        kind: "movepointer",
        summary: "Travel with a negative peak.",
        risk: "interaction",
        options: '{"pointpath":{"start":{"x":1,"y":2},"end":{"x":3,"y":4}},"speedprofile":{"peak":-1}}',
      }),
    ).toMatchObject({ allowed: false, reason: "The reviewed peak velocity must be zero or a positive number." });
    expect(
      gate({
        id: "m8",
        kind: "movepointer",
        summary: "Travel with a negative jitter.",
        risk: "interaction",
        options: '{"pointpath":{"start":{"x":1,"y":2},"end":{"x":3,"y":4}},"speedprofile":{"jitter":-3}}',
      }),
    ).toMatchObject({
      allowed: false,
  ...
  it("requires the matching targetref mode of the resolution kinds", () => {
    const point: toolstep = {
      id: "c1",
      kind: "clickpoint",
      summary: "Click the reviewed coordinates.",
      risk: "sensitive",
      options: '{"targetref":{"mode":"point","x":120,"y":40}}',
    };
    expect(gate(point)).toEqual({ allowed: true });
    expect(
      gate({ id: "c2", kind: "clickpoint", summary: "Click without a point reference.", risk: "sensitive" }),
    ).toMatchObject({ allowed: false, reason: "A reviewed point target reference is required in options." });
    expect(
      gate({
        id: "c3",
        kind: "clickpoint",
        summary: "Click with the wrong mode.",
        risk: "sensitive",
        options: '{"targetref":{"mode":"selector","selector":"#go"}}',
      }),
    ).toMatchObject({ allowed: false, reason: "A reviewed point target reference is required in options." });
    expect(
      gate({
        id: "t1",
        kind: "clicktext",
        summary: "Click the reviewed text.",
        risk: "interaction",
        options: '{"targetref":{"mode":"text","text":"Sign in"}}',
      }),
    ).toEqual({ allowed: true });
    expect(
      gate({
        id: "t2",
        kind: "clicktext",
        summary: "Click text with the wrong mode.",
        risk: "interaction",
        options: '{"targetref":{"mode":"point","x":1,"y":2}}',
      }),
    ).toMatchObject({ allowed: false, reason: "A reviewed text target reference is required in options." });
    expect(
      gate({
        id: "a1",
        kind: "clickaria",
        summary: "Click the reviewed aria pair.",
        risk: "interaction",
        options: '{"targetref":{"mode":"aria","role":"button","name":"Submit"}}',
      }),
    ).toEqual({ allowed: true });
    expect(
      gate({
        id: "a2",
        kind: "clickaria",
        summary: "Click aria with the wrong mode.",
        risk: "interaction",
        options: '{"targetref":{"mode":"name","name":"Submit"}}',
      }),
    ).toMatchObject({ allowed: false, reason: "A reviewed aria target reference is required in options." });
    expect(
      gate({
        id: "n1",
        kind: "clickname",
        summary: "Click the reviewed accessible name.",
        risk: "interaction",
        options: '{"targetref":{"mode":"name","name":"Email"}}',
      }),
    ).toEqual({ allowed: true });
    expect(
      gate({
        id: "n2",
        kind: "clickname",
        summary: "Click a name with the wrong mode.",
        risk: "interaction",
        options: '{"targetref":{"mode":"text","text":"Email"}}',
      }),
    ).toMatchObject({ allowed: false, reason: "A reviewed name target reference is required in options." });
    expect(
      gate({
        id: "x1",
        kind: "resolvexpath",
        summary: "Resolve the reviewed xpath.",
        risk: "read",
        options: '{"targetref":{"mode":"xpath","xpath":"//button[@id=\'go\']"}}',
      }),
    ).toEqual({ allowed: true });
    expect(
      gate({ id: "x2", kind: "resolvexpath", summary: "Resolve without an xpath reference.", risk: "read" }),
    ).toMatchObject({ allowed: false, reason: "A reviewed xpath target reference is required in options." });
  });
  ...
  it("validates the typing, key and control kinds grammar", () => {
    expect(
      gate({
        ...
        target: "#query",
        value: "devthink",
        summary: "Type slowly into the reviewed field.",
        risk: "sensitive",
        options: '{"delay":120}',
      }),
    ).toEqual({ allowed: true });
    expect(
      gate({
        ...
        target: "#query",
        value: "devthink",
        summary: "Type with a negative delay.",
        risk: "sensitive",
        options: '{"delay":-1}',
      }),
    ).toMatchObject({
    ...
    expect(
      gate({ id: "k3", kind: "typetime", target: "#query", summary: "Type without reviewed text.", risk: "sensitive" }),
    ).toMatchObject({ allowed: false, reason: "A reviewed value is required." });
    expect(
      gate({
        id: "k4",
    ...
    expect(
      gate({
        ...
        target: "#name",
        value: "Ada",
        summary: "Set the reviewed value.",
        risk: "sensitive",
      }),
    ).toEqual({ allowed: true });
    expect(
      gate({
        ...
        target: "#editor",
        value: "draft",
        summary: "Type into the editable region.",
        risk: "sensitive",
      }),
    ).toEqual({ allowed: true });
    expect(
      gate({
        ...
        kind: "keyhold",
        value: "Shift",
        summary: "Hold the reviewed key.",
        risk: "sensitive",
        options: '{"holdid":"shift1","modifiers":["shift"]}',
      }),
    ).toEqual({ allowed: true });
    expect(
      gate({
        ...
        kind: "keyhold",
        value: "Shift",
        summary: "Hold with an empty hold id.",
        risk: "sensitive",
        options: '{"holdid":""}',
      }),
    ).toMatchObject({ allowed: false, reason: "The reviewed hold id must be a non-empty string." });
    expect(
      gate({ id: "k9", kind: "keyrelease", value: "shift1", summary: "Release the held key.", risk: "sensitive" }),
    ).toEqual({ allowed: true });
    expect(
      gate({
        id: "s1",
        kind: "submitsearch",
        target: "#search",
        summary: "Submit the reviewed search.",
        risk: "sensitive",
        options: '{"results":"#results","timeout":4000}',
      }),
    ).toEqual({ allowed: true });
    expect(
      gate({
        ...
        kind: "submitsearch",
        target: "#search",
        summary: "Submit without a results region.",
        risk: "sensitive",
      }),
    ).toMatchObject({ allowed: false, reason: "A reviewed results region selector is required in options." });
    expect(
      gate({
        id: "s3",
        kind: "submitsearch",
        target: "#search",
        summary: "Submit with a negative timeout.",
        risk: "sensitive",
        options: '{"results":"#results","timeout":-2}',
      }),
    ).toMatchObject({
      allowed: false,
      reason: "The submitsearch timeout must be zero or a positive number of milliseconds.",
    });
    expect(
      gate({
        id: "s4",
        kind: "selectmulti",
        target: "#tags",
        summary: "Choose several options.",
        risk: "sensitive",
        options: '{"values":["a","b"]}',
      }),
    ).toEqual({ allowed: true });
    expect(
      gate({ id: "s5", kind: "selectmulti", target: "#tags", summary: "Choose without values.", risk: "sensitive" }),
    ).toMatchObject({ allowed: false, reason: "A reviewed list of option values is required in options." });
    expect(
      gate({
        id: "s6",
        kind: "selectmulti",
        target: "#tags",
        summary: "Choose empty values.",
        risk: "sensitive",
        options: '{"values":[]}',
      }),
    ).toMatchObject({ allowed: false, reason: "A reviewed list of option values is required in options." });
    expect(
      gate({
        ...
        target: "#plan",
        value: "beta",
        summary: "Pick the reviewed radio option.",
        risk: "sensitive",
      }),
    ).toEqual({ allowed: true });
    expect(
      gate({
        ...
        target: "#volume",
        value: "55",
        summary: "Drag the reviewed slider.",
        risk: "sensitive",
      }),
    ).toEqual({ allowed: true });
    expect(
      gate({
        ...
        target: "#volume",
        value: "loud",
        summary: "Drag with a non numeric value.",
        risk: "sensitive",
      }),
    ).toMatchObject({ allowed: false, reason: "The reviewed slider value must be a number." });
    expect(
      gate({
        id: "d1",
        kind: "setdate",
    ...
    expect(
      gate({
        ...
        target: "#start",
        value: "14/07/2026",
        summary: "Set a badly formed date.",
        risk: "sensitive",
      }),
    ).toMatchObject({ allowed: false, reason: "The reviewed date must use the yyyy-mm-dd form." });
    expect(
      gate({
        id: "d3",
        kind: "setcolor",
        target: "#tint",
        value: "#ff00aa",
        summary: "Set the reviewed color.",
        risk: "sensitive",
      }),
    ).toEqual({ allowed: true });
    expect(
      gate({
        ...
        target: "#tint",
        value: "red",
        summary: "Set a badly formed color.",
        risk: "sensitive",
      }),
    ).toMatchObject({ allowed: false, reason: "The reviewed color must use the #rrggbb form." });
    expect(
      gate({
        id: "e1",
        kind: "expanddetails",
        target: "#faq",
        summary: "Open the reviewed section.",
        risk: "interaction",
      }),
    ).toEqual({ allowed: true });
    expect(
      gate({ id: "v1", kind: "verifyvisible", target: "#card", summary: "Verify the card renders.", risk: "read" }),
    ).toEqual({ allowed: true });
    expect(
      gate({ id: "v2", kind: "verifyenabled", target: "#card", summary: "Verify the card is enabled.", risk: "read" }),
    ).toEqual({ allowed: true });
    expect(gate({ id: "v3", kind: "mapclicks", summary: "Map every clickable element.", risk: "read" })).toEqual({
      allowed: true,
    });
  });
  ...
  it("validates the dialog, shadow and frame kinds with their origin grants", () => {
    expect(
      gate({
        id: "g1",
        kind: "dismissdialog",
        summary: "Accept the next dialog.",
        risk: "sensitive",
        options: '{"accept":true}',
      }),
    ).toEqual({ allowed: true });
    ...
    expect(
      gate({ id: "g3", kind: "dismissdialog", summary: "Arm without a reviewed policy.", risk: "sensitive" }),
    ).toMatchObject({ allowed: false, reason: "A reviewed accept flag or prompt answer is required in options." });
    expect(
      gate({
        id: "g4",
        kind: "dismissdialog",
        summary: "Arm with a non boolean accept.",
        risk: "sensitive",
        options: '{"accept":"yes"}',
      }),
    ).toMatchObject({ allowed: false, reason: "The reviewed dialog accept flag must be a boolean." });
    expect(
      gate({
        id: "g5",
        kind: "dismissdialog",
        summary: "Arm with an empty answer.",
        risk: "sensitive",
        options: '{"accept":true,"answer":""}',
      }),
    ).toMatchObject({ allowed: false, reason: "The reviewed prompt answer must be a non-empty string." });
    expect(
      gate({
        id: "w1",
        kind: "pierceshadow",
        target: "#inner",
        summary: "Click through the shadow root.",
        risk: "interaction",
        options: '{"shadow":["my-widget",".part"]}',
      }),
    ).toEqual({ allowed: true });
    expect(
      gate({
        ...
        kind: "pierceshadow",
        target: "#inner",
        summary: "Click with a broken shadow path.",
        risk: "interaction",
        options: '{"shadow":["my-widget",7]}',
      }),
    ).toMatchObject({ allowed: false, reason: "The reviewed shadow path must be a list of non-empty selectors." });
    expect(
      gate({
        id: "f1",
        kind: "enterframe",
        summary: "Click inside the reviewed frame.",
        risk: "sensitive",
        options: '{"framepath":[0],"kind":"click","target":"#go"}',
      }),
    ).toEqual({ allowed: true });
    expect(
      gate({
        id: "f2",
        kind: "enterframe",
        summary: "Route without a frame path.",
        risk: "sensitive",
        options: '{"kind":"click","target":"#go"}',
      }),
    ).toMatchObject({ allowed: false, reason: "A reviewed frame path of frame indexes is required in options." });
    expect(
      gate({
        id: "f3",
        kind: "enterframe",
        summary: "Route with a negative frame index.",
        risk: "sensitive",
        options: '{"framepath":[-1],"kind":"click","target":"#go"}',
      }),
    ).toMatchObject({ allowed: false, reason: "A reviewed frame path of frame indexes is required in options." });
    expect(
      gate({
        id: "f4",
        kind: "enterframe",
        summary: "Route with a wrapped wrapper.",
        risk: "sensitive",
        options: '{"framepath":[0],"kind":"retryaction"}',
      }),
    ).toMatchObject({ allowed: false, reason: "The reviewed inner step cannot be another wrapper kind." });
    const outsidegrants: agentsession = { ...session, grants: ["https://other.example"] };
    expect(
      gate(
        { id: "w3", kind: "pierceshadow", target: "#inner", summary: "Click outside the grants.", risk: "interaction" },
        outsidegrants,
      ),
    ).toMatchObject({ allowed: false, reason: "The shadow or frame step is outside the session origin grants." });
    expect(
      gate(
        {
          id: "f5",
          kind: "enterframe",
          summary: "Route outside the grants.",
          risk: "sensitive",
          options: '{"framepath":[0],"kind":"click","target":"#go"}',
        },
        outsidegrants,
  ...
  it("validates the retry wrapper and keeps attempt counts free of code ceilings", () => {
    expect(
      gate({
        id: "y1",
        kind: "retryaction",
        summary: "Retry the reviewed click.",
        risk: "interaction",
        options: '{"kind":"click","target":"#go","retryrule":{"attempts":3,"settle":250,"tolerance":6}}',
      }),
    ).toEqual({ allowed: true });
    expect(
      gate({
        id: "y2",
        kind: "retryaction",
        summary: "Retry with a very large reviewed attempt count.",
        risk: "interaction",
        options: '{"kind":"click","target":"#go","retryrule":{"attempts":500}}',
      }),
    ).toEqual({ allowed: true });
    expect(
      gate({
        id: "y3",
        kind: "retryaction",
        summary: "Retry without a rule.",
        risk: "interaction",
        options: '{"kind":"click","target":"#go"}',
      }),
    ).toMatchObject({ allowed: false, reason: "A reviewed retry rule with attempts is required in options." });
    expect(
      gate({
        id: "y4",
        kind: "retryaction",
        summary: "Retry with zero attempts.",
        risk: "interaction",
        options: '{"kind":"click","target":"#go","retryrule":{"attempts":0}}',
      }),
    ).toMatchObject({
      allowed: false,
      reason: "The reviewed retry attempts must be a positive integer with no code ceiling.",
    });
    expect(
      gate({
        id: "y5",
        kind: "retryaction",
        summary: "Retry with a negative settle window.",
        risk: "interaction",
        options: '{"kind":"click","target":"#go","retryrule":{"attempts":2,"settle":-1}}',
      }),
    ).toMatchObject({
      allowed: false,
    ...
    expect(
      gate({
        id: "y6",
        kind: "retryaction",
        summary: "Retry with a negative tolerance.",
        risk: "interaction",
        options: '{"kind":"click","target":"#go","retryrule":{"attempts":2,"tolerance":-4}}',
      }),
    ).toMatchObject({
      allowed: false,
    ...
    expect(
      gate({
        id: "y7",
        kind: "retryaction",
        summary: "Retry without an inner step.",
        risk: "interaction",
        options: '{"retryrule":{"attempts":2}}',
      }),
    ).toMatchObject({ allowed: false, reason: "A reviewed step id or inline step kind is required in options." });
    expect(
      gate({
        id: "y8",
        kind: "retryaction",
        summary: "Retry referencing a sibling step id.",
        risk: "interaction",
        options: '{"stepid":"step","retryrule":{"attempts":2}}',
      }),
    ).toEqual({ allowed: true });
    expect(
      gate({
        id: "y9",
        kind: "retryaction",
        summary: "Retry with both a step id and an inline step.",
        risk: "interaction",
        options: '{"stepid":"step","kind":"click","target":"#go","retryrule":{"attempts":2}}',
      }),
    ).toMatchObject({
      allowed: false,
      reason: "The reviewed wrapper must reference a step id or an inline step, not both.",
    });
  });
  ...
  it("validates every targetref mode and rejects empty references", () => {
    ...
    expect(validatetargetref({ mode: "selector" })).toMatchObject({
      allowed: false,
      reason: "The selector target reference needs a non-empty selector.",
    });
    expect(validatetargetref({ mode: "text", text: "Sign in" })).toEqual({ allowed: true });
    expect(validatetargetref({ mode: "text", text: "  " })).toMatchObject({
      allowed: false,
      reason: "The text target reference needs non-empty text.",
    });
    expect(validatetargetref({ mode: "aria", role: "button", name: "Submit" })).toEqual({ allowed: true });
    expect(validatetargetref({ mode: "aria", role: "button" })).toMatchObject({
      allowed: false,
      reason: "The aria target reference needs a non-empty name.",
    });
    expect(validatetargetref({ mode: "aria", name: "Submit" })).toMatchObject({
      allowed: false,
      reason: "The aria target reference needs a non-empty role.",
    });
    expect(validatetargetref({ mode: "name", name: "Email" })).toEqual({ allowed: true });
    expect(validatetargetref({ mode: "name" })).toMatchObject({
      allowed: false,
      reason: "The name target reference needs a non-empty name.",
    });
    expect(validatetargetref({ mode: "xpath", xpath: "//button[@id='go']" })).toEqual({ allowed: true });
    expect(validatetargetref({ mode: "xpath", xpath: "" })).toMatchObject({
      allowed: false,
      reason: "The xpath target reference needs a non-empty expression.",
    });
    expect(validatetargetref({ mode: "index", index: 3 })).toEqual({ allowed: true });
    expect(validatetargetref({ mode: "index", index: 0 })).toMatchObject({
      allowed: false,
      reason: "The index target reference needs a positive integer map number.",
    });
    expect(validatetargetref({ mode: "index", index: 2.5 })).toMatchObject({
      allowed: false,
      reason: "The index target reference needs a positive integer map number.",
    });
    expect(validatetargetref({ mode: "point", x: 120, y: 40 })).toEqual({ allowed: true });
    expect(validatetargetref({ mode: "point", x: 120 })).toMatchObject({
      allowed: false,
      reason: "The point target reference needs numeric x and y coordinates.",
    });
    expect(validatetargetref({ mode: "telepathy" })).toMatchObject({
      allowed: false,
      reason: "The target reference mode must be selector, text, aria, name, xpath, index or point.",
    });
    expect(validatetargetref("selector")).toMatchObject({
      allowed: false,
      reason: "The reviewed target reference must be an object.",
    });
    expect(validatetargetref(["selector"])).toMatchObject({
      allowed: false,
      reason: "The reviewed target reference must be an object.",
...
describe("observation depth policy", () => {
  ...
  it("runs passive observation kinds without extra options and targets where reviewed", () => {
    expect(gate({ id: "o1", kind: "a11ytree", summary: "Capture the accessibility tree.", risk: "read" })).toEqual({
      allowed: true,
    });
    expect(gate({ id: "o2", kind: "readertree", summary: "Extract the reader view.", risk: "read" })).toEqual({
      allowed: true,
    });
    expect(gate({ id: "o3", kind: "readoutline", summary: "Read the heading outline.", risk: "read" })).toEqual({
      allowed: true,
    });
    expect(gate({ id: "o4", kind: "readlang", summary: "Read the page language.", risk: "read" })).toEqual({
      allowed: true,
    });
    expect(gate({ id: "o5", kind: "listshadow", summary: "List open shadow roots.", risk: "read" })).toEqual({
      allowed: true,
    });
    expect(gate({ id: "o6", kind: "listframes", summary: "List the iframes.", risk: "read" })).toEqual({
      allowed: true,
    });
    expect(gate({ id: "o7", kind: "readselection", summary: "Read the current selection.", risk: "read" })).toEqual({
      allowed: true,
    });
    expect(gate({ id: "o8", kind: "readopengraph", summary: "Read the open graph fields.", risk: "read" })).toEqual({
      allowed: true,
    });
    expect(
      gate({ id: "o9", kind: "readvisible", target: "#main", summary: "Read the visible text.", risk: "read" }),
    ).toEqual({ allowed: true });
    expect(
      gate({
        id: "o10",
        kind: "deriveselector",
        target: "#checkout",
        summary: "Derive a stable selector.",
        risk: "read",
      }),
    ).toEqual({ allowed: true });
    expect(
      gate({
        id: "o11",
        kind: "fingerprintsection",
        target: "#prices",
        summary: "Fingerprint the section.",
        risk: "read",
      }),
    ).toEqual({ allowed: true });
    expect(gate({ id: "o12", kind: "classifypage", summary: "Classify the page template.", risk: "read" })).toEqual({
      allowed: true,
    });
    expect(gate({ id: "o13", kind: "countpages", summary: "Count the pagination.", risk: "read" })).toEqual({
      allowed: true,
    });
    expect(gate({ id: "o14", kind: "detectlanguage", summary: "Detect the text language.", risk: "read" })).toEqual({
      allowed: true,
    });
  });
  ...
  it("requires a reviewed lifetime window for every watch kind", () => {
    ...
    expect(gate({ id: "w1", kind: "watchmutate", summary: "Watch the feed.", risk: "read", options })).toEqual({
      allowed: true,
    });
    expect(gate({ id: "w2", kind: "watchfocus", summary: "Watch focus changes.", risk: "read", options })).toEqual({
      allowed: true,
    });
    expect(gate({ id: "w3", kind: "watchbanner", summary: "Watch consent banners.", risk: "read", options })).toEqual({
      allowed: true,
    });
    expect(gate({ id: "w4", kind: "watchmutate", summary: "Watch without a lifetime.", risk: "read" })).toMatchObject({
      allowed: false,
      reason: "A reviewed watch lifetime window in milliseconds is required in options.",
    });
    expect(
      gate({
        id: "w5",
        kind: "watchmutate",
        summary: "Watch with a zero lifetime.",
        risk: "read",
        options: '{"lifetime":0}',
      }),
    ).toMatchObject({
    ...
    expect(
      gate({
        id: "w6",
        kind: "watchmutate",
        summary: "Watch with a huge reviewed lifetime.",
        risk: "read",
        options: '{"lifetime":3600000}',
      }),
    ).toEqual({ allowed: true });
    expect(
      gate({
        id: "w7",
        kind: "watchmutate",
        summary: "Watch with reviewed scopes and event filters.",
        risk: "read",
        options: '{"lifetime":1000,"scopes":["#feed"],"events":["childList"],"poll":120}',
      }),
    ).toEqual({ allowed: true });
    expect(
      gate({
        id: "w8",
        kind: "watchmutate",
        summary: "Watch with a broken scope list.",
        risk: "read",
        options: '{"lifetime":1000,"scopes":"#feed"}',
      }),
    ).toMatchObject({ allowed: false, reason: "The reviewed watch scopes must be a list of non-empty selectors." });
    expect(
      gate({
        id: "w9",
        kind: "watchmutate",
        summary: "Watch with a broken event filter list.",
        risk: "read",
        options: '{"lifetime":1000,"events":[""]}',
      }),
    ).toMatchObject({ allowed: false, reason: "The reviewed watch event kinds must be a list of non-empty strings." });
    expect(
      gate({
        id: "w10",
        kind: "watchmutate",
        summary: "Watch with a negative poll interval.",
        risk: "read",
        options: '{"lifetime":1000,"poll":-5}',
      }),
    ).toMatchObject({
      allowed: false,
  ...
  it("validates the reviewed quiet rule without any code ceiling", () => {
    expect(
      gate({
        id: "q1",
        kind: "waitquiet",
        summary: "Wait for network quiet.",
        risk: "read",
        options: '{"quietrule":{"idle":500,"poll":100,"timeout":8000}}',
      }),
    ).toEqual({ allowed: true });
    expect(
      gate({
        id: "q2",
        kind: "waitquiet",
        summary: "Wait with an unbounded timeout.",
        risk: "read",
        options: '{"quietrule":{"idle":500}}',
      }),
    ).toEqual({ allowed: true });
    expect(
      gate({
        id: "q3",
        kind: "waitquiet",
        summary: "Wait with a huge reviewed idle threshold.",
        risk: "read",
        options: '{"quietrule":{"idle":600000}}',
      }),
    ).toEqual({ allowed: true });
    expect(gate({ id: "q4", kind: "waitquiet", summary: "Wait without a quiet rule.", risk: "read" })).toMatchObject({
      allowed: false,
      reason: "A reviewed quietrule with an idle threshold is required in options.",
    });
    expect(
      gate({
        id: "q5",
        kind: "waitquiet",
        summary: "Wait with a zero idle threshold.",
        risk: "read",
        options: '{"quietrule":{"idle":0}}',
      }),
    ).toMatchObject({
      allowed: false,
    ...
    expect(
      gate({
        id: "q6",
        kind: "waitquiet",
        summary: "Wait with a negative poll.",
        risk: "read",
        options: '{"quietrule":{"idle":500,"poll":-1}}',
      }),
    ).toMatchObject({
      allowed: false,
    ...
    expect(
      gate({
        id: "q7",
        kind: "waitquiet",
        summary: "Wait with a negative timeout.",
        risk: "read",
        options: '{"quietrule":{"idle":500,"timeout":-2}}',
      }),
    ).toMatchObject({
      allowed: false,
  ...
  it("requires exactly two reviewed observation versions for diffsnapshots", () => {
    expect(
      gate({
        id: "d1",
        kind: "diffsnapshots",
        summary: "Diff two observation versions.",
        risk: "read",
        options: '{"versions":[2,3]}',
      }),
    ).toEqual({ allowed: true });
    expect(gate({ id: "d2", kind: "diffsnapshots", summary: "Diff without versions.", risk: "read" })).toMatchObject({
      allowed: false,
      reason: "Two reviewed observation version numbers are required in options.",
    });
    expect(
      gate({
        id: "d3",
        kind: "diffsnapshots",
        summary: "Diff with one version.",
        risk: "read",
        options: '{"versions":[2]}',
      }),
    ).toMatchObject({ allowed: false, reason: "Two reviewed observation version numbers are required in options." });
    expect(
      gate({
        id: "d4",
        kind: "diffsnapshots",
        summary: "Diff with a zero version.",
        risk: "read",
        options: '{"versions":[0,3]}',
      }),
    ).toMatchObject({ allowed: false, reason: "Two reviewed observation version numbers are required in options." });
  });
  ...
  it("gates the json state read behind the session origin grants", () => {
    ...
    const outsidegrants: agentsession = { ...session, grants: ["https://other.example"] };
    expect(gate(readjson, outsidegrants)).toMatchObject({
      allowed: false,
      reason: "The json state read is outside the session origin grants.",
    });
  });
});
...
describe("navigation mastery policy", () => {
  ...
  function gate(
    ...
    input?: {
      ...
      planstate?: typeof plan.state;
      verdicts?: Array<{ url: string; safe: boolean; reasons: string[]; at: number }>;
    },
  ...
  it("grades the sensitive navigation kinds and the read only navigation detection kinds", () => {
    const sensitivekinds: actionkind[] = [
      "openlink",
      "openprivate",
      "reloadcache",
      "stopnav",
  ...
  it("requires a reviewed navtarget for openlink and openprivate and keeps the private container exclusive", () => {
    expect(
      gate({
        id: "n1",
        kind: "openlink",
        summary: "Open the reviewed url in a new tab.",
        risk: "sensitive",
        options: '{"navtarget":{"url":"https://example.com/pricing","container":"tab"}}',
      }),
    ).toEqual({ allowed: true });
    expect(gate({ id: "n2", kind: "openlink", summary: "Open without a navtarget.", risk: "sensitive" })).toMatchObject(
      { allowed: false, reason: "A reviewed navtarget with a url is required in options." },
    );
    expect(
      gate({
        id: "n3",
        kind: "openlink",
        summary: "Open an http url.",
        risk: "sensitive",
        options: '{"navtarget":{"url":"http://example.com","container":"tab"}}',
      }),
    ).toMatchObject({ allowed: false, reason: "The reviewed navtarget url must use HTTPS." });
    expect(
      gate({
        id: "n4",
        kind: "openlink",
        summary: "Open the private container.",
        risk: "sensitive",
        options: '{"navtarget":{"url":"https://example.com","container":"private"}}',
      }),
    ).toMatchObject({
      allowed: false,
      reason: "The openlink step cannot open the private container; use openprivate.",
    });
    expect(
      gate({
        id: "n5",
        kind: "openprivate",
        summary: "Open the reviewed url in a private window.",
        risk: "sensitive",
        options: '{"navtarget":{"url":"https://example.com","container":"private"}}',
      }),
    ).toEqual({ allowed: true });
    expect(
      gate({
        id: "n6",
        kind: "openprivate",
        summary: "Open privately without the private container.",
        risk: "sensitive",
        options: '{"navtarget":{"url":"https://example.com","container":"tab"}}',
      }),
    ).toMatchObject({ allowed: false, reason: "The openprivate step requires the private container." });
  });

  it("requires an approved session before a navlist run starts", () => {
    const navlist: toolstep = {
      id: "n7",
      kind: "navlist",
      summary: "Navigate the reviewed urls.",
      risk: "sensitive",
      options: '{"urls":["https://example.com/a","https://example.com/b"]}',
    };
    expect(gate(navlist)).toEqual({ allowed: true });
    expect(gate(navlist, { planstate: "pending" })).toMatchObject({
      allowed: false,
      reason: "The plan has not received explicit approval.",
    });
    expect(gate({ id: "n8", kind: "navlist", summary: "Navigate without urls.", risk: "sensitive" })).toMatchObject({
      allowed: false,
      reason: "A reviewed non-empty list of HTTPS urls is required in options as urls.",
    });
  });
  ...
  it("blocks navigation away from the task tab until the user consents through a session grant", () => {
    const navlist: toolstep = {
      id: "n9",
      kind: "navlist",
      summary: "Leave the granted origins.",
      risk: "sensitive",
      options: '{"urls":["https://example.com/a","https://stranger.example/b"]}',
    };
    expect(gate(navlist)).toMatchObject({
      allowed: false,
      reason:
        "Navigation to https://stranger.example leaves the task tab origins and needs the user consent of a session grant first.",
    });
  ...
  it("requires checksafe verification before an unreviewed origin opens", () => {
    const openlink: toolstep = {
      id: "n10",
      kind: "openlink",
      summary: "Open an unreviewed origin.",
      risk: "sensitive",
      options: '{"navtarget":{"url":"https://stranger.example/report","container":"tab"}}',
    };
    expect(gate(openlink)).toMatchObject({
      allowed: false,
      reason: expect.stringContaining("outside the session grants and has no safe checksafe verdict"),
    });
    expect(
      gate(openlink, { verdicts: [{ url: "https://stranger.example/report", safe: true, reasons: [], at: 1 }] }),
    ).toEqual({ allowed: true });
    expect(
      gate(openlink, {
        verdicts: [
          {
            url: "https://stranger.example/report",
            safe: false,
            reasons: ["the host is a private network target"],
            at: 1,
          },
        ],
      }),
  ...
  it("keeps the navigation rate window a user configured choice with no hardcoded cap", () => {
    const navrate: toolstep = {
      id: "n11",
      kind: "navrate",
      summary: "Apply the reviewed rate limit.",
      risk: "sensitive",
      options: '{"ratelimit":{"domain":"example.com","window":600000,"ceiling":3}}',
    };
    ...
    expect(
      gate({
        id: "n12",
        kind: "navrate",
        summary: "Apply a huge reviewed window.",
        risk: "sensitive",
        options: '{"ratelimit":{"window":86400000,"ceiling":100000}}',
      }),
    ).toEqual({ allowed: true });
    expect(gate({ id: "n13", kind: "navrate", summary: "Apply without a limit.", risk: "sensitive" })).toMatchObject({
      allowed: false,
      reason: "A reviewed ratelimit with a window and a ceiling is required in options.",
    });
    expect(
      gate({
        id: "n14",
        kind: "navrate",
        summary: "Apply with a zero window.",
        risk: "sensitive",
        options: '{"ratelimit":{"window":0,"ceiling":3}}',
      }),
    ).toMatchObject({
      allowed: false,
    ...
    expect(
      gate({
        id: "n15",
        kind: "navrate",
        summary: "Apply with a zero ceiling.",
        risk: "sensitive",
        options: '{"ratelimit":{"window":1000,"ceiling":0}}',
      }),
    ).toMatchObject({
      allowed: false,
  ...
  it("validates the waitprofile and urlpattern grammars of the navigation family", () => {
    const navprofile: toolstep = {
      id: "n16",
      kind: "navprofile",
      summary: "Apply the per site wait profile.",
      risk: "sensitive",
      options:
        '{"waitprofile":{"signals":["load","networkidle"],"idle":500,"overrides":[{"origin":"https://slow.example","idle":1500}]}}',
    };
    ...
    expect(
      gate({
        id: "n17",
        kind: "navprofile",
        summary: "Apply without signals.",
        risk: "sensitive",
        options: '{"waitprofile":{"idle":500}}',
      }),
    ).toMatchObject({ allowed: false, reason: "The reviewed waitprofile needs a non-empty list of load signals." });
    expect(
      gate({
        id: "n18",
        kind: "waiturl",
        summary: "Wait for the reviewed pattern.",
        risk: "read",
        options:
          '{"urlpattern":{"mode":"prefix","url":"https://example.com/results","query":{"page":"*"}},"timeout":4000}',
      }),
    ).toEqual({ allowed: true });
    expect(gate({ id: "n19", kind: "waiturl", summary: "Wait without a pattern.", risk: "read" })).toMatchObject({
      allowed: false,
      reason: "A reviewed urlpattern is required in options.",
    });
    expect(
      gate({
        id: "n20",
        kind: "waiturl",
        summary: "Wait with a broken mode.",
        risk: "read",
        options: '{"urlpattern":{"mode":"glob","url":"https://example.com"}}',
      }),
    ).toMatchObject({ allowed: false, reason: "The reviewed urlpattern mode must be exact, prefix, host or pattern." });
    expect(
      gate({
        id: "n21",
        kind: "waitload",
        summary: "Wait for the load event.",
        risk: "read",
    ...
    expect(
      gate({
        id: "n22",
        kind: "spawait",
        summary: "Wait for the spa route change.",
        risk: "read",
        options: '{"timeout":2000,"poll":50}',
      }),
    ).toEqual({ allowed: true });
  });
  ...
  it("validates the query, fragment, link and deep link grammars", () => {
    expect(
      gate({
        id: "n23",
        kind: "rewritequery",
        summary: "Rewrite the query parameters.",
        risk: "sensitive",
        options: '{"set":{"page":"3"},"remove":["session"]}',
      }),
    ).toEqual({ allowed: true });
    expect(
      gate({ id: "n24", kind: "rewritequery", summary: "Rewrite without edits.", risk: "sensitive" }),
    ).toMatchObject({ allowed: false, reason: "Reviewed query parameters to set or remove are required in options." });
    expect(
      gate({
        id: "n25",
        kind: "setfragment",
        value: "pricing",
        summary: "Set the reviewed fragment.",
        risk: "sensitive",
      }),
    ).toEqual({ allowed: true });
    expect(
      gate({
        ...
        kind: "followlink",
        value: "Read the docs",
        summary: "Follow the reviewed link.",
        risk: "sensitive",
      }),
    ).toEqual({ allowed: true });
    expect(
    ...
    expect(
      gate({
        ...
        kind: "followlink",
        value: "Read the docs",
        summary: "Follow with a broken fragment flag.",
        risk: "sensitive",
        options: '{"fragment":"yes"}',
      }),
    ).toMatchObject({ allowed: false, reason: "The reviewed followlink fragment flag must be a boolean." });
    expect(
      gate({
        id: "n29",
        kind: "spanav",
        value: "Open settings",
        summary: "Route the single page app.",
        risk: "sensitive",
        options: '{"routepattern":{"mode":"prefix","url":"https://example.com/settings"},"timeout":2500}',
      }),
    ).toEqual({ allowed: true });
    expect(
      gate({
        id: "n30",
        kind: "deeplink",
        summary: "Build the deep link.",
        risk: "sensitive",
        options:
          '{"app":"github","params":{"owner":"wenathlan","repo":"extension"},"navtarget":{"url":"https://example.com","container":"tab"}}',
      }),
    ).toEqual({ allowed: true });
    expect(
      gate({
        id: "n31",
        kind: "deeplink",
        summary: "Build without an app.",
        risk: "sensitive",
        options: '{"navtarget":{"url":"https://example.com","container":"tab"}}',
      }),
    ).toMatchObject({ allowed: false, reason: "A reviewed deep link app pattern is required in options." });
  });
  ...
  it("validates the auth, print, prefetch, preconnect, clipboard and batch grammars", () => {
    expect(
      gate({
        ...
        summary: "Answer the basic auth prompt.",
        risk: "sensitive",
      }),
    ).toEqual({ allowed: true });
    expect(
      gate({
        ...
        summary: "Answer without https.",
        risk: "sensitive",
      }),
    ).toMatchObject({ allowed: false, reason: "A reviewed HTTPS origin or url is required as the auth target." });
    expect(
      gate({
        id: "n34",
        kind: "printpdf",
    ...
    expect(
      gate({
        id: "n35",
        kind: "prefetch",
        summary: "Prefetch the predicted pages.",
        risk: "sensitive",
        options: '{"urls":["https://example.com/next"]}',
      }),
    ).toEqual({ allowed: true });
    expect(gate({ id: "n36", kind: "prefetch", summary: "Prefetch without urls.", risk: "sensitive" })).toMatchObject({
      allowed: false,
      reason: "A reviewed non-empty list of HTTPS urls is required in options as urls.",
    });
    expect(
      gate({
        id: "n37",
        kind: "preconnect",
        summary: "Preconnect to the expected origins.",
        risk: "sensitive",
        options: '{"origins":["https://cdn.example","https://api.example"]}',
      }),
    ).toEqual({ allowed: true });
    expect(
      gate({ id: "n38", kind: "preconnect", summary: "Preconnect without origins.", risk: "sensitive" }),
    ).toMatchObject({ allowed: false, reason: "A reviewed non-empty list of HTTPS origins is required in options." });
    expect(
      gate({
        id: "n39",
        kind: "openclipboard",
        value: "clipboard url",
        summary: "Open the clipboard url on consent.",
        risk: "sensitive",
      }),
    ).toEqual({ allowed: true });
    expect(
    ...
    expect(
      gate({
        ...
        kind: "checksafe",
        value: "http://example.com",
        summary: "Verify without https.",
        risk: "read",
      }),
    ).toMatchObject({ allowed: false, reason: "A reviewed HTTPS url is required for the safety check." });
    expect(
      gate({
        id: "n42",
        kind: "batchopen",
        summary: "Open the curated list.",
        risk: "sensitive",
        options: '{"urls":["https://example.com/a","https://example.com/b"]}',
      }),
    ).toEqual({ allowed: true });
...
describe("policy tabs and windows command", () => {
  ...
  it("restricts group and layout mutations to the active session", () => {
    expect(layoutmutationgranted(undefined, now)).toMatchObject({
      allowed: false,
      reason: "Group and layout mutations stay inside the active session.",
    });
  ...
  it("validates the tabquery grammar of the querytabs family", () => {
    ...
    expect(validatetabquery({})).toMatchObject({
      allowed: false,
      reason: "The reviewed tabquery needs a url, title, id or pattern matcher.",
    });
  ...
  it("validates the tab and window command grammars of pin, mute, move, group, switch and zoom kinds", () => {
    ...
    expect(
      gate({ id: "w2", kind: "pintab", value: "12", summary: "Pin without a flag.", risk: "sensitive" }),
    ).toMatchObject({ allowed: false, reason: "A reviewed pinned flag is required in options." });
    expect(
      gate({
        id: "w3",
        kind: "mutetab",
        value: "12",
        summary: "Mute without a flag.",
        risk: "sensitive",
        options: '{"muted":"yes"}',
      }),
    ).toMatchObject({ allowed: false, reason: "A reviewed muted flag is required in options." });
    expect(
      gate({ id: "w4", kind: "movetab", value: "12", summary: "Move without an index.", risk: "sensitive" }),
    ).toMatchObject({ allowed: false, reason: "A reviewed non-negative target index is required in options." });
    expect(
      gate({
        id: "w5",
        kind: "movetabwindow",
        value: "12",
        summary: "Move across windows.",
        risk: "sensitive",
        options: '{"windowid":3}',
      }),
    ).toEqual({ allowed: true });
    expect(
      gate({ id: "w6", kind: "movetabwindow", value: "12", summary: "Move without a window.", risk: "sensitive" }),
    ).toMatchObject({ allowed: false, reason: "A reviewed target window id is required in options." });
    expect(
      gate({
        id: "w7",
        kind: "grouptabs",
        summary: "Group the research tabs.",
        risk: "sensitive",
        options: '{"group":{"name":"research","color":"blue","tabids":[1,2]}}',
      }),
    ).toEqual({ allowed: true });
    expect(
      gate({
        id: "w8",
        kind: "grouptabs",
        summary: "Group with a bad color.",
        risk: "sensitive",
        options: '{"group":{"name":"research","color":"chartreuse","tabids":[1]}}',
      }),
    ).toMatchObject({ allowed: false, reason: "The reviewed group color must be a Chromium tab group color." });
    expect(
      gate({
        id: "w9",
        kind: "colorgroup",
        summary: "Color the group.",
        risk: "sensitive",
        options: '{"name":"research","color":"pink"}',
      }),
    ).toEqual({ allowed: true });
    expect(
      gate({
        id: "w10",
        kind: "collapsegroup",
        summary: "Collapse the group.",
        risk: "sensitive",
        options: '{"name":"research","collapsed":true}',
      }),
    ).toEqual({ allowed: true });
    expect(
      gate({ id: "w11", kind: "switchtab", summary: "Switch without a direction.", risk: "sensitive" }),
    ).toMatchObject({
      allowed: false,
      reason: "A reviewed switch direction of next or previous is required in options.",
    });
    expect(
      gate({
        id: "w12",
    ...
    expect(
      gate({ id: "w13", kind: "zoomin", summary: "Zoom in by the step.", risk: "sensitive", options: '{"step":0.25}' }),
    ).toEqual({ allowed: true });
    expect(
      gate({
        id: "w14",
        kind: "zoomout",
        summary: "Zoom out by a bad step.",
        risk: "sensitive",
        options: '{"step":0}',
      }),
    ).toMatchObject({
      allowed: false,
      reason: "The reviewed zoom step must be a positive number with no code ceiling.",
    });
  });
  ...
  it("validates the badge, meta, restore, layout, reopen and incognito grammars", () => {
    ...
    expect(
      gate({
        ...
        kind: "badgetab",
        value: "7",
        summary: "Badge the tab.",
        risk: "sensitive",
        options: '{"label":"3/5"}',
      }),
    ).toEqual({ allowed: true });
    expect(
      gate({ id: "b2", kind: "badgetab", value: "7", summary: "Badge without a label.", risk: "sensitive" }),
    ).toMatchObject({ allowed: false, reason: "A reviewed badge label is required in options." });
    expect(
      gate({
        id: "b3",
        kind: "attachmeta",
        value: "7",
        summary: "Attach metadata.",
        risk: "read",
        options: '{"labels":["research"],"taskrefs":["plan-1"]}',
      }),
    ).toEqual({ allowed: true });
    expect(gate({ id: "b4", kind: "attachmeta", value: "7", summary: "Attach nothing.", risk: "read" })).toMatchObject({
      allowed: false,
      reason: "Reviewed labels or task refs are required in options to attach metadata.",
    });
    expect(
      gate({
        id: "b5",
        kind: "restoretab",
        value: "https://example.com/closed",
        summary: "Restore the closed tab.",
        risk: "sensitive",
      }),
    ).toEqual({ allowed: true });
    expect(
      gate({
        id: "b6",
        kind: "restoretab",
        value: "http://example.com/closed",
        summary: "Restore an insecure url.",
        risk: "sensitive",
      }),
    ).toMatchObject({ allowed: false });
    expect(
      gate({ id: "b7", kind: "savelayout", summary: "Save the layout.", risk: "read", options: '{"name":"work"}' }),
    ).toEqual({ allowed: true });
    expect(
      gate({ id: "b8", kind: "restorelayout", summary: "Restore without a name.", risk: "sensitive" }),
    ).toMatchObject({ allowed: false, reason: "A reviewed layout name is required in options." });
    expect(
      gate({
        id: "b9",
        kind: "reopenrun",
        summary: "Reopen the run.",
        risk: "sensitive",
        options: '{"run":"session-42"}',
      }),
    ).toEqual({ allowed: true });
    expect(gate({ id: "b10", kind: "reopenrun", summary: "Reopen without a run.", risk: "sensitive" })).toMatchObject({
      allowed: false,
      reason: "A reviewed run id is required in options to reopen its tabs.",
    });
    expect(
      gate({
        id: "b11",
        kind: "incognitowindow",
        value: "https://example.com/private",
        summary: "Open the private window.",
        risk: "sensitive",
      }),
    ).toEqual({ allowed: true });
    expect(
      gate({
        id: "b12",
        kind: "incognitowindow",
        value: "http://example.com/private",
        summary: "Open an insecure window.",
        risk: "sensitive",
      }),
    ).toMatchObject({ allowed: false, reason: "A reviewed HTTPS url is required to open an incognito window." });
    expect(
      gate({
        id: "b13",
        kind: "searchtabs",
        value: "quarterly report",
        summary: "Search the open tabs.",
        risk: "read",
      }),
    ).toEqual({ allowed: true });
    expect(
      gate({
        id: "b14",
        kind: "watchtab",
        summary: "Watch tab events.",
        risk: "read",
        options: '{"lifetime":5000,"events":["title","closed"]}',
      }),
    ).toEqual({ allowed: true });
    expect(gate({ id: "b15", kind: "watchtab", summary: "Watch without a lifetime.", risk: "read" })).toMatchObject({
      allowed: false,
      reason: "A reviewed watch lifetime window in milliseconds is required in options.",
    });
  });
  ...
  it("runs a reviewed group mutation inside the session gates and refuses it outside an approved plan", () => {
    const groupstep: toolstep = {
      id: "g1",
      kind: "grouptabs",
      summary: "Group the research tabs.",
      risk: "sensitive",
      options: '{"group":{"name":"research","color":"blue","tabids":[4]}}',
    };
    expect(canexecute({ session, plan, step: groupstep, tabid: 4, origin: "https://example.com", now })).toEqual({
      allowed: true,
    });
    expect(
      canexecute({
    ...
    ).toMatchObject({ allowed: false, reason: "The plan has not received explicit approval." });
    expect(
      canexecute({
        session,
        plan,
        step: { ...groupstep, options: '{"group":{"name":"research","color":"blue"}}' },
        tabid: 4,
        origin: "https://example.com",
        now,
      }),
    ).toMatchObject({ allowed: false, reason: "The reviewed group needs a non-empty list of member tab ids." });
  });
  ...
  it("grades the forms and data family with sensitive fills and read only inspection", () => {
    for (const kind of [
      ...
      expect(actionrisk(kind as actionkind)).toBe("sensitive");
    for (const kind of [
      "detectfields",
      "generatevalues",
      "saveprofiles",
      ...
      expect(actionrisk(kind as actionkind)).toBe("read");
  ...
  it("requires an asksubmit review before every form submission", () => {
    const ask: toolstep = {
      ...
      kind: "asksubmit",
      value: "#checkout",
      summary: "Ask before submitting.",
      risk: "read",
    };
    const submit: toolstep = {
      id: "submit",
      kind: "submitform",
      target: "#checkout",
      summary: "Submit the form.",
      risk: "sensitive",
      options: '{"consentref":"ask"}',
    };
    expect(submitreviewgranted([ask, submit], "submit")).toEqual({ allowed: true });
    expect(submitreviewgranted([submit], "submit")).toMatchObject({
      allowed: false,
      reason: "Form submission requires an asksubmit review step before it.",
    });
    expect(submitreviewgranted([submit, ask], "submit")).toMatchObject({ allowed: false });
    expect(
      canexecute({
        session,
        plan: { ...plan, steps: [submit] },
        step: submit,
        tabid: 4,
        origin: "https://example.com",
        now,
      }),
    ).toMatchObject({ allowed: false, reason: "Form submission requires an asksubmit review step before it." });
    expect(
      canexecute({
        session,
        plan: { ...plan, steps: [ask, submit] },
        step: submit,
        tabid: 4,
        origin: "https://example.com",
        now,
  ...
  it("requires explicit consent before any password field is filled", () => {
    const password: toolstep = {
      id: "pw",
      kind: "consentpassword",
      target: "#password",
      value: "reviewed-secret",
      summary: "Fill the password after consent.",
      risk: "sensitive",
    };
    expect(passwordconsentgranted(password)).toMatchObject({
      allowed: false,
      reason: "A password fill requires a reviewed consent ref in options.",
    });
    expect(passwordconsentgranted({ ...password, options: '{"consentref":"consent-1"}' })).toEqual({ allowed: true });
    expect(
      canexecute({
        session,
        plan: { ...plan, steps: [password] },
        step: password,
        tabid: 4,
        origin: "https://example.com",
        now,
    ...
    expect(
      validateformrecord({ entries: [{ match: { mode: "name", name: "password" }, kind: "password", value: "x" }] }),
    ).toMatchObject({ allowed: false, reason: expect.stringContaining("consentpassword") });
    const fill: toolstep = {
      id: "fill",
      kind: "fillform",
      ...
      options: JSON.stringify({
        formrecord: { entries: [{ match: { mode: "name", name: "password" }, kind: "password", value: "x" }] },
      }),
    };
    expect(validatestep(fill, "https://example.com")).toMatchObject({
      allowed: false,
      reason: expect.stringContaining("Password entries are refused"),
    });
  });

  it("refuses generated values that look like real card numbers or personal identifiers", () => {
    expect(generatedvalueallowed("4539 1488 0343 6467")).toMatchObject({
      allowed: false,
      reason: expect.stringContaining("real card number"),
    });
    ...
    expect(generatedvalueallowed("123-45-6789")).toMatchObject({
      allowed: false,
      reason: expect.stringContaining("personal identifier"),
    });
  ...
  it("validates the form record, value rule and field pair grammars", () => {
    ...
    expect(
      gate({
        id: "f1",
        kind: "fillform",
        summary: "Fill the checkout form.",
        risk: "sensitive",
        options: JSON.stringify({ formrecord: record }),
      }),
    ).toEqual({ allowed: true });
    expect(gate({ id: "f2", kind: "fillform", summary: "No record.", risk: "sensitive" })).toMatchObject({
      allowed: false,
      reason: "A reviewed form record with entries is required in options.",
    });
    expect(
      gate({
        id: "f3",
        kind: "fillform",
        summary: "Empty entries.",
        risk: "sensitive",
        options: '{"formrecord":{"entries":[]}}',
      }),
    ).toMatchObject({ allowed: false });
    expect(
      gate({
        ...
        risk: "sensitive",
        options: JSON.stringify({
          formrecord: { entries: [{ match: { mode: "label", label: "x" }, kind: "blob", value: "y" }] },
        }),
      }),
    ).toMatchObject({ allowed: false, reason: "Every reviewed form record entry needs a known field kind." });
    expect(
      gate({
        id: "l1",
        kind: "filllabel",
        summary: "Fill by label.",
        risk: "sensitive",
        options: '{"fields":[{"label":"Full name","value":"Ana Alves"}]}',
      }),
    ).toEqual({ allowed: true });
    expect(
      gate({
        ...
        risk: "sensitive",
        options: '{"fields":[{"value":"Ana"}]}',
      }),
    ).toMatchObject({ allowed: false });
    expect(
      gate({
        id: "p1",
        kind: "fillplaceholder",
        summary: "Fill by placeholder.",
        risk: "sensitive",
        options: '{"fields":[{"placeholder":"you@example.org","value":"ana@example.com"}]}',
      }),
    ).toEqual({ allowed: true });
    expect(
      gate({
        ...
        risk: "read",
        options: '{"valuegen":{"kind":"email","locale":"pt","seed":7}}',
      }),
    ).toEqual({ allowed: true });
    expect(
      gate({
        ...
        summary: "Bad seed.",
        risk: "read",
        options: '{"valuegen":{"kind":"email","seed":"seven"}}',
      }),
    ).toMatchObject({ allowed: false });
  ...
  it("validates the submission, retry, chain, typeahead, calendar, card, code and profile grammars", () => {
    const gate = (step: toolstep): policyevaluation => validatestep(step, "https://example.com");
    expect(
      gate({ id: "s1", kind: "submitform", target: "#checkout", summary: "Submit.", risk: "sensitive" }),
    ).toMatchObject({
      allowed: false,
      reason: "A reviewed consent ref of an approved asksubmit ticket is required in options.",
    });
    expect(
      gate({
        id: "s2",
    ...
    expect(
      gate({
        ...
        summary: "Zero wait.",
        risk: "sensitive",
        options: '{"backoff":{"wait":0,"factor":2}}',
      }),
    ).toMatchObject({ allowed: false });
    expect(
      gate({
        ...
        kind: "retryform",
        target: "#checkout",
        summary: "Shrinking factor.",
        risk: "sensitive",
        options: '{"backoff":{"wait":500,"factor":0.5}}',
      }),
    ).toMatchObject({ allowed: false });
    expect(
      gate({
        ...
        kind: "retryform",
        target: "#checkout",
        summary: "Huge windows are a user choice.",
        risk: "sensitive",
        options: '{"backoff":{"wait":600000,"factor":10},"attempts":1000}',
      }),
    ).toEqual({ allowed: true });
    expect(
      gate({
        ...
        target: "#country",
        value: "br",
        summary: "Chain the states.",
        risk: "sensitive",
        options: '{"child":"#state"}',
      }),
    ).toEqual({ allowed: true });
    expect(
      gate({ id: "c2", kind: "selectchain", target: "#country", value: "br", summary: "No child.", risk: "sensitive" }),
    ).toMatchObject({ allowed: false });
    expect(
      gate({
        id: "t1",
        ...
        target: "#city",
        value: "sao",
        summary: "Pick the city.",
        risk: "sensitive",
        options: '{"pick":"São Paulo"}',
      }),
    ).toEqual({ allowed: true });
    expect(
      gate({ id: "t2", kind: "picktypeahead", target: "#city", value: "sao", summary: "No pick.", risk: "sensitive" }),
    ).toMatchObject({ allowed: false });
    expect(
      gate({
        id: "d1",
    ...
    expect(
      gate({
        id: "k1",
        kind: "fillcard",
        summary: "Fill the card.",
        risk: "sensitive",
        options: JSON.stringify({
          segments: [{ match: { mode: "label", label: "Card number" }, value: "4111 1111 1111 1111" }],
          pause: 80,
        }),
      }),
    ).toEqual({ allowed: true });
    expect(
      gate({ id: "k2", kind: "fillcard", summary: "Empty segments.", risk: "sensitive", options: '{"segments":[]}' }),
    ).toMatchObject({ allowed: false });
    expect(
      gate({
        id: "o1",
        ...
        target: "#code",
        value: "123456",
        summary: "Type the code.",
        risk: "sensitive",
        options: '{"source":"reviewed"}',
      }),
    ).toEqual({ allowed: true });
    expect(
      gate({ id: "o2", kind: "fillcode", target: "#code", value: "123456", summary: "No source.", risk: "sensitive" }),
    ).toMatchObject({ allowed: false });
    expect(
      gate({
        id: "a1",
        ...
        target: "#receipt",
        value: "report.pdf",
        summary: "Attach the artifact.",
        risk: "sensitive",
      }),
    ).toEqual({ allowed: true });
    expect(
      gate({ id: "w1", kind: "runwizard", summary: "Advance the wizard.", risk: "sensitive", options: '{"steps":3}' }),
    ).toEqual({ allowed: true });
    expect(
      gate({ id: "w2", kind: "runwizard", summary: "Bad step count.", risk: "sensitive", options: '{"steps":0}' }),
    ).toMatchObject({ allowed: false });
    expect(
      gate({
        id: "v1",
        ...
        options: JSON.stringify({
          name: "personal",
          formrecord: { entries: [{ match: { mode: "label", label: "Full name" }, kind: "text", value: "Ana Alves" }] },
        }),
      }),
    ).toEqual({ allowed: true });
    expect(
      gate({
        ...
        risk: "read",
        options:
          '{"formrecord":{"entries":[{"match":{"mode":"label","label":"Full name"},"kind":"text","value":"Ana"}]}}',
      }),
    ).toMatchObject({ allowed: false });
  });
  ...
  it("requires profile origin grants before a profile fills a page", () => {
    const profile: formprofile = {
      name: "personal",
      fields: [{ match: { mode: "label", label: "Full name" }, kind: "text", value: "Ana Alves" }],
      grants: ["https://example.com"],
      savedat: 10,
    };
    expect(profilegrantgranted(profile, "https://example.com")).toEqual({ allowed: true });
    expect(profilegrantgranted(profile, "https://other.example")).toMatchObject({
      allowed: false,
  ...
  it("refuses exports outside the session origin grants before any data leaves local memory", () => {
    const exportstep: toolstep = {
      id: "export",
      kind: "exportcsv",
      summary: "Export the scraped dataset.",
      risk: "sensitive",
      options: '{"dataset":"d1"}',
    };
    expect(exportgranted(session, "https://example.com")).toEqual({ allowed: true });
    expect(exportgranted(session, "https://other.example")).toMatchObject({
      allowed: false,
      reason: expect.stringContaining("origin grants"),
    });
    expect(
      canexecute({
        session: { ...session, grants: ["https://data.example"] },
        plan,
        step: exportstep,
        tabid: 4,
        origin: "https://example.com",
    ...
    ).toMatchObject({ allowed: false, reason: expect.stringContaining("leaves local memory") });
    expect(canexecute({ session, plan, step: exportstep, tabid: 4, origin: "https://example.com", now })).toEqual({
      allowed: true,
    });
  });
  ...
  it("validates the extraction, transform, export and provenance grammar with no code ceilings", () => {
    ...
    expect(
      gate({
        ...
        kind: "scrapetable",
        target: "table.prices",
        summary: "Scrape the table.",
        risk: "read",
        options: '{"rowlimit":5000}',
      }),
    ).toEqual({ allowed: true });
    expect(gate({ id: "s2", kind: "scrapetable", summary: "No table target.", risk: "read" })).toMatchObject({
      allowed: false,
      reason: "A page target is required.",
    });
    expect(
      gate({
        id: "s3",
    ...
    expect(
      gate({
        ...
        target: "table",
        summary: "Follow the pages.",
        risk: "sensitive",
        options: '{"next":"a.next","pages":900,"wait":400}',
      }),
    ).toEqual({ allowed: true });
    expect(
      gate({ id: "p2", kind: "paginateextract", target: "table", summary: "No next selector.", risk: "sensitive" }),
    ).toMatchObject({ allowed: false, reason: "A reviewed next control selector is required in options." });
    expect(
      gate({
        id: "p3",
        kind: "paginateextract",
    ...
    expect(
      gate({
        ...
        summary: "Export csv.",
        risk: "sensitive",
        options: '{"dataset":"d1","delimiter":";","name":"catalog"}',
      }),
    ).toEqual({ allowed: true });
    expect(gate({ id: "e2", kind: "exportcsv", summary: "No dataset.", risk: "sensitive" })).toMatchObject({
      allowed: false,
      reason: "A reviewed dataset id is required in options.",
    });
    expect(
      gate({
        id: "e3",
        kind: "exportcsv",
        summary: "Bad delimiter.",
        risk: "sensitive",
        options: '{"dataset":"d1","delimiter":";;"}',
      }),
    ).toMatchObject({ allowed: false });
    expect(
      gate({ id: "x1", kind: "exportexcel", summary: "Export excel.", risk: "sensitive", options: '{"dataset":"d1"}' }),
    ).toEqual({ allowed: true });
    expect(
      gate({ id: "c1", kind: "copytable", summary: "Copy the table.", risk: "sensitive", options: '{"dataset":"d1"}' }),
    ).toEqual({ allowed: true });
    expect(
      gate({
        id: "k1",
        kind: "streamdisk",
        summary: "Stream huge extracts.",
        risk: "sensitive",
        options: '{"dataset":"d1","chunk":100000}',
      }),
    ).toEqual({ allowed: true });
    expect(
      gate({ id: "k2", kind: "streamdisk", summary: "No chunk size.", risk: "sensitive", options: '{"dataset":"d1"}' }),
    ).toMatchObject({
      allowed: false,
    ...
    expect(
      gate({
        id: "h1",
        kind: "pushsheets",
        summary: "Push the sheet.",
        risk: "sensitive",
        options: '{"dataset":"d1","sheet":"https://sheets.example/tab","reviewed":true}',
      }),
    ).toEqual({ allowed: true });
    expect(
      gate({
        id: "h2",
        kind: "pushsheets",
        summary: "Missing reviewed flag.",
        risk: "sensitive",
        options: '{"dataset":"d1","sheet":"https://sheets.example/tab"}',
      }),
    ).toMatchObject({ allowed: false, reason: expect.stringContaining("reviewed flag") });
    expect(
      gate({
        id: "h3",
        kind: "pushsheets",
        summary: "HTTP sheet.",
        risk: "sensitive",
        options: '{"dataset":"d1","sheet":"http://sheets.example","reviewed":true}',
      }),
    ...
    expect(
      gate({
        id: "i1",
        kind: "importcsv",
        summary: "Import the csv.",
        risk: "read",
        options: '{"csv":"name,price\\nDesk,10","mapping":{"name":"sku"}}',
      }),
    ).toEqual({ allowed: true });
    expect(
      gate({ id: "i2", kind: "importcsv", summary: "Empty csv.", risk: "read", options: '{"csv":" "}' }),
    ).toMatchObject({ allowed: false });
    expect(
      gate({
        id: "l1",
        kind: "looprows",
        summary: "Loop the rows.",
        risk: "read",
        options:
          '{"dataset":"d1","kind":"filllabel","target":"form","options":{"fields":[{"label":"{{name}}","value":"{{price}}"}]}}',
      }),
    ).toEqual({ allowed: true });
    expect(
      gate({ id: "l2", kind: "looprows", summary: "No inner step.", risk: "read", options: '{"dataset":"d1"}' }),
    ).toMatchObject({ allowed: false, reason: "A reviewed step id or inline step kind is required in options." });
    expect(
      gate({
        id: "l3",
        ...
        summary: "Nested loop.",
        risk: "read",
        options: '{"dataset":"d1","kind":"looprows"}',
      }),
    ).toMatchObject({ allowed: false, reason: "The reviewed inner step cannot be another wrapper kind." });
    expect(
      gate({
        id: "t1",
        kind: "transformvalues",
        summary: "Transform values.",
        risk: "read",
        options: '{"dataset":"d1","rules":[{"expression":"replace:$=>","sources":["price"],"target":"price"}]}',
      }),
    ).toEqual({ allowed: true });
    expect(
      gate({
        ...
        risk: "read",
        options: '{"dataset":"d1","rules":[{"expression":"explode","sources":["price"],"target":"price"}]}',
      }),
    ).toMatchObject({ allowed: false, reason: expect.stringContaining("expression") });
    expect(
      gate({
        id: "t3",
        kind: "transformvalues",
        summary: "Empty sources.",
        risk: "read",
        options: '{"dataset":"d1","rules":[{"expression":"trim","sources":[],"target":"price"}]}',
      }),
    ).toMatchObject({ allowed: false });
    expect(validatetransformrule({ expression: "replace:usd=>$", sources: ["price"], target: "price" })).toEqual({
      allowed: true,
    });
    expect(validatetransformrule({ expression: "replace:=>x", sources: ["price"], target: "price" })).toMatchObject({
      allowed: false,
    });
    expect(
      gate({
        ...
        summary: "Dedupe rows.",
        risk: "read",
        options: '{"dataset":"d1","keys":["sku"]}',
      }),
    ).toEqual({ allowed: true });
    expect(
      gate({ id: "d2", kind: "deduperows", summary: "No keys.", risk: "read", options: '{"dataset":"d1","keys":[]}' }),
    ).toMatchObject({ allowed: false });
    expect(
      gate({
        id: "m1",
        kind: "mergepages",
        summary: "Merge the pages.",
        risk: "read",
        options: '{"datasets":["d1","d2"]}',
      }),
    ).toEqual({ allowed: true });
    expect(
      gate({ id: "m2", kind: "mergepages", summary: "One dataset.", risk: "read", options: '{"datasets":["d1"]}' }),
    ).toEqual({ allowed: true });
    expect(gate({ id: "m3", kind: "mergepages", summary: "No datasets.", risk: "read" })).toMatchObject({
      allowed: false,
    });
    expect(
      gate({
        id: "a1",
        kind: "stamplerows",
        summary: "Stamp the rows.",
        risk: "read",
        options: '{"dataset":"d1","url":"https://example.com/list"}',
      }),
    ).toEqual({ allowed: true });
    expect(
      gate({
        id: "g1",
        kind: "previewgrid",
        summary: "Preview the grid.",
        risk: "read",
        options: '{"dataset":"d1","sample":25}',
      }),
    ).toEqual({ allowed: true });
    expect(
      gate({
    ...
    expect(
      gate({
        id: "r1",
        kind: "resumeextract",
        summary: "Resume the extraction.",
        risk: "sensitive",
        options: '{"session":"e1"}',
      }),
    ).toEqual({ allowed: true });
    expect(gate({ id: "r2", kind: "resumeextract", summary: "No session id.", risk: "sensitive" })).toMatchObject({
      allowed: false,
    });
    expect(
      gate({
        ...
        risk: "read",
        options: '{"artifact":"catalog.csv"}',
      }),
    ).toEqual({ allowed: true });
    expect(gate({ id: "v2", kind: "logprovenance", summary: "No artifact.", risk: "read" })).toMatchObject({
      allowed: false,
    });
  });
});
...
describe("files, clipboard and downloads policy", () => {
  ...
  it("validates the downloadspec, mimefilter and cleanuprule grammars with no code ceilings", () => {
    expect(
      validatedownloadspec({
        urls: ["https://example.com/a.pdf", "https://example.com/b.zip"],
        filename: "report",
        complete: "checksum",
      }),
    ).toEqual({ allowed: true });
    expect(validatedownloadspec({ urls: [] })).toMatchObject({ allowed: false });
    expect(validatedownloadspec({ urls: ["http://example.com/a.pdf"] })).toMatchObject({
      allowed: false,
      reason: expect.stringContaining("HTTPS"),
    });
    expect(validatedownloadspec({ urls: ["https://example.com/a.pdf"], complete: "sha" })).toMatchObject({
      allowed: false,
    });
    expect(validatemimefilter({ include: ["application/pdf*"], exclude: [], default: "deny" })).toEqual({
      allowed: true,
    });
    expect(validatemimefilter({ exclude: [], default: "deny" })).toMatchObject({
      allowed: false,
      reason: expect.stringContaining("include"),
    });
    expect(validatemimefilter({ include: ["text/*"], exclude: [], default: "block" })).toMatchObject({
      allowed: false,
      reason: expect.stringContaining("deny or allow"),
    });
    expect(validatecleanuprule({ age: 86_400_000, kind: "any", keep: "latest" })).toEqual({ allowed: true });
    expect(validatecleanuprule({ age: 0, kind: "any", keep: "none" })).toMatchObject({
      allowed: false,
      reason: expect.stringContaining("no code ceiling"),
    });
  ...
  it("validates the files grammar: batch references, consent refs, capture naming and cleanup rules", () => {
    expect(
      gate({
        id: "b1",
        kind: "batchdownload",
        summary: "Download the batch.",
        risk: "sensitive",
        options: JSON.stringify({
          downloadspec: { urls: ["https://example.com/a.pdf"], filename: "report", complete: "size" },
          concurrent: 25,
        }),
      }),
    ).toEqual({ allowed: true });
    expect(gate({ id: "b2", kind: "batchdownload", summary: "No spec.", risk: "sensitive" })).toMatchObject({
      allowed: false,
      reason: expect.stringContaining("downloadspec"),
    });
    expect(
      gate({
        id: "b3",
        kind: "batchdownload",
        summary: "Zero concurrent window.",
        risk: "sensitive",
        options: '{"downloadspec":{"urls":["https://example.com/a.pdf"]},"concurrent":0}',
      }),
    ).toMatchObject({ allowed: false, reason: expect.stringContaining("concurrent") });
    expect(
      gate({
        id: "b4",
        kind: "batchdownload",
        summary: "Huge batch is a user choice.",
        risk: "sensitive",
        options:
          '{"downloadspec":{"urls":["https://example.com/a.pdf","https://example.com/b.pdf"]},"concurrent":10000}',
      }),
    ).toEqual({ allowed: true });
    expect(
      gate({ id: "p1", kind: "pausedownload", value: "report.pdf", summary: "Pause the download.", risk: "sensitive" }),
    ).toEqual({ allowed: true });
    expect(
      gate({ id: "p2", kind: "pausedownload", summary: "Pause without a reference.", risk: "sensitive" }),
    ).toMatchObject({ allowed: false, reason: "A reviewed value is required." });
    expect(
      gate({
        id: "v1",
        ...
        risk: "read",
        options: '{"bytes":2048,"checksum":"fnv1a-abc"}',
      }),
    ).toEqual({ allowed: true });
    expect(
      gate({
    ...
    expect(
      gate({
        id: "i1",
        kind: "interceptmime",
        summary: "Intercept pdf downloads.",
        risk: "sensitive",
        options: '{"mimefilter":{"include":["application/pdf*"],"exclude":[],"default":"deny"}}',
      }),
    ).toEqual({ allowed: true });
    expect(gate({ id: "i2", kind: "interceptmime", summary: "Missing filter.", risk: "sensitive" })).toMatchObject({
      allowed: false,
      reason: expect.stringContaining("mimefilter"),
    });
    expect(
      gate({
        id: "n1",
        kind: "namecaptures",
        summary: "Name the captures.",
        risk: "read",
        options: '{"task":"task-1","steps":["snap","peek"],"extension":"png"}',
      }),
    ).toEqual({ allowed: true });
    expect(gate({ id: "n2", kind: "namecaptures", summary: "No task.", risk: "read" })).toMatchObject({
      allowed: false,
      reason: "A reviewed task id is required in options for capture naming.",
    });
    expect(
      gate({
        id: "c1",
        kind: "cleanupartifacts",
        summary: "Sweep old artifacts.",
        risk: "sensitive",
        options: '{"rules":[{"age":60000,"kind":"any","keep":"latest"}]}',
      }),
    ).toEqual({ allowed: true });
    expect(
      gate({ id: "c2", kind: "cleanupartifacts", summary: "Empty rules.", risk: "sensitive", options: '{"rules":[]}' }),
    ).toMatchObject({ allowed: false });
    expect(
      gate({
        id: "w1",
        kind: "writeclipboard",
        value: "Reviewed summary text.",
        summary: "Write the clipboard.",
        risk: "sensitive",
      }),
    ).toEqual({ allowed: true });
    expect(
      gate({
        ...
        kind: "scanvirus",
        value: "q-1",
        summary: "Scan the quarantined file.",
        risk: "sensitive",
        options: '{"scanner":"clamav"}',
      }),
    ).toEqual({ allowed: true });
    expect(
      gate({
        id: "e1",
        kind: "exportnetlog",
        summary: "Export the netlog.",
        risk: "read",
        options: '{"stepid":"nav-1"}',
      }),
    ).toEqual({ allowed: true });
  });
  ...
  it("requires an approved consent prompt per clipboard read", () => {
    ...
    expect(clipboardconsentgranted(read)).toMatchObject({
      allowed: false,
      reason: "A clipboard read requires a reviewed consent ref in options.",
    });
    expect(gate(read)).toMatchObject({
      allowed: false,
      reason: "A clipboard read requires a reviewed consent ref in options.",
    });
  ...
  it("refuses download interception outside the session origin grants and gates downloads by origin", () => {
    ...
    expect(gate(intercept, { ...session, grants: ["https://other.example"] })).toMatchObject({
      allowed: false,
      reason: "The download interception is outside the session origin grants.",
    });
    expect(gate(intercept)).toEqual({ allowed: true });
    expect(downloadgranted(session, "https://example.com/file.pdf")).toEqual({ allowed: true });
    expect(
      downloadgranted({ ...session, grants: ["https://data.example"] }, "https://example.com/file.pdf"),
    ).toMatchObject({ allowed: false, reason: expect.stringContaining("origin grants") });
    expect(downloadgranted(session, "not a url")).toMatchObject({
      allowed: false,
      reason: "The reviewed download URL is invalid.",
    });
  });
  ...
  it("releases quarantined files only after a clean scan verdict", () => {
    ...
    expect(quarantinereleasegranted({ ...base, scan: "pending" })).toMatchObject({
      allowed: false,
...
describe("media capture policy", () => {
  ...
  it("grades every capture kind read only and validates the options payload of every capture kind", () => {
    ...
    expect(
      gate({
        id: "v1",
        kind: "shotview",
        summary: "Shoot the viewport.",
        risk: "read",
        options: '{"capture":{"format":"png","quality":92,"pixelratio":2,"annotate":true,"exporttarget":"memory"}}',
      }),
    ).toEqual({ allowed: true });
    expect(
      gate({
        id: "v2",
        kind: "shotview",
        summary: "Shoot with a huge ratio.",
        risk: "read",
        options: '{"capture":{"pixelratio":8}}',
      }),
    ).toEqual({ allowed: true });
    expect(
      gate({
        ...
        summary: "Bad format.",
        risk: "read",
        options: '{"capture":{"format":"gif"}}',
      }),
    ).toMatchObject({ allowed: false, reason: "The reviewed capture format must be png, jpeg or webp." });
    expect(
      gate({
        id: "v4",
        kind: "shotview",
        summary: "Quality above the format range.",
        risk: "read",
        options: '{"capture":{"quality":140}}',
      }),
    ).toMatchObject({ allowed: false, reason: expect.stringContaining("no code cap") });
    expect(
      gate({
        id: "v5",
        kind: "shotview",
        summary: "Ratio below one.",
        risk: "read",
        options: '{"capture":{"pixelratio":0.5}}',
      }),
    ).toMatchObject({ allowed: false, reason: expect.stringContaining("no code ceiling") });
    expect(
      gate({
        id: "v6",
        kind: "shotview",
        summary: "Unknown export target.",
        risk: "read",
        options: '{"capture":{"exporttarget":"disk"}}',
      }),
    ).toMatchObject({ allowed: false, reason: expect.stringContaining("memory, download or clipboard") });
    expect(
      gate({
        id: "f1",
        kind: "shotfullpage",
        summary: "Shoot the full page.",
        risk: "read",
        options: '{"settle":200,"overlap":40,"wait":60000}',
      }),
    ).toEqual({ allowed: true });
    expect(
      gate({ id: "f2", kind: "shotfullpage", summary: "Negative settle.", risk: "read", options: '{"settle":-1}' }),
    ).toMatchObject({ allowed: false, reason: expect.stringContaining("settle") });
    expect(
      gate({
        id: "f3",
        kind: "shotfullpage",
        summary: "Fractional overlap.",
        risk: "read",
        options: '{"overlap":1.5}',
      }),
    ).toMatchObject({ allowed: false, reason: expect.stringContaining("overlap") });
    expect(
      gate({
        id: "e1",
        kind: "shotelement",
        target: "#card",
        summary: "Shoot the element.",
        risk: "read",
        options: '{"capture":{"format":"webp","exporttarget":"clipboard"}}',
      }),
    ).toEqual({ allowed: true });
    expect(gate({ id: "e2", kind: "shotelement", summary: "Element without a target.", risk: "read" })).toMatchObject({
      allowed: false,
      reason: "A page target is required.",
    });
    expect(
      gate({
        id: "s1",
        kind: "shotregion",
        summary: "Shoot the reviewed region.",
        risk: "read",
        options: '{"regionrect":{"x":10,"y":20,"width":400,"height":300},"reviewed":true}',
      }),
    ).toEqual({ allowed: true });
    expect(
      gate({
        id: "s2",
        kind: "shotregion",
        summary: "Region without the reviewed flag.",
        risk: "read",
        options: '{"regionrect":{"x":10,"y":20,"width":400,"height":300}}',
      }),
    ).toMatchObject({
      allowed: false,
    ...
    expect(
      gate({
        id: "s3",
        kind: "shotregion",
        summary: "Negative coordinates.",
        risk: "read",
        options: '{"regionrect":{"x":-5,"y":20,"width":400,"height":300},"reviewed":true}',
      }),
    ).toMatchObject({ allowed: false, reason: "The reviewed regionrect refuses negative coordinates." });
    expect(
      gate({
        id: "s4",
        kind: "shotregion",
        summary: "Empty rect.",
        risk: "read",
        options: '{"regionrect":{"x":0,"y":0,"width":0,"height":10},"reviewed":true}',
      }),
    ).toMatchObject({ allowed: false, reason: expect.stringContaining("positive width and height") });
    expect(
      gate({
        id: "s5",
        kind: "shotregion",
        ...
        risk: "read",
        options: '{"regionrect":{"x":0,"y":0,"width":600,"height":400},"reviewed":true,"container":"#feed","steps":9}',
      }),
    ).toEqual({ allowed: true });
    expect(
      gate({
        id: "c1",
        kind: "contactsheet",
        summary: "Tile the element captures.",
        risk: "read",
        options: '{"elements":["#a","#b","#c"],"sheet":{"cellsize":240,"columns":4,"label":"both"}}',
      }),
    ).toEqual({ allowed: true });
    expect(gate({ id: "c2", kind: "contactsheet", summary: "No elements.", risk: "read" })).toMatchObject({
      allowed: false,
      reason: expect.stringContaining("element selectors"),
    });
    expect(
      gate({
        id: "c3",
        kind: "contactsheet",
        summary: "Bad label style.",
        risk: "read",
        options: '{"elements":["#a"],"sheet":{"cellsize":240,"columns":3,"label":"fancy"}}',
      }),
    ).toMatchObject({ allowed: false, reason: expect.stringContaining("label style") });
    expect(
      gate({
        id: "c4",
        kind: "contactsheet",
        summary: "Huge cell counts stay a user choice.",
        risk: "read",
        options: '{"elements":["#a","#b"],"sheet":{"cellsize":800,"columns":50,"label":"index"}}',
      }),
    ).toEqual({ allowed: true });
  });
  ...
  it("requires the active tab grant of the live session for every capture kind", () => {
    ...
    expect(capturegate(session, 9, "https://example.com", now)).toMatchObject({
      allowed: false,
      reason: expect.stringContaining("active tab grant"),
    });
    expect(capturegate({ ...session, grants: ["https://other.example"] }, 4, "https://example.com", now)).toMatchObject(
      { allowed: false, reason: expect.stringContaining("session origin grants") },
    );
    expect(capturegate({ ...session, stoppedat: now }, 4, "https://example.com", now)).toMatchObject({
      allowed: false,
    });
    expect(capturegate({ ...session, pausedat: now - 1 }, 4, "https://example.com", now)).toMatchObject({
      allowed: false,
      reason: expect.stringContaining("paused"),
    });
    expect(capturegate({ ...session, expiresat: now - 1 }, 4, "https://example.com", now)).toMatchObject({
      allowed: false,
    });
    ...
    expect(
      canexecute({
        session: { ...session, grants: ["https://other.example"] },
        plan: {
          id: "p1",
          objective: "Shoot",
          origin: "https://example.com",
    ...
    ).toMatchObject({ allowed: false, reason: expect.stringContaining("session origin grants") });
  });
  ...
  it("routes capture export targets, keeps stitch budgets inside the reviewed wait window and exposes retention", () => {
    ...
    expect(captureexportgranted("clipboard")).toMatchObject({
      allowed: true,
      reason: expect.stringContaining("clipboardwrite"),
    });
    expect(captureexportgranted("download")).toMatchObject({
      allowed: true,
      reason: expect.stringContaining("reviewed download flow"),
    });
    expect(captureexportgranted("disk" as never)).toMatchObject({
      allowed: false,
      reason: expect.stringContaining("memory, download or clipboard"),
    });
    expect(stitchbudgetallowed(4, 150, 600)).toEqual({ allowed: true });
    expect(stitchbudgetallowed(4, 150, 599)).toMatchObject({
      allowed: false,
      reason: expect.stringContaining("exceeds the reviewed wait window"),
    });
  ...
  it("validates capture naming rules against the allowed segment set and wraps any existing action kind with state pairs", () => {
    ...
    expect(validatecapturenaming({ run: true, pixelratio: true })).toMatchObject({
      allowed: false,
      reason: expect.stringContaining("unknown pixelratio segment"),
    });
    expect(validatecapturenaming({ run: false, step: false, sequence: false, kind: false })).toMatchObject({
      allowed: false,
      reason: expect.stringContaining("at least one enabled segment"),
    });
    ...
    expect(validateregionrect({ x: 0, y: -2, width: 10, height: 10 })).toMatchObject({
      allowed: false,
      reason: "The reviewed regionrect refuses negative coordinates.",
    });
...
describe("media capture part two policy", () => {
  const gate = (step: toolstep): policyevaluation => validatestep(step, "https://example.com");
  const now = 1_800_000_000_000;
  const mediasession: agentsession = {
    id: "session",
    tabid: 4,
    origin: "https://example.com",
    startedat: now,
  ...
  it("requires the active tab grant of the live session for every media kind", () => {
    ...
    expect(mediagate(mediasession, 9, "https://example.com", now)).toMatchObject({
      allowed: false,
      reason: expect.stringContaining("active tab grant"),
    });
    expect(
      mediagate({ ...mediasession, grants: ["https://other.example"] }, 4, "https://example.com", now),
    ).toMatchObject({ allowed: false, reason: expect.stringContaining("session origin grants") });
    expect(mediagate({ ...mediasession, pausedat: now - 1 }, 4, "https://example.com", now)).toMatchObject({
      allowed: false,
      reason: expect.stringContaining("paused"),
    });
    expect(mediagate({ ...mediasession, expiresat: now - 1 }, 4, "https://example.com", now)).toMatchObject({
      allowed: false,
    });
    ...
    expect(
      canexecute({
        session: { ...mediasession, grants: ["https://other.example"] },
        plan: {
          id: "p1",
          objective: "Read",
          origin: "https://example.com",
  ...
  it("requires a reviewed consent ref before any recording starts and exposes the duration window", () => {
    const recording: toolstep = {
      id: "r1",
      kind: "recordscreen",
      summary: "Record the tab.",
      risk: "sensitive",
      options: '{"consentref":"consent-1"}',
    };
    expect(recordingconsentgranted(recording)).toEqual({ allowed: true });
    expect(recordingconsentgranted({ ...recording, options: '{"fps":2}' })).toMatchObject({
      allowed: false,
      reason: expect.stringContaining("consent ref"),
    });
  ...
  it("validates the pdf grammar: paper sizes, margins, scale, break points and the memory or download export route", () => {
    expect(
      gate({
        id: "pdf1",
        kind: "capturepdf",
        summary: "Print the report.",
        risk: "read",
        options:
          '{"pdf":{"paperwidth":8.5,"paperheight":11,"margins":{"top":0.5,"right":0.5,"bottom":0.5,"left":0.5},"scale":1.2,"landscape":true,"paginate":true},"breakpoints":["#section-2"]}',
      }),
    ).toEqual({ allowed: true });
    expect(
      gate({
        id: "pdf2",
        kind: "capturepdf",
        summary: "Huge paper stays a user choice.",
        risk: "read",
        options: '{"pdf":{"paperwidth":40,"paperheight":80,"scale":4}}',
      }),
    ).toEqual({ allowed: true });
    expect(
      gate({
        ...
        risk: "read",
        options: '{"pdf":{"margins":{"top":-0.2}}}',
      }),
    ).toMatchObject({ allowed: false, reason: expect.stringContaining("negative margins are refused") });
    expect(
      gate({
        id: "pdf4",
        kind: "capturepdf",
        summary: "Zero paper width.",
        risk: "read",
        options: '{"pdf":{"paperwidth":0}}',
      }),
    ).toMatchObject({ allowed: false, reason: expect.stringContaining("paper width") });
    expect(
      gate({ id: "pdf5", kind: "capturepdf", summary: "Bad scale.", risk: "read", options: '{"pdf":{"scale":-1}}' }),
    ).toMatchObject({ allowed: false, reason: expect.stringContaining("scale") });
    expect(
      gate({
        id: "pdf6",
        kind: "capturepdf",
        summary: "Clipboard route.",
        risk: "read",
        options: '{"exporttarget":"clipboard"}',
      }),
    ).toMatchObject({ allowed: false, reason: expect.stringContaining("memory or download") });
    expect(
      gate({
        id: "pdf7",
        kind: "capturepdf",
        summary: "Empty break points.",
        risk: "read",
        options: '{"breakpoints":[]}',
      }),
    ).toMatchObject({ allowed: false, reason: expect.stringContaining("break points") });
    expect(
      gate({
        id: "pdf8",
        kind: "capturepdf",
  ...
  it("validates the recording, frame, canvas and stream grammars with no code ceilings", () => {
    expect(
      gate({
        ...
        summary: "Record the tab.",
        risk: "sensitive",
        options: '{"recording":{"scope":"run","fps":30,"bitrate":8000,"audio":true},"duration":5000,"consentref":"c1"}',
      }),
    ).toEqual({ allowed: true });
    expect(
      gate({
        ...
        summary: "Bad scope.",
        risk: "sensitive",
        options: '{"recording":{"scope":"screen"},"consentref":"c1"}',
      }),
    ).toMatchObject({ allowed: false, reason: expect.stringContaining("scope must be tab or run") });
    expect(
      gate({
        id: "rec3",
        kind: "recordscreen",
        summary: "Zero fps.",
        risk: "sensitive",
        options: '{"recording":{"fps":0},"consentref":"c1"}',
      }),
    ).toMatchObject({ allowed: false, reason: expect.stringContaining("fps") });
    expect(
      gate({
        id: "rec4",
        kind: "recordscreen",
        summary: "No consent.",
        risk: "sensitive",
        options: '{"recording":{"scope":"tab"}}',
      }),
    ).toMatchObject({ allowed: false, reason: expect.stringContaining("consent ref") });
    expect(
      gate({
        id: "rec5",
        kind: "captureaudio",
        summary: "Record the audio.",
        risk: "sensitive",
        options: '{"recording":{"audio":true},"duration":1000,"consentref":"c2"}',
      }),
    ).toEqual({ allowed: true });
    expect(
      gate({
        ...
        summary: "Grab the frame.",
        risk: "read",
        options: '{"timestamp":12.5,"poster":true,"capture":{"format":"webp"}}',
      }),
    ).toEqual({ allowed: true });
    expect(
      gate({
    ...
    ).toMatchObject({ allowed: false, reason: expect.stringContaining("timestamp") });
    expect(
      gate({ id: "frame3", kind: "captureframe", summary: "No target.", risk: "read", options: '{"timestamp":1}' }),
    ).toMatchObject({ allowed: false, reason: "A page target is required." });
    expect(
      gate({
        id: "can1",
        kind: "shotcanvas",
        target: "canvas.chart",
        summary: "Shoot the canvas.",
        risk: "read",
        options: '{"capture":{"format":"png"}}',
      }),
    ).toEqual({ allowed: true });
    expect(gate({ id: "can2", kind: "shotcanvas", summary: "No target.", risk: "read" })).toMatchObject({
      allowed: false,
      reason: "A page target is required.",
    });
    expect(gate({ id: "stream1", kind: "probestream", summary: "Probe the streams.", risk: "read" })).toEqual({
      allowed: true,
    });
    expect(
      gate({
        id: "stream2",
        kind: "probestream",
        summary: "Empty selector.",
        risk: "read",
        options: '{"selector":" "}',
      }),
    ).toMatchObject({ allowed: false, reason: expect.stringContaining("selector") });
    expect(gate({ id: "media1", kind: "readmedia", summary: "Read the sources.", risk: "read" })).toEqual({
      allowed: true,
    });
    expect(gate({ id: "assets1", kind: "readassets", summary: "Read the assets.", risk: "read" })).toEqual({
      allowed: true,
    });
  });
  ...
  it("validates the image filter, lapse budget, conversion and thumbnail grammars", () => {
    expect(
      gate({
        id: "img1",
        kind: "downloadimages",
        summary: "Download the images.",
        risk: "sensitive",
        options:
          '{"imagefilter":{"selector":"main img","minwidth":200,"minheight":100,"formats":["png","image/webp"]},"naming":{"run":true,"step":true,"sequence":true,"kind":true}}',
      }),
    ).toEqual({ allowed: true });
    expect(gate({ id: "img2", kind: "downloadimages", summary: "No filter.", risk: "sensitive" })).toMatchObject({
      allowed: false,
      reason: expect.stringContaining("imagefilter is required"),
    });
    expect(
      gate({
        id: "img3",
        ...
        summary: "Empty selector.",
        risk: "sensitive",
        options: '{"imagefilter":{"selector":""}}',
      }),
    ).toMatchObject({ allowed: false, reason: expect.stringContaining("selector grammar") });
    expect(
      gate({
        id: "img4",
        kind: "downloadimages",
        summary: "Negative width.",
        risk: "sensitive",
        options: '{"imagefilter":{"minwidth":-1}}',
      }),
    ).toMatchObject({ allowed: false, reason: expect.stringContaining("minimum width") });
    expect(
      gate({
        id: "img5",
        kind: "downloadimages",
        summary: "Empty formats.",
        risk: "sensitive",
        options: '{"imagefilter":{"formats":[]}}',
      }),
    ).toMatchObject({ allowed: false, reason: expect.stringContaining("format list") });
    expect(
      gate({
        id: "lapse1",
        kind: "timelapse",
        summary: "Shoot the lapse.",
        risk: "read",
        options: '{"lapse":{"interval":500,"duration":5000,"format":"webp"},"wait":6000}',
      }),
    ).toEqual({ allowed: true });
    expect(
      gate({
        id: "lapse2",
        kind: "timelapse",
        summary: "Duration over budget.",
        risk: "read",
        options: '{"lapse":{"interval":500,"duration":5000},"wait":4000}',
      }),
    ).toMatchObject({ allowed: false, reason: expect.stringContaining("exceeds the reviewed wait budget") });
    expect(
      gate({
        id: "lapse3",
        kind: "timelapse",
        summary: "Negative interval.",
        risk: "read",
        options: '{"lapse":{"interval":-500,"duration":5000}}',
      }),
    ).toMatchObject({ allowed: false, reason: expect.stringContaining("interval") });
    expect(gate({ id: "lapse4", kind: "timelapse", summary: "No plan.", risk: "read" })).toMatchObject({
      allowed: false,
      reason: expect.stringContaining("lapse plan"),
    });
    ...
    expect(
      gate({
        id: "conv1",
        kind: "convertimage",
        summary: "Convert the capture.",
        risk: "read",
        options: '{"capture":"cap-1","convert":{"source":"png","target":"webp","quality":85}}',
      }),
    ).toEqual({ allowed: true });
    expect(
      gate({
        id: "conv2",
        kind: "convertimage",
        summary: "List of captures.",
        risk: "read",
        options: '{"captures":["cap-1","cap-2"],"convert":{"target":"jpeg"}}',
      }),
    ).toEqual({ allowed: true });
    expect(
      gate({
        ...
        summary: "Bad target.",
        risk: "read",
        options: '{"capture":"cap-1","convert":{"target":"gif"}}',
      }),
    ).toMatchObject({ allowed: false, reason: expect.stringContaining("conversion target") });
    expect(
      gate({
        id: "conv4",
        kind: "convertimage",
        summary: "No capture.",
        risk: "read",
        options: '{"convert":{"target":"png"}}',
      }),
    ).toMatchObject({ allowed: false, reason: expect.stringContaining("capture id") });
    expect(
      gate({
        id: "conv5",
        kind: "convertimage",
        summary: "Both id and list.",
        risk: "read",
        options: '{"capture":"cap-1","captures":["cap-2"],"convert":{"target":"png"}}',
      }),
    ).toMatchObject({ allowed: false, reason: expect.stringContaining("not both") });
    expect(
      gate({
        id: "thumb1",
        kind: "makethumbs",
        summary: "Thumb the captures.",
        risk: "read",
        options: '{"captures":["cap-1"],"thumb":{"size":320,"fit":"cover","suffix":"thumb"}}',
      }),
    ).toEqual({ allowed: true });
    expect(
      gate({
        id: "thumb2",
        kind: "makethumbs",
        summary: "Fixed size set stays a user choice.",
        risk: "read",
        options: '{"capture":"cap-1","thumb":{"size":1024,"fit":"contain","suffix":"small"}}',
      }),
    ).toEqual({ allowed: true });
    expect(
      gate({
        ...
        summary: "Bad fit.",
        risk: "read",
        options: '{"capture":"cap-1","thumb":{"size":320,"fit":"stretch","suffix":"t"}}',
      }),
    ).toMatchObject({ allowed: false, reason: expect.stringContaining("fit") });
    expect(
      gate({
        id: "thumb4",
        kind: "makethumbs",
        summary: "Zero size.",
        risk: "read",
        options: '{"capture":"cap-1","thumb":{"size":0,"fit":"cover","suffix":"t"}}',
      }),
    ).toMatchObject({ allowed: false, reason: expect.stringContaining("size") });
  });
});
...
describe("network observation part one policy", () => {
  const gate = (step: toolstep): policyevaluation => validatestep(step, "https://example.com");
  const now = 1_800_000_000_000;
  const httpsession: agentsession = {
    id: "session",
    tabid: 4,
    origin: "https://example.com",
    startedat: now,
  ...
  const fetchplan = (step: toolstep): agentplan => ({
    id: "p1",
    objective: "Fetch",
    origin: "https://example.com",
    steps: [step],
  ...
  it("grades the http family: fetch and parsing read only while typed calls stay sensitive", () => {
    ...
    const restget: toolstep = {
      id: "r1",
      kind: "callrest",
      summary: "Read the endpoint.",
      risk: "sensitive",
      options: '{"endpoint":"issues"}',
    };
    const restpost: toolstep = {
      id: "r2",
      kind: "callrest",
      summary: "Create the issue.",
      risk: "sensitive",
      options: '{"endpoint":"create","method":"POST"}',
    };
    const mutation: toolstep = {
      id: "g1",
      kind: "callgraphql",
      summary: "Mutate.",
      risk: "sensitive",
      options: '{"endpoint":"graph","graphql":{"query":"mutation{like}","operationkind":"mutation"}}',
    };
    const query: toolstep = {
      id: "g2",
      kind: "callgraphql",
      summary: "Query.",
      risk: "sensitive",
      options: '{"endpoint":"graph","graphql":{"query":"query{hero}","operationkind":"query"}}',
    };
  ...
  it("restricts every outbound request to granted origins through origincheck", () => {
    ...
    expect(origincheck({ ...httpsession, grants: ["https://api.example"] }, "https://api.example/data")).toEqual({
      allowed: true,
    });
    expect(origincheck(httpsession, "https://api.example/data")).toMatchObject({
      allowed: false,
      reason: expect.stringContaining("outside the session origin grants"),
    });
    expect(origincheck(httpsession, "http://example.com/data")).toMatchObject({
      allowed: false,
      reason: expect.stringContaining("HTTPS"),
    });
    expect(origincheck(httpsession, "https://user:pass@example.com/data")).toMatchObject({
      allowed: false,
      reason: expect.stringContaining("credentials"),
    });
    expect(origincheck(httpsession, "not a url")).toMatchObject({
      allowed: false,
  ...
  it("requires header consent before any custom header leaves the extension", () => {
    ...
    const plain: toolstep = {
      ...
      summary: "Fetch.",
      risk: "read",
      options: '{"fetch":{"url":"https://example.com/a","headers":{"x-request-id":"42"}}}',
    };
    expect(fetchconsentrefgranted(plain)).toMatchObject({
      allowed: false,
      reason: expect.stringContaining("consent ref"),
    });
    expect(
      fetchconsentrefgranted({
        ...plain,
        options: '{"fetch":{"url":"https://example.com/a","headers":{"x-request-id":"42"}},"consentref":"c1"}',
      }),
    ).toEqual({ allowed: true });
    expect(
      fetchconsentrefgranted({
        ...
        summary: "No headers.",
        risk: "read",
        options: '{"fetch":{"url":"https://example.com/a"}}',
      }),
    ).toEqual({ allowed: true });
    expect(
      fetchconsentrefgranted({
        ...plain,
        options: '{"fetch":{"url":"https://example.com/a","headers":{"Authorization":"Bearer x"}}}',
      }),
    ).toMatchObject({ allowed: false, reason: expect.stringContaining("credential bearing header Authorization") });
    expect(
      fetchconsentrefgranted({
        ...plain,
        options: '{"fetch":{"url":"https://example.com/a","headers":{"Authorization":"Bearer x"}},"consentref":"c1"}',
      }),
    ).toEqual({ allowed: true });
  });
  ...
  it("covers fetch consents per origin with header names and an expiry window", () => {
    ...
    expect(fetchconsentcovers({ ...consent, expiresat: now - 1 }, "https://api.example", ["x-api-key"], now)).toBe(
      false,
    );
  ...
  it("keeps fetch waits inside the reviewed wait budget with no ceiling on the bounds themselves", () => {
    ...
    expect(fetchbudgetallowed(1000, 2, 500, 4000)).toMatchObject({
      allowed: false,
      reason: expect.stringContaining("exceeds the reviewed wait budget"),
    });
    ...
    expect(fetchbudgetallowed(-1, 0, 0, 100)).toMatchObject({
      allowed: false,
      reason: expect.stringContaining("timeout"),
    });
    expect(fetchbudgetallowed(1000, 2, 500, -5)).toMatchObject({
      allowed: false,
      reason: expect.stringContaining("wait budget"),
    });
  });
  ...
  it("validates the fetch, stream and consent grammar of fetchurl", () => {
    expect(
      gate({
        id: "f1",
        kind: "fetchurl",
        summary: "Fetch a reviewed url.",
        risk: "read",
        options:
          '{"fetch":{"url":"https://api.example/data","method":"POST","headers":{"x-a":"1"},"body":"{}","mode":"cors"},"fetchoptions":{"timeout":1000,"retries":2,"backoff":100,"follow":3},"stream":{"budget":4096},"wait":5000,"consentref":"c1"}',
      }),
    ).toEqual({ allowed: true });
    expect(gate({ id: "f2", kind: "fetchurl", summary: "No request.", risk: "read" })).toMatchObject({
      allowed: false,
      reason: expect.stringContaining("options.fetch"),
    });
    expect(
      gate({ id: "f3", kind: "fetchurl", summary: "Empty url.", risk: "read", options: '{"fetch":{"url":" "}}' }),
    ).toMatchObject({ allowed: false, reason: expect.stringContaining("non-empty url") });
    expect(
      gate({
        id: "f4",
        kind: "fetchurl",
        summary: "Bad verb.",
        risk: "read",
        options: '{"fetch":{"url":"https://a.example","method":"TRACE"}}',
      }),
    ).toMatchObject({ allowed: false, reason: expect.stringContaining("known HTTP verb") });
    expect(
      gate({
        id: "f5",
        kind: "fetchurl",
        summary: "Empty header name.",
        risk: "read",
        options: '{"fetch":{"url":"https://a.example","headers":{" ":"v"}}}',
      }),
    ).toMatchObject({ allowed: false, reason: expect.stringContaining("empty names") });
    expect(
      gate({
        id: "f6",
        kind: "fetchurl",
        summary: "Bad mode.",
        risk: "read",
        options: '{"fetch":{"url":"https://a.example","mode":"navigate"}}',
      }),
    ).toMatchObject({ allowed: false, reason: expect.stringContaining("cors, no-cors or same-origin") });
    expect(
      gate({
        id: "f7",
        kind: "fetchurl",
        summary: "No consent.",
        risk: "read",
        options: '{"fetch":{"url":"https://a.example","headers":{"x-a":"1"}}}',
      }),
    ).toMatchObject({ allowed: false, reason: expect.stringContaining("consent ref") });
    expect(
      gate({
        id: "f8",
        kind: "fetchurl",
        summary: "Fractional retries.",
        risk: "read",
        options: '{"fetch":{"url":"https://a.example"},"fetchoptions":{"retries":1.5}}',
      }),
    ).toMatchObject({ allowed: false, reason: expect.stringContaining("retries") });
    expect(
      gate({
        id: "f9",
        kind: "fetchurl",
        summary: "Negative stream budget.",
        risk: "read",
        options: '{"fetch":{"url":"https://a.example"},"stream":{"budget":-1}}',
      }),
    ).toMatchObject({ allowed: false, reason: expect.stringContaining("byte budget") });
    expect(
      gate({
        id: "f10",
        kind: "fetchurl",
        summary: "Budget over wait.",
        risk: "read",
        options: '{"fetch":{"url":"https://a.example"},"fetchoptions":{"timeout":1000,"retries":2},"wait":2000}',
      }),
    ).toMatchObject({ allowed: false, reason: expect.stringContaining("exceeds the reviewed wait budget") });
  });
  ...
  it("validates the parsejson and parsehtml grammar with stored call ids", () => {
    expect(
      gate({
        id: "j1",
        kind: "parsejson",
        summary: "Parse the body.",
        risk: "read",
        options:
          '{"call":"call-1","fields":[{"name":"title","path":"data.title","kind":"text"},{"name":"count","path":"data.items.0.count","kind":"number","default":0}]}',
      }),
    ).toEqual({ allowed: true });
    expect(
      gate({
        ...
        summary: "No call.",
        risk: "read",
        options: '{"fields":[{"name":"a","path":"b"}]}',
      }),
    ).toMatchObject({ allowed: false, reason: expect.stringContaining("options.call") });
    expect(
      gate({ id: "j3", kind: "parsejson", summary: "No fields.", risk: "read", options: '{"call":"call-1"}' }),
    ).toMatchObject({ allowed: false, reason: expect.stringContaining("json path rules") });
    expect(
      gate({
        id: "j4",
        kind: "parsejson",
        summary: "Bad path.",
        risk: "read",
        options: '{"call":"c","fields":[{"name":"a","path":"data..title"}]}',
      }),
    ).toMatchObject({ allowed: false, reason: expect.stringContaining("dotted path") });
    expect(
      gate({
        id: "j5",
        kind: "parsejson",
        summary: "Bad kind.",
        risk: "read",
        options: '{"call":"c","fields":[{"name":"a","path":"b","kind":"yaml"}]}',
      }),
    ).toMatchObject({ allowed: false, reason: expect.stringContaining("text, number, boolean or json") });
    expect(
      gate({
        id: "h1",
        kind: "parsehtml",
        summary: "Query the markup.",
        risk: "read",
        options:
          '{"call":"call-1","queries":[{"selector":"a.link","attribute":"href","multi":true},{"selector":"h1"}]}',
      }),
    ).toEqual({ allowed: true });
    expect(
      gate({ id: "h2", kind: "parsehtml", summary: "No queries.", risk: "read", options: '{"call":"call-1"}' }),
    ).toMatchObject({ allowed: false, reason: expect.stringContaining("html queries") });
    expect(
      gate({
        id: "h3",
        kind: "parsehtml",
        summary: "Empty selector.",
        risk: "read",
        options: '{"call":"c","queries":[{"selector":""}]}',
      }),
    ).toMatchObject({ allowed: false, reason: expect.stringContaining("selector grammar") });
    expect(
      gate({
        ...
        summary: "Bad multi.",
        risk: "read",
        options: '{"call":"c","queries":[{"selector":"a","multi":"yes"}]}',
      }),
    ).toMatchObject({ allowed: false, reason: expect.stringContaining("multi flag") });
  });
  ...
  it("validates the typed call grammar with endpoints, graphql operations and api key references", () => {
    expect(
      gate({
        id: "r1",
        kind: "callrest",
        summary: "Call the endpoint.",
        risk: "sensitive",
        options:
          '{"endpoint":"issues","payload":{"owner":"w"},"method":"POST","success":[200,201],"apikeys":["github"],"fetchoptions":{"timeout":500}}',
      }),
    ).toEqual({ allowed: true });
    expect(gate({ id: "r2", kind: "callrest", summary: "No endpoint.", risk: "sensitive" })).toMatchObject({
      allowed: false,
      reason: expect.stringContaining("typed endpoint name"),
    });
    expect(
      gate({
        id: "r3",
        ...
        risk: "sensitive",
        options: '{"endpoint":"issues","payload":[]}',
      }),
    ).toMatchObject({ allowed: false, reason: expect.stringContaining("payload") });
    expect(
      gate({
        id: "r4",
        kind: "callrest",
        summary: "Bad success list.",
        risk: "sensitive",
        options: '{"endpoint":"issues","success":["200"]}',
      }),
    ).toMatchObject({ allowed: false, reason: expect.stringContaining("success status list") });
    expect(
      gate({
        id: "r5",
        kind: "callrest",
        summary: "Bad api key list.",
        risk: "sensitive",
        options: '{"endpoint":"issues","apikeys":[""]}',
      }),
    ).toMatchObject({ allowed: false, reason: expect.stringContaining("api key reference list") });
    expect(
      gate({
        id: "g1",
        kind: "callgraphql",
        ...
        risk: "sensitive",
        options:
          '{"endpoint":"graph","graphql":{"query":"query{hero}","operationkind":"query","variables":{"id":"1"},"operationname":"Hero"}}',
      }),
    ).toEqual({ allowed: true });
    expect(
      gate({
        id: "g2",
        kind: "callgraphql",
        summary: "No graphql request.",
        risk: "sensitive",
        options: '{"endpoint":"graph"}',
      }),
    ).toMatchObject({ allowed: false, reason: expect.stringContaining("options.graphql") });
    expect(
      gate({
        id: "g3",
        kind: "callgraphql",
        ...
        risk: "sensitive",
        options: '{"endpoint":"graph","graphql":{"query":"q","operationkind":"subscription"}}',
      }),
    ).toMatchObject({ allowed: false, reason: expect.stringContaining("unknown operation kinds are refused") });
    expect(
      gate({
        id: "g4",
        kind: "callgraphql",
        summary: "Empty operation text.",
        risk: "sensitive",
        options: '{"endpoint":"graph","graphql":{"query":" ","operationkind":"query"}}',
      }),
    ).toMatchObject({ allowed: false, reason: expect.stringContaining("operation text") });
  });
  ...
  it("requires payload schemas and clean header names in typed endpoint definitions", () => {
    expect(
      validateendpointrecord({
        name: "issues",
        method: "GET",
        url: "https://api.example/{owner}/issues",
    ...
    expect(validateendpointrecord({ name: "x", method: "GET", url: "https://api.example" })).toMatchObject({
      allowed: false,
      reason: expect.stringContaining("payload schema"),
    });
    expect(
      validateendpointrecord({
        name: "x",
    ...
    expect(
      validateendpointrecord({ name: "x", method: "GET", url: "https://api.example", schema: { fields: [] } }),
    ).toMatchObject({ allowed: false, reason: expect.stringContaining("non-empty field list") });
    expect(
      validateendpointrecord({
        name: "x",
        method: "GET",
        url: "https://api.example",
        schema: { fields: [{ name: "a", kind: "yaml" }] },
      }),
    ).toMatchObject({ allowed: false, reason: expect.stringContaining("string, number or boolean") });
  });

  it("gates execution on the origin grants and refuses ungranted outbound targets", () => {
    const fetchstep: toolstep = {
      id: "f1",
      kind: "fetchurl",
      summary: "Fetch the granted origin.",
      risk: "read",
      options: '{"fetch":{"url":"https://example.com/data"}}',
    };
    expect(
      canexecute({
        session: httpsession,
        plan: fetchplan(fetchstep),
        step: fetchstep,
        tabid: 4,
        origin: "https://example.com",
        now,
    ...
    const crossorigin: toolstep = {
      id: "f2",
      kind: "fetchurl",
      summary: "Fetch an ungranted origin.",
      risk: "read",
      options: '{"fetch":{"url":"https://api.example/data"}}',
    };
    expect(
      canexecute({
        session: httpsession,
        plan: fetchplan(crossorigin),
        step: crossorigin,
        tabid: 4,
        origin: "https://example.com",
        now,
      }),
    ).toMatchObject({ allowed: false, reason: expect.stringContaining("outside the session origin grants") });
    expect(
      canexecute({
        session: { ...httpsession, grants: ["https://api.example"] },
        plan: fetchplan(crossorigin),
        step: crossorigin,
        tabid: 4,
        origin: "https://example.com",
    ...
    const reststep: toolstep = {
      id: "r1",
      kind: "callrest",
      summary: "Call the typed endpoint.",
      risk: "sensitive",
      options: '{"endpoint":"issues"}',
    };
    expect(
    ...
    const withheaders: toolstep = {
      id: "f3",
      kind: "fetchurl",
      summary: "Fetch with headers.",
      risk: "read",
      options: '{"fetch":{"url":"https://example.com/data","headers":{"x-a":"1"}}}',
    };
    expect(
      canexecute({
        session: httpsession,
        plan: fetchplan(withheaders),
        step: withheaders,
        tabid: 4,
        origin: "https://example.com",
        now,
...
describe("network observation part two policy", () => {
  ...
  const plan = (step: toolstep): agentplan => ({
    id: "p2",
    objective: "Observe",
    origin: "https://example.com",
    steps: [step],
  ...
  it("grades the socket and watch family with conditional capturebodies and extractapi risks", () => {
    ...
    expect(resolvedrisk(socketstep("capturebodies", { body: { mimes: ["image/png", "application/json"] } }))).toBe(
      "sensitive",
    );
    ...
    expect(
      resolvedrisk(socketstep("extractapi", { replay: { endpoint: "https://api.example/items", verb: "GET" } })),
    ).toBe("read");
    expect(
      resolvedrisk(socketstep("extractapi", { replay: { endpoint: "https://api.example/items", verb: "POST" } })),
    ).toBe("sensitive");
  ...
  it("restricts channels to granted origins through socketgate", () => {
    ...
    expect(socketgate(httpsession, "wss://api.example/live")).toMatchObject({
      allowed: false,
      reason: expect.stringContaining("outside the session origin grants"),
    });
    expect(socketgate(grantedsession, "ws://api.example/live")).toMatchObject({
      allowed: false,
      reason: expect.stringContaining("wss websocket urls or https event stream urls"),
    });
    expect(socketgate(grantedsession, "http://api.example/stream")).toMatchObject({
      allowed: false,
      reason: expect.stringContaining("wss websocket urls or https event stream urls"),
    });
    expect(socketgate(grantedsession, "wss://user:pass@api.example/live")).toMatchObject({
      allowed: false,
      reason: expect.stringContaining("credentials"),
    });
    expect(socketgate(grantedsession, "not a url")).toMatchObject({
      allowed: false,
      reason: expect.stringContaining("valid url"),
    });
  });
  ...
  it("requires the webrequest grant before watchrequests runs and the host grant for observed origins", () => {
    ...
    expect(watchgate(httpsession, undefined, now)).toMatchObject({
      allowed: false,
      reason: expect.stringContaining("webrequest grant"),
    });
    expect(watchgate(httpsession, {}, now)).toMatchObject({
      allowed: false,
      reason: expect.stringContaining("webrequest grant"),
    });
    expect(watchgate({ ...httpsession, stoppedat: now }, { webrequestgrant: true }, now)).toMatchObject({
      allowed: false,
      reason: expect.stringContaining("No active browser session"),
    });
    expect(watchgate({ ...httpsession, pausedat: now - 1 }, { webrequestgrant: true }, now)).toMatchObject({
      allowed: false,
      reason: expect.stringContaining("paused"),
    });
    expect(watchgate({ ...httpsession, expiresat: now - 1 }, { webrequestgrant: true }, now)).toMatchObject({
      allowed: false,
    ...
    expect(observedorigingranted(httpsession, "https://api.example/items")).toMatchObject({
      allowed: false,
      reason: expect.stringContaining("outside the session origin grants"),
    });
    expect(observedorigingranted(grantedsession, "not a url")).toMatchObject({
      allowed: false,
      reason: expect.stringContaining("does not parse"),
    });
  });
  ...
  it("validates the socket grammar of every socket kind", () => {
    expect(
      validatestep(
        socketstep("opensocket", {
          socket: { url: "wss://api.example/live", reconnect: 2, backoff: 100, backoffceiling: 1000, lifetime: 5000 },
        }),
        "https://example.com",
      ),
    ).toEqual({ allowed: true });
    expect(validatestep(socketstep("opensocket", { socket: {} }), "https://example.com")).toMatchObject({
      allowed: false,
      reason: expect.stringContaining("options.socket"),
    });
    expect(
      validatestep(
        socketstep("opensocket", { socket: { url: "wss://api.example/live", reconnect: 1.5 } }),
        "https://example.com",
      ),
    ).toMatchObject({ allowed: false, reason: expect.stringContaining("reconnect budget") });
    expect(
      validatestep(
        socketstep("opensocket", { socket: { url: "wss://api.example/live", backoff: -1 } }),
        "https://example.com",
      ),
    ).toMatchObject({ allowed: false, reason: expect.stringContaining("backoff") });
    expect(
      validatestep(
        socketstep("opensocket", { socket: { url: "wss://api.example/live", lifetime: 0 } }),
        "https://example.com",
      ),
    ).toMatchObject({ allowed: false, reason: expect.stringContaining("lifetime window") });
    expect(
      validatestep(
        socketstep("sendmessage", { message: { channel: "ch1", stream: "orders", payload: '{"id":1}' } }),
        "https://example.com",
      ),
    ).toEqual({ allowed: true });
    expect(validatestep(socketstep("sendmessage", { message: { payload: "x" } }), "https://example.com")).toMatchObject(
      { allowed: false, reason: expect.stringContaining("channel") },
    );
    expect(
      validatestep(socketstep("sendmessage", { message: { channel: "ch1", stream: " " } }), "https://example.com"),
    ).toMatchObject({ allowed: false, reason: expect.stringContaining("stream name") });
    expect(
      validatestep(socketstep("sendmessage", { message: { channel: "ch1", payload: 3 } }), "https://example.com"),
    ).toMatchObject({ allowed: false, reason: expect.stringContaining("payload") });
    expect(
      validatestep(
        socketstep("waitmessage", { channel: "ch1", filter: { stream: "orders", path: "id", limit: 2 }, wait: 500 }),
        "https://example.com",
      ),
    ).toEqual({ allowed: true });
    expect(
      validatestep(socketstep("waitmessage", { filter: { path: "bad path" } }), "https://example.com"),
    ).toMatchObject({ allowed: false, reason: expect.stringContaining("dotted path") });
    expect(validatestep(socketstep("waitmessage", { filter: { limit: 0 } }), "https://example.com")).toMatchObject({
      allowed: false,
      reason: expect.stringContaining("match limit"),
    });
    expect(validatestep(socketstep("waitmessage", { wait: -1 }), "https://example.com")).toMatchObject({
      allowed: false,
      reason: expect.stringContaining("wait budget"),
    });
    expect(
      validatestep(
        socketstep("subscribesse", {
          subscription: { url: "https://api.example/stream", lifetime: 5000, cancel: { kind: "stop", value: "done" } },
        }),
        "https://example.com",
      ),
    ).toEqual({ allowed: true });
    expect(
      validatestep(
        socketstep("subscribesse", { subscription: { url: "https://api.example/stream" } }),
        "https://example.com",
      ),
    ).toMatchObject({ allowed: false, reason: expect.stringContaining("cancellation path") });
    expect(
      validatestep(
        socketstep("subscribesse", {
          subscription: { url: "https://api.example/stream", lifetime: -5, cancel: { kind: "stop", value: "done" } },
        }),
        "https://example.com",
      ),
    ).toMatchObject({ allowed: false, reason: expect.stringContaining("lifetime window") });
    ...
    expect(
      validatestep(
        socketstep("longpoll", { poll: { url: "https://api.example/poll", cursorfield: "c", interval: 50 } }),
        "https://example.com",
      ),
    ).toMatchObject({ allowed: false, reason: expect.stringContaining("stop condition") });
    expect(
      validatestep(
        socketstep("longpoll", {
          poll: { url: "https://api.example/poll", cursorfield: "c", interval: 0, stop: { field: "d", equals: "y" } },
        }),
        "https://example.com",
      ),
    ).toMatchObject({ allowed: false, reason: expect.stringContaining("options.poll") });
    expect(
      validatestep(
        socketstep("longpoll", {
          poll: { url: "https://api.example/poll", cursorfield: "c", interval: 600, stop: { field: "d", equals: "y" } },
          wait: 500,
        }),
        "https://example.com",
      ),
  ...
  it("validates the watch grammar with required redaction lists and byte ceilings", () => {
    expect(
      validatestep(socketstep("watchrequests", { watch: { window: 500 }, limit: 10 }), "https://example.com"),
    ).toEqual({ allowed: true });
    expect(validatestep(socketstep("watchrequests", {}), "https://example.com")).toEqual({ allowed: true });
    expect(validatestep(socketstep("watchrequests", { watch: { window: -1 } }), "https://example.com")).toMatchObject({
      allowed: false,
      reason: expect.stringContaining("watch window"),
    });
    expect(validatestep(socketstep("watchrequests", { limit: 0 }), "https://example.com")).toMatchObject({
      allowed: false,
      reason: expect.stringContaining("match limit"),
    });
    expect(
      validatestep(
        socketstep("readheaders", { headers: { allow: ["content-type"], redact: ["set-cookie"] } }),
        "https://example.com",
      ),
    ).toEqual({ allowed: true });
    expect(
      validatestep(socketstep("readheaders", { headers: { allow: ["content-type"] } }), "https://example.com"),
    ).toMatchObject({ allowed: false, reason: expect.stringContaining("redaction list") });
    expect(
      validatestep(socketstep("readheaders", { headers: { redact: ["set-cookie"] } }), "https://example.com"),
    ).toMatchObject({ allowed: false, reason: expect.stringContaining("allowlist") });
    expect(validatestep(socketstep("readheaders", {}), "https://example.com")).toMatchObject({
      allowed: false,
      reason: expect.stringContaining("options.headers"),
    });
    expect(
      validatestep(
        socketstep("capturebodies", {
          body: { urlpattern: "api.example", mimes: ["application/json"], ceiling: 4096 },
        }),
        "https://example.com",
      ),
    ).toEqual({ allowed: true });
    expect(validatestep(socketstep("capturebodies", { body: { mimes: [] } }), "https://example.com")).toMatchObject({
      allowed: false,
      reason: expect.stringContaining("mime list"),
    });
    expect(validatestep(socketstep("capturebodies", { body: { ceiling: -1 } }), "https://example.com")).toMatchObject({
      allowed: false,
      reason: expect.stringContaining("byte ceiling"),
    });
    expect(validatestep(socketstep("capturebodies", {}), "https://example.com")).toMatchObject({
      allowed: false,
      reason: expect.stringContaining("options.body"),
    });
    expect(validatestep(socketstep("mapapi", { limit: 5 }), "https://example.com")).toEqual({ allowed: true });
    expect(validatestep(socketstep("mapapi", { limit: 0 }), "https://example.com")).toMatchObject({
      allowed: false,
      reason: expect.stringContaining("match limit"),
    });
    expect(
      validatestep(
        socketstep("extractapi", {
          replay: {
    ...
    expect(
      validatestep(
        socketstep("extractapi", { replay: { endpoint: "https://api.example/items", verb: "TRACE" } }),
        "https://example.com",
      ),
    ).toMatchObject({ allowed: false, reason: expect.stringContaining("known HTTP verb") });
    expect(
      validatestep(
        socketstep("extractapi", { replay: { endpoint: "http://api.example/items" } }),
        "https://example.com",
      ),
    ).toMatchObject({ allowed: false, reason: expect.stringContaining("HTTPS") });
    expect(
      validatestep(
        socketstep("extractapi", { replay: { endpoint: "https://api.example/items", paths: ["bad path"] } }),
        "https://example.com",
      ),
    ).toMatchObject({ allowed: false, reason: expect.stringContaining("dotted path") });
    expect(validatestep(socketstep("extractapi", {}), "https://example.com")).toMatchObject({
      allowed: false,
      reason: expect.stringContaining("options.replay"),
    });
  });
  ...
  it("gates socket channels, watchrequests and replays at execution time", () => {
    ...
    ).toMatchObject({ allowed: false, reason: expect.stringContaining("webrequest grant") });
    expect(
      canexecute({
        session: httpsession,
        plan: plan(watch),
    ...
    expect(sockettarget(socketstep("opensocket", { socket: { url: "wss://api.example/live" } }))).toBe(
      "wss://api.example/live",
    );
    expect(
      sockettarget(
        socketstep("subscribesse", {
          subscription: { url: "https://api.example/stream", cancel: { kind: "stop", value: "done" } },
        }),
      ),
    ).toBe("https://api.example/stream");
...
describe("network control policy", () => {
  const now = 1_800_000_000_000;
  const grants: agentsession = {
    id: "session",
    tabid: 4,
    origin: "https://example.com",
    startedat: now,
    expiresat: now + 1000,
    grants: ["https://example.com", "https://api.example"],
  };
  ...
  it("requires the reviewed block rule behind the blockgate", () => {
    const reviewed: toolstep = {
      id: "b",
      kind: "blockrequest",
      summary: "Block ads",
      risk: "sensitive",
      options: JSON.stringify({ block: { urlpattern: "https://ads.example/*", reviewed: true } }),
    };
    expect(blockgate(grants, reviewed, now).allowed).toBe(true);
    const unreviewed: toolstep = {
      id: "b2",
      kind: "blockrequest",
      summary: "Block ads",
      risk: "sensitive",
      options: JSON.stringify({ block: { urlpattern: "https://ads.example/*" } }),
    };
  ...
  it("requires the reviewed consent ref and bypass list behind the proxygate", () => {
    const route: toolstep = {
      id: "p",
      kind: "routeproxy",
      summary: "Route through the reviewed proxy",
      risk: "sensitive",
      options: JSON.stringify({
        consentref: "consent-1",
        proxy: { scheme: "socks5", host: "proxy.example", port: 1080, bypass: ["https://api.example"] },
      }),
    };
    ...
    expect(
      proxygate(
        grants,
        {
          ...
          options: JSON.stringify({
            consentref: "c",
            proxy: { scheme: "socks5", host: "proxy.example", port: 1080, bypass: [] },
          }),
        },
        now,
      ).reason,
    ).toContain("bypass list");
  });

  it("requires the provider consent prompt of authflow and the consent prompt of saveapikey", () => {
    const authstep: toolstep = {
      id: "a",
      kind: "authflow",
      summary: "Run the oauth flow",
      risk: "sensitive",
      options: JSON.stringify({ consentref: "prompt-1" }),
    };
    ...
    const keystorestep: toolstep = {
      id: "k",
      kind: "saveapikey",
      summary: "Store the api key",
      risk: "sensitive",
      options: JSON.stringify({ consentref: "prompt-2" }),
    };
  ...
  it("validates the control options grammar of every control kind", () => {
      ...
      validatestep(
        {
          ...
          summary: "Block",
          risk: "sensitive",
          options: JSON.stringify({ block: { urlpattern: "ads.example", reviewed: true } }),
        },
        "https://example.com",
      ).reason,
    ).toContain("https origin pattern");
    expect(
      validatestep(
        {
          id: "3",
          ...
          risk: "sensitive",
          options: JSON.stringify({
            mock: { urlpattern: "https://api.example/status", status: 204, body: "{}", reviewed: true },
          }),
        },
        "https://example.com",
      ).allowed,
      ...
      validatestep(
        {
          ...
          summary: "Mock",
          risk: "sensitive",
          options: JSON.stringify({ mock: { urlpattern: "https://api.example/status", status: 204, body: "{}" } }),
        },
        "https://example.com",
      ).reason,
    ).toContain("reviewed with its full body");
    expect(
      validatestep(
        {
          id: "5",
          ...
          risk: "sensitive",
          options: JSON.stringify({
            rules: [{ urlpattern: "https://api.example/*", name: "accept", operation: "append", value: "json" }],
          }),
        },
        "https://example.com",
      ).allowed,
      ...
      validatestep(
        {
          ...
          risk: "sensitive",
          options: JSON.stringify({
            rules: [{ urlpattern: "api.example", name: "accept", operation: "set", value: "json" }],
          }),
        },
        "https://example.com",
      ).reason,
    ).toContain("origin pattern explicitly");
    expect(
      validatestep(
        {
          id: "7",
          ...
          summary: "Set cookies",
          risk: "sensitive",
          options: JSON.stringify({ cookies: [{ name: "session", domain: "example.com", path: "/", value: "v" }] }),
        },
        "https://example.com",
      ).allowed,
    ).toBe(true);
      ...
      validatestep(
        {
          ...
          summary: "Clear",
          risk: "sensitive",
          options: JSON.stringify({ domain: "example.com" }),
        },
        "https://example.com",
      ).allowed,
    ).toBe(true);
    expect(
      validatestep(
        { id: "9", kind: "clearcookies", summary: "Clear", risk: "sensitive", options: "{}" },
        "https://example.com",
      ).reason,
    ).toContain("domain");
    expect(
      ...
      validatestep(
        {
          ...
          options: JSON.stringify({
            consentref: "c",
            key: { name: "k", origins: ["https://api.example"], header: "authorization", value: "secret" },
          }),
        },
        "https://example.com",
      ).allowed,
      ...
      validatestep(
        {
          ...
          options: JSON.stringify({
            consentref: "c",
            proxy: { scheme: "https", host: "proxy.example", port: 1080, bypass: ["https://api.example"] },
          }),
        },
        "https://example.com",
      ).allowed,
      ...
      validatestep(
        {
          ...
          summary: "Post",
          risk: "sensitive",
          options: JSON.stringify({ form: { url: "https://api.example/submit", fields: [{ name: "q", value: "v" }] } }),
        },
        "https://example.com",
      ).allowed,
    ).toBe(true);
      ...
      validatestep(
        {
          ...
          options: JSON.stringify({
            upload: {
              ...
              fields: [],
              files: [{ name: "f", filename: "a.csv", mime: "text/csv", content: "a", reviewed: true }],
            },
          }),
        },
        "https://example.com",
      ...
      validatestep(
        {
          ...
          options: JSON.stringify({
            upload: {
              url: "https://api.example/upload",
              files: [{ name: "f", filename: "a.csv", mime: "text/csv", content: "a", reviewed: false }],
            },
          }),
        },
        "https://example.com",
      ).reason,
    ).toContain("reviewed flag");
  });
  ...
  it("resolves the control target url and refuses ungranted targets at execution time", () => {
    ...
    const outside: toolstep = {
      id: "o",
      kind: "postform",
      summary: "Post elsewhere",
      risk: "sensitive",
      options: JSON.stringify({
        form: { url: "https://elsewhere.example/submit", fields: [{ name: "q", value: "v" }] },
      }),
    };
    const plan: agentplan = {
      id: "plan",
    ...
    expect(
      canexecute({ session: grants, plan, step: outside, tabid: 4, origin: "https://example.com", now }).reason,
    ).toContain("outside the session origin grants");
    const cookieplan: agentplan = {
      id: "plan",
      objective: "Cookies",
      origin: "https://example.com",
      steps: [],
    ...
    const badcookie: toolstep = {
      id: "c",
      kind: "clearcookies",
      summary: "Clear elsewhere",
      risk: "sensitive",
      options: JSON.stringify({ domain: "elsewhere.example" }),
    };
    expect(
      canexecute({ session: grants, plan: cookieplan, step: badcookie, tabid: 4, origin: "https://example.com", now })
        .reason,
    ).toContain("outside the session origin grants");
  });
});
...
describe("debugging policy", () => {
  ...
  const watchstep = (kind: actionkind, reviewed: Record<string, unknown>): toolstep => ({
    id: "w",
    kind,
    summary: "Watch the page",
    risk: "read",
    options: JSON.stringify(reviewed),
  });
  const planof = (step: toolstep): agentplan => ({
    id: "plan",
    objective: "Debug",
    origin: "https://example.com",
    steps: [step],
  ...
  it("scopes timeline capture to the run tab only behind the timelinegate", () => {
    ...
    expect(
      canexecute({ session: grants, plan: planof(step), step: other, tabid: 9, origin: "https://example.com", now })
        .reason,
    ).toContain("outside the approved tab");
    expect(
      canexecute({
        session: grants,
        plan: { ...planof(step), state: "pending" },
        step: other,
        tabid: 4,
        origin: "https://example.com",
        now,
      }).reason,
    ).toContain("not received explicit approval");
  });

  it("requires the per origin console consent before console capture begins", () => {
    const consents: consoleconsentrecord[] = [
      {
        id: "c1",
        prompt: "Console capture on https://example.com",
        origin: "https://example.com",
        stepid: "w",
        approved: true,
        at: now,
    ...
    expect(consoleconsentcovers("https://other.example", consents).reason).toContain(
      "needs the reviewed console consent",
    );
    expect(
      consoleconsentcovers("https://example.com", [
        { id: "c2", prompt: "pending", origin: "https://example.com", stepid: "w", at: now },
      ]).allowed,
    ).toBe(false);
  });
  ...
  it("validates the debug options grammar of every debugging kind", () => {
    expect(
      validatestep(
        watchstep("watchconsole", {
          watch: { window: 250 },
          level: "warn",
          depth: 3,
          redact: ["secret"],
          spam: { pattern: "retry", windowsize: 500, collapse: 2 },
          rotation: { maxentries: 40, overflowtarget: "overflow1" },
        }),
        "https://example.com",
      ),
    ).toEqual({ allowed: true });
    expect(
      validatestep(
        watchstep("watcherrors", { watch: { window: 250 }, level: "error", sources: ["error", "rejection"] }),
        "https://example.com",
      ),
    ).toEqual({ allowed: true });
    expect(
      validatestep(watchstep("watchtasks", { watch: { window: 250 }, threshold: 50 }), "https://example.com"),
    ).toEqual({ allowed: true });
    expect(validatestep(watchstep("watchconsole", { watch: { window: 250 } }), "https://example.com").reason).toContain(
      "redaction pattern list",
    );
    expect(
      validatestep(
        watchstep("watchconsole", { watch: { window: 250 }, redact: ["secret"], level: "verbose" }),
        "https://example.com",
      ).reason,
    ).toContain("level floor");
    expect(
      validatestep(
        watchstep("watchconsole", { watch: { window: 250 }, redact: ["secret"], depth: 0 }),
        "https://example.com",
      ).reason,
    ).toContain("depth");
    expect(
    ...
    expect(
      validatestep(
        watchstep("watchconsole", {
          watch: { window: 250 },
          redact: ["secret"],
          rotation: { maxentries: 0, overflowtarget: "x" },
        }),
        "https://example.com",
      ).reason,
    ).toContain("rotation rule");
    expect(
      validatestep(watchstep("watcherrors", { watch: { window: 600 }, wait: 500 }), "https://example.com").reason,
    ).toContain("wait budget");
    expect(validatestep(watchstep("watcherrors", { watch: { window: -1 } }), "https://example.com").reason).toContain(
      "watch window",
    );
    expect(
      validatestep(watchstep("watcherrors", { watch: { window: 250 }, sources: ["chatter"] }), "https://example.com")
        .reason,
    ).toContain("source filters");
    expect(
      validatestep(watchstep("watchtasks", { watch: { window: 250 }, threshold: -3 }), "https://example.com").reason,
    ).toContain("threshold");
  });
  ...
  it("allows timeline capture without any other action kind and exposes the retention window", () => {
    ...
    expect(
      canexecute({ session: grants, plan: planof(solo), step: solo, tabid: 4, origin: "https://example.com", now }),
    ).toEqual({ allowed: true });
...
describe("debugging part two policy", () => {
  ...
  const cdpstep = (kind: actionkind, reviewed: Record<string, unknown>): toolstep => ({
    id: "c",
    kind,
    summary: "Debug the page",
    risk: "sensitive",
    options: JSON.stringify(reviewed),
  });
  const attachoptions = {
    domains: ["Runtime", "Debugger"],
    teardown: { revertsteps: ["revert breakpoints", "revert overrides"], resumepolicy: "pause" },
  };
  const attach: toolstep = {
    ...
    summary: "Attach",
    risk: "sensitive",
    options: JSON.stringify(attachoptions),
  };
  const planof = (steps: toolstep[]): agentplan => ({
    id: "plan",
    objective: "Debug",
    origin: "https://example.com",
    steps,
  ...
  it("grades the devtools kinds: watchcdp read, stepping interaction and the raw protocol sensitive", () => {
    ...
    for (const kind of ["setbreakpoint", "stepcode", "watchexpr"] as const)
      expect(actionrisk(kind)).toBe("interaction");
  ...
  it("requires the approved debugger consent covering every requested domain before the first attach", () => {
    const approved: debuggergrant[] = [
      {
        id: "g1",
        prompt: "Debugger attach",
        origin: "https://example.com",
        domains: ["Runtime", "Debugger"],
        approved: true,
        consentedat: now,
    ...
    expect(debuggerconsentcovers("https://example.com", ["Runtime", "DOM"], approved).reason).toContain(
      "reviewed debugger consent",
    );
    expect(debuggerconsentcovers("https://elsewhere.example", ["Runtime"], approved).reason).toContain(
      "reviewed debugger consent",
    );
  ...
  it("validates the cdp options grammar of every kind", () => {
    ...
    expect(validatestep(cdpstep("attachcdp", { domains: ["Runtime"] }), "https://example.com").reason).toContain(
      "teardown plan",
    );
    expect(validatestep(cdpstep("attachcdp", { domains: ["Nothing"] }), "https://example.com").reason).toContain(
      "domain grammar",
    );
    expect(
      validatestep(
        cdpstep("attachcdp", { domains: ["Runtime"], teardown: { revertsteps: [], resumepolicy: "pause" } }),
        "https://example.com",
      ).reason,
    ).toContain("teardown plan");
    expect(
      validatestep(
        cdpstep("attachcdp", {
          domains: ["Runtime", "Debugger"],
          teardown: { revertsteps: ["x"] },
          allowlist: { domains: ["Runtime", "DOM"], methods: ["Runtime.evaluate"] },
        }),
        "https://example.com",
      ).reason,
    ).toContain("inside the enabled domains");
    expect(
      validatestep(
        cdpstep("attachcdp", {
          domains: ["Runtime"],
    ...
    expect(validatestep(cdpstep("cdpcmd", { command: { method: "Runtime.evaluate" } }), "https://example.com")).toEqual(
      { allowed: true },
    );
    expect(
      validatestep(cdpstep("cdpcmd", { command: { method: "runtime.evaluate" } }), "https://example.com").reason,
    ).toContain("Domain.method");
    expect(
      validatestep(cdpstep("cdpcmd", { command: { method: "Runtime.evaluate", params: [] } }), "https://example.com")
        .reason,
    ).toContain("params");
    expect(
      validatestep(cdpstep("cdpcmd", { command: { method: "Runtime.evaluate", resultpath: 4 } }), "https://example.com")
        .reason,
    ).toContain("result path");
    expect(
      validatestep(
        cdpstep("watchcdp", { events: [{ domain: "Log", event: "entryAdded" }], watch: { window: 200 } }),
        "https://example.com",
      ),
    ).toEqual({ allowed: true });
    expect(
      validatestep(cdpstep("watchcdp", { events: [], watch: { window: 200 } }), "https://example.com").reason,
    ).toContain("domain event rule");
    expect(
      validatestep(cdpstep("watchcdp", { events: [{ domain: "Log", event: "entryAdded" }] }), "https://example.com")
        .reason,
    ).toContain("lifetime window");
    expect(
      validatestep(
        cdpstep("watchcdp", { events: [{ domain: "Log", event: "entryAdded" }], watch: { window: 500 }, wait: 200 }),
        "https://example.com",
      ).reason,
    ).toContain("wait budget");
    expect(
      validatestep(
        cdpstep("setbreakpoint", { breakpoint: { url: "https://example.com/app.js", line: 3 } }),
        "https://example.com",
      ),
    ).toEqual({ allowed: true });
    expect(
      validatestep(
        cdpstep("setbreakpoint", { breakpoint: { url: "https://example.com/app.js" } }),
        "https://example.com",
      ).reason,
    ).toContain("line");
    expect(
      validatestep(
        cdpstep("setbreakpoint", { breakpoint: { url: "http://example.com/app.js", line: 3 } }),
        "https://example.com",
      ).reason,
    ).toContain("HTTPS");
    expect(validatestep(cdpstep("stepcode", { mode: "stepinto" }), "https://example.com")).toEqual({ allowed: true });
    expect(validatestep(cdpstep("stepcode", { mode: "stepoutward" }), "https://example.com").reason).toContain(
      "stepover, stepinto, stepout or resume",
    );
    expect(
      validatestep(
        cdpstep("watchexpr", { expression: { expression: "items.length" }, reviewed: true }),
        "https://example.com",
      ),
    ).toEqual({ allowed: true });
    expect(
      validatestep(cdpstep("watchexpr", { expression: { expression: "items.length" } }), "https://example.com").reason,
    ).toContain("reviewed");
    expect(
      validatestep(
        cdpstep("overridescript", {
          override: { urlpattern: "https://example.com/app.js", source: "window.x = 1;" },
          reviewed: true,
        }),
        "https://example.com",
      ),
    ...
    expect(
      validatestep(
        cdpstep("overridescript", { override: { urlpattern: "https://example.com/app.js", source: "window.x = 1;" } }),
        "https://example.com",
      ).reason,
    ).toContain("reviewed");
    expect(
      validatestep(
        cdpstep("overridescript", { override: { urlpattern: "app.js", source: "x" }, reviewed: true }),
        "https://example.com",
      ).reason,
    ).toContain("named https origin");
  });
  ...
  it("validates breakpoint conditions against the reviewed expression grammar", () => {
    ...
    const refused = [
      validatebreakpointcondition("total = 5"),
      validatebreakpointcondition("fetch('/x')"),
      validatebreakpointcondition("alert(1)"),
      validatebreakpointcondition("a; b"),
    ];
  ...
  it("resolves the plan allowlist from the attachcdp step", () => {
    ...
    const gated = {
      ...attach,
      options: JSON.stringify({ ...attachoptions, allowlist: { domains: ["Runtime"], methods: ["Runtime.evaluate"] } }),
    };
    expect(planallowlist([gated])).toEqual({ domains: ["Runtime", "Debugger"], methods: ["Runtime.evaluate"] });
    expect(
      planallowlist([
        cdpstep("watchcdp", { events: [{ domain: "Log", event: "entryAdded" }], watch: { window: 100 } }),
      ]),
    ).toBeUndefined();
    expect(
      planallowlist([{ ...attach, options: JSON.stringify({ teardown: { revertsteps: ["x"] } }) }]),
    ).toBeUndefined();
  });
  ...
  it("gates cdp execution behind the run tab, the plan attach and the origin grants", () => {
    ...
    expect(
      canexecute({ ...base, step: cdpstep("cdpcmd", { command: { method: "DOM.getSnapshot" } }) }).reason,
    ).toContain("domain allowlist");
    ...
    const oroutside = cdpstep("overridescript", {
      override: { urlpattern: "https://elsewhere.example/app.js", source: "x" },
      reviewed: true,
    });
...
describe("debugging part three policy", () => {
  ...
  const grants: agentsession = {
    ...
    startedat: now,
    expiresat: now + 1000,
    grants: ["https://example.com", "https://cdn.example"],
  };
  const profilestep = (kind: actionkind, reviewed: Record<string, unknown>): toolstep => ({
    id: "p",
    kind,
    summary: "Profile the page",
    risk: "read",
    options: JSON.stringify(reviewed),
  });
  const approvedgrant: debuggergrant[] = [
    {
      id: "g1",
      prompt: "Debugger attach",
      origin: "https://example.com",
  ...
  it("grades the profiling kinds: measurement read only, the heavy heap and cpu captures sensitive", () => {
    ...
    for (const kind of ["heapshot", "profilecpu", "capturesourcemaps"] as const)
      expect(actionrisk(kind)).toBe("sensitive");
  ...
  it("gates every profiling kind behind the run tab, the granted target origins and the reviewed debugger grant", () => {
    ...
    expect(
      targetgate({ ...base, targets: [{ kind: "page", url: "https://example.com" }], grants: approvedgrant }),
    ).toEqual({ allowed: true });
    expect(
      targetgate({ ...base, targets: [{ kind: "iframe", url: "https://cdn.example/frame" }], grants: approvedgrant }),
    ).toEqual({ allowed: true });
    expect(
      targetgate({
        ...base,
        targets: [{ kind: "worker", url: "https://elsewhere.example/worker.js" }],
        grants: approvedgrant,
      }).reason,
    ).toContain("outside the granted origins");
    expect(
      targetgate({ ...base, targets: [{ kind: "serviceworker", url: "https://cdn.example/sw.js" }], grants: [] })
        .reason,
    ).toContain("reviewed debugger grant");
    expect(targetgate({ ...base, targets: [], grants: undefined })).toEqual({ allowed: true });
    expect(
      targetgate({ session: grants, tabid: 9, origin: "https://example.com", targets: [], grants: approvedgrant, now })
        .reason,
    ).toContain("refuses tab 9");
    expect(
      targetgate({
        ...base,
        targets: [],
        grants: [{ ...approvedgrant[0]!, origin: "https://example.com", revokedat: now }],
      }).reason,
    ).toContain("reviewed debugger grant");
  });
  ...
  it("requires the per origin source map consent before any map file is fetched", () => {
    const approved = [
      { id: "c1", prompt: "Source map capture", origin: "https://example.com", approved: true, consentedat: now },
    ];
    ...
    expect(sourcemapconsentcovers("https://example.com", [{ ...approved[0]!, revokedat: now }]).reason).toContain(
      "revoked",
    );
  });
  ...
  it("validates the profiling options grammar of every kind", () => {
    expect(
      validatestep(
        profilestep("measureflow", {
          flow: { prefix: "flow", steps: ["s1"], metrics: ["navigation", "blocking"] },
          watch: { window: 250 },
        }),
        "https://example.com",
      ),
    ...
    expect(
      validatestep(
        profilestep("measureflow", {
          flow: { prefix: "flow", steps: [], metrics: ["navigation"] },
          watch: { window: 250 },
        }),
        "https://example.com",
      ).reason,
    ...
    expect(
      validatestep(
        profilestep("measureflow", {
          flow: { prefix: "flow", steps: ["s1"], metrics: ["nope"] },
          watch: { window: 250 },
        }),
        "https://example.com",
      ).reason,
    ...
    expect(
      validatestep(
        profilestep("measureflow", { flow: { prefix: "flow", steps: ["s1"], metrics: ["navigation"] } }),
        "https://example.com",
      ).reason,
    ).toContain("watch window");
    expect(
      validatestep(
        profilestep("measureflow", {
          flow: { prefix: "flow", steps: ["s1"], metrics: ["navigation"] },
          watch: { window: 500 },
          wait: 200,
        }),
        "https://example.com",
    ...
    expect(validatestep(profilestep("heapshot", { heap: { interval: 1000 } }), "https://example.com")).toEqual({
      allowed: true,
    });
    expect(validatestep(profilestep("heapshot", { heap: { interval: -5 } }), "https://example.com").reason).toContain(
      "interval",
    );
    expect(
      validatestep(profilestep("trackmemory", { growth: { slope: 2, interval: 50 } }), "https://example.com"),
    ).toEqual({ allowed: true });
    expect(
      validatestep(profilestep("trackmemory", { growth: { interval: 50 } }), "https://example.com").reason,
    ).toContain("slope");
    expect(validatestep(profilestep("trackmemory", { growth: { slope: -1 } }), "https://example.com").reason).toContain(
      "slope",
    );
    expect(validatestep(profilestep("profilecpu", { profile: { duration: 400 } }), "https://example.com")).toEqual({
      allowed: true,
    });
    expect(
      validatestep(profilestep("profilecpu", { profile: { duration: 500 }, wait: 200 }), "https://example.com").reason,
    ).toContain("wait budget");
    expect(validatestep(profilestep("profilecpu", {}), "https://example.com").reason).toContain("duration");
    expect(
      validatestep(profilestep("watchshifts", { watch: { window: 300 }, threshold: 0.1 }), "https://example.com"),
    ).toEqual({ allowed: true });
    expect(validatestep(profilestep("watchshifts", { threshold: 0.1 }), "https://example.com").reason).toContain(
      "observation window",
    );
    expect(
      validatestep(profilestep("watchshifts", { watch: { window: 300 }, threshold: -1 }), "https://example.com").reason,
    ).toContain("threshold");
    expect(
      validatestep(
        profilestep("traceload", { trace: { categories: ["scripting", "network"], window: 300 } }),
        "https://example.com",
      ),
    ).toEqual({ allowed: true });
    expect(
      validatestep(profilestep("traceload", { trace: { categories: ["secret"], window: 300 } }), "https://example.com")
        .reason,
    ).toContain("category grammar");
    expect(
      validatestep(profilestep("traceload", { trace: { categories: ["scripting"] } }), "https://example.com").reason,
    ).toContain("window");
    expect(
      validatestep(
        profilestep("traceload", { trace: { categories: ["scripting"], window: 300, exporttarget: "clipboard" } }),
        "https://example.com",
      ).reason,
    ).toContain("export target");
    expect(
      validatestep(
        profilestep("annotatetrace", { trace: { traceid: "t1" }, annotations: [{ stepid: "s1", label: "open" }] }),
        "https://example.com",
      ),
    ).toEqual({ allowed: true });
    expect(
      validatestep(profilestep("annotatetrace", { trace: { traceid: "t1" } }), "https://example.com").reason,
    ).toContain("step annotations");
    expect(
      validatestep(
        profilestep("annotatetrace", { annotations: [{ stepid: "s1", label: "open" }] }),
        "https://example.com",
      ).reason,
    ).toContain("trace id");
    expect(validatestep(profilestep("replaytrace", { trace: { traceid: "t1" } }), "https://example.com")).toEqual({
      allowed: true,
    });
    ...
    expect(
      validatestep(
        profilestep("capturesourcemaps", { scripts: ["https://example.com/app.js"] }),
        "https://example.com",
      ),
    ).toEqual({ allowed: true });
    expect(
      validatestep(profilestep("capturesourcemaps", { scripts: ["http://example.com/app.js"] }), "https://example.com")
        .reason,
    ).toContain("HTTPS");
  });
  ...
  it("gates profiling execution behind the target gate and the origin grants of the scripts", () => {
    ...
    const flow = profilestep("measureflow", {
      flow: { prefix: "flow", steps: ["s1"], metrics: ["navigation"] },
      watch: { window: 100 },
    });
...
describe("emulation policy", () => {
  ...
  const emustep = (kind: actionkind, reviewed: Record<string, unknown>): toolstep => ({
    id: "e",
    kind,
  ...
  it("requires review of every layer before it applies through the emugate", () => {
    const plan: agentplan = {
      ...
      objective: "Emulate",
      origin: "https://example.com",
      steps: [emustep("emulatedevice", { reviewed: true, device, revertplan })],
      createdat: now,
      expiresat: now + 1000,
      state: "approved",
    };
    const base = { session: emusession, plan, tabid: 4, origin: "https://example.com", now };
    expect(emugate({ ...base, step: emustep("emulatedevice", { reviewed: true, device, revertplan }) })).toEqual({
      allowed: true,
    });
    expect(emugate({ ...base, step: emustep("emulatedevice", { device, revertplan }) }).reason).toContain(
      "reviewed flag",
    );
    expect(emugate({ ...base, step: emustep("emulatedevice", { reviewed: true, device }) }).reason).toContain(
      "revert plan",
    );
    expect(
      emugate({
        ...base,
        step: emustep("emulatedevice", { reviewed: true, device, revertplan }),
        plan: { ...plan, state: "pending" },
      }).reason,
  ...
  it("allows stacking layers only when the reviewed plan lists the steps", () => {
    ...
    const two: agentplan = {
      ...
      steps: [
        ...one.steps,
        emustep("emulatedevice", { reviewed: true, device: { ...device, name: "tablet" }, revertplan }),
      ],
    };
  ...
  it("requires the location consent of the reviewed coordinates before the location layer applies", () => {
    ...
    expect(
      locationconsentgate("https://example.com", 38.7223, -9.1393, [{ ...consents[0]!, revokedat: now }]).reason,
    ).toContain("revoked");
  });

  it("validates the emulation options grammar of every kind", () => {
    expect(
      validatestep(emustep("emulatedevice", { reviewed: true, device, revertplan }), "https://example.com"),
    ).toEqual({ allowed: true });
    expect(
      validatestep(
        emustep("emulatedevice", { reviewed: true, device: { ...device, width: 0 }, revertplan }),
        "https://example.com",
      ).reason,
    ).toContain("reviewed preset");
    expect(
      validatestep(
        emustep("emulatedevice", { reviewed: true, device, revertplan, reload: "yes" }),
        "https://example.com",
      ).reason,
    ).toContain("reload flag");
    expect(validatestep(emustep("emulatedevice", { reviewed: true, device }), "https://example.com").reason).toContain(
      "revert plan",
    );
    expect(
      validatestep(
        emustep("emulatenetwork", { reviewed: true, network, revertplan, window: 2000 }),
        "https://example.com",
      ),
    ).toEqual({ allowed: true });
    expect(
      validatestep(
        emustep("emulatenetwork", { reviewed: true, network: { ...network, latency: -1 }, revertplan }),
        "https://example.com",
      ).reason,
    ).toContain("reviewed preset");
    expect(
      validatestep(
        emustep("emulatenetwork", { reviewed: true, network, revertplan, window: -1 }),
        "https://example.com",
      ).reason,
    ).toContain("offline window");
    expect(
      validatestep(emustep("emulatelocate", { reviewed: true, location, revertplan }), "https://example.com"),
    ).toEqual({ allowed: true });
    expect(
      validatestep(
        emustep("emulatelocate", { reviewed: true, location: { ...location, latitude: 91 }, revertplan }),
        "https://example.com",
      ).reason,
    ).toContain("reviewed preset");
    expect(validatestep(emustep("setuseragent", { reviewed: true, agent, revertplan }), "https://example.com")).toEqual(
      { allowed: true },
    );
    expect(
      validatestep(
        emustep("setuseragent", { reviewed: true, agent: { ...agent, useragent: "no version" }, revertplan }),
        "https://example.com",
      ).reason,
    ).toContain("reviewed preset");
    expect(
      validatestep(
        emustep("setuseragent", { reviewed: true, agent: { ...agent, brands: [] }, revertplan }),
        "https://example.com",
      ).reason,
    ).toContain("reviewed preset");
    expect(
      validatestep(
        emustep("overridepermission", {
          reviewed: true,
          permission: { name: "geolocation", state: "granted", runscope: true },
          revertplan,
        }),
        "https://example.com",
      ),
    ...
    expect(
      validatestep(
        emustep("overridepermission", {
          reviewed: true,
          permission: { name: "screen-capture", state: "granted" },
          revertplan,
        }),
        "https://example.com",
      ).reason,
    ).toContain("browser permission set");
    expect(
      validatestep(
        emustep("blackboxscripts", {
          reviewed: true,
    ...
    expect(
      validatestep(emustep("blackboxscripts", { reviewed: true, rules: [], revertplan }), "https://example.com").reason,
    ).toContain("non-empty rule list");
  });
  ...
  it("gates emulation execution behind the emugate inside canexecute", () => {
    ...
    expect(canexecute({ ...base, step: emustep("emulatedevice", { device, revertplan }) }).reason).toContain(
      "reviewed flag",
    );
    expect(
      canexecute({
        ...base,
        step: emustep("emulatelocate", {
          reviewed: true,
          location: { name: "x", latitude: 91, longitude: 0, accuracy: 1 },
          revertplan,
        }),
      }).reason,
    ).toContain("reviewed preset");
...
describe("workflow editor policy", () => {
  ...
  const model = {
    ...
    nodes: [{ step: { id: "s1", kind: "wait" as actionkind, label: "Wait" }, x: 40, y: 60 }],
    edges: [],
    blocks: [],
    layout: { width: 640, height: 480, viewportx: 0, viewporty: 0, zoom: 1 },
    minimap: { width: 160, height: 100, scale: 1, zoom: 1, viewport: { x: 0, y: 0, width: 160, height: 100 } },
    dirty: false,
  };
  ...
  it("validates per site overrides and refuses secret fields in exports", () => {
    ...
    const clean = {
      ...
      version: 1,
      origins: ["https://example.com"],
      steps: [{ id: "s1", kind: "wait" as actionkind, label: "Wait" }],
      blocks: [],
      risk: "read" as const,
      createdat: now,
    };
    ...
    expect(
      exportcontentreview({
        workflow: {
          ...clean,
          steps: [{ ...(clean.steps[0] as object), options: '{"authorization":"Bearer x"}' } as never],
        } as never,
        templates: [],
      }).allowed,
    ).toBe(false);
    expect(watchdogconfigvalid({ enabled: true, stallthreshold: 1, action: "retry" }).allowed).toBe(true);
    expect(watchdogconfigvalid({ enabled: true, stallthreshold: 1, action: "retry", zombiewindow: -1 }).allowed).toBe(
      false,
    );
  });
});
...
describe("agent protocol policy", () => {
  it("validates tool schemas against the action kind grammar and refuses drifted catalogs", () => {
    ...
    expect(
      validatetoolcatalog({
        version: 1,
        domains: [{ ...(catalog.domains[0] as tooldomain), tools: [{ ...click, kind: "notakind" as never }] }],
      }).allowed,
    ).toBe(false);
    expect(
      validatetoolcatalog({
        version: 1,
        domains: [{ ...(catalog.domains[0] as tooldomain), tools: [{ ...click, description: "" }] }],
      }).allowed,
    ).toBe(false);
    expect(
      validatetoolcatalog({
        ...
        domains: [
          {
            ...(catalog.domains[0] as tooldomain),
            tools: [{ ...click, inputschema: { type: "object" as never, properties: {}, required: [] } }],
          },
        ],
      }).allowed,
    ).toBe(false);
    ...
    expect(
      validatestep(
        { id: "l1", kind: "listruns", summary: "List the stored workflow run records.", risk: "read" },
        "https://example.com",
      ),
    ).toEqual({ allowed: true });
  });
  ...
  it("keeps the localhost bind as the default, grades remote binding sensitive and requires explicit enablement", () => {
    ...
    expect(
      serverenablementgate({ ...defaultmcpconfig(), enabled: true, transports: ["stdio", "carrierpigeon" as never] })
        .allowed,
    ).toBe(false);
    ...
    expect(
      serverenablementgate({ ...defaultmcpconfig(), enabled: true, framesize: 100_000, queuedepth: 50 }).allowed,
    ).toBe(true);
  ...
  it("gates tool dispatch on the pairing, session, plan, origin and the approved step", () => {
    ...
    const approvedplan: agentplan = {
      ...plan,
      steps: [
        { ...step, id: "c1", kind: "click" },
        { id: "o1", kind: "observe", summary: "Snapshot", risk: "read" },
      ],
    };
    expect(
      tooldispatchgate({
    ...
    expect(
      tooldispatchgate({
        client: { ...client, disconnectedat: now },
        tool: read,
        session: grantsession,
        plan: approvedplan,
        origin: "https://example.com",
    ...
    expect(
      tooldispatchgate({
        client,
        tool: read,
        session: { ...grantsession, stoppedat: now - 1 },
        plan: approvedplan,
        origin: "https://example.com",
        now,
      }).allowed,
    ...
    expect(
      tooldispatchgate({
        client,
        tool: read,
        session: { ...grantsession, pausedat: now - 1 },
        plan: approvedplan,
        origin: "https://example.com",
        now,
      }).allowed,
    ...
    expect(
      tooldispatchgate({
        client,
        tool: read,
        session: { ...grantsession, expiresat: now - 1 },
        plan: approvedplan,
        origin: "https://example.com",
        now,
      }).allowed,
    ...
    expect(
      tooldispatchgate({
        ...
        tool: read,
        session: grantsession,
        plan: { ...approvedplan, state: "pending" },
        origin: "https://example.com",
        now,
      }).allowed,
    ).toBe(false);
...
describe("agent protocol part two policy", () => {
  ...
  it("validates allowlist entries against the known client identities", () => {
    ...
    expect(
      allowlistentryvalid(
        { fingerprint: "aa11", displayname: "Laptop agent", namespaces: ["browser"], grantedat: now, history: [] },
        identities,
      ).allowed,
    ).toBe(true);
    expect(
      allowlistentryvalid(
        { fingerprint: "bb22", displayname: "Stranger", namespaces: ["browser"], grantedat: now, history: [] },
        identities,
      ).allowed,
    ).toBe(false);
    expect(
    ...
    expect(
      allowlistentryvalid(
        { fingerprint: "aa11", displayname: "", namespaces: ["browser"], grantedat: now, history: [] },
        identities,
      ).allowed,
    ).toBe(false);
    expect(
      allowlistentryvalid(
        { fingerprint: "aa11", displayname: "Laptop agent", namespaces: [], grantedat: now, history: [] },
        identities,
      ).allowed,
    ).toBe(false);
    expect(
    ...
    expect(
      allowlistentryvalid(
        { fingerprint: "", displayname: "Laptop agent", namespaces: ["browser"], grantedat: now, history: [] },
        identities,
      ).allowed,
    ).toBe(false);
  });
  ...
  it("requires tls for any non localhost transport and the explicit review for remote enablement", () => {
    ...
    expect(
      remotetransporttls({
        ...defaultmcpconfig(),
        bind: "0.0.0.0",
        httpstream: { endpoint: "/mcp", streampath: "/mcp/stream", tls: { mode: "on" } },
      }).allowed,
    ).toBe(true);
    expect(
      remotetransporttls({
        ...defaultmcpconfig(),
        remoteaccess: { endpoint: "https://agent.example", tls: { mode: "off" as const } },
      }).allowed,
    ).toBe(false);
    expect(
      remotetransporttls({
        ...defaultmcpconfig(),
        remoteaccess: { endpoint: "https://agent.example", tls: { mode: "required" as const } },
      }).allowed,
    ).toBe(true);
    ...
    expect(
      remoteenablementgate({ ...reviewed, remoteaccess: { endpoint: "", tls: { mode: "required" as const } } }).allowed,
    ).toBe(false);
    expect(
      remoteenablementgate({
        ...reviewed,
        remoteaccess: { endpoint: "https://agent.example", tls: { mode: "required" }, maxclients: 0 },
      }).allowed,
    ).toBe(false);
    expect(
      remoteenablementgate({
        ...reviewed,
        remoteaccess: { endpoint: "https://agent.example", tls: { mode: "required" }, maxclients: 5 },
      }).allowed,
    ).toBe(true);
    expect(
      remoteenablementgate({
        ...reviewed,
        remoteaccess: { endpoint: "https://agent.example", tls: { mode: "required" }, tokenlifetimems: 0 },
      }).allowed,
    ).toBe(false);
    expect(
      remoteenablementgate({
        ...
        remoteaccess: {
          ...
          tls: { mode: "required" },
          approvaltimeout: { windowms: 0, ontimeout: "refuse" },
        },
      }).allowed,
    ).toBe(false);
...
describe("agent protocol part three policy", () => {
  it("grades subscriptions as read only when the filters exclude the mutation mirror", () => {
    ...
    expect(subscriptiongrade({ ...base, kinds: ["callstarted", "progress"], tool: "browser.click" }).allowed).toBe(
      true,
    );
  });
  ...
  it("grades sampling callbacks as sensitive behind the page content grant", () => {
    ...
    expect(samplinggrade({ request: { ...request, pagecontent: "the page text" }, pagegrant: true }).allowed).toBe(
      true,
    );
    expect(samplinggrade({ request: { ...request, pagecontent: "the page text" }, pagegrant: false }).allowed).toBe(
      false,
    );
  ...
  it("validates rate limits as user configured values with no silent defaults", () => {
    ...
    expect(
      callratelimitvalid({ clientid: "client1", windowms: 60_000, budget: 5, windowstartedat: now, used: 0 }).allowed,
    ).toBe(true);
    expect(callratelimitvalid({ clientid: "client1", windowms: 0, windowstartedat: now, used: 0 }).allowed).toBe(false);
    expect(
      callratelimitvalid({ clientid: "client1", windowms: 60_000, budget: 0, windowstartedat: now, used: 0 }).allowed,
    ).toBe(false);
    expect(callratelimitvalid({ clientid: "client1", windowms: 60_000, windowstartedat: now, used: 0 }).allowed).toBe(
      true,
    );
  ...
  it("keeps dry run results free of page mutations and mocks inside test contexts", () => {
    ...
    const mock = {
      tool: "browser.readtext",
      result: { content: "canned", iserror: false },
      testcontext: true,
      createdat: now,
    };
...
describe("policy llm integration gates", () => {
  ...
  it("validates provider configs as user configured values with no hardcoded defaults", () => {
    ...
    expect(
      providervalid(
        provider({ authref: { name: "k", origins: [], header: "authorization", storageid: " ", configuredat: now } }),
      ).reason,
    ).toMatch(/storage id/i);
  });
  ...
  it("requires the plan review before any model drafted plan executes", () => {
    const draft: plandraft = {
      id: "d1",
      goal: "Read the page",
      steps: [{ id: "s1", kind: "observe", summary: "Observe the page." }],
      openquestions: [],
      providerid: "prov1",
      model: "model-a",
      state: "approved",
  ...
  it("requires fresh review for the replanned tail steps", () => {
    const replan: replanrecord = {
      ...
      completedstepids: ["s1"],
      failedstepids: ["s2"],
      tail: [{ id: "t1", kind: "reload", summary: "Reload.", freshreview: true }],
      reason: "The step timed out.",
      providerid: "prov1",
      model: "model-a",
      state: "approved",
      createdat: now,
    ...
    expect(replanreviewgate({ ...replan, tail: [{ id: "t1", kind: "reload", summary: "Reload." }] }).reason).toMatch(
      /fresh review marker/i,
    );
  });
  ...
  it("refuses tool calls the guardrails marked invalid or refused", () => {
    ...
    const invalid: modeloutput = {
      raw: "nope",
      verdict: "invalid",
      reason: "The model answer is not json.",
      attempts: 3,
    };
    expect(guardverdictgate(invalid).reason).toMatch(/not json/i);
    const refused: modeloutput = {
      raw: "I cannot comply.",
      verdict: "refused",
  ...
  it("lints model drafts against the action grammar before review", () => {
    const draft: plandraft = {
      ...
      goal: "Do things",
      steps: [
        { id: "s1", kind: "teleport", summary: "Teleport." },
        { id: "s2", kind: "click", summary: "Click nothing." },
        { id: "s3", kind: "navigate", target: "#nav", value: "https://example.com", summary: "Open the page." },
      ],
      openquestions: [],
      providerid: "prov1",
      model: "model-a",
...
describe("multi agent policy gates", () => {
  it("validates the task queue lanes, priorities and completion policy as user configured", () => {
    ...
    expect(
      queuelanesvalid({ lanes: [" "], priorities: [], completionpolicy: "all", items: [], claims: [] }).allowed,
    ).toBe(false);
    expect(
      queuelanesvalid({ lanes: [], priorities: [Number.NaN], completionpolicy: "all", items: [], claims: [] }).allowed,
    ).toBe(false);
    expect(
      queuelanesvalid({ lanes: [], priorities: [], completionpolicy: "sometimes" as never, items: [], claims: [] })
        .allowed,
    ).toBe(false);
    const withitem = {
      lanes: ["extraction"],
      ...
      items: [{ id: "t1", lane: "review", priority: 1, payload: "Task", state: "queued" as const, enqueuedat: now }],
      claims: [],
    };
    ...
    const withpriority = {
      ...withitem,
      items: [
        { id: "t1", lane: "extraction", priority: 9, payload: "Task", state: "queued" as const, enqueuedat: now },
      ],
    };
  ...
  it("permits work stealing only inside one user approved swarm with the lane ownership rules", () => {
    expect(workstealgrade({ swarmapproved: true, agentrole: "worker", lane: "review" })).toMatchObject({
      allowed: true,
    });
    expect(workstealgrade({ swarmapproved: false, agentrole: "worker", lane: "review" }).reason).toMatch(
      /approved swarm/i,
    );
    expect(
      workstealgrade({
        ...
        agentrole: "worker",
        lane: "review",
        ownership: [{ lane: "review", roles: ["planner"] }],
      }).allowed,
    ).toBe(false);
    expect(
      workstealgrade({
  ...
  it("validates the per agent budget ceilings as positive user values", () => {
    expect(agentbudgetvalid({ agentid: "a1", maxtokens: 1000, maxsteps: 5, configuredat: now })).toMatchObject({
      allowed: true,
    });
    expect(agentbudgetvalid({ agentid: "a1", maxcost: 2, currency: "usd", configuredat: now })).toMatchObject({
      allowed: true,
    });
  ...
  it("validates the per agent scope grants against the session grant list", () => {
    ...
    expect(
      agentscopevalid({
        scope: { agentid: "a1", origins: ["https://example.com"], toolnamespaces: ["memory", "system"] },
        grants,
      }),
    ).toMatchObject({ allowed: true });
    expect(
      agentscopevalid({ scope: { agentid: "a1", origins: ["https://outside.example"], toolnamespaces: [] }, grants })
        .reason,
    ).toMatch(/does not carry/i);
    expect(
      agentscopevalid({ scope: { agentid: "a1", origins: [], toolnamespaces: ["kitchen" as never] }, grants }).reason,
    ).toMatch(/catalog namespaces/i);
    expect(agentscopevalid({ scope: { agentid: " ", origins: [], toolnamespaces: [] }, grants }).reason).toMatch(
      /agent id/i,
    );
    expect(agentscopevalid({ scope: { agentid: "a1", origins: [" "], toolnamespaces: [] }, grants }).reason).toMatch(
      /user configured name/i,
    );
  });

  it("grades the spawn requests with the risk class of the requested role", () => {
    expect(spawngrade({ parentid: "a1", role: "worker", task: "Extract", depth: 1 }).reason).toMatch(
      /grades sensitive/i,
    );
    expect(spawngrade({ parentid: "a1", role: "observer", task: "Watch", depth: 2 }).reason).toMatch(/grades read/i);
    expect(spawngrade({ parentid: "a1", role: "auditor", task: "Audit", depth: 1 }).reason).toMatch(
      /grades sensitive/i,
    );
  ...
  it("grades the cross agent messages that carry page content as egress", () => {
    ...
    expect(messageegressgrade({ message: { ...message, payload: " " }, carriespagecontent: false }).reason).toMatch(
      /payload/i,
    );
  });
  ...
  it("grades the blackboard entries by the inherited consent class of the source extraction", () => {
    ...
    expect(
      blackboardconsentgrade({
        ...
        author: "a1",
        section: "findings",
        consentclass: "secret" as never,
        postedat: now,
      }).reason,
    ).toMatch(/three consent classes/i);
    expect(
      blackboardconsentgrade({
        id: "e1",
        key: " ",
...
describe("multi agent part two gates", () => {
  ...
  it("validates the leader election rule as the user configured it", () => {
    ...
    expect(leaderelectionvalid({ rule: { kind: "named", agentid: "a1" }, agents }).reason).toMatch(
      /named rule elects the agent a1/i,
    );
    expect(leaderelectionvalid({ rule: { kind: "named" }, agents }).reason).toMatch(/agent id the user named/i);
    expect(leaderelectionvalid({ rule: { kind: "named", agentid: "missing" }, agents }).reason).toMatch(
      /not a live agent/i,
    );
  ...
  it("grades the critic review as read only over agent outputs", () => {
    ...
    ).toMatch(/issues the critic found/i);
    expect(
      criticreviewgrade({
        id: "r5",
        reviewerid: " ",
  ...
  it("grades the verifier methods against the list the user allows", () => {
    ...
    expect(verifiermethodgrade({ method: "re-read", allowed: ["re-read", "compare"] }).reason).toMatch(
      /inside the methods the user allowed/i,
    );
    expect(verifiermethodgrade({ method: "screenshot", allowed: ["re-read", "compare"] }).reason).toMatch(
      /not one of the methods/i,
    );
  ...
  it("requires the handoff to preserve the original session grants", () => {
    ...
    expect(handoffgrantgate({ record, toscope: undefined, sessiongrants: ["https://example.com"] }).reason).toMatch(
      /unbounded inside the original session grants/i,
    );
    expect(
      handoffgrantgate({
        record,
        toscope: { agentid: "a2", origins: ["https://example.com"], toolnamespaces: [] },
        sessiongrants: ["https://example.com"],
      }).allowed,
    ).toBe(true);
    expect(
      handoffgrantgate({
        record,
        toscope: { agentid: "a2", origins: ["https://other.example"], toolnamespaces: [] },
        sessiongrants: ["https://example.com"],
      }).reason,
    ).toMatch(/never widens the session grants/i);
    expect(
      handoffgrantgate({ record: { ...record, toagentid: " " }, toscope: undefined, sessiongrants: [] }).reason,
    ).toMatch(/transferring and receiving agents/i);
  });

  it("validates the lock scope so one lock never spans unrelated origins", () => {
    expect(
      lockscopevalid({
        key: "https://example.com|#form",
        holder: "a1",
        kind: "exclusive",
        origin: "https://example.com",
        selector: "#form",
    ...
    ).toMatch(/lock key must compose/i);
    expect(
      lockscopevalid({
        key: " |#form",
        holder: "a1",
    ...
    ).toMatch(/one origin and one selector/i);
    expect(
      lockscopevalid({
        key: "https://example.com|#form",
        holder: "a1",
        kind: "sticky" as never,
        origin: "https://example.com",
        selector: "#form",
        acquiredat: now,
      }).reason,
    ).toMatch(/exclusive or shared/i);
  });
  ...
  it("keeps every escalation human decided", () => {
    expect(
      escalationgate({
        id: "e1",
        agentid: "a1",
        subject: "Which origin next",
        context: "Both hold half the table.",
        state: "open",
        raisedat: now,
      }).reason,
    ).toMatch(/human decided/i);
    expect(
      escalationgate({
        id: "e2",
        agentid: "a1",
    ...
    expect(
      escalationgate({ id: "e3", agentid: " ", subject: "s", context: "c", state: "open", raisedat: now }).reason,
    ).toMatch(/names the agent/i);
    expect(
      escalationgate({ id: "e4", agentid: "a1", subject: "s", context: "c", state: "decided", raisedat: now }).reason,
    ).toMatch(/decision the user wrote/i);
  });
...
describe("execution environment gates", () => {
  const evaluatestep = {
    ...
    summary: "Read the title inside the isolated world.",
    risk: "sensitive" as const,
  };
  const markupstep = {
    id: "m1",
    kind: "setattribute" as const,
    target: "#note",
    options: JSON.stringify({ name: "data-note", value: "inert", markup: "<p>untrusted</p>" }),
    summary: "Render the untrusted note.",
    risk: "sensitive" as const,
  };
  const readstep = {
    id: "r1",
    kind: "readhtml" as const,
    target: "#main",
    summary: "Read the html of the main element.",
    risk: "read" as const,
  };
  const clickstep = {
    id: "c1",
  ...
  it("refuses steps whose environment sits outside the session environment grant list", () => {
    ...
    expect(
      environmentgrantgate({ ...readstep, environment: "offscreenworker" }, ["pagecontext", "isolatedworld"]).reason,
    ).toMatch(/widens the environment grants/i);
    expect(
      environmentgrantgate({ ...readstep, environment: "offscreenworker" }, ["pagecontext", "offscreenworker"]).allowed,
    ).toBe(true);
  ...
  it("refuses offscreenworker steps when the offscreen capability grant is absent", () => {
    expect(offscreencapabilitygate({ environment: "offscreenworker", granted: false }).reason).toMatch(
      /inline parsing inside the page/i,
    );
    expect(offscreencapabilitygate({ environment: "offscreenworker", granted: true }).allowed).toBe(true);
    expect(offscreencapabilitygate({ environment: "pagecontext", granted: false }).reason).toMatch(
      /needs no offscreen capability/i,
    );
  });
  ...
  it("limits the keepalive port to sessions with an active reviewed plan", () => {
    ...
    expect(keepalivegate({ session: { ...session, stoppedat: now }, plan: approved, now }).reason).toMatch(
      /stopped session/i,
    );
    expect(keepalivegate({ session: { ...session, pausedat: now }, plan: approved, now }).reason).toMatch(/pauses/i);
    expect(keepalivegate({ session: { ...session, expiresat: now - 1 }, plan: approved, now }).reason).toMatch(
      /expired session/i,
    );
    expect(keepalivegate({ session, plan: { ...approved, state: "pending" as const }, now }).reason).toMatch(
      /active reviewed plan/i,
    );
  ...
  it("grades sandbox renders by the origins the user allows", () => {
    ...
    expect(sandboxorigingate({ origin: "https://other.example", allowed: ["https://example.com"] }).reason).toMatch(
      /outside the origins/i,
    );
...
describe("policy security part two", () => {
      ...
      errors: [
        {
          ...
          found: "string",
          reason:
            "The allowlist field expects an object while the command carries a string; schemastrict refuses the shape mismatch before dispatch.",
        },
      ],
    });
    ...
    expect(
      origincheckgate({
        verdict: {
          ...
          origin: "",
          reason: "The sender is this extension itself.",
        },
      }).allowed,
    ).toBe(true);
    const dropped = origincheckgate({
      verdict: {
        accepted: false,
        sender: "outside",
        origin: "https://lookalike.example",
        reason:
          "The sender sits absent from the connectallow list; the guard drops the message without handler execution.",
      },
    });
  ...
  it("drops messages from senders absent from the connectallow list that ships empty", () => {
    expect(connectallowgate({ senderid: "other-extension", extensionid: "devthink", connectallow: [] }).allowed).toBe(
      false,
    );
    ...
    expect(
      connectallowgate({
        ...
        connectallow: [{ senderid: "other-extension", displayname: "Reviewed bridge", addedat: 1 }],
      }).allowed,
    ).toBe(true);
    expect(
      connectallowgate({
        ...
        extensionid: "devthink",
        connectallow: [
          { senderid: "other-extension", displayname: "Reviewed bridge", origin: "https://other.example", addedat: 1 },
        ],
      }).allowed,
    ).toBe(false);
  });
  ...
  it("routes payment, delete and credential classes through their confirm gates", () => {
    ...
    expect(confirmpaygate({ classes: ["payment"], state: "open" }).reason).toMatch(
      /pauses until the human resolves it/,
    );
  ...
  it("validates the phishguard threshold as a user choice and blocks verdicts that cross it", () => {
    ...
    const blocked = phishverdictof({
      origin: "https://pay.example.com.lookalike.example",
  ...
  it("applies safedefaults to origins without an originprofile: reads pass, sensitive classes deny", () => {
    ...
    expect(safedefaultsgate({ profile: undefined, classes: ["credential"], sensitive: true }).reason).toMatch(
      /originprofile editor/,
    );
    const profile = {
      profileid: "p",
      origin: "https://example.com",
...
describe("interface surfaces policy", () => {
  it("keeps palette actions behind their existing permission and session gates", () => {
    expect(paletteactiongate({ action: { command: "starttask" }, granted: [], sessionactive: false }).allowed).toBe(
      true,
    );
    expect(
      paletteactiongate({
        action: { command: "opendashboardpage", permission: "tabs" },
        granted: ["tabs"],
        sessionactive: false,
      }).allowed,
    ).toBe(true);
    ...
    ).toMatch(/capability granted/);
    expect(
      paletteactiongate({ action: { command: "cancelrun", session: true }, granted: [], sessionactive: false }).reason,
    ).toMatch(/active browser session/);
  });

  it("routes taskinput submissions through the proposal flow with no direct execution path", () => {
    expect(
      taskinputproposalgate({ text: "Collect the invoices", origin: "https://shop.example", direct: false }).allowed,
    ).toBe(true);
    expect(
      taskinputproposalgate({ text: "Collect the invoices", origin: "https://shop.example", direct: true }).reason,
    ).toMatch(/never executes a goal directly/);
  ...
  it("binds each stepapprove resolution to one step with one distinct human action", () => {
    ...
    expect(stepapprovegate({ stepids: ["s1", "s2"], resolution: "approve", surface: "popup" }).reason).toMatch(
  ...
  it("lets an onboarding completion write exactly one consent scoped event", () => {
    ...
    expect(onboardingconsentgate({ consentevents: ["onboardingconsentgranted"] }).reason).toMatch(
  ...
  it("lets the audit excerpt copy only a verified range of the logstream", () => {
    ...
    expect(logstreamegressgate({ verified: false, entries: 4 }).reason).toMatch(
      /only a verified range leaves the stream/,
    );
  ...
  it("gates the ecosystem family of 1.1.66 behind the same review posture", () => {
    expect(
      librarymanifestgate({
        errors: [{ path: "title", expected: "string", found: "undefined", reason: "needs its title" }],
      }).allowed,
    ).toBe(false);
    expect(librarymanifestgate({ errors: [] }).allowed).toBe(true);
    expect(
      librarygrantgate({ requiredgrants: ["https://new.test"], heldgrants: ["https://example.com"] }).reason,
    ).toMatch(/grant diff shows them/);
    expect(librarysensitivegate({ sensitive: true, freshconsent: false }).allowed).toBe(false);
    expect(libraryquarantinegate({ verified: false, signaturepresent: false, signaturevalid: false }).reason).toMatch(
      /quarantines/,
    );
...
describe("the frozen option grammar of the api freeze", () => {
  it("keeps the option grammar of every kind aligned with the frozen plan schema", async () => {
    if (!existsSync("dist/schemas/plan.schema.json"))
      return; /* the schema artifacts ride the build: the pass runs after pnpm build in the validate chain and the ci lanes */
    const schema = JSON.parse(await readFile("dist/schemas/plan.schema.json", "utf8")) as {
      options?: { properties?: Record<string, { type?: string; description?: string }> };
      step?: { properties?: Record<string, unknown> };
      reviewsurface?: { messages?: string[] };
    };
    ...
    for (const field of ["id", "kind", "target", "value", "options", "summary", "risk"])
      expect(schema.step?.properties?.[field]).toBeDefined();
  ...
  it("parses the reviewed options of a step exactly as the frozen grammar freezes them", () => {
    const stepwithoptions: toolstep = {
      id: "s1",
      kind: "presskey",
      value: "Enter",
      options: '{"modifiers":["shift"]}',
      summary: "Press shift enter.",
      risk: "sensitive",
    };
...
describe("the 1.1.95 security hardening gates", () => {
  ...
  it("masks the secret shaped key value pairs of free form log text", () => {
    const masked = masklogtext({
      text: `the step typed password=hunter2-secret and token: "abc123" into the login form`,
      shapes: ["password", "token", "secret", "apikey"],
    });
  ...
  it("answers the local signature check of the virus scanning hook before any remote endpoint fires", () => {
    ...
    const clean = localscangate({
      hook: { scanner: "localscan", localsignatures: [signature] },
      digest: "b".repeat(64),
    });
  ...
  it("selects every stored key of one run trace while the other runs survive", () => {
    const trace = runtracekeys({
      runid: "run-one",
      keys: ["runs", "runlogrun-one", "runscopesrun-one", "provlog:run-one", "runlogrun-two"],
    });
  ...
  it("masks and redacts the transparency export payload over its capture derived entries", () => {
    const report = {
      ...
      posture: "denydefault",
      grants: [],
      password: "typed login secret",
      captures: [{ id: "c1", token: "capture token" }],
    };
    const masked = transparencyexportmask({
      report,
      shapes: ["password", "token"],
      regions: [
        {
          id: "r1",
          origin: "https://example.com",
          ...
          width: 10,
          height: 10,
          reason: "the login field stays redacted",
          source: "userdrawn",
          createdat: 1,
        },
      ],
    ...
    expect(
      (masked.payload.captures as Array<{ redacted?: boolean; redactedregions?: number; token?: string }>)[0]?.redacted,
    ).toBe(true);
...
describe("per agent scope gates", () => {
  ...
  it("gates the tool namespaces and origins of one agent scope to what the user granted", () => {
    const scope = agentscopeof({
      agentid: "w1",
      requested: { origins: ["https://example.com"], toolnamespaces: ["memory", "workflow"] },
      grants: ["https://example.com", "https://other.example"],
      allowedkinds: ["observe", "readtext", "click", "extract"],
    });
    expect(scopegate({ scope, origin: "https://example.com", namespace: "memory" })).toMatchObject({ allowed: true });
    expect(scopegate({ scope, origin: "https://outside.example", namespace: "memory" }).reason).toMatch(
      /grants no access to the origin/,
    );
    expect(scopegate({ scope, origin: "https://example.com", namespace: "browser" }).reason).toMatch(
      /grants no access to the browser tool namespace/,
    );
    expect(scopegate({ scope: undefined, origin: "https://example.com", namespace: "browser" })).toMatchObject({
      allowed: true,
    });
    expect(() =>
      agentscopeof({
        agentid: "w2",
        requested: { origins: ["https://outside.example"] },
        grants: ["https://example.com"],
      }),
    ).toThrow(/never widens past the grants/);
    expect(() =>
      agentscopeof({
        agentid: "w3",
        requested: { actionkinds: ["recordscreen"] },
        allowedkinds: ["observe", "readtext"],
      }),
    ).toThrow(/narrows every step to the allowed kinds/);
  });
  ...
  it("narrows the child scope inside the parent grants without ever widening", () => {
    const parent = agentscopeof({
      agentid: "a1",
      requested: {
        origins: ["https://example.com", "https://docs.example"],
        toolnamespaces: ["memory", "workflow"],
        actionkinds: ["observe", "readtext", "click"],
      },
    });
    const child = childscopeof({
      agentid: "g1",
      parent,
      narrowed: { origins: ["https://docs.example"], actionkinds: ["observe"] },
    });
    ...
    expect(() => childscopeof({ agentid: "g2", parent, narrowed: { origins: ["https://outside.example"] } })).toThrow(
      /never widens past its parent/,
    );
    expect(() => childscopeof({ agentid: "g3", parent, narrowed: { actionkinds: ["recordscreen"] } })).toThrow(
      /never widens past its parent/,
    );
    const orphan = childscopeof({ agentid: "g4" });
    expect(orphan).toEqual({ agentid: "g4", origins: [], toolnamespaces: [] });
    const readonlychild = childscopeof({
      agentid: "g5",
      parent: readonlyscope({ agentid: "o1", readkinds: ["observe"] }),
    });
