...
describe("streamparse progressive observations", () => {
  ...
  it("tokenizes one chunk into text, element and attribute tokens and refuses chunks over the byte bound", () => {
    const chunk = streamparsechunkof({
      index: 0,
      text: '<h1 class="title">Hello</h1> world',
      chunkbytes: 100,
      last: true,
    });
    ...
    expect(() => streamparsechunkof({ index: 0, text: "a".repeat(50), chunkbytes: 10, last: true })).toThrow(
      /byte bound/i,
    );
  });
...
describe("chunkextract windowing, resume and merge", () => {
  ...
  it("opens a cursor with its table fingerprint and slices the table into row windows", () => {
    ...
    expect(() => openchunkextractcursor({ tableid: "t", fingerprint: "f", rows: 5, window: 0 })).toThrow(
      /positive row count/i,
    );
  });
  ...
  it("resumes from its cursor after an interruption and refuses changed fingerprints", () => {
    ...
    const completed = resumechunkextract({
      cursor: { ...advanced, complete: true },
      tableid: "table1",
      fingerprint: "abc12345",
    });
  ...
  it("reads the cursor progress line and shapes the worker task payloads", () => {
    ...
    expect(() =>
      chunkextracttaskof({ runid: "run1", stepid: "s1", tableid: "t", fingerprint: "f", rowindex: 0, window: 0 }),
    ).toThrow(/positive row window/i);
  });
});
