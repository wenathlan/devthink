...
import { appendvisit, auditeventof, buckettimeline, finalurlof, lockblocks, lockwindowof, releasesessionlock, acquiresessionlock, runtimelineof, runwithvisits, sessionwithlock, stepeventof, stalelocks, tabisolate, tabkey, tabnamespace, visitof, visitsummary } from "../run.js";
import { lockwindowvalid, sessionlockgate, tabisolategate, urlhistorygate, urlhistoryscopegate, timelinereadonlygate } from "../policy.js";
import { memoryitemframe, parseproposal, requestbody, runtimelinereport, urlvisitscheck } from "../protocol.js";
import { sessionmemory } from "../memory.js";
import { protocolversion, type agentplan, type agentsession, type auditevent, type lockrecord, type runrecord, type toolstep, type urlvisit } from "../types.js";
...
function runof(runid: string): runrecord { return { runid, planid: "plan", sessionid: "session", state: "running", createdat: now, updatedat: now }; }
...
describe("urlhistory", () => {
  it("appends one urlvisit per navigation, folds consecutive duplicates and stays scoped per run", () => {
    ...
    expect(urlhistorygate({ visit: { url: "https://example.com/page", runid: "run1" }, origin: "https://example.com" }).allowed).toBe(true);
    expect(urlhistorygate({ visit: { url: "https://evil.example/page", runid: "run1" }, origin: "https://example.com" }).allowed).toBe(false);
    expect(urlhistorygate({ visit: { url: "https://other.example/page", runid: "run1" }, origin: "https://example.com", grants: ["https://other.example"] }).allowed).toBe(true);
    expect(urlvisitscheck({ visits: [first, visitof({ url: "https://evil.example/x", runid: "run1", now })], origin: "https://example.com" })).toEqual({ ok: false, refused: ["https://evil.example/x"] });
    ...
    expect(finalurlof({ plannedurl: "https://example.com/a", finalurl: "https://example.com/b" })).toBe("https://example.com/b");
  });
});
...
describe("runtimeline", () => {
  it("merges step results, audit events and url visits into one ordered stream and buckets the phases", () => {
    ...
    const audits: auditevent[] = [{ id: "a1", kind: "approval", at: now + 50, summary: "The user approved the plan.", sessionid: "session", planid: "plan" }, { id: "a2", kind: "visit", at: now + 150, summary: "The run visited the page.", sessionid: "session", planid: "plan" }];
  ...
  it("persists the urlhistory and the runtimeline per run through the memory accessors", async () => {
    ...
    await store.appendtimelineevent(stepeventof({ runid: "run1", stepid: "s2", summary: "read", ok: true, now: now + 10 }));
...
describe("sessionlock", () => {
  it("acquires, blocks the concurrent run by naming the holder, releases and expires with the user window", () => {
    ...
    expect(lock).toEqual({ holder: "run1", runid: "run1", sessionid: "session", acquiredat: now, expiresat: now + 120_000 });
    ...
    expect(stalelocks([lock, { ...lock, runid: "run2", holder: "run2", expiresat: now + 999_999 }], now + 121_000).map(entry => entry.runid)).toEqual(["run1"]);
  ...
  it("preserves the lockid across proposal replays and carries the tab namespace with the observation provenance", () => {
    const proposal = { version: protocolversion, plan: { id: "plan", objective: "Submit once", origin: "https://example.com", steps: [{ id: "s1", kind: "observe", summary: "Read the page." }] }, lockid: "run1" };
    const parsed = parseproposal(proposal, "https://example.com");
    expect(parsed.lockid).toBe("run1");
    expect(parseproposal({ version: protocolversion, plan: proposal.plan }, "https://example.com").lockid).toBeUndefined();
    const body = JSON.parse(requestbody({ objective: "o", session, observation: { schemaversion: 1, url: "https://example.com", title: "t", textpreview: "", textlength: 0, forms: [], interactive: [], capturedat: now }, capabilities: { tabs: false, downloads: false, clipboardread: false, clipboardwrite: false, reportedat: now }, runstate: "running", tabnamespace: "tab:4", provenance: [{ origin: "https://example.com", runid: "run1", stepid: "s1", capturedat: now }] }));
    ...
    expect(() => memoryitemframe({ key: "k", value: 1, provenance: { origin: "https://example.com", runid: "run1", capturedat: now } })).toThrow(/provenance/i);
  });
});
