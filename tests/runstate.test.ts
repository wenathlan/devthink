...
function run(overrides: Partial<ReturnType<typeof openrun>> = {}) {
  return {
    ...openrun({ runid: "run1", sessionid: "session1", planid: "plan1", profileid: "profile1", interval, now }),
    ...overrides,
  };
}
...
describe("run state and keepalive", () => {
  it("opens the typed run state with its start event and an open keepalive port", () => {
    ...
    expect(() => openrun({ runid: " ", sessionid: "s", planid: "p", profileid: "d", interval, now })).toThrow(
      /run and session ids/i,
    );
    expect(() => openrun({ runid: "r", sessionid: "s", planid: "p", profileid: "d", interval: 0, now })).toThrow(
  ...
  it("records the url history, the environment and the worker turnaround of every step", () => {
    ...
    expect(state.lastprovenance).toEqual({
      origin: "https://example.com",
      stepid: "s2",
      environment: "offscreenworker",
    });
    ...
    expect(() =>
      recordenvironment(state, { stepid: " ", environment: "pagecontext", origin: "o", now: now + 6 }),
    ).toThrow(/step id/i);
  ...
  it("resumes exactly the pending step after a service worker restart", () => {
    ...
    const reaped = zombiesweep({
      states: [markpending(run(), "s3", now)],
      now: now + interval * 4,
      interval,
      missedlimit: 3,
    }).states[0];
  ...
  it("detects and reaps zombie runs whose heartbeat fell silent past the tolerated intervals", () => {
    ...
    const sweep = zombiesweep({
      states: [live, silent, stopped],
      now: now + interval * 3 + 1,
      interval,
      missedlimit: 3,
    });
  ...
  it("locks one session against concurrent runs through the storage level run lock", () => {
    ...
    const concurrent = acquirerunlock({
      locks: acquired.locks,
      sessionid: "session1",
      runid: "run2",
      holder: "executor",
      now: now + 1,
    ...
    expect(releaserunlock({ locks: acquired.locks, sessionid: "session1", runid: "runX", now: now + 5 }).released).toBe(
      false,
    );
    expect(() => acquirerunlock({ locks: [], sessionid: " ", runid: "r", holder: "h", now })).toThrow(
      /session and run ids/i,
    );
  });
  ...
  it("expires the run locks past their user configured expiry while locks without one keep holding", () => {
    ...
    const withother = acquirerunlock({
      locks,
      sessionid: "session2",
      runid: "run2",
      holder: "executor",
  ...
  it("serializes the steps that share one tab across parallel branches", () => {
    ...
    const single = serializesteps({
      branches: [
        { branchid: "a", steps: [{ stepid: "a1", tabid: 7 }] },
        { branchid: "b", steps: [{ stepid: "b1", tabid: 8 }] },
      ],
    });
  ...
  it("seals the persisted run state with its integrity digest and refuses a tampered seal", async () => {
    const state = recordenvironment(run(), {
      stepid: "s1",
