...
async function cell(spec, body) {
  try {
    ...
    cells.push({ family: spec.family, name: spec.name, outcome: "fail", detail: error instanceof Error ? error.message : String(error) });
  }
}
...
export async function runmatrixverifysuite() {
  if (!existsSync("dist/index.js") || !existsSync("dist/policy.js") || !existsSync("dist/protocol.js")) {
    console.error("MATRIXVERIFY The gate reads the compiled library, policy and protocol bundles; run pnpm build before the matrix verification.");
    return { release, cells: [{ family: "build", name: "the compiled bundles exist", outcome: "fail", detail: "the dist bundles are absent" }], coverage: 0, summary: { total: 1, passed: 0, failed: 1 } };
  }
  ...
  for (const browser of faketabprovider.kinds) {
    await cell({ family: "kinds", name: `every action kind answers the step policy on the ${browser} fake tab` }, async () => {
      let reads = 0;
      let interactions = 0;
      let sensitive = 0;
      for (const kind of catalog) {
        const step = { id: `matrix-${browser}-${kind}`, kind, summary: `The matrix cell of ${kind} on ${browser}`, target: policy.needstarget(kind) ? "https://example.org/#main" : undefined, value: kind === "navigate" ? "https://example.org/" : undefined };
        const verdict = policy.validatestep(step, "https://example.org");
        check(verdict !== undefined && typeof verdict.allowed === "boolean", `the kind ${kind} crashed the step policy instead of answering a verdict`);
  ...
  await cell({ family: "surfaces", name: "every browser surface page opens with its script and style" }, async () => {
    ...
    for (const page of [...interfacepages, ...hostpages]) {
      ...
      if (interfacepages.includes(page)) check(html.includes("style.css"), `the interface page ${page} misses the shared style sheet`);
    }
  ...
  await cell({ family: "cli", name: "the cli surface answers the manifest and help commands in library mode" }, async () => {
    ...
    check(manifest.stdout.includes("1.1.") || manifest.stdout.includes(release), "the manifest command printed no release line");
    check(help.stdout.includes("manifest") && help.stdout.includes("serve"), "the help command listed no command surface");
  ...
  await cell({ family: "mcp", name: "the mcp surface handshakes through the serve initialization and the stdio bridge contract" }, async () => {
    const catalogbuilt = library.buildtoolcatalog();
    const handshake = library.serveinitialize({ params: { clientinfo: { name: "matrixverify", version: "1" } }, config: { transport: "stdio", enabled: true }, catalog: catalogbuilt, templates: [] });
  ...
  await cell({ family: "migration", name: "the migration paths from version one plans answer the protocol negotiation" }, async () => {
    ...
    check(fresh.agreed === true && fresh.major === 2, `a fresh client did not agree on the frozen major two: ${JSON.stringify(fresh)}`);
    check(legacy.agreed === false && (legacy.reason ?? "").includes("migrateplan"), "the version one client did not refuse below the sunset floor with the migration path");
    check(future.agreed === false && (future.reason ?? "").includes("supported"), "the future client did not refuse with the supported range");
    check(shared.shared === true && shared.major === 2, "the client list that carries both majors did not negotiate up to the frozen major two");
  ...
  await cell({ family: "importers", name: "the workflow importer dry runs every workflow fixture of the dist set" }, async () => {
    ...
    for (const file of files) {
      const outcome = await execute(process.execPath, ["dist/cli.js", "runworkflow", join(workflowdirectory, file), "--dryrun", "--quiet"], { cwd: process.cwd() });
  ...
  await cell({ family: "importers", name: "the session importer round trips one session file" }, async () => {
    const record = library.newsessionrecord({ id: "matrix-session", name: "matrix", createdat: 1_800_000_000_000, tabs: [], captures: [], storage: [], cookies: [] });
    const file = library.exportsessionfile([record], 1_800_000_000_000);
    const imported = library.importsessionfile(file);
    check(imported !== undefined && imported.recordids[0] === "matrix-session", "the session file did not round trip through the importer");
    const payload = library.importexportpayloadof({ profile: "matrix", originprofiles: [], siteprofiles: [], notes: [], preferences: {}, at: 1_800_000_000_000 });
  ...
  await cell({ family: "gates", name: "the pool audit gate runs green over the 626 item pool" }, async () => runcells("tests/poolaudit.mjs"));
  await cell({ family: "gates", name: "the api freeze gate runs green over the frozen contract" }, async () => runcells("tests/apifreeze.mjs"));
  await cell({ family: "gates", name: "the csp audit gate runs green over the content security policy" }, async () => runcells("tests/cspaudit.mjs"));
  await cell({ family: "gates", name: "the permission diff gate runs green over the versioned baseline" }, async () => runcells("tests/permdiff.mjs"));
  for (const [gate, artifact, field] of [["agentcert", "tests/artifacts/agentcert.json", (report) => report.summary.failed], ["costcert", "tests/artifacts/costcert.json", (report) => report.summary.failed], ["doccheck", "tests/artifacts/doccheck.json", (report) => report.summary.failed], ["sweep", "tests/artifacts/sweep.json", (report) => report.summary.open]]) {
...
if (invokeddirectly) {
  ...
  console.log(`Matrix verified for ${release}: ${report.summary.passed} of ${report.summary.total} cells green, ${report.coverage}% coverage of the candidate matrix.`);
