...
describe("generated release notes", () => {
  ...
  it("carries the per channel sections of the publishing pipeline with every channel artifact", async () => {
    ...
    const releasenotes = await readFile("docs/releasenotes.md", "utf8");
    for (const channel of ["npm", "nuget", "maven", "container", "rubygems", "vscode", "firefox", "safari", "chromium", "site", "declarations", "provenance", "github"]) {
...
describe("the 2.0.13 maintenance ladder release notes", () => {
  ...
  it("keeps the 2.0.12 marginal ceiling record with the 2.0.11 chain behind it", async () => {
    ...
    for (const marker of ["a sixteen millisecond overrun of a green test", "the native ceiling itself was the defect, not the test", "the esm core build reads sixty seconds natively", "the whole-matrix build reads one hundred twenty seconds", "the same `devthink_test_timeout_ms` the emulated arm64 container leg exports"]) expect(twelvesection.toLowerCase()).toContain(marker);
  });
  ...
  it("keeps the 2.0.11 sanitizer record with the 2.0.10 chain behind it", async () => {
    ...
    for (const marker of ["closes the two code scanning alerts the re-analysis of the security page surfaced", "the script stripper of `withoutscripts`", "incomplete multi character sanitization", "an unterminated block still strips to the fragment end", "the sweep answers the same ten of ten checks green", "the code scanning page answers zero open alerts"]) expect(elevensection.toLowerCase()).toContain(marker);
  });
  ...
  it("keeps the 2.0.10 emulation record with the 2.0.9 chain behind it", async () => {
    ...
    for (const marker of ["the arm64 leg runs interpreted through qemu", "fourteen hit the five second vitest default timeout", "the linearity budget of the adversarial star storm", "nothing is skipped, nothing is relaxed", "the vitest config keeps the env-scaled timeout"]) expect(tensection.toLowerCase()).toContain(marker);
  });
  ...
  it("keeps the 2.0.9 browser fix record with the 2.0.8 chain behind it", async () => {
    ...
    for (const marker of ["the delegated browser verification of the 2.0.8 release zip against the sandbox host", "cannot read properties of undefined (reading 'length')", "the context now unwraps the envelope at the source", "the registry now answers the boolean grammar the commands declare", "a `line` text helper that never existed", "line is not defined", "every reviewed step has executed and the plan is closed", "zero console errors answer across the host, the popup and the sidepanel"]) expect(ninesection.toLowerCase()).toContain(marker);
  });
  ...
  it("keeps the 2.0.8 metadata and container record with the 2.0.7 chain behind it", async () => {
    ...
    for (const marker of ["the verify lanes caught the release metadata drift before anything published", "the apifreeze test refused", "points at the commit that never released", "the api freeze artifact and the cap manifests re-freeze at 2.0.8", "zero added, zero removed, nothing reordered", "the runners own the authoritative build"]) expect(eightsection.toLowerCase()).toContain(marker);
  });
  ...
  it("keeps the 2.0.7 security and container record with the 2.0.6 ladder chain behind it", async () => {
    ...
    for (const marker of ["the two open code scanning alerts", "linear string scans", "the four arch/os entries", "linux/amd64, linux/arm64", "the attestation feature stays", "referrers fallback tag", "the digest files that pin the exact image hash", "dynamically from the registry answer", "no digest written in the workflow"]) expect(sevensection.toLowerCase()).toContain(marker);
    const six = /^##\s+2\.0\.6(?:\s|$)/m.exec(changelog);
    expect(six).not.toBeNull();
    const sixsection = changelog.slice(six?.index ?? 0).split(/^##\s+/m)[1] ?? "";
    for (const marker of ["end-user verification of the published 2.0.5 package", "the package-scoped checksums verifying all 141 shipped files", "six icon errors and exit one from the installed package", "the browser bundle rides the release zip channel", "the identity-digest fail-soft", "a declared icon missing from a present bundle remains an error", "the unbundled context answers the release-zip note with exit zero"]) expect(sixsection.toLowerCase()).toContain(marker);
    const five = /^##\s+2\.0\.5(?:\s|$)/m.exec(changelog);
    expect(five).not.toBeNull();
    const fivesection = changelog.slice(five?.index ?? 0).split(/^##\s+/m)[1] ?? "";
    for (const marker of ["works-on-my-machine", "the validate:candidate script now runs the soak and the wcag sweeps"]) expect(fivesection.toLowerCase()).toContain(marker);
    const four = /^##\s+2\.0\.4(?:\s|$)/m.exec(changelog);
    expect(four).not.toBeNull();
    const foursection = changelog.slice(four?.index ?? 0).split(/^##\s+/m)[1] ?? "";
    for (const marker of ["counts the `package/` root prefix", "three real directory segments"]) expect(foursection.toLowerCase()).toContain(marker);
    const three = /^##\s+2\.0\.3(?:\s|$)/m.exec(changelog);
    expect(three).not.toBeNull();
    const threesection = changelog.slice(three?.index ?? 0).split(/^##\s+/m)[1] ?? "";
    for (const marker of ["flat package organization pass", "one file per correlated domain", "nothing nested past two directories"]) expect(threesection.toLowerCase()).toContain(marker);
  });
  ...
  it("assembles the full chain from the 1.1.31 base into the release notes", async () => {
    ...
    for (const phase of ["The base", "The agentic core", "The platform depth", "The interface and ecosystem surface", "The operator surface", "The freeze, the certification and the candidates"]) expect(releasenotes).toContain(`### ${phase} (`);
    for (const marker of ["1.1.31 — the frozen baseline the chain builds on", "1.1.98 — the clean repository shape restoration", "1.1.99 — release candidate one", "### The migration steps for upgraders", "### The frozen protocol guarantees", "### The certified scenarios", "### The performance budget results", "### The security review summary"]) expect(releasenotes).toContain(marker);
  });
});
