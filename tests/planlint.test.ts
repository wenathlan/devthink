...
describe("planlint of plan files", () => {
  it("parses one plan file under schemastrict and refuses unknown fields with the path that names them", () => {
    ...
    expect(() => parseplanfile({ ...file(), steps: [{ ...(firststep as object), extra: 1 } as object] })).toThrow(
      /unknown field.*extra/,
    );
    expect(() => parseplanfile({ ...file(), origin: "http://example.org" })).toThrow(/HTTPS/);
    expect(() => parseplanfile({ ...file(), steps: [] })).toThrow(/at least one step/);
    expect(() => parseplanfile({ ...file(), steps: [{ id: "s", kind: "loop", label: "l", bound: 0 }] })).toThrow(
      /positive integer/,
    );
    expect(() =>
      parseplanfile({ ...file(), steps: [{ id: "s", kind: "retryaction", label: "l", attempts: -2 }] }),
  ...
  it("applies the same originprofile rules as the extension: a denied kind never lints clean", () => {
    const denied = lintplanfile({
      file: file({
        steps: [{ id: "type", kind: "type", label: "Type the query", target: "input", gate: true }],
        denials: ["type"],
      }),
      ruleset,
      capabilities,
  ...
  it("demands the same fresh class consents the extension demands", () => {
    ...
    expect(
      withoutconsent.some(
        (diagnostic) => diagnostic.code === "plan.consent.class" && diagnostic.message.includes("payment"),
      ),
    ).toBe(true);
    const consent: classconsent = {
      id: "c1",
      origin: "https://example.org",
      sensitiveclass: "payment",
      grantedat: now - 1000,
    } as classconsent;
  ...
  it("flags sensitive steps without an explicit gate declaration", () => {
    ...
    expect(
      ungated.some((diagnostic) => diagnostic.code === "plan.gate.declaration" && diagnostic.severity === "error"),
    ).toBe(true);
    const gated = lintplanfile({
      file: file({ steps: [{ id: "type", kind: "type", label: "Type the query", target: "input", gate: true }] }),
      ruleset,
      capabilities,
      now,
    });
  ...
  it("flags selectors that cannot resolve without a live page as info", () => {
    const templated = lintplanfile({
      file: file({
        steps: [{ id: "click", kind: "click", label: "Click the hit", target: "{{firsthit}}", gate: true }],
      }),
      ruleset,
      capabilities,
      now,
  ...
  it("flags unbounded loops and missing retry bounds as warnings", () => {
    ...
    const unbounded = lintplanfile({
      file: file({ steps: [{ id: "spin", kind: "loop", label: "Spin the rows" }] }),
      ruleset,
      capabilities,
      now,
    });
    expect(
      unbounded.some((diagnostic) => diagnostic.code === "plan.control.loopbound" && diagnostic.severity === "warn"),
    ).toBe(true);
    const unretrying = lintplanfile({
      file: file({ steps: [{ id: "retry", kind: "retryaction", label: "Retry the click" }] }),
      ruleset,
      capabilities,
      now,
    });
    expect(
      unretrying.some((diagnostic) => diagnostic.code === "plan.control.retrybound" && diagnostic.severity === "warn"),
    ).toBe(true);
    const bounded = lintplanfile({
      file: file({
        steps: [
          { id: "spin", kind: "loop", label: "Spin the rows", bound: 12 },
          { id: "retry", kind: "retryaction", label: "Retry the click", attempts: 3 },
        ],
      }),
      ruleset,
      capabilities,
  ...
  it("runs only the rules the compiled set carries so a cache from another release never lints half", () => {
    ...
    expect(
      lintplanfile({ file: typing, ruleset: withoutgate, capabilities, now }).some(
        (diagnostic) => diagnostic.code === "plan.gate.declaration",
      ),
    ).toBe(false);
    expect(
      lintplanfile({ file: typing, ruleset, capabilities, now }).some(
        (diagnostic) => diagnostic.code === "plan.gate.declaration",
      ),
    ).toBe(true);
  });
    ...
    const diagnostics: planlintdiagnostic[] = [
      {
        ...
        path: "steps[0].target",
        severity: "info",
        message: "A templated selector resolves only against the live page.",
      },
      {
        code: "plan.control.loopbound",
        path: "steps[1].bound",
        severity: "warn",
        message: "The loop carries no bound.",
      },
      {
        code: "plan.gate.declaration",
        path: "steps[2].gate",
        severity: "error",
        message: "A sensitive step declares no gate.",
      },
    ];
...
describe("the portable rule set", () => {
  ...
  it("gates the compiled set against drift", () => {
    ...
    expect(portablerulesetgate({ ...ruleset, rules: [...ruleset.rules, ruleset.rules[0] as never] }).allowed).toBe(
      false,
    );
    ...
    expect(portablerulesetgate({ ...ruleset, rules: ruleset.rules.map((rule) => ({ ...rule, id: "" })) }).allowed).toBe(
      false,
    );
