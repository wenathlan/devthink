...
function provider(id: string, over: Partial<providerconfig> = {}): providerconfig {
  return { id, name: `The ${id} gateway`, endpoint: `https://${id}.example/v1`, style: "chatcompletions", models: ["model-a", "model-b"], status: "available", createdat: now, ...over };
}

/** Builds one model route fixture. */
function route(over: Partial<modelroute> = {}): modelroute {
  return { id: "route1", kind: "draftplan", providerid: "prov1", model: "model-a", revision: 1, updatedat: now, ...over };
}
...
describe("modelroute", () => {
  ...
  it("keeps every route of one task kind in revision order", () => {
    const routes = [route({ revision: 1 }), route({ id: "route2", revision: 5 }), route({ id: "route3", kind: "reflect" })];
  ...
  it("refuses a route whose model stays outside the provider model list", () => {
    const resolved = resolveroute({ routes: [route({ model: "model-z" })], providers: [provider("prov1")], kind: "draftplan" });
  ...
  it("marks providers unavailable and available again after their calls", () => {
    const providers = markprovider({ providers: [provider("prov1"), provider("prov2")], providerid: "prov1", available: false, now });
  ...
  it("reports why nothing fell back without a fallback pair or with an unavailable fallback provider", () => {
    expect(fallbackroute({ routes: [route()], providers: [provider("prov1")], kind: "draftplan" }).reason).toMatch(/no user configured fallback/i);
    const routes = [route({ fallbackproviderid: "prov2", fallbackmodel: "model-b" })];
