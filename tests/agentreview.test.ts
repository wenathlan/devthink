...
describe("agentreview escalation blocking and review request routing", () => {
  it("blocks the raising agent until the human answers and lifts the hold with the decision", () => {
    const raised = escalate({
      id: "e1",
      agentid: "a1",
      subject: "Which pricing table should the extraction target?",
      context: "The page carries two tables and the plan covers one.",
      now,
    });
    ...
    const decided = resolveescalation({
      escalation: raised,
      decision: "Extract the second table; the first one repeats the header row.",
      now: now + 1_000,
    });
  ...
  it("routes one review request between two agents of the shared origin and records the verdict beside the original output", () => {
    ...
    expect(
      reviewrequestgate({ from, to: record({ id: "a3", name: "outsider", origin: "https://other.example" }) }).allowed,
    ).toBe(false);
    const request = reviewrecordof({
      id: "r1",
      fromagentid: "a1",
      toagentid: "a2",
      subject: "the extracted pricing rows",
      output: "rows: 12, columns: 4, currency: brl",
      now,
    });
    expect(request.state).toBe("open");
    expect(() =>
      reviewrecordof({ id: "r2", fromagentid: "a1", toagentid: "a1", subject: "self", output: "own", now }),
    ).toThrow(/never reviews its own/i);
    expect(() => recordverdict({ records: [request], id: "r1", reviewerid: "a3", verdict: "approve", now })).toThrow(
      /waits for the agent a2/i,
    );
    expect(() => recordverdict({ records: [request], id: "r1", reviewerid: "a2", verdict: "reject", now })).toThrow(
      /names its issues/i,
    );
    const answered = recordverdict({
      ...
      reviewerid: "a2",
      verdict: "changes",
      issues: ["the currency column missed the second page"],
      now: now + 1_000,
    });
    ...
    expect(() => recordverdict({ records: [answered], id: "r1", reviewerid: "a2", verdict: "approve", now })).toThrow(
      /never rewrites/i,
    );
  });
});
...
describe("agentreview runreplay capture and audit reconstruction", () => {
  it("captures the ordered steps and results of an agent run", () => {
    ...
    expect(() => runreplay({ id: "p2", agentid: "a1", runid: "run1", steps: [], now })).toThrow(
      /at least one recorded step/i,
    );
  });
  ...
  it("reconstructs a run from the audit trail when the run memory is gone and marks the reconstruction", () => {
    const events = [
      {
        id: "ev1",
        kind: "action",
        summary: "The observe step s1 completed.",
        at: now + 100,
        agentid: "a1",
        stepid: "s1",
        planid: "run1",
      },
      {
        id: "ev2",
        kind: "error",
        summary: "The extract step s2 failed.",
        at: now + 200,
        agentid: "a1",
        stepid: "s2",
        planid: "run1",
      },
      {
        id: "ev3",
        kind: "action",
        summary: "The navigate step s3 of another agent completed.",
        at: now + 300,
        agentid: "a2",
        stepid: "s3",
        planid: "run2",
    ...
    expect(() => reconstructreplay({ id: "p4", agentid: "a9", runid: "run9", events, now })).toThrow(
      /carries no event/i,
    );
...
describe("agentreview outputcompare alignment and consensusvote tallies", () => {
  it("aligns two agent outputs field by field with matching, conflicting and missing fields", () => {
    const comparison = outputcompare({
      id: "c1",
      subject: "the extracted totals",
      left: { agentid: "a1", fields: { rows: "12", currency: "brl", totals: "4" } },
      right: { agentid: "a2", fields: { rows: "12", currency: "usd" } },
      now,
    });
  ...
  it("collects one vote per agent, computes the outcome by the quorum rule and keeps the dissent", () => {
    const carried = consensusrecordof({
      id: "v1",
      proposal: "the fleet extracts the second pricing table",
      votes: [
    ...
    const failed = consensusrecordof({
      id: "v3",
      proposal: "the fleet opens a second tab",
      votes: [
    ...
    expect(() => consensusrecordof({ id: "v5", proposal: "no quorum", votes: [], quorum: 0, now })).toThrow(
