...
describe("protocol", () => {
    ...
    const proposal = parseproposal(
      {
        ...
        plan: {
          ...
          steps: [{ kind: "observe", summary: "Capture an approved page snapshot" }],
        },
      },
      "https://example.com",
    );
  ...
  it("rejects unsupported versions and cross-origin routes", () => {
    expect(() =>
      parseproposal({ version: "1.0.0", plan: { objective: "x", steps: [] } }, "https://example.com"),
    ).toThrow("protocol");
    expect(() =>
      parseproposal(
        {
          ...
          plan: {
            objective: "x",
            steps: [{ kind: "navigate", value: "https://elsewhere.example", summary: "Leave site" }],
          },
        },
        "https://example.com",
      ),
  ...
  it("accepts the expanded action vocabulary with derived risk levels", () => {
    const proposal = parseproposal(
      {
        version: protocolversion,
        plan: {
          objective: "Review a richer flow",
          steps: [
    ...
    expect(proposal.plan.steps.map((step) => step.risk)).toEqual([
      "interaction",
      "interaction",
      "sensitive",
      "read",
  ...
  it("accepts plans with an unlimited number of reviewed steps", () => {
    const steps = Array.from({ length: 150 }, (_, index) => ({
      kind: "wait",
      value: "1",
      summary: `Reviewed pause ${index + 1}.`,
    }));
    const proposal = parseproposal(
      { version: protocolversion, plan: { objective: "Long reviewed flow", steps } },
      "https://example.com",
    );
    expect(proposal.plan.steps).toHaveLength(150);
    expect(() =>
      parseproposal({ version: protocolversion, plan: { objective: "Empty flow", steps: [] } }, "https://example.com"),
    ...
    const proposal = parseproposal(
      {
        version: protocolversion,
        plan: {
          objective: "Long lived review",
          expiresat: twohours,
          steps: [{ kind: "observe", summary: "Observe once." }],
        },
      },
      "https://example.com",
    );
  ...
  it("shapes the proposal request with the capability report and wraps step outcomes", () => {
    const body = JSON.parse(
      requestbody({
        objective: "Read prices",
        session: { id: "session", tabid: 4, origin: "https://example.com", startedat: 1, expiresat: 2 },
        observation: {
          schemaversion: 2,
        ...
        capabilities: { tabs: true, downloads: false, clipboardread: false, clipboardwrite: false, reportedat: 1 },
      }),
    );
    ...
    const envelope = JSON.parse(
      outcomeresponse({
        outcome: { stepid: "step", ok: true, summary: "Done.", details: { count: 3 }, at: 5 },
        plan: {
          id: "plan",
          objective: "o",
  ...
  it("attaches the resolvedtarget summary to the outcome envelope for review", () => {
    ...
    const resolvedtarget = {
      ...
      tag: "button",
      label: "Sign in",
      geometry: { x: 12, y: 30, width: 90, height: 32 },
    };
    const envelope = JSON.parse(
      outcomeresponse({
        outcome: {
          ...
          details: { mode: "text", resolvedtarget },
          at: 9,
        },
        plan,
        resolvedtarget,
    ...
    const proposal = parseproposal(
      {
        version: protocolversion,
        plan: {
          objective: "Complete the login flow",
          steps: [
            { id: "map", kind: "mapclicks", summary: "Number every clickable element." },
            {
              kind: "movepointer",
              summary: "Travel the reviewed path.",
              options:
                '{"pointpath":{"start":{"x":0,"y":0},"end":{"x":120,"y":60},"duration":400},"speedprofile":{"easing":"easeinout","peak":800}}',
            },
            {
              kind: "clicktext",
              summary: "Click the sign in control by text.",
              options: '{"targetref":{"mode":"text","text":"Sign in"}}',
            },
            {
              kind: "clickaria",
              summary: "Click the submit control by aria pair.",
            ...
            },
            {
              ...
              summary: "Type the reviewed email slowly.",
              options: '{"delay":90}',
            },
            ...
            {
              kind: "retryaction",
              summary: "Retry the moving banner click.",
              options: '{"stepid":"banner","retryrule":{"attempts":3,"settle":200,"tolerance":4}}',
            },
      ...
      parseproposal(
        {
          version: protocolversion,
          plan: {
            objective: "Retry a ghost",
            steps: [
              {
                id: "wrapper",
                kind: "retryaction",
                summary: "Retry the ghost click.",
                options: '{"stepid":"ghost","retryrule":{"attempts":2}}',
              },
            ],
          },
        },
      ...
      parseproposal(
        {
          version: protocolversion,
          plan: {
            objective: "Frame a ghost",
            steps: [
              {
                id: "frame",
                kind: "enterframe",
                summary: "Route inside the frame.",
                options: '{"framepath":[0],"stepid":"ghost"}',
              },
            ],
          },
        },
  ...
  it("accepts the observation depth vocabulary with its reviewed options grammar", () => {
        ...
        plan: {
          ...
          steps: [
            ...
            { kind: "watchmutate", summary: "Watch the feed.", options: '{"lifetime":2000,"scopes":["#feed"]}' },
            {
              ...
              options: '{"quietrule":{"idle":400,"timeout":6000}}',
            },
    ...
    expect(() =>
      parseproposal(
        {
          version: protocolversion,
          plan: { objective: "Watch without a lifetime", steps: [{ kind: "watchmutate", summary: "Watch the feed." }] },
        },
        "https://example.com",
      ),
    ).toThrow("watch lifetime");
  });
  ...
  it("wraps the observation envelope with its a11y, reader, listpattern and tableshape sections", () => {
    ...
    const observationcapture: observation = {
      ...
      reader: {
        title: "Example",
        byline: "",
        blocks: [{ kind: "p", text: "text", words: 1 }],
        words: 1,
        characters: 4,
      },
      listpattern: [{ container: "ul", itemselector: "li.item", repeat: 3, samples: ["a", "b", "c"] }],
      tableshape: [
        { selector: "table", headers: ["Name"], columns: [{ label: "Name", cells: 3 }], rows: 3, caption: "" },
      ],
    };
  ...
  it("reports page language, template, scroll lock and banner state in the context payload", () => {
    expect(
      signalsreport({
        signals: { language: "pt-BR", template: "article", scrolllocked: true, banner: "cookie", refreshedat: 7 },
      }),
    ).toMatchObject({
      version: protocolversion,
      language: "pt-BR",
    ...
    expect(signalsreport({ signals: { language: "en", refreshedat: 8 } })).toEqual({
      version: protocolversion,
      language: "en",
    });
  ...
  it("accepts the navigation mastery vocabulary with its reviewed options grammar", () => {
    const proposal = parseproposal(
      {
        version: protocolversion,
        plan: {
          objective: "Move anywhere with review",
          steps: [
            {
              kind: "openlink",
              summary: "Open the reviewed url in a new tab.",
              options: '{"navtarget":{"url":"https://example.com/report","container":"tab","position":"adjacent"}}',
            },
            {
              kind: "openprivate",
              summary: "Open the reviewed url in a private window.",
              options: '{"navtarget":{"url":"https://example.com/report","container":"private"}}',
            },
            ...
            },
            {
              ...
              options:
                '{"waitprofile":{"signals":["load","networkidle"],"idle":500,"overrides":[{"origin":"https://example.com","idle":1500}]}}',
            },
            ...
            },
            {
              ...
              options: '{"origins":["https://cdn.example"]}',
            },
            {
              kind: "deeplink",
            ...
            {
              kind: "pausenav",
              summary: "Pause navigation for the consent prompt.",
              options: '{"reason":"a consent banner is open"}',
            },
            { kind: "navintent", value: "open the pricing page", summary: "Record the navigation intent." },
            {
              kind: "navrate",
              summary: "Apply the per domain rate limit.",
      ...
      parseproposal(
        {
          version: protocolversion,
          plan: {
            objective: "Open without a target",
            steps: [{ kind: "openlink", summary: "Open without a navtarget." }],
          },
        },
        "https://example.com",
      ),
    ...
    expect(() =>
      parseproposal(
        {
          version: protocolversion,
          plan: { objective: "Rate without a limit", steps: [{ kind: "navrate", summary: "Apply without a limit." }] },
        },
        "https://example.com",
      ),
    ).toThrow("ratelimit");
  ...
  it("carries the navigation trail of a session in the context envelope", () => {
    ...
    expect(report.trail[1]).toMatchObject({
...
describe("protocol tabs and windows command", () => {
      ...
      matches: [
        {
          ...
          muted: true,
          discarded: false,
          meta: { tabid: 4, taskrefs: ["plan"], provenance: "plan step", labels: ["research"], at: 5 },
        },
      ],
  ...
  it("carries layout snapshots with their window bounds and group states in the session context", () => {
    const layout: tablayout = {
      ...
      windows: [
        {
          windowid: 10,
          state: { bounds: { left: 0, top: 0, width: 1280, height: 800 }, maximized: false, profile: "normal" },
        },
      ],
      savedat: 42,
    };
  ...
  it("wraps form reports, error reports and wizard states in versioned envelopes", () => {
    const plan: agentplan = {
      ...
      origin: "https://example.com",
      steps: [{ id: "s1", kind: "detectfields", summary: "Detect the fields.", risk: "read" }],
      createdat: 1,
      expiresat: 2,
      state: "approved",
    };
    const report: formreport = {
      form: "#checkout",
      fields: [
        { selector: "#name", label: "Full name", kind: "text", matched: true },
        { selector: "#card", label: "Card number", kind: "card", matched: false },
      ],
    };
    const formenvelope = JSON.parse(formreportresponse({ report, plan })) as {
      version: string;
      planid: string;
      planstate: string;
      report: formreport;
    ...
    const errorenvelope = JSON.parse(errorreportresponse({ report: error, plan })) as {
      version: string;
      report: errorreport;
    };
    ...
    expect(wizardreport({ sessionid: "session", wizards, picks })).toEqual({
      version: protocolversion,
      sessionid: "session",
      wizards,
      picks,
  ...
  it("wraps dataset payloads with column specs and sampled rows in the dataset envelope", () => {
    ...
    const datasetvalue: dataset = {
      ...
      columns,
      rows,
      sources: [{ row: 0, url: "https://example.com/list", at: 5, stepid: "s1" }],
      at: 6,
    };
    const payload = JSON.parse(
      datasetresponse({ dataset: datasetvalue, plan: { ...plan, state: "approved" }, sample: 3 }),
    ) as { version: string; planstate: string; dataset: dataset & { totalrows: number } };
  ...
  it("carries extraction progress and provenance records per exported artifact in the context envelopes", () => {
    const sessionvalue: extractsession = {
      id: "e1",
      datasetid: "d1",
      name: "catalog",
      target: "table",
      next: "a.next",
      planned: 3,
      pages: ["https://example.com/1"],
      rows: 10,
      cursor: 1,
      startedat: 1,
      updatedat: 2,
    };
    expect(extractionreport({ sessions: [sessionvalue] })).toEqual({
      version: protocolversion,
    ...
    const proposal = parseproposal(
      {
        version: protocolversion,
        plan: {
          objective: "Scrape the catalog",
          steps: [
            {
              kind: "scrapetable",
              target: "table.prices",
              options: '{"name":"catalog","rowlimit":200}',
              summary: "Scrape the price table.",
            },
            {
              kind: "transformvalues",
              options: '{"dataset":"d1","rules":[{"expression":"upper","sources":["name"],"target":"name"}]}',
              summary: "Uppercase the names.",
            },
          ],
        },
      },
      ...
      parseproposal(
        {
          version: protocolversion,
          plan: {
            objective: "Bad transform",
            steps: [
              {
                kind: "transformvalues",
                options: '{"dataset":"d1","rules":{"expression":"upper"}}',
                summary: "Rules must be a list.",
              },
            ],
          },
        },
    ...
    const proposal = parseproposal(
      {
        version: protocolversion,
        plan: {
          objective: "Fill and submit the checkout",
          steps: [
            { kind: "detectfields", summary: "Detect the form fields." },
            {
              kind: "fillform",
              options:
                '{"formrecord":{"form":"#checkout","entries":[{"match":{"mode":"label","label":"Full name"},"kind":"text","value":"Ana Alves"}]}}',
              summary: "Fill from the structured record.",
            },
            { kind: "asksubmit", value: "#checkout", summary: "Ask before submitting." },
            {
              ...
              summary: "Submit through the owning form.",
            },
            ...
            steps: [
              {
                kind: "submitform",
                target: "#checkout",
                options: '{"consentref":"missing"}',
                summary: "Submit without asksubmit.",
              },
            ],
          },
        },
...
describe("files, clipboard and downloads protocol", () => {
  ...
  it("wraps download, netlog and quarantine payloads in the versioned envelopes", () => {
    ...
    const envelope = JSON.parse(downloadreport({ downloads, plan: { ...plan, state: "approved" } })) as {
      version: string;
      planid: string;
      planstate: string;
    ...
    expect(envelope.downloads[0]).toMatchObject({
      state: "complete",
      path: "/downloads/report.pdf",
      checksum: "fnv1a-abc",
      bytes: 2048,
    ...
    const entries: quarantineentry[] = [
      {
        ...
        reason: "mime application/pdf matched",
        scan: "clean",
        release: "user-9",
        at: 9,
        updatedat: 10,
    ...
    const proposal = parseproposal(
      {
        version: protocolversion,
        plan: {
          objective: "Move the files safely",
          steps: [
            {
              kind: "batchdownload",
              options:
                '{"downloadspec":{"urls":["https://example.com/report.pdf","https://example.com/data.zip"],"filename":"report","complete":"size"},"concurrent":3}',
              summary: "Download the reviewed batch.",
            },
            ...
            {
              kind: "readclipboard",
              options: '{"consentref":"clip-1","prompt":"Read the tracking number."}',
              summary: "Read the clipboard once.",
            },
      ...
      parseproposal(
        {
          version: protocolversion,
          plan: {
            objective: "Read without consent",
            steps: [{ kind: "readclipboard", summary: "Read the clipboard without a consent ref." }],
          },
        },
        "https://example.com",
      ),
      ...
      parseproposal(
        {
          ...
          plan: {
            ...
            steps: [
              {
                kind: "interceptmime",
                options: '{"mimefilter":{"include":[],"exclude":[],"default":"deny"}}',
                summary: "Empty include list.",
              },
            ],
          },
        },
...
describe("media capture protocol", () => {
  ...
  it("wraps the capture block into the outcome envelope and carries the capture report", () => {
    const outcome: stepoutcome = {
      stepid: "shot-1",
      ok: true,
      summary: "Captured the visible viewport.",
      details: { capture: { id: "cap-1", kind: "shotview", format: "png", width: 1280, height: 800, bytes: 5200 } },
      at: 3,
    };
    const envelope = JSON.parse(
      outcomeresponse({ outcome, plan, capture: { id: "cap-1", format: "png", bytes: 5200 } }),
    ) as { version: string; capture: { id: string; format: string; bytes: number } };
    expect(envelope).toMatchObject({ version: protocolversion, planid: "p1", planstate: "approved" });
  ...
  it("accepts the capture vocabulary, refuses unknown formats and negative region coordinates", () => {
    const proposal = parseproposal(
      {
        version: protocolversion,
        plan: {
          objective: "See the page with review",
          steps: [
            {
              kind: "shotview",
              options: '{"capture":{"format":"png","pixelratio":2}}',
              summary: "Shoot the viewport.",
            },
            {
              kind: "shotfullpage",
              options: '{"settle":150,"overlap":40,"wait":60000}',
            ...
            {
              kind: "contactsheet",
              options: '{"elements":["#a","#b"],"sheet":{"cellsize":240,"columns":3,"label":"both"}}',
              summary: "Tile the cards.",
            },
          ],
        },
      },
      ...
      parseproposal(
        {
          version: protocolversion,
          plan: {
            objective: "Bad format",
            steps: [{ kind: "shotview", options: '{"capture":{"format":"gif"}}', summary: "Shoot with a bad format." }],
          },
        },
        "https://example.com",
      ),
      ...
      parseproposal(
        {
          version: protocolversion,
          plan: {
            objective: "Negative region",
            steps: [
              {
                kind: "shotregion",
                options: '{"regionrect":{"x":-1,"y":0,"width":40,"height":30},"reviewed":true}',
                summary: "Shoot a negative region.",
              },
            ],
          },
        },
    ...
    ).toThrow("negative coordinates");
    expect(() =>
      parseproposal(
        {
          version: protocolversion,
          plan: {
            objective: "Unreviewed region",
            steps: [
              {
                kind: "shotregion",
                options: '{"regionrect":{"x":0,"y":0,"width":40,"height":30}}',
                summary: "Shoot an unreviewed region.",
              },
            ],
          },
        },
...
describe("media capture part two protocol", () => {
  const plan = {
    id: "p1",
    objective: "See and hear the page",
    origin: "https://example.com",
    steps: [],
    createdat: 1,
    expiresat: 2,
  ...
  it("wraps the media block into the outcome envelope and carries the media report", () => {
    const outcome: stepoutcome = {
      stepid: "pdf-1",
      ok: true,
      summary: "Composed a derived pdf report.",
      details: { media: { id: "pdf-1", kind: "pdf", bytes: 4200 } },
      at: 3,
    };
    const envelope = JSON.parse(
      outcomeresponse({ outcome, plan, media: { id: "pdf-1", kind: "pdf", bytes: 4200 } }),
    ) as { version: string; media: { id: string; kind: string; bytes: number } };
    expect(envelope).toMatchObject({ version: protocolversion, planid: "p1", planstate: "approved" });
    ...
    const records: mediarecord[] = [
      {
        ...
        pagewidth: 612,
        pageheight: 792,
        margins: { top: 28.8, right: 28.8, bottom: 28.8, left: 28.8 },
        landscape: false,
        scale: 1,
        bytes: 4200,
        at: 3,
  ...
  it("accepts the media vocabulary and rejects negative margins, bad scopes and negative intervals", () => {
    const proposal = parseproposal(
      {
        version: protocolversion,
        plan: {
          objective: "Capture media with review",
          steps: [
            {
              kind: "capturepdf",
              options:
                '{"pdf":{"paperwidth":8.5,"paperheight":11,"margins":{"top":0.4,"right":0.4,"bottom":0.4,"left":0.4},"scale":1,"paginate":true},"breakpoints":["#section-2"],"exporttarget":"memory"}',
              summary: "Print the report.",
            },
            {
              kind: "recordscreen",
              options: '{"recording":{"scope":"run","fps":2},"duration":1000,"consentref":"c1"}',
              summary: "Record the run.",
            },
            {
              kind: "captureaudio",
            ...
            },
            {
              ...
              options: '{"timestamp":5,"poster":true}',
              summary: "Grab the frame.",
            },
            {
              kind: "downloadimages",
              options: '{"imagefilter":{"selector":"main img","minwidth":200}}',
              summary: "Download the images.",
            },
            {
              kind: "shotcanvas",
              target: "canvas.chart",
              options: '{"capture":{"format":"webp"}}',
            ...
            {
              kind: "timelapse",
              options: '{"lapse":{"interval":500,"duration":5000},"wait":6000}',
              summary: "Shoot the lapse.",
            },
            {
              kind: "convertimage",
              options: '{"capture":"cap-1","convert":{"target":"webp","quality":85}}',
              summary: "Convert the capture.",
            },
            {
              kind: "makethumbs",
              options: '{"captures":["cap-1"],"thumb":{"size":240,"fit":"cover","suffix":"thumb"}}',
      ...
      parseproposal(
        {
          version: protocolversion,
          plan: {
            objective: "Negative margins",
            steps: [
              {
                kind: "capturepdf",
                options: '{"pdf":{"margins":{"left":-0.2}}}',
                summary: "Print with negative margins.",
              },
            ],
          },
        },
    ...
    ).toThrow("negative margins are refused");
    expect(() =>
      parseproposal(
        {
          version: protocolversion,
          plan: {
            ...
            steps: [
              {
                kind: "recordscreen",
                options: '{"recording":{"scope":"window"},"consentref":"c1"}',
                summary: "Record with a bad scope.",
              },
            ],
          },
        },
    ...
    ).toThrow("scope must be tab or run");
    expect(() =>
      parseproposal(
        {
          version: protocolversion,
          plan: {
            objective: "Negative interval",
            steps: [
              {
                kind: "timelapse",
                options: '{"lapse":{"interval":-500,"duration":5000}}',
                summary: "Shoot a negative interval.",
              },
            ],
          },
        },
      ...
      parseproposal(
        {
          version: protocolversion,
          plan: {
            objective: "Recording without consent",
            steps: [
      ...
      parseproposal(
        {
          version: protocolversion,
          plan: {
            objective: "Lapse over budget",
            steps: [
              {
                kind: "timelapse",
                options: '{"lapse":{"interval":500,"duration":9000},"wait":5000}',
                summary: "Shoot past the budget.",
              },
            ],
          },
        },
    ...
    ).toThrow("exceeds the reviewed wait budget");
  });
});
...
describe("network observation part one protocol", () => {
  const plan = {
    id: "p1",
    objective: "Speak to the network under review",
    origin: "https://example.com",
    steps: [],
    createdat: 1,
    expiresat: 2,
  ...
  it("wraps the transport block into the outcome envelope and carries the calls report without bodies", () => {
    const outcome: stepoutcome = {
      stepid: "f1",
      ok: true,
      summary: "Fetched the reviewed url.",
      details: { transport: { status: 200, headers: ["content-type"], bytes: 512, duration: 340 }, retries: 1 },
      at: 3,
    };
    const envelope = JSON.parse(
      outcomeresponse({
        outcome,
        plan,
        transport: { status: 200, headers: ["content-type"], bytes: 512, duration: 340 },
      }),
    ) as { version: string; transport: { status: number; headers: string[]; bytes: number; duration: number } };
    expect(envelope).toMatchObject({ version: protocolversion, planid: "p1", planstate: "approved" });
  ...
  it("rejects fetch requests to ungranted origins and header allowlists with empty names", () => {
    ...
    const granted = parseproposal(
      {
        version: protocolversion,
        plan: {
          objective: "Fetch the granted api",
          steps: [
            {
              kind: "fetchurl",
              options: '{"fetch":{"url":"https://api.example/data"}}',
              summary: "Fetch the granted api origin.",
            },
          ],
        },
      },
      "https://example.com",
      ["https://example.com", "https://api.example"],
    );
      ...
      parseproposal(
        {
          version: protocolversion,
          plan: {
            objective: "Empty header name",
            steps: [
              {
                kind: "fetchurl",
                options: '{"fetch":{"url":"https://example.com/data","headers":{"":"v"}}}',
                summary: "Fetch with an empty header name.",
              },
            ],
          },
        },
    ...
    const proposal = parseproposal(
      {
        version: protocolversion,
        plan: {
          objective: "Observe the network",
          steps: [
            {
              kind: "fetchurl",
              options:
                '{"fetch":{"url":"https://example.com/data","method":"POST","headers":{"x-request-id":"42"},"body":"{}","mode":"cors"},"fetchoptions":{"timeout":1000,"retries":2,"backoff":100,"follow":3},"stream":{"budget":4096},"wait":5000,"consentref":"c1"}',
              summary: "Fetch a reviewed url.",
            },
            {
              kind: "parsejson",
              options: '{"call":"c1","fields":[{"name":"title","path":"data.title","kind":"text"}]}',
              summary: "Parse the fetched body.",
            },
            {
              kind: "parsehtml",
              options: '{"call":"c1","queries":[{"selector":"a.link","attribute":"href","multi":true}]}',
              summary: "Query the fetched markup.",
            },
            {
              kind: "callgraphql",
              options: '{"endpoint":"graph","graphql":{"query":"query{hero}","operationkind":"query"}}',
              summary: "Run the reviewed query.",
            },
          ],
        },
      },
      ...
      parseproposal(
        {
          ...
          plan: {
            ...
            steps: [
              {
                kind: "parsejson",
                options: '{"call":"c1","fields":[{"name":"a","path":"bad..path"}]}',
                summary: "Parse a bad path.",
              },
            ],
          },
        },
...
describe("network observation part two protocol", () => {
  ...
  it("wraps step outcomes with the network block of exchange counts and channel state", () => {
    const plan: agentplan = {
      ...
      objective: "Observe",
      origin,
      steps: [{ id: "s1", kind: "watchrequests", summary: "Watch.", risk: "read" }],
      createdat: now,
      expiresat: now + 1000,
      state: "approved",
    };
    const outcome: stepoutcome = {
      stepid: "s1",
      ok: true,
      summary: "Observed 3 requests.",
      details: { network: { exchanges: 3, channelstate: "none", messages: 0 } },
      at: now,
    };
    const parsed = JSON.parse(
      outcomeresponse({ outcome, plan, network: { exchanges: 3, channelstate: "none", messages: 0 } }),
    ) as { network?: { exchanges: number; channelstate: string; messages: number } };
  ...
  it("rejects channel urls outside the grants, subscriptions without cancellation and lifetimes beyond the plan window", () => {
    const planof = (steps: Array<Record<string, unknown>>, expiresat?: number): unknown => ({
      version,
      plan: { objective: "Observe", origin, steps, ...(expiresat !== undefined ? { expiresat } : {}) },
    });
    const stepinput = (kind: string, options: Record<string, unknown>): Record<string, unknown> => ({
      id: "s1",
      kind,
      summary: "Observe.",
      options: JSON.stringify(options),
    });
    const socket = parseproposal(
      planof([stepinput("opensocket", { socket: { url: "wss://api.example/live" } })]) as Record<string, unknown>,
      origin,
      [origin, "https://api.example"],
    );
    ...
    expect(() =>
      parseproposal(
        planof([stepinput("opensocket", { socket: { url: "wss://other.example/live" } })]) as Record<string, unknown>,
        origin,
        [origin],
      ),
    ).toThrow("outside the grants");
    expect(() =>
      parseproposal(
        planof([
          stepinput("subscribesse", {
            subscription: { url: "https://other.example/stream", cancel: { kind: "stop", value: "done" } },
          }),
    ...
    expect(() =>
      parseproposal(
        planof([stepinput("subscribesse", { subscription: { url: "https://api.example/stream" } })]) as Record<
          string,
          unknown
        >,
        origin,
    ...
    ).toThrow("cancellation path");
    expect(() =>
      parseproposal(
        planof([
          stepinput("longpoll", { poll: { url: "https://api.example/poll", cursorfield: "c", interval: 50 } }),
        ]) as Record<string, unknown>,
        origin,
        ["https://api.example"],
      ),
    ...
    expect(() =>
      parseproposal(
        planof(
          [stepinput("opensocket", { socket: { url: "wss://api.example/live", lifetime: 60_000 } })],
          Date.now() + 5000,
        ),
        origin,
        ["https://api.example"],
      ),
    ).toThrow("exceeds the reviewed plan window");
    expect(() =>
      parseproposal(
        planof(
          [
            stepinput("subscribesse", {
              subscription: {
                url: "https://api.example/stream",
                lifetime: 60_000,
                cancel: { kind: "lifetime", value: 60_000 },
              },
            }),
          ],
          Date.now() + 5000,
    ...
    expect(() =>
      parseproposal(
        planof(
          [stepinput("opensocket", { socket: { url: "wss://api.example/live", lifetime: 4000 } })],
          Date.now() + 5000,
        ),
        origin,
        ["https://api.example"],
  ...
  it("grades conditional capturebodies and extractapi steps through the resolved risk", () => {
    ...
    const privatebodies = parseproposal(
      {
        version,
        plan: {
          objective: "Capture",
          origin,
          steps: [stepinput("capturebodies", { body: { mimes: ["application/json"], ceiling: 100 } })],
        },
      },
      origin,
      [origin],
    ...
    const publicbodies = parseproposal(
      {
        version,
        plan: {
          objective: "Capture",
          origin,
          steps: [stepinput("capturebodies", { body: { mimes: ["image/png"], ceiling: 100 } })],
        },
      },
      origin,
      [origin],
    ...
    const readreplay = parseproposal(
      {
        version,
        plan: {
          objective: "Replay",
          origin,
          steps: [stepinput("extractapi", { replay: { endpoint: "https://api.example/items", verb: "GET" } })],
        },
      },
      origin,
      [origin, "https://api.example"],
    ...
    const mutreplay = parseproposal(
      {
        version,
        plan: {
          objective: "Replay",
          origin,
          steps: [stepinput("extractapi", { replay: { endpoint: "https://api.example/items", verb: "POST" } })],
        },
      },
      origin,
      [origin, "https://api.example"],
    ...
    const send = parseproposal(
      {
        version,
        plan: {
          objective: "Send",
          origin,
          steps: [stepinput("sendmessage", { message: { channel: "ch1", payload: "x" } })],
        },
      },
      origin,
      [origin],
...
describe("network control protocol", () => {
      ...
      tokens: [
        {
          ...
          origin: "https://api.example",
          scopes: ["read"],
          accessstorageid: "token-t1-access",
          expiresat: 99,
          at: 1,
        },
      ],
  ...
  it("carries the traffic control state with mock bodies held back", () => {
    const report = controlreport({
      blocks: [
        { id: "b1", runid: "run1", stepid: "s1", urlpattern: "https://ads.example/*", hits: 2, registeredat: 1 },
      ],
      mocks: [
        {
          id: "m1",
      ...
      cookies: [
        { id: "c1", runid: "run1", stepid: "s1", kind: "write", domain: "example.com", names: ["session"], at: 1 },
      ],
      proxies: [
        {
          id: "p1",
  ...
  it("rejects block rules without an origin pattern and proxy routes without a bypass list", () => {
    ...
    ).toThrow("Block rules without a named origin pattern are refused.");
    expect(() =>
      parseproposal(
        {
          version: protocolversion,
    ...
    ).toThrow("Proxy routes without a bypass list are refused.");
  });
});
...
describe("debugging protocol", () => {
  ...
  it("attaches the timeline block to the outcome envelope and carries the timeline and console diff reports", () => {
    const outcome: stepoutcome = {
      stepid: "w1",
      ok: true,
      summary: "Captured 4 console calls.",
      details: { errorids: ["e1"], rejectionids: [] },
      at: 5,
    };
    const envelope = JSON.parse(
      outcomeresponse({
        outcome,
        plan,
        timeline: { entries: 4, levels: { error: 1, warn: 1, info: 2, log: 0, debug: 0, trace: 0 }, collapsed: 2 },
      }),
    ) as { version: string; timeline: { entries: number; levels: Record<string, number>; collapsed: number } };
    expect(envelope.version).toBe(protocolversion);
    expect(envelope.timeline).toEqual({
      entries: 4,
      levels: { error: 1, warn: 1, info: 2, log: 0, debug: 0, trace: 0 },
      collapsed: 2,
    });
    ...
    const timeline = timelinereport({
      entries: [{ id: "t1", runid: "plan", stepid: "w1", time: 3, level: "error", source: "error", message: "boom" }],
      errors: [
        {
          id: "e1",
          runid: "plan",
          stepid: "w1",
          message: "boom",
          frames: [{ url: "https://example.com/app.js", line: 42 }],
      ...
      rejections: [],
      longtasks: [
        { id: "l1", runid: "plan", stepid: "w1", duration: 90, starttime: 10, attributions: ["same-origin"], at: 3 },
      ],
      levelcounts: { error: 1 },
    });
    ...
    const diff = consolediffreport({
      diff: {
        ...
        lines: [
          ...
          { kind: "repeated", text: "boot", count: 1 },
        ],
        added: 1,
        removed: 1,
        repeated: 1,
          ...
          steps: [
            {
              ...
              summary: "Capture console output at every level.",
            },
            {
              kind: "watcherrors",
              options: JSON.stringify({ watch: { window: 250 }, level: "error" }),
              summary: "Capture errors with stacks.",
            },
            {
              kind: "watchtasks",
              options: JSON.stringify({ watch: { window: 250 }, threshold: 50 }),
              summary: "Capture long task timings.",
            },
          ],
        },
      },
      ...
      parseproposal(
        {
          ...
          plan: {
            ...
            steps: [
              {
                kind: "watchconsole",
                options: JSON.stringify({ watch: { window: 250 }, redact: ["s"], level: "verbose" }),
                summary: "Bad level floor.",
              },
            ],
          },
        },
      ...
      parseproposal(
        {
          ...
          plan: {
            ...
            steps: [
              {
                kind: "watchconsole",
                options: JSON.stringify({ watch: { window: 250 } }),
                summary: "Missing redaction.",
              },
            ],
          },
        },
      ...
      parseproposal(
        {
          ...
          plan: {
            ...
            steps: [
              {
                kind: "watchconsole",
                options: JSON.stringify({ watch: { window: 250 }, redact: ["secret"] }),
...
describe("debugging part two protocol", () => {
  ...
  it("attaches the cdp block to the outcome envelope and carries the cdp report with sources held back", () => {
      ...
      steps: [
        {
          ...
          summary: "Evaluate",
          risk: "sensitive",
          options: JSON.stringify({ command: { method: "Runtime.evaluate" } }),
        },
      ],
      createdat: 1,
      expiresat: 9000,
    ...
    const envelope = JSON.parse(
      outcomeresponse({ outcome, plan, cdp: { sessionid: "cdp1", state: "attached", commandids: ["cmd1", "cmd2"] } }),
    ) as { cdp: { sessionid: string; state: string; commandids: string[] } };
    ...
    const pause: pausestate = {
      ...
      stepid: "b1",
      reason: "breakpoint",
      callframes: [{ url: "https://example.com/app.js", line: 5 }],
      hitbreakpoint: "bp1",
      domsnapshotid: "dom-3",
      at: 4,
    };
    const watch: watchexpression = {
      ...
      scope: "topframe",
      reviewed: true,
      values: [{ pauseid: "p1", value: "3", at: 4 }],
      at: 2,
    };
    ...
    const grant: debuggergrant = {
      id: "g1",
      prompt: "Debugger attach prompt",
      origin: "https://example.com",
      domains: ["Runtime"],
      approved: true,
      consentedat: 1,
    };
    const report = cdpreport({
      sessions: [session],
      commands: [command],
      events: [rule],
  ...
  it("rejects cdp proposals outside the attach contract and accepts the reviewed chain", () => {
    const attach = {
      ...
      summary: "Attach the devtools session.",
    };
    const command = {
      kind: "cdpcmd",
      options: JSON.stringify({ command: { method: "Runtime.evaluate" } }),
    ...
    const accepted = parseproposal(
      { version: protocolversion, plan: { objective: "Debug", steps: [attach, command] } },
      "https://example.com",
    );
      ...
      parseproposal(
        {
          ...
          plan: {
            ...
            steps: [
              {
                kind: "attachcdp",
                options: JSON.stringify({ domains: ["Runtime"] }),
                summary: "Attach without a teardown plan.",
              },
            ],
          },
        },
      ...
      parseproposal(
        {
          ...
          plan: {
            ...
            steps: [
              {
                kind: "attachcdp",
                options: JSON.stringify({ domains: [], teardown: { revertsteps: ["x"] } }),
                summary: "Attach without domains.",
              },
            ],
          },
        },
    ...
    expect(() =>
      parseproposal(
        { version: protocolversion, plan: { objective: "Debug", steps: [command] } },
        "https://example.com",
      ),
    ).toThrow("attachcdp step");
    expect(() =>
      parseproposal(
        {
          version: protocolversion,
          plan: {
            ...
            steps: [
              ...
              {
                kind: "cdpcmd",
                options: JSON.stringify({ command: { method: "DOM.getSnapshot" } }),
                summary: "Command outside the allowlist.",
              },
            ],
          },
        },
      ...
      parseproposal(
        {
          ...
          plan: {
            ...
            steps: [
              {
                kind: "watchcdp",
                options: JSON.stringify({ events: [{ domain: "Log", event: "entryAdded" }], watch: { window: 100 } }),
                summary: "Watch events on an ungranted origin.",
              },
            ],
          },
        },
    ...
    const gated = {
      ...
      summary: "Attach with method gates.",
    };
    expect(
      parseproposal(
        { version: protocolversion, plan: { objective: "Debug", steps: [gated, command] } },
        "https://example.com",
      ).plan.steps,
    ).toHaveLength(2);
    expect(() =>
      parseproposal(
        {
          ...
          plan: {
            ...
            steps: [
              ...
              {
                kind: "cdpcmd",
                options: JSON.stringify({ command: { method: "Runtime.callFunctionOn" } }),
                summary: "Gated out.",
              },
            ],
          },
...
describe("debugging part three protocol", () => {
  it("attaches the profile block to the outcome envelope and carries the profile report with consent prompts held back", () => {
    ...
    const envelope = JSON.parse(outcomeresponse({ outcome, plan, profile: { metrics: 3, samples: 5 } })) as {
    ...
    const trace: tracerecord = {
      ...
      bytesize: 400,
      events: 2,
      annotations: [{ stepid: "s1", label: "open", offset: 100 }],
      startedat: 1,
      endedat: 4,
    };
    const map: sourcemapref = {
      id: "sm1",
      runid: "plan",
      stepid: "sm",
      origin: "https://example.com",
    ...
    const consent: sourcemapconsent = {
      id: "c1",
      prompt: "Source map capture prompt",
      origin: "https://example.com",
      approved: true,
      consentedat: 1,
    };
  ...
  it("rejects profiling proposals outside the grants, the reviewed categories and the annotation contract", () => {
    const flow = {
      ...
      summary: "Measure the flow.",
    };
    expect(
      parseproposal({ version: protocolversion, plan: { objective: "Profile", steps: [flow] } }, "https://example.com")
        .plan.steps[0]?.risk,
    ).toBe("read");
    expect(() =>
      parseproposal(
        { version: protocolversion, plan: { objective: "Profile", steps: [flow] } },
        "https://example.com",
        ["https://other.example"],
      ),
    ).toThrow("outside the grants");
      ...
      parseproposal(
        {
          ...
          plan: {
            ...
            steps: [
              {
                kind: "traceload",
                options: JSON.stringify({ trace: { categories: ["secret"], window: 100 } }),
                summary: "Trace with an unreviewed category.",
              },
            ],
          },
        },
      ...
      parseproposal(
        {
          ...
          plan: {
            ...
            steps: [
              {
                kind: "heapshot",
                options: JSON.stringify({ target: { kind: "iframe", url: "https://elsewhere.example/frame" } }),
                summary: "Snapshot an ungranted frame.",
              },
            ],
          },
        },
    ...
    ).toThrow("outside the granted origins");
    const framed = {
      kind: "heapshot",
      options: JSON.stringify({ target: { kind: "worker", url: "https://cdn.example/worker.js" } }),
      summary: "Snapshot a granted worker.",
    };
    expect(
      parseproposal(
        { version: protocolversion, plan: { objective: "Profile", steps: [framed] } },
        "https://example.com",
        ["https://example.com", "https://cdn.example"],
      ).plan.steps,
    ).toHaveLength(1);
    expect(() =>
      parseproposal(
            ...
            steps: [
              {
                ...
                options: JSON.stringify({ scripts: ["https://elsewhere.example/app.js"] }),
                summary: "Capture maps of an ungranted origin.",
              },
            ],
          },
        },
      ...
      parseproposal(
        {
          ...
          plan: {
            ...
            steps: [
              {
                kind: "annotatetrace",
                options: JSON.stringify({ trace: { traceid: "t1" } }),
                summary: "Annotate without annotations.",
              },
            ],
          },
        },
    ...
    ).toThrow("step annotations");
    const annotated = {
      kind: "annotatetrace",
      options: JSON.stringify({ trace: { traceid: "t1" }, annotations: [{ stepid: "s1", label: "open" }] }),
      summary: "Annotate the trace.",
    };
    expect(
      parseproposal(
        { version: protocolversion, plan: { objective: "Profile", steps: [annotated] } },
        "https://example.com",
      ).plan.steps,
    ).toHaveLength(1);
  });
...
describe("emulation protocol", () => {
  ...
  it("carries the applied and reverted layer names in the emulation block of the response envelope", () => {
    const envelope = JSON.parse(
      outcomeresponse({ outcome, plan, emulation: { applied: ["phone"], reverted: ["slow3g"] } }),
    ) as { emulation?: { applied: string[]; reverted: string[] } };
  ...
  it("builds the emulation report envelope with the layer history, preset libraries, blackbox rules and permission overrides while the consent prompt text stays out", () => {
    const report = emulationreport({
      ...
      agents: [agent],
      blackbox: [
        { origin: "https://example.com", rules: [{ urlpatterns: ["https://cdn.example/**"], tracescope: "both" }] },
      ],
      permissions: [
        {
          id: "p1",
  ...
  it("rejects emulation steps without a revert plan, location values out of range and unknown permission names", () => {
      ...
      parseproposal(
        {
          ...
          plan: {
            ...
            steps: [
              {
                kind: "emulatedevice",
                options: JSON.stringify({ reviewed: true, device }),
                summary: "Mask without a revert plan.",
              },
            ],
          },
        },
      ...
      parseproposal(
        {
          ...
          plan: {
            ...
            steps: [
              {
                kind: "emulatelocate",
                options: JSON.stringify({ reviewed: true, location: { ...location, latitude: 91 }, revertplan }),
                summary: "Mask a coordinate out of range.",
              },
            ],
          },
        },
    ...
    ).toThrow("latitude and longitude ranges");
    expect(() =>
      parseproposal(
        {
          version: protocolversion,
          plan: {
            ...
            steps: [
              {
                kind: "overridepermission",
                options: JSON.stringify({
                  reviewed: true,
                  permission: { name: "screen-capture", state: "granted" },
                  revertplan,
                }),
                summary: "Override an unknown permission.",
              },
            ],
          },
        },
    ...
    ).toThrow("unknown permission names");
    expect(() =>
      parseproposal(
        {
          version: protocolversion,
          plan: {
            ...
            steps: [
              {
                kind: "setuseragent",
                options: JSON.stringify({ reviewed: true, agent, revertplan }),
                summary: "Mask the agent.",
              },
            ],
          },
        },
    ...
    const proposal = parseproposal(
      {
        ...
        plan: {
          ...
          steps: [
            {
              kind: "emulatedevice",
              options: JSON.stringify({ reviewed: true, device, revertplan, reload: true }),
              summary: "Wear the reviewed phone mask.",
            },
            {
              kind: "blackboxscripts",
              options: JSON.stringify({
                reviewed: true,
                rules: [{ urlpatterns: ["https://cdn.example/**"], tracescope: "both" }],
                revertplan,
              }),
              summary: "Blackbox the vendor scripts.",
            },
          ],
        },
      },
...
describe("workflow editor protocol", () => {
  it("builds the editor state envelope for panel synchronization", () => {
    const envelope = editorstate({
      ...
      breakpoints: ["s2"],
      overrides: [
        { id: "o1", workflowid: "wf1", pattern: "https://example.com", deltas: { waitms: 500 }, createdat: 8 },
      ],
      imports: [
        { id: "imp1", workflowid: "wf1", name: "imported", version: 1, steps: 2, risk: "sensitive", importedat: 9 },
      ],
      backgroundruns: { wf1: true },
      watchdog: {
        config: { enabled: true, stallthreshold: 30_000, action: "pause" },
        events: [
          { id: "w1", runid: "r1", verdict: "stalled", action: "pause", outcome: "Paused at the checkpoint.", at: 10 },
        ],
      },
    });
    expect(envelope.version).toBe(protocolversion);
  ...
  it("parses run history queries with filters and echoes them in the report", () => {
    ...
    expect(runhistoryquery({ workflowid: "wf1", outcome: "done", since: 100, limit: 25 })).toEqual({
      workflowid: "wf1",
      outcome: "done",
      since: 100,
      limit: 25,
...
describe("agent protocol protocol", () => {
  it("builds toolcall and toolresult frames with the json rpc grammar", () => {
    ...
    const refusal = toolresultframe({
      id: null,
      error: { code: "consentrefused", message: "The consent gates refused the tool call." },
    });
  ...
  it("carries tool errors with the json rpc error codes in the response envelope", () => {
    ...
    ) as { tool?: { clientid: string; tool: string; ok: boolean; code?: string } };
    expect(refused.tool).toEqual({
      clientid: "client1",
      tool: "browser.click",
      origin: "https://example.com",
      ok: false,
    ...
    const ran = JSON.parse(
      outcomeresponse({
        ...
        plan,
        tool: { clientid: "client1", tool: "browser.readtext", origin: "https://example.com", ok: true },
      }),
    ) as { tool?: { ok: boolean; code?: string } };
...
describe("agent protocol part two envelopes", () => {
  ...
  it("builds the http stream transport report with its event channels", () => {
    const stream = {
      ...
      tls: { mode: "on" as const },
      heartbeatms: 15_000,
      idlewindowms: 45_000,
    };
    const report = httpstreamreport({
      stream,
      channels: [
        { id: "c1", clientid: "client1", openedat: now - 2000, lastbeatat: now - 1000 },
        { id: "c2", clientid: "client2", openedat: now - 2000, lastbeatat: now - 50_000 },
      ],
      now,
    });
    ...
    expect(
      httpstreamreport({
        stream: { endpoint: "/mcp", streampath: "/mcp/stream", tls: { mode: "off" } },
        channels: [],
        now,
      }).heartbeatms,
    ).toBe(30_000);
  ...
  it("builds the approval gate request and response frames with redacted arguments", () => {
    ...
    const approved = approvalframes({
      id: 9,
      request: { ...gate, state: "approved" },
  ...
  it("builds the tls, allowlist and heartbeat reports", () => {
    expect(tlsreport({ mode: "required", certificatefingerprint: "sha256:aa", verifiedat: now })).toEqual({
      version: protocolversion,
      mode: "required",
      certificaterequired: true,
      verified: true,
    });
    expect(tlsreport({ mode: "off" })).toEqual({
      version: protocolversion,
      mode: "off",
    ...
    const entries = [
      {
        ...
        namespaces: ["browser" as const, "memory" as const],
        grantedat: now,
        history: [
          { at: now, actor: "user", change: "Granted." },
          { at: now + 1, actor: "user", change: "Rescoped." },
        ],
      },
    ];
    ...
    const beats = heartbeatreport({
      channels: [
        { id: "c1", clientid: "client1", openedat: now, lastbeatat: now },
        { id: "c2", clientid: "client2", openedat: now - 1000, lastbeatat: now - 500 },
      ],
      now,
      idlewindow: 1000,
    });
...
describe("agent protocol part three envelopes", () => {
  ...
  it("builds the event subscription frames and the event notification frames", () => {
    const subscription = {
      id: "sub1",
      clientid: "client1",
      kinds: ["callresult" as const, "progress" as const],
      origin: "https://example.com",
      createdat: now,
    };
  ...
  it("builds the page state delta reports of the resource watchers", () => {
    const report = resourcedeltareport({
      watchid: "watch1",
      clientid: "client1",
      resource: "page",
      delta: { url: "https://example.com/next" },
      now,
    });
  ...
  it("builds the sampling callback frames with the exact prompt payload", () => {
    ...
    const answered = samplingframes({
      id: 5,
      request: { ...pending, state: "answered", answeredat: now + 1, answer: "the model answer" },
    });
      ...
      prompts: [
        {
          ...
          arguments: [{ name: "objective", description: "The plan objective.", required: true }],
          template: "Review {{objective}}.",
        },
      ],
    });
  ...
  it("builds the stream chunk and progress notice frames", () => {
    ...
    const notice = progressnoticeframe({
      callid: "call1",
      percent: 60,
      message: "Typing the reviewed field.",
      cancellable: true,
      at: now,
    });
  ...
  it("builds the cancellation frames with the preserved partial result", () => {
    const frames = cancelframes({
      id: 8,
      frame: { callid: "call1", reason: "the user asked", at: now },
      partial: { content: "the first chunks", iserror: false },
    });
    expect(frames.cancel.method).toBe("calls/cancel");
    expect(frames.cancel.params?.reason).toBe("the user asked");
    expect(frames.cancelled.result).toMatchObject({
      cancelled: true,
      callid: "call1",
      partial: { content: "the first chunks" },
    });
  });

  it("builds the structured error, rate limit, idempotency, batch, call log, in flight, dry run and mock reports", () => {
    const error = structurederrorreport({
      error: { code: "ratelimited", message: "The budget is spent.", retryhint: "wait", retryafter: 4000 },
      usage: { clientid: "client1", used: 5, budget: 5 },
    });
    ...
    const limits = ratelimitreport({
      limits: [
        { clientid: "client1", windowms: 60_000, budget: 5, windowstartedat: now, used: 3 },
        { clientid: "client2", windowms: 30_000, windowstartedat: now, used: 0 },
      ],
      now,
    });
    expect(limits.limits[0]).toMatchObject({ used: 3, unbounded: false });
    expect(limits.limits[1]?.unbounded).toBe(true);
    const replay = idempotencyreplayframe({
      id: 11,
      key: "key-1",
    ...
    const idempotency = idempotencyreport(
      [
        {
          key: "key-1",
          clientid: "client1",
    ...
    const batch = batchreport({
      batch: {
        ...
        clientid: "client1",
        calls: [
          { id: "m1", name: "browser.readtext", params: {} },
          { id: "m2", name: "memory.list", params: {} },
        ],
        stoponerror: true,
        state: "stopped",
        createdat: now,
        finishedat: now + 100,
        outcomes: [
          { callid: "m1", tool: "browser.readtext", ok: true, at: now + 1 },
          { callid: "m2", tool: "memory.list", ok: false, at: now + 2 },
        ],
      },
    });
    expect(batch).toMatchObject({ batchid: "batch1", state: "stopped", done: 2, total: 2 });
    const log = calllogreport({
      calls: [
        {
          id: "call1",
          clientid: "client1",
    ...
    const dry = dryrunreport({
      callid: "call1",
      tool: "browser.click",
      argsvalid: false,
      consentok: true,
      findings: ["The required argument stepid stays empty."],
      executed: false,
      mutations: [],
      at: now,
    });
    expect(dry.findings[0]).toMatch(/stepid stays empty/);
    const mocks = mockreport([
      { tool: "browser.readtext", result: { content: "canned", iserror: false }, testcontext: true, createdat: now },
    ]);
...
describe("protocol llm envelopes", () => {
  ...
  it("carries a model drafted plan in the model proposal envelope for review", () => {
    const draft: plandraft = {
      ...
      steps: [
        { id: "s1", kind: "navigate", value: "https://docs.example", summary: "Open the docs page." },
        { id: "t1", kind: "reload", summary: "Reload.", freshreview: true },
      ],
      openquestions: ["which section?"],
      providerid: "prov1",
      model: "model-a",
      state: "draft",
      lintfindings: ["The drafted step s9 violates the action grammar."],
      createdat: now,
    };
  ...
  it("reports the usage totals and guard verdicts in the model outcome envelope", () => {
    ...
    const envelope = modeloutcome({
      runid: "run1",
      outputs,
      totals: { prompttokens: 10, completiontokens: 5, totaltokens: 15, cost: 0.02, calls: 2 },
    });
    expect(envelope.version).toBe(protocolversion);
  ...
  it("carries the agents, the task queue and the mailboxes in the swarm state report", () => {
    const state: swarmstate = {
      ...
      mailboxes: [
        {
          agentid: "a1",
          inbox: [{ id: "m1", senderid: "a2", recipient: "a1", routing: "direct", payload: "Ready.", sentat: now }],
          outbox: [],
          unread: 1,
        },
      ],
      ...
      lanes: [
        {
          ...
          milestones: [{ label: "Rows read", done: true, at: now }],
        },
...
describe("execution environment envelopes", () => {
  it("documents the environment grammar of every reviewed action kind", () => {
    ...
    expect(grammar.kinds.find((entry) => entry.kind === "readhtml")?.environments).toEqual([
      "pagecontext",
      "offscreenworker",
    ]);
  ...
  it("wraps the environment view with the step environments, turnarounds, offscreen registry, workers and keepalive", () => {
      ...
      offscreen: [
        {
          ...
          justification: "Heavy parsing of reviewed snapshots.",
          createdat: 1,
        },
      ],
      workers: 3,
    ...
    expect(report.environments).toEqual([
      { stepid: "s1", environment: "pagecontext" },
      { stepid: "s2", environment: "offscreenworker" },
    ]);
  ...
  it("returns the execution environment beside the step outcome in the response envelope", () => {
    ...
    const envelope = JSON.parse(
      outcomeresponse({
        outcome: { stepid: "s1", ok: true, summary: "Done.", environment: "offscreenworker", at: 1 },
        plan,
      }),
    ) as { outcome: { environment?: string } };
...
describe("security envelopes", () => {
      ...
      windows: [
        {
          ...
          duration: 60_000,
          expiresat: 61_000,
          boundary: "60000 milliseconds the user chose",
          kinds: [],
          state: "active",
        },
      ],
  ...
  it("wraps one run log chain verification with the seal hash when the log sealed", () => {
    ...
    const sealed = logchainreport({
      runid: "run2",
      valid: false,
      entries: 3,
      brokenat: 1,
      reason: "The chain reports tamper evidence.",
    });
...
describe("protocol transparency", () => {
  it("serves the transparencypage data in one versioned envelope with the denydefault posture", () => {
    const report = transparencyreport({
      grants: [
        {
          ...
          boundary: "the user revokes the entry",
          grantedat: 1,
        },
      ],
      windows: [
      ...
      permdiffs: [
        { fromversion: "1.1.61", toversion: "1.1.62", added: ["optional:downloads"], removed: [], computedat: 1 },
      ],
  ...
  it("wraps one interface surface snapshot in the versioned envelope", () => {
    const report = surfacesnapshot({
      ...
      palette: [
        {
          ...
          score: 100,
          reason: "The query matches the starttask command exactly.",
        },
      ],
      ...
      logstream: {
        ...
        chainvalid: true,
        reason: "The logstream chain of 1 event verifies link by link.",
      },
      plancards: [
        {
          risk: "sensitive",
  ...
  it("wraps the ecosystem snapshot in the versioned envelope", () => {
    const report = ecosystemviews({
      ...
      syncbridge: {
        ...
        conflicts: [{ id: "conflict-1", manifestid: "invoice", localversion: "1.0.0", remoteversion: "2.0.0" }],
      },
      attention: [
        {
          id: "attention:gatewait:run1:g1",
          cause: "gatewait",
          severity: "critical",
          runid: "run1",
          summary: "A gate waits for one human action.",
          deeplink: "devthink://gate/g1?run=run1",
          at: 1,
        },
      ],
      backgroundruns: [
        {
          ...
          state: "running",
          keepaliveheld: true,
          progress: "Running with the keepalive signal held",
        },
      ],
      replay: {
        runid: "run1",
        ...
        playing: false,
        steps: [
          { stepid: "s1", index: 0, summary: "completed" },
          { stepid: "s2", index: 1, summary: "waited at a gate" },
        ],
        restored: { stepid: "s2", summary: "The gate resolved after a human action.", captureid: "c-99" },
      },
      compare: {
        runids: ["runa", "runb"],
        metrics: ["agreement", "divergence", "durationdelta"],
        firstdivergence: 1,
        steps: [
          {
            stepid: "s2",
...
const freezesession: agentsession = {
  id: "sess",
  tabid: 7,
  origin: "https://example.com",
  startedat: freezenow - 5000,
  expiresat: freezenow + 600_000,
  grants: ["https://example.com"],
};
const freezeplan: agentplan = {
  id: "plan1",
  objective: "Validate the frozen contracts",
  origin: "https://example.com",
  steps: [{ id: "s1", kind: "observe", summary: "Snapshot the tab", risk: "read" }],
  createdat: freezenow - 4000,
  expiresat: freezenow + 600_000,
  state: "approved",
};
const v2client: clientrecord = {
  id: "v2client",
  transport: "stdio",
  paired: true,
  connectedat: freezenow - 1000,
  pairedat: freezenow - 500,
  capabilities: {
    protocolversion: "2.0.0",
    name: "v2client",
    version: "2",
    toolversion: freezecatalog.version,
    tools: 0,
    namespaces: ["browser"],
    transports: ["stdio"],
  },
};
const v1client: clientrecord = {
  ...v2client,
  id: "v1client",
  capabilities: {
    protocolversion: "1.1.54",
    name: "v1client",
    version: "1",
    toolversion: freezecatalog.version,
    tools: 0,
...
describe("the protocolv2 frozen contract of 1.1.91", () => {
  it("validates every frozen message against its schema file", async () => {
    ...
    for (const entry of frozenmessagecatalog) {
      const schema = JSON.parse(await readFile(`dist/${entry.schema}`, "utf8")) as {
        $id?: string;
        version?: string;
        title?: string;
        description?: string;
  ...
  it("wraps concrete messages in the versioned envelope their schema freezes", async () => {
    if (!existsSync("dist/schemas"))
      return; /* the schema artifacts ride the build: the pass runs after pnpm build in the validate chain and the ci lanes, and a lane that skipped the build asserts the frozen catalog above and leaves the envelope walk to the chain that builds first */
    const planschema = JSON.parse(await readFile("dist/schemas/plan.schema.json", "utf8")) as {
      plan?: { properties?: Record<string, { type?: string }> };
    };
    const proposal = parseproposal(
      {
        version: protocolversion,
    ...
    for (const field of ["id", "objective", "origin", "steps", "createdat", "expiresat", "state"])
      expect(planschema.plan?.properties?.[field]).toBeDefined();
    expect(planschema.plan?.properties?.createdat?.type).toBe("number");
    expect(proposal.version).toBe(protocolversion);
    expect(proposal.plan.state).toBe("pending");
    const memoryschema = JSON.parse(await readFile("dist/schemas/memory.schema.json", "utf8")) as {
      provenancefields?: { fields?: string[] };
    };
    ...
    const envelopeschema = JSON.parse(await readFile("dist/schemas/envelope.schema.json", "utf8")) as {
      outcomes?: { values?: string[] };
      errorcodetable?: { items?: Array<{ code?: string; retry?: string }> };
    };
    ...
    const envelope = JSON.parse(
      outcomeresponse({ outcome: { stepid: "step", ok: true, summary: "Done.", at: 5 }, plan: freezeplan }),
    );
    expect(envelope.version).toBe(protocolversion);
    for (const entry of errorcodetable)
      expect(
        envelopeschema.errorcodetable?.items?.some((item) => item.code === entry.code && item.retry === entry.retry),
      ).toBe(true);
  ...
  it("answers the strict unknown field refusal for every client after the sunset", async () => {
    ...
    const strict = await handleframe({
      frame: strictframe as jsonrpcframe,
      client: v2client,
      catalog: freezecatalog,
      config: freezeconfig,
      session: freezesession,
      plan: freezeplan,
      origin: freezesession.origin,
      tabid: freezesession.tabid,
      now: freezenow,
      execute: async (step) => ({ content: `ran ${step.kind}`, iserror: false }),
    });
    ...
    const sunset = await handleframe({
      frame: strictframe as jsonrpcframe,
      client: v1client,
      catalog: freezecatalog,
      config: freezeconfig,
...
describe("the quarantine review flow messages", () => {
  it("wraps the verdict report of a held quarantine entry in the versioned envelope", () => {
    ...
    const report = quarantineverdictreport({
      entry: {
        id: held.entry.id,
        path: held.entry.path,
        reason: held.entry.reason,
        scan: held.entry.scan,
        status: held.entry.status ?? "held",
        at: held.entry.at,
        updatedat: held.entry.updatedat,
      },
    });
    expect(report.version).toBe(protocolversion);
  ...
  it("releases only a clean verdict and never reports a held or flagged file as released", () => {
    ...
    const cleanreport = quarantineverdictreport({
      entry: {
        id: clean.entry.id,
    ...
    expect(() =>
      quarantineverdictreport({
        entry: {
          id: flagged.entry.id,
          path: flagged.entry.path,
          reason: flagged.entry.reason,
          scan: "flagged",
          status: "released",
          at: flagged.entry.at,
          updatedat: flagged.entry.updatedat,
        },
      }),
    ).toThrow("Only a clean scanner verdict releases a quarantined file");
  });

  it("carries the quarantine entries with their digests through the session context report", () => {
    const held = enforcequarantine({
      path: "/sandbox/report.pdf",
      reason: "quarantine step",
      digest: `${"b".repeat(64)}`,
      now: 1000,
    });
...
describe("the purge and export completeness messages", () => {
  it("reports the full inventory scope of a purge with the audit hashes preserved", () => {
    const now = 4000;
    const inventory = ["runs", "memoryitems", "captures", "provlog"].map((key) =>
      inventoryentry({
        key,
        dataclass: key === "memoryitems" ? "memory" : key === "provlog" ? "provenance" : key,
        value: [{}],
        now,
      }),
    );
    const purge = purgeonrequest({
      policy: { scope: ["all"], confirmation: "delete everything", at: now },
...
describe("protocol coordination messages", () => {
  ...
  it("wraps the lessonshare matches with the agent, the finding, the origin and the reuse count", () => {
    const lessons: lessonrecord[] = [
      ...
      },
      {
        id: "l2",
        agentid: "w2",
        finding: "The login form refuses pasted passwords.",
  ...
  it("wraps the shared cost ledger with every entry attributed to its agent and the split", () => {
    ...
    expect(report.entries[0]).toMatchObject({
      ...
      runid: "run-1",
      units: 4,
      description: "The pricing table read.",
    });
  ...
  it("parses the spawn request against the fleet registry and answers the spawn reply with the lineage", () => {
    ...
    const parsed = parsespawnrequest(
      {
        parentid: "a1",
        objective: "Index the footer links.",
        role: "worker",
        depth: 1,
        narrowscope: { origins: ["https://example.com"] },
      },
      { maxdepth: 2 },
      agents,
      spawns,
    ...
    expect(() =>
      parsespawnrequest({ parentid: "a1", objective: "Too deep.", depth: 9 }, { maxdepth: 2 }, agents, spawns),
    ).toThrow(/depth limit/);
    expect(() =>
      parsespawnrequest({ parentid: "ghost", objective: "Unknown.", depth: 1 }, { maxdepth: 2 }, agents, spawns),
