...
// ---------------------------------------------------------------------------
// gateway child server — tsx http.ts with a temp config dir
// ---------------------------------------------------------------------------
...
const httpts = join(import.meta.dirname, "..", "..", "gateway-http.ts");
...
`,
  ...
  gateway = spawn(tsxbin, [httpts], {
    cwd: configdir,
    env: {
      ...process.env,
      PORT: String(serverport),
      GATEWAY_DATABASE_URL: `file:${join(configdir, "test.db")}`,
    },
    detached: true,
