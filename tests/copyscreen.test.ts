...
function installfakechrome(state: fakestate): void {
  const chromeapi = {
    runtime: { id: fakeextensionid, getURL: (path: string) => `chrome-extension://${fakeextensionid}/${path}`, getManifest: () => ({ version: "2.0.0", name: "devthink", permissions: ["storage"] }), onMessage: { addListener: () => undefined }, onStartup: { addListener: () => undefined }, onInstalled: { addListener: () => undefined }, onConnect: { addListener: () => undefined }, sendMessage: async () => ({ ok: true, value: {} }) },
    tabs: { onUpdated: { addListener: () => undefined }, onActivated: { addListener: () => undefined }, onRemoved: { addListener: () => undefined }, captureVisibleTab: async () => "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mP8z8DwHwAFAAH/q842iQAAAABJRU5ErkJggg==" },
  ...
  Object.defineProperty(globalThis, "navigator", { value: { onLine: true, clipboard: { write: async (payload: Array<{ items: Record<string, Blob> }>) => {
    if (state.clipboardrefuses) throw new Error("The clipboard refused the write.");
    for (const item of payload) for (const [kind, blob] of Object.entries(item.items)) state.clipboardwrites.push({ kind, bytes: blob.size });
  }, readText: async () => "" } }, configurable: true });
}

/** The minimal reviewed plan one copyscreen step rides in. */
function planof(): { id: string; objective: string; origin: string; steps: Array<Record<string, unknown>>; createdat: number; expiresat: number; state: string } {
  return { id: "plan-copyscreen", objective: "Copy the visible tab to the clipboard.", origin: "https://example.com", steps: [{ id: "step-copy", kind: "copyscreen", summary: "Copy the visible tab screenshot.", risk: "sensitive" }], createdat: 1_800_000_000_000, expiresat: 1_800_000_900_000, state: "approved" };
}
...
describe("the copyscreen clipboard routing", () => {
  it("routes the captureVisibleTab screenshot through a ClipboardItem onto navigator.clipboard.write and records the clip entry", async () => {
    ...
    const output = await background.executefilesstep({ id: "step-copy", kind: "copyscreen", summary: "Copy the visible tab screenshot.", risk: "sensitive" } as never, undefined, planof() as never, 7, "https://example.com");
