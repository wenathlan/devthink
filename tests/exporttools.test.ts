...
import { chainrows, csvfield, csvof, exportchain, exportdescriptorof, exportextractions, exportnotes, exportrecords, noterow, unmaskedfieldsof } from "../export.js";
...
describe("exporttools descriptors", () => {
  it("validates the format and scope pair of every export", () => {
    ...
    expect(() => exportdescriptorof("csv", "profiles")).toThrow(/runs, extractions, notes, session, audit and extraction/);
  });
...
describe("exporttools records", () => {
  it("honors the mask verdicts in every format: the verdicts mask the fields they hold back", () => {
    ...
    const csv = exportrecords({ descriptor: exportdescriptorof("csv", "extractions"), records: [{ name: "email", password: "[redacted]", author: "user", note: "plain" }], shapes: [...defaultmaskshapes] });
    expect(csv.result.rows).toBe(1);
    expect(csv.content).toContain("[redacted]");
    expect(csv.content).toContain("plain");
    const json = exportrecords({ descriptor: exportdescriptorof("json", "extractions"), records: [{ name: "email", password: "[redacted]", author: "user", note: "plain" }], shapes: [...defaultmaskshapes] });
    expect(JSON.parse(json.content)[0].password).toBe("[redacted]");
    expect(JSON.parse(json.content)[0].author).toBe("[redacted]");
    const log = exportrecords({ descriptor: exportdescriptorof("log", "extractions"), records: [{ name: "email", password: "[redacted]", note: "plain" }], shapes: [...defaultmaskshapes] });
    expect(log.content).toContain("password=[redacted]");
    expect(log.content).toContain("note=plain");
    const usershapes = exportrecords({ descriptor: exportdescriptorof("csv", "extractions"), records: [{ internalcode: "raw" }], shapes: [...defaultmaskshapes, "internalcode"] });
  ...
  it("refuses to export unmasked values in full under any format", () => {
    ...
    const refused = exportrecords({ descriptor: exportdescriptorof("csv", "extractions"), records, shapes: [...defaultmaskshapes] });
  ...
  it("exports notes with the sealed bodies intact and the plain bodies masked", () => {
    const notes = [{
      id: "n1", origin: "https://example.org", title: "Release notes", author: "user", sensitive: true, sealedbody: "sealed:abc", createdat: now, updatedat: now
    }, {
      id: "n2", origin: "https://example.org", title: "Plain note", body: "the changelog", author: "user", sensitive: false, createdat: now, updatedat: now
    }];
    const row = noterow(notes[0] as never);
    expect(row.sealedbody).toBe("sealed:abc");
    expect(row.body).toBeUndefined();
    const exported = exportnotes({ descriptor: exportdescriptorof("json", "notes"), notes, shapes: [...defaultmaskshapes] });
    ...
    const extractions = exportextractions({ descriptor: exportdescriptorof("csv", "extractions"), rows: [{ title: "row", password: "[redacted]" }], shapes: [...defaultmaskshapes] });
    expect(extractions.content).toContain("[redacted]");
    const refusedextraction = exportextractions({ descriptor: exportdescriptorof("csv", "extractions"), rows: [{ title: "row", password: "hunter2" }], shapes: [...defaultmaskshapes] });
...
describe("exporttools chains", () => {
  it("verifies the log chain before any byte leaves the store", async () => {
    ...
    log = await appendlogentry({ log, kind: "flowrun", summary: "The flowrun completed.", origin: "https://example.org", at: now + 10 });
    const rows = chainrows(log.entries);
    expect(rows).toHaveLength(2);
    expect(rows[0]).toMatchObject({ kind: "step", origin: "https://example.org", stepid: "observe" });
    const csv = await exportchain({ descriptor: exportdescriptorof("csv", "runs"), log, shapes: [...defaultmaskshapes] });
    expect(csv.result.rows).toBe(2);
    expect(csv.content.split("\n")[0]).toContain("kind");
    const json = await exportchain({ descriptor: exportdescriptorof("json", "runs"), log, shapes: [...defaultmaskshapes] });
