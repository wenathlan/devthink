...
describe("pipeline streaming to disk with cursor checkpoints", () => {
  it("streams the rows chunk by chunk with only the active chunk riding the write while the cursor checkpoints after every chunk and the namespace stays bound to the run", () => {
    ...
    const result = streamdisk({
      ...
      chunk: 2,
      write: (chunk) => {
        writes.push(chunk.map((entry) => Number(entry.values.price)));
        return chunk.map((entry) => JSON.stringify(entry.values)).length;
      },
      checkpoint: (cursor) => {
    ...
    const resumed = streamdisk({
      pipeline: pipeline(),
      rows,
      cursor: { pipelineid: "p1", offset: 4, chunk: 2, updatedat: now },
      chunk: 2,
      write: (chunk) => {
        continued.push(...chunk.map((entry) => Number(entry.values.price)));
        return 1;
      },
      now,
    });
    ...
    const foreign = streamdisk({
      pipeline: pipeline(),
      rows,
      cursor: { pipelineid: "other", offset: 4, chunk: 2, updatedat: now },
      chunk: 10,
      write: () => 1,
      now,
    });
    expect(foreign.rowswritten).toBe(5);
    expect(() => streamdisk({ pipeline: pipeline(), rows, chunk: 0, write: () => 1, now })).toThrow(
...
describe("pipeline resume from an interrupted cursor", () => {
  it("skips the rows the cursor offset and the persisted keys already cover while the resume refuses a finished pipeline, a foreign cursor and a moved origin", () => {
    ...
    const result = resumeextract({
      pipeline: interrupted,
      rows,
      cursor: { pipelineid: "p1", offset: 2, chunk: 1, updatedat: now },
      persistedkeys: ["r3"],
      now,
    });
    ...
    expect(() =>
      resumeextract({
        pipeline: interrupted,
        rows,
        cursor: { pipelineid: "other", offset: 0, chunk: 0, updatedat: now },
        now,
      }),
    ).toThrow(/another pipeline/);
    expect(resumegate({ pipeline: interrupted, planid: "plan1", origin: "https://example.com" }).allowed).toBe(true);
    expect(resumegate({ pipeline: interrupted, planid: "plan1", origin: "https://outside.example" }).allowed).toBe(
      false,
    );
    expect(
      resumegate({ pipeline: pipeline({ planid: "plan1" }), planid: "plan2", origin: "https://example.com" }).allowed,
    ).toBe(false);
    const state = pipelinestate({
      pipelines: [result.pipeline, pipeline({ id: "p2", state: "paused" }), pipeline({ id: "p3", state: "finished" })],
    });
    expect(state).toEqual({
      running: 1,
      paused: 1,
      finished: 1,
      perrun: [{ runid: "run1", running: 1, paused: 1, finished: 1 }],
    });
    ...
    expect(() => extractbatchof({ id: " ", pipeline: result.pipeline, rows: [], cursor: result.cursor, now })).toThrow(
      /needs its id/,
    );
  });
});
...
describe("pipeline transform rules with raw retention", () => {
  it("reshapes one field at a time through the reviewed operations while the raw value stays beside the transformed one and an unknown operation refuses without dropping a row", () => {
    ...
    const reviewed: transformrule[] = [
      { expression: "", sources: [], target: "name", field: "name", operation: "trim" },
    ];
    expect(transformgate({ rule: rules[0]!, reviewed }).allowed).toBe(true);
    expect(
      transformgate({
        rule: { expression: "", sources: [], target: "name", field: "name", operation: "case" },
        reviewed,
      }).allowed,
    ).toBe(false);
  });
...
describe("pipeline dedupe over the configured key", () => {
  it("normalizes the key values before comparing while the first occurrence stays, the dropped rows report by count and by key, and an unconfigured key refuses", () => {
    ...
    expect(dedupeconfiggate({ key: { columns: ["name"], normalization: "case" }, pipelineid: "p1" }).allowed).toBe(
      true,
    );
...
describe("pipeline source stamps, grid preview and provenance queries", () => {
  it("stamps every row and cell with its source while the grid preview infers the column kinds and sorts without touching the extract and the provlog answers by row key with the secrets redacted", () => {
    ...
    expect(
      rowstampgate({ stored: [{ key: "r1", capturedat: now }], row: { key: "r1", capturedat: now } }).allowed,
    ).toBe(true);
    expect(
      rowstampgate({ stored: [{ key: "r1", capturedat: now }], row: { key: "r1", capturedat: now + 1 } }).allowed,
    ).toBe(false);
    ...
    expect(() =>
      pipelinegridpreview({ rows: stamped.rows, sort: { column: "ghost", direction: "ascending" } }),
    ).toThrow(/knows no ghost/);
    const transformed = transformvalues({
      rows: stamped.rows,
      rules: [{ expression: "", sources: [], target: "name", field: "name", operation: "case" }],
    }).rows;
    ...
    expect(
      pipelinetargetgate({ fields: ["name", "price"], target: ["name", "price", "sku"], pipelineid: "p1" }).allowed,
    ).toBe(true);
    ...
    const streamlog = provlog({
      ...
      operation: "stream",
      rowkeys: ["r1", "r2"],
      summary: "Streamed 2 rows of the pricing pipeline with token=ghp_abcdefghijklmnopqrstuvwx to the sink.",
      now,
    });
    ...
    const transformlog = provlog({
      ...
      operation: "transform",
      rowkeys: ["r1"],
      summary: "Upper cased the name field.",
      now: now + 1000,
    });
    expect(() =>
      provlog({
        entries: transformlog.entries,
        id: "log1",
        pipelineid: "p1",
        runid: "run1",
        operation: "stream",
