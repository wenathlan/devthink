...
function stub(body: string, status = 200, seen: string[] = [], headers: Record<string, string> = {}): fetchtransport {
  return async (url, init): Promise<transportresponse> => {
    seen.push(
      `${init.method} ${url} ${init.headers.authorization ?? init.headers["x-api-key"] ?? ""} ${init.body ?? ""}`,
    );
...
describe("gateway adapter request composition against the recorded fixtures", () => {
  it("shapes every adapter request with its wire path, key placement and message envelope and parses its recorded answer", async () => {
    const messages = [
      ...
      { role: "user" as const, content: "draft one step" },
    ];
    const openai = openaicompatadapter.build({
      config: config(),
    ...
    const anthropic = anthropicgatewayadapter.build({
      config: config({ kind: "anthropicgateway", baseurl: "https://gateway.example" }),
      model: "fixture-claude-shape",
      messages,
      apikey: "secret",
    });
    ...
    const gemini = geminigatewayadapter.build({
      config: config({ kind: "geminigateway", baseurl: "https://gateway.example" }),
      model: "fixture-gemini-shape",
      messages,
      apikey: "secret",
    });
    ...
    const ollama = ollamalocaladapter.build({
      config: config({ kind: "ollamalocal", baseurl: "http://localhost:11434" }),
      model: "fixture-local:7b",
  ...
  it("joins the base url, the per provider path prefix and the wire path of every adapter", () => {
    expect(gatewayurl({ baseurl: "https://gateway.example/v1/", pathprefix: "team" }, "/chat/completions")).toBe(
      "https://gateway.example/v1/team/chat/completions",
    );
    expect(gatewayurl({ baseurl: "https://gateway.example" }, "/v1/messages")).toBe(
      "https://gateway.example/v1/messages",
    );
    expect(gatewayurl({ baseurl: "http://localhost:11434/", pathprefix: "/ns/" }, "/api/chat")).toBe(
      "http://localhost:11434/ns/api/chat",
    );
  });
});
...
describe("gateway streaming deltas, cancel contract and sidepanel rendering", () => {
  ...
  it("streams through the shared contract, fires the onevent seam per token and stops at the cancelled position", async () => {
    ...
    const streamed = await gatewaystream({
      config: config(),
      model: "fixture-model-a",
      ...
      onevent: (token) => {
        ...
        if (events.length === 2) cancel.cancel("The test cancelled mid stream.");
      },
    });
    ...
    cancelled.cancel();
    await expect(
      gatewaycall({ config: config(), model: "m", messages: [], cancel: cancelled, transport, sleep: instant }),
    ).rejects.toMatchObject({ code: "cancelled" });
  });

  it("renders the streamed tokens incrementally through the cursor batches of the sidepanel chat", () => {
    const tokens: gatewaychattoken[] = [
      { seq: 1, text: "hello ", at: now },
      { seq: 2, text: "streamed ", at: now },
    ];
...
describe("gateway capabilityad advertisement and tool schema mapping", () => {
  it("serializes the tool catalog into the provider specific schema of every adapter with the consent metadata and the plan review gate", () => {
    ...
    if (consenttool !== undefined) {
      const declared = capabilityadvertisement({ kind: "openaicompat", tools }).consent.find(
        (entry) => entry.tool === consenttool.name,
      );
      expect(declared?.review).toBe(consenttool.consentmeta?.review);
    }
    const withtools = openaicompatadapter.build({
      config: config(),
      model: "m",
...
describe("gateway baseurlconfig validation and the empty default posture", () => {
  it("validates the scheme, host and path shape before saving and keeps every remote default empty", () => {
    ...
    expect(gatewaybaseurlgate({ kind: "openaicompat", baseurl: "https://gateway.example/v1#frag" }).allowed).toBe(
      false,
    );
    ...
    const filled = validategatewayconfig({
      providerid: "local1",
      kind: "ollamalocal",
      baseurl: "",
      enabled: false,
    ...
    const refused = validategatewayconfig({
      providerid: "r1",
      kind: "openaicompat",
      baseurl: "",
      enabled: false,
...
describe("gateway key vault behind the seam", () => {
  it("stores under the explicit consent, masks every log surface, revokes with one click and refuses the export leak", async () => {
    ...
    const masked = maskrequest({
      url: "https://gateway.example/v1/models?key=secret-key-material",
      method: "POST",
      headers: {
        authorization: "Bearer secret-key-material",
        "x-api-key": "secret-key-material",
    ...
    const second = await storeproviderkey({
      seam: vault,
      config: provider,
      profileid: "p",
      value: "another-key-material",
      consent: true,
      now,
    });
...
describe("gateway modellist discovery, cache window and annotations", () => {
  it("fetches through the adapter, annotates the context and modality hints and serves the cache inside the window", async () => {
    ...
    const cached = await fetchmodellist({
      config: config(),
      transport,
      cachewindow: 60_000,
      cache: { providerid: "prov1", models: fresh.models, fetchedat: now },
      now: () => now + 1000,
    });
    ...
    const stale = await fetchmodellist({
      ...
      transport,
      cachewindow: 60_000,
      cache: { providerid: "prov1", models: [], fetchedat: now },
      now: () => now + 61_000,
    });
    ...
    const gemini = await fetchmodellist({
      config: config({ kind: "geminigateway", baseurl: "https://gateway.example" }),
      transport: stub(await fixture("gemini-models.json")),
      now: () => now,
    });
    expect(gemini.models[0]).toMatchObject({
      id: "fixture-gemini-shape",
      label: "Fixture Gemini Shape",
      contextwindow: 32768,
    });
    const anthropic = await fetchmodellist({
      config: config({ kind: "anthropicgateway", baseurl: "https://gateway.example" }),
      transport: stub(await fixture("anthropic-models.json")),
      now: () => now,
    });
    expect(anthropic.models[1]).toMatchObject({ id: "fixture-claude-wide", contextwindow: 200000 });
    const ollama = await fetchmodellist({
      config: config({ kind: "ollamalocal", baseurl: "http://localhost:11434" }),
      transport: stub(await fixture("ollama-tags.json")),
      now: () => now,
    });
...
describe("gateway routing with the local fallback", () => {
  it("maps the task kind through the user rules and falls back to the local provider when the remote gates refuse", () => {
    ...
    const routes: modelroute[] = [
      { id: "r1", kind: "chat", providerid: "prov1", model: "model-a", revision: 1, updatedat: now },
    ];
    ...
    const gated = resolvegatewayroute({
      kind: "chat",
      routes,
      providers,
      configs: [config({ providerid: "prov1", enabled: false, consented: false })],
      local,
    });
    expect(gated.source).toBe("local");
    expect(gated.local?.model).toBe("fixture-local:7b");
    const notallowed = resolvegatewayroute({
      kind: "chat",
      routes,
      providers,
      configs: [config()],
    ...
    const nolocal = resolvegatewayroute({
      kind: "chat",
      routes,
      providers,
      configs: [config({ providerid: "prov1", enabled: false, consented: false })],
    ...
    const fallbackpair: modelroute[] = [
      {
        id: "r2",
        kind: "chat",
        providerid: "prov1",
        model: "model-a",
        fallbackproviderid: "prov2",
        fallbackmodel: "model-b",
        revision: 1,
        updatedat: now,
      },
    ];
    const providers2: providerconfig[] = [
      ...providers,
      {
        id: "prov2",
        name: "The fallback gateway",
        endpoint: "https://fallback.example/v1",
        style: "chatcompletions" as const,
        models: ["model-b"],
        status: "available",
        createdat: now,
      },
    ];
    const fell = resolvegatewayroute({
      kind: "chat",
      routes: fallbackpair,
...
describe("gateway token budget accounting and threshold warnings", () => {
  it("records the usage with the request id and session, reports the per session and per run totals and warns at the configured ratio", () => {
    ...
    const localrecord = gatewayusagerecord({
      providerid: "local",
    ...
    const costwarned = gatewaybudgetwarning({
      budget: { maxcost: 2, warnratio: 0.5, configuredat: now },
      totals: { totaltokens: 0, cost: 1.2 },
    });
    expect(costwarned.warned).toBe(true);
    const halted = gatewaybudgetcheck({
      budget: { maxtokens: 100, configuredat: now },
      totals: { totaltokens: 200, cost: 0 },
    });
    expect(halted.allowed).toBe(false);
    expect(halted.asksuser).toBe(true);
    const estimates = costestimates({
      configs: [config({ costpermilliontokens: 2, currency: "usd" })],
      providers,
      records: [record],
    });
...
describe("gateway consent gate blocks the provider calls before approval", () => {
  it("refuses the disabled, unconsented and unconfigured providers while the local runtime needs no remote consent", async () => {
    ...
    ).rejects.toMatchObject({ code: "noconsent" });
    await expect(
      gatewaycall({
        config: config({ consented: false }),
        model: "m",
        messages: [],
        transport: stub(body),
        sleep: instant,
    ...
    await expect(
      gatewaycall({ config: config({ baseurl: "" }), model: "m", messages: [], transport: stub(body), sleep: instant }),
    ).rejects.toMatchObject({ code: "notconfigured" });
    const localanswer = await gatewaycall({
      config: config({ kind: "ollamalocal", baseurl: "http://localhost:11434" }),
      model: "fixture-local:7b",
      messages: [{ role: "user", content: "hi" }],
      transport: stub(await fixture("ollama-completion.json")),
      sleep: instant,
    });
    expect(localanswer.text).toContain("run locally");
    expect(
      gatewayconsentgate({ config: { kind: "ollamalocal", baseurl: "http://localhost:11434", enabled: true } }).allowed,
    ).toBe(true);
    expect(
      gatewayconsentgate({
        config: { kind: "openaicompat", baseurl: "https://gateway.example", enabled: true, consented: true },
      }).allowed,
    ).toBe(true);
  });
});
...
describe("gateway provenance, structured errors and audit reports", () => {
  it("correlates every call through its request id and reports the provenance envelope", () => {
    const report = gatewaycallreport({
      requestid: "req1",
      providerid: "prov1",
      kind: "openaicompat",
      model: "model-a",
...
describe("gateway timeout, transient retries and rate limit honoring", () => {
  it("races the timeout, retries the transient failures with the jittered backoff and honors the retry-after header of the 429 answer", async () => {
    ...
    let calls = 0;
    const flaky = async (): Promise<transportresponse> => {
      calls += 1;
      if (calls === 1) return { status: 503, headers: {}, body: '{"error":"transient"}' };
      return { status: 200, headers: {}, body };
    };
    const retried = await gatewaycall({
      config: config(),
      model: "m",
      messages: [],
      options: { retries: 2, backoff: 10, jitter: 5 },
      transport: flaky,
      sleep,
      jitter: () => 1,
      now: () => now,
    ...
    let limited = 0;
    const ratelimited = async (): Promise<transportresponse> => {
      limited += 1;
      if (limited === 1) return { status: 429, headers: { "retry-after": "1" }, body: '{"error":"rate limited"}' };
      return { status: 200, headers: {}, body };
    };
    const honored = await gatewaycall({
      config: config(),
      model: "m",
      messages: [],
      options: { retries: 1 },
    ...
    let exhausted = 0;
    const alwaysexhausted = async (): Promise<transportresponse> => {
      exhausted += 1;
      return { status: 429, headers: { "retry-after": "1" }, body: '{"error":"rate limited"}' };
    };
    await expect(
      gatewaycall({ config: config(), model: "m", messages: [], transport: alwaysexhausted, sleep, now: () => now }),
    ).rejects.toMatchObject({ code: "ratelimited", retryafter: 1000 });
    await expect(
      gatewaycall({ config: config(), model: "m", messages: [], transport: stub("no", 401), sleep, now: () => now }),
    ...
    await expect(
      gatewaycall({
        ...
        model: "m",
        messages: [],
        transport: stub("<html>broken</html>"),
        sleep,
        now: () => now,
      }),
...
describe("gateway fixture server runs the full request cycle offline", () => {
  it("serves the recorded shapes over localhost http for the call, the stream and the model list", async () => {
    ...
    try {
      ...
      const outcome = await gatewaycall({
        config: config({ baseurl: `http://127.0.0.1:${server.port}/openaicompat` }),
        model: "fixture-model-a",
      ...
      const streamed = await gatewaystream({
        config: config({ kind: "ollamalocal", baseurl: `http://127.0.0.1:${server.port}/ollama` }),
        model: "fixture-local:7b",
        messages: [{ role: "user", content: "stream" }],
        transport,
        sleep: instant,
      ...
      const models = await fetchmodellist({
        config: config({ kind: "geminigateway", baseurl: `http://127.0.0.1:${server.port}/gemini` }),
        transport,
        now: () => now,
      });
...
describe("gateway prompt template library of the per task templates", () => {
  it("ships the per task templates with their variable slots and composes through the shared prompt library", () => {
    ...
    const composed = composegatewayprompt({
      ...
      variables: { goal: "check the docs", tools: "none" },
    });
...
describe("hardcoded provider url scan of the source tree", () => {
  it("asserts no provider endpoint literal and no default cloud url in the shipped sources", async () => {
    ...
    for (const entry of await readdir(join(root, "tests")))
      if (entry.endsWith(".mjs")) files.push(join(root, "tests", entry));
    ...
    for (const path of files) {
      ...
      for (const host of providerhosts)
        expect(host.test(content), `${path} carries the provider endpoint literal ${String(host)}`).toBe(false);
    }
      ...
      for (const url of urls)
        expect(
          url.includes("${") ||
            ...
            /^https:\/\/(example\.(org|com|net|invalid|test))\b/.test(url) ||
            /^https:\/\/(\*\.)?origin\b/.test(url) ||
            deeplinkapps.some((app) => url.startsWith(app)),
          `${path} carries the non example url literal ${url}`,
        ).toBe(true);
    }
    const gateway = await readFile(join(root, "llm.ts"), "utf8");
...
describe("api key literal scan of the built bundles", () => {
  it("asserts no api key literal shape in any built dist bundle", async () => {
    ...
    if (!distexists)
      return; /* the validate chain runs the tests before the build; the build gate scans every bundle itself and the next pass asserts the fresh dist */
    ...
    for (const entry of await readdir(join(root, "dist"))) {
      ...
      for (const shape of keyshapes)
        expect(shape.test(content), `dist/${entry} carries an api key literal shape`).toBe(false);
    }
  });
});
