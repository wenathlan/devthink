...
function runof(state: runrecord["state"]): runrecord {
  return { runid: "run1", planid: "plan", sessionid: "session", state, createdat: now, updatedat: now };
}
...
describe("rollback", () => {
  it("builds one compensating step for each mutating kind and none for read only steps", () => {
    ...
    expect(sideeffects(steps, ["s1", "s2", "s3", "s4", "s5", "s6", "s7"]).map((step) => step.id)).toEqual([
      "s1",
      "s3",
      "s4",
      "s5",
    ...
    const items = rollbackrun({
      runid: "run1",
      steps,
      executedstepids: ["s1", "s2", "s3", "s4", "s5", "s6", "s7"],
      origin: "https://example.com",
      now,
    });
    ...
    const failed = runfail({
      run: runof("running"),
      progress: { planid: "plan", completedsteps: ["s1"], updatedat: now },
      now,
    });
    ...
    const completed = runcomplete({
      run: runof("running"),
      plan,
      progress: { planid: "plan", completedsteps: ["s1", "s2"], updatedat: now },
      now,
    });
  ...
  it("pairs one cancellation with its optional rollback behind the explicit user choice", () => {
    ...
    expect(
      rollbackorigingate({
        items: [{ ...rolled.compensations[0]!, origin: "https://evil.example" }],
        origin: "https://example.com",
      }).allowed,
    ).toBe(false);
...
describe("zombies and heartbeats", () => {
  it("reaps the running runs whose heartbeat fell stale", () => {
    ...
    const zombies = zombiecheck({
      runs: [runof("running"), { ...runof("running"), runid: "run2" }],
      heartbeats: { run1: stale, run2: live },
      now,
      window: 60_000,
    });
...
describe("offlinequeue", () => {
  it("enqueues, dequeues, replays and expires the approved plans in sequence order", () => {
    ...
    expect(() => enqueueoffline([], { plan: { ...plan, state: "pending" }, sessionid: "session", now })).toThrow(
      /approved/i,
    );
    const queue = enqueueoffline([], { plan, sessionid: "session", now });
    const second = enqueueoffline(queue, {
      plan: { ...plan, id: "plan2", steps: [{ id: "s9", kind: "observe", summary: "Read", risk: "read" }] },
      sessionid: "session",
      now,
    });
    ...
    expect(
      queuedtaskexpirygate({
        task: { planid: expiredtask.planid, expiresat: expiredtask.expiresat },
        now: expiredtask.expiresat + 1,
      }).allowed,
    ).toBe(false);
...
describe("idempotent replays", () => {
  ...
  it("refuses the steps of queued, cancelled and rolledback runs and stale sensitive heartbeats inside canexecute", () => {
    ...
    expect(
      canexecute({ ...base, run: runof("running"), heartbeat: { runid: "run1", beat: 2, at: now - 120_000 } }).allowed,
    ).toBe(false);
    expect(
      canexecute({
        ...base,
        run: runof("running"),
        heartbeat: { runid: "run1", beat: 2, at: now - 120_000 },
        settings: { heartbeatwindow: 300_000 },
      }).allowed,
    ).toBe(true);
  });
});
...
describe("resilience memory", () => {
  it("persists runs, checkpoints, heartbeats, the queue, the rollbacks, the executed keys and the user choices", async () => {
    ...
    await store.setcheckpoint(
      makecheckpoint({ runid: "run1", stepid: "s1", completed: ["s1"], digest: "digest1", now }),
    );
    ...
    await store.addrollback({
      runid: "run1",
      stepid: "s1",
      kind: "click",
      origin: "https://example.com",
      compensation: "report the outcome",
      at: now,
    });
  ...
  it("summarizes one run with its state, step counts and checkpoint position", () => {
    const summary = runsummaryof({
      ...
      plan,
      progress: { planid: "plan", completedsteps: ["s1"], updatedat: now },
      checkpoint: makecheckpoint({ runid: "run1", stepid: "s1", completed: ["s1"], digest: "digest1", now }),
    });
