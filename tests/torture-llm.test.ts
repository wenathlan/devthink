...
describe("torture: llm request shaping across the four protocol styles", () => {
  ...
  it("shapes the responses style with the system instruction and the user turns", () => {
    const shaped = buildrequest({
      provider: provider({ style: "responses", endpoint: "https://api.example.com/v1/responses" }),
      model: "gpt-resp",
      messages: [
  ...
  it("shapes the messages style with the x-api-key header and the system field", () => {
    const shaped = buildrequest({
      provider: provider({ style: "messages", endpoint: "https://api.example.com/v1/messages" }),
      model: "claude-test",
      messages: [
  ...
  it("shapes the gemini style with the key on the query string and the parts array", () => {
    const shaped = buildrequest({
      provider: provider({ style: "gemini", endpoint: "https://api.example.com/v1/gemini" }),
      model: "gem-test",
      messages: [
  ...
  it("appends the key on a gemini endpoint that already carries a query string", () => {
    const shaped = buildrequest({
      provider: provider({ style: "gemini", endpoint: "https://api.example.com/v1/gemini?alt=sse" }),
      model: "gem",
      messages: messages(),
      apikey: "k",
    });
  ...
  it("preserves the user headers over the shape headers of every style", () => {
    const withauth = buildrequest({
      provider: provider({ headers: { authorization: "Token custom", "x-trace": "abc" } }),
      model: "m",
      messages: messages(),
      apikey: "ignored",
    });
...
describe("torture: llm completion parsing across the four protocol styles", () => {
  ...
  it("refuses the chat completions answer without a message content", () => {
    ...
    expect(parsecompletion("chatcompletions", JSON.stringify({ choices: [{ message: {} }] })).reason).toMatch(
      /message content/i,
    );
    expect(
      parsecompletion("chatcompletions", JSON.stringify({ choices: [{ message: { content: 5 } }] })).reason,
    ).toMatch(/message content/i);
  });
  ...
  it("reads the messages style answer from the content text blocks and joins them", () => {
    ...
    expect(
      parsecompletion("messages", JSON.stringify({ content: [{ type: "image", text: "ignored" }] })).reason,
    ).toMatch(/text block/i);
  });
  ...
  it("carries no usage block when the answer ships none", () => {
    const parsed = parsecompletion(
      "chatcompletions",
      JSON.stringify({ choices: [{ message: { content: "no usage" } }] }),
    );
...
describe("torture: llm guardrail stripping and parse output", () => {
  ...
  it("reads the valid parsed object when the schema matches", () => {
    const guard: parseguard = {
      schema: { intent: { type: "string", required: true }, entities: { type: "array", required: false } },
      retries: 1,
    };
  ...
  it("refuses the invalid type and the missing required field", () => {
    const guard: parseguard = {
      schema: { intent: { type: "string", required: true }, confidence: { type: "number", required: true } },
      retries: 1,
    };
...
describe("torture: llm tool briefs and usage accounting", () => {
  ...
  it("renders the tool briefs in openapi style with the consent notice", () => {
    ...
    const rendered = rendertemplate({
      template: { id: "t1", name: "n", body: "tools list", variables: [], version: 1, createdat: now },
      variables: {},
    });
  ...
  it("records the usage and aggregates per run, per step and per session filter", () => {
    const usageof = (id: string, over: Partial<usagerecord>): usagerecord => ({
      id,
      providerid: "p1",
...
describe("torture: llm cost budget check", () => {
  it("allows every call when the budget is undefined or unbounded", () => {
    expect(budgetcheck({ budget: undefined, totals: { totaltokens: 1000, cost: 1 } })).toMatchObject({
      allowed: true,
      halted: false,
      asksuser: false,
    });
    expect(
      budgetcheck({ budget: { currency: "usd", configuredat: now }, totals: { totaltokens: 1000, cost: 1 } }),
    ).toMatchObject({ allowed: true, halted: false });
  });

  it("halts at the exact token ceiling and the exact cost ceiling", () => {
    const tokenceil = budgetcheck({
      budget: { maxtokens: 1000, currency: "usd", configuredat: now },
      totals: { totaltokens: 1000, cost: 0 },
    });
    ...
    const costceil = budgetcheck({
      budget: { maxcost: 5, currency: "usd", configuredat: now },
      totals: { totaltokens: 0, cost: 5 },
    });
  ...
  it("ignores non finite ceilings", () => {
    const nan = budgetcheck({
      budget: { maxtokens: Number.NaN, maxcost: Number.NaN, currency: "usd", configuredat: now },
      totals: { totaltokens: 1000, cost: 1 },
    });
...
describe("torture: llm model route resolution and fallback", () => {
  ...
  it("validates the route entries and refuses the half defined fallback pair", () => {
    ...
    expect(routevalid({ id: "x", kind: "", providerid: "p1", model: "m", revision: 1, updatedat: now }).allowed).toBe(
      false,
    );
    expect(routevalid({ id: "x", kind: "k", providerid: "", model: "m", revision: 1, updatedat: now }).allowed).toBe(
      false,
    );
    expect(routevalid({ id: "x", kind: "k", providerid: "p", model: "", revision: 1, updatedat: now }).allowed).toBe(
      false,
    );
    expect(
      routevalid({
  ...
  it("resolves the newest available route of one kind", () => {
    const resolved = resolveroute({
      routes,
      providers: [
        provider({ id: "p1", status: "available", models: ["gpt-test"] }),
        provider({ id: "p2", status: "available", models: ["claude-test"] }),
      ],
      kind: "plan",
    });
  ...
  it("reports the missing provider and the unavailable provider honestly", () => {
    expect(
      resolveroute({
        routes: [{ id: "r", kind: "plan", providerid: "ghost", model: "m", revision: 1, updatedat: now }],
        providers: [],
        kind: "plan",
      }).reason,
    ).toMatch(/missing provider/i);
    expect(
      resolveroute({
        routes,
        providers: [
          provider({ id: "p1", status: "available" }),
          provider({ id: "p2", status: "unavailable", models: ["claude-test"] }),
        ],
        kind: "plan",
      }).reason,
  ...
  it("reports the unrouted kind and the invalid route", () => {
    ...
    expect(
      resolveroute({
        routes: [{ id: "r", kind: "plan", providerid: "", model: "", revision: 1, updatedat: now }],
        providers,
        kind: "plan",
      }).reason,
    ).toMatch(/failed its validation/i);
  });
  ...
  it("falls back to the user configured pair when the primary misses or stays unavailable", () => {
    ...
    expect(fallbackroute({ routes: [routes[0]!], providers, kind: "plan" }).reason).toMatch(
  ...
  it("marks one provider available or unavailable without touching the others", () => {
    ...
    expect(markprovider({ providers: marked, providerid: "p2", available: true, now: now + 1 })[1]?.status).toBe(
      "available",
    );
  });

  it("bumps the revision and keeps the updatedat timestamp", () => {
    const bumped = bumprevision(
      { id: "r1", kind: "plan", providerid: "p1", model: "m", revision: 5, updatedat: now },
      now + 1,
    );
  ...
  it("returns every route of one kind in the newest revision first order", () => {
    const list = routesfor(
      [
        { id: "r1", kind: "k", providerid: "p1", model: "m", revision: 1, updatedat: now },
        { id: "r2", kind: "k", providerid: "p2", model: "m", revision: 3, updatedat: now + 2 },
        { id: "r3", kind: "other", providerid: "p1", model: "m", revision: 2, updatedat: now + 1 },
      ],
      "k",
    );
...
describe("torture: llm prompt template library", () => {
  const baseTemplate = (over: Partial<prompttemplate> = {}): prompttemplate => ({
    id: "t1",
    name: "summary",
  ...
  it("renders the template when every variable is supplied", () => {
    const rendered = rendertemplate({
      ...
      variables: { topic: "the page", audience: "operators" },
    });
    expect(rendered.text).toBe("Summarize the page for operators.");
  });

  it("refuses to render when a variable stays empty", () => {
    expect(rendertemplate({ template: baseTemplate(), variables: { topic: "", audience: "ops" } }).reason).toMatch(
      /stay empty/i,
    );
  ...
  it("serialises the non string variable values into the rendered text", () => {
    const rendered = rendertemplate({
      template: { id: "t", name: "n", body: "config {{value}}", variables: ["value"], version: 1, createdat: now },
      variables: { value: { key: 1 } },
    });
  ...
  it("requires the consent notice for the sensitive flow", () => {
    ...
    ).toMatch(/consent notice/i);
    const withnotice = rendertemplate({
      template: baseTemplate(),
      variables: { topic: "the page", audience: "operators" },
      sensitive: true,
      consentnotice: "approved by the operator",
    });
