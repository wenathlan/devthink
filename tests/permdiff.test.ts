...
describe("the permdiff logic", () => {
  it("reads the permission set of a manifest in its declared order with its set tags", () => {
    ...
    expect(set.required).toEqual(["required:activeTab", "required:storage", "required:scripting", "required:sidePanel"]);
  ...
  it("reports every removed permission and every move between the required and the optional sets", () => {
    ...
    const moved = { ...basemanifest, permissions: basemanifest.permissions.slice(0, 3), optional_permissions: [...basemanifest.optional_permissions, "sidePanel"] };
  ...
  it("resolves the previous release version of the package", async () => {
    ...
    expect(manifest.permissions.length + manifest.optional_permissions.length + manifest.optional_host_permissions.length).toBe(permissionsetof(manifest).entries.length);
    ...
    const digest = createHash("sha256").update([...new Set(entries)].sort().join("\n")).digest("hex");
