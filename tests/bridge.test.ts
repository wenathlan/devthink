...
describe("servercontract envelope accept and reject per operation", () => {
  it("accepts the four operation bodies and rejects every schema failure with the field that named it", () => {
    ...
    const create = composeenvelope({
      ...
      at: now,
      body: sessioncreatebody({ role: "extension", memberid: "ext", capabilities, pairingcode: "DT-TESTCODE" }),
    });
    ...
    const post = composeenvelope({
      op: "eventpost",
      opid: stableopid("site", 2),
      at: now,
      sessionid: "session-e2e",
      token: "tok-e2e",
      body: eventpostbody({ event: chateventof("collect the pricing table", "site reviewer") }),
    });
    expect(parseenvelope(post).body?.kind).toBe("chat");
    const stream = composeenvelope({
      op: "eventstream",
      opid: stableopid("site", 3),
      at: now,
      sessionid: "session-e2e",
      token: "tok-e2e",
      body: eventstreambody({ streams: ["chat", "review"], since: now - 1000 }),
    });
    ...
    expect(() => sessionjoinbody({ role: "site", memberid: "site", capabilities })).toThrow(
      /names its session id or rides its pairing code/,
    );
    expect(() => eventpostbody({ event: { kind: "nosuchkind" as "chat", stream: "chat", payload: {} } })).toThrow(
      /outside the servercontract event types/,
    );
    expect(() => eventpostbody({ event: { kind: "chat", stream: " ", payload: {} } })).toThrow(
      /names its multiplexed stream/,
    );
...
describe("servercontract capability negotiation and event payloads", () => {
  it("negotiates the shared version with the intersected operations and events while refusing empty intersections", () => {
    const negotiation = negotiatecapabilities(capabilities, {
      version: 1,
      operations: ["sessionjoin", "eventpost"],
      events: ["chat", "planreview"],
      heartbeat: true,
      multiplex: true,
    });
    ...
    expect(negotiatecapabilities(capabilities, { version: 0, operations: ["eventpost"], events: ["chat"] }).ok).toBe(
      false,
    );
    expect(
      negotiatecapabilities(
        { ...capabilities, heartbeat: false },
        { version: 1, operations: ["eventpost"], events: ["chat"], heartbeat: true },
      ).heartbeat,
    ).toBe(false);
    ...
    const card: bridgereviewcard = {
      proposalid: "plan-1",
      title: "Collect the pricing table",
      steps: [
        { id: "s1", kind: "readtext", origin: "https://shop.example", sensitive: false },
        { id: "s2", kind: "submitticket", origin: "https://shop.example", sensitive: true },
      ],
      at: now,
    };
    ...
    const review = planrevieweventpayload({
      proposalid: "plan-1",
      decision: "changes",
      by: "site reviewer",
      note: "skip the checkout",
      at: now,
    });
    expect(review.decision).toBe("changes");
    expect(() =>
      planrevieweventpayload({ proposalid: "plan-1", decision: "later" as "approved", by: "site", at: now }),
    ).toThrow(/approved, changes or refused/);
    expect(progresseventpayload({ stepid: "s1", status: "done" })).toEqual({ stepid: "s1", status: "done" });
    expect(heartbeatevent(now)).toEqual({
      kind: "progress",
      stream: "heartbeat",
      payload: { stepid: "heartbeat", status: "alive" },
    });
    const plan: agentplan = {
      ...
      steps: [
        { id: "s1", kind: "readtext", target: "main", state: "pending" } as never,
        { id: "s2", kind: "readforms", target: "form", state: "pending" } as never,
      ],
      createdat: now,
      expiresat: now + 60000,
      state: "approved",
    ...
    expect(digest.steps).toEqual([
      { id: "s1", kind: "readtext", status: "done" },
      { id: "s2", kind: "readforms", status: "pending" },
    ]);
...
describe("socketrelay connect, reconnect and backoff", () => {
  it("connects through the seam, retries the failed opens with the exponential backoff waits and rotates the token on every retry", async () => {
    ...
    const refused = await connectrelay({
      state: newrelayclient({ url: "wss://relay.example", memberid: "ext", role: "extension", capabilities, now }),
      open: () => Promise.reject(new Error("connection refused")),
      attempts: 3,
      backoffbase: 10,
      sleep: async () => {
        /* the fake sleep waits nothing */
      },
      now: () => now,
    });
    ...
    const disabled = await connectrelay({
      state: newrelayclient({ url: "", memberid: "ext", role: "extension", capabilities, now }),
      open,
      now: () => now,
    });
    ...
    expect(() =>
      newrelayclient({ url: "https://relay.example", memberid: "ext", role: "extension", capabilities, now }),
    ).toThrow(/wss or ws url/);
    expect(() =>
      newrelayclient({ url: "wss://relay.example", memberid: " ", role: "extension", capabilities, now }),
    ).toThrow(/names its member id/);
  });
});
...
describe("socketrelay frame authentication, multiplexing and rate caps", () => {
  it("verifies the session token on every frame, routes the streams and caps the frames of one window", () => {
    ...
    expect(
      verifyframeauth(
        { version: 1, op: "eventpost", opid: "op-site-1", sessionid: "session-e2e", token: "tok-e2e", at: now },
        state,
      ),
    ).toBe(true);
    expect(
      verifyframeauth(
        { version: 1, op: "eventpost", opid: "op-site-1", sessionid: "session-other", token: "tok-e2e", at: now },
        state,
      ),
    ).toBe(false);
    expect(
      verifyframeauth(
        { version: 1, op: "eventpost", opid: "op-site-1", sessionid: "session-e2e", token: "tok-wrong", at: now },
        state,
      ),
    ).toBe(false);
    const connected: relayclientstate = { ...state, state: "connected" };
    const chatframe = frameof({
      version: 1,
      op: "eventpost",
      opid: "op-site-1",
      sessionid: "session-e2e",
    ...
    const progressframe = frameof({
      ...
      token: "tok-e2e",
      at: now,
      body: eventpostbody({ event: heartbeatevent(now) }),
    });
    ...
    const forged = frameof({
      ...
      token: "tok-wrong",
      at: now,
      body: eventpostbody({ event: chateventof("hello", "site") }),
    });
    ...
    expect(
      bridgeframerate({ rate: { used: 1, windowstartedat: now }, cap: 2, window: 1000, now: now + 500 }).allowed,
    ).toBe(true);
    ...
    expect(
      bridgeframerate({ rate: { used: 5, windowstartedat: now }, cap: 2, window: 1000, now: now + 1000 }).allowed,
    ).toBe(true);
...
describe("sharedauth pairing, rotation and revocation", () => {
  it("mints the code with the countdown, exchanges it once per origin, rotates on reconnect and revokes all sessions with one click", async () => {
    ...
    const wrongorigin = await exchangebridgepairing({
      records: [minted],
    ...
    const exchange = await exchangebridgepairing({
      records: [minted],
      origin,
      code: minted.code.code,
      sessionid: "session-e2e",
    ...
    const reused = await exchangebridgepairing({
      records: [{ ...minted, code: exchange.used ?? minted.code }],
      origin,
      code: minted.code.code,
      sessionid: "session-e2e",
      now: now + 1000,
    ...
    expect((await verifybridgetoken({ tokens, origin, sessionid: "session-other", raw: exchange.raw!, now })).ok).toBe(
      false,
    );
...
describe("bridge consent gate, kill switch, member shape and url validation", () => {
  it("blocks the first connection without the recorded consent, refuses every operation under the engaged kill switch and validates the serverurl shape", () => {
    ...
    expect(bridgechoices({ serverurl: "wss://relay.example", bridgeratelimit: 10, bridgeratewindow: 1000 })).toEqual({
      serverurl: "wss://relay.example",
      bridgeratelimit: 10,
      bridgeratewindow: 1000,
    });
...
describe("bridge origin checks reject unpaired senders", () => {
  it("refuses frames of foreign sessions, anonymous senders and unpaired origins while the paired frame passes", () => {
    const origin = "https://relay.example";
    expect(
      bridgeframeorigingate({ framesession: "session-e2e", sessionid: "session-e2e", origin, paired: true }).allowed,
    ).toBe(true);
    expect(
      bridgeframeorigingate({ framesession: "session-other", sessionid: "session-e2e", origin, paired: true }).allowed,
    ).toBe(false);
    expect(bridgeframeorigingate({ sessionid: "session-e2e", origin, paired: false }).allowed).toBe(false);
    expect(bridgeframeorigingate({ framesender: " ", sessionid: "session-e2e", origin, paired: true }).allowed).toBe(
      false,
    );
    expect(
      bridgeframeorigingate({ framesender: "site member", sessionid: "session-e2e", origin, paired: true }).allowed,
    ).toBe(true);
  });
});
...
describe("bridge data minimization keeps page content off the wire", () => {
  it("holds the page content keys without the explicit consent flag, keeps the chat task text and records the bridge audit kind", () => {
    ...
    const proposal = bridgepayload("planproposal", {
      proposalid: "plan-1",
      title: "Collect the pricing table",
      steps: [],
      html: "<main>the whole page</main>",
    ...
    expect(() =>
      bridgepayloadreport({
        kind: "progress",
        payload: { stepid: "s1", status: "done", screenshot: "data:image/png;base64,bytes" },
      }),
    ).toThrow(/page content never crosses the bridge/);
  ...
  it("buffers the frames while the socket is down, deduplicates the operation ids and replays each frame exactly once", () => {
    ...
    const second = postevent(
      ...
      planproposaleventof({
        proposalid: "plan-1",
        title: "Collect",
        steps: [{ id: "s1", kind: "readtext", origin: "https://shop.example", sensitive: false }],
        at: now,
      }),
      now + 1,
    );
    ...
    void used;
    const duplicated: relayclientstate = {
      ...replay.state,
      queue: [...replay.state.queue, { ...unsent, attempts: 0 }],
    };
  ...
  it("sends the heartbeat frame on the user interval and expires the quiet session inside the idle window", () => {
    ...
    const active = receiveframe(
      ...
      frameof({
        ...
        token: "tok-e2e",
        at: now + 20000,
        body: eventpostbody({ event: chateventof("still here", "site") }),
      }),
      now + 20000,
    );
...
describe("chatbridge widget render and review decisions never execute", () => {
  it("renders the chat rows and review cards, collects the decisions and maps them onto the review gate without executing anything", () => {
    const events: bridgeeventrecord[] = [
      {
        ...
        opid: "op-site-1",
        stream: "chat",
        payload: { text: "collect the pricing table", from: "site reviewer" },
        at: now,
        delivered: true,
      },
      {
        ...
        opid: "op-ext-1",
        stream: "chat",
        payload: { text: "the plan proposes two steps", from: "extension" },
        at: now + 1,
        delivered: true,
      },
      {
      ...
      },
      {
        ...
        opid: "op-site-2",
        stream: "review",
        payload: { proposalid: "plan-1", decision: "approved", by: "site reviewer" },
        at: now + 3,
        delivered: true,
      },
      {
    ...
    expect(() => carddecision({ ...cards[0]!, proposalid: " " }, "approved", "site", now)).toThrow(
      /names its proposal/,
    );
    ...
    const pairing = pairingpanel(
      mintbridgepairing({ origin: "https://relay.example", now, lifetime: 120000 }),
      now + 30000,
    );
    ...
    const walksites = (dir: string): void => {
      for (const entry of readdirSync(dir)) {
        if (entry === "node_modules" || entry === "dist")
          continue; /* the installed toolchain and the build output never ship with the site sources */
...
describe("hardcoded relay url scan of the source tree", () => {
  it("asserts no wss or ws endpoint literal and no url at all inside the four bridge modules", () => {
    ...
    for (const path of bridgefiles)
      expect(
        readFileSync(path, "utf8").replaceAll('indexOf("://")', "").includes("://"),
        `${path} carries a url literal`,
      ).toBe(false);
    const codefiles = [
      ...readdirSync(root)
        .filter((file) => file.endsWith(".ts") && statSync(join(root, file)).isFile())
        .map((file) => join(root, file)),
      ...readdirSync(sitedirOf(root))
        .filter((file) => file.endsWith(".html") || file.endsWith(".js") || file.endsWith(".json"))
        .map((file) => join(sitedirOf(root), file)),
    ];
...
function sitedirOf(root: string): string {
    ...
    try {
      const open: socketopen = (url) =>
        new Promise((resolve, reject) => {
          ...
          const socket: relaysocket = {
            send: (frame) => websocket.send(frame),
            close: (code) => websocket.close(code ?? 1000),
          };
      ...
      const extensionconnected = await connectrelay({
        state: extstate,
        open,
      ...
      extsocket.onmessage = (frame) => {
        const received = receiveframe(extclient, frame, Date.now());
        extclient = received.state;
      };
      const sitestate = newrelayclient({
        url: relay.url,
        memberid: "site-reviewer",
        role: "site",
        capabilities,
        now: Date.now(),
      });
      const siteconnected = await connectrelay({
        state: sitestate,
        open,
        pairingcode: relay.pairingcode,
        now: () => Date.now(),
      ...
      sitesocket.onmessage = (frame) => {
        const received = receiveframe(siteclient, frame, Date.now());
        siteclient = received.state;
      };
      const posted = postevent(
        siteclient,
        sitesocket,
        chateventof("collect the pricing table of this page", "site reviewer"),
        Date.now(),
      );
      ...
      const review = postevent(
        siteclient,
        sitesocket,
        { kind: "planreview", stream: "review", payload: decisionpayload(decision) },
        Date.now(),
      );
      ...
      await new Promise((resolve) => setTimeout(resolve, 300));
      const reconnected = await connectrelay({
        state: { ...extclient, state: "reconnecting" },
        open,
        now: () => Date.now(),
      });
      ...
      const replaystate = replaybridgequeue(
        {
          ...reconnected.state,
          queue: [
            {
              opid: "op-ext-replay-1",
              op: "eventpost",
              sessionid: reconnected.state.sessionid,
              body: eventpostbody({ event: chateventof("queued while the socket was down", "extension") }),
              queuedat: Date.now(),
              attempts: 0,
            },
          ],
      ...
      await new Promise((resolve) => setTimeout(resolve, 300));
      const reused = await connectrelay({
        state: newrelayclient({ url: relay.url, memberid: "late-site", role: "site", capabilities, now: Date.now() }),
        open,
        pairingcode: relay.pairingcode,
        now: () => Date.now(),
      });
      ...
      expect(session.events.filter((event) => event.kind === "chat").length).toBe(
        2,
      ); /* the original round trip plus the replayed queued frame */
