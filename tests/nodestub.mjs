...
const stubmessage = (name) =>
  `The node builtin ${name} of the merged devthink core stays behind the platform seam: this platform-portable bundle stubs it, so import the node, bun or deno entry of the library when the workbench surface needs the real builtin.`;
...
export const nodestubplugin = {
  ...
  setup(build) {
    ...
    build.onLoad({ filter: /.*/, namespace: "devthinknodebuiltin" }, (args) => ({
