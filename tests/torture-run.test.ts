...
function plan(over: Partial<agentplan> = {}): agentplan {
  return {
    id: "p1",
    objective: "Check the pricing page",
...
describe("torture: keepalive lifecycle and zombie reaping", () => {
  ...
  it("refuses negative and non finite intervals at open time", () => {
    for (const interval of [0, -1, Number.NaN, Number.POSITIVE_INFINITY, Number.NEGATIVE_INFINITY]) {
      expect(() => openrun({ runid: "r1", sessionid: "s1", planid: "p1", profileid: "u1", interval, now })).toThrow(
        /positive user value/i,
      );
    }
    expect(() => openrun({ runid: "", sessionid: "s1", planid: "p1", profileid: "u1", interval: 1_000, now })).toThrow(
      /run and session ids/i,
    );
  });
  ...
  it("reaps exactly the runs whose silence passed the tolerance and only active ones", () => {
    ...
    const silent = {
      ...base,
      runid: "silent",
      keepalive: { ...base.keepalive, runid: "silent", lastbeatat: now - 10_000 },
    };
    ...
    const result = zombiesweep({
      states: [alive, silent, stopped, alreadyreaped],
      now,
      interval: 1_000,
      missedlimit: 3,
    });
  ...
  it("reaps at exactly the tolerated boundary plus one and never at the boundary", () => {
    ...
    const overboundary = {
      ...base,
      runid: "over",
      keepalive: { ...base.keepalive, runid: "over", lastbeatat: now - 3_001 },
    };
  ...
  it("refuses invalid sweep parameters", () => {
    ...
    expect(() => zombiesweep({ states: [], now, interval: Number.NaN, missedlimit: 3 })).toThrow(
      /positive user value/i,
    );
  ...
  it("records urls, environments and turnarounds with validation", () => {
    ...
    expect(() =>
      recordenvironment(state, { stepid: "", environment: "pagecontext", origin: "https://example.com", now }),
    ).toThrow(/step id/i);
    state = recordenvironment(state, {
      stepid: "s1",
...
describe("torture: offline queue ordering and replay", () => {
  it("queues only approved plans with monotonic sequence and drains in order", () => {
    const queue = [
      ...
      enqueueoffline(enqueueoffline([], { plan: plan({ id: "p1" }), sessionid: "s1", now }), {
        plan: plan({ id: "p2" }),
        sessionid: "s2",
        now: now + 1,
      }),
    ];
    expect(() => enqueueoffline(queue[0]!, { plan: plan({ id: "px", state: "draft" }), sessionid: "s1", now })).toThrow(
      /only an approved plan/i,
    );
...
describe("torture: checkpoints, digests and resume", () => {
  it("refuses checkpoints without their ids and digest", () => {
    ...
    expect(() => makecheckpoint({ runid: "r1", stepid: "s1", completed: [], digest: " ", now })).toThrow(
      /page digest/i,
    );
  ...
  it("freezes progress on failure with the run id stamped", () => {
    ...
    const failed = runfail({
      run: moving,
      progress: { planid: "p1", completedsteps: ["s1"], updatedat: now },
      now: now + 1,
    });
  ...
  it("completes only a run whose plan finished every step", () => {
    ...
    ).toThrow(/never completes/i);
    const done = runcomplete({
      run: moving,
      plan: planfixture,
      progress: { planid: "p1", completedsteps: ["s1", "s2"], updatedat: now },
      now: now + 1,
    });
...
describe("torture: heartbeats and the zombie check", () => {
  ...
  it("zombiecheck names only running runs with stale heartbeats", () => {
    const runs = [
      run({ runid: "alive", state: "running" }),
      run({ runid: "zombie", state: "running" }),
      run({ runid: "queued", state: "queued" }),
      run({ runid: "done", state: "completed" }),
    ];
...
describe("torture: side effects and rollback compensations", () => {
  ...
  it("cancel pairs the rollback choice with compensations only under the explicit user choice", () => {
    ...
    const moving = advancerun(run(), "running", now);
    const none = cancelrollback({
      run: moving,
      steps,
      executedstepids: ["w1", "r1"],
      origin: "https://example.com",
      choice: "none",
      now: now + 1,
    });
...
describe("torture: budget alerts and timeout cancels", () => {
  it("budget alerts fire at their reached severity with critical pausing and warnings informational", () => {
    const atwarning = budgetalertstate({
      budget: runbudgetof({ runid: "r1", steps: 80, stepbudget: 100, pressure: 0.5, now }),
    ...
    const atcritical = budgetalertstate({
      budget: runbudgetof({ runid: "r1", steps: 100, stepbudget: 100, pressure: 0.5, now }),
      thresholds: { warning: 0.8, critical: 0.95 },
    });
    expect(atcritical.map((alert) => alert.level)).toEqual(["critical"]);
    expect(runpausesforalerts(atcritical)).toBe(true);
    const over = budgetalertstate({
      budget: runbudgetof({ runid: "r1", steps: 101, stepbudget: 100, pressure: 0.5, now }),
      thresholds: { warning: 0.5, critical: 0.5 },
    });
    expect(runpausesforalerts(over)).toBe(true);
    const under = budgetalertstate({
      budget: runbudgetof({ runid: "r1", steps: 10, stepbudget: 100, pressure: 0.5, now }),
      thresholds: { warning: 0.8, critical: 0.95 },
    });
    expect(under).toEqual([]);
    const nobudget = budgetalertstate({
      budget: runbudgetof({ runid: "r1", steps: 10, pressure: 0.5, now }),
      thresholds: { warning: 0.5, critical: 0.5 },
    });
  ...
  it("timeout cancels at exactly the bound and one past it while memory pressure alerts beside the steps", () => {
    ...
    const memory = budgetalertstate({
      budget: runbudgetof({ runid: "r1", steps: 1, pressure: 1, memorybudget: 1, now }),
