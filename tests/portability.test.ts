...
describe("importexport and dropimport", () => {
  it("builds the settings bundle with its honest exclusion list", () => {
    const payload = importexportpayloadof({ profile: "work", originprofiles: [{ origin: "https://example.com", decision: "allow" }], siteprofiles: [{ origin: "https://example.com", theme: "dark" }], notes: [{ origin: "https://example.com", title: "Pricing notes", sensitive: false }], preferences: { logstreambuffer: 200 }, at: now });
    ...
    expect(() => importexportpayloadof({ profile: " ", originprofiles: [], siteprofiles: [], notes: [], preferences: {}, at: now })).toThrow(/profile name/);
  });

  it("refuses secretvault values and unmasked logs under any flag", () => {
    const clean = importexportpayloadof({ profile: "work", originprofiles: [], siteprofiles: [], notes: [], preferences: { paletterecents: 5 }, at: now });
    ...
    const logbearing = { ...clean, contents: { ...clean.contents, unmaskedlogs: [{ summary: "the password hunter2 typed in the clear" }] } };
    ...
    expect(() => importexportpayloadof({ profile: "work", originprofiles: [{ token: "abc123" }], siteprofiles: [], notes: [], preferences: {}, at: now })).toThrow(/secretvault value/);
  });

  it("applies an import to the current preferences only through a validated bundle", () => {
    const payload = importexportpayloadof({ profile: "work", originprofiles: [], siteprofiles: [], notes: [], preferences: { paletterecents: 9, uilanguage: "pt" }, at: now });
