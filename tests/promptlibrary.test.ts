...
import { latesttemplate, rendertemplate, removetemplate, savetemplate, searchtemplates, templatevariables } from "../llm.js";
...
describe("promptlibrary", () => {
  ...
  it("renders a template with its variables substituted", () => {
    const template: prompttemplate = { id: "t1", name: "pagesummary", body: "Summarize {{url}} in a {{tone}} tone.", variables: ["url", "tone"], version: 1, createdat: now };
  ...
  it("refuses the render when a declared variable stays empty", () => {
    const template: prompttemplate = { id: "t1", name: "pagesummary", body: "Summarize {{url}}.", variables: ["url"], version: 1, createdat: now };
  ...
  it("requires the consent notice of a sensitive flow and rides it in the render", () => {
    const template: prompttemplate = { id: "t1", name: "extract", body: "Extract {{fields}} from the page.", variables: ["fields"], version: 1, createdat: now };
    expect(rendertemplate({ template, variables: { fields: "names" }, sensitive: true }).reason).toMatch(/consent notice/i);
    const rendered = rendertemplate({ template, variables: { fields: "names" }, sensitive: true, consentnotice: "The extraction leaves the browser only with the granted fields." });
  ...
  it("stores template versions with change notes and keeps the history", () => {
    const first = savetemplate({ templates: [], name: "pagesummary", body: "Summarize {{url}}.", notes: "first version", now });
    expect(first[0]?.version).toBe(1);
    const second = savetemplate({ templates: first, name: "pagesummary", body: "Summarize {{url}} in {{tone}}.", notes: "adds the tone", now: now + 1000 });
  ...
  it("searches the library by name, body, notes and variables", () => {
    const templates = [
      ...
      { id: "t2", name: "extract", body: "Extract {{fields}}.", variables: ["fields"], version: 3, notes: "the field extractor", createdat: now + 1 }
    ];
  ...
  it("removes every version of one template name", () => {
    const stored = savetemplate({ templates: savetemplate({ templates: [], name: "pagesummary", body: "v1", now }), name: "pagesummary", body: "v2", now: now + 1 });
