...
import { datagridcolumnsof, datagridof, exportdatagrid, exportmenudescriptors, exportrowsof, filterdatagridrows, infercolumntype, maskedvalueof, selectrowrange, sortdatagridrows } from "../views.js";
...
describe("datagrid and exportmenu", () => {
  ...
  it("builds the datagrid view of an extraction result and refuses empty shapes", () => {
    const view = datagridof({ title: "  Pricing table  ", origin: "https://example.com", runid: "run-1", rows: [{ item: "screwdriver", price: "12" }, { item: "hammer", price: "20" }], at: now });
    ...
    expect(() => datagridof({ title: "", origin: "https://example.com", runid: "run-1", rows: [{ a: "b" }], at: now })).toThrow(/title/);
    expect(() => datagridof({ title: "grid", origin: " ", runid: "run-1", rows: [{ a: "b" }], at: now })).toThrow(/origin/);
    expect(() => datagridof({ title: "grid", origin: "https://example.com", runid: "run-1", rows: [], at: now })).toThrow(/row/);
  });

  it("sorts and filters rows locally inside the surface", () => {
    const view = datagridof({ title: "grid", origin: "https://example.com", runid: "run-1", rows: [{ item: "hammer", price: "20" }, { item: "screwdriver", price: "12" }, { item: "wrench", price: "15" }], at: now });
  ...
  it("selects a row range for a partial export and refuses a range outside the grid", () => {
    const view = datagridof({ title: "grid", origin: "https://example.com", runid: "run-1", rows: [{ a: "1" }, { a: "2" }, { a: "3" }], at: now });
  ...
  it("offers csv, json and clipboard formats scoped to the selection, the step and the run", () => {
    ...
    expect(menu.filter(descriptor => descriptor.format === "clipboard").every(descriptor => descriptor.destination === "clipboard")).toBe(true);
  });

  it("writes masked values only, honoring the maskinputs verdicts", () => {
    const view = datagridof({ title: "grid", origin: "https://example.com", runid: "run-1", rows: [{ user: "ada", token: "s3cr3tlongvalue" }], at: now });
    ...
    const parsed = JSON.parse(json.text) as { rows: Array<Record<string, unknown>>; columns: Array<{ field: string; type: string }> };
