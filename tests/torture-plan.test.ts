...
describe("torture: plan file parse under schemastrict", () => {
  it("parses one minimal plan file with the documented step fields and grants", () => {
    const parsed = parseplanfile({
      ...
      goal: "go",
      origin: "https://example.com",
      steps: [{ id: "s1", kind: "navigate", label: "open" }],
      grants: ["navigate"],
      denials: ["click"],
    });
  ...
  it("refuses unknown root and step fields under schemastrict", () => {
    ...
    expect(() =>
      parseplanfile({
        ...
        goal: "g",
        origin: "https://example.com",
        steps: [{ id: "s1", kind: "navigate", label: "open", ghost: "haunt" }],
      }),
    ).toThrow(/unknown field/i);
  });
  ...
  it("refuses the non https origin and the empty step list", () => {
    ...
    expect(() => parseplanfile({ version: "1", goal: "g", origin: "https://example.com", steps: [] })).toThrow(
      /at least one step/i,
    );
  });
  ...
  it("refuses blank ids, kinds, labels and accepts the optional target, value and options", () => {
    expect(() =>
      parseplanfile({
        ...
        goal: "g",
        origin: "https://example.com",
        steps: [{ id: " ", kind: "navigate", label: "open" }],
      }),
    ...
    expect(() =>
      parseplanfile({
        ...
        goal: "g",
        origin: "https://example.com",
        steps: [{ id: "s1", kind: "navigate", label: "" }],
      }),
    ).toThrow(/non-empty string/i);
    const parsed = parseplanfile({
      version: "1",
      goal: "g",
      origin: "https://example.com",
      steps: [{ id: "s1", kind: "type", label: "open", target: "#user", value: "alice", options: "{}" }],
    });
  ...
  it("refuses the non integer and non positive loop bound and retry attempts", () => {
    expect(() =>
      parseplanfile({
        ...
        goal: "g",
        origin: "https://example.com",
        steps: [{ id: "s1", kind: "navigate", label: "open", bound: 0 }],
      }),
    ...
    expect(() =>
      parseplanfile({
        ...
        goal: "g",
        origin: "https://example.com",
        steps: [{ id: "s1", kind: "navigate", label: "open", bound: 1.5 }],
      }),
    ).toThrow(/positive integer/i);
    expect(() =>
      parseplanfile({
        ...
        goal: "g",
        origin: "https://example.com",
        steps: [{ id: "s1", kind: "navigate", label: "open", attempts: -1 }],
      }),
    ).toThrow(/positive integer/i);
  });
  ...
  it("accepts the boolean gate declaration and refuses the non boolean one", () => {
    const parsed = parseplanfile({
      ...
      goal: "g",
      origin: "https://example.com",
      steps: [{ id: "s1", kind: "click", label: "open", gate: true }],
    });
    ...
    expect(() =>
      parseplanfile({
        ...
        goal: "g",
        origin: "https://example.com",
        steps: [{ id: "s1", kind: "click", label: "open", gate: "yes" }],
      }),
  ...
  it("refuses the grant list with blank or non string entries", () => {
    ...
    ).toThrow(/non-empty action kind/i);
    expect(() =>
      parseplanfile({
        version: "1",
        goal: "g",
  ...
  it("preserves unicode, cjk and emoji ids and goals without truncation", () => {
    const parsed = parseplanfile({
      ...
      goal: "完成调查 🚀",
      origin: "https://example.com",
      steps: [{ id: "步1", kind: "navigate", label: "打开页面" }],
    });
  ...
  it("lists the documented step fields in the reviewed order", () => {
    expect([...planfilestepfields]).toEqual([
      "id",
      "kind",
      "label",
      "target",
...
describe("torture: plan lint diagnostics and exit code", () => {
  it("raises the loopbound warn for every unbounded loop and the retrybound warn for the missing attempts", () => {
    const file = planfile({
      steps: [
        { id: "s1", kind: "loop", label: "loop the rows" },
        { id: "s2", kind: "retryaction", label: "retry the click" },
        { id: "s3", kind: "navigate", label: "open" },
      ],
    });
    const diagnostics = lintplanfile({
      file,
      ruleset: ruleset(["loopbound", "retrybound"]),
  ...
  it("raises the static selector info for templated targets and the goal and origin errors at the end", () => {
    const file = planfile({
      goal: "",
      origin: "",
      steps: [{ id: "s1", kind: "readtext", label: "open", target: "{{row}}" }],
    });
  ...
  it("computes the deterministic ruleset cache key from the version and the rule ids", () => {
    expect(
      rulesetcachekey({
        ...
        compiledat: now,
        rules: [
          { id: "loopbound", family: "planlint", validates: "loopbound" },
          { id: "retrybound", family: "planlint", validates: "retrybound" },
        ],
      }),
    ).toBe("planlint-2-loopbound+retrybound");
  });
});
...
describe("torture: plan step risk and sensitive class derivation", () => {
  ...
  it("lists the sensitive classes the plan needs consent for", () => {
    const file = planfile({
      steps: [
        { id: "s1", kind: "navigate", label: "open" },
        { id: "s2", kind: "type", label: "type", value: "alice" },
      ],
    });
    ...
    const corrections: correctionentry[] = [
      {
        ...
        stepid: "s1",
        original: "open",
        corrected: "open the page",
        reason: "made the summary clearer",
        source: "edited",
        at: now,
      },
    ];
...
describe("torture: plan stepapprove resolution and immutable log event", () => {
  ...
  it("builds the approve, reject and edit resolutions with the edited shape", () => {
    const approve = stepresolutionof({
      stepid: "s1",
      planid: "p1",
      origin: "https://example.com",
      resolution: "approve",
    ...
    const edit = stepresolutionof({
      stepid: "s1",
      planid: "p1",
      origin: "https://example.com",
      resolution: "edit",
      surface,
      edited: "the new step shape",
      at: now,
    });
...
describe("torture: plan diff preview and mask verdicts", () => {
  it("compares the before and after states and reports the added, changed and removed fields", () => {
    const diff = diffpreviewof({
      stepid: "s1",
      before: { name: "alice", age: "30", removed: "x" },
      after: { name: "alice2", age: "30", added: "y" },
      provenance: "offscreenworker",
    });
    expect(diff.changes.map((c) => c.field)).toEqual(["name", "removed", "added"]);
    expect(diff.changes.find((c) => c.field === "name")).toMatchObject({
      kind: "changed",
      before: "alice",
      after: "alice2",
    });
  ...
  it("derives the mask verdicts of the sensitive fields", () => {
    const verdicts = maskverdictsof({ name: "alice", password: "hunter2", token: "sk-1234567890" }, [
      "password",
      "token",
    ]);
...
describe("torture: plan steps timeline nodes from progress", () => {
  ...
  it("reports the done status from the completedsteps when no outcome carries one", () => {
    const nodes = stepstimelinenodes({
      plan: approvedplan(),
      progress: { planid: "plan1", completedsteps: ["s1", "s2"], updatedat: now },
      now,
    });
  ...
  it("reads the failed outcome and the waiting gate wait status", () => {
    const nodes = stepstimelinenodes({
      ...
      progress: {
        planid: "plan1",
        completedsteps: [],
        outcomes: [{ stepid: "s1", ok: false, summary: "boom", at: now }],
        gatewaits: { s2: { gateid: "s2", kind: "confirm", openedat: now, resolvedat: now, waitedms: 1 } },
        updatedat: now,
      },
      now,
    });
  ...
  it("carries the duration, the environment and the result summary from the progress", () => {
    const nodes = stepstimelinenodes({
      ...
      progress: {
        planid: "plan1",
        completedsteps: ["s1"],
        outcomes: [{ stepid: "s1", ok: true, summary: "done", at: now }],
        turnarounds: { s1: 42 },
        environments: { s1: "offscreenworker" },
        updatedat: now,
      },
      now,
    });
...
describe("torture: plan logstream hash chain integrity", () => {
  it("chains the events under the genesis hash and verifies the chain link by link", async () => {
    const first = await logstreameventof({
      level: "info",
      source: "console",
      origin: "https://example.com",
      summary: "first event",
      masked: false,
      maskverdict: "",
      previous: logstreamgenesis,
      at: now,
    });
    const second = await logstreameventof({
      level: "warn",
      source: "error",
      origin: "https://example.com",
      ...
      stepid: "s1",
      masked: true,
      maskverdict: "the password stays masked",
      previous: first.hash.current,
      at: now + 1,
    });
  ...
  it("detects the broken link and the tampered hash", async () => {
    ...
    const broken = await logstreameventof({
      level: "info",
      source: "console",
      origin: "https://example.com",
      summary: "second",
  ...
  it("copies the verified excerpt range and refuses the out of range and the unverified range", async () => {
    ...
    const third = await logstreameventof({
      level: "info",
      source: "console",
      origin: "https://example.com",
      summary: "third",
