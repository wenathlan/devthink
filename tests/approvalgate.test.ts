...
import { approvalprompt, defaultapprovalwindowms, expireapprovals, listapprovals, redactparams, requireapproval, resolveapproval } from "../gates.js";
...
describe("approval gate lifecycle", () => {
  it("raises gates for sensitive tool calls with the full arguments and timeout", () => {
    const gate = requireapproval({ clientid: "client1", tool: "browser.click", reason: "The click runs only as the approved plan step it names.", params: { stepid: "s1", target: "#go" }, now, timeout: 5000, id: "gate-1" });
    ...
    const secretgate = requireapproval({ clientid: "client1", tool: "browser.type", reason: "Typing.", params: { stepid: "s2", value: "hunter2" }, now, secretfields: ["value"] });
  ...
  it("resolves pending gates with the actor and latency while closed gates stay closed", () => {
    const pending = requireapproval({ clientid: "client1", tool: "browser.click", reason: "Click.", params: { stepid: "s1" }, now, id: "gate-1" });
    const approved = resolveapproval({ requests: [pending], id: "gate-1", decision: "approved", actor: "user", now: now + 4000 });
    ...
    const again = resolveapproval({ requests: approved.requests, id: "gate-1", decision: "refused", actor: "user", now: now + 5000 });
    expect(again.exec).toBeUndefined();
    expect(again.requests[0]?.state).toBe("approved");
    const refused = resolveapproval({ requests: [pending], id: "gate-1", decision: "refused", actor: "user", now: now + 1500 });
    expect(refused.exec?.decision).toBe("refused");
    expect(refused.exec?.latencyms).toBe(1500);
    expect(resolveapproval({ requests: [pending], id: "missing", decision: "approved", actor: "user", now }).exec).toBeUndefined();
  });

  it("expires unanswered gates past their window with the default refusal", () => {
    const short = requireapproval({ clientid: "client1", tool: "browser.click", reason: "Click.", params: {}, now, timeout: 1000, id: "gate-1" });
    const endless = requireapproval({ clientid: "client2", tool: "browser.type", reason: "Type.", params: {}, now, id: "gate-2" });
...
describe("approval prompts and secret redaction", () => {
  it("redacts the fields the user marked secret while the rest stays visible", () => {
    expect(redactparams({ stepid: "s1", value: "hunter2", target: "#go" }, ["value"])).toEqual({ stepid: "s1", value: "[redacted]", target: "#go" });
  ...
  it("carries the client identity, the called tool and the redacted arguments in every prompt", () => {
    const gate = requireapproval({ clientid: "client1", tool: "browser.type", reason: "The typing runs only as the approved plan step it names.", params: { stepid: "s1", value: "hunter2" }, now, secretfields: ["value"] });
...
describe("sensitive tool gating", () => {
  it("grades every sensitive tool with the approval gate requirement and passes read tools free", () => {
    ...
    const drift = { ...catalog.domains[0]!.tools[9]!, consentmeta: { review: "review", riskclass: "read" as const, approvalrequired: false, originscope: "session" as const } };
