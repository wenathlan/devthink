...
describe("incrsnapshot delta computation", () => {
  it("computes only the changed regions with stable fingerprints", () => {
    ...
    const delta = snapshotdeltaof({
      ...
      runid: "run1",
      base,
      current: [...base, { region: "modal", content: "new" }],
      now,
    });
    ...
    const changed = snapshotdeltaof({
      baseref: "base:1",
      runid: "run1",
      base,
      current: [
        { region: "header", content: "Example header" },
        { region: "table", content: "one two four" },
        { region: "footer", content: "Example footer" },
      ],
      now,
    });
    expect(changed.changes.map((change) => change.kind)).toEqual(["changed"]);
    const removed = snapshotdeltaof({
      baseref: "base:1",
      runid: "run1",
      base,
      current: [
  ...
  it("keeps the full snapshot cadence as a user choice and refuses baseless deltas", () => {
    expect(() => snapshotdeltaof({ baseref: " ", runid: "run1", base: [], current: [], now })).toThrow(
      /base snapshot ref/i,
    );
    const full = snapshotdeltaof({
      baseref: "base:1",
      runid: "run1",
      base,
      current: base,
