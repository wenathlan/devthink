...
import { comparepairof, comparepairsforsteps, overlayslider, shotpanelof, shotpanelpan, shotpanelzoom } from "../views.js";
...
describe("shotpanel and compareviewer", () => {
  it("opens the shotpanel view of a granted origin with its redaction verdicts and refuses ungranted captures", () => {
    const view = shotpanelof({ stepid: "step-2", runid: "run-1", captureid: "shot-9", provenance: "fullpage", origin: "https://example.com", granted: ["https://example.com"], redactions: [{ region: "top right card", verdict: "The card number region stays redacted." }], at: now });
    ...
    expect(() => shotpanelof({ stepid: "step-2", runid: "run-1", captureid: "shot-9", provenance: "viewport", origin: "https://other.example", granted: ["https://example.com"], at: now })).toThrow(/allowlist/);
    expect(shotpanelgate({ captureorigin: "https://example.com", granted: ["https://example.com"] }).allowed).toBe(true);
    expect(shotpanelgate({ captureorigin: "https://other.example", granted: ["https://example.com"] }).reason).toMatch(/granted origins/);
  });

  it("zooms and pans the large stitched captures", () => {
    let view = shotpanelof({ stepid: "step-2", runid: "run-1", captureid: "shot-9", provenance: "fullpage", origin: "https://example.com", granted: ["https://example.com"], at: now });
  ...
  it("pairs the before and after captures of every executed write step and overlays them with a slider", () => {
    ...
    expect(() => comparepairof({ stepid: "step-5", beforecaptureid: "shot-10", aftercaptureid: "shot-10" })).toThrow(/distinct/);
