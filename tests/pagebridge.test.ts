...
import { buildclickablemap, matcharia, matchindex, matchname, matchtext, piercescopes, walkframepath, type candidatefields, type framedescription, type scopetree } from "../page.js";
import { evaluatexpath, parsexpath } from "../page.js";
import { clickplan, pathhops, pointersequence } from "../page.js";
import { appendvalue, colorvalue, datevalue, expandstate, heldkeys, multichoices, presshold, radiochoice, releasehold, slidervalue, typetimeschedule, valueevents } from "../page.js";
import { innerstep, runretries } from "../page.js";
import { dialoganswer, parsedialogpolicy } from "../page.js";
import { builda11ytree, buildreader, captureselection, detecttextlanguage, documentlanguage, opengraphfields, pageoutline, shadowpaths, taglanguage, visibletext, visibleentries, type pagenode } from "../page.js";
import { bannermatches, classifytemplate, detectlistpatterns, infinitescrollranges, lazysurvey, normalizetable, overlaygeometry, paginationestimate, scrolllockstate, scrollreport, sectionfingerprint, virtualizedcontainers, type bannercandidate, type imageshape, type siblingsample } from "../page.js";
import { batchmutations, diffsummaries, nodehash, parsewatchoptions, quietfor, quietresolution, rankselectors, scanjson, type nodesummary } from "../page.js";
import { deeplinkurl, evaluatesignals, fragmenturl, loadphase, matchlinkfragment, matchlinktext, parsewaitprofile, parsenavtarget, pickrecenttab, profilefororigin, resolvecontainer, rewritequeryurl, spauroutechanged, urlmatches } from "../page.js";
import { authfor, batchopenset, buildredirectchain, checksafe, classifynavchange, curatelinks, detecthttpstate, domainof, finalurl, interstitialpolicy, navwatchevent, parseratelimit, prefetchcandidates, preconnectorigins, rateallows, ratewindow, recordratehit, redirectcount } from "../page.js";
import { assigntasktabs, audiotabs, badgefromprogress, buildlayout, clonetabs, closeselection, discardcandidates, layoutrestoreplan, normalizedtaburl, parsetabquery, querymatches, regroupaftermoves, renamegroup, restorediscarded, searchtabmatches, switcherlist, switchtarget, tasktabgauge, tasktabsinwindow, trackedtasktabs, watchtabdispatch, windowprofilegrants, zoomstep, type tabshape, type windowshape } from "../commands.js";
import { associateerrors, attachplan, cardmask, classifyfield, detecthoneypots, detectlogin, detecttemplate, filloperations, generatevalue, matchfield, pairentries, parseformrecord, valueshash, captchadetected, type errorcontext, type fieldshape, type fieldsurvey } from "../page.js";
import { backoffwaits, calendarplan, cardgroups, codeready, dependentloaded, parsebackoff, parsedateparts, typeaheadpick, wizardadvance } from "../page.js";
import { applyexpression, classifycolumn, columnspecof, dedupebykeys, expandspans, mapcolumns, mergedatasets, nextcontrol, normalizeheader, parsecsv, readgrid, rowhash, rowsfresh, samplerows, tocsv, toexcel, tojson, transformrows, type cellshape, type rowshape } from "../page.js";
import { advancecursor, advancestream, artifactrecordof, backpressure, builddataset, checksum, chunkplan, exportartifact, exportcontent, gridpreview, interpolate, loopstep, loopvariables, mergetaskrules, newextractsession, newstream, provenancefor, remainingpages, retainedexports, sheetpayload, sortrows, streamfrom } from "../data.js";
import { advancecounter, advancedownload, capturefilename, capturenames, capturesteps, clipentryof, cliphash, concurrentwindow, conflictfree, downloadfilename, mimeallowed, netlogentry, netlogforstep, newquarantine, quarantinedpath, redactheaders, referencedartifacts, released, scanresult, scanverdictof, sweepplan, transitionallowed, verifybytes } from "../commands.js";
import type { artifactinventoryentry, bodyrecord, downloadrecord, exchangerecord, responseentry, shotrecord } from "../types.js";
import { annotationplanof, blendrows, buildname, buildsheet, buildstitchplan, captureelement, captureoptionsof, captureregion, capturestates, capturestitched, capturevisible, croprect, crossesviewport, fixedheadermatch, pairstates, regionsteps, scaledrect, seamweights } from "../capture.js";
import { assetentries, buildpdf, convertdirectiveof, dedupeimages, finishrecording, frameinterval, imagematches, imagefilterof, imagenames, lapseframes, lapseplanof, mediaentries, newrecording, pdfoptionsof, pdfpagesize, pdfsegments, pdftextlayout, recordingoptionsof, streamsummaries, thumbdirectiveof, thumbgeometry } from "../capture.js";
import { channeloptionsof, channelorigin, closechannel, collectmessages, cursorfrom, matchmessage, messagefilterof, newchannel, openchannel, parsessetext, pollcursorof, polldecision, pollurl, publishmessage, receivemessage, reconnectwaits, sequenceintegrity, sserequestheaders, subscriptionoptionsof, type busstate } from "../net.js";
import { apientries, apireplayspecof, bodyfilterof, bodymatches, capturedheaders, capturebody, correlationid, failureclass, filterexchanges, headerfilterof, newexchange, pairexchange, payloadshapeof, privatemime, rankapis, replayurl, resourcefacts, extractvalues, type resourcefact } from "../net.js";
import { applyheaderules, blockruleof, cookiedomaingranted, cookierecordof, matchurlpattern, mockfor, mockspecof, newblockrule, newheaderule, newmockspec, patternorigin, proxyrouteof, ratelimitreadof, ratelimitwait, redactedcookies, revertrule, retryafterof, headeruleof } from "../net.js";
import { authorizeurl, capturecode, formpayloadof, multipartchunks, multipartpayloadof, oauthflowof, parsetokens, revocationruleof, tokenrequest, urlencodeform } from "../auth.js";
import { argkind, attachtimeline, blockingduration, consolediff, consolecapture, errorcapture, filterentries, levelrank, loglevels, longtaskcapture, netfailureentryof, redactconsoletext, rejectioncapture, rotatelogs, serializearg, spamdetect, stackframes, timelinecounts, watcherdetached } from "../run.js";
import { debugwatchoptions, cdpstepoptions } from "../page.js";
import { profilestepoptions } from "../page.js";
import { annotatetrace, annotationof, attachtargetof, capturesourcemaps, cpusnap, expireprofilerecords, flowspecof, growsampleof, growthtrend, heapsnap, heapintervalallowed, mapurlof, measure, profilerkinds, replaytrace, rewritesourcelocation, shiftentryof, stepwindows, tracestart, tracetofile } from "../debug.js";
...
import { allowlistcovers, attachcdpsession, breakpointinputof, capturepause, cdpallowlistof, cdpdomains, cdpkinds, cdpeventruleof, detachcdpsession, methoddomain, overridematches, overrideinputof, recordwatchvalue, sendcdpcommand, serializecdpcommand, stepmodeof, teardowncdpsession, teardownplanof, watchcdpevents, watchexpressionof } from "../debug.js";
import type { breakpointspec, cdpeventrule, cdpsession, scriptoverride, watchexpression } from "../types.js";
import type { timelineentry } from "../types.js";
import { cookiegate, credentialheadername, fetchbudgetallowed, fetchconsentcovers, fetchconsentrefgranted, generatedvalueallowed, ishttpkind, mutationcallof, origincheck, outboundtarget, validateendpointrecord, validatestep } from "../policy.js";
import { callgraphql, callrest, fetchrequestof, fetchoptionsof, graphqlopenvelope, graphqlrequestof, htmlqueriesof, jsonpathrulesof, parsehtmlbody, readpath, readstream, sendfetch, streamwindowof, statusclassof, templateurl, unwrapgraphql, type domparse } from "../http.js";
...
function candidate(overrides: Partial<candidatefields>): candidatefields {
  return { tag: "button", id: "", role: "button", name: "", label: "", text: "", selector: "button:nth-of-type(1)", ...overrides };
}
...
function node(overrides: Partial<pagenode> = {}): pagenode {
  return { tag: "div", selector: "div", id: "", classes: [], role: "", name: "", text: "", value: "", states: [], hidden: false, children: [], ...overrides };
}

describe("pagebridge resolution", () => {
  it("resolves elements by visible text and refuses ambiguity with candidate lists", () => {
    const page = [
      candidate({ tag: "div", role: "", selector: "div:nth-of-type(1)", text: "Sign in to continue", label: "Sign in to continue" }),
  ...
  it("builds the numbered clickable map with stable numbers and matches map indexes", () => {
    ...
    expect(map.entries[1]).toMatchObject({ number: 2, selector: "#search", role: "button", label: "Search", mode: "selector" });
      ...
      children: [
        {
          ...
          text: "",
          children: [
            { tag: "head", attributes: {}, text: "", children: [{ tag: "title", attributes: {}, text: "Shop", children: [] }] },
            {
              tag: "body",
              ...
              children: [
                ...
                { tag: "a", attributes: { href: "https://example.com/about", class: "nav" }, text: "About", children: [] },
                { tag: "a", attributes: { href: "https://example.com/prices", class: "nav" }, text: "Prices", children: [] },
              ],
            },
          ],
        },
  ...
  it("pierces open shadow scopes recursively until the reviewed target is found", () => {
    const inner: scopetree<candidatefields> = { host: candidate({ tag: "my-button", selector: "my-button" }), candidates: [candidate({ tag: "button", text: "Hidden", label: "Hidden", selector: "button.inner" })], shadows: [] };
    const outer: scopetree<candidatefields> = { candidates: [candidate({ tag: "div", text: "Outside", selector: "div" })], shadows: [inner] };
  ...
  it("walks reviewed frame paths and refuses cross origin or absent hops", () => {
    ...
    const outerframe: framedescription = { frames: [{ sameorigin: false }, { sameorigin: true, document: innerframe }] };
...
describe("pagebridge pointer math", () => {
  ...
  it("honors the easeinout shape and waypoint legs", () => {
    ...
    const eased = pathhops({ start: { x: 0, y: 0 }, end: { x: 100, y: 0 }, duration: 160 }, { easing: "easeinout" }, () => 0, 16);
    expect(eased[0]?.x).toBeLessThan(linear[0]?.x as number);
    expect(eased[9]?.x).toBe(100);
    const waypointed = pathhops({ start: { x: 0, y: 0 }, end: { x: 100, y: 100 }, waypoints: [{ x: 100, y: 0 }], duration: 320 }, undefined, () => 0, 16);
  ...
  it("applies speedprofile jitter and peak velocity caps to hop delays", () => {
    const jittered = pathhops({ start: { x: 0, y: 0 }, end: { x: 100, y: 0 }, duration: 160 }, { jitter: 10 }, () => 0.5, 16);
  ...
  it("never lets a reviewed duration override the reviewed peak velocity", () => {
    const rushed = pathhops({ start: { x: 0, y: 0 }, end: { x: 3000, y: 0 }, duration: 300 }, { peak: 100 }, () => 0, 16);
  ...
  it("plans coordinate click sequences with and without the shift modifier", () => {
    ...
    expect(plain.map(event => event.type)).toEqual(["pointerover", "pointermove", "pointerdown", "mousedown", "pointerup", "mouseup", "click"]);
...
describe("pagebridge control math", () => {
  it("schedules timed typing with a per keystroke delay", () => {
    expect(typetimeschedule("abc", 80)).toEqual([{ key: "a", delay: 0 }, { key: "b", delay: 80 }, { key: "c", delay: 80 }]);
  });
  ...
  it("splits multi select choices and refuses missing values", () => {
    const options = [{ value: "br", label: "Brazil" }, { value: "pt", label: "Portugal" }];
  ...
  it("picks radio choices by value or label", () => {
    const group = [{ value: "a", label: "Alpha" }, { value: "b", label: "Beta" }];
...
describe("pagebridge retry loop", () => {
  it("reruns a failed interaction when the fixture moves the element between attempts", async () => {
    ...
    const outcome = await runretries(
      ...
      async () => position,
      async attempt => { attempts.push(attempt); if (attempt === 1) { position = { x: 30, y: 40 }; return { ok: false, summary: "click missed" }; } return { ok: true, summary: "click landed" }; },
    );
  ...
  it("stops retrying when the target stays within the reviewed tolerance", async () => {
    ...
    const outcome = await runretries(
      ...
      async () => ({ x: 0, y: 0 }),
      async attempt => { attempts.push(attempt); return { ok: false, summary: "still failing" }; },
    );
  ...
  it("retries again when the target disappears between attempts", async () => {
    ...
    const outcome = await runretries(
      ...
      async attempt => { attempts.push(attempt); if (attempt === 1) { present = false; return { ok: false, summary: "gone" }; } return { ok: true, summary: "back" }; },
    );
  ...
  it("extracts the reviewed inner step of a wrapper from inline options", () => {
    const step: toolstep = { id: "wrap", kind: "retryaction", summary: "Retry the click.", risk: "interaction", options: "{\"kind\":\"click\",\"target\":\"#btn\",\"value\":\"x\",\"options\":{\"modifiers\":[\"shift\"]},\"retryrule\":{\"attempts\":3}}" };
...
describe("pagebridge dialog policy", () => {
  it("parses reviewed dialog policies from step options", () => {
    const accept: toolstep = { id: "d1", kind: "dismissdialog", summary: "Accept the dialog.", risk: "sensitive", options: "{\"accept\":true}" };
    const answer: toolstep = { id: "d2", kind: "dismissdialog", summary: "Answer the prompt.", risk: "sensitive", options: "{\"answer\":\"devthink\"}" };
    expect(parsedialogpolicy(accept)).toEqual({ accept: true });
    expect(parsedialogpolicy(answer)).toEqual({ accept: false, answer: "devthink" });
    expect(parsedialogpolicy({ id: "d3", kind: "dismissdialog", summary: "Empty policy.", risk: "sensitive" })).toBeNull();
  });
...
describe("pagebridge observation", () => {
  it("captures the a11y tree with roles, names, states and values on fixture trees", () => {
    const tree = node({
      ...
      children: [
        ...
        node({ tag: "div", hidden: true, children: [node({ tag: "p", text: "hidden text", selector: "p:nth-of-type(1)" })] }),
        node({ tag: "ul", role: "list", children: [node({ tag: "li", role: "listitem", text: "First", name: "First", selector: "li:nth-of-type(1)" })] }),
      ],
    });
    const a11y = builda11ytree(tree);
    expect(a11y.role).toBe("document");
    expect(a11y.childcount).toBe(3);
    expect(a11y.children[0]).toMatchObject({ role: "button", name: "Submit order", states: ["disabled"], childcount: 0 });
  ...
  it("returns the rendered text of each visible element and excludes hidden subtrees", () => {
    const tree = node({
      ...
      children: [
        ...
        node({ tag: "div", hidden: true, children: [node({ tag: "p", text: "Invisible", selector: "p:nth-of-type(2)" })] }),
  ...
  it("extracts the reader view with the text density heuristic separating article from chrome", () => {
    const article = node({
      ...
      children: [
        ...
        node({ tag: "p", text: "The accessibility tree gives every element a role and a name.", selector: "p:nth-of-type(1)" }),
        node({ tag: "p", text: "Reader views score text density to keep the article only.", selector: "p:nth-of-type(2)" }),
      ],
    });
    const nav = node({
      tag: "nav",
  ...
  it("reads the current user selection through the selection api", () => {
    expect(captureselection({ getSelection: () => ({ toString: () => "  selected words  " }) })).toEqual({ text: "selected words", length: 14 });
  ...
  it("resolves the page language from document, meta and content signals and routes extracted text", () => {
    expect(documentlanguage({ lang: "pt-BR", meta: "en", text: "the page" })).toEqual({ language: "pt-BR", source: "document" });
    expect(documentlanguage({ lang: "", meta: "fr", text: "the page" })).toEqual({ language: "fr", source: "meta" });
    expect(documentlanguage({ lang: "", meta: "", text: "Este é um texto de exemplo sobre a página" })).toEqual({ language: "pt", source: "content" });
  ...
  it("lists the host paths of nested open shadow roots", () => {
    const inner: scopetree<candidatefields> = { host: candidate({ tag: "my-list", selector: "my-list" }), candidates: [], shadows: [] };
    const outer: scopetree<candidatefields> = { candidates: [], shadows: [inner] };
    expect(shadowpaths(outer)).toEqual(["my-list"]);
    const nested: scopetree<candidatefields> = { host: candidate({ tag: "my-row", selector: "my-row" }), candidates: [], shadows: [] };
...
describe("pagebridge detection", () => {
  ...
  it("normalizes header cells into tableshape column specs with caption", () => {
    ...
    expect(headerless.columns).toEqual([{ label: "column 1", cells: 1 }, { label: "column 2", cells: 1 }]);
  });
  ...
  it("counts pagination entries and estimates the total pages", () => {
    ...
    expect(paginationestimate([{ text: "next", selector: "a", current: false }])).toMatchObject({ current: 0, total: 0, links: 1 });
  });
  ...
  it("detects lazy loaded images and placeholder states", () => {
    const images: imageshape[] = [
      { selector: "#hero", src: "https://example.com/hero.jpg", datasrc: "", loading: "lazy", width: 1200, height: 600 },
      ...
      { selector: "#eager", src: "https://example.com/eager.jpg", datasrc: "", loading: "eager", width: 400, height: 300 },
    ];
  ...
  it("detects scroll locks and modal states with reasons", () => {
    expect(scrolllockstate({ bodyoverflow: "hidden", htmloverflow: "", bodyposition: "", modal: false, scrollable: true })).toMatchObject({ locked: true, reasons: ["overflow hidden"], scrollable: true });
    expect(scrolllockstate({ bodyoverflow: "auto", htmloverflow: "", bodyposition: "fixed", modal: false, scrollable: true })).toMatchObject({ locked: true, reasons: ["fixed body"] });
    expect(scrolllockstate({ bodyoverflow: "auto", htmloverflow: "", bodyposition: "", modal: true, scrollable: true })).toMatchObject({ locked: true, reasons: ["modal open"] });
    expect(scrolllockstate({ bodyoverflow: "auto", htmloverflow: "", bodyposition: "", modal: false, scrollable: false })).toMatchObject({ locked: false, reasons: [] });
  });

  it("classifies the page template from its dominant structural signals", () => {
    expect(classifytemplate({ paragraphs: 0, headings: 1, lists: 0, tables: 0, forms: 1, inputs: 2, password: true })).toBe("login");
    expect(classifytemplate({ paragraphs: 6, headings: 2, lists: 0, tables: 0, forms: 1, inputs: 1, password: false })).toBe("article");
    expect(classifytemplate({ paragraphs: 1, headings: 1, lists: 0, tables: 2, forms: 0, inputs: 0, password: false })).toBe("table");
    expect(classifytemplate({ paragraphs: 1, headings: 1, lists: 0, tables: 0, forms: 1, inputs: 4, password: false })).toBe("form");
    expect(classifytemplate({ paragraphs: 0, headings: 0, lists: 3, tables: 0, forms: 0, inputs: 0, password: false })).toBe("list");
    expect(classifytemplate({ paragraphs: 0, headings: 0, lists: 0, tables: 0, forms: 0, inputs: 0, password: false })).toBe("generic");
  });

  it("computes stable section fingerprints that change with the structure", () => {
    const section = { tag: "section", attributes: { id: "prices", "data-kind": "grid" }, children: 12, textlength: 3400 };
  ...
  it("normalizes scroll positions of the window and containers with edge flags", () => {
    const report = scrollreport(
      { scrollx: 0, scrolly: 0, scrollheight: 2000, clientheight: 800 },
      [
    ...
    expect(report.containers[0]).toMatchObject({ selector: "#feed", scrolltop: 1200, scrollrange: 1200, atbottom: true });
...
describe("pagebridge watch", () => {
  it("parses reviewed watch options with lifetimes, scopes and poll intervals", () => {
    const step: toolstep = { id: "w1", kind: "watchmutate", summary: "Watch the feed.", risk: "read", options: "{\"watchid\":\"feedwatch\",\"scopes\":[\"#feed\"],\"events\":[\"childList\"],\"lifetime\":2500,\"poll\":120}" };
    expect(parsewatchoptions(step, "fallback")).toMatchObject({ watchid: "feedwatch", scopes: ["#feed"], events: ["childList"], lifetime: 2500, poll: 120 });
    const bare: toolstep = { id: "w2", kind: "watchfocus", summary: "Watch focus.", risk: "read", options: "{\"lifetime\":800}" };
    ...
    const broken: toolstep = { id: "w3", kind: "watchbanner", summary: "Watch banners.", risk: "read", options: "not json" };
  ...
  it("batches mutation records so records inside one throttle window flush together", () => {
    ...
    expect(batchmutations(events, 200)).toEqual([
  ...
  it("matches consent banner shapes against the known keyword vocabulary", () => {
    const candidates: bannercandidate[] = [
      { selector: "#cookiebar", id: "cookiebar", classes: ["banner"], text: "We use cookies to improve your experience.", controls: ["Accept all", "Reject"] },
    ...
    expect(reports[0]).toMatchObject({ kind: "cookie", selector: "#cookiebar", controls: ["Accept all", "Reject"], at: 5000 });
  ...
  it("measures network quiet from resource timing entries and decides from probe samples", () => {
    ...
    const reached = quietresolution([{ at: 0, quietfor: 200 }, { at: 300, quietfor: 640 }, { at: 600, quietfor: 700 }], 500, 5000);
    expect(reached).toMatchObject({ ok: true, quietfor: 640, waited: 300, samples: 3 });
    const timedout = quietresolution([{ at: 0, quietfor: 100 }, { at: 700, quietfor: 120 }], 500, 600);
  ...
  it("diffs node summaries into added, removed and changed sets by hashing", () => {
    const summary = (selector: string, tag: string, text: string): nodesummary => ({ selector, tag, text, attributes: {} });
  ...
  it("ranks derived selector candidates by stability score", () => {
    const candidates = rankselectors({ id: "checkout", tag: "button", attributes: { "data-testid": "checkout", name: "checkout", style: "color: red" }, text: "Checkout now", index: 2, siblings: 3 });
...
describe("navigation mastery", () => {
  it("resolves navtarget containers across tabs, windows and private profiles", () => {
    ...
    const tab = resolvecontainer({ url: "https://example.com/a", container: "tab", position: "end", private: false }, windows);
  ...
  it("parses wait profiles and folds the per origin overrides in", () => {
    const step: toolstep = { id: "p1", kind: "navprofile", summary: "Apply the wait profile.", risk: "sensitive", options: "{\"waitprofile\":{\"signals\":[\"load\"],\"idle\":400,\"timeout\":5000,\"overrides\":[{\"origin\":\"https://slow.example\",\"signals\":[\"load\",\"networkidle\"],\"idle\":2000}]}}" };
  ...
  it("evaluates wait load signals against the profile thresholds", () => {
    ...
    expect(evaluatesignals(["load"], samples, 1000)).toMatchObject({ ok: true, satisfied: ["load"], waited: 400, samples: 3 });
    ...
    expect(evaluatesignals(["load", "networkidle"], samples.slice(0, 2), 90)).toMatchObject({ ok: false, satisfied: ["load"], waited: 120 });
  ...
  it("matches urls against every urlpattern mode with query and fragment parts", () => {
    expect(urlmatches("https://example.com/report?page=3", { mode: "prefix", url: "https://example.com/report" })).toBe(true);
    expect(urlmatches("https://example.com/other?page=3", { mode: "prefix", url: "https://example.com/report" })).toBe(false);
    expect(urlmatches("https://example.com/report?page=3", { mode: "exact", url: "https://example.com/report?page=3" })).toBe(true);
    expect(urlmatches("https://example.com/report?page=4", { mode: "exact", url: "https://example.com/report?page=3" })).toBe(false);
    ...
    expect(urlmatches("https://example.com/shop/item/42", { mode: "pattern", url: "https://example.com/shop/*/*" })).toBe(true);
    expect(urlmatches("https://example.com/shop/item/42/reviews", { mode: "pattern", url: "https://example.com/shop/*/*" })).toBe(false);
    expect(urlmatches("https://example.com/shop/item/42/reviews", { mode: "pattern", url: "https://example.com/shop/**" })).toBe(true);
    expect(urlmatches("https://example.com/search?q=devthink", { mode: "host", url: "https://example.com", query: { q: "devthink" } })).toBe(true);
    expect(urlmatches("https://example.com/search?q=other", { mode: "host", url: "https://example.com", query: { q: "devthink" } })).toBe(false);
    expect(urlmatches("https://example.com/search?q=anything", { mode: "host", url: "https://example.com", query: { q: "*" } })).toBe(true);
    expect(urlmatches("https://example.com/search?q=anything", { mode: "host", url: "https://example.com", query: { page: "2" } })).toBe(false);
    expect(urlmatches("https://example.com/docs#faq", { mode: "prefix", url: "https://example.com/docs", fragment: "faq" })).toBe(true);
    expect(urlmatches("https://example.com/docs#top", { mode: "prefix", url: "https://example.com/docs", fragment: "faq" })).toBe(false);
  });

  it("rewrites query parameters and applies fragments", () => {
    const outcome = rewritequeryurl("https://example.com/search?q=devthink&page=2&session=abc", { page: "3", sort: "price" }, ["session"]);
  ...
  it("builds deep links into common web apps and refuses unknown patterns", () => {
    expect(deeplinkurl("github", { owner: "wenathlan", repo: "extension" })).toBe("https://github.com/wenathlan/extension");
    expect(deeplinkurl("github", { owner: "wenathlan", repo: "extension", path: "/issues" })).toBe("https://github.com/wenathlan/extension/issues");
    expect(deeplinkurl("youtube", { id: "abc123" })).toBe("https://www.youtube.com/watch?v=abc123");
    expect(deeplinkurl("youtube", { search: "devthink extension" })).toBe("https://www.youtube.com/results?search_query=devthink%20extension");
    ...
    expect(deeplinkurl("wikipedia", { title: "Navegador", language: "pt" })).toBe("https://pt.wikipedia.org/wiki/Navegador");
    expect(deeplinkurl("amazon", { search: "mechanical keyboard" })).toBe("https://www.amazon.com/s?k=mechanical%20keyboard");
  ...
  it("detects spa route changes and picks recent closed tabs", () => {
    ...
    expect(pickrecenttab(recents, ["https://example.com/a", "https://example.com/b", "https://example.com/c"])).toBeNull();
  });
  ...
  it("assembles redirect chains from navigation watch fixtures with statuses and timing", () => {
    ...
    expect(chain.hops.map(hop => hop.url)).toEqual(["https://example.com/start", "https://example.com/hop1", "https://example.com/hop2", "https://example.com/final"]);
    ...
    const direct = buildredirectchain([{ event: "beforenavigate", url: "https://example.com/a", timestamp: 10 }, { event: "completed", url: "https://example.com/a", timestamp: 40, status: 200 }]);
  ...
  it("detects http errors, offline and certificate interstitials without ever bypassing them", () => {
    expect(detecthttpstate({ offline: false, errors: [], statuses: [200] })).toMatchObject({ httperror: false, offline: false, certificate: false, reasons: [] });
  ...
  it("applies per domain rate limit windows with user configured ceilings", () => {
    ...
    const step: toolstep = { id: "r1", kind: "navrate", summary: "Apply the limit.", risk: "sensitive", options: "{\"ratelimit\":{\"domain\":\"example.com\",\"window\":60000,\"ceiling\":2}}" };
    ...
    expect(parseratelimit({ id: "r3", kind: "navrate", summary: "Broken limit.", risk: "sensitive", options: "{\"ratelimit\":{\"window\":0,\"ceiling\":2}}" })).toBeNull();
  ...
  it("verifies url safety and refuses batch opening on unsafe urls", () => {
    ...
    expect(checksafe("https://user:pass@partner.example/report").reasons).toContain("the url carries embedded credentials");
    ...
    const curated = curatelinks(["https://a.example/1", "https://localhost/x", "https://b.example/2"], url => checksafe(url));
    ...
    expect(batch.refused).toEqual([{ url: "https://localhost/x", reasons: ["the host localhost is a private network target"] }]);
  ...
  it("verifies prefetch candidates against the session grants and dedupes preconnect origins", () => {
    ...
    const verdict = prefetchcandidates(["https://example.com/a", "https://stranger.example/b", "https://partner.example/c"], grants);
    ...
    expect(preconnectorigins(["https://cdn.example", " https://cdn.example ", "https://api.example", ""])).toEqual(["https://cdn.example", "https://api.example"]);
  });
  ...
  it("parses navigation targets from step options", () => {
    const step: toolstep = { id: "t1", kind: "openlink", summary: "Open the url.", risk: "sensitive", options: "{\"navtarget\":{\"url\":\"https://example.com/report\",\"container\":\"window\",\"position\":\"end\"}}" };
    expect(parsenavtarget(step)).toEqual({ url: "https://example.com/report", container: "window", position: "end", private: false });
    expect(parsenavtarget({ ...step, options: "{\"navtarget\":{\"url\":\"https://example.com/report\",\"container\":\"private\"}}" })).toMatchObject({ container: "private", private: true });
    expect(parsenavtarget({ id: "t2", kind: "openlink", summary: "No target.", risk: "sensitive" })).toBeNull();
    expect(parsenavtarget({ id: "t3", kind: "openlink", summary: "Broken target.", risk: "sensitive", options: "{\"navtarget\":\"https://example.com\"}" })).toBeNull();
  });
});

describe("tabs and windows command", () => {
  function tab(overrides: Partial<tabshape> = {}): tabshape {
    return { tabid: 1, url: "https://example.com/a", title: "Example A", index: 0, windowid: 10, active: true, pinned: false, audible: false, muted: false, discarded: false, ...overrides };
  }
  function windowfixture(overrides: Partial<windowshape> = {}): windowshape {
    return { windowid: 10, left: 0, top: 0, width: 1280, height: 800, state: "normal", incognito: false, focused: true, ...overrides };
  }
  ...
  it("resolves querytabs matchers against the live tab set with wildcard patterns", () => {
    const tabs = [
      ...
      tab({ tabid: 3, url: "https://partner.example/report", title: "Partner Report", index: 2, windowid: 11, active: false }),
    ];
    ...
    expect(parsetabquery({ id: "q1", kind: "querytabs", summary: "Query tabs.", risk: "read", options: "{\"tabquery\":{\"pattern\":\"https://example.com/**\"}}" })).toEqual({ pattern: "https://example.com/**" });
  ...
  it("searches open tabs by title and url and lists audio tabs", () => {
    const tabs = [
      ...
      tab({ tabid: 3, url: "https://podcast.example/episode", title: "Episode", muted: true, audible: true, active: false }),
    ];
  ...
  it("keeps tabgroup membership through movetab and movetabwindow and renames groups", () => {
    const groups = [{ groupid: "g1", name: "research", color: "blue", tabids: [1, 2, 3], collapsed: false, savedat: 10 }];
    const moved = [tab({ tabid: 3, index: 0, active: false }), tab({ tabid: 1, index: 1, active: false }), tab({ tabid: 4, index: 2 }), tab({ tabid: 2, index: 3, windowid: 11, active: false })];
    ...
    const emptied = regroupaftermoves([{ groupid: "g2", name: "gone", color: "red", tabids: [9], collapsed: false, savedat: 1 }], moved, 40);
    expect(emptied[0]?.tabids).toEqual([9]);
  });

  it("saves and restores tab layouts with window bounds and group states", () => {
    const tabs = [tab({ tabid: 1 }), tab({ tabid: 2, url: "https://example.com/b", title: "B", index: 1, pinned: true, active: false })];
    const windows = [windowfixture(), windowfixture({ windowid: 11, left: 40, top: 60, width: 900, height: 600, state: "maximized", incognito: true, focused: false })];
    const groups = [{ groupid: "g1", name: "research", color: "blue", tabids: [1, 2, 9], collapsed: true, savedat: 10 }];
    ...
    expect(layout.windows[1]?.state).toMatchObject({ bounds: { left: 40, top: 60, width: 900, height: 600 }, maximized: true, profile: "incognito" });
  ...
  it("separates incognito windows from session grant inheritance and computes window close blockers", () => {
    ...
    const tabs = [tab({ tabid: 1, windowid: 10 }), tab({ tabid: 2, windowid: 10, active: false }), tab({ tabid: 3, windowid: 11, active: false })];
  ...
  it("captures session snapshots and reopens the tabs of a previous run", () => {
    const tabs = [tab({ tabid: 1 }), tab({ tabid: 2, url: "https://example.com/b", title: "B", index: 1, active: false })];
    const layout = buildlayout("run", tabs, [windowfixture()], [], [], 100);
    const snapshot = { id: "snap", sessionid: "run-1", layout, capturedat: 100 };
    expect(layoutrestoreplan(snapshot.layout, ["https://stranger.example/open"])).toEqual(["https://example.com/a", "https://example.com/b"]);
  ...
  it("validates the tabcreate background and window options and the windowcreate bounds grammar", () => {
    ...
    expect(gate({ id: "c1", kind: "tabcreate", value: "https://example.com/a", summary: "Open in background.", risk: "sensitive", options: "{\"background\":true,\"window\":11}" })).toEqual({ allowed: true });
    expect(gate({ id: "c2", kind: "tabcreate", value: "https://example.com/a", summary: "Bad background flag.", risk: "sensitive", options: "{\"background\":\"yes\"}" })).toMatchObject({ allowed: false, reason: "The reviewed background flag must be a boolean." });
    expect(gate({ id: "c3", kind: "tabcreate", value: "https://example.com/a", summary: "Bad window id.", risk: "sensitive", options: "{\"window\":\"eleven\"}" })).toMatchObject({ allowed: false, reason: "The reviewed target window id must be a non-negative integer." });
    expect(gate({ id: "c4", kind: "windowcreate", value: "https://example.com/a", summary: "Open a placed window.", risk: "sensitive", options: "{\"left\":10,\"top\":20,\"width\":800,\"height\":600,\"state\":\"maximized\"}" })).toEqual({ allowed: true });
    expect(gate({ id: "c5", kind: "windowcreate", value: "https://example.com/a", summary: "Bad state.", risk: "sensitive", options: "{\"state\":\"tile\"}" })).toMatchObject({ allowed: false, reason: "The reviewed window state must be normal, maximized, minimized or fullscreen." });
  });

  it("matches closepattern targets and refuses the session tab and unreviewed patterns", () => {
    const tabs = [tab({ tabid: 4 }), tab({ tabid: 5, url: "https://example.com/a/", title: "Example A", index: 1, active: false }), tab({ tabid: 6, url: "https://partner.example/x", title: "Partner", index: 2, active: false })];
    ...
    const gate = (step: toolstep): { allowed: boolean; reason?: string } => validatestep(step, "https://example.com");
    expect(gate({ id: "p1", kind: "closepattern", summary: "Close without review.", risk: "sensitive", options: "{\"tabquery\":{\"pattern\":\"https://example.com/**\"}}" })).toMatchObject({ allowed: false, reason: "The close pattern needs the explicit reviewed flag before any tab closes." });
    expect(gate({ id: "p2", kind: "closepattern", summary: "Close reviewed.", risk: "sensitive", options: "{\"tabquery\":{\"pattern\":\"https://example.com/**\"},\"reviewed\":true}" })).toEqual({ allowed: true });
    expect(gate({ id: "p3", kind: "closepattern", summary: "Close without a matcher.", risk: "sensitive", options: "{\"reviewed\":true}" })).toMatchObject({ allowed: false, reason: "A reviewed tabquery with at least one matcher is required in options." });
  });

  it("computes switchtab neighbors with wraparound and zoom steps without crossing zero", () => {
    const tabs = [tab({ tabid: 1, index: 0, active: false }), tab({ tabid: 2, index: 1, active: true }), tab({ tabid: 3, index: 2, active: false })];
  ...
  it("orders the quick switcher by recency with filter keys", () => {
    const tabs = [tab({ tabid: 1, title: "Docs", index: 0, active: false }), tab({ tabid: 2, title: "Music", url: "https://music.example/p", index: 1, active: false }), tab({ tabid: 3, title: "Report", index: 2, active: false })];
    const recency = [{ tabid: 3, at: 30 }, { tabid: 1, at: 10 }];
...
describe("forms and data", () => {
  function field(overrides: Partial<fieldshape> = {}): fieldshape {
    return { selector: "#name", tag: "input", type: "text", name: "fullname", label: "Full name", placeholder: "Your name", arialabel: "Name field", autocomplete: "name", ...overrides };
  }

  it("resolves controls by label, placeholder, aria label and name attributes", () => {
    const fields = [field(), field({ selector: "#email", name: "email", label: "Email address", placeholder: "you@example.org", arialabel: "Email field", autocomplete: "email" })];
    ...
    expect(matchfield(fields, { mode: "placeholder", placeholder: "you@example" }).map(item => item.selector)).toEqual(["#email"]);
    expect(matchfield(fields, { mode: "placeholder", placeholder: "Email field" }).map(item => item.selector)).toEqual(["#email"]);
    expect(matchfield(fields, { mode: "arialabel", arialabel: "name field" }).map(item => item.selector)).toEqual(["#name"]);
    expect(matchfield(fields, { mode: "arialabel", arialabel: "Your name" }).map(item => item.selector)).toEqual(["#name"]);
  ...
  it("resolves form record entries, skipping honeypots and refusing unmatched or ambiguous entries", () => {
    ...
    const record = parseformrecord({ form: "#form", entries: [
    ...
    expect(valueshash([{ label: "Full name", value: "Ana" }])).not.toBe(valueshash([{ label: "Full name", value: "Bruno" }]));
  });
  ...
  it("associates errors through describedby refs and sibling text, and plans retry backoff windows", () => {
    ...
    const step: toolstep = { id: "r", kind: "retryform", target: "#checkout", summary: "Retry the submission.", risk: "sensitive", options: "{\"backoff\":{\"wait\":500,\"factor\":2},\"attempts\":4}" };
  ...
  it("resolves artifact attachments, splits card groups, masks segments and gates code sources", () => {
    ...
    expect(attachplan("report.pdf", artifacts)).toEqual({ artifact: { id: "a1", name: "report.pdf", kind: "printpdf" } });
  ...
  it("flags honeypots, detects logins and templates, and hands off captchas", () => {
    ...
    const loginfields = [field({ selector: "#user", name: "username", label: "Username" }), field({ selector: "#pass", name: "password", label: "Password", type: "password" })];
    expect(detectlogin(loginfields, ["Sign in", "Forgot password?"])).toEqual({ login: true, markers: ["password field", "identifier field", "session link"] });
    ...
    expect(detecttemplate(checkout, "Complete the checkout and place order now")).toEqual({ template: "checkout", markers: ["checkout", "card number", "place order"] });
    const signup = [field({ selector: "#confirm", name: "confirm", label: "Confirm password" })];
    expect(detecttemplate(signup, "create account and accept the terms of service")).toEqual({ template: "signup", markers: ["create account", "confirm password", "terms"] });
...
describe("pagedata table reading", () => {
  ...
  it("walks header and body rows into column specs and keyed rows", () => {
    ...
    expect(result.rows).toEqual([{ name: "Desk", price: "120" }, { name: "Chair", price: "45" }]);
  ...
  it("extracts nested tables into child datasets linked to their parent rows", () => {
    const result = readgrid([
      ...
      row([cell("A-1"), cell("2 rows", { nested: { selector: "table.items", rows: [row([cell("Sku", { header: true })], true), row([cell("sku-9")])] } })]),
    ]);
  ...
  it("hashes rows over reviewed keys and deduplicates with removal counts", () => {
    ...
    const result = dedupebykeys([first, twin, { name: "Chair", price: "45" }, { name: "Desk", price: "999" }], ["name"]);
  ...
  it("applies reviewed transform expressions and surfaces unsupported expressions as errors", () => {
    ...
    const transformed = transformrows([{ name: " Desk ", price: "120" }], [
  ...
  it("merges datasets across pages aligning columns and filling gaps", () => {
    ...
    expect(merged.rows).toEqual([{ sku: "a", price: "10", stock: "" }, { sku: "b", price: "", stock: "3" }]);
  });
  ...
  it("attaches source url, timestamp and step ref to every sampled row", () => {
    ...
    expect(stamped.rows[0]).toEqual({ sku: "a", source: "https://example.com/list", capturedat: "5000", step: "step-9" });
  ...
  it("serializes datasets to csv and json and parses csv back with quoted fields", () => {
    ...
    const rows = [{ name: "Desk, oak", price: "\"120\"" }, { name: "Chair", price: "45" }];
...
describe("datacommand dataset records", () => {
  const grid = { columns: [columnspecof("Sku"), columnspecof("Price")], rows: [{ sku: "a", price: "10" }, { sku: "b", price: "20" }] };
  ...
  it("builds dataset records and exports csv, json and excel artifacts with checksums", () => {
    ...
    expect(artifactrecordof(artifact)).toEqual({ id: "a1", kind: "csv", name: "catalog.csv", stepid: "step-1", at: 2000 });
  });
  ...
  it("builds provenance records with row ranges and checksums and applies artifact retention", () => {
    const record = provenancefor({ id: "a1", name: "catalog.csv", rowcount: 120, checksum: checksum("rows") }, "https://example.com/list", "step-3", 9000);
    expect(record).toEqual({ artifact: "a1", name: "catalog.csv", url: "https://example.com/list", stepid: "step-3", rowstart: 1, rowend: 120, checksum: checksum("rows"), at: 9000 });
    expect(provenancefor({ id: "a2", name: "empty.csv", rowcount: 0, checksum: "fnv1a-0" }, "https://example.com", "step-4", 9100).rowstart).toBe(0);
  ...
  it("interpolates row variables into looprows inner steps and exposes the variables", () => {
    ...
    const inner: toolstep = { id: "fill", kind: "filllabel", summary: "Fill the row fields.", risk: "sensitive", options: "{\"fields\":[{\"label\":\"{{name}}\",\"value\":\"{{price}}\"}]}" };
  ...
  it("builds grid previews, sorts rows and shapes sheet push payloads", () => {
    const datasetvalue = builddataset("d1", "catalog", { ...grid, rows: [{ sku: "b", price: "20" }, { sku: "a", price: "10" }, { sku: "c", price: "30" }] }, 1000);
    ...
    expect(sheetpayload(datasetvalue, "https://sheets.example/tab")).toEqual({ sheet: "https://sheets.example/tab", columns: ["sku", "price"], rows: datasetvalue.rows });
  });

  it("merges task transform rules and dedupe keys per task", () => {
    const first = mergetaskrules(undefined, "plan-1", [{ expression: "trim", sources: ["name"], target: "name" }], ["sku"], 1000);
    expect(first).toEqual({ taskid: "plan-1", transforms: [{ expression: "trim", sources: ["name"], target: "name" }], dedupekeys: ["sku"], at: 1000 });
...
describe("files, clipboard and downloads command", () => {
  const record = (overrides: Partial<downloadrecord> = {}): downloadrecord => ({ id: "d1", url: "https://example.com/report.pdf", filename: "report.pdf", state: "queued", at: 100, updatedat: 100, ...overrides });
  ...
  it("walks the batch queue state transitions and refuses impossible ones", () => {
    ...
    const done = advancedownload(started, "complete", 300, { path: "/downloads/report.pdf", bytes: 2048, checksum: "fnv1a-abc" });
    expect(done).toMatchObject({ state: "complete", path: "/downloads/report.pdf", bytes: 2048, checksum: "fnv1a-abc" });
  ...
  it("verifies completed downloads against the reviewed size and checksum expectations", () => {
    ...
    expect(verifybytes(complete, { checksum: "fnv1a-other" })).toMatchObject({ ok: false, matches: { checksum: false } });
    expect(verifybytes(record({ state: "running" }), {})).toMatchObject({ ok: false, matches: { state: false } });
    expect(verifybytes(record({ state: "complete" }), { bytes: 2048 })).toMatchObject({ ok: false, matches: { size: false } });
  ...
  it("matches mime filters with exclude winning, include patterns and the deny default", () => {
    const filter = { include: ["application/pdf*", "text/csv"], exclude: ["application/pdf-malformed"], default: "deny" as const };
  ...
  it("correlates netlog records with steps through request ids and redacts header values", () => {
    const entry = netlogentry({ url: "https://example.com/list", method: "GET", status: 200, timing: 120, requestid: "nav-1-2", stepid: "nav-1", at: 900 });
    expect(entry).toEqual({ url: "https://example.com/list", method: "GET", status: 200, timing: 120, requestid: "nav-1-2", stepid: "nav-1", at: 900 });
    const records = [entry, netlogentry({ url: "https://example.com/cart", method: "GET", status: 304, timing: 40, requestid: "nav-2-1", stepid: "nav-2", at: 950 })];
    expect(netlogforstep(records, "nav-1")).toEqual([entry]);
    expect(netlogforstep(records, "nav-3")).toEqual([]);
    expect(redactheaders({ authorization: "Bearer token", cookie: "session=1", accept: "text/html" })).toEqual({ authorization: "[redacted]", cookie: "[redacted]", accept: "[redacted]" });
  });

  it("keeps clipboard payloads out of entries while carrying the hash and origin provenance", () => {
    const payload = "tracking 12345678";
    const entry = clipentryof("read", { hash: cliphash(payload), length: payload.length }, "https://example.com", "clip-step", 1000);
    expect(entry).toEqual({ kind: "read", hash: cliphash(payload), length: 17, origin: "https://example.com", stepid: "clip-step", at: 1000 });
    expect(JSON.stringify(entry)).not.toContain("tracking");
    expect(cliphash("a")).not.toBe(cliphash("b"));
    const write = clipentryof("write", { hash: cliphash("reviewed text"), length: 13 }, "https://example.com", "write-step", 2000);
    expect(write.kind).toBe("write");
    const screen = clipentryof("screen", { hash: cliphash("pngdata"), length: 7 }, "https://example.com", "screen-step", 3000);
  ...
  it("keeps quarantined files outside the downloads folder and maps scan hook verdicts", () => {
    ...
    expect(entry).toEqual({ id: "q1", path: "devthink-quarantine/invoice.pdf", reason: "reviewed quarantine", scan: "pending", at: 5000, updatedat: 5000 });
    ...
    expect(released(scanresult(entry, "clean", 6000), "user-9", 7000)).toMatchObject({ release: "user-9", updatedat: 7000 });
  });
  ...
  it("stamps consistent capture names from task, step and sequence parts with per task counters", () => {
    ...
    expect(capturesteps({}, { steps: [{ id: "one", kind: "observe", summary: "", risk: "read" }, { id: "two", kind: "click", summary: "", risk: "sensitive" }] })).toEqual(["one", "two"]);
  });
  ...
  it("plans cleanup sweeps by age and kind while keeping references and keep policies", () => {
    ...
    expect(sweepplan(entries, [{ age: 5000, kind: "export-csv", keep: "none" }], now, [])).toMatchObject({ remove: ["a1"], keep: ["a2", "a3", "a4"] });
    expect(sweepplan(entries, [{ age: 5000, kind: "any", keep: "none" }], now, ["referenced.json"]).remove).toEqual(["a1"]);
    expect(sweepplan(entries, [{ age: 5000, kind: "any", keep: "none" }], now, []).remove.sort()).toEqual(["a1", "a4"]);
    expect(sweepplan(entries, [{ age: 20000, kind: "any", keep: "none" }], now, [])).toEqual({ remove: [], keep: ["a1", "a2", "a3", "a4"] });
    expect(sweepplan(entries, [{ age: 5000, kind: "any", keep: "all" }], now, [])).toEqual({ remove: [], keep: ["a1", "a2", "a3", "a4"] });
    expect(sweepplan(entries, [{ age: 5000, kind: "any", keep: "latest" }], now, [])).toMatchObject({ remove: ["a1"], keep: ["a2", "a3", "a4"] });
    ...
    const plansteps = { steps: [{ id: "s1", kind: "attachfile", value: "referenced.json" }, { id: "s2", kind: "attachfile", value: "old.csv" }, { id: "s3", kind: "click", value: "x" }] };
...
describe("media capture", () => {
  it("tiles full page captures with scroll offsets and overlap rows while fixed headers never repeat", () => {
    const plan = buildstitchplan({ scrollwidth: 1280, scrollheight: 3200, viewportwidth: 1280, viewportheight: 800, overlap: 40 });
    ...
    const wide = buildstitchplan({ scrollwidth: 2600, scrollheight: 800, viewportwidth: 1280, viewportheight: 800, overlap: 0 });
    expect(wide).toMatchObject({ columns: 3, rows: 1 });
    expect(wide.tiles.map(tile => tile.x)).toEqual([0, 1280, 1320]);
    const clamped = buildstitchplan({ scrollwidth: 1280, scrollheight: 1500, viewportwidth: 1280, viewportheight: 800, overlap: 100 });
  ...
  it("scales element rects at pixel ratios of one, two and three and clamps crops to the viewport", () => {
    ...
    expect(croprect({ x: -20, y: 100, width: 200, height: 900 }, { width: 1280, height: 800 })).toEqual({ x: 0, y: 100, width: 200, height: 700 });
    expect(croprect({ x: 1200, y: 0, width: 200, height: 100 }, { width: 1280, height: 800 })).toEqual({ x: 1200, y: 0, width: 80, height: 100 });
  ...
  it("places contact sheet cells on a labeled grid with the reviewed label style", () => {
    const cells = [{ selector: "#a", label: "Card one" }, { selector: "#b" }, { selector: "#c", label: "Card three" }, { selector: "#d" }, { selector: "#e" }];
    ...
    expect(buildsheet([{ selector: "#a" }], { cellsize: 120, columns: 4, label: "selector" }).cells[0]?.caption).toBe("#a");
    expect(buildsheet([], { cellsize: 120, columns: 3, label: "both" })).toMatchObject({ columns: 3, rows: 1, cells: [] });
  });

  it("pairs before and after state shots around a click action and skips the pair when the action fails", () => {
    const before: shotrecord = { id: "b1", runid: "run-1", stepid: "s1", kind: "shotview", format: "png", width: 1280, height: 800, capturedat: 1 };
    const after: shotrecord = { id: "a1", runid: "run-1", stepid: "s1", kind: "shotview", format: "png", width: 1280, height: 800, capturedat: 2 };
    const paired = pairstates(before, after, { kind: "click", target: "#submit", domsnapshotid: "7" }, 3, "p1");
    expect(paired.pair).toEqual({ id: "p1", beforeid: "b1", afterid: "a1", actionkind: "click", target: "#submit", domsnapshotid: "7", at: 3 });
    ...
    expect(capturestates({ policy: "manual", before, after, actionkind: "click", at: 3, id: "p4" }).pair).toBeUndefined();
    expect(capturestates({ policy: "beforeafter", before, after, actionkind: "click", target: "#submit", domsnapshotid: "7", at: 3, id: "p4" }).pair).toMatchObject({ id: "p4", beforeid: "b1", afterid: "a1" });
    expect(capturestates({ policy: "beforeafter", before, after: undefined, actionkind: "click", at: 3, id: "p5" }).skipped).toBe("after");
  });
  ...
  it("stamps unique capture names inside a long run through the sequence counter", () => {
    ...
    for (let sequence = 1; sequence <= 250; sequence += 1) names.add(buildname(rule, { run: "Order Run", step: "Snap Shot", sequence, kind: "ShotView" }, "PNG"));
    ...
    expect(buildname({ run: true, step: false, sequence: false, kind: true }, { run: "run", step: "s", sequence: 3, kind: "shotregion" }, "webp")).toBe("run-shotregion.webp");
    expect(buildname({ run: false, step: false, sequence: false, kind: false }, { run: "r", step: "s", sequence: 1, kind: "k" }, "")).toBe("capture.png");
    expect(buildname(rule, { run: "", step: "", sequence: 2, kind: "" }, "jpeg")).toBe("capture-capture-2-capture.jpeg");
  });
  ...
  it("normalizes capture options with defaults and builds shot records for every capture kind", () => {
    ...
    expect(captureoptionsof({ format: "webp", quality: "high", pixelratio: 2, annotate: 1, exporttarget: "clipboard" })).toEqual({ format: "webp", pixelratio: 2, exporttarget: "clipboard" });
    expect(captureoptionsof({ quality: 92, annotate: true })).toEqual({ quality: 92, annotate: true });
    const view = capturevisible({ runid: "run-1", stepid: "s1", options: { format: "jpeg", quality: 90, pixelratio: 2, annotate: true, exporttarget: "clipboard" }, viewport: { width: 1280, height: 800 }, dataurl: "data:image/jpeg;base64,abc", at: 5, id: "c1", name: "run-1-s1-1-shotview.jpeg" });
    expect(view).toMatchObject({ id: "c1", runid: "run-1", stepid: "s1", kind: "shotview", format: "jpeg", width: 2560, height: 1600, capturedat: 5, bytes: "data:image/jpeg;base64,abc", name: "run-1-s1-1-shotview.jpeg", annotated: true, exporttarget: "clipboard" });
    const stitch = buildstitchplan({ scrollwidth: 1280, scrollheight: 1600, viewportwidth: 1280, viewportheight: 800, overlap: 0 });
    const full = capturestitched({ runid: "run-1", stepid: "s2", options: {}, plan: stitch, dataurl: "data:image/png;base64,zzz", at: 6, id: "c2" });
    expect(full).toMatchObject({ kind: "shotfullpage", format: "png", width: 1280, height: 1600 });
    const element = captureelement({ runid: "run-1", stepid: "s3", options: { pixelratio: 3 }, rect: { x: 10, y: 20, width: 100, height: 50 }, dataurl: "data:image/png;base64,q", at: 7, id: "c3", target: "#card" });
    expect(element).toMatchObject({ kind: "shotelement", width: 300, height: 150, target: "#card" });
    const region = captureregion({ runid: "run-1", stepid: "s4", options: {}, rect: { x: 0, y: 0, width: 400, height: 300 }, dataurl: "data:image/png;base64,r", at: 8, id: "c4", target: "#feed" });
    expect(region).toMatchObject({ kind: "shotregion", width: 400, height: 300, target: "#feed" });
    const plan = annotationplanof({ step: 3, width: 1280, height: 800, rect: { x: 10, y: 20, width: 100, height: 50 }, url: "https://example.com/list", at: 1_800_000_000_000 });
    ...
    expect(annotationplanof({ step: 1, width: 40, height: 30, url: "https://example.com", at: 0 }).marker).toEqual({ x: 8, y: 8, number: 1 });
  });
});
...
describe("media capture part two", () => {
  it("normalizes pdf options, swaps the page size for landscape and keeps every bound a user choice", () => {
    ...
    expect(pdfoptionsof({ paperwidth: 8.5, paperheight: 14, scale: 2, landscape: true, paginate: true })).toEqual({ paperwidth: 8.5, paperheight: 14, scale: 2, landscape: true, paginate: true });
    expect(pdfoptionsof({ paperwidth: "a4", margins: { top: 0.5, left: 0.25 } })).toEqual({ margins: { top: 0.5, right: 0.4, bottom: 0.4, left: 0.25 } });
  ...
  it("splits long reports into page segments at reviewed break points and walks viewport steps when absent", () => {
    ...
    expect(pdfsegments(1600, 800, [])).toEqual([{ top: 0, height: 800 }, { top: 800, height: 800 }]);
    expect(pdfsegments(2000, 800, [])).toEqual([{ top: 0, height: 800 }, { top: 800, height: 800 }, { top: 1600, height: 400 }]);
    expect(pdfsegments(2000, 800, [1000])).toEqual([{ top: 0, height: 1000 }, { top: 1000, height: 800 }, { top: 1800, height: 200 }]);
    expect(pdfsegments(2000, 800, [500, 500, -10, 9999])).toEqual([{ top: 0, height: 500 }, { top: 500, height: 800 }, { top: 1300, height: 700 }]);
  ...
  it("composes a valid derived pdf document with reviewed page size, margins, scale, landscape and text escaping", () => {
    const composed = buildpdf(["Quarterly report\nRevenue climbed (again) by 12% — plain ascii stays", "Second page"], { paperwidth: 8.5, paperheight: 11, margins: { top: 0.5, right: 0.5, bottom: 0.5, left: 0.5 }, scale: 1, landscape: false, paginate: true });
    ...
    const startxref = Number(composed.document.slice(composed.document.lastIndexOf("startxref") + "startxref".length).trim().split(/\s+/)[0]);
  ...
  it("wraps long report text into page lines at the reviewed geometry", () => {
    const lines = pdftextlayout("one two three four five six seven eight nine ten", { paperwidth: 8.5, paperheight: 11, margins: { top: 0.5, right: 0.5, bottom: 0.5, left: 0.5 }, scale: 1 });
    expect(lines.length).toBeGreaterThan(0);
    expect(lines.every(line => line.length <= 100)).toBe(true);
    const small = pdftextlayout("word ".repeat(400), { paperwidth: 3, paperheight: 5, margins: { top: 1, right: 1, bottom: 1, left: 1 }, scale: 1 });
  ...
  it("opens, times and closes recordings with reviewed scopes, fps windows and clean stops", () => {
    ...
    const record = newrecording({ id: "rec-1", runid: "plan", stepid: "step", tabid: 4, kind: "screen", options, at: 1000 });
    expect(record).toMatchObject({ id: "rec-1", runid: "plan", stepid: "step", tabid: 4, kind: "screen", scope: "run", format: "frames", startedat: 1000, fps: 4, bitrate: 2500, audio: true, frames: [] });
    ...
    const audio = newrecording({ id: "rec-2", runid: "plan", stepid: "step", tabid: 4, kind: "audio", options: recordingoptionsof({}), at: 2000 });
  ...
  it("matches observed images against the reviewed filter, deduplicates identical urls and stamps counter names", () => {
    const filter = imagefilterof({ selector: "main img", minwidth: 200, minheight: 100, formats: ["png", "image/webp"] });
    ...
    const images = [
      ...
      { url: "https://example.com/a.png", alt: "Chart duplicate", width: 400, height: 300, bytes: 1200, mime: "image/png" },
    ...
    expect(names).toEqual(["order-run-grab-step-1-image.png", "order-run-grab-step-2-image.png", "order-run-grab-step-3-image.png"]);
    expect(imagenames({ run: true, step: false, sequence: false, kind: true }, "r", "s", 2, "webp")).toEqual(["r-image.webp", "r-image.webp"]);
  });
  ...
  it("orders lapse frames at the reviewed interval inside the reviewed duration", () => {
    ...
    expect(lapseplanof({ interval: 500, duration: 2000, format: "webp" })).toEqual({ interval: 500, duration: 2000, format: "webp" });
    expect(lapseplanof({ interval: 500, duration: 2000, format: "gif" })).toEqual({ interval: 500, duration: 2000, format: "png" });
  ...
  it("normalizes conversion and thumbnail directives and computes cover and contain thumbnail geometry", () => {
    expect(convertdirectiveof({ source: "png", target: "webp", quality: 90 })).toEqual({ source: "png", target: "webp", quality: 90 });
    ...
    expect(thumbdirectiveof({ size: 240, fit: "cover", suffix: "thumb" })).toEqual({ size: 240, fit: "cover", suffix: "thumb" });
    ...
    expect(cover).toMatchObject({ sw: 200, sh: 200, sx: 100, sy: 0, dx: 0, dy: 0, dw: 100, dh: 100, width: 100, height: 100 });
    const contain = thumbgeometry({ width: 400, height: 200 }, { size: 100, fit: "contain", suffix: "thumb" });
    expect(contain).toMatchObject({ sx: 0, sy: 0, sw: 400, sh: 200, dw: 100, dh: 50, dx: 0, dy: 25, width: 100, height: 100 });
  ...
  it("normalizes raw media, asset and stream probe entries from the page bridge", () => {
    expect(mediaentries([
      { url: "https://example.com/video.mp4", mime: "video/mp4", duration: 61.5, width: 1280, height: 720, codecs: "avc1.42E01E", tracks: ["en"] },
      { url: "https://example.com/audio.mp3", mime: "audio/mpeg" },
      { url: "" },
    ])).toEqual([
      { url: "https://example.com/video.mp4", mime: "video/mp4", duration: 61.5, width: 1280, height: 720, codecs: "avc1.42E01E", tracks: ["en"] },
      { url: "https://example.com/audio.mp3", mime: "audio/mpeg", duration: 0, width: 0, height: 0, codecs: "", tracks: [] },
    ...
    expect(streamsummaries([
      { kind: "webrtc", label: "stream-1", live: true, tracks: [{ kind: "video", label: "Camera", width: 1280, height: 720, framerate: 30, state: "live" }, { kind: "audio", label: "Mic", state: "live" }] },
      { kind: "webrtc", label: "stream-2", live: false, tracks: [] },
    ])).toEqual([
      { kind: "webrtc", tracks: 2, label: "stream-1", live: true, detail: [{ kind: "video", label: "Camera", width: 1280, height: 720, framerate: 30, state: "live" }, { kind: "audio", label: "Mic", state: "live" }] },
...
describe("network observation part one", () => {
  const instant = async (): Promise<void> => { /* the injected sleeper keeps retries instant in tests */ };
  ...
  it("sends one reviewed fetch through the transport seam with status classes and header names", async () => {
    ...
    const transport = async (url: string, init: { method: string; headers: Record<string, string>; body?: string }) => {
      ...
      return { status: 200, headers: { "content-type": "application/json", "x-request-id": "abc" }, body: "{\"ok\":true}" };
    };
    const result = await sendfetch({ request: { url: "https://api.example/data", headers: { "x-review": "1" } }, transport, sleep: instant });
  ...
  it("retries failed requests with the reviewed backoff and reports each retry", async () => {
    ...
    const result = await sendfetch({ request: { url: "https://api.example/pulse" }, options: { retries: 3, backoff: 250 }, transport, sleep: async milliseconds => { waits.push(milliseconds); }, onretry: (attempt, wait, reason) => retries.push({ attempt, wait, reason }) });
  ...
  it("times an attempt out through the reviewed timeout and refuses after the retries", async () => {
    const transport = async () => new Promise(() => { /* the hanging transport never resolves */ }) as Promise<{ status: number; headers: Record<string, string>; body: string }>;
    await expect(sendfetch({ request: { url: "https://api.example/slow" }, options: { timeout: 5, retries: 1 }, transport, sleep: instant })).rejects.toThrow("failed after 2 attempts");
    await expect(sendfetch({ request: { url: "https://api.example/slow" }, options: { timeout: 5, retries: 1 }, transport, sleep: instant })).rejects.toThrow("timed out");
  });

  it("honors the reviewed redirect follow limit and refuses chains past it", async () => {
    let hops = 0;
    const transport = async (url: string) => {
      if (url.endsWith("/start")) return { status: 302, headers: { location: "https://api.example/one" }, body: "", location: "https://api.example/one" };
      if (url.endsWith("/one")) { hops += 1; return { status: 302, headers: {}, body: "", location: "https://api.example/two" }; }
      return { status: 200, headers: {}, body: "arrived" };
    };
    const bounded = await sendfetch({ request: { url: "https://api.example/start", method: "POST" }, options: { follow: 2 }, transport, sleep: instant });
    expect(bounded.status).toBe(200);
    expect(bounded.redirects).toBe(2);
    expect(bounded.url).toBe("https://api.example/two");
    await expect(sendfetch({ request: { url: "https://api.example/start" }, options: { follow: 1 }, transport, sleep: instant })).rejects.toThrow("follow limit");
  });
  ...
  it("parses fetched markup through the parse seam with attribute values, text and element counts", () => {
    const parse: domparse = markup => ({
      query: selector => selector === "a.link"
        ? [{ text: " Docs ", attributes: { href: "/docs", rel: "help" } }, { text: " Blog ", attributes: { href: "/blog" } }]
        : selector === "h1"
          ? [{ text: `parsed ${markup.length} characters`, attributes: {} }]
          : [],
    });
    const results = parsehtmlbody({ body: "<html><body>markup</body></html>", queries: [
      { selector: "a.link", attribute: "href", multi: true },
      { selector: "h1" },
    ], parse });
    expect(results[0]).toEqual({ selector: "a.link", attribute: "href", multi: true, count: 2, values: ["/docs", "/blog"] });
  ...
  it("streams large bodies chunk by chunk with the reviewed handler path and aborts past the byte budget", async () => {
    const seen: string[] = [];
    const totals: number[] = [];
    const window: streamwindow = { budget: 100, onchunk: (chunk: string, total: number) => { seen.push(chunk); totals.push(total); } };
    const done = await readstream({ chunks: ["a".repeat(30), "b".repeat(40), "c".repeat(50)], window });
    expect(done).toEqual({ chunks: 2, bytes: 70, aborted: true, reason: expect.stringContaining("byte budget of 100") });
    ...
    let stop = false;
    const aborted = await readstream({ chunks: ["first", "second", "third"], window: { abort: () => stop, onchunk: () => { stop = true; } } });
    expect(aborted).toMatchObject({ chunks: 1, bytes: 5, aborted: true, reason: expect.stringContaining("abort flag") });
  });

  it("executes typed rest calls with schema validation, url templating and reviewed status classes", async () => {
    const endpoint: endpointrecord = { name: "issues", method: "GET", url: "https://api.example/repos/{owner}/{repo}/issues", schema: { fields: [{ name: "owner", kind: "string", required: true }, { name: "repo", kind: "string", required: true }, { name: "open", kind: "boolean", default: true }] }, version: 1, at: 1 };
    await expect(callrest({ endpoint, payload: { repo: "extension" }, transport: async () => ({ status: 200, headers: {}, body: "" }), sleep: instant })).rejects.toThrow("required field owner");
    const calls: Array<{ url: string; method: string; body?: string }> = [];
    const ok = await callrest({ endpoint, payload: { owner: "wenathlan", repo: "extension" }, transport: async (url, init) => { calls.push({ url, method: init.method, ...(init.body !== undefined ? { body: init.body } : {}) }); return { status: 200, headers: {}, body: "[{\"id\":1}]" }; }, sleep: instant });
    ...
    const custom = await callrest({ endpoint, payload: { owner: "a", repo: "b" }, success: [404], transport: async () => ({ status: 404, headers: {}, body: "{\"error\":\"not found\"}" }), sleep: instant });
    expect(custom.ok).toBe(true);
    expect(custom.errors).toEqual([]);
    const failing = await callrest({ endpoint, payload: { owner: "a", repo: "b" }, transport: async () => ({ status: 500, headers: {}, body: "{\"errors\":[{\"message\":\"boom\"},\"secondary\"]}" }), sleep: instant });
    expect(failing.ok).toBe(false);
    expect(failing.errors).toEqual(["boom", "secondary"]);
    const post: endpointrecord = { name: "create", method: "POST", url: "https://api.example/create", schema: { fields: [{ name: "title", kind: "string", required: true }] }, version: 1, at: 1 };
    const created = await callrest({ endpoint: post, payload: { title: "note" }, transport: async (url, init) => { calls.push({ url, method: init.method, ...(init.body !== undefined ? { body: init.body } : {}) }); return { status: 201, headers: {}, body: "{}" }; }, sleep: instant });
  ...
  it("executes typed graphql calls with envelope wrapping and data and error unwrapping", async () => {
    const endpoint: endpointrecord = { name: "graph", method: "POST", url: "https://api.example/graphql", schema: { fields: [{ name: "unused", kind: "string", required: true }] }, version: 1, at: 1 };
    let sentbody = "";
    const ok = await callgraphql({ endpoint, request: { query: "query Hero($id: ID!){ hero(id:$id){ name } }", operationkind: "query", variables: { id: "1" }, operationname: "Hero" }, transport: async (url, init) => { sentbody = init.body ?? ""; return { status: 200, headers: {}, body: "{\"data\":{\"hero\":{\"name\":\"Luke\"}}}" }; }, sleep: instant });
    expect(JSON.parse(sentbody)).toEqual({ query: "query Hero($id: ID!){ hero(id:$id){ name } }", variables: { id: "1" }, operationName: "Hero" });
    expect(ok.data).toEqual({ hero: { name: "Luke" } });
    expect(ok.errors).toEqual([]);
    const failed = await callgraphql({ endpoint, request: { query: "mutation Like{ like }", operationkind: "mutation" }, transport: async () => ({ status: 200, headers: {}, body: "{\"errors\":[{\"message\":\"auth required\"},\"rate limited\"]}" }), sleep: instant });
    expect(failed.data).toBeUndefined();
    expect(failed.errors).toEqual(["auth required", "rate limited"]);
    const nonjson = await callgraphql({ endpoint, request: { query: "query Q{ q }", operationkind: "query" }, transport: async () => ({ status: 502, headers: {}, body: "bad gateway" }), sleep: instant });
  ...
  it("normalizes fetch requests, fetch options, stream windows, path rules, html queries and graphql requests", () => {
    expect(fetchrequestof({ url: " https://api.example/x ", method: "post", headers: { "x-a": "1" }, body: "b", mode: "cors" })).toEqual({ url: "https://api.example/x", method: "POST", headers: { "x-a": "1" }, body: "b", mode: "cors" });
    expect(fetchrequestof({ url: "" })).toBeUndefined();
    expect(fetchrequestof("nope")).toBeUndefined();
    expect(fetchoptionsof({ timeout: 5, retries: 2, backoff: 100, follow: 3 })).toEqual({ timeout: 5, retries: 2, backoff: 100, follow: 3 });
    expect(fetchoptionsof(null)).toEqual({});
    expect(streamwindowof({ budget: 4096 })).toEqual({ budget: 4096 });
    expect(jsonpathrulesof([{ name: "a", path: "x.y" }, { name: "", path: "x" }, { path: "y" }, "bad", { name: "b", path: "z", kind: "number", default: 3 }])).toEqual([{ name: "a", path: "x.y" }, { name: "b", path: "z", kind: "number", default: 3 }]);
    expect(htmlqueriesof([{ selector: " a ", attribute: "href", multi: true }, { selector: "" }, 3])).toEqual([{ selector: "a", attribute: "href", multi: true }]);
    expect(graphqlrequestof({ query: " query ", operationkind: "query", variables: { a: 1 }, operationname: "Q" })).toEqual({ query: " query ", operationkind: "query", variables: { a: 1 }, operationname: "Q" });
    expect(graphqlrequestof({ query: "q", operationkind: "subscription" })).toBeUndefined();
    expect(templateurl("https://api.example/{owner}/{repo}/issues?state={state}", { owner: "w", repo: "e", state: "open" })).toBe("https://api.example/w/e/issues?state=open");
...
describe("network observation part two sockets", () => {
  ...
  it("opens a reviewed channel through the connect seam with reconnection backoff and failure classes", async () => {
    ...
    const record = newchannel({ id: "ch1", runid: "run", stepid: "s1", kind: "websocket", url: "wss://api.example/stream", protocols: ["chat"], at: 1000 });
    expect(record.origin).toBe("https://api.example");
    expect(record.state).toBe("connecting");
    const waits: number[] = [];
    const opened = await openchannel({ record, options: { reconnect: 3, backoff: 100 }, connect, sleep: async ms => { waits.push(ms); }, now: () => 5000 });
    expect(opened.state).toBe("open");
    expect(opened.reconnects).toBe(2);
    expect(opened.openedat).toBe(5000);
    expect(waits).toEqual([100, 200]);
    expect(attempts).toEqual([1, 2, 3]);
    const failing = await openchannel({ record: newchannel({ id: "ch2", runid: "run", stepid: "s1", kind: "websocket", url: "wss://api.example/down", at: 1 }), options: { reconnect: 1, backoff: 10 }, connect: async () => { throw new Error("network down"); }, sleep: instant, now: () => 2 });
  ...
  it("multiplexes named message streams over one channel with channel scoped sequences", () => {
    ...
    const broken = [first.envelope, second.envelope, third.envelope, { channelid: "ch1", stream: "inbound", payload: "x", sequence: 9, at: 1 }];
  ...
  it("matches message filters by stream and dotted json path and collects up to the reviewed limit", () => {
    ...
    expect(matchmessage({ path: "id" }, { channelid: "ch1", stream: "orders", payload: "not json", sequence: 10, at: 1 })).toBe(false);
    expect(matchmessage({ path: "missing.deep" }, { channelid: "ch1", stream: "orders", payload: "{\"id\":1}", sequence: 11, at: 1 })).toBe(false);
    expect(matchmessage(undefined, { channelid: "ch1", stream: "orders", payload: "{}", sequence: 12, at: 1 })).toBe(true);
  ...
  it("parses server sent events with ids, names, data lines and retry, resuming from the last event id", () => {
    const parsed = parsessetext("id: 41\nevent: userjoin\ndata: first\ndata: second\nretry: 2500\n\n: comment\n\nid: 42\ndata: solo\n\npartial data");
    ...
    expect(subscriptionoptionsof({ url: " https://api.example/stream ", lifetime: 5000, lasteventid: "9", cancel: { kind: "stop", value: "done" } })).toEqual({ url: "https://api.example/stream", lifetime: 5000, lasteventid: "9", cancel: { kind: "stop", value: "done" } });
    expect(subscriptionoptionsof({ url: "https://api.example/stream" })).toBeUndefined();
    expect(subscriptionoptionsof({ url: "https://api.example/stream", cancel: { kind: "never", value: 1 } })).toBeUndefined();
  });

  it("runs long poll cursor loops with stop conditions, poll ceilings and cancellation", () => {
    const parsedcursor = pollcursorof({ url: "https://api.example/poll", cursorfield: "cursor", interval: 50, stop: { field: "done", equals: "yes" }, maxpolls: 3, param: "since" });
    if (!parsedcursor) throw new Error("The reviewed poll cursor fixture must parse.");
    const cursor = parsedcursor;
    expect(pollcursorof({ url: "https://api.example/poll", cursorfield: "cursor", interval: 0, stop: { field: "done", equals: "yes" } })).toBeUndefined();
    expect(pollcursorof({ url: "https://api.example/poll", cursorfield: "", interval: 10, stop: { field: "done", equals: "yes" } })).toBeUndefined();
    ...
    expect(polldecision({ cursor, polls: 0, response: { done: "yes" }, now: 1000 })).toMatchObject({ continue: false, reason: expect.stringContaining("stop condition") });
    expect(polldecision({ cursor, polls: 2, response: { cursor: "9" }, now: 1000 })).toMatchObject({ continue: false, reason: expect.stringContaining("poll ceiling") });
    expect(polldecision({ cursor, polls: 0, response: { cursor: "9" }, cancelled: () => true, now: 1000 })).toMatchObject({ continue: false, reason: expect.stringContaining("cancelled") });
    expect(polldecision({ cursor, polls: 0, response: { cursor: "9" }, expiresat: 999, now: 1000 })).toMatchObject({ continue: false, reason: expect.stringContaining("plan expiry") });
  });

  it("normalizes channel open options and channel origins", () => {
    expect(channeloptionsof({ url: " wss://api.example/live ", protocols: ["a", " ", 3], reconnect: 2, backoff: 100, backoffceiling: 1000, lifetime: 9000 })).toEqual({ url: "wss://api.example/live", options: { protocols: ["a"], reconnect: 2, backoff: 100, backoffceiling: 1000, lifetime: 9000 } });
...
describe("network observation part two netwatch", () => {
  it("derives request facts from the timing buffers and marks failed requests with status and error class", () => {
    const facts = resourcefacts([
      { name: "https://example.com/app.js", initiatorType: "script", entryType: "resource", startTime: 10, duration: 40, transferSize: 1200, nextHopProtocol: "h2" },
      { name: "https://api.example/items", initiatorType: "fetch", entryType: "resource", startTime: 20, duration: 90, transferSize: 0, nextHopProtocol: "" },
      { name: "https://example.com/", initiatorType: "navigation", entryType: "navigation", startTime: 0, duration: 500, transferSize: 4000, nextHopProtocol: "h2", responseStatus: 500 },
      { name: "", initiatorType: "script", entryType: "resource", startTime: 1, duration: 1, transferSize: 1, nextHopProtocol: "h2" },
      { name: "https://example.com/paint", initiatorType: "paint", entryType: "paint", startTime: 2, duration: 1, transferSize: 0, nextHopProtocol: "" },
    ]);
    ...
    expect(failureclass({ ...(facts[2] as resourcefact), status: 500 })).toEqual({ errorclass: "httperror", status: 500 });
    expect(failureclass({ ...(facts[0] as resourcefact), failed: true })).toEqual({ errorclass: "networkerror", status: 0 });
    const exchange = newexchange({ id: "e1", runid: "run", stepid: "s1", correlationid: correlationid("run", 0), fact: facts[1] as resourcefact, at: 50 });
    ...
    expect(newexchange({ id: "e2", runid: "run", stepid: "s1", correlationid: correlationid("run", 1), fact: facts[0] as resourcefact, at: 51 }).method).toBe("GET");
    expect(correlationid("run", 0)).toBe("run-1");
  });

  it("joins request and response pairs through correlation ids and refuses mismatches", () => {
    const exchange = newexchange({ id: "e1", runid: "run", stepid: "s1", correlationid: "run-1", fact: { url: "https://api.example/items", initiator: "fetch", entrytype: "resource", start: 1, duration: 30, transfer: 0, protocol: "h2" }, at: 10 });
    const response: responseentry = { correlationid: "run-1", status: 200, headers: { "content-type": "application/json" }, bytes: 128, mime: "application/json", bodyref: "body1", at: 12 };
    ...
    expect(filterexchanges([newexchange({ id: "e9", runid: "run", stepid: "s1", correlationid: "run-9", fact: { url: "https://api.example/x", initiator: "fetch", entrytype: "resource", start: 1, duration: 90, transfer: 0, protocol: "" }, at: 1 })], { status: "failed" })).toHaveLength(1);
  });

  it("applies header allowlists and redaction lists before any header value is stored", () => {
    const filter = headerfilterof({ allow: ["Content-Type", " x-request-id ", "X-Auth-Token"], redact: ["X-Auth-Token"] });
    ...
    expect(capturedheaders({ "Content-Type": "application/json", "X-Auth-Token": "secret", "X-Other": "kept" }, filter)).toEqual({ "content-type": "application/json", "x-auth-token": "[redacted]" });
  ...
  it("captures bodies through reviewed filters with byte ceilings and truncation", () => {
    ...
    const exchange = newexchange({ id: "e1", runid: "run", stepid: "s1", correlationid: "run-1", fact: { url: "https://api.example/items", initiator: "fetch", entrytype: "resource", start: 1, duration: 30, transfer: 0, protocol: "h2" }, at: 10 });
    expect(bodymatches(filter, { url: "https://api.example/items", mime: "application/json; charset=utf-8" })).toBe(true);
    ...
    const captured = capturebody({ ref: "body1", runid: "run", exchange: { ...exchange, mime: "application/json" }, body: "{\"items\":[1,2,3]}", mime: "application/json", filter, at: 20 });
    ...
    const refused = capturebody({ ref: "body2", runid: "run", exchange: { ...exchange, url: "https://other.example/x" }, body: "x", mime: "text/html", filter, at: 20 });
  ...
  it("maps and ranks page api endpoints by frequency, json share and payload stability", () => {
    const mk = (id: string, correlationid: string, url: string, bodyref?: string): exchangerecord => ({ id, runid: "run", stepid: "s1", correlationid, url, origin: new URL(url).origin, method: "GET", status: 200, statusclass: "success", source: "extension", timing: 10, bytes: 20, at: 1, ...(bodyref !== undefined ? { bodyref } : {}) });
    const exchanges = [
    ...
    const bodies: bodyrecord[] = [
      { ref: "b1", runid: "run", correlationid: "run-1", url: "https://api.example/items?page=1", mime: "application/json", bytes: 10, body: "{\"id\":1,\"name\":\"a\"}", at: 1 },
      { ref: "b2", runid: "run", correlationid: "run-2", url: "https://api.example/items?page=2", mime: "application/json", bytes: 10, body: "{\"id\":2,\"name\":\"b\"}", at: 1 },
      { ref: "b3", runid: "run", correlationid: "run-3", url: "https://api.example/items?page=3", mime: "application/json", bytes: 10, body: "not json", at: 1 },
      { ref: "b4", runid: "run", correlationid: "run-4", url: "https://api.example/users?u=1", mime: "application/json", bytes: 10, body: "{\"user\":{\"id\":1}}", at: 1 },
    ];
  ...
  it("builds reviewed api replay specs with overrides and extraction paths", () => {
    const parsedspec = apireplayspecof({ endpoint: " https://api.example/items ", verb: "get", overrides: { page: "2", limit: "10" }, paths: ["items.0.id", "total"] });
    if (!parsedspec) throw new Error("The reviewed api replay fixture must parse.");
    const spec = parsedspec;
    expect(spec).toEqual({ endpoint: "https://api.example/items", verb: "GET", overrides: { page: "2", limit: "10" }, paths: ["items.0.id", "total"] });
    ...
    expect(apireplayspecof({ endpoint: "https://api.example/items", overrides: { a: 1, b: "x" } })?.overrides).toEqual({ b: "x" });
    expect(extractvalues("{\"items\":[{\"id\":7}],\"total\":1}", ["items.0.id", "total", "missing"])).toEqual([{ path: "items.0.id", value: 7 }, { path: "total", value: 1 }, { path: "missing", missing: true }]);
...
describe("network control rules", () => {
  ...
  it("serves the first reviewed mock fixture that matches and never unreviewed ones", () => {
    const spec = newmockspec({ id: "m1", runid: "run1", stepid: "s1", urlpattern: "https://api.example/status", status: 204, body: "{\"ok\":true}", reviewed: true, at: 5 });
    ...
    const replay = mockspecof({ urlpattern: "https://api.example/items", status: 200, bodyref: "body-7", reviewed: true });
    expect(replay).toMatchObject({ bodyref: "body-7", status: 200 });
    const resolved = newmockspec({ id: "m2", runid: "run1", stepid: "s1", urlpattern: "https://api.example/items", status: 200, body: "captured-body", bodyref: "body-7", reviewed: true, at: 2 });
  ...
  it("applies set, append and remove header operations with the provenance of every applied rule", () => {
    const set = newheaderule({ id: "h1", runid: "run1", stepid: "s1", urlpattern: "https://api.example/*", name: "accept", operation: "set", value: "application/json", at: 1 });
    const append = newheaderule({ id: "h2", runid: "run1", stepid: "s1", urlpattern: "https://api.example/*", name: "x-trace", operation: "append", value: "b", at: 1 });
    const remove = newheaderule({ id: "h3", runid: "run1", stepid: "s1", urlpattern: "https://api.example/*", name: "cookie", operation: "remove", at: 1 });
    const other = newheaderule({ id: "h4", runid: "run1", stepid: "s1", urlpattern: "https://other.example/*", name: "accept", operation: "set", value: "text/html", at: 1 });
    const result = applyheaderules("https://api.example/items", { accept: "text/html", cookie: "session=1", "x-trace": "a" }, [set, append, remove, other]);
    expect(result.headers).toEqual({ accept: "application/json", "x-trace": "a, b" });
    expect(result.applied.map(rule => rule.id)).toEqual(["h1", "h2", "h3"]);
    expect(headeruleof({ urlpattern: "https://api.example", name: "accept", operation: "set", value: "1" })?.operation).toBe("set");
    expect(headeruleof({ urlpattern: "https://api.example", name: "accept", operation: "remove", value: "1" })).toBeUndefined();
  });
  ...
  it("scopes cookies to granted domains and refuses every other domain", () => {
    ...
    expect(redactedcookies([record ?? { name: "", domain: "", path: "", value: "secret", }])).toEqual([{ name: "session", domain: "example.com", path: "/", expiresat: 50 }]);
  });

  it("parses proxy routes only with a non-empty bypass list", () => {
    expect(proxyrouteof({ scheme: "socks5", host: "proxy.example", port: 1080, bypass: ["https://api.example"] })).toMatchObject({ scheme: "socks5", port: 1080 });
    expect(proxyrouteof({ scheme: "socks5", host: "proxy.example", port: 1080, bypass: [] })).toBeUndefined();
    expect(proxyrouteof({ scheme: "socks5", host: "proxy.example", port: 0, bypass: ["https://api.example"] })).toBeUndefined();
    expect(proxyrouteof({ scheme: "ftp", host: "proxy.example", port: 1080, bypass: ["https://api.example"] })).toBeUndefined();
  });

  it("parses rate limit reads and waits until the reset window passes", () => {
    const now = 1_000_000;
    const epoch = ratelimitreadof({ "x-ratelimit-remaining": "4", "x-ratelimit-limit": "10", "x-ratelimit-reset": String(Math.floor(now / 1000) + 30) }, "https://api.example", now);
...
describe("auth flows and uploads", () => {
  const flow = { provider: "providerco", authorizeurl: "https://auth.providerco/authorize", tokenurl: "https://api.providerco/token", scopes: ["read", "write"], redirectorigin: "https://app.example/callback" };
  ...
  it("builds the provider consent url with the state token", () => {
    ...
    expect(oauthflowof({ provider: "p", authorizeurl: "https://a.example", tokenurl: "https://t.example", scopes: ["read"], redirectorigin: "https://r.example" })).toBeDefined();
    expect(oauthflowof({ provider: "p", authorizeurl: "https://a.example", tokenurl: "https://t.example", scopes: [], redirectorigin: "https://r.example" })).toBeUndefined();
  });

  it("captures the redirect code only on the granted redirect origin with the matching state", () => {
    const good = capturecode("https://app.example/callback?code=abc&state=state-1", "https://app.example/callback", "state-1");
    expect(good).toEqual({ code: "abc" });
    expect(capturecode("https://evil.example/callback?code=abc&state=state-1", "https://app.example/callback", "state-1")?.error).toContain("outside the granted redirect origin");
    expect(capturecode("https://app.example/callback?code=abc&state=other", "https://app.example/callback", "state-1")?.error).toContain("state token");
    expect(capturecode("https://app.example/callback?error=access_denied&state=state-1", "https://app.example/callback", "state-1")?.error).toContain("access_denied");
    expect(capturecode("https://app.example/callback?state=state-1", "https://app.example/callback", "state-1")?.error).toContain("no authorization code");
  });
  ...
  it("parses token responses and revocation rules", () => {
    ...
    expect(revocationruleof({ tokenids: ["t1"], reason: "user demand" })).toMatchObject({ tokenids: ["t1"], reason: "user demand" });
    expect(revocationruleof({ tokenids: [], reason: "x" })).toBeUndefined();
  });

  it("encodes urlencoded form payloads with the form grammar", () => {
    expect(urlencodeform([{ name: "a b", value: "1&2" }, { name: "é", value: "x/y" }])).toBe("a%20b=1%262&%C3%A9=x%2Fy");
    expect(formpayloadof({ url: "https://api.example/submit", fields: [{ name: "q", value: "devthink" }] })).toMatchObject({ url: "https://api.example/submit" });
  ...
  it("streams multipart payloads chunk by chunk without buffering the whole body", () => {
    const payload = multipartpayloadof({ url: "https://api.example/upload", fields: [{ name: "label", value: "report" }], files: [{ name: "file", filename: "report.csv", mime: "text/csv", content: "a,b\n1,2\n", reviewed: true }], boundary: "BOUND" });
    ...
    expect(multipartpayloadof({ url: "https://api.example/upload", files: [{ name: "f", filename: "a.txt", mime: "text/plain", content: "x", reviewed: false }] })).toBeUndefined();
  });
});
...
describe("run timeline", () => {
  ...
  function entry(overrides: Partial<timelineentry> = {}): timelineentry {
    return { id: `e${Math.random().toString(36).slice(2, 8)}`, runid: "run1", stepid: "s1", time: 100, level: "info", source: "console", message: "hello", ...overrides };
  }
  ...
  it("parses stack frames and captures errors and rejections with redaction", () => {
    ...
    expect(frames[0]).toMatchObject({ functionname: "loadItems", url: "https://example.com/app.js", line: 42, column: 17 });
    expect(frames[1]).toMatchObject({ url: "https://example.com/main.js", line: 7 });
    const error = errorcapture({ message: "boom with key=zzz", sourceurl: "https://example.com/app.js", line: 42, stacktext: stack, redact: ["zzz"] });
  ...
  it("marks failed requests of the run as net failure entries", () => {
    const failed = netfailureentryof({ id: "nf1", at: 9, exchange: { id: "x1", runid: "run1", stepid: "s1", correlationid: "run1-3", url: "https://api.example/items", origin: "https://api.example", method: "?", status: 0, statusclass: "unknown", errorclass: "networkerror", source: "page", timing: 12, bytes: 0, at: 8 } });
    if (!failed) throw new Error("The failed exchange must mark the timeline.");
    expect(failed).toMatchObject({ runid: "run1", url: "https://api.example/items", errorclass: "networkerror", correlationid: "run1-3" });
    const httperror = netfailureentryof({ id: "nf2", at: 9, exchange: { id: "x2", runid: "run1", stepid: "s1", correlationid: "run1-4", url: "https://api.example/missing", origin: "https://api.example", method: "GET", status: 404, statusclass: "clienterror", source: "page", timing: 20, bytes: 90, at: 8 } });
    expect(httperror?.errorclass).toBe("httperror");
    expect(netfailureentryof({ id: "nf3", at: 9, exchange: { id: "x3", runid: "run1", stepid: "s1", correlationid: "run1-5", url: "https://api.example/ok", origin: "https://api.example", method: "GET", status: 200, statusclass: "success", source: "page", timing: 20, bytes: 90, at: 8 } })).toBeNull();
  });

  it("observes long task entries with attribution and blocking duration per step window", () => {
    const captured = longtaskcapture({ entries: [{ starttime: 10, duration: 90, attributions: ["same-origin"] }, { starttime: 40, duration: 45, attributions: [] }, { starttime: 60, duration: 20, attributions: ["cross-origin"] }], threshold: 30 });
    expect(captured).toHaveLength(2);
    expect(captured[0]).toMatchObject({ duration: 90, starttime: 10, attributions: ["same-origin"] });
    const blocking = blockingduration([{ starttime: 10, duration: 90 }, { starttime: 40, duration: 45 }, { starttime: 400, duration: 500 }], "s1", { startedat: 5, endedat: 100 });
  ...
  it("collapses repeated messages into counts and flags patterns over the reviewed threshold", () => {
    const repeats = [entry({ message: "retrying", time: 100 }), entry({ message: "retrying", time: 150 }), entry({ message: "retrying", time: 200 }), entry({ message: "done", time: 220 })];
  ...
  it("classifies console lines between two runs as added, removed and repeated", () => {
    const diff = consolediff({ baseid: "run1", targetid: "run2", baselines: ["boot", "ready", "ready", "gone"], targetlines: ["boot", "ready", "fresh"], now: 9 });
  ...
  it("counts entries per level and parses the debug watch options of every kind", () => {
    ...
    const step: toolstep = { id: "w1", kind: "watchconsole", summary: "Watch the console", risk: "read", options: JSON.stringify({ watch: { window: 250 }, level: "warn", depth: 3, redact: ["secret"], spam: { pattern: "retry", windowsize: 500, collapse: 2 }, rotation: { maxentries: 50, overflowtarget: "overflow1" } }) };
...
describe("cdp bus", () => {
  ...
  function session(overrides: Partial<cdpsession> = {}): cdpsession {
    return { id: "cdp1", runid: "plan", stepid: "a1", tabid: 4, origin: "https://example.com", attachedat: 10, domains: ["Runtime", "Debugger"], debuggerversion: "devthink instrumented harness 1.1.46 (no chrome.debugger permission)", ...overrides };
  }

  /** Fixture breakpoint factory on plain shapes. */
  function breakpoint(overrides: Partial<breakpointspec> = {}): breakpointspec {
    return { id: "bp1", runid: "plan", stepid: "b1", url: "https://example.com/app.js", line: 12, hits: 2, registeredat: 5, ...overrides };
  }

  /** Fixture script override factory on plain shapes. */
  function override(overrides: Partial<scriptoverride> = {}): scriptoverride {
    return { id: "ov1", runid: "plan", stepid: "o1", urlpattern: "https://cdn.example/vendor.js", source: "window.fixture = true;", reviewed: true, hits: 1, appliedat: 6, ...overrides };
  }
  ...
  it("attaches the instrumented session with the reviewed domains and detaches it cleanly", () => {
    ...
    const attached = attachcdpsession({ id: "cdp1", runid: "plan", stepid: "a1", tabid: 4, origin: "https://example.com", domains: ["Runtime", "Runtime", "Debugger"], now: 10, debuggerversion: "harness" });
  ...
  it("validates method names, allowlists and raw command records with duration and error class", () => {
    ...
    const command = sendcdpcommand({ id: "c1", sessionid: "cdp1", runid: "plan", stepid: "c1", method: "Runtime.evaluate", params: { expression: "1 + 1" }, duration: 12, at: 20 });
    expect(command).toMatchObject({ domain: "Runtime", duration: 12 });
    expect(command.errorclass).toBeUndefined();
    const malformed = sendcdpcommand({ id: "c2", sessionid: "cdp1", runid: "plan", stepid: "c2", method: "not a method", duration: 3, at: 21 });
    expect(malformed.domain).toBe("");
    expect(malformed.errorclass).toBe("malformedmethod");
    const failed = sendcdpcommand({ id: "c3", sessionid: "cdp1", runid: "plan", stepid: "c3", method: "DOM.getSnapshot", duration: 7, errorclass: "uninstrumented", at: 22 });
    expect(failed.errorclass).toBe("uninstrumented");
  });

  it("serializes concurrent commands per session in send order", () => {
    const first = sendcdpcommand({ id: "c1", sessionid: "cdp1", runid: "plan", stepid: "s", method: "Runtime.evaluate", duration: 5, at: 1 });
    const second = sendcdpcommand({ id: "c2", sessionid: "cdp1", runid: "plan", stepid: "s", method: "DOM.getSnapshot", duration: 6, at: 2 });
    const third = sendcdpcommand({ id: "c3", sessionid: "cdp1", runid: "plan", stepid: "s", method: "Page.enable", duration: 7, at: 3 });
  ...
  it("subscribes domain event rules with match filters, counts per domain and closes the subscription", () => {
    ...
    const rules: cdpeventrule[] = [
      { id: "r1", sessionid: "cdp1", runid: "plan", stepid: "w1", domain: "Log", event: "entryAdded", match: "fixture", events: 0, registeredat: 1 },
      { id: "r2", sessionid: "cdp1", runid: "plan", stepid: "w1", domain: "Page", event: "loadEventFired", events: 0, registeredat: 1 },
    ];
  ...
  it("registers breakpoints and captures pause states with call frames and dom snapshots", () => {
    const input = breakpointinputof({ url: "https://example.com/app.js", line: 12, column: 4, condition: "items.length > 0" });
    ...
    const pause = capturepause({ id: "p1", runid: "plan", stepid: "b1", reason: "breakpoint", callframes: [{ functionname: "load", url: "https://example.com/app.js", line: 12, column: 4 }], hitbreakpoint: "bp1", domsnapshotid: "dom-7", at: 30 });
  ...
  it("records watch expression values at every pause with the pause scope", () => {
    ...
    const expression: watchexpression = { id: "we1", runid: "plan", stepid: "w1", expression: "items.length", scope: "topframe", reviewed: true, values: [], at: 2 };
    const first = recordwatchvalue(expression, "p1", "3", 30);
    const second = recordwatchvalue(first, "p2", "7", 40);
    expect(second.values).toEqual([{ pauseid: "p1", value: "3", at: 30 }, { pauseid: "p2", value: "7", at: 40 }]);
  });
  ...
  it("tears sessions down safely when the user detaches the debugger", () => {
    ...
    const userdetach = teardowncdpsession({ session: attached, breakpoints: active.breakpoints, overrides: active.overrides, plan, userdetached: true, at: 90 });
    ...
    const rundetach = teardowncdpsession({ session: attached, breakpoints: active.breakpoints, overrides: active.overrides, plan, userdetached: false, at: 95 });
    ...
    expect(teardowncdpsession({ session: attached, breakpoints: [revertedbp], overrides: [], plan: undefined, userdetached: false, at: 96 }).revertedbreakpoints).toEqual([]);
  });

  it("parses the cdp step options of every kind through the page normalizers", () => {
    const attach: toolstep = { id: "a1", kind: "attachcdp", summary: "Attach", risk: "sensitive", options: JSON.stringify({ domains: ["Runtime", "Debugger"], teardown: { revertsteps: ["revert breakpoints"], resumepolicy: "pause" }, allowlist: { domains: ["Runtime"], methods: ["Runtime.evaluate"] } }) };
    ...
    const cmd: toolstep = { id: "c1", kind: "cdpcmd", summary: "Evaluate", risk: "sensitive", options: JSON.stringify({ command: { method: "Runtime.evaluate", params: { expression: "1" }, resultpath: "result.value" } }) };
    expect(cdpstepoptions(cmd).command).toEqual({ method: "Runtime.evaluate", params: { expression: "1" }, resultpath: "result.value" });
    const watch: toolstep = { id: "w1", kind: "watchcdp", summary: "Watch events", risk: "read", options: JSON.stringify({ events: [{ domain: "Log", event: "entryAdded", match: "x" }], watch: { window: 250 } }) };
...
describe("profilers", () => {
  ...
  const spec = { prefix: "flow", steps: ["s1", "s2"], metrics: ["navigation", "paint", "lcp", "fid", "interaction", "blocking"] };
  const entries = [
  ...
  it("lists the profiling kinds of the debugging part three family", () => {
    expect(profilerkinds).toEqual(["measureflow", "heapshot", "trackmemory", "profilecpu", "watchshifts", "traceload", "annotatetrace", "replaytrace", "capturesourcemaps"]);
  });

  it("marks the start and end of every step in the flow window and sums the durations", () => {
    const windows = stepwindows(spec, entries.filter(entry => entry.type === "mark"));
    expect(windows).toEqual([{ stepid: "s1", start: 0, end: 100 }, { stepid: "s2", start: 100, end: 250 }]);
    const metrics = measure({ runid: "run", stepid: "m1", spec, entries, now });
    const byname = (name: string): Array<{ duration: number; steps: string[] }> => metrics.filter(metric => metric.name === name).map(metric => ({ duration: metric.duration, steps: metric.steps }));
    expect(byname("navigation")).toEqual([{ duration: 800, steps: ["s1", "s2"] }]);
    expect(byname("paint")).toEqual([{ duration: 0, steps: ["s1"] }, { duration: 0, steps: ["s1"] }]);
    ...
    expect(byname("blocking")).toEqual([{ duration: 70, steps: ["s1"] }, { duration: 20, steps: ["s2"] }]);
  ...
  it("keeps flow metrics inside the reviewed metric set of the spec", () => {
    ...
    expect(flowspecof({ prefix: "f", steps: ["s1"], metrics: ["navigation"] })).toEqual({ prefix: "f", steps: ["s1"], metrics: ["navigation"] });
  });

  it("captures heap snapshots on demand with byte and node counts bounded by the user chosen interval only", () => {
    const record = heapsnap({ id: "h1", runid: "run", stepid: "hs", origin: "https://example.com", usedbytes: 12_000_000, limitbytes: 40_000_000, nodecount: 1450, now });
  ...
  it("computes growth trend slopes and flags the steps above the reviewed slope", () => {
    const samples = [
      ...
      growsampleof({ id: "g2", runid: "run", stepid: "s2", usedbytes: 3_000_000, limitbytes: 4_000_000, now: now + 1000 }),
    ];
  ...
  it("captures cpu profiles around the step window with the hot functions ranked by sampled time", () => {
    const record = cpusnap({ id: "c1", runid: "run", stepid: "cpu", origin: "https://example.com", duration: 500, samples: [{ name: "render", time: 50 }, { name: "parse", time: 20 }, { name: "render", time: 30 }], now });
    expect(record).toMatchObject({ duration: 500, samplecount: 3, hotfunctions: ["render", "parse"] });
    expect(cpusnap({ id: "c2", runid: "run", stepid: "cpu", origin: "https://example.com", duration: 10, samples: [], now }).hotfunctions).toEqual([]);
  });

  it("scores layout shifts with their impacted element selectors", () => {
    const entry = shiftentryof({ id: "sh1", runid: "run", stepid: "ws", score: 0.12, starttime: 40, selectors: ["img#hero", "div#main"], at: now });
  ...
  it("records one trace, exports it with step annotations aligned to the run timeline and replays it offline", () => {
    const trace = tracestart({ id: "t1", runid: "run", stepid: "tl", origin: "https://example.com", categories: ["scripting", "painting"], now: 1000 });
    expect(trace).toMatchObject({ categories: ["scripting", "painting"], bytesize: 0, events: 0 });
    const annotated = annotatetrace({ trace, annotations: [{ stepid: "s1", label: "open panel", offset: 0 }, { stepid: "s2", label: "run search", offset: 0 }], timeline: [{ stepid: "s1", time: 1500 }, { stepid: "s2", time: 2600 }], now: 3000 });
    expect(annotated.annotations).toEqual([{ stepid: "s1", label: "open panel", offset: 500 }, { stepid: "s2", label: "run search", offset: 1600 }]);
    const file = tracetofile({ ...annotated, endedat: 3000 }, [{ name: "keydown", category: "scripting", offset: 300 }, { name: "layout-shift", category: "painting", offset: 1600 }]);
  ...
  it("normalizes the attach targets of page, iframe, worker and service worker", () => {
    expect(attachtargetof({ kind: "page", url: "https://example.com" })).toEqual({ kind: "page", url: "https://example.com" });
    expect(attachtargetof({ kind: "iframe", url: "https://cdn.example/frame" })).toEqual({ kind: "iframe", url: "https://cdn.example/frame" });
    expect(attachtargetof({ kind: "worker", url: "https://example.com/worker.js" })).toEqual({ kind: "worker", url: "https://example.com/worker.js" });
    expect(attachtargetof({ kind: "serviceworker", url: "https://example.com/sw.js" })).toEqual({ kind: "serviceworker", url: "https://example.com/sw.js" });
  ...
  it("captures source map declarations of the loaded scripts and rewrites stack locations through the parsed maps", () => {
    expect(mapurlof("https://example.com/app.js", "console.log(1);\n//# sourceMappingURL=app.js.map")).toBe("https://example.com/app.js.map");
    expect(mapurlof("https://example.com/app.js", "console.log(1);")).toBeUndefined();
    const refs = capturesourcemaps({ runid: "run", stepid: "sm", origin: "https://example.com", scripts: [{ url: "https://example.com/app.js", source: "//# sourceMappingURL=app.js.map" }, { url: "https://example.com/other.js", source: "no map" }], now });
    expect(refs).toHaveLength(1);
    expect(refs[0]).toMatchObject({ scripturl: "https://example.com/app.js", mapurl: "https://example.com/app.js.map", parsed: false });
    const map = { sources: ["src/app.ts"], mappings: "AAAA;EACC" };
    expect(rewritesourcelocation({ url: "https://example.com/app.js", line: 1 }, map)).toEqual({ url: "src/app.ts", line: 1 });
    expect(rewritesourcelocation({ url: "https://example.com/app.js", line: 0 }, map)).toEqual({ url: "src/app.ts", line: 0 });
    expect(rewritesourcelocation({ url: "https://example.com/app.js", line: 9 }, map)).toBeUndefined();
    expect(rewritesourcelocation({ url: "https://example.com/app.js", line: 0 }, { sources: [], mappings: "AAAA" })).toBeUndefined();
  });

  it("expires the heavy profile bytes after the retention window while the metadata survives", () => {
    const heaps = [{ id: "h1", runid: "run", stepid: "hs", origin: "https://example.com", bytesize: 10, nodecount: 5, capturedat: now - 2000 }];
    const profiles = [{ id: "c1", runid: "run", stepid: "cpu", origin: "https://example.com", duration: 5, samplecount: 2, hotfunctions: ["render"], at: now - 500 }];
    const traces = [{ id: "t1", runid: "run", stepid: "tl", origin: "https://example.com", categories: ["scripting"], bytesize: 40, events: 2, annotations: [{ stepid: "s1", label: "x", offset: 0 }], startedat: now - 2000, endedat: now - 2000 }];
    ...
    expect(expireprofilerecords({ heaps, profiles, traces, retention: undefined, now }).heaps[0]?.bytesexpired).toBeUndefined();
  });

  it("parses the profiling step options of the page bridge", () => {
    const step: toolstep = { id: "m1", kind: "measureflow", summary: "Measure", risk: "read", options: JSON.stringify({ flow: spec, watch: { window: 250 }, threshold: 0.1, trace: { categories: ["scripting"], window: 100, exporttarget: "download", traceid: "t1" } }) };
    expect(profilestepoptions(step)).toMatchObject({ flow: { prefix: "flow", steps: ["s1", "s2"], metrics: spec.metrics }, watchwindow: 250, threshold: 0.1, categories: ["scripting"], exporttarget: "download", traceid: "t1" });
    expect(profilestepoptions({ id: "h", kind: "heapshot", summary: "Heap", risk: "sensitive" })).toMatchObject({ watchwindow: 0, heapinterval: 0, duration: 0, threshold: 0, categories: [], scripts: [] });
    expect(profilestepoptions({ id: "t", kind: "trackmemory", summary: "Track", risk: "read", options: JSON.stringify({ growth: { slope: 2, interval: 50 } }) })).toMatchObject({ growth: { slope: 2, interval: 50 } });
  });
});

describe("page emulation", () => {
  const emulationstep = (kind: toolstep["kind"], options: Record<string, unknown>): toolstep => ({ id: "e1", kind, summary: "Emulate the run tab", risk: "sensitive", options: JSON.stringify(options) });
  ...
  it("lists the emulation kinds of the 1.1.48 family", () => {
    expect(emulationkinds).toEqual(["emulatedevice", "emulatenetwork", "emulatelocate", "setuseragent", "overridepermission", "blackboxscripts"]);
  });
  ...
  it("refuses emulation steps with malformed or absent reviewed presets before any mask applies", async () => {
    ...
    expect((await runemulationstep(emulationstep("emulatenetwork", { network: { name: "x", latency: -1, download: 1, upload: 1 } }))).ok).toBe(false);
    expect((await runemulationstep(emulationstep("emulatelocate", { location: { name: "x", latitude: 91, longitude: 0, accuracy: 1 } }))).ok).toBe(false);
    expect((await runemulationstep(emulationstep("setuseragent", { agent: { ...agent, useragent: "no version" } }))).ok).toBe(false);
    expect((await runemulationstep(emulationstep("overridepermission", { permission: { name: "screen-capture", state: "granted" } }))).ok).toBe(false);
  ...
  it("registers the blackbox patterns in the page registry and reports the offline window of the network layer without touching page state", async () => {
    ...
    const offline = await runemulationstep(emulationstep("emulatenetwork", { reviewed: true, network, revertplan, window: 2000 }));
  ...
  it("carries the honest derivation note beside every emulation result because no debugger or platform permission exists in the manifest", async () => {
    ...
    const blackbox = await runemulationstep(emulationstep("blackboxscripts", { reviewed: true, rules: [{ urlpatterns: ["https://cdn.example/**"], tracescope: "both" }], revertplan }));
