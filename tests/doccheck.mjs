...
if (!existsSync("dist/policy.js") || !existsSync("dist/index.js") || !existsSync("dist/protocol.js")) {
  console.error("DOCCHECK The gate reads the compiled kind, tool and message catalogs; run pnpm build before the documentation check.");
  process.exitCode = 1;
}
if (process.exitCode !== 1) {
  ...
  record("kindcoverage", missingkinds.length === 0 && extraentries.length === 0 && entries.length === catalog.length, [...missingkinds.map(kind => `the kind ${kind} documents nowhere`), ...extraentries.map(kind => `the entry ${kind} stayed outside the catalog`)]);
  ...
  const fieldproblems = [];
  const sections = kinddocstext.split(/^### `/m).slice(1).map(section => ["### `" + section, section.split("\n")]);
  for (const [heading, body] of sections) {
    const kind = /^### `([a-z0-9]+)`/.exec(heading)?.[1] ?? "";
    if (kind === "" || !catalog.includes(kind)) continue;
    if (!body.some(line => line.startsWith("- options grammar:"))) fieldproblems.push(`the ${kind} entry documents no option grammar`);
    ...
    else if (consentline.slice("- consent class: ".length).trim() !== riskof[kind]) fieldproblems.push(`the ${kind} entry states the consent class ${consentline.slice(18).trim()} while policy grades ${riskof[kind]}`);
    if (!body.some(line => line.startsWith("- capability:"))) fieldproblems.push(`the ${kind} entry states no capability requirement`);
    ...
    else if (!existsSync(testline.slice("- policy test: ".length).trim())) fieldproblems.push(`the ${kind} entry links the absent policy test ${testline.slice(15).trim()}`);
    if (!body.some(line => line.startsWith("- schema anchor:"))) fieldproblems.push(`the ${kind} entry links no schema anchor`);
  }
  ...
  for (const [heading, body] of sections) {
    ...
    if (blockstart < 0) { exampleproblems.push(`the ${kind} entry carries no example`); continue; }
    const blockend = body.findIndex((line, index) => index > blockstart && line.startsWith("```"));
    if (blockend < 0) { exampleproblems.push(`the ${kind} example never closes`); continue; }
    let example;
    try { example = JSON.parse(body.slice(blockstart + 1, blockend).join("\n")); } catch { exampleproblems.push(`the ${kind} example fails to parse`); continue; }
    if (example.kind !== kind) exampleproblems.push(`the ${kind} example carries the kind ${example.kind}`);
    if (example.id !== "step1" || typeof example.summary !== "string" || example.summary === "") exampleproblems.push(`the ${kind} example misses the required id or summary`);
    if (example.risk !== riskof[kind]) exampleproblems.push(`the ${kind} example carries the risk ${example.risk} while policy grades ${riskof[kind]}`);
    if (targetof[kind] === true && typeof example.target !== "string") exampleproblems.push(`the ${kind} example misses the target the kind requires`);
    if (targetof[kind] === false && "target" in example) exampleproblems.push(`the ${kind} example carries a target the kind never reads`);
  }
  ...
  for (const file of docfiles) {
    ...
    for (const match of content.matchAll(/\[[^\]]*\]\(([^)]+)\)/g)) {
      ...
      const resolved = pathpart === "" ? join("docs", file) : pathpart.startsWith("/") ? pathpart.slice(1) : join("docs", pathpart);
      if (!existsSync(resolved)) { linkproblems.push(`the ${file} link ${target} resolves to no file`); continue; }
      if (anchorpart !== undefined && anchorpart !== "" && resolved.endsWith(".md")) {
        const anchordoc = await readFile(resolved, "utf8");
        if (!anchordoc.includes(`#${anchorpart.replaceAll("-", " ")}`) && !new RegExp(`^#+\\s+.*${anchorpart}`, "im").test(anchordoc)) linkproblems.push(`the ${file} link ${target} resolves to no anchor`);
      }
    }
    let fence = false;
  ...
  const clicommands = [...clisource.matchAll(/"(manifest|describe|commands|planlint|migrateplan|flowrun|runworkflow|exportdata|headless|serve|native|export|init|doctor|help)"/g)].map(match => match[1]);
  const uniquecommands = [...new Set(clicommands)];
  for (const command of uniquecommands) if (!refdocstext.includes(`\`${command}\``)) tableproblems.push(`the cli command ${command} documents nowhere in the reference`);
  for (const message of frozenmessagecatalog) if (!refdocstext.includes(`\`${message.type}\``)) tableproblems.push(`the protocol message ${message.type} documents nowhere in the reference`);
  ...
  for (const kind of auditkinds) if (!refdocstext.includes(`\`${kind}\``)) tableproblems.push(`the audit kind ${kind} documents nowhere in the reference`);
  const tools = alltools(buildtoolcatalog());
  for (const tool of tools) if (!refdocstext.includes(`\`${tool.name}\``)) tableproblems.push(`the mcp tool ${tool.name} documents nowhere in the reference`);
  for (const code of ["parse", "method", "params", "internal", "consentrefused", "stepfailed", "schemaerror", "unsupported", "cancelled"]) if (!refdocstext.includes(`\`${code}\``)) tableproblems.push(`the error code ${code} documents nowhere in the reference`);
  const configkeys = ["devthink.relayurl", "devthink.ratelimit", "devthink.phishguard", "DEVTHINK_HTTP_BIND", "DEVTHINK_HTTP_PORT", "DEVTHINK_MCP_BIND", "DEVTHINK_MCP_PORT", "DEVTHINK_MCP_PATH", "DEVTHINK_RELAY_PATH"];
  const configurationtext = await text("docs/configuration.md");
  for (const key of configkeys) if (!configurationtext.toLowerCase().includes(key.toLowerCase().split(".")[key.split(".").length - 1])) tableproblems.push(`the configuration key ${key} documents nowhere in docs/configuration.md`);
  ...
  record("flowdiagrams", mermaidblocks >= 12 && modulelinks === mermaidblocks, [`the flow documentation carries ${mermaidblocks} mermaid diagrams with ${modulelinks} module link lines`]);
  ...
  let tags = [];
  try { tags = (await execute("git", ["tag", "-l", "v*"])).stdout.split("\n").filter(tag => tag !== ""); } catch { tags = []; }
  const changelogtext = await text("CHANGELOG.md");
  for (const tag of tags) {
    const version = tag.slice(1);
    if (!new RegExp(`^##\\s+${version.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}(?:\\s|$)`, "m").test(changelogtext)) changelogproblems.push(`the released version ${version} documents nowhere in the changelog`);
  }
  ...
  if (!roadmaptext.includes("1.1.31 to 2.0.0")) roadmapproblems.push("the roadmap title no longer spans the full chain");
  ...
  for (const file of (await readdir("dist/caps")).filter(name => name.endsWith(".json"))) {
    ...
    if (!readmetext.toLowerCase().includes(surface)) readmeproblems.push(`the readme documents the ${surface} surface nowhere`);
  }
  ...
  const report = {
    ...
    checks,
    summary: { total: checks.length, passed: checks.filter(check => check.ok).length, failed: checks.filter(check => !check.ok).length },
  };
  ...
  if (report.summary.failed > 0) {
    ...
    console.log(`Documentation verified for ${release}: ${catalog.length} kinds, ${auditkinds.length} audit kinds, ${tools.length} mcp tools, ${frozenmessagecatalog.length} protocol messages, ${uniquecommands.length} cli commands, ${mermaidblocks} flow diagrams; ${report.summary.passed} check families green.`);
  }
}
