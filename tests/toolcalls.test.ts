...
import { applymock, applyratelimit, begincall, batchrisk, checkidempotency, dryruntool, endcall, expireidempotency, maptoolerror, recordidempotency, callretryhintof, runbatch, structurederrorof } from "../tools.js";
...
const session = { id: "sess", tabid: 7, origin: "https://example.com", startedat: now - 5000, expiresat: now + 600_000, grants: ["https://example.com"] };
const plan = { id: "plan1", objective: "Run the reviewed tool calls", origin: "https://example.com", steps: [{ id: "s1", kind: "click" as const, target: "#go", summary: "Click the reviewed button", risk: "sensitive" as const }], createdat: now - 4000, expiresat: now + 600_000, state: "approved" as const };
...
describe("toolcalls rate limits", () => {
  ...
  it("keeps absent limits and budgets unbounded with no silent default", () => {
    ...
    const unbounded = applyratelimit({ limits: [{ clientid: "client1", windowms: 60_000, windowstartedat: now, used: 5 }], clientid: "client1", now });
    expect(unbounded.allowed).toBe(true);
    expect(unbounded.budget).toBeUndefined();
    const other = applyratelimit({ limits: [limit({ clientid: "client2", budget: 1, used: 1 })], clientid: "client1", now });
...
describe("toolcalls structured errors and retry hints", () => {
  ...
  it("maps executor failures onto structured errors with their retry hints", () => {
    ...
    const consent = maptoolerror({ failure: { code: "consentrefused", message: "The consent gates refused the tool call." } });
    ...
    expect(structurederrorof({ code: "ratelimited", message: "The budget is spent.", retryhint: "wait", retryafter: 4000 }).retryafter).toBe(4000);
  });
});

describe("toolcalls idempotency", () => {
  it("replays the stored result for a repeated key of the same client", () => {
    const records = recordidempotency({ records: [], key: "key-1", clientid: "client1", tool: "browser.readtext", result: { content: "the stored answer", iserror: false }, now, window: 60_000 });
    ...
    expect(checkidempotency({ records, key: "key-2", clientid: "client1", now }).reason).toMatch(/names no stored record/i);
    expect(checkidempotency({ records, key: "key-1", clientid: "client2", now: now + 1000 }).reason).toMatch(/another client/i);
  });

  it("expires the records past their window so the keys never replay", () => {
    const records = recordidempotency({ records: [], key: "key-1", clientid: "client1", tool: "browser.readtext", result: { content: "stored", iserror: false }, now, window: 5000 });
    expect(checkidempotency({ records, key: "key-1", clientid: "client1", now: now + 4999 }).replay).toBeDefined();
    expect(checkidempotency({ records, key: "key-1", clientid: "client1", now: now + 5000 }).reason).toMatch(/expired/i);
    const live: idempotencyrecord = { key: "key-2", clientid: "client1", tool: "memory.list", result: { content: "stored", iserror: false }, createdat: now, expiresat: now + 10_000 };
    expect(expireidempotency([records[0] as idempotencyrecord, live], now + 6000)).toEqual([live]);
    const overwritten = recordidempotency({ records, key: "key-1", clientid: "client1", tool: "browser.readtext", result: { content: "new answer", iserror: false }, now: now + 1000, window: 5000 });
...
describe("toolcalls batch execution", () => {
  ...
  it("runs the ordered members and stops on the first error when the flag requests it", async () => {
    const calls = [member("m1", "browser.readtext", { target: "#title" }), member("m2", "browser.readtext", { target: "#body" }), member("m3", "memory.list")];
    const order: string[] = [];
    const outcome = await runbatch({ calls, stoponerror: true, now, execute: async call => { order.push(call.id); return call.id === "m2" ? { ok: false, error: structurederrorof({ code: "internal", message: "The page executor timed out.", retryhint: "retry" }) } : { ok: true, result: { content: `ran ${call.name}`, iserror: false } }; } });
    ...
    const keepgoing = await runbatch({ calls, stoponerror: false, now, execute: async call => call.id === "m2" ? { ok: false } : { ok: true, result: { content: `ran ${call.name}`, iserror: false } } });
...
describe("toolcalls dry runs and mocks", () => {
  it("evaluates arguments and consent without side effects", () => {
    const tool = catalog.domains.flatMap(domain => domain.tools).find(candidate => candidate.name === "browser.click");
    expect(tool).toBeDefined();
    const dry = dryruntool({ tool: tool as never, params: { stepid: "s1" }, client: pairedclient, session, plan, origin: session.origin, now });
    expect(dry.argsvalid).toBe(true);
    expect(dry.consentok).toBe(true);
    expect(dry.executed).toBe(false);
    expect(dry.mutations).toEqual([]);
    const refused = dryruntool({ tool: tool as never, params: {}, client: { ...pairedclient, paired: false }, session, plan, origin: session.origin, now });
    ...
    const read = catalog.domains.flatMap(domain => domain.tools).find(candidate => candidate.name === "browser.readtext");
    const wrongtype = dryruntool({ tool: read as never, params: { target: 42 }, client: pairedclient, session, plan, origin: session.origin, now });
  ...
  it("answers with canned results in test contexts and never outside them", () => {
    const mock: toolmock = { tool: "browser.readtext", result: { content: "the canned read", iserror: false }, testcontext: true, createdat: now };
    ...
    const real: toolmock = { tool: "browser.click", result: { content: "canned click", iserror: false }, testcontext: false, createdat: now };
...
describe("toolcalls call contexts", () => {
  it("isolates concurrent client state and closes with the outcome", () => {
    ...
    const three = begincall({ clientid: "client1", tool: "browser.click", dryrun: true, batchid: "batch1", now: now + 2 });
    ...
    expect(begincall({ clientid: "client1", tool: "browser.readtext", idempotencykey: "  ", now }).idempotencykey).toBeUndefined();
    ...
    const failed = endcall({ contexts: closed.contexts, callid: two.callid, ok: false, errorcode: "internal", partial: { content: "partial", iserror: false }, now: now + 60 });
    ...
    expect(endcall({ contexts: closed.contexts, callid: one.callid, ok: true, now: now + 70 }).context?.state).toBe("done");
  });
});
