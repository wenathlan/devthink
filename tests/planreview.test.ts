...
const plan: agentplan = {
  ...
  steps: [
    ...
    { id: "s2", kind: "click", risk: "interaction", target: "#buy", summary: "Open the checkout." },
    {
      ...
      target: "#card",
      value: "vault:card1",
      summary: "Type the card number from the vault.",
    },
...
const corrections: correctionentry[] = [
  {
    ...
    stepid: "s3",
    source: "edited",
    original: "{kind:type,value:plain}",
    corrected: "{kind:type,value:vault:card1}",
    reason: "The vault marker replaced the raw value.",
    at: now - 1000,
  },
  {
    id: "c2",
    ...
    source: "rejected",
    original: "{kind:click}",
    reason: "Other origin never matches.",
    at: now - 1000,
  },
];
...
describe("plan review surface", () => {
  ...
  it("resolves one step per distinct human action with approve, reject and edit provenance", () => {
    ...
    const edit = stepresolutionof({
      ...
      resolution: "edit",
      surface: "sidepanel",
      edited: "{kind:type,value:vault:card1}",
      at: now,
    });
  ...
  it("binds each stepapprove action to one step with no batch approval and no background provenance", () => {
    ...
    expect(stepapprovegate({ stepids: ["s2", "s3"], resolution: "approve", surface: "sidepanel" }).reason).toMatch(
      /batch of 2/,
    );
    expect(stepapprovegate({ stepids: [], resolution: "approve", surface: "sidepanel" }).allowed).toBe(false);
    expect(stepapprovegate({ stepids: ["s2"], resolution: "reject", surface: "background" }).reason).toMatch(
  ...
  it("compares the observed before state with the predicted after state of write class steps only", () => {
    ...
    const preview = diffpreviewof({
      stepid: "s3",
      before: { card: "vault:card1", holder: "Ada", city: "Lisbon" },
      after: { card: "vault:card1", holder: "Ada Lovelace", zip: "1000" },
      maskverdicts: { card: "The card value stays masked (11 characters) and never renders in the clear." },
      provenance: "inline",
    });
    expect(preview.changes.map((change) => `${change.kind}:${change.field}`).sort()).toEqual([
      "added:zip",
      "changed:holder",
      "removed:city",
    ]);
  ...
  it("derives the stepstimeline nodes from progress records with no new state", () => {
    const progress: planprogress = {
      ...
      outcomes: [{ stepid: "s2", ok: true, summary: "Checkout opened.", at: now - 500 }],
      environments: { s1: "offscreenworker" },
      turnarounds: { s1: 120 },
      gatewaits: { s3: { gateid: "g1", kind: "confirmpay", openedat: now - 100, resolvedat: now, waitedms: 100 } },
      updatedat: now,
    };
    ...
    expect(stepstimelinenodes({ plan: cancelled, now }).map((node) => node.status)).toEqual([
      "halted",
      "halted",
      "halted",
      "halted",
    ...
    const failed: planprogress = {
      planid: plan.id,
      completedsteps: [],
      outcomes: [{ stepid: "s1", ok: false, summary: "The table never loaded.", at: now }],
      updatedat: now,
    };
  ...
  it("appends logstream events behind the hash chain with level, source and step ref", async () => {
    const first = await logstreameventof({
      ...
      masked: false,
      maskverdict: "The source payload carries no masked value.",
    ...
    const second = await logstreameventof({
      ...
      source: "background",
      origin: plan.origin,
      summary: "The step waited at its gate.",
      stepid: "s3",
      masked: true,
      maskverdict: "The card value stayed masked.",
  ...
  it("copies a verified range as an audit excerpt and refuses an unverified chain", async () => {
    ...
    const second = await logstreameventof({
      ...
      stepid: "s1",
      masked: true,
      maskverdict: "The typed value stayed masked.",
      previous: first.hash.current,
      at: now + 1,
    });
