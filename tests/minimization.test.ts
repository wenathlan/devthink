...
import { cleanupafterrun, enforcequarantine, encryptsync, exportall, fixedtelemetrypolicy, jarscope, localfieldsof, localfirst, newjar, optinsync, purgeonrequest, sealjar, streambundle, syncpass, type keyderive, type payloadencrypt, type syncsend } from "../export.js";
import { cleanupgate, encryptsyncgate, exportallgate, jargate, localgate, minimizationstripgrade, notelemetryinvariant, purgegate, quarantineopengate, syncgate } from "../policy.js";
import { outboundpayloadcheck, quarantineverdictreport, requestbody, syncconsentreport, syncpayloadreport } from "../protocol.js";
...
describe("minimization localfirst stripping and localrule blocking", () => {
  it("keeps the extraction on the device, strips the identity fields the review never listed and holds the local rule fields from every outbound payload", () => {
    const rules: localrule[] = [{ origin: "https://shop.example", fields: ["sessiontoken", "cartsecret"], at: now - 1000 }];
    const pass = localfirst({ origin: "https://shop.example", rows: [{ title: "pricing table", email: "buyer@example.org", userid: "u-17", sessiontoken: "st-abc", total: "42" }, { title: "shipping table", phone: "+1 555 0100", total: "7" }], rules, now });
    expect(pass.rows).toEqual([{ title: "pricing table", total: "42" }, { title: "shipping table", total: "7" }]);
    ...
    const reviewed = localfirst({ origin: "https://shop.example", rows: [{ email: "buyer@example.org", total: "42" }], reviewedfields: ["email", "total"], rules, now });
    expect(reviewed.rows).toEqual([{ email: "buyer@example.org", total: "42" }]);
    expect(reviewed.stripped).toEqual([]);
    expect(localfieldsof({ sessiontoken: "x", total: "42" } as Record<string, unknown>, rules)).toEqual(["sessiontoken"]);
    ...
    expect(() => outboundpayloadcheck({ payload: { sessiontoken: "x" }, localfields: ["sessiontoken"] })).toThrow(/never leaves the device/);
...
describe("minimization notelemetry invariant with zero outbound calls", () => {
  it("keeps every counter inside the local memory while the worker issues no outbound usage call at all", async () => {
    ...
    let outboundcalls = 0;
    const counting: syncsend = async () => { outboundcalls += 1; return { delivered: false }; };
    ...
    const wire = JSON.parse(requestbody({ objective: "collect the pricing table", session: { id: "s1", tabid: 1, origin: "https://shop.example", startedat: now, expiresat: now + 1000, jarid: "jar-1" }, observation: { schemaversion: 1, url: "https://shop.example", title: "Shop", textpreview: "", textlength: 0, forms: [], interactive: [], capturedat: now }, capabilities: { tabs: true, downloads: true, clipboardread: false, clipboardwrite: false, reportedat: now } })) as Record<string, unknown>;
...
describe("minimization optinsync consent and encryptsync refusal", () => {
  it("stays disabled until the user opts each data class in, records the consent per class and refuses the sync without the passphrase", async () => {
    ...
    expect(() => optinsync({ offered, enable: ["telemetry"], now })).toThrow(/never enables a class the listing did not offer/);
    ...
    expect(() => syncconsentreport({ consent: [{ dataclass: "runs", at: now }, { dataclass: "Runs", at: now }] })).toThrow(/one consent stamp/);
    await expect(encryptsync({ payload: "{\"runs\":[]}", derive, encrypt })).rejects.toThrow(/needs the user passphrase/);
    const encrypted = await encryptsync({ payload: "{\"runs\":[]}", passphrase: "correct horse battery staple", derive, encrypt });
    ...
    await expect(syncpass({ payload: "{\"captures\":[]}", classes: ["settings"], settings, derive, encrypt, send: async () => ({ delivered: true }), now })).rejects.toThrow(/never opted in/);
    const delivered = await syncpass({ payload: "{\"runs\":[]}", classes: ["runs"], settings, passphrase: "correct horse battery staple", derive, encrypt, send: async () => ({ delivered: true, endpoint: "https://sync.example" }), now });
    ...
    const envelope = syncpayloadreport({ classes: delivered.record.classes, payloadhash: delivered.record.payloadhash, formattag: delivered.record.formattag, encrypted: delivered.record.encrypted, syncedat: delivered.record.syncedat });
    expect(envelope.encrypted).toBe(true);
    expect(() => syncpayloadreport({ classes: ["runs"], payloadhash: "h", formattag: "devthink-sync-1", encrypted: false, syncedat: now })).toThrow(/plaintext/);
  });
});
...
describe("minimization purgeonrequest scopes and cleanup retention", () => {
  it("deletes the stored keys by scope on the typed confirmation while the audit hashes survive and the retained artifacts stay", () => {
    ...
    const policy: purgepolicy = { scope: ["runs", "memory", "captures", "settings", "provenance"], confirmation: "purge everything", at: now - 2000 };
    expect(() => purgeonrequest({ policy, typed: "purge everythin", inventory, now })).toThrow(/typed confirmation/);
    expect(purgegate({ scope: policy.scope, confirmation: policy.confirmation, typed: "purge everything" }).allowed).toBe(true);
    ...
    const partial = purgeonrequest({ policy: { scope: ["captures"], confirmation: "purge everything", at: now }, typed: "", inventory, now });
    expect(partial.deleted).toEqual(["captures"]);
    expect(partial.kept.sort()).toEqual(["audit", "memoryitems", "provlog:run1", "runs", "settings"]);
    expect(() => purgeonrequest({ policy: { scope: [], confirmation: "x", at: now }, typed: "", inventory, now })).toThrow(/names its scope/);
    const artifacts = [
    ...
    expect(() => cleanupafterrun({ runid: "run1", artifacts, classes: ["audit"], now })).toThrow(/never deletes the audit history/);
...
describe("minimization exportall completeness and streaming", () => {
  it("bundles every stored record into one portable file and streams the chunks without a size cap", () => {
    ...
    const bundle = exportall({ runs: ["run1", "run2"], memory: ["mem1"], captures: ["cap1", "cap2", "cap3"], settings: true, provenance: ["prov1", "prov2"], now });
    ...
    expect(() => exportall({ runs: [], memory: [], captures: [], settings: false, provenance: [], now })).toThrow(/exports nothing/);
  });
});
...
describe("minimization cookiejar sealing and enforcequarantine verdicts", () => {
  it("assigns one jar per run, scopes the cookie read and write to the active jar, seals it at the completion and releases or deletes the quarantined files by the verdict alone", () => {
    ...
    const written = jarscope({ jar, operation: "write", cookies: [{ name: "session", domain: "shop.example", value: "abc" }, { name: "cart", domain: "shop.example", value: "2" }], now: now + 100 });
    expect(written.jar.cookies).toHaveLength(2);
    const rewritten = jarscope({ jar: written.jar, operation: "write", cookies: [{ name: "cart", domain: "shop.example", value: "3" }], now: now + 200 });
    ...
    expect(() => jarscope({ jar: sealed, operation: "write", cookies: [{ name: "late", domain: "shop.example", value: "x" }], now: now + 500 })).toThrow(/sealed jar refuses every write/);
    ...
    const held = enforcequarantine({ path: "/sandbox/invoice.pdf", reason: "download moved out of the downloads folder", now });
    expect(held.entry.scan).toBe("pending");
    expect(held.entry.status).toBe("held");
    expect(held.action).toBe("hold");
    const clean = enforcequarantine({ path: "/sandbox/invoice.pdf", reason: "download moved out of the downloads folder", scan: "clean", now });
    expect(clean.action).toBe("release");
    expect(clean.entry.status).toBe("released");
    const flagged = enforcequarantine({ path: "/sandbox/invoice.pdf", reason: "download moved out of the downloads folder", scan: "flagged", now });
    ...
    const verdictreport = quarantineverdictreport({ entry: { id: clean.entry.id, path: clean.entry.path, reason: clean.entry.reason, scan: clean.entry.scan, status: clean.entry.status ?? "held", at: clean.entry.at, updatedat: clean.entry.updatedat } });
    expect(verdictreport.released).toBe(true);
    expect(() => quarantineverdictreport({ entry: { id: "q1", path: "/sandbox/x", reason: "r", scan: "flagged", status: "released", at: now, updatedat: now } })).toThrow(/Only a clean scanner verdict releases/);
  });
});
