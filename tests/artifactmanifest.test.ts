...
describe("artifactmanifest", () => {
  it("records the name, the size and the sha256 checksum of every artifact with its channels", () => {
    const manifest = artifactmanifestof({
      version: "1.1.87",
      artifacts: [{ name: "devthink1.1.87.zip", size: 1024, checksum: valid }],
    });
  ...
  it("sorts the artifacts by name so the document renders deterministically", () => {
    ...
    expect(manifest.artifacts.map((entry) => entry.name)).toEqual([
      "devthink-site-1.1.87.zip",
      "devthink-vscode-1.1.87.vsix",
      "devthink1.1.87.zip",
    ]);
    /* the text of the same set renders byte identical on every run because no timestamp enters the document */
    expect(artifactmanifesttext(manifest)).toBe(
      artifactmanifesttext(
        artifactmanifestof({
          version: "1.1.87",
          artifacts: [
  ...
  it("refuses an empty artifact set, a duplicate name and a non sha256 checksum", () => {
    ...
    expect(() =>
      artifactmanifestof({ version: "1.1.87", artifacts: [{ name: "a.zip", size: 1, checksum: "short" }] }),
    ).toThrow();
    expect(() =>
      artifactmanifestof({ version: "1.1.87", artifacts: [{ name: "", size: 1, checksum: valid }] }),
    ).toThrow();
    expect(() =>
      artifactmanifestof({ version: "not-a-version", artifacts: [{ name: "a.zip", size: 1, checksum: valid }] }),
    ).toThrow();
  });

  it("resolves the publishing channels of the release artifact names", () => {
    expect(artifactchannels()).toEqual([
      "github",
      "npmjs",
      "githubpackages",
      "nuget",
  ...
  it("verifies a built or downloaded set matches the manifest exactly", () => {
    ...
    const okcheck = artifactmanifestcheck({
      manifest,
      files: [
    ...
    const missing = artifactmanifestcheck({
      manifest,
      files: [{ name: "devthink-site-1.1.87.zip", checksum: checksumof("two") }],
    });
    expect(missing.ok).toBe(false);
    expect(missing.missing).toEqual(["devthink1.1.87.zip"]);
    const mismatched = artifactmanifestcheck({
      manifest,
      files: [
        { name: "devthink1.1.87.zip", checksum: checksumof("changed") },
        { name: "devthink-site-1.1.87.zip", checksum: checksumof("two") },
      ],
    });
    expect(mismatched.ok).toBe(false);
    expect(mismatched.mismatched).toEqual(["devthink1.1.87.zip"]);
    const unexpected = artifactmanifestcheck({
      manifest,
      files: [
        { name: "devthink1.1.87.zip", checksum: checksumof("one") },
        { name: "devthink-site-1.1.87.zip", checksum: checksumof("two") },
    ...
    const withsums = artifactmanifestcheck({
      manifest,
      files: [
        { name: "devthink1.1.87.zip", checksum: checksumof("one") },
        { name: "devthink-site-1.1.87.zip", checksum: checksumof("two") },
  ...
  it("freezes the rollback pin of the previous release artifact set", () => {
    const previous = artifactmanifestof({
      version: "1.1.86",
      artifacts: [{ name: "devthink1.1.86.zip", size: 10, checksum: checksumof("previous") }],
    });
    ...
    expect(pin.artifacts).toEqual([
      {
        name: "devthink1.1.86.zip",
