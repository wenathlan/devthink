...
describe("torture: fleet naming discipline", () => {
  it("refuses reserved words, duplicates, invalid identifiers and blanks", () => {
    for (const reserved of ["user", "operator", "human", "system"]) {
      expect(() =>
        agentname({ records: [], id: "a1", proposed: reserved, origin: "https://example.com", now }),
      ).toThrow(/reserved/i);
    }
    expect(() =>
      agentname({
        records: [record({ name: "scout" })],
        id: "a2",
        proposed: "Scout",
        origin: "https://example.com",
        now,
    ...
    for (const invalid of ["", " ", "1abc", "-abc", "a b", "a-b", "a.b", "über", "abc!", "日本", "a_b"]) {
      expect(() =>
        agentname({ records: [], id: "a1", proposed: invalid, origin: "https://example.com", now }),
      ).toThrow();
    }
    expect(agentname({ records: [], id: "a1", proposed: "A", origin: "https://example.com", now }).name).toBe("a");
    expect(() => agentname({ records: [], id: "", proposed: "scout", origin: "https://example.com", now })).toThrow(
      /needs its id/i,
    );
  ...
  it("lowercases and trims the proposal before validating", () => {
    ...
    expect(
      agentname({ records: [], id: "a1", proposed: "scout", role: "reviewer", origin: "https://example.com", now })
        .role,
    ).toBe("reviewer");
  });
  ...
  it("accepts long single token identifiers and digits after the first letter", () => {
    ...
    expect(agentname({ records: [], id: "a1", proposed: longname, origin: "https://example.com", now }).name).toBe(
      longname,
    );
    expect(agentname({ records: [], id: "a1", proposed: "agent42", origin: "https://example.com", now }).name).toBe(
      "agent42",
    );
  });
});
...
describe("torture: scope intersection and narrowing", () => {
  it("refuses a request that lands entirely outside the grants", () => {
    expect(() =>
      agentscopeof({ agentid: "a1", requested: { origins: ["https://evil.com"] }, grants: ["https://example.com"] }),
    ).toThrow(/never widens/i);
    expect(() =>
      agentscopeof({ agentid: "a1", requested: { actionkinds: ["nuke"] }, allowedkinds: ["readtext", "click"] }),
    ).toThrow(/allowed catalog/i);
  ...
  it("narrows partial requests while keeping the granted intersection", () => {
    const scope = agentscopeof({
      agentid: "a1",
      requested: { origins: ["https://example.com", "https://evil.com"], actionkinds: ["readtext", "nuke"] },
      grants: ["https://example.com", "https://shop.example"],
      allowedkinds: ["readtext", "click"],
    });
  ...
  it("keeps whitespace and empty entries out of every scope list", () => {
    const scope = agentscopeof({
      agentid: "a1",
      requested: { origins: ["", "  ", "https://example.com"], actionkinds: ["", "  ", "readtext"] },
    });
    expect(scope.origins).toEqual(["https://example.com"]);
    expect(scope.actionkinds).toEqual(["readtext"]);
  });
  ...
  it("lookalike origins never intersect a grant by suffix or subdomain tricks", () => {
    expect(() =>
      agentscopeof({
        agentid: "a1",
        requested: { origins: ["https://example.com.evil.com"] },
    ...
    expect(() =>
      agentscopeof({
        agentid: "a1",
        requested: { origins: ["https://evil.com/example.com"] },
        grants: ["https://example.com"],
      }),
    ).toThrow(/never widens/i);
    const narrowed = agentscopeof({
      agentid: "a1",
      requested: { origins: ["https://example.com", "http://example.com", "https://example.com.evil.com"] },
      grants: ["https://example.com"],
    });
...
describe("torture: pause, unpause and the killswitch", () => {
    ...
    const queues: taskqueue[] = [
      {
        ...
        items: [{ id: "t1", lane: "default", priority: 1, payload: "work", state: "queued", enqueuedat: now }],
  ...
  it("fleet overview counts every control state exactly", () => {
    const fleet = [
      record({ id: "a1" }),
      record({ id: "a2", state: "paused" }),
      record({ id: "a3", state: "stopped" }),
      record({ id: "a4", state: "stopped" }),
    ];
...
describe("torture: mailbox routing and delivery", () => {
  it("direct messages refuse unknown recipients and self addressing", () => {
    ...
    ).toThrow(/needs its payload/i);
    expect(() => resolverecipients({ agents, senderid: "a1", recipient: "a3", routing: "direct" })).toThrow(
      /not registered/i,
    );
  });
  ...
  it("role routing reaches only the live agents of that role", () => {
    const agents = [
      identity("a1", { role: "worker" }),
      identity("a2", { role: "reviewer" }),
      identity("a3", { role: "worker", state: "stopped" }),
    ];
  ...
  it("receives drain resets the unread counter, marks read times and keeps read state on re-drain", () => {
    const agents = [identity("a1"), identity("a2")];
    let mailboxes: agentmailbox[] = sendmessage({
      mailboxes: [],
      agents,
      id: "m1",
      senderid: "a1",
...
describe("torture: escalation holds and review verdicts", () => {
    ...
    const escalations: escalationrecord[] = [
      {
        id: "e1",
        agentid: "a1",
        subject: "payment flow needs a human",
        state: "open",
        raisedat: now,
        context: "the checkout needs consent",
      },
      {
        id: "e2",
        agentid: "a2",
  ...
  it("review requests refuse self review, blanks and unknown reviewers", () => {
    expect(() =>
      reviewrecordof({ id: "", fromagentid: "a1", toagentid: "a2", subject: "s", output: "o", now }),
    ).toThrow(/needs its id/i);
    expect(() =>
      reviewrecordof({ id: "r1", fromagentid: "a1", toagentid: "a1", subject: "s", output: "o", now }),
    ).toThrow(/never reviews its own/i);
    expect(() =>
      reviewrecordof({ id: "r1", fromagentid: "a1", toagentid: "a2", subject: " ", output: "o", now }),
    ).toThrow(/subject/i);
    expect(() =>
      reviewrecordof({ id: "r1", fromagentid: "a1", toagentid: "a2", subject: "s", output: "", now }),
    ).toThrow(/original output/i);
    const open = reviewrecordof({
      id: "r1",
      fromagentid: "a1",
      toagentid: "a2",
      subject: "verify the totals",
      output: "42",
      now,
    });
    expect(open.state).toBe("open");
    expect(() => recordverdict({ records: [open], id: "r2", reviewerid: "a2", verdict: "approve", now })).toThrow(
      /does not exist/i,
    );
    expect(() => recordverdict({ records: [open], id: "r1", reviewerid: "a3", verdict: "approve", now })).toThrow(
      /never answers a review addressed to another agent/i,
    );
    expect(() => recordverdict({ records: [open], id: "r1", reviewerid: "a2", verdict: "reject", now })).toThrow(
      /names its issues/i,
    );
  });
  ...
  it("verdicts answer once while rejections demand their issues", () => {
    ...
    expect(() =>
      recordverdict({ records: [open], id: "r1", reviewerid: "a2", verdict: "reject", now: now + 1 }),
    ).toThrow(/names its issues/i);
    expect(() =>
      recordverdict({ records: [open], id: "r1", reviewerid: "a2", verdict: "changes", now: now + 1 }),
    ).toThrow(/names its issues/i);
    const answered = recordverdict({
      ...
      reviewerid: "a2",
      verdict: "reject",
      issues: ["the totals miss the tax row"],
      now: now + 1,
    });
    ...
    expect(() =>
      recordverdict({ records: [answered], id: "r1", reviewerid: "a2", verdict: "approve", now: now + 2 }),
    ).toThrow(/never rewrites/i);
    const approved = recordverdict({
      records: [reviewrecordof({ id: "r2", fromagentid: "a1", toagentid: "a2", subject: "s", output: "o", now })],
      id: "r2",
      reviewerid: "a2",
      verdict: "approve",
      now: now + 1,
...
describe("torture: output comparison", () => {
  it("grades matching, conflicting and missing fields with sorted keys", () => {
    const comparison = outputcompare({
      id: "c1",
      subject: "the pricing table",
      left: { agentid: "a1", fields: { price: "42", tax: "8", note: "left only" } },
      right: { agentid: "a2", fields: { price: "42", tax: "9", extra: "right only" } },
      now,
    });
  ...
  it("refuses blanks, self comparison and unicode keys sort deterministically", () => {
    expect(() =>
      outputcompare({
        id: "",
        subject: "s",
        left: { agentid: "a1", fields: {} },
        right: { agentid: "a2", fields: {} },
        now,
      }),
    ).toThrow(/needs its id/i);
    expect(() =>
    ...
    expect(() =>
      outputcompare({
        ...
        subject: "s",
        left: { agentid: "a1", fields: {} },
        right: { agentid: "a1", fields: {} },
        now,
      }),
    ).toThrow(/never competes with itself/i);
    const comparison = outputcompare({
      id: "c1",
      subject: "s",
      left: { agentid: "a1", fields: { b: "1", a: "2", é: "3" } },
      right: { agentid: "a2", fields: { b: "1", a: "9", é: "3" } },
      now,
    });
...
describe("torture: consensus voting", () => {
  it("refuses blank ids, proposals, quorums and votes", () => {
    ...
    expect(() => consensusrecordof({ id: "c1", proposal: "p", votes: [], quorum: 0, now })).toThrow(
      /positive whole number/i,
    );
    expect(() => consensusrecordof({ id: "c1", proposal: "p", votes: [], quorum: 1.5, now })).toThrow(
      /positive whole number/i,
    );
    expect(() => consensusrecordof({ id: "c1", proposal: "p", votes: [], quorum: -1, now })).toThrow(
      /positive whole number/i,
    );
    expect(() =>
      consensusrecordof({
        id: "c1",
        proposal: "p",
        votes: [{ agentid: "", vote: "yes", castat: now }],
        quorum: 1,
        now,
      }),
    ).toThrow(/names its agent/i);
  });
  ...
  it("duplicate voters refuse loudly", () => {
    ...
    expect(() => consensusrecordof({ id: "c1", proposal: "adopt the plan", votes, quorum: 1, now })).toThrow(
      /one per agent/i,
    );
  });

  it("carries at exactly the quorum, fails only when complete and stays open otherwise", () => {
    const twoyes = consensusrecordof({
      id: "c1",
      proposal: "p",
      votes: [
        { agentid: "a1", vote: "yes", castat: now },
        { agentid: "a2", vote: "yes", castat: now },
      ],
      quorum: 2,
      now,
    });
    ...
    const oneyes = consensusrecordof({
      id: "c1",
      proposal: "p",
    ...
    const incomplete = consensusrecordof({
      id: "c1",
      proposal: "p",
      votes: [{ agentid: "a1", vote: "yes", castat: now }],
      quorum: 2,
    ...
    const complete = consensusrecordof({
      id: "c1",
      proposal: "p",
      votes: [
        { agentid: "a1", vote: "yes", castat: now },
        { agentid: "a2", vote: "no", castat: now },
      ],
      quorum: 2,
      voters: 2,
      now,
    ...
    const abstaincarried = consensusrecordof({
      id: "c1",
      proposal: "p",
      votes: [
        { agentid: "a1", vote: "abstain", castat: now },
        { agentid: "a2", vote: "yes", castat: now },
      ],
      quorum: 1,
      voters: 2,
