...
import { agentcontrolsof, aggregatereportdownloadof, conflictlogof, costperagentof, dashboardemptyguidanceof, dashboardviewerof, escalationinboxof, messageflowof, multiagentoverviewof, multiagentviewof, queuelaneviewof, timelinescrubof, topologyinventoryof } from "../dashdone.js";
import type { agentidentity, agentmailbox, agentusage, conflictscan, escalationrecord, killswitch, leaderworker, resourcelock, resultreport, taskqueue, swarmaction, swarmcost } from "../types.js";
...
function queue(): taskqueue {
  return { lanes: ["extraction", "indexing"], priorities: [2, 1], completionpolicy: "all", items: [
    { id: "t1", lane: "extraction", priority: 2, payload: "Read the pricing table", state: "claimed", enqueuedat: now },
    { id: "t2", lane: "indexing", priority: 1, payload: "Index the footer links", state: "queued", enqueuedat: now + 1 },
    { id: "t3", lane: "extraction", priority: 1, payload: "Read the hero", state: "done", enqueuedat: now + 2 },
  ], claims: [{ agentid: "w1", taskid: "t1", claimedat: now, heartbeatat: now }, { agentid: "w1", taskid: "t3", claimedat: now + 2, heartbeatat: now + 3 }] };
}

/** Builds the mailboxes of the fleet with one unread direct message. */
function mailboxes(): agentmailbox[] {
  return [
    { agentid: "w1", inbox: [{ id: "m1", senderid: "a1", recipient: "w1", routing: "direct", payload: "Start with the table.", sentat: now + 1 }], outbox: [], unread: 1 },
    { agentid: "a1", inbox: [], outbox: [{ id: "m1", senderid: "a1", recipient: "w1", routing: "direct", payload: "Start with the table.", sentat: now + 1, readat: now + 2 }], unread: 0 },
  ];
}
...
describe("the multi agent dashboard view panels", () => {
  it("shapes the overview panel with the topology status, the state counts and the killswitch", () => {
    const topology: leaderworker = { id: "top1", leaderid: "a1", workerids: ["w1", "w2"], criticids: ["c1"], verifierids: ["v1"], assignments: [], rule: { kind: "first" }, electedat: now };
    const overview = multiagentoverviewof({ agents: fleet(), queue: queue(), topology, killswitch: { engaged: true, engagedat: now, reason: "The user halted the swarm." }, unread: 3 });
    ...
    const bare = multiagentoverviewof({ agents: [], queue: { lanes: [], priorities: [], completionpolicy: "all", items: [], claims: [] }, killswitch: { engaged: false }, unread: 0 });
  ...
  it("shapes the per agent status cards with the live progress and the usage counters", () => {
    const view = multiagentviewof({ agents: fleet(), queue: queue(), mailboxes: mailboxes(), killswitch: { engaged: false }, usage: [{ agentid: "w1", tokens: 280, cost: 0.028, steps: 2, updatedat: now }], costs: [], conflicts: [], locks: [], escalations: [], timeline: [], now });
    const scribe = view.cards.find(card => card.agentid === "w1");
    expect(scribe).toMatchObject({ name: "Scribe", role: "worker", state: "active", tabid: 102, depth: 0, currenttask: "Read the pricing table", claimed: 1, done: 1, tokens: 280, cost: 0.028, steps: 2, unread: 1 });
  ...
  it("shapes the shared queue view with the lane filter and the empty lane state", () => {
    const view = multiagentviewof({ agents: fleet(), queue: queue(), mailboxes: mailboxes(), killswitch: { engaged: false }, usage: [], costs: [], conflicts: [], locks: [], escalations: [], timeline: [], now });
    expect(view.queueview.lanes).toEqual(["extraction", "indexing"]);
    expect(view.queueview.rows).toHaveLength(3);
    expect(view.queueview.rows[0]).toMatchObject({ taskid: "t1", lane: "extraction", priority: 2, state: "claimed", holder: "w1" });
    ...
    const emptylane = queuelaneviewof({ queue: { lanes: ["extraction"], priorities: [1], completionpolicy: "all", items: [], claims: [] }, lane: "extraction" });
  ...
  it("shapes the message flow view as one time ordered stream between the agents", () => {
    ...
    expect(flow.rows[0]).toMatchObject({ messageid: "m1", senderid: "a1", recipient: "w1", routing: "direct inbox", payload: "Start with the table.", read: false });
  ...
  it("shapes the conflict and arbitration log from the scans and the live locks", () => {
    const scan: conflictscan = { id: "scan1", writers: [{ agentid: "w1", origin: "https://example.com", selector: "#submit" }, { agentid: "w2", origin: "https://example.com", selector: "#submit" }], overlaps: [{ origin: "https://example.com", selector: "#submit", writers: ["w1", "w2"] }], suggestedorder: ["w1", "w2"], clean: false, scannedat: now };
    const lock: resourcelock = { key: "https://example.com|#submit", holder: "w1", kind: "exclusive", origin: "https://example.com", selector: "#submit", acquiredat: now + 1 };
    ...
    expect(log.rows[1]).toMatchObject({ source: "conflict", key: "https://example.com|#submit", writers: ["w1", "w2"] });
    const clean = conflictlogof({ conflicts: [{ id: "scan2", writers: [{ agentid: "w1", origin: "https://example.com", selector: "#a" }], overlaps: [], suggestedorder: [], clean: true, scannedat: now }], locks: [] });
  ...
  it("shapes the cost per agent panel from the accounting the costcert totals feed", () => {
    ...
    expect(costview.rows[0]).toMatchObject({ tokens: 280, cost: 0.028, steps: 2, share: Math.round((280 / 370) * 100) / 100 });
  ...
  it("shapes the escalation inbox with the open count and the decided rows", () => {
    const escalations: escalationrecord[] = [
      { id: "e1", agentid: "w2", subject: "The paywall blocks the footer.", context: "The read needs the user decision.", state: "open", raisedat: now },
      { id: "e2", agentid: "w1", subject: "Two rows disagree.", context: "The merge needs the policy order.", state: "decided", decision: "Keep the re-read count.", raisedat: now - 1, decidedat: now },
    ];
  ...
  it("shapes the timeline scrubber over the interleaved events with the positions", () => {
    const actions: swarmaction[] = [
      ...
      { id: "x1", kind: "click", agentid: "w1", summary: "The interactive agent opened the pricing page.", at: now + 100 },
    ];
  ...
  it("shapes the aggregate report download and its empty state", () => {
    const report: resultreport = { id: "rep1", title: "The pricing table read", sections: [{ title: "rowcount", entries: [{ id: "m1", agentid: "w1", taskid: "t1", key: "rowcount", value: "14", mergedat: now }], sources: ["w1"] }], sources: ["w1", "w2"], confidence: "The verifier re-read both counts.", createdat: now };
  ...
  it("shapes the per agent controls the viewer role and the killswitch gate", () => {
    ...
    expect(agentcontrolsof({ agent: active, viewer: { readonly: false }, killswitch: { engaged: false } })).toMatchObject({ pause: true, resume: false, stop: true, killswitch: true });
    const paused = fleet()[2]!;
    expect(agentcontrolsof({ agent: paused, viewer: { readonly: false }, killswitch: { engaged: false } })).toMatchObject({ pause: false, resume: true, stop: true });
    expect(agentcontrolsof({ agent: active, viewer: { readonly: true }, killswitch: { engaged: false } }).reason).toMatch(/no run role/);
    expect(agentcontrolsof({ agent: active, viewer: { readonly: false }, killswitch: { engaged: true, engagedat: now } }).reason).toMatch(/kill switch stays engaged/);
  });
  ...
  it("shapes the full multi agent view with every panel in one call", () => {
    ...
    const view = multiagentviewof({
      ...
      conflicts: [],
      locks: [],
      escalations: [{ id: "e1", agentid: "w2", subject: "The paywall blocks the footer.", context: "The read needs the user decision.", state: "open", raisedat: now }],
      timeline: [{ id: "x1", kind: "click", agentid: "w1", summary: "The interactive agent opened the pricing page.", at: now }],
      report: { id: "rep1", title: "The pricing table read", sections: [{ title: "rowcount", entries: [{ id: "m1", agentid: "w1", taskid: "t1", key: "rowcount", value: "14", mergedat: now }], sources: ["w1"] }], sources: ["w1"], createdat: now },
...
describe("the dashboard panel resize of the 2.0.2 final polish", () => {
  ...
  it("round trips the panelwidths preference through the surface layout seam and clamps the stored row", async () => {
    ...
    await store.setsurfacelayout({ surface: "dashboardpage", preferences: { panelwidths: panelwidthslayout(widths) }, updatedat: now });
    ...
    expect(dashboardpage).toContain("const move = resizepanel({ widths: dragstartwidths, delta: event.clientX - dragstartx,");
    expect(dashboardpage).toContain("const preferences = { ...(layout.layout?.preferences ?? {}), panelwidths: panelwidthslayout(panelwidths) };");
    const design = await readFile("web/extension/index.html", "utf8");
    expect(design).toContain('<div class="panelresizer" id="panelresizer" role="separator" aria-orientation="vertical"');
    expect(design).toContain('aria-valuemin="280" aria-valuemax="720"');
    for (const panel of ["multiagentoverview", "agentstatuscards", "sharedqueueview", "queuelanefilter", "messageflow", "conflictlog", "agentcostpanel", "escalationinbox", "timelinescrubber", "aggregatetimeline", "killswitch", "reportdownload", "sessiongrid", "flowlibrary", "backgroundruns", "historyhits", "sitenotes", "transparency", "onboarding", "perfsummary", "perfchart", "budgetalerts", "selectorprofile", "dropzone", "historyquery", "libraryquery"]) {
