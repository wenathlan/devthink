...
describe("sandbox frame", () => {
  ...
  it("builds the sandbox render descriptor with its nonce, source origin and sanitized markup", () => {
    ...
    expect(() =>
      sandboxrenderof({ id: "r2", markup: " ", sourceorigin: "https://example.com", stepid: "s1", now }),
    ).toThrow(/markup/i);
    expect(() => sandboxrenderof({ id: "r3", markup: "<p/>", sourceorigin: " ", stepid: "s1", now })).toThrow(
      /source origin/i,
    );
    expect(() =>
      sandboxrenderof({ id: "r4", markup: "<p/>", sourceorigin: "https://example.com", stepid: " ", now }),
    ).toThrow(/step/i);
  });
  ...
  it("posts the render through the devthinksandbox channel and accepts the nonce checked answer", () => {
    ...
    const accepted = acceptrenderresult({
      ...
      message: {
        ...
        nonce: render.nonce,
        ok: true,
        text: "<p>hello</p> inert text",
        summary: "The frame answered.",
      },
      now: now + 10,
    });
  ...
  it("refuses stale, off channel, unknown nonce and replayed sandbox messages so a render result never reenters the dom outside the frame", () => {
    ...
    expect(
      acceptrenderresult({
        renders: [render],
        message: { channel: "other", type: "renderresult", nonce: render.nonce },
        now: now + 10,
      }).accepted,
    ).toBe(false);
    expect(
      acceptrenderresult({
        renders: [render],
        message: { channel: "devthinksandbox", type: "other", nonce: render.nonce },
        now: now + 10,
      }).accepted,
    ).toBe(false);
    expect(
      acceptrenderresult({
        renders: [render],
        message: { channel: "devthinksandbox", type: "renderresult", nonce: "unknown000000000" },
        now: now + 10,
      }).reason,
    ).toMatch(/no nonce of a known unanswered render/i);
    const early = acceptrenderresult({
      renders: [render],
      message: {
        channel: "devthinksandbox",
        type: "renderresult",
    ...
    const plain = acceptrenderresult({
      renders: [render],
      message: {
        channel: "devthinksandbox",
        type: "renderresult",
    ...
    const replayed = acceptrenderresult({
      renders: plain.renders,
      message: {
        channel: "devthinksandbox",
        type: "renderresult",
        nonce: render.nonce,
    ...
    const failed = acceptrenderresult({
      renders: [second],
      message: {
        channel: "devthinksandbox",
        type: "renderresult",
  ...
  it("records the provenance of every sandbox render with its source origin and nonce", () => {
    ...
    expect(renderprovenance(render)).toEqual({
