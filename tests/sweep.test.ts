...
describe("the release candidate defect sweep artifact", () => {
  it("declares every check family the gate runs over the trails, the sources and the runtime", async () => {
    ...
    for (const family of ["audittrails", "todoentries", "replay", "scans", "runtime"]) expect(source).toContain(`record("${family}"`);
  ...
  it("covers every check family with a passing outcome once the gate has run", async () => {
    ...
    const trails = ["tests/artifacts/agentcert.json", "tests/artifacts/costcert.json", "tests/artifacts/doccheck.json", "tests/artifacts/pentest.json"];
