...
import { credspayload, deletepayload, gateforstep, gatekindfor, gateprompttext, gatestateof, nobatchresolution, opengate, paypayload, resolvegate } from "../gates.js";
...
describe("confirmgates", () => {
  ...
  it("shows the amount, payee origin and target element on confirmpay gates", () => {
    ...
    const gate = opengate({ kind: "confirmpay", stepid: "step", runid: "run", origin: "https://shop.example", payload, now });
  ...
  it("shows the target, scope and irreversibility on confirmdelete gates", () => {
    const gate = opengate({ kind: "confirmdelete", stepid: "step", runid: "run", origin: "https://shop.example", payload: deletepayload({ target: "#orders", scope: "the order history", irreversibility: "The deletion destroys the order history the site cannot restore." }), now });
  ...
  it("shows the credential label only on confirmcreds gates and never the value", () => {
    ...
    const gate = opengate({ kind: "confirmcreds", stepid: "step", runid: "run", origin: "https://bank.example", payload, now });
  ...
  it("resolves exactly one open gate through one explicit human action and stays terminal", () => {
    const gate = opengate({ kind: "confirmpay", stepid: "step", runid: "run", origin: "https://shop.example", payload: paypayload({ payeeorigin: "https://shop.example" }), now });
    const resolved = resolvegate({ gates: [gate], gateid: gate.gateid, decision: "resolved", actor: "user", now: now + 1200 });
    ...
    const again = resolvegate({ gates: resolved.gates ?? [], gateid: gate.gateid, decision: "refused", actor: "user", now: now + 1500 });
  ...
  it("refuses resolutions without an acting user and batch approvals", () => {
    const gate = opengate({ kind: "confirmdelete", stepid: "step", runid: "run", origin: "https://shop.example", payload: deletepayload({ scope: "history", irreversibility: "gone" }), now });
    expect(() => resolvegate({ gates: [gate], gateid: gate.gateid, decision: "resolved", actor: "", now })).toThrow(/acting user/);
  ...
  it("builds the gate of one gated step from its reviewed shape", () => {
    const paystep = { id: "pay", kind: "fillcard" as const, target: "#card", options: JSON.stringify({ amount: "12.00", payeeorigin: "https://shop.example" }) };
    const paygate = gateforstep({ step: paystep, classes: ["payment"], runid: "run", origin: "https://shop.example", now });
    ...
    const deletegate = gateforstep({ step: deletestep, classes: ["delete"], runid: "run", origin: "https://shop.example", now });
    ...
    const credgate = gateforstep({ step: credstep, classes: ["credential"], runid: "run", origin: "https://bank.example", credentiallabel: "Bank login", now });
    expect(credgate?.payload).toEqual({ label: "Bank login" });
    expect(gateforstep({ step: credstep, classes: ["credential"], runid: "run", origin: "https://bank.example", now })).toBeUndefined();
    expect(gateforstep({ step: paystep, classes: ["publish"], runid: "run", origin: "https://shop.example", now })).toBeUndefined();
  });
});
