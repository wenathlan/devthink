...
if (!/export const protocolmajor = 2 as const;/.test(versionsource)) throw new Error("version.ts must pin the frozen protocolv2 major two.");
if (!/export const protocolfloormajor = 2 as const;/.test(versionsource)) throw new Error("version.ts must pin the protocol floor two the 2.0.0 sunset raised when the deprecation window closed.");
if (!typesource.includes("export const protocolmajorversion = protocolmajor;")) throw new Error("types.ts must derive the protocol major constant from version.ts.");
if (!typesource.includes("export const pinnedprotocolversion")) throw new Error("The library surface must export the pinned protocol version import.");
if (!mcpsource.includes("negotiateprotocol")) throw new Error("The mcp negotiation must route through the protocolv2 negotiation family.");
if (!verifyworkflow.includes("node tests/apifreeze.mjs")) throw new Error("The verify workflow must gate the api freeze after the validation chain.");
...
if (transparencymatch === null) throw new Error("The web design file must declare the transparencypage surface template the transparency page renders from.");
for (const section of ["#grants", "#windows", "#permissions", "#datakinds", "#integrity", "#senders", "#permdiffs", "#safedefaults"]) {
  if (!transparencymatch[1].includes(`id="${section.slice(1)}"`)) throw new Error(`The transparencypage surface must carry its ${section} section; the transparency page lists grants, permissions, data kinds, integrity, senders, permdiffs and safedefaults.`);
}
for (const reference of [...transparencymatch[1].matchAll(/(?:src|href)\s*=\s*["']([^"']+)["']/g)].map(match => match[1])) {
  if (/^(https?|wss?|data):/i.test(reference)) throw new Error(`The transparencypage surface loads the remote resource ${reference}; the transparency page runs entirely offline.`);
}
...
for (const gate of ["agentcert", "costcert"]) {
  if (!(await readFile(`tests/${gate}.mjs`, "utf8").catch(() => undefined))) throw new Error(`The ${gate} gate script must exist under tests/ beside the runtime policy.`);
  if (!verifyworkflow.includes(`node tests/${gate}.mjs`)) throw new Error(`The verify workflow must run the ${gate} gate of the multi agent certification.`);
}
if (!(await readFile("dashdone.ts", "utf8").catch(() => undefined))) throw new Error("The dashdone family module must exist at the root beside the hardening family.");
const dashdonedeclaration = await readFile("tsconfig.build.json", "utf8");
if (!dashdonedeclaration.includes("dashdone.ts")) throw new Error("The declaration emit must include the dashdone module so dist/dashdone.d.ts ships beside the bundle.");
const dashboardmatch = /<template data-surface="dashboardpage">([\s\S]*?)<\/template>/.exec(webindex);
if (dashboardmatch === null) throw new Error("The web design file must declare the dashboardpage surface template the newtab override serves.");
for (const panel of ["multiagentoverview", "agentstatuscards", "sharedqueueview", "queuelanefilter", "messageflow", "conflictlog", "agentcostpanel", "escalationinbox", "timelinescrubber", "aggregatetimeline", "killswitch", "reportdownload"]) {
  if (!dashboardmatch[1].includes(`id="${panel}"`)) throw new Error(`The dashboardpage surface must carry its ${panel} multi agent panel; the dashdone completion ships every panel or none.`);
}
for (const reference of [...dashboardmatch[1].matchAll(/(?:src|href)\s*=\s*["']([^"']+)["']/g)].map(match => match[1])) {
  if (/^(https?|wss?|data):/i.test(reference)) throw new Error(`The dashboardpage surface loads the remote resource ${reference}; the multi agent dashboard runs entirely offline.`);
}

function minimum(range, label) {
  const match = /^>=(\d+\.\d+\.\d+) <(\d+)$/.exec(range ?? "");
  if (!match || !semver.test(match[1])) throw new Error(`${label} must use a bounded >=x.y.z <next-major engine range.`);
