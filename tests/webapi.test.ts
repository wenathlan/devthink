  ...
  it("parses the event data id and retry fields chunk by chunk while the last event id resumes the stream after a reconnect and a cancelled step closes the channel cleanly", async () => {
    ...
    const open: streamopen = async (url, headers) => {
      ...
      return {
        ok: true,
        status: 200,
        read: async () => (index < chunks.length ? { done: false, text: chunks[index++] ?? "" } : { done: true }),
      };
    };
    ...
    let reads = 0;
    const cancelledopen: streamopen = async () => ({
      ok: true,
      status: 200,
      read: async () => {
        reads += 1;
        return reads <= 2 ? { done: false, text: chunks[reads - 1] ?? "" } : { done: true };
      },
    });
    const cancelled = await subevents({
      record: subscription(),
      open: cancelledopen,
      cancelled: () => reads >= 2,
      now: () => now,
    ...
    const failed = await subevents({
      record: subscription(),
      open: async () => ({
        ok: false,
        status: 403,
        read: async () => ({ done: true }),
        error: "The event stream channel refused the subscription.",
      }),
      now: () => now,
    });
...
describe("web api long poll loop", () => {
  it("retries a timed out request under the configured backoff while the loop stops on the reviewed stop condition, the cancellation flag and the plan expiry", async () => {
    expect(longpollrequestof({ url: "https://example.com/poll", timeout: 500, cursor: "c0" })).toEqual({
      url: "https://example.com/poll",
      timeout: 500,
      cursor: "c0",
    });
    ...
    const sleeps: number[] = [];
    let index = 0;
    const fetchpoll: pollfetch = async (url, body) => {
      calls.push({ url, ...(body !== undefined ? { body } : {}) });
      const answer = answers[index++] ?? { status: 200, headers: {}, body: "{}" };
      return answer;
    };
    const result = await longpoll({
      request: { url: "https://example.com/poll", timeout: 500 },
      cursor: pollcursorfixture(),
      fetchpoll,
      backoff: 5,
      now: () => now,
    ...
    const cancelled = await longpoll({
      request: { url: "https://example.com/poll" },
      cursor: pollcursorfixture(),
      fetchpoll,
      cancelled: () => true,
      now: () => now,
    ...
    const ceiling = await longpoll({
      request: { url: "https://example.com/poll" },
      cursor: pollcursorfixture({ maxpolls: 1 }),
      fetchpoll: async () => ({ status: 200, headers: {}, body: JSON.stringify({ cursor: "c9", done: "false" }) }),
      now: () => now,
      sleep: async () => {},
    });
...
describe("web api graphql subscription mapping", () => {
  it("maps the next error and complete messages of the graphql-ws grammar into step results while the subscribe frame carries the query and its variables", () => {
    const parsed = graphqlsubscriptionof({
      query: "subscription { price(symbol: $symbol) }",
      channel: "wss://example.com/ws",
      variables: { symbol: "dev" },
    });
    ...
    expect(JSON.parse(frame)).toEqual({
      id: "op1",
      type: "subscribe",
      payload: { query: "subscription { price(symbol: $symbol) }", variables: { symbol: "dev" } },
    });
    const messages = [
    ...
    expect(mapped.results).toEqual([
      { id: "op1", data: { price: 10 } },
      { id: "op1", data: { price: 20 } },
    ]);
    ...
    expect(parsegraphqlmessage(JSON.stringify({ type: "ka" }))).toEqual({
      kind: "error",
      errors: ["The graphql subscription message carries the unknown type ka."],
    });
    expect(
      graphqlsub({ subscription: parsed!, messages: [JSON.stringify({ type: "next", payload: { price: 30 } })] })
        .completed,
    ).toBe(false);
  });
});
  ...
  it("encodes the reviewed fields with the urlencoded content type while the multipart body streams its parts chunk by chunk with progress and refuses unreviewed files and ungranted origins", () => {
    const form = formpost({
      payload: {
        ...
        fields: [
          { name: "name", value: "Dev Think" },
          { name: "price", value: "1299.50" },
        ],
      },
    ...
    const refusedform = formpost({
      payload: { url: "https://outside.example/submit", fields: [] },
      grants: ["https://example.com"],
    });
    ...
    const unreviewed = multipartpost({
      payload: {
        ...payload,
        files: [{ name: "file", filename: "x.csv", mime: "text/csv", content: "x", reviewed: false }],
      },
      grants: ["https://example.com"],
    });
    expect(unreviewed.ok).toBe(false);
    if (!unreviewed.ok) expect(unreviewed.refusal).toContain("explicit reviewed flag");
    const outside = multipartpost({
      payload: { url: "https://outside.example/upload", fields: [], files: [] },
      grants: ["https://example.com"],
    });
...
describe("web api correlation ids", () => {
  it("assigns one id per outbound request of the run while the responses join their pairs and the export reads the per run map", () => {
    ...
    expect(() => joincorrelation({ context: joined, requestid: "req-9", status: 200, now })).toThrow(
      /carries no request req-9/,
    );
...
describe("web api rate limit respect and response caching", () => {
  it("delays the next request of an origin until the reset window passes while the per run cache serves repeated read only calls and expires by its headers", async () => {
    ...
    const waits: number[] = [];
    const respected = await ratelimitrespect({
    ...
    expect(stored.entry?.key).toBe(
      cachekeyof({ runid: "run1", url: "https://example.com/api/price", method: "GET", body: "" }),
    );
    expect(cachekeyof({ runid: "run1", url: "https://example.com/api/price", method: "GET", body: "" })).not.toBe(
      cachekeyof({ runid: "run2", url: "https://example.com/api/price", method: "GET", body: "" }),
    );
    ...
    const postrefusal = cacheresponse({
      entries: [],
      url: "https://example.com/api/price",
      method: "POST",
      body: "{}",
    ...
    const credrefusal = cacheresponse({
      entries: [],
      url: "https://example.com/api/price",
      method: "GET",
      body: "",
    ...
    const observed = apicallrecordof({
      id: "a1",
      runid: "run1",
      stepid: "st1",
      url: "https://example.com/api/price?symbol=dev",
      method: "GET",
      status: 200,
