...
describe("attentionfeed collection and ranking", () => {
  it("collects gate waits, phishguard blocks, deferrals and failures with their deep links", () => {
      ...
      phishblocks: [
        {
          ...
          reason: "distance 0.9 over the threshold",
        },
      ],
      deferrals: [{ runid: "run2", origin: "https://example.com", reason: "the bucket reset waits" }],
      failures: [
        { runid: "run3", stepid: "step9", origin: "https://example.com", message: "the selector matched nothing" },
      ],
      now,
    });
    ...
    expect(() =>
      attentionentryof({ cause: "gatewait", runid: " ", origin: "https://example.com", summary: "waits", at: now }),
    ).toThrow(/run ref/);
    expect(() =>
      attentionentryof({ cause: "gatewait", runid: "run1", origin: "https://example.com", summary: " ", at: now }),
    ).toThrow(/summary/);
  });
  ...
  it("deduplicates repeated causes per run while the first occurrence keeps its time", () => {
    ...
    const othergate = attentionentryof({
      cause: "gatewait",
      runid: "r",
      origin: "https://example.com",
      summary: "waits",
