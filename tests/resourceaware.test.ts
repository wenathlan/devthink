...
describe("tabsuspend", () => {
  ...
  it("restores a tab before the next step that needs it and preserves the run state across the suspend and restore", () => {
    const state = tabsuspendstateof({
      tabid: 7,
      runid: "run1",
      restoreurl: "https://example.com/a",
      discarded: true,
      now,
    });
    expect(state.discarded).toBe(true);
    expect(() =>
      tabsuspendstateof({ tabid: 7, runid: " ", restoreurl: "https://example.com/a", discarded: false, now }),
    ).toThrow(/run id/i);
    ...
    const intact = tabsuspendrestoreplan({
      state: { ...state, discarded: false },
      nextneedsurl: "https://example.com/a",
    });
  ...
  it("stores and clears the tabsuspend states per run", async () => {
    ...
    await store.setsuspendstate(
      tabsuspendstateof({ tabid: 7, runid: "run1", restoreurl: "https://example.com/a", discarded: false, now }),
    );
...
describe("artifactcompress", () => {
  it("compresses captures and logs at rest and decompresses them lazily on read", () => {
    ...
    expect(artifactreadplan(artifactcompressof({ artifactid: "cap1", codec: "store", lazy: false, now })).decode).toBe(
      "immediate",
    );
...
describe("readparallel", () => {
  it("groups independent page reads into parallel lanes and joins them before the dependent steps", () => {
    const group = readparallelgroupof({
      runid: "run1",
      reads: [
        { stepid: "s1", origin: "https://a.example" },
        { stepid: "s2", origin: "https://b.example" },
        { stepid: "s3", origin: "https://c.example" },
      ],
      lanes: 2,
    });
    ...
    expect(
      readparallelgroupof({ runid: "run1", reads: [{ stepid: "s1", origin: "https://a.example" }] }).lanes,
    ).toHaveLength(1);
...
describe("warmselectors", () => {
  it("carries validated selectors between adjacent steps and revalidates on a fingerprint change only", () => {
    const record = carrywarmselector({
      selector: "#button",
      resolution: "resolved:#button",
      fingerprint: "fp1",
      from: "s1",
    });
    expect(record.carriedfrom).toBe("s1");
    expect(() => carrywarmselector({ selector: " ", resolution: "r", fingerprint: "fp1", from: "s1" })).toThrow(
      /selector/i,
    );
    expect(() => carrywarmselector({ selector: "#button", resolution: "r", fingerprint: " ", from: "s1" })).toThrow(
      /fingerprint/i,
    );
