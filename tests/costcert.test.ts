...
describe("the costcert reconciliation artifact", () => {
  it("declares every accounting check of the release candidate reconciliation with its family and module", async () => {
    ...
    for (const family of ["replay", "token accounting", "shared accounting", "budgets", "refunds", "routing", "export", "audit trail", "report"]) expect(families.has(family)).toBe(true);
  ...
  it("covers every accounting check with a passing outcome once the gate has run", async () => {
    ...
    if (!stored) return;
    const report = JSON.parse(await readFile(artifactpath, "utf8")) as { release: string; entries: Array<{ id: number; title: string; outcome: string; detail: string }>; summary: { total: number; passed: number; failed: number } };
  ...
  it("keeps the reconciliation invariants of the artifact summary and the recorded checks", async () => {
    ...
    const report = JSON.parse(await readFile(artifactpath, "utf8")) as { release: string; entries: Array<{ id: number; outcome: string }>; summary: { total: number; passed: number; failed: number } };
