...
import { sbomcomponentof, sbomcoveragecheck, sbominventory, sbominventorytext, sbomlicense, sbomnameof, sbomspecversion } from "../pack.js";

const valid = "a".repeat(64);

describe("sbom", () => {
  it("builds the cyclonedx component record of one artifact with its hash and channels", () => {
    const component = sbomcomponentof({ name: "devthink1.1.87.zip", size: 2048, checksum: valid, channels: ["github", "chromium"] });
  ...
  it("covers every artifact of the release in one cyclonedx inventory", () => {
    ...
    const inventory = JSON.parse(sbominventorytext({ version: "1.1.87", artifacts })) as { bomformat: string; specversion: string; components: Array<{ name?: string; hashes?: Array<{ alg?: string; content?: string }> }>; metadata: { component: { name: string; version: string; licenses: Array<{ license: { id: string } }> } } };
  ...
  it("renders byte identical output for the same artifact set because no timestamp enters the document", () => {
    ...
    expect(sbominventorytext({ version: "1.1.87", artifacts })).toBe(sbominventorytext({ version: "1.1.87", artifacts }));
  ...
  it("refuses an empty artifact set and a duplicate artifact name", () => {
    ...
    expect(() => sbominventory({ version: "1.1.87", artifacts: [{ name: "a.zip", size: 1, checksum: valid }, { name: "a.zip", size: 1, checksum: valid }] })).toThrow();
    expect(() => sbominventory({ version: "not-a-version", artifacts: [{ name: "a.zip", size: 1, checksum: valid }] })).toThrow();
  });

  it("reports the artifacts the inventory misses or records with another hash", () => {
    const inventory = JSON.parse(sbominventorytext({ version: "1.1.87", artifacts: [{ name: "a.zip", size: 1, checksum: "a".repeat(64) }] }));
    const missing = sbomcoveragecheck({ inventory, artifacts: [{ name: "a.zip", checksum: "a".repeat(64) }, { name: "b.zip", checksum: "b".repeat(64) }] });
