...
describe("agentfleet naming and scope narrowing", () => {
  it("assigns unique lowercase names and refuses duplicates, reserved words and out of rule names", () => {
    ...
    expect(() =>
      agentname({ records: [first], id: "a2", proposed: "scout", origin: "https://example.com", now }),
    ).toThrow(/already registered/i);
    expect(() =>
      agentname({ records: [first], id: "a2", proposed: "user", origin: "https://example.com", now }),
    ).toThrow(/reserved/i);
    expect(() =>
      agentname({ records: [first], id: "a2", proposed: "Scout Two", origin: "https://example.com", now }),
    ).toThrow(/lowercase identifier/i);
  ...
  it("intersects the requested scope with the session grants and narrows the action kinds", () => {
    ...
    const scope = agentscopeof({
      agentid: "a1",
      requested: {
        origins: ["https://example.com", "https://denied.example"],
        actionkinds: ["observe", "readtext", "click"],
      },
      grants,
      allowedkinds: ["observe", "readtext", "navigate", "click"],
    });
    ...
    expect(() =>
      agentscopeof({
        agentid: "a1",
        requested: { origins: ["https://denied.example"] },
        grants,
        allowedkinds: ["observe"],
      }),
    ).toThrow(/outside the session grants/i);
    expect(() =>
      agentscopeof({ agentid: "a1", requested: { actionkinds: ["typetime"] }, grants, allowedkinds: ["observe"] }),
    ).toThrow(/outside the allowed catalog/i);
    ...
    expect(agentscopegate({ scope, kind: "click", origin: "https://denied.example", risk: "sensitive" }).allowed).toBe(
      false,
    );
  });
});
...
describe("agentfleet budget grant, spend refusal and reporting", () => {
  it("grants a budget from user configuration, spends one unit per step and refuses past the ceiling", () => {
    ...
    expect(() => spendbudget({ state: twosteps, steps: 0, tokens: 0, durationms: 5_000, now })).toThrow(
      /budgeted milliseconds/i,
    );
...
describe("agentfleet pause isolation and killswitch completeness", () => {
  ...
  it("engages the killswitch in one call: every record stops, every agent run cancels, every queue clears and each stopped agent counts", () => {
    const fleet = [
      record(),
      record({ id: "a2", name: "mapper" }),
      record({ id: "a3", name: "watcher", state: "paused" }),
    ];
    const userrun: runrecord = {
      runid: "run3",
      planid: "plan3",
      sessionid: "session1",
      state: "running",
    ...
    const runs = [
      run(),
      run({ runid: "run2", agentid: "a2", state: "queued" }),
      userrun,
      run({ runid: "run4", agentid: "a2", state: "completed" }),
    ];
    const queue: taskqueue = {
      ...emptyqueue({ lanes: ["main"] }),
      items: [{ id: "t1", lane: "main", priority: 1, payload: "task", state: "queued", enqueuedat: now }],
      claims: [],
    };
