...
import { conflictsfor, fileproviderof, resolveconflict, syncbridgeexportpayload, syncbridgehookof, syncbridgeoptinflip, syncbridgeproviders, syncbridgescan, syncbridgevalidate, syncdigestof, webproviderstub } from "../flow.js";
...
function manifest(id: string, version: string): flowlibrarymanifest {
  return { id, title: `Flow ${id}`, description: "A shared template.", version, publisher: "example publisher", steps: [{ id: "open", kind: "focus", label: "Focus" }], kinds: ["focus"], requiredgrants: ["https://example.com"], dataexpectations: [], sensitive: false };
}

function hook(overrides: Partial<syncbridgehook> = {}): syncbridgehook {
  return { id: "sync:file:1", provider: "file", direction: "both", optin: false, endpoint: "manifests.json", state: "idle", createdat: now, ...overrides };
}
...
describe("syncbridge hooks and opt in gating", () => {
  ...
  it("flips the explicit opt in and lists the providers", () => {
    const created = syncbridgehookof({ provider: "web", direction: "pull", endpoint: "https://registry.example.test", now });
  ...
  it("keeps the bridge on manifests only and refuses secrets and logs in full", () => {
    expect(syncbridgevalidate({ kind: "syncbridge", manifests: [manifest("a", "1")], digests: [], exclusions: [] }).ok).toBe(true);
    expect(syncbridgevalidate({ kind: "syncbridge", manifests: [{ ...manifest("a", "1"), apikey: "sk-live" } as unknown as Record<string, unknown>] as unknown }).ok).toBe(false);
    expect(syncbridgevalidate({ kind: "syncbridge", manifests: [], secrets: ["hunter2"] }).ok).toBe(false);
    expect(syncbridgevalidate({ kind: "syncbridge", manifests: [], logs: [{ summary: "typed hunter2" }] }).ok).toBe(false);
...
describe("syncbridge digests and conflict detection", () => {
  it("computes manifest digests and moves only matching manifests", async () => {
    ...
    const scan = await syncbridgescan({ hook: active, local: [{ manifestid: "digest", version: "1.0.0", digest }], remote: [{ manifestid: "digest", version: "1.0.0", digest }], now });
  ...
  it("surfaces both versions on digest conflicts instead of a silent overwrite", async () => {
    const active = hook({ optin: true });
    const scan = await syncbridgescan({ hook: active, local: [{ manifestid: "flow", version: "1.0.0", digest: "a".repeat(64) }], remote: [{ manifestid: "flow", version: "2.0.0", digest: "b".repeat(64) }], now });
    ...
    const conflict: syncbridgeconflict = { id: scan.conflicts[0]?.id ?? "c", hookid: active.id, manifestid: "flow", local: { digest: "a".repeat(64), version: "1.0.0" }, remote: { digest: "b".repeat(64), version: "2.0.0" }, detectedat: now };
  ...
  it("refuses to scan or push behind an opted out hook", async () => {
    ...
    const scan = await syncbridgescan({ hook: off, local: [], remote: [{ manifestid: "flow", version: "1.0.0", digest: "a".repeat(64) }], now });
    expect(scan.synced).toHaveLength(0);
