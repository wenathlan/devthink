...
describe("blackboard post and read", () => {
  ...
  it("refuses entries without a key, a value, an author or with a json value that does not parse", () => {
    expect(() =>
      postentry({ board: board(), id: "e1", key: " ", value: "v", section: "facts", author: "a1", now }),
    ).toThrow(/key/i);
    expect(() =>
      postentry({ board: board(), id: "e1", key: "k", value: " ", section: "facts", author: "a1", now }),
    ).toThrow(/value/i);
    expect(() =>
      postentry({ board: board(), id: "e1", key: "k", value: "v", section: "facts", author: " ", now }),
    ).toThrow(/author/i);
    expect(() =>
      postentry({ board: board(), id: "e1", key: "k", value: "v", section: "secret" as never, author: "a1", now }),
    ).toThrow(/sections/i);
    expect(() =>
      postentry({
        board: board(),
  ...
  it("reads the entries of one section with the freshness filter", () => {
    ...
    state = postentry({
      ...
      id: "e3",
      key: "late",
      value: "Found the totals",
      section: "findings",
      author: "a2",
      now: now + 10_000,
    });
    expect(readentries({ board: state, now: now + 20_000 })).toHaveLength(3);
    expect(readentries({ board: state, section: "facts", now: now + 20_000 }).map((entry) => entry.key)).toEqual([
      "fact",
    ]);
...
describe("blackboard retirement", () => {
  it("retires the stale entries by the user configured window and never without one", () => {
    let state = postentry({
      board: board({ retirementwindow: 10_000 }),
      id: "e1",
      key: "old",
      value: "Stale fact",
      section: "facts",
...
describe("blackboard cross agent visibility and consent", () => {
  ...
  it("inherits the consent class of the source extraction into the entry", () => {
    const posted = postentry({
      ...
      id: "e1",
      key: "form.values",
      value: "The form carried the address",
      section: "findings",
      author: "a1",
  ...
  it("summarizes the sections with the authors and the freshest posting time", () => {
    ...
    expect(summary.find((section) => section.section === "goals")).toMatchObject({
      entries: 1,
      authors: ["user"],
      freshestat: now,
    });
