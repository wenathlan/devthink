...
describe("torture: tab queries, patterns and the star storm", () => {
      ...
      parsetabquery(
        step("querytabs", {
          options: JSON.stringify({
            tabquery: { url: "https://example.com", title: "Example", id: 3, pattern: "https://example.com/*" },
          }),
        }),
      ),
    ).toEqual({ url: "https://example.com", title: "Example", id: 3, pattern: "https://example.com/*" });
    expect(
      parsetabquery(step("querytabs", { options: JSON.stringify({ tabquery: { id: -1, url: "", title: 5 } }) })),
    ).toEqual({});
  ...
  it("resolves tab queries where every matcher that exists must hold", () => {
    const tabs = [
      tab({ tabid: 1, url: "https://example.com/a", title: "Alpha" }),
      tab({ tabid: 2, url: "https://example.com/b", title: "beta" }),
      tab({ tabid: 3, url: "https://other.example/c", title: "Gamma" }),
    ];
  ...
  it("normalizes urls for clone comparison by dropping fragments and trailing slashes", () => {
    ...
    expect(normalizedtaburl("")).toBe("");
    const tabs = [
      tab({ tabid: 1, url: "https://example.com/p" }),
      tab({ tabid: 2, url: "https://example.com/p/" }),
      tab({ tabid: 3, url: "https://example.com/p#x" }),
      tab({ tabid: 4, url: "https://other.example" }),
    ];
  ...
  it("searches, audio lists, discard candidates and restores across the live tab set", () => {
    const tabs = [
      tab({ tabid: 1, title: "Pricing table", url: "https://example.com/pricing" }),
      tab({ tabid: 2, title: "Docs", url: "https://docs.example.org/guide", audible: true, active: false }),
      tab({ tabid: 3, title: "Music", muted: true, audible: true, discarded: true, active: false }),
    ];
  ...
  it("captures layouts, restores only the closed urls and regroups membership through moves", () => {
    const tabs = [
      tab({ tabid: 1, index: 0, pinned: true }),
      tab({ tabid: 2, index: 1 }),
      tab({ tabid: 3, index: 2, windowid: 7 }),
    ];
    const windows = [
      {
        windowid: 1,
    ...
    const groups: tabgrouprecord[] = [
      { groupid: "g1", name: "Research", color: "blue", tabids: [3, 2, 99], collapsed: false, savedat: now - 10 },
    ];
    ...
    expect(layout.windows[1]?.state).toEqual({
      bounds: { left: 10, top: 10, width: 400, height: 300 },
      maximized: true,
      profile: "scratch",
    });
    expect(layoutrestoreplan(layout, ["https://example.com/page/"])).toEqual([]);
    expect(layoutrestoreplan(layout, ["https://other.example/x"])).toEqual([
  ...
  it("zooms without crossing zero, switches with wraparound and orders the switcher by recency", () => {
    ...
    const tabs = [
      tab({ tabid: 1, index: 0, title: "Alpha" }),
      tab({ tabid: 2, index: 1, title: "Beta tab 2" }),
      tab({ tabid: 3, index: 2, title: "Gamma" }),
    ];
  ...
  it("dispatches watched tab events through filters and derives badges and tab budgets", () => {
    const events = [
      { watchid: "w1", event: "title" as const, tabid: 1, at: now },
      { watchid: "w1", event: "closed" as const, tabid: 2, at: now + 1 },
      { watchid: "w2", event: "title" as const, tabid: 3, at: now + 2 },
    ];
...
describe("torture: download states, filenames, mime filters and quarantine routing", () => {
  ...
  it("walks only the legal batch queue transitions and merges evidence", () => {
    ...
    const running = advancedownload(record, "running", now + 1, {
      bytes: 42,
      checksum: "sha",
      downloadid: 7,
      path: "/tmp/file.bin",
  ...
  it("filters mime types where exclude wins, then include, then the default", () => {
    ...
    expect(
      mimeallowed({ include: ["application/json"], exclude: [], default: "deny" as const }, "application/json"),
    ).toBe(true);
  ...
  it("redacts every header value, correlates netlog rows and hashes clipboard payloads", () => {
    expect(redactheaders({ Authorization: "Bearer abc", "X-Safe": "1" })).toEqual({
    ...
    expect(clipentryof("read", { hash: cliphash("payload"), length: 7 }, "https://example.com", "s1", now)).toEqual({
      kind: "read",
    ...
    expect(
      JSON.stringify(clipentryof("write", { hash: "h", length: 2 }, "https://example.com", "s", now)),
  ...
  it("routes quarantined paths that no traversal, backslash or absolute prefix escapes", () => {
    ...
    expect(quarantinedpath("")).toBe("devthink-quarantine/");
    expect(newquarantine("q1", "../../etc/passwd", "reviewed quarantine", now).path).toBe(
      "devthink-quarantine/etc/passwd",
    );
  });
  ...
  it("reads namecaptures step lists from options or the plan and sweeps cleanup rules", () => {
    ...
    expect(
      capturesteps({}, { steps: [step("observe", { id: "one", risk: "read" }), step("click", { id: "two" })] }),
    ).toEqual(["one", "two"]);
    expect(referencedartifacts(undefined, [])).toEqual([]);
    const plan = {
      steps: [
        step("attachfile", { id: "af1", value: "invoice.pdf" }),
        step("attachfile", { id: "af2", value: "gone.pdf" }),
        step("attachfile", { id: "af3", value: " " }),
        step("observe", { id: "o1" }),
      ],
    };
...
describe("torture: navigation intent, prefetch and deep links", () => {
  it("predicts nav intent from https step values with dedupe and history weighting", () => {
    const steps = [
      step("navigate", { value: "https://example.com/one" }),
      step("navigate", { value: "https://example.com/one" }),
      step("navigate", { value: "https://example.com/two" }),
      step("navigate", { value: "http://insecure.example" }),
      step("navigate", { value: "  " }),
    ];
    const visits: urlvisit[] = [
      { url: "https://example.com/two", at: now - 100, runid: "run1" },
      { url: "https://example.com/two", at: now - 50, runid: "run1" },
    ];
  ...
  it("warms only the granted origins while a stale plan drops every prediction", () => {
    const warmed = prefetchpage({
      id: "w1",
      planid: "plan1",
      urls: ["https://example.com/a", "https://example.com/a", "https://other.example/b", "not a url"],
    ...
    const stale = prefetchpage({
      ...
      stored: {
        id: "w1",
        planid: "plan1",
        predictedurls: [{ url: "https://example.com/old", confidence: 0.9 }],
        createdat: now,
      },
      now,
    });
  ...
  it("preconnects only granted, deduped, non empty origins", () => {
    const result = preconnectorigin({
      origins: ["https://example.com", "https://example.com", " ", "https://other.example"],
      grants: ["https://example.com"],
      now,
    });
  ...
  it("builds deep links with encoded parameters and refuses unknown apps and half parameters", () => {
    expect(deeplinkapp({ app: "github", params: { owner: "wenathlan", repo: "extension" } }).url).toBe(
    ...
    expect(deeplinkapp({ app: "youtube", params: { id: "dQw4w9WgXcQ" } }).url).toBe(
      "https://www.youtube.com/watch?v=dQw4w9WgXcQ",
    );
    expect(deeplinkapp({ app: "youtube", params: { search: "dev think" } }).url).toBe(
      "https://www.youtube.com/results?search_query=dev%20think",
    );
    expect(deeplinkapp({ app: "wikipedia", params: { title: "Ünicode" } }).url).toBe(
      "https://en.wikipedia.org/wiki/%C3%9Cnicode",
    );
    expect(deeplinkapp({ app: "maps", params: { query: "a b&c" } }).url).toBe(
      "https://www.google.com/maps/search/a%20b%26c",
    );
    expect(
      deeplinkapp({
        app: "custom",
        params: { p: "v" },
        patterns: [{ app: "custom", origin: "https://custom.example", route: "/{p}", params: ["p"] }],
      }).url,
    ).toBe("https://custom.example/v");
    expect(() =>
      deeplinkapp({
        app: "custom",
        params: {},
    ...
    ).toThrow(/needs its reviewed parameters/);
    expect(
      deeplinkapp({
        app: "github",
        params: { owner: "wenathlan" },
        patterns: [
          { app: "github", origin: "https://evil.example", route: "/{owner}/{repo}", params: ["owner", "repo"] },
        ],
      }).url,
    ).toBe("https://github.com/wenathlan");
...
describe("torture: reopening, trails and consent pauses", () => {
  ...
  it("reopens the most recent fresh record under a grant recheck and refuses replayed records", () => {
    ...
    const nextfresh = reopentab({
      records: [{ ...records[0]!, reopenedat: now - 5 }, records[1]!, { ...records[2]!, reopenedat: now - 4 }],
      grants: ["https://example.com"],
      now: now + 10,
    });
    ...
    expect(() =>
      reopentab({
        records: [{ ...records[0]!, reopenedat: now - 5 }],
        grants: ["https://example.com"],
        url: "https://example.com/a",
        now,
      }),
    ).toThrow(/already reopened/);
    expect(() => reopentab({ records: [{ ...records[2]! }], grants: ["https://example.com"], now })).toThrow(
      /lost its grant/,
    );
    ...
    expect(() => reopentab({ records, grants: [], url: "https://missing.example/z", now })).toThrow(
      /No closed tab record/,
    );
  });
  ...
  it("restores the navigation trail in visit order folding consecutive duplicates per run", () => {
    ...
    expect(restoretrail({ visits, runid: "run1" }).map((entry) => entry.url)).toEqual([
      "https://example.com/a",
      "https://example.com/b",
    ]);
    expect(restoretrail({ visits, runid: "run2" }).map((entry) => entry.url)).toEqual([
      "https://example.com/c",
      "https://example.com/a",
    ]);
    ...
    const ties: urlvisit[] = [
      { url: "https://b.example", at: now, runid: "r" },
      { url: "https://a.example", at: now, runid: "r" },
    ];
    expect(restoretrail({ visits: ties }).map((entry) => entry.url)).toEqual([
      "https://a.example",
      "https://b.example",
    ]);
  });
  ...
  it("freezes navigation behind a consent prompt and hands the queued navigation back on resume", () => {
    const paused = pausenavconsent({
      reason: "a payment consent is open",
    ...
    expect(paused).toMatchObject({
      pausedat: now,
      reason: "a payment consent is open",
      pendingurl: "https://example.com/pay",
      pendingstepid: "s9",
    });
...
describe("torture: navigation rate windows at their exact boundaries", () => {
  it("counts with no configured window and ceiling while the sliding window ages hits exactly at the boundary", () => {
    ...
    const boundary = navratelimit({
      stored: { domain: "example.com", count: 2, resetat: now, hits: [now - 1000], window: 1000, ceiling: 2 },
      url: "https://example.com/a",
      now,
    });
    expect(boundary.allowed).toBe(true);
    expect(boundary.window.count).toBe(1);
    const onebefore = navratelimit({
      stored: { domain: "example.com", count: 2, resetat: now, hits: [now - 1, now - 1], window: 1000, ceiling: 2 },
      url: "https://example.com/a",
      now,
    });
    ...
    const blocked = navratelimit({
      stored: { domain: "example.com", count: 2, resetat: now, hits: [now - 500, now - 250], window: 1000, ceiling: 2 },
      url: "https://example.com/a",
      now,
    });
    ...
    const single = navratelimit({
      stored: { domain: "example.com", count: 1, resetat: now, hits: [now - 250], window: 1000, ceiling: 1 },
      url: "https://example.com/a",
      now,
    });
  ...
  it("opens clipboard urls only when https, parseable and granted", () => {
    expect(openclipboardurl({ text: "https://example.com/notes", grants: ["https://example.com"] }).url).toBe(
      "https://example.com/notes",
    );
    expect(() => openclipboardurl({ text: "  not a url  ", grants: [] })).toThrow(/no valid url/);
    expect(() => openclipboardurl({ text: "http://example.com/notes", grants: ["http://example.com"] })).toThrow(
      /HTTPS/,
    );
    expect(() => openclipboardurl({ text: "ftp://example.com/f", grants: [] })).toThrow(/HTTPS/);
    expect(() => openclipboardurl({ text: "https://other.example/x", grants: ["https://example.com"] })).toThrow(
      /outside the session grants/,
    );
  });
});
...
describe("torture: url safety with lookalikes, homographs and scheme tricks", () => {
  it("refuses lookalike hosts that imitate granted origins", () => {
    ...
    expect(checksafeurl({ url: "https://example.com.evil.com/", granted, now }).reasons.join(" ")).toContain(
      "imitates the granted origin",
    );
  ...
  it("refuses punycode and unicode homograph hosts", () => {
    expect(checksafeurl({ url: "https://xn--80ak6aa92e.com/", granted: [], now }).reasons.join(" ")).toContain(
      "punycode",
    );
    expect(checksafeurl({ url: "https://аpple.com/", granted: ["https://apple.com"], now }).safe).toBe(false);
    expect(
      checksafeurl({ url: "https://аpple.com/", granted: ["https://apple.com"], now }).reasons.join(" "),
    ).toContain("punycode");
  });
  ...
  it("refuses weak schemes, embedded credentials, private and raw hosts, and unparseable urls", () => {
    ...
    expect(checksafeurl({ url: "https://user:pass@example.com/", granted: [], now }).reasons.join(" ")).toContain(
      "embedded credentials",
    );
    expect(checksafeurl({ url: "https://localhost/app", granted: [], now }).reasons.join(" ")).toContain(
      "private network",
    );
    expect(checksafeurl({ url: "https://127.0.0.1/app", granted: [], now }).reasons.join(" ")).toContain(
      "private network",
    );
    expect(checksafeurl({ url: "https://10.1.2.3/", granted: [], now }).reasons.join(" ")).toContain("private network");
    expect(checksafeurl({ url: "https://192.168.0.9/", granted: [], now }).reasons.join(" ")).toContain(
      "private network",
    );
    expect(checksafeurl({ url: "https://172.16.0.1/", granted: [], now }).reasons.join(" ")).toContain(
      "private network",
    );
    ...
    expect(checksafeurl({ url: "not a url at all", granted: [], now })).toMatchObject({
      safe: false,
      reasons: ["the url does not parse"],
    });
...
describe("torture: curated batch opening with size bounds and full rate windows", () => {
  it("refuses the whole batch when one url is unsafe and never opens a tab", () => {
    const batch = batchopenlinks({
      id: "b1",
      urls: ["https://example.com/a", "https://example.com.evil.com/"],
      grants: ["https://example.com"],
      windows: [],
      now,
    });
  ...
  it("refuses a batch that passes the user ceiling without a code cap", () => {
    ...
    ).toThrow(/passes the user configured ceiling/);
    expect(
      batchopenlinks({
        id: "b3",
        urls: ["https://example.com/a", "https://example.com/b"],
  ...
  it("moves urls onto waits when the per domain window is full instead of dropping them", () => {
    const full: ratelimitwindow = {
      domain: "example.com",
      count: 2,
      resetat: now + 500,
      hits: [now - 400, now - 300],
      window: 1000,
      ceiling: 2,
    };
    const batch = batchopenlinks({
      id: "b5",
      urls: ["https://example.com/a", "https://example.com/b", "https://other.example/c"],
    ...
    expect(batch.waits).toEqual([
      { url: "https://example.com/a", waitms: 600 },
      { url: "https://example.com/b", waitms: 600 },
    ]);
