...
import { assemblelapse, beforeafter, consoletimeline, diffbase, diffshot, exportcaptures, namecaptures, nettimeline, thumbshot, timelapse, type pixeldiff } from "../evidence.js";
import { captureexportgate, captureretentiongrade, consolemaskgate, diffbasegate, diffthresholdgrade, forensicsreadonlygate, forensicscopegate, nettraceorigingate, thumbnailsizereadonlygrade, timelapsegate, timelapseintervalgrade } from "../policy.js";
...
function baseline(over: Partial<diffbaserecord> = {}): diffbaserecord {
  return { id: "base1", runid: "run1", captureid: "cap-base", pagestate: "checkout page with the pay button", ...(over.threshold !== undefined ? { threshold: over.threshold } : {}), beat: 3, at: now, ...over };
}
...
describe("forensics beforeafter pairing and step linkage", () => {
  it("pairs the pre and post captures around a sensitive step while a read only step skips its pre capture and both captures link to the stepid under the run heartbeat", () => {
    const sensitive = beforeafter({ runid: "run1", stepid: "st1", stepkind: "click", risk: "sensitive", pre: { id: "cap-pre", at: now - 40 }, post: { id: "cap-post", at: now }, beat: 5, now });
    ...
    const readonly = beforeafter({ runid: "run1", stepid: "st2", stepkind: "readtable", risk: "read", post: { id: "cap-post2", at: now }, beat: 5, now });
    ...
    expect(() => beforeafter({ runid: "", stepid: "st1", stepkind: "click", risk: "sensitive", now })).toThrow(/names its run/);
    expect(() => beforeafter({ runid: "run1", stepid: "  ", stepkind: "click", risk: "sensitive", now })).toThrow(/names its step/);
...
describe("forensics consoletimeline ordering and reload continuity", () => {
  it("collects the console entries in capture order with the stepid active at each entry while the run wide sequence numbers continue across page reloads", () => {
    ...
    expect(timeline.map(entry => entry.text)).toEqual(["first line", "cart recalculated", "after reload", "payment failed"]);
    ...
    const continued = consoletimeline({ runid: "run1", entries: [{ level: "log", text: "next batch", source: "console", at: now + 1000 }], steps: [{ id: "st3", startedat: now + 950 }], startsequence: 4, beat: 4, now });
    expect(continued[0]?.sequence).toBe(5);
    expect(continued[0]?.stepid).toBe("st3");
    const fallback = consoletimeline({ runid: "run1", entries: [{ level: "log", text: "before every step", source: "console", at: now - 100 }], steps: [{ id: "st0", startedat: now }], now });
...
describe("forensics nettimeline joining through correlateids", () => {
  it("attaches each traced request to the step running at its timestamp and joins the request with its response through the correlation map of the run", () => {
    ...
    expect(nettraceorigingate({ url: "https://outside.example/api", granted: ["https://shop.example"] }).allowed).toBe(false);
...
describe("forensics diffbase storage and diffshot scoring", () => {
  it("freezes the baseline for a page state and computes the changed regions with the similarity score while the regression flag answers the user threshold alone", async () => {
    const stored = diffbase({ runid: "run1", captureid: "cap-base", pagestate: "checkout page", threshold: 0.7, beat: 2, now });
    expect(stored.threshold).toBe(0.7);
    expect(stored.pagestate).toBe("checkout page");
    expect(() => diffbase({ runid: "run1", captureid: "", pagestate: "checkout page", now })).toThrow(/names its stored capture/);
    expect(() => diffbase({ runid: "run1", captureid: "cap-base", pagestate: "  ", now })).toThrow(/names its page state/);
    expect(() => diffbase({ runid: "run1", captureid: "cap-base", pagestate: "checkout", threshold: 1.5, now })).toThrow(/between zero and one/);
    ...
    const walk: pixeldiff = async (_baseline, _capture, grid) => {
      const blocks = Array.from({ length: grid.cols * grid.rows }, (_unused, index) => index === 0 || index === 1 || index === grid.cols ? 1 : 0);
      return { width: 400, height: 800, blocks };
    };
    const result = await diffshot({ baseline: stored, capture: { id: "cap-new", bytes: "data:image/png;base64,new", width: 400, height: 800 }, diff: walk, blocks: { cols: 4, rows: 3 }, stepid: "st5", beat: 7, now });
    expect(result.baselineid).toBe(stored.id);
    expect(result.captureid).toBe("cap-new");
    expect(result.score).toBe(0.75);
    expect(result.regression).toBe(false);
    expect(result.regions).toEqual([{ x: 0, y: 0, width: 200, height: 267 }, { x: 0, y: 267, width: 100, height: 267 }]);
    expect(result.summary).toContain("0.75");
    const flagged = await diffshot({ baseline: { ...stored, threshold: 0.99 }, capture: { id: "cap-new", bytes: "data:image/png;base64,new", width: 400, height: 800 }, diff: walk, blocks: { cols: 4, rows: 3 }, stepid: "st5", now });
    expect(flagged.regression).toBe(true);
    const unthresholded = await diffshot({ baseline: baseline(), capture: { id: "cap-new", bytes: "data:image/png;base64,new", width: 400, height: 800 }, diff: walk, blocks: { cols: 4, rows: 3 }, stepid: "st5", now });
    expect(unthresholded.regression).toBe(false);
    expect(unthresholded.summary).toContain("no user threshold configured");
    const mismatch: pixeldiff = async (_baseline, _capture, grid) => ({ width: 400, height: 800, blocks: Array.from({ length: grid.cols * grid.rows - 1 }, () => 0) });
    await expect(diffshot({ baseline: stored, capture: { id: "cap-new", bytes: "x", width: 400, height: 800 }, diff: mismatch, now })).rejects.toThrow(/exactly the reviewed blocks/);
...
describe("forensics thumbshot sizing and timelapse assembly", () => {
  it("scales the thumbnail inside the user edge with the aspect ratio held while the lapse plans its frames on the interval and assembles them into the ordered sequence", () => {
    ...
    expect(() => thumbshot({ runid: "run1", capture: { id: "cap4", width: 100, height: 100 }, edge: 0, now })).toThrow(/positive number of pixels/);
    ...
    expect(() => timelapse({ runid: "run1", interval: 0, duration: 2000, startedat: now })).toThrow(/positive number of milliseconds/);
    expect(assemblelapse([{ runid: "run1", captureid: "f3", sequence: 3, at: now + 1000 }, { runid: "run1", captureid: "f1", sequence: 1, at: now }, { runid: "run1", captureid: "f2", sequence: 2, at: now + 500 }]).map(frame => frame.captureid)).toEqual(["f1", "f2", "f3"]);
    expect(assemblelapse([{ runid: "run1", captureid: "old", sequence: 2, at: now }, { runid: "run1", captureid: "new", sequence: 2, at: now + 100 }])).toHaveLength(1);
...
describe("forensics namecaptures uniqueness and exportcaptures bundles", () => {
  it("stamps the lowercase file names unique per run with the counter bump while the bundle assembles only with the provlog provenance of every capture and the redaction masks", () => {
    ...
    const named = namecaptures({ rule, parts: [
    ...
    const retaken = namecaptures({ rule, parts: [{ plan: "Run One", step: "Step A", timestamp: now, sequence: 1 }], taken: named.taken });
    ...
    const provlog: provlogentry[] = [
      { id: "prov1", pipelineid: "run1", runid: "run1", operation: "capture", rowkeys: ["cap1"], summary: "The capture cap1 bundles with its provenance.", at: now },
      { id: "prov2", pipelineid: "run1", runid: "run1", operation: "capture", rowkeys: ["cap2"], summary: "The capture cap2 bundles with its provenance.", at: now },
    ];
    const bundle = exportcaptures({ runid: "run1", captures: ["cap1", "cap2"], pairs: ["pair1"], consoleentries: 4, netentries: 2, diffs: ["diff1"], thumbnails: ["thumb1"], lapses: ["f1", "f2"], names: [{ captureid: "cap1", name: "run-one-1" }, { captureid: "cap2", name: "run-one-2" }], provlog, masked: true, now });
    ...
    expect(() => exportcaptures({ runid: "run1", captures: ["cap1", "cap3"], pairs: [], consoleentries: 0, netentries: 0, diffs: [], thumbnails: [], lapses: [], names: [], provlog, masked: true, now })).toThrow(/provenance covers 1 of them/);
    expect(() => exportcaptures({ runid: "run1", captures: ["cap1"], pairs: [], consoleentries: 0, netentries: 0, diffs: [], thumbnails: [], lapses: [], names: [], provlog: [], masked: true, now })).toThrow(/never leaves the device/);
    expect(() => exportcaptures({ runid: "run1", captures: ["cap1"], pairs: [], consoleentries: 0, netentries: 0, diffs: [], thumbnails: [], lapses: [], names: [], provlog, masked: false, now })).toThrow(/unmasked capture never exports/);
