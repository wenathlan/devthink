        ...
        if (content[index] === "$" && content[index + 1] === "{") {
          ...
          while (index < content.length && depth > 0) {
            if (content[index] === "{") depth += 1;
              ...
              index += 1;
              while (index < content.length && content[index] !== inner) {
                if (content[index] === "\\") index += 1;
                index += 1;
              }
            }
...
export function platformnames(name) {
  return (
    ["MAX_SAFE_INTEGER", "MIN_SAFE_INTEGER", "POSITIVE_INFINITY", "NEGATIVE_INFINITY"].includes(name) ||
    /^[A-Z][A-Z0-9]*(_[A-Z0-9]+)*$/.test(name) ||
    name.endsWith("_exports") ||
    name.endsWith("_default") ||
    wireformatnames(name)
  );
}
...
export function wireformatnames(name) {
  return (
    ...
    maeneconfigfields(name) ||
    gatewaywirefields(name)
  );
}
...
export function underscorednames(content) {
  return (content.match(/\b[A-Za-z][A-Za-z0-9]*_[A-Za-z0-9][A-Za-z0-9_]*\b/g) ?? []).filter(
