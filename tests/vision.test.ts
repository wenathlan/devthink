...
import { cropshot, frameocr, groundshot, imagehashof, imageocr, mergelines, mergeparagraphs, ocrtext, pairquery, pairshot, pdfocr, proposeredactionmasks, redactshot, regionocr, visioncacheput, visioncacheserve, visioncost, visionshot, type framegrab, type maskfill, type ocrread, type pdfrasterize, type videostateread, type visionsend } from "../capture.js";
import { framebudgetgate, groundgate, ocrgate, pairgate, redactgate, regionboundsgate, visioncacheexpirygate, visionchoices, visionconsentgate, visioncostgrade, visiongate, visionsensitivegrade } from "../policy.js";
...
function description(over: Partial<visiondescription> = {}): visiondescription {
  return { id: "d1", runid: "run1", stepid: "st1", imageid: "cap1", prompt: "Describe the checkout page.", text: "A checkout page with a total and a pay button.", regions: [{ label: "pay button", box: { x: 160, y: 700, width: 80, height: 40 } }, { label: "order total", box: { x: 40, y: 640, width: 120, height: 30 } }], at: now, ...over };
}
...
describe("vision imageocr recognition", () => {
  it("merges the recognized word boxes into lines and paragraphs by their own geometry while every word carries its confidence and the line reports the lowest one", async () => {
    ...
    const result = await imageocr({ record: { runid: "run1", stepid: "st1", imageid: "cap1" }, image: "data:image/png;base64,abc", read, id: "ocr1", now });
    ...
    await expect(imageocr({ record: { runid: "run1", stepid: "st1", imageid: "cap1" }, image: "  ", read, now })).rejects.toThrow(/needs its captured image/);
    expect(ocrgate("imageocr").allowed).toBe(true);
    expect(ocrgate("visioncall").allowed).toBe(false);
    expect(visionchoices({ visionmodel: "see-1", visionendpoint: "https://vision.example/model" })).toEqual({ model: "see-1", endpoint: "https://vision.example/model" });
...
describe("vision regionocr bounds and pdfocr page streaming", () => {
  it("clamps the drawn region into the viewport while the word boxes offset to absolute coordinates and pdfocr reads one page at a time through the streamcursor checkpoints", async () => {
    const read: ocrread = async (_image, region) => [{ text: "hello", box: { x: 0, y: 0, width: 50, height: 20 }, confidence: 0.9 }, { text: "world", box: { x: 5, y: 2, width: 50, height: 18 }, confidence: 0.8 }];
    const clamped = await regionocr({ record: { runid: "run1", stepid: "st1", imageid: "cap1" }, image: "data:image/png;base64,abc", region: { x: 380, y: 700, width: 120, height: 80 }, viewport: { width: 400, height: 800 }, read, id: "ocr2", now });
    ...
    await expect(regionocr({ record: { runid: "run1", stepid: "st1", imageid: "cap1" }, image: "data:image/png;base64,abc", region: { x: 500, y: 900, width: 40, height: 30 }, viewport: { width: 400, height: 800 }, read, now })).rejects.toThrow(/falls entirely outside/);
    const inside = regionboundsgate({ region: { x: 10, y: 10, width: 100, height: 50 }, viewport: { width: 400, height: 800 } });
    expect(inside.allowed).toBe(true);
    const crossing = regionboundsgate({ region: { x: 350, y: 10, width: 100, height: 50 }, viewport: { width: 400, height: 800 } });
    expect(crossing.allowed).toBe(true);
    expect(crossing.reason).toContain("clamps");
    expect(regionboundsgate({ region: { x: 450, y: 10, width: 100, height: 50 }, viewport: { width: 400, height: 800 } }).allowed).toBe(false);
    expect(regionboundsgate({ region: { x: 10, y: 10, width: 0, height: 50 }, viewport: { width: 400, height: 800 } }).allowed).toBe(false);
    const rasterized: string[] = [];
    const rasterize: pdfrasterize = async (documentid, page) => { rasterized.push(`${documentid}:p${page}`); return `data:image/png;base64,page${page}`; };
    const checkpoints: streamcursor[] = [];
    const pass = await pdfocr({ record: { runid: "run1", stepid: "st1", documentid: "scan-1" }, pages: 3, rasterize, read, checkpoint: cursor => { checkpoints.push({ ...cursor }); }, now });
    ...
    const resumed = await pdfocr({ record: { runid: "run1", stepid: "st1", documentid: "scan-1" }, pages: 3, rasterize, read, ...(checkpoints[0] !== undefined ? { cursor: checkpoints[0] } : {}), now });
    expect(resumed.skipped).toBe(1);
    expect(resumed.pagesread).toBe(2);
    expect(rasterized).toHaveLength(5);
    await expect(pdfocr({ record: { runid: "run1", stepid: "st1", documentid: "scan-1" }, pages: 3, rasterize, read, cursor: { pipelineid: "scan-other", offset: 1, chunk: 1, updatedat: now }, now })).rejects.toThrow(/another document/);
...
describe("vision frameocr pause enforcement", () => {
  it("refuses the frame of a playing video before any pixel moves while a paused video seeks to its reviewed position and reads the frame", async () => {
    ...
    const grabs: number[] = [];
    const grab: framegrab = async (_selector, positionms) => { grabs.push(positionms); return { image: "data:image/png;base64,frame", width: 640, height: 360 }; };
    await expect(frameocr({ record: { runid: "run1", stepid: "st1" }, frame: { runid: "run1", stepid: "st1", selector: "video.tutorial", positionms: 12_000, at: now }, state: stateread, grab, read, now })).rejects.toThrow(/refuses a moving picture/);
    expect(grabs).toHaveLength(0);
    stateindex = 1;
    const played = await frameocr({ record: { runid: "run1", stepid: "st1" }, frame: { runid: "run1", stepid: "st1", selector: "video.tutorial", positionms: 12_000, at: now }, state: stateread, grab, read, now });
    expect(grabs).toEqual([12_000]);
    expect(played.result.words[0]?.text).toBe("chapter");
    expect(played.frame.positionms).toBe(12_000);
    await expect(frameocr({ record: { runid: "run1", stepid: "st1" }, frame: { runid: "run1", stepid: "st1", selector: "video.tutorial", positionms: 90_000, at: now }, state: stateread, grab, read, now })).rejects.toThrow(/outside the video/);
...
describe("vision cropshot scaling and pairshot alignment", () => {
  it("scales the element crop into image pixels with the device pixel ratio awareness while the pair aligns with the nearest same viewport dom snapshot", () => {
    const crop = cropshot({ image: { width: 1280, height: 800 }, bounds: { x: 100, y: 300, width: 200, height: 80 }, pixelratio: 2 });
    ...
    const clamped = cropshot({ image: { width: 1280, height: 800 }, bounds: { x: 600, y: 380, width: 120, height: 60 }, pixelratio: 2 });
    expect(clamped.crop).toEqual({ x: 1200, y: 760, width: 80, height: 40 });
    expect(() => cropshot({ image: { width: 1280, height: 800 }, bounds: { x: 1400, y: 900, width: 60, height: 30 }, pixelratio: 2 })).toThrow(/fall entirely outside/);
    const paired = pairshot({ runid: "run1", stepid: "st1", image: { id: "cap1", capturetime: now, viewport: { width: 640, height: 400 } }, snapshots: [{ id: "dom-older", capturetime: now - 900, viewport: { width: 640, height: 400 } }, { id: "dom-nearest", capturetime: now - 80, viewport: { width: 640, height: 400 } }, { id: "dom-other-viewport", capturetime: now - 10, viewport: { width: 320, height: 200 } }], id: "pair1", now });
    ...
    const unmatched = pairshot({ runid: "run1", stepid: "st1", image: { id: "cap2", capturetime: now, viewport: { width: 640, height: 400 } }, snapshots: [{ id: "dom-other-viewport", capturetime: now - 10, viewport: { width: 320, height: 200 } }], now });
    ...
    const query = pairquery({ query: "Order total", ocrtextvalue: "The Order total: 42.50 appears once.", domtext: "Checkout summary shows the order total row." });
    expect(query.matches.map(match => match.source).sort()).toEqual(["dom", "image"]);
    expect(query.matches[0]?.excerpt).toContain("order total");
    expect(pairquery({ query: "missing needle", ocrtextvalue: "nothing here", domtext: "nothing there" }).matches).toHaveLength(0);
...
describe("vision redactshot masking and audit records", () => {
  it("applies the redactionmask through the fill seam before any sharing while the proposals derive from the sensitive field shapes and the audit summary records the covered regions", async () => {
    ...
    const fill: maskfill = async (image, regions) => { filled.push(...regions.map(region => ({ ...region }))); return `${image}:masked`; };
    const mask: redactionmask = { id: "mask1", runid: "run1", captureid: "cap1", regions: [{ x: 10, y: 20, width: 100, height: 30 }, { x: 380, y: 700, width: 120, height: 40 }], reason: "the payment card digits stay on the device", source: "userdrawn", at: now };
    ...
    await expect(redactshot({ image: "data:image/png;base64,abc", width: 400, height: 800, mask: { ...mask, regions: [] }, fill })).rejects.toThrow(/masks nothing/);
    await expect(redactshot({ image: "data:image/png;base64,abc", width: 400, height: 800, mask: { ...mask, regions: [{ x: 900, y: 900, width: 10, height: 10 }] }, fill })).rejects.toThrow(/falls outside the capture/);
    const proposals = proposeredactionmasks({ runid: "run1", captureid: "cap1", fields: [{ name: "cardnumber", rect: { x: 10, y: 20, width: 120, height: 24 } }, { name: "quantity", rect: { x: 10, y: 60, width: 60, height: 24 } }, { name: "password", rect: { x: 10, y: 100, width: 90, height: 24 } }], now });
    ...
    expect(visiongate({ endpoint: "https://vision.example/model", model: "see-1", granted: ["https://vision.example"] }).allowed).toBe(true);
    expect(visiongate({ endpoint: "https://outside.example/model", model: "see-1", granted: ["https://vision.example"] }).allowed).toBe(false);
...
describe("vision groundshot ranking and visioncache hits", () => {
  it("ranks the grounding candidates by the averaged text and geometry scores while the visioncache serves the repeated image hash without a model call under the user retention", async () => {
    ...
    const grounded = groundshot({ description: description(), elements, pixelratio: 2, record: { runid: "run1", stepid: "st1" }, id: "g1", now });
    ...
    expect(grounded.matches[0]?.score).toBeGreaterThan(grounded.matches.find(match => match.selector === "button.pay")?.score ?? 0);
    ...
    expect(() => groundshot({ description: { ...description(), regions: [] }, elements, pixelratio: 1, record: { runid: "run1", stepid: "st1" }, now })).toThrow(/needs its labeled regions/);
    const send: visionsend = async () => ({ text: "A checkout page.", regions: [{ label: "pay button", box: { x: 160, y: 700, width: 80, height: 40 } }] });
    const described = await visionshot({ record: { runid: "run1", stepid: "st1", imageid: "cap1" }, image: "data:image/png;base64,abc", prompt: "Describe the page.", send, id: "d2", now });
    expect(described.prompt).toBe("Describe the page.");
    expect(described.regions[0]?.label).toBe("pay button");
    await expect(visionshot({ record: { runid: "run1", stepid: "st1", imageid: "cap1" }, image: "data:image/png;base64,abc", prompt: "   ", send, now })).rejects.toThrow(/non-empty reviewed string/);
    const hash = imagehashof("data:image/png;base64,abc");
    const stored = visioncacheput({ entries: [], entry: { hash, runid: "run1", imageid: "cap1", kind: "ocr", ocr: { id: "ocr1", runid: "run1", stepid: "st1", imageid: "cap1", text: "Order total 42.50", words: [], lines: [], paragraphs: [], at: now }, hits: 0, at: now } });
    expect(stored).toHaveLength(1);
    const replaced = visioncacheput({ entries: stored, entry: { hash, runid: "run1", imageid: "cap1", kind: "ocr", ocr: { id: "ocr2", runid: "run1", stepid: "st1", imageid: "cap1", text: "Order total 42.50", words: [], lines: [], paragraphs: [], at: now }, hits: 3, at: now } });
    ...
    const missed = visioncacheserve({ entries: replaced, hash: imagehashof("data:image/png;base64,other"), kind: "ocr", now: now + 1000 });
    ...
    const cost = visioncost({ runid: "run1", calls: [{ kind: "imageocr", model: true, at: now }, { kind: "pairshot", model: false, at: now }, { kind: "visionshot", model: true, at: now }] });
