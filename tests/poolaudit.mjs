...
const stopwords = new Set(["the", "a", "an", "by", "for", "with", "of", "to", "in", "on", "at", "into", "from", "before", "after", "across", "when", "via", "and", "or", "its", "their", "this", "that", "one", "two", "several", "any", "all", "new", "existing", "user", "users", "page", "pages", "tab", "tabs", "browser", "browsers", "current", "open", "opened", "set", "get", "without", "between", "during", "per", "each", "more", "than", "over", "under", "out", "up", "down", "off", "can", "not", "only", "also", "then", "them", "they", "some", "such", "other", "another", "same", "different", "based", "like", "using", "use", "used"]);
...
async function symbolcorpus() {
  ...
  const auditkinds = [...(await readFile("types.ts", "utf8")).matchAll(/^\s*"([a-z][a-z0-9]+)",?$/gm)].map(match => match[1]);
  ...
  const declarations = (await readdir("dist")).filter(file => file.endsWith(".d.ts")).map(file => readFile(join("dist", file), "utf8"));
  for (const text of await Promise.all(declarations)) {
    for (const match of text.matchAll(/(?:interface|type|function|const|class)\s+([a-z][a-z0-9]+)/g)) corpus.add(match[1]);
  }
  return corpus;
}
...
async function parsepool() {
  ...
  if (poolstart === -1) throw new Error("The consolidated feature pool heading of docs/12.crxfeaturemining.md is absent.");
  const pooltext = text.slice(poolstart);
  const sections = pooltext.split(/^### /m).slice(1);
  const items = [];
  for (const section of sections) {
    const group = section.split("\n")[0].trim();
    for (const match of section.matchAll(/^(\d+)\. (.+)$/gm)) items.push({ id: Number(match[1]), group, text: match[2].trim() });
  }
  return items;
}
...
function dispositionof(item, corpus) {
  const tokens = item.text.toLowerCase().split(/[^a-z0-9]+/).filter(token => token.length >= 3 && !stopwords.has(token));
  if (tokens.length === 0) return { status: "unknown", tokens: 0, matched: 0 };
  const matched = tokens.filter(token => [...corpus].some(symbol => symbol.includes(token) || (token.includes(symbol) && symbol.length >= 4) || stems(token).some(form => form.length >= 4 && symbol.includes(form))));
...
export async function runpoolauditsuite() {
  ...
  if (!existsSync("dist/index.js") || !existsSync("dist/policy.js") || !existsSync("dist/protocol.js")) {
    record("compiled bundles exist for the corpus", false, "the dist bundles are absent; run pnpm build before the pool audit");
    return { release, checks, summary: { total: checks.length, passed: 0, failed: 1 }, pool: { total: 0, implemented: 0, planned: 0, unknown: 0 } };
  }
  const corpus = await symbolcorpus();
  const pool = await parsepool();
  record("the consolidated pool parses to the 626 items", pool.length === expectedcount, `the pool parsed ${pool.length} items of ${expectedcount} expected under the context sections`);
  ...
  record("every pool item carries its disposition", counts.unknown === 0, counts.unknown === 0 ? `every item matched the shipped symbol corpus of ${corpus.size} symbols: ${counts.implemented} implemented, ${counts.planned} planned` : `${counts.unknown} items stayed unknown against the corpus`);
  record("the implemented count holds the recorded floor", counts.implemented >= implementedfloor, `${counts.implemented} implemented of the floor ${implementedfloor}; a regression below the floor fails the release`);
  const groups = {};
...
if (invokeddirectly) {
  ...
  console.log(`Pool audit of ${report.pool.total} items against ${report.corpus} shipped symbols: ${report.pool.implemented} implemented, ${report.pool.planned} planned, ${report.pool.unknown} unknown.`);
