...
describe("sessionmemory", () => {
  ...
  it("stores every clickable map under its observation version", async () => {
    ...
    await store.setmap({
      version: first,
      builtat: 10,
      entries: [{ number: 1, selector: "#go", role: "button", label: "Go", mode: "selector" }],
    });
    await store.setmap({
      version: second,
      builtat: 20,
    ...
    expect((await store.getmap(second))?.entries[1]).toMatchObject({
      number: 2,
  ...
  it("keeps the navigation trail of a session with timestamps and step refs", async () => {
    ...
    await store.addtrailentry("session", {
  ...
  it("stores wait profiles per origin with user configured values", async () => {
    ...
    await store.setwaitprofile({
      origin: "https://example.com",
      profile: { signals: ["load", "networkidle"], idle: 900, timeout: 8000 },
      at: 20,
    });
    ...
    expect(profiles.find((record) => record.origin === "https://example.com")?.profile).toMatchObject({
      idle: 900,
      timeout: 8000,
    });
  ...
  it("stores redirect chains and final urls per navigation step", async () => {
    ...
    const chain = {
      hops: [
        { url: "https://example.com/a", status: 302, at: 10 },
        { url: "https://example.com/b", status: 200, at: 25 },
      ],
      startedat: 10,
      endedat: 25,
    };
    await store.addnavrecord({
      stepid: "s1",
      sessionid: "session",
      origin: "https://example.com",
  ...
  it("stores navintent records, rate limit windows, curated lists and safety verdicts", async () => {
    const store = new sessionmemory(new fakeadapter());
    await store.addnavintent({
      id: "i1",
      intent: "open the pricing page",
      origin: "https://example.com",
      sessionid: "session",
      stepid: "s1",
      at: 10,
    });
    expect((await store.getnavintents())[0]).toMatchObject({
      intent: "open the pricing page",
      origin: "https://example.com",
    });
    await store.setratestate({
      domain: "example.com",
      limit: { domain: "example.com", window: 60000, ceiling: 3 },
      openedat: 100,
      count: 1,
    });
    await store.setratestate({
      domain: "example.com",
      limit: { domain: "example.com", window: 60000, ceiling: 5 },
      openedat: 200,
      count: 0,
    });
    await store.setratestate({
      domain: "other.example",
      limit: { domain: "other.example", window: 30000, ceiling: 2 },
      openedat: 300,
      count: 0,
    });
    ...
    await store.addcurated({
      id: "c1",
      links: [
        { url: "https://example.com/a", verdict: "safe", reasons: [] },
        { url: "http://bad.example", verdict: "unsafe", reasons: ["the url must use HTTPS"] },
      ],
      at: 10,
    });
...
describe("sessionmemory tabs and windows command", () => {
  ...
  it("stores tabgroup definitions with their color choices and member tabs", async () => {
    ...
    expect(groups.find((group) => group.name === "research")).toMatchObject({
      color: "pink",
      collapsed: true,
      tabids: [4],
    });
  ...
  it("stores tabmeta records with task provenance per tab", async () => {
    ...
    expect(metas.find((meta) => meta.tabid === 4)).toMatchObject({
      taskrefs: ["plan-2"],
      labels: ["final"],
  ...
  it("stores session snapshots and the closed tab history for later restore", async () => {
    ...
    const layout = {
      name: "run",
      tabs: [{ url: "https://example.com/a", title: "A", pinned: false, index: 0, windowid: 10 }],
      groups: [],
      windows: [],
      savedat: 10,
    };
  ...
  it("stores form profiles with origin grants, replaces them by name and removes them", async () => {
    ...
    const profile = {
      name: "personal",
      fields: [{ match: { mode: "label" as const, label: "Full name" }, kind: "text" as const, value: "Ana Alves" }],
    ...
    await store.setprofile({
      ...
      fields: [
        ...profile.fields,
        { match: { mode: "name" as const, name: "email" }, kind: "email" as const, value: "a@b.c" },
      ],
      savedat: 30,
    });
  ...
  it("stores datasets, imports, extract sessions, streams, provenance, task rules and sheet endpoints", async () => {
    ...
    await store.setdataset({
      id: "d1",
      name: "catalog",
      columns,
      rows,
      sources: [{ row: 0, url: "https://example.com", at: 1, stepid: "s1" }],
      at: 3,
    });
    ...
    await store.settaskrules({
      taskid: "plan-1",
      transforms: [{ expression: "trim", sources: ["sku"], target: "sku" }],
      dedupekeys: [],
      at: 10,
    });
    await store.settaskrules({
    ...
    await store.setsheetendpoint({
      endpoint: "https://sheets.example/tab",
...
describe("sessionmemory files, clipboard and downloads", () => {
  it("stores downloads, netlogs with retention, clip consents, clips and quarantines", async () => {
    ...
    await store.setclipconsent({
      id: "clip-1",
      prompt: "Read the tracking number.",
      origin: "https://example.com",
      stepid: "s1",
      at: 7,
    });
    ...
    await store.setquarantine({
      id: "q1",
      path: "devthink-quarantine/x.pdf",
      reason: "mime matched",
      scan: "pending",
      at: 9,
      updatedat: 9,
  ...
  it("applies netlog retention and keeps every record without the setting", async () => {
    ...
    expect((await bounded.getnetlog()).map((record) => record.url)).toEqual([
      "https://example.com/3",
      "https://example.com/2",
    ]);
  });
  ...
  it("stores cleanup rules, run history, capture counters, inventories, scan hooks and mime filters", async () => {
    ...
    await store.setscanhook({
      scanner: "clamav",
      endpoint: "https://scanner.example/verdict",
    ...
    await store.setscanhook({
      scanner: "second",
      endpoint: "https://other.example/v",
      origin: "https://other.example",
      configuredat: 7,
    });
    await store.setscanhook({
      scanner: "clamav",
      endpoint: "https://scanner.example/v2",
      origin: "https://scanner.example",
      configuredat: 8,
    });
...
describe("capture memory", () => {
  it("stores capture records with bytes, filters by run, step and kind, and returns pairs of one run", async () => {
    ...
    await store.addcapture({
      ...
      height: 800,
      capturedat: 1,
      bytes: "data:image/png;base64,aaa",
    });
    await store.addcapture({
      id: "c2",
      runid: "run-1",
      ...
      height: 4000,
      capturedat: 2,
      bytes: "data:image/png;base64,bbb",
    });
    await store.addcapture({
      id: "c3",
      runid: "run-2",
      ...
      height: 400,
      capturedat: 3,
      bytes: "data:image/webp;base64,ccc",
    });
...
describe("media memory", () => {
  it("stores media records per run, filters by run and kind and returns recordings with their file reference", async () => {
    ...
    await store.addmedia({
      ...
      name: "run-1-s1-1-capturepdf.pdf",
    });
    await store.addmedia({
      id: "rec-1",
      runid: "run-1",
      ...
      fps: 2,
      frames: ["f1", "f2"],
      file: "run-1-s2-2-recordscreen.json",
      bytes: 300,
      at: 2,
    });
    await store.addmedia({
    ...
    expect((await store.getmediarecords()).map((record) => record.id)).toEqual([
      "asset-1",
      "stream-1",
      "canvas-1",
      "frame-1",
    ...
    expect(await store.getrecording("rec-1")).toMatchObject({
      kind: "screen",
      file: "run-1-s2-2-recordscreen.json",
      frames: ["f1", "f2"],
      duration: 5,
  ...
  it("expires media bytes after the retention window while keeping the recording index and metadata", async () => {
    ...
    expect(
      (await unbounded.getmediarecords()).every(
        (record) => (record as { bytesexpired?: boolean }).bytesexpired === undefined,
      ),
    ).toBe(true);
  });
  ...
  it("stores image batches with match counts and recording consent decisions per origin", async () => {
    ...
    await store.addimagebatch({
      ...
      runid: "run-1",
      stepid: "s1",
      images: [{ url: "https://example.com/b.png", alt: "", width: 50, height: 50, bytes: 10, mime: "image/png" }],
      matched: 1,
      downloaded: 0,
      at: 3,
    });
    expect((await store.getimagebatches()).map((batch) => batch.id)).toEqual(["batch-1", "batch-2"]);
    expect((await store.getimagebatches())[0]).toMatchObject({
      matched: 1,
      downloaded: 0,
      images: [{ url: "https://example.com/b.png" }],
    });
    await store.setrecordingconsent({
      id: "consent-1",
      prompt: "Record the checkout flow.",
      origin: "https://example.com",
      stepid: "s3",
      at: 4,
    });
...
describe("network observation memory", () => {
  const call = (id: string, origin: string, body?: string): callrecord => ({
    id,
    runid: "run-1",
    stepid: "f1",
    kind: "fetch",
  ...
  it("stores endpoint definitions with version history and returns the newest schema", async () => {
    ...
    await store.setendpoint({
      name: "issues",
      method: "GET",
      url: "https://api.example/v2/issues",
      schema,
      version: 1,
      at: 2,
    });
...
describe("network observation part two memory", () => {
  it("stores channel records with message counters and stream subscriptions", async () => {
    ...
    const subscription = {
      ...
      events: 0,
      names: [] as string[],
      cancel: { kind: "stop" as const, value: "done" },
      openedat: 1,
    };
  ...
  it("queues message envelopes with sequence integrity and drains consumed matches", async () => {
    ...
    expect((await memory.getmessages("ch1", "orders")).map((envelope) => envelope.payload)).toEqual([
      '{"id":1}',
      '{"id":2}',
    ]);
    await memory.addmessage({ channelid: "ch1", stream: "orders", payload: '{"id":1}', sequence: 1, at: 1 });
    ...
    await memory.drainmessages([
      { channelid: "ch1", sequence: 1 },
      { channelid: "ch1", sequence: 3 },
    ]);
  ...
  it("expires captured bodies after the configured retention window while the metadata survives", async () => {
    const memory = new sessionmemory(new fakeadapter());
    const body = (ref: string, correlationid: string): bodyrecord => ({
      ref,
      runid: "run",
      correlationid,
      url: "https://api.example/items",
  ...
  it("stores the page api map per origin", async () => {
    ...
    await memory.setapimap("https://other.example", [
      { ...entry, origin: "https://other.example", endpoint: "https://other.example/x" },
    ]);
...
describe("network control storage", () => {
  ...
  it("stores token metadata per provider without token values and lists per provider", async () => {
    ...
    await store.addtoken({
      ...
      refreshstorageid: "token-t1-refresh",
      expiresat: 99,
      at: 1,
    });
    await store.addtoken({
      ...
      origin: "https://other.example",
      scopes: ["write"],
      accessstorageid: "token-t2-access",
      expiresat: 99,
      at: 2,
    });
...
describe("run timeline storage", () => {
  ...
  it("stores error and rejection records with stack frames and long tasks with attributions", async () => {
    const store = new sessionmemory(new fakeadapter());
    await store.adderrorrecord({
      id: "e1",
      runid: "run1",
      stepid: "s1",
      message: "boom",
      frames: [{ functionname: "load", url: "https://example.com/app.js", line: 42, column: 7 }],
      sourceurl: "https://example.com/app.js",
      line: 42,
      at: 1,
    });
    await store.addrejectionrecord({
      id: "r1",
      runid: "run1",
      stepid: "s1",
      reason: "TypeError: failed",
      frames: [],
      at: 2,
    });
    await store.addlongtask({
      id: "l1",
      runid: "run1",
      stepid: "s1",
      duration: 90,
      starttime: 10,
      attributions: ["same-origin"],
      at: 3,
    });
  ...
  it("stores the console diff result, rotation targets and per origin console consents", async () => {
    const store = new sessionmemory(new fakeadapter());
    await store.addconsolediff({
      base: "run1",
      target: "run2",
      lines: [{ kind: "added", text: "fresh" }],
      added: 1,
      removed: 0,
      repeated: 0,
      at: 9,
...
describe("devtools session storage", () => {
  ...
  it("stores event rules, breakpoints with hit counts, watch expressions and overrides with provenance", async () => {
    ...
    await store.setwatchexpression({
      id: "we1",
      runid: "plan",
      stepid: "w1",
    ...
    await store.setwatchexpression({
      ...
      reviewed: true,
      values: [
        { pauseid: "p1", value: "3", at: 4 },
        { pauseid: "p2", value: "8", at: 6 },
      ],
      at: 2,
    });
    expect((await store.getwatchexpressions())[0]?.values).toHaveLength(2);
    await store.addscriptoverride({
      id: "ov1",
      runid: "plan",
      stepid: "o1",
  ...
  it("stores pause states per run and expires the call frames under the retention window", async () => {
    ...
    await store.addpause({
      ...
      stepid: "b1",
      reason: "step",
      callframes: [{ url: "https://example.com/app.js", line: 6 }],
      at: 2,
    });
    ...
    await retained.addpause({
      ...
      stepid: "b1",
      reason: "step",
      callframes: [{ url: "https://example.com/app.js", line: 7 }],
      at: 3,
    });
  ...
  it("stores debugger consents per origin and revokes every approved record of one origin", async () => {
    const store = new sessionmemory(new fakeadapter());
    await store.setdebuggergrant({
      id: "g1",
      prompt: "Debugger attach on https://example.com",
      origin: "https://example.com",
      domains: ["Runtime"],
      approved: true,
      consentedat: 1,
    ...
    await store.setdebuggergrant({
      id: "g2",
      prompt: "Debugger attach on https://api.example",
      origin: "https://api.example",
      domains: ["Network"],
      approved: true,
      consentedat: 2,
...
describe("profiler storage", () => {
  ...
  it("stores growth samples with computed trends and cpu profiles with hot functions", async () => {
    ...
    await store.setcpuprofile({
      ...
      duration: 500,
      samplecount: 3,
      hotfunctions: ["render", "parse"],
      at: now,
    });
    expect((await store.getcpuprofiles())[0]).toMatchObject({
      duration: 500,
      samplecount: 3,
      hotfunctions: ["render", "parse"],
    });
  ...
  it("stores shift entries, traces with annotations and source map references with per origin consents", async () => {
    ...
    await store.settracerecord({
      id: "t1",
      runid: "plan",
      stepid: "tl",
      origin: "https://example.com",
      categories: ["scripting", "network"],
      bytesize: 400,
      events: 2,
    ...
    expect(
      (await store.listtraces({ runid: "plan", categories: ["scripting", "network"] })).map((trace) => trace.id),
    ).toEqual(["t1"]);
    ...
    await store.setsourcemapconsent({
      id: "c1",
...
describe("emulation memory", () => {
  it("stores the emulation state per run with the layer history and expires reverted prior states after the retention window", async () => {
    ...
    await store.setemulationstate({
      ...
      tabid: 4,
      origin: "https://example.com",
      layers: [{ ...layer, revertedat: Date.now() - 5 }],
      updatedat: 3,
    });
  ...
  it("stores blackbox rule sets per origin, permission override history and location consents", async () => {
    const store = new sessionmemory(new fakeadapter());
    await store.setblackboxrules("https://example.com", [
      { urlpatterns: ["https://cdn.example/**"], tracescope: "both" },
    ]);
    await store.setblackboxrules("https://example.com", [
      { urlpatterns: ["https://vendor.example/*"], tracescope: "traces" },
    ]);
    const rules = await store.getblackboxrules();
    expect(rules).toHaveLength(1);
    expect(rules[0]?.rules[0]?.urlpatterns).toEqual(["https://vendor.example/*"]);
    await store.addpermissionoverride({
      id: "p1",
      runid: "run",
      stepid: "s1",
      origin: "https://example.com",
    ...
    await store.setlocationconsent({
      id: "c1",
      prompt: "Where?",
      origin: "https://example.com",
      latitude: 38.7223,
...
describe("workflow editor memory", () => {
  it("stores workflow versions and version diffs for the timeline", async () => {
    const store = new sessionmemory(new fakeadapter());
    await store.addworkflowversion({
      workflowid: "wf1",
      version: 1,
      createdat: 1,
      note: "first",
    ...
    await store.addversiondiff({
      workflowid: "wf1",
      from: 1,
      to: 2,
      added: [{ stepid: "s3", kind: "wait", label: "Fresh" }],
      removed: [],
      changed: [],
      at: 4,
    });
    await store.addversiondiff({
      ...
      added: [],
      removed: [],
      changed: [{ stepid: "s1", kind: "readtext", label: "Open", changes: ["label"] }],
      at: 5,
    });
  ...
  it("stores run history with retention and answers the filters", async () => {
    ...
    for (const [index, outcome] of ["done", "failed", "done", "paused"].entries()) {
      await store.addrunhistory({
        runid: `r${index}`,
        workflowid: index % 2 === 0 ? "wf1" : "wf2",
        outcome,
        steps: index,
        total: 4,
        duration: index,
  ...
  it("stores editor layouts, breakpoints, overrides, watchdog events, imports and background flags", async () => {
    ...
    expect(await store.geteditorlayout("wf1")).toEqual({
      width: 900,
      height: 700,
      viewportx: 12,
      viewporty: 34,
...
describe("agent protocol memory", () => {
  it("stores client records with pairing state, negotiated capability sets and the server config and state", async () => {
    ...
    await store.setmcpstate({
      state: "running",
      startedat: 20,
      bridge: {
        id: "bridge1",
        host: "com.wenathlan.devthink",
        connected: true,
        pid: 4242,
        startedat: 20,
        restarts: 0,
  ...
  it("records tool list, negotiate, connect and tool call audit events", async () => {
    ...
    await store.addaudi({
      ...
      kind: "protocol",
      at: 50,
      summary: "A new mcp client client1 connected on the stdio transport and waits for the pairing approval.",
    });
    await store.addaudi({
      id: "a2",
      kind: "protocol",
      at: 51,
      summary: "The mcp client client1 listed the tool catalog of 30 tools.",
    });
    await store.addaudi({
      id: "a3",
      kind: "protocol",
      at: 52,
      summary: "The mcp client client1 negotiated its capability set with the server.",
    });
    await store.addaudi({
      id: "a4",
      kind: "tool",
      at: 53,
      summary:
        "The mcp client client1 called the browser.click tool on https://example.com and it ran behind the consent gates.",
      sessionid: "sess",
    });
...
describe("sessionmemory agent protocol part two", () => {
  ...
  it("stores session tokens only in their tokenhash form", async () => {
    const store = new sessionmemory(new fakeadapter());
    await store.setsessiontokens([
      { id: "t1", clientid: "client1", hash: "sha256:abc", scopes: ["browser"], issuedat: now, expiresat: now + 1000 },
    ]);
    ...
    await store.setsessiontokens([
      ...tokens,
      { id: "t2", clientid: "client2", hash: "sha256:def", scopes: ["memory"], issuedat: now, expiresat: now + 1000 },
    ]);
  ...
  it("stores allowlist entries, client identities and stream channels", async () => {
    ...
    await store.setallowlistentry({
      fingerprint: "aa11",
      displayname: "Laptop agent",
      namespaces: ["browser"],
      grantedat: now,
      history: [{ at: now, actor: "user", change: "Granted." }],
    });
    await store.setallowlistentry({
      fingerprint: "bb22",
    ...
    await store.setallowlistentry({
      ...
      namespaces: ["system"],
      grantedat: now,
      history: [{ at: now + 1, actor: "user", change: "Rescoped." }],
    });
    ...
    await store.setstreamchannels([
      { id: "c1", clientid: "client1", openedat: now, lastbeatat: now, closedat: now + 5 },
    ]);
...
describe("sessionmemory agent protocol part three", () => {
  ...
  it("stores subscriptions, watches and sampling requests with their provenance", async () => {
    ...
    await store.setresourcewatches([
      { id: "watch1", clientid: "client1", resource: "page", baseline: { url: "https://example.com" }, createdat: now },
    ]);
    ...
    await store.setsamplingrequests([
      { ...sampling, state: "answered" as const, answeredat: now + 1, answer: "the model answer" },
    ]);
  ...
  it("stores idempotency records, rate limit counters and batch outcomes", async () => {
    const store = new sessionmemory(new fakeadapter());
    await store.setidempotencyrecords([
      {
        key: "key-1",
        clientid: "client1",
        tool: "browser.readtext",
    ...
    const batch = {
      id: "batch1",
      clientid: "client1",
      calls: [{ id: "m1", name: "browser.readtext", params: {} }],
      stoponerror: true,
    ...
    await store.setbatchcall({
      ...batch,
      state: "done" as const,
      finishedat: now + 1,
      outcomes: [{ callid: "m1", tool: "browser.readtext", ok: true, at: now + 1 }],
    });
  ...
  it("stores call contexts, chunks, progress notices, mocks and the dry run toggle", async () => {
    const store = new sessionmemory(new fakeadapter());
    await store.setcallcontexts([
      { callid: "call1", clientid: "client1", tool: "browser.readtext", state: "inflight", startedat: now, chunks: 0 },
    ]);
    expect((await store.getcallcontexts())[0]?.state).toBe("inflight");
    for (let index = 0; index < 30; index += 1)
      await store.addstreamchunk({
        callid: "call1",
        seq: index + 1,
        content: "x",
        done: index === 29,
    ...
    for (let index = 0; index < 30; index += 1)
      await store.addprogressnotice({ callid: "call1", message: "working", cancellable: true, at: now + index });
    expect(await store.getprogressnotices()).toHaveLength(25);
    await store.settoolmock({
      tool: "browser.readtext",
...
describe("sessionmemory llm integration", () => {
  ...
  it("stores plandrafts, replans and reflectnotes for the audit history", async () => {
    ...
    const draft: plandraft = {
      id: "d1",
      goal: "Read the page",
      steps: [{ id: "s1", kind: "observe", summary: "Observe." }],
      openquestions: [],
      providerid: "prov1",
      model: "model-a",
      state: "approved",
    ...
    const note: reflectnote = {
      ...
      runid: "run1",
      stepid: "s1",
      outcome: "The step landed.",
      lesson: "Wait before the click.",
      advice: "Add a settle window.",
      providerid: "prov1",
      model: "model-a",
      createdat: now,
    };
  ...
  it("stores the parsed command and the guard refusal notices with their reasons", async () => {
    ...
    const parse = {
      ...
      entities: [{ name: "url", value: "https://docs.example" }],
      confidence: 0.9,
      model: "model-a",
      providerid: "prov1",
      parsedat: now,
...
describe("sessionmemory swarm persistence", () => {
  ...
  it("stores the agent identities, the task queue and the blackboard as first class records", async () => {
    ...
    await store.setagents([
      { id: "a1", name: "Scout", role: "worker", depth: 0, state: "active", tabid: 7, registeredat: now },
    ]);
    expect((await store.getagents())[0]).toMatchObject({ name: "Scout", tabid: 7, role: "worker" });
    await store.settaskqueue({
      lanes: ["extraction"],
      priorities: [1],
      completionpolicy: "all",
      items: [
        { id: "t1", lane: "extraction", priority: 1, payload: "Read the table", state: "queued", enqueuedat: now },
      ],
      claims: [],
    });
    expect((await store.gettaskqueue())?.items).toHaveLength(1);
    await store.setblackboard({
      sections: ["goals", "facts", "findings", "scratch"],
      entries: [
        {
          id: "e1",
          key: "goal",
  ...
  it("stores the lessons learned of the fleet with one row per lesson id", async () => {
    ...
    expect(stored.find((lesson) => lesson.id === "l1")?.finding).toBe(
      "The pricing table loads only after the hero settles.",
    );
  });

  it("stores the mailbox contents with the user configured retention", async () => {
    const store = new sessionmemory(new fakeadapter());
    await store.setmailboxes([
      {
        agentid: "a1",
        inbox: [
          { id: "m1", senderid: "a2", recipient: "a1", routing: "direct", payload: "One", sentat: now },
          { id: "m2", senderid: "a2", recipient: "a1", routing: "direct", payload: "Two", sentat: now + 1 },
        ],
        outbox: [],
        unread: 2,
      },
  ...
  it("stores the spawn history, the per agent usage and the lifecycle events", async () => {
    ...
    await store.addagentevent({
      id: "e1",
      kind: "register",
      agentid: "a1",
      summary: "The user registered the agent Scout.",
      at: now,
    });
  ...
  it("returns the swarm overview at a glance from the stored records", async () => {
    ...
    await store.setmailboxes([
      {
        agentid: "a2",
        inbox: [{ id: "m1", senderid: "a1", recipient: "a2", routing: "direct", payload: "Hi.", sentat: now }],
        outbox: [],
        unread: 1,
      },
    ]);
...
describe("sessionmemory orchestration persistence", () => {
  ...
  it("stores the topology, splits, reviews and verifier checks of the 1.1.59 family", async () => {
    const store = new sessionmemory(new fakeadapter());
    await store.settopology({
      id: "top1",
      leaderid: "a1",
      workerids: ["a2"],
      criticids: ["a3"],
      verifierids: ["a4"],
      assignments: [{ workerid: "a2", taskid: "t1", slice: "Read the table", assignedat: now }],
      rule: { kind: "first" },
      electedat: now,
    });
    expect((await store.gettopology())?.leaderid).toBe("a1");
    await store.setplannersplits([
      {
        id: "s1",
        planownerid: "a1",
        runownerid: "a2",
        taskid: "t1",
        stepreports: [{ stepid: "step1", outcome: "done", detail: "Done.", reportedat: now }],
        splitat: now,
      },
    ]);
    expect((await store.getplannersplits())[0]?.stepreports).toHaveLength(1);
    await store.addcriticreview({
      id: "r1",
      reviewerid: "a3",
      subjectagentid: "a2",
      verdict: "changes",
  ...
  it("stores the handoff log with the resumed state, the locks and the conflict scans", async () => {
    ...
    await store.addconflictscan({
      id: "s1",
      writers: [{ agentid: "a1", origin: "https://example.com", selector: "#form" }],
      overlaps: [],
      suggestedorder: [],
      clean: true,
      scannedat: now,
  ...
  it("stores the merged report with provenance, the board snapshots with retention, the escalations, the consensus rounds, the shared costs and the interleaved timeline", async () => {
    const store = new sessionmemory(new fakeadapter());
    await store.setreport({
      id: "rep1",
      title: "The extraction",
      sections: [
        {
          title: "Task t1",
          entries: [{ id: "m1", agentid: "a1", taskid: "t1", key: "rowcount", value: "42", mergedat: now }],
          sources: ["a1"],
        },
      ],
      sources: ["a1"],
    ...
    await store.addescalation({
      id: "e1",
      agentid: "a1",
      subject: "Which origin",
      context: "Both hold half.",
      state: "open",
      raisedat: now,
    });
    await store.updateescalation({
      ...
      context: "Both hold half.",
      state: "decided",
      decision: "The second origin.",
      raisedat: now,
      decidedat: now + 1,
    });
    expect((await store.getescalations())[0]?.decision).toBe("The second origin.");
    await store.setconsensusround({
      id: "c1",
      subject: "The lane order",
      votes: [],
      quorum: 2,
      state: "open",
      openedat: now,
    ...
    await store.setconsensusround({
      id: "c1",
      subject: "The lane order",
      votes: [{ agentid: "a1", vote: "yes", votedat: now + 1 }],
      quorum: 2,
      state: "open",
      openedat: now,
    });
...
describe("execution environment persistence", () => {
  ...
  it("expires the stale stopped run states past the user configured window while the keepalive summaries survive", async () => {
    ...
    const stopped = {
      ...
      urlhistory: [{ url: "https://example.com/a", stepid: "s1", at: 1 }],
      environments: { s1: "pagecontext" as const },
      turnarounds: {},
      keepalive: {
        runid: "run1",
        sessionid: "s1",
    ...
    const active = {
      ...stopped,
      runid: "run2",
      keepalive: { ...stopped.keepalive, runid: "run2", state: "active" as const, portopen: true },
      updatedat: 100,
    };
  ...
  it("stores worker events with provenance, the offscreen registry, sandbox renders and the run locks", async () => {
    const store = new sessionmemory(new fakeadapter());
    await store.addworkerevent({
      id: "w1",
      runid: "run1",
      kind: "spawn",
      workers: 2,
      reason: "Two parses waited.",
      provenance: { origin: "https://example.com", stepid: "s1", environment: "offscreenworker" },
      at: 1,
    });
    await store.addworkerevent({
      id: "w2",
    ...
    await store.updateoffscreenentry({
      document: "offscreen.html",
      runid: "run1",
      reasons: ["DOM_PARSER", "WORKERS"],
      justification: "Heavy parsing of reviewed snapshots.",
  ...
  it("tracks the storage quota usage of the run state and exports every run state as one record", async () => {
    ...
    const state = {
      ...
      state: "active" as const,
      urlhistory: [],
      environments: { s1: "offscreenworker" as const },
      turnarounds: { s1: 8 },
      keepalive: {
        runid: "run1",
        sessionid: "s1",
  ...
  it("stores the automation allowlist, the origin profiles, the consent windows and the class consents of the security family", async () => {
    ...
    expect((await store.getautomationallowlist()).map((entry) => entry.origin)).toEqual([
      "https://example.com",
      "https://other.example",
    ]);
    await store.removeallowlistorigin("https://other.example", "default");
    expect(await store.getautomationallowlist()).toHaveLength(1);
    await store.saveoriginprofile({
      profileid: "p1",
      origin: "https://example.com",
      grants: ["fillcard" as const],
      denials: [],
      createdat: 1,
      updatedat: 1,
    });
    ...
    await store.addrevocation({
      ...
      haltedstepids: ["s1", "s2"],
      actor: "user",
      reason: "The user revoked the consent mid run.",
      at: 3,
    });
  ...
  it("stores the immutable run log and exports only the verified chain", async () => {
    const store = new sessionmemory(new fakeadapter());
    let log = { runid: "run1", sessionid: "s1", entries: [] as never[], updatedat: 1 } as Parameters<
      sessionmemory["setimmutablelog"]
    >[0];
...
describe("sessionmemory security part two", () => {
  ...
  it("stores ratelimit bucket state per origin and per session", async () => {
    const store = new sessionmemory(new fakeadapter());
    await store.saveratelimitbucket({
      origin: "https://example.com",
      sessionid: "s",
      limit: 5,
      window: 1000,
  ...
  it("stores phishguard verdicts and expires the ones past the freshness window", async () => {
    const store = new sessionmemory(new fakeadapter());
    await store.addphishverdict({
      origin: "https://pay.example.com",
      matchedorigin: "https://pay.example.com",
      distance: 0.2,
      threshold: 0.4,
      blocked: false,
  ...
  it("stores permdiff records of each installed version with the last permission set", async () => {
    const store = new sessionmemory(new fakeadapter());
    await store.addpermdiff({
      fromversion: "1.1.61",
      toversion: "1.1.62",
      added: ["optional:downloads"],
      removed: [],
      computedat: 1,
    });
