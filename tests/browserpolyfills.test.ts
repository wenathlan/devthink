...
describe("browserpolyfills", () => {
  ...
  it("normalizes the callback style into a promise across browsers", async () => {
    ...
    const call = (...args: unknown[]) => {
      for (const a of args) calls.push(a);
  ...
  it("keeps session tokens under the same keys on every browser", async () => {
    ...
    const local = {
      ...
      get: (key: string | undefined, callback?: (entries: Record<string, unknown>) => void) => {
        if (callback !== undefined) callback(stored);
      },
    };
    const session = {
      set: (entries: Record<string, unknown>, callback?: () => void) => {
  ...
  it("surfaces unsupported api calls as structured errors with the retry hint none", () => {
    const struct = browserpolyfillstructerrorof({
      family: "storage",
      message: "missing",
      browser: "safari",
      now: 1000,
