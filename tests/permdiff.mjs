...
export function diffpermissionsets(current, previous) {
  ...
  for (const set of ["required", "optional", "optionalhost"]) {
    if (JSON.stringify(current[set]) !== JSON.stringify(previous[set]) && current[set].every(entry => previous[set].includes(entry)) && previous[set].every(entry => current[set].includes(entry))) reordered.push(set);
  }
...
async function releasemanifestof(version) {
  for (const resolver of [
    async () => ({ source: `git tag v${version}`, text: (await execute("git", ["show", `v${version}:manifest.json`])).stdout }),
    async () => {
      const log = (await execute("git", ["log", "--all", "--oneline", `--grep=release v${version}`, "-1"])).stdout.trim();
      if (log === "") throw new Error(`no release commit of v${version}`);
      const commit = log.split(" ")[0] ?? "";
      return { source: `git commit ${commit}`, text: (await execute("git", ["show", `${commit}:manifest.json`])).stdout };
    },
    async () => {
      ...
      return { source: `git tag v${version} fetched from origin`, text: (await execute("git", ["show", `v${version}:manifest.json`])).stdout };
    },
  ]) {
    try {
      const resolved = await resolver();
      return { ...resolved, manifest: JSON.parse(resolved.text) };
    } catch { /* the next resolver answers */ }
  }
  return undefined;
}
...
function baselinesetof(baseline) {
  return permissionsetof({
    ...
    optional_host_permissions: (baseline.permissionset?.optionalhost ?? []).map(entry => entry.slice("optionalhost:".length)),
  });
}
...
if (baseline === undefined && mode !== "sync") refuse(`The permission baseline lives in ${artifactpath}; a missing baseline never verifies the permission set. Run pnpm permdiff:sync to record it.`);
const bootstrapped = baseline === undefined ? await releasemanifestof(previousof(release)) : undefined;
const baselineset = baseline !== undefined ? baselinesetof(baseline) : bootstrapped !== undefined ? permissionsetof(bootstrapped.manifest) : permissionsetof({});
...
if (justificationtext === undefined) refuse(`The permdiff justification table lives in ${justificationdoc}; the document is missing.`);
const tablesection = justificationtext !== undefined && justificationtext.includes(tableheading) ? justificationtext.slice(justificationtext.indexOf(tableheading)) : "";
if (added.length > 0 && tablesection === "") refuse(`The permdiff justification table heading is missing from ${justificationdoc}; every permission addition needs its written justification row.`);
const justified = [];
const unjustified = [];
...
if (unjustified.length > 0) refuse(`The permission addition${unjustified.length === 1 ? "" : "s"} ${unjustified.join(", ")} carry no justification row in the permdiff justification table; an unjustified permission blocks the release.`);
...
if (baseline !== undefined && baseline.release === release && (added.length > 0 || removed.length > 0 || reordered.length > 0)) refuse(`The permission set drifted inside the release ${release} the baseline stamps; restore the set or bump the version and run pnpm permdiff:sync after the justification review.`);
...
if (baseline !== undefined && failures.length === 0) {
  ...
  if (tagged !== undefined && JSON.stringify(permissionsetof(tagged.manifest).entries) !== JSON.stringify(baselineset.entries)) refuse(`The permission baseline of ${baseline.release} disagrees with the git tag v${baseline.release}; the checked-in baseline must mirror the released permission set.`);
}

/* the permission set hash of the release: the audit trail records the same digest the background stamps at install. */
const permissionhash = createHash("sha256").update([...new Set(current.entries)].sort().join("\n")).digest("hex");
...
if (failures.length > 0) {
  ...
  console.log(`Permission baseline of ${release} recorded against ${report.previous} (${report.source}): ${added.length} added, ${removed.length} removed, ${reordered.length === 0 ? "none" : reordered.join(", ")} reordered.`);
  console.log(`Permission set hash: ${permissionhash} over ${new Set(current.entries).size} permission entries; the baseline landed in ${artifactpath}.`);
} else if (baseline === undefined || baseline.release !== release) {
  console.error(`PERMDIFF The permission baseline records the release ${baseline?.release ?? "nothing"} while the package carries ${release}; review the justification rows and run pnpm permdiff:sync after the release bump.`);
  process.exitCode = 1;
} else {
  console.log(`Permission diff verified ${release} against the baseline of ${baseline.release}: ${added.length} added, ${removed.length} removed, ${reordered.length === 0 ? "none" : reordered.join(", ")} reordered.`);
  console.log(`Permission set hash: ${permissionhash} over ${new Set(current.entries).size} permission entries; the baseline of ${artifactpath} carries the recorded set of the release.`);
}
