...
import {
  channellive, closeidlechannels, createlinepump, defaultheartbeatms, defaulthttpstream, defaultidlewindowms, enforcemaxclients, fetchrequestof, fetchoptionsof, graphqlopenvelope, graphqlrequestof, httpanswer, httpendpoint, httpframepipeline, openstreamchannel, parsepost, parsehtmlbody, payloadvalid, payloadwithdefaults, readpath, readstream, relayconnectionclose, relayconnectionopen, relayframeof, relayidleconnections, relaylivetokens, relaypendingpairings, relayserverframe, newrelayserverstate, routepath, sendfetch, sseframe, splitlines, starttls, statusclassof, streampathof, streamwindowof, templateurl, tlsdecision, unwrapgraphql, heartbeat, listremotestatus,
} from "../http.js";
...
function response(status: number, body = "", headers: Record<string, string> = {}, location?: string): transportresponse {
  return { status, headers, body, ...(location !== undefined ? { location } : {}) };
}

/** Builds one allowlist fixture entry the pipeline trusts. */
async function pipelinefixture(): Promise<{ tokens: Awaited<ReturnType<typeof issuetoken>>["token"][]; raw: string; allowlist: ReturnType<typeof grantallowlistentry> }> {
...
describe("torture: url templating and request normalization", () => {
  ...
  it("refuses malformed fetch request shapes while normalizing the clean ones", () => {
    ...
    expect(fetchrequestof({ url: " https://x.example ", method: " delete ", body: "x", mode: "cors" })).toEqual({ url: "https://x.example", method: "DELETE", body: "x", mode: "cors" });
    expect(fetchrequestof({ url: "https://x.example", mode: "websocket" as never })).toEqual({ url: "https://x.example" });
    expect(fetchrequestof({ url: "https://x.example", headers: { keep: "1", drop: 2, alsodrop: null } })).toEqual({ url: "https://x.example", headers: { keep: "1" } });
...
describe("torture: sendfetch transport loop", () => {
  it("follows redirect chains inside the limit and refuses past it", async () => {
    ...
    await expect(sendfetch({ request, options: { follow: 2 }, transport, sleep: instant })).rejects.toThrow(/exceeded the reviewed follow limit of 2/);
    await expect(sendfetch({ request, options: { follow: 3 }, transport, sleep: instant })).resolves.toMatchObject({ status: 200, redirects: 3 });
  });

  it("refuses the chain one hop past the reviewed follow limit", async () => {
    const transport: fetchtransport = async (url): Promise<transportresponse> => url.endsWith("/a") ? response(302, "", {}, "/b") : url.endsWith("/b") ? response(302, "", {}, "/c") : response(404, "gone");
    await expect(sendfetch({ request: { url: "https://a.example/a" }, options: { follow: 1 }, transport, sleep: instant })).rejects.toThrow(/exceeded the reviewed follow limit of 1/);
    await expect(sendfetch({ request: { url: "https://a.example/a" }, options: { follow: 0 }, transport, sleep: instant })).rejects.toThrow(/exceeded the reviewed follow limit of 0/);
  });

  it("resumes a retry from the redirect position the failed attempt reached", async () => {
    const transport: fetchtransport = async (url): Promise<transportresponse> => url.endsWith("/a") ? response(302, "", {}, "/b") : url.endsWith("/b") ? response(302, "", {}, "/c") : response(404, "gone");
    const outcome = await sendfetch({ request: { url: "https://a.example/a" }, options: { follow: 1, retries: 1 }, transport, sleep: instant });
  ...
  it("downgrades the post body to a bodiless GET on a 301, 302 or 303 redirect", async () => {
    ...
    const outcome = await sendfetch({ request: { url: "https://a.example/pay", method: "POST", body: "secret-payload" }, transport, sleep: instant });
    expect(outcome.status).toBe(200);
  ...
  it("keeps the post body through a 307 and 308 redirect", async () => {
    ...
    await sendfetch({ request: { url: "https://a.example/pay", method: "POST", body: "kept" }, transport, sleep: instant });
  ...
  it("resolves relative and absolute redirect locations against the current url", async () => {
    ...
    const absolute: fetchtransport = async (url): Promise<transportresponse> => url === "https://a.example/page" ? response(302, "", {}, "https://b.example/x") : response(200, "ok");
    const crossed = await sendfetch({ request: { url: "https://a.example/page" }, transport: absolute, sleep: instant });
    expect(crossed.url).toBe("https://b.example/x");
  });

  it("stops only at a redirect without a location header", async () => {
    const transport: fetchtransport = async (url): Promise<transportresponse> => url === "https://a.example/hop" ? response(302, "", {}, "") : response(200, "ok");
  ...
  it("retries failed attempts with the backoff wait and reports the reason", async () => {
    ...
    const outcome = await sendfetch({ request: { url: "https://a.example/" }, options: { retries: 3, backoff: 25 }, transport, sleep: async ms => { waits.push(ms); }, onretry: (attempt, wait, reason) => { waits.push(-attempt); void reason; } });
  ...
  it("throws the attempt accounting when every attempt fails", async () => {
    const transport: fetchtransport = async (): Promise<transportresponse> => { throw new Error("refused"); };
    await expect(sendfetch({ request: { url: "https://a.example/" }, options: { retries: 2 }, transport, sleep: instant })).rejects.toThrow(/failed after 3 attempts with 2 retries: refused/);
    await expect(sendfetch({ request: { url: "https://a.example/" }, transport, sleep: instant })).rejects.toThrow(/failed after 1 attempt with 0 retries/);
  });

  it("times one attempt out inside the reviewed timeout and answers the attempt accounting", async () => {
    let slowcalls = 0;
    const transport: fetchtransport = async (url): Promise<transportresponse> => {
      if (url.includes("slow")) { slowcalls += 1; await new Promise(resolve => setTimeout(resolve, 30)); return response(200, "late"); }
      return response(200, "fast");
    };
    await expect(sendfetch({ request: { url: "https://a.example/slow" }, options: { timeout: 5, retries: 0 }, transport, sleep: instant })).rejects.toThrow(/failed after 1 attempt with 0 retries: The request timed out after 5 milliseconds/);
    expect(slowcalls).toBe(1);
    const hanging: fetchtransport = async (): Promise<transportresponse> => new Promise(() => undefined);
    await expect(sendfetch({ request: { url: "https://a.example/" }, options: { timeout: 5 }, transport: hanging, sleep: instant })).rejects.toThrow(/timed out after 5 milliseconds/);
  });
  ...
  it("times the attempt out and retries until the next attempt succeeds", async () => {
    ...
    const outcome = await sendfetch({ request: { url: "https://a.example/" }, options: { timeout: 5, retries: 1 }, transport, sleep: instant });
  ...
  it("never strips a body from a POST and never adds one to a GET", async () => {
    ...
    const transport: fetchtransport = async (_url, init): Promise<transportresponse> => { seen.push({ method: init.method, ...(init.body !== undefined ? { body: init.body } : {}) }); return response(200, ""); };
    await sendfetch({ request: { url: "https://a.example/", method: "POST", body: "payload" }, transport, sleep: instant });
    await sendfetch({ request: { url: "https://a.example/", method: "GET", body: "dropped" }, transport, sleep: instant });
...
describe("torture: stream reading budget", () => {
  ...
  it("stops cleanly on the reviewed abort flag between chunks", async () => {
    ...
    expect(halted).toEqual({ chunks: 0, bytes: 0, aborted: true, reason: "The reviewed abort flag stopped the stream." });
  });

  it("reads a pull function to its exhaustion", async () => {
    let pulls = 0;
    const outcome = await readstream({ chunks: async () => { pulls += 1; return pulls <= 2 ? "x" : undefined; }, window: {} });
...
describe("torture: json path reading and graphql envelopes", () => {
  it("reads dotted paths with array indexes, defaults and misses", () => {
    ...
    expect(readpath(parsed, [{ name: "n", path: "user.name", kind: "text" }])[0]).toMatchObject({ name: "n", value: "ada" });
    ...
    expect(readpath(parsed, [{ name: "b", path: "missing.deep", kind: "number", default: -1 }])[0]).toMatchObject({ value: -1, missing: true });
  ...
  it("wraps and unwraps graphql envelopes while malformed answers refuse", () => {
    expect(graphqlopenvelope({ query: "query { x }", operationkind: "query", variables: { a: 1 }, operationname: "op" })).toBe(JSON.stringify({ query: "query { x }", variables: { a: 1 }, operationName: "op" }));
    expect(unwrapgraphql({ data: { x: 1 }, errors: [{ message: "bad" }, "plain", 5] })).toEqual({ data: { x: 1 }, errors: ["bad", "plain", "5"] });
    ...
    expect(graphqlrequestof({ query: "x", operationkind: "query", variables: [1] })).toEqual({ query: "x", operationkind: "query" });
  });

  it("parses html bodies through the injected seam with the multi flag", () => {
    const seam = (markup: string) => ({ query: (selector: string) => selector === "p" ? markup.split("|").map(text => ({ text, attributes: {} })) : [] });
    const outcome = parsehtmlbody({ body: "a|b|c", queries: [{ selector: "p", multi: true }, { selector: "p" }], parse: seam });
  ...
  it("validates and defaults payload schemas", () => {
    const schema = { fields: [{ name: "id", kind: "string" as const, required: true }, { name: "n", kind: "number" as const }, { name: "b", kind: "boolean" as const, default: false }] };
    expect(payloadvalid({ id: "x", n: 1 }, schema)).toEqual({ ok: true, errors: [] });
    expect(payloadvalid({}, undefined)).toEqual({ ok: false, errors: ["The typed endpoint call needs a reviewed payload schema before it runs."] });
...
describe("torture: http transport endpoints and routing", () => {
  it("builds the endpoint records from the config and refuses path confusion", () => {
    const config: mcpserverconfig = { ...defaultmcpconfig(), port: 9000, httpstream: { endpoint: "/agent", streampath: "/agent/stream", tls: { mode: "off" } } };
    const endpoint = httpendpoint(config);
    expect(endpoint).toMatchObject({ kind: "http", endpoint: "http://127.0.0.1:9000/agent", bind: localhostbind, port: 9000, localhost: true, path: "/agent" });
    expect(streampathof(config)).toMatchObject({ path: "/agent/stream", bind: localhostbind, localhost: true });
    const defaulted = httpendpoint({ ...defaultmcpconfig(), port: 7436, httpstream: { endpoint: "  ", streampath: " ", tls: { mode: "off" } } });
    expect(defaulted.path).toBe("/mcp");
    expect(streampathof({ ...defaultmcpconfig(), port: 7436 }).path).toBe("/mcp/stream");
    expect(streampathof({ ...defaultmcpconfig(), port: 7436, httpstream: { endpoint: "/mcp", streampath: "agent", tls: { mode: "off" } } }).path).toBe("agent");
  });
  ...
  it("decides tls under every hostile configuration", () => {
    expect(tlsdecision({ config: { mode: "off" }, presented: { fingerprint: "sha256:zz" }, now })).toEqual({ tls: false, verified: false });
    expect(tlsdecision({ config: { mode: "required" }, now }).reason).toMatch(/presented no certificate/i);
    expect(tlsdecision({ config: { mode: "on", certificatefingerprint: "sha256:aa" }, presented: { fingerprint: "sha256:AA" }, now }).reason).toMatch(/does not match the user configured fingerprint/i);
    expect(tlsdecision({ config: { mode: "required", certificatefingerprint: "sha256:aa" }, presented: { fingerprint: "sha256:aa" }, now })).toEqual({ tls: true, verified: true });
    expect(tlsdecision({ config: { mode: "required", certificatefingerprint: "" }, presented: { fingerprint: "" }, now })).toEqual({ tls: true, verified: true });
  });
});
...
describe("torture: posted frame intake and sse framing", () => {
  it("parses one frame, one batch and refuses the mixed or garbage bodies", () => {
    ...
    expect(parsepost(JSON.stringify([frame, { jsonrpc: "2.0", id: 2, method: "ping" }]))).toEqual({ message: [frame, { jsonrpc: "2.0", id: 2, method: "ping" }] });
    expect(parsepost(JSON.stringify([frame, "garbage"]))).toEqual({ error: { code: "parse", message: "The posted body does not parse as one json rpc message." } });
    expect(parsepost("[]")).toEqual({ error: { code: "parse", message: "The posted body does not parse as one json rpc message." } });
    expect(parsepost("null")).toEqual({ error: { code: "parse", message: "The posted body does not parse as one json rpc message." } });
    expect(parsepost("42")).toEqual({ error: { code: "parse", message: "The posted body does not parse as one json rpc message." } });
    expect(parsepost("{nope")).toEqual({ error: { code: "parse", message: "The posted body does not parse as one json rpc message." } });
    expect(parsepost("")).toEqual({ error: { code: "parse", message: "The posted body does not parse as one json rpc message." } });
  });

  it("formats the http answer and the server sent event frames exactly", () => {
    expect(httpanswer({ jsonrpc: "2.0", id: 7, result: { pong: true } })).toBe(`${JSON.stringify({ jsonrpc: "2.0", id: 7, result: { pong: true } })}\n`);
    expect(httpanswer([{ jsonrpc: "2.0", id: 1, result: 1 }, { jsonrpc: "2.0", id: 2, result: 2 }] as never)).toBe(`${JSON.stringify([{ jsonrpc: "2.0", id: 1, result: 1 }, { jsonrpc: "2.0", id: 2, result: 2 }])}\n`);
...
describe("torture: stream channels, heartbeats and the client ceiling", () => {
  it("opens, beats and sweeps channels with exact idle window edges", () => {
    ...
    const beaten = heartbeat({ channels: [open, { ...open, clientid: "c2", closedat: now - 5 }], clientid: "c1", now: now + 1000 });
  ...
  it("treats the exact idle window edge as dead while one millisecond under stays live", () => {
    ...
    const swept = closeidlechannels({ channels: [channel, { ...channel, id: "ch2", lastbeatat: now }], now, idlewindow: 10_000 });
  ...
  it("enforces the client ceiling at the exact boundary", () => {
    ...
    expect(enforcemaxclients({ clients: [{ ...clients[0]!, disconnectedat: now }, clients[1]!], maxclients: 2 }).allowed).toBe(true);
  ...
  it("reports the remote status of a configured transport", () => {
    const config: mcpserverconfig = { ...defaultmcpconfig(), enabled: true, remoteaccess: { endpoint: "https://agent.example", tls: { mode: "on" }, maxclients: 5 } };
    const status = listremotestatus({ config, channels: [], clients: [{ id: "c1", transport: "http", paired: false, connectedat: now }], tokens: [], now });
...
describe("torture: the ordered remote frame pipeline", () => {
  it("refuses before the token when tls fails and never leaks pairing state", async () => {
    ...
    const refused = await httpframepipeline({ config, presented: { fingerprint: "sha256:zz" }, tokens: fixture.tokens, rawtoken: "totally-wrong-token", allowlist: fixture.allowlist, fingerprint: "fp-unknown", now });
  ...
  it("answers the fixed auth message for missing, wrong, revoked and expired tokens", async () => {
    ...
    const wrong = await httpframepipeline({ config, tokens: fixture.tokens, rawtoken: "", allowlist: fixture.allowlist, fingerprint: "fp-1", now });
    expect(wrong.error?.message).toBe("The remote frame failed its authentication handshake.");
    const emptytokens = await httpframepipeline({ config, tokens: [], rawtoken: fixture.raw, allowlist: fixture.allowlist, fingerprint: "fp-1", now });
    ...
    const revoked = await httpframepipeline({ config, tokens: [revokedtoken], rawtoken: fixture.raw, allowlist: fixture.allowlist, fingerprint: "fp-1", now });
    ...
    const expired = await httpframepipeline({ config, tokens: [expiredtoken], rawtoken: fixture.raw, allowlist: fixture.allowlist, fingerprint: "fp-1", now });
  ...
  it("refuses unknown fingerprints and ungranted scopes after the token verified", async () => {
    const config: mcpserverconfig = { ...defaultmcpconfig(), enabled: true };
    const fixture = await pipelinefixture();
    const unknownfp = await httpframepipeline({ config, tokens: fixture.tokens, rawtoken: fixture.raw, allowlist: fixture.allowlist, fingerprint: "fp-attacker", now });
    expect(unknownfp.error?.message).toMatch(/fp-attacker is not on the allowlist/);
    const namespaceoff = await httpframepipeline({ config, tokens: fixture.tokens, rawtoken: fixture.raw, allowlist: fixture.allowlist, fingerprint: "fp-1", toolname: "memory.list", now });
    expect(namespaceoff.error?.code).toBe("consentrefused");
    expect(namespaceoff.error?.message).toMatch(/grants no memory tools/);
    const unknownnamespace = await httpframepipeline({ config, tokens: fixture.tokens, rawtoken: fixture.raw, allowlist: fixture.allowlist, fingerprint: "fp-1", toolname: "spy.satellite", now });
  ...
  it("verifies a clean frame and returns its token", async () => {
    ...
    const tokenless = await httpframepipeline({ config, tokens: fixture.tokens, rawtoken: fixture.raw, allowlist: [], fingerprint: "fp-1", now });
...
describe("torture: relay server state machine", () => {
  ...
  function createsession(state: ReturnType<typeof newrelayserverstate>, code: string, connectionid = "conn-ext"): { state: ReturnType<typeof newrelayserverstate>; token: string; sessionid: string } {
    const outcome = relayserverframe({ state, connectionid, frame: relayframeof({ op: "sessioncreate", opid: "op-1", at: now, body: { role: "extension", pairingcode: code } }), now, idof: (() => { let n = 0; return () => `${++n}`; })(), hashof: token => `h(${token})` });
    const reply = JSON.parse(outcome.replies[0] ?? "{}") as { sessionid: string; token: string; body: { sessionid: string; token: string } };
  ...
  it("refuses the unregistered connection and the unparseable frame without state drift", () => {
    ...
    const emptyid = relayserverframe({ state: registered, connectionid: "conn-1", frame: JSON.stringify({ version: 1, op: "eventpost", opid: "", at: now }), now });
  ...
  it("registers connections once and drops their tokens on close", () => {
    ...
    expect(created.state.tokens.some(record => record.connectionid === "conn-1" && record.token === created.token)).toBe(true);
  ...
  it("serves the sessioncreate operation for the extension member only", () => {
    ...
    const attacker = relayserverframe({ state, connectionid: "conn-attacker", frame: relayframeof({ op: "sessioncreate", opid: "op-a", at: now, body: { role: "site" } }), now, idof: () => "9", hashof: t => `h(${t})` });
    expect(attacker.replies[0]).toContain("serves the extension member only");
    expect(attacker.state.sessions).toHaveLength(0);
    const created = relayserverframe({ state, connectionid: "conn-ext", frame: relayframeof({ op: "sessioncreate", opid: "op-1", at: now, body: { role: "extension", memberid: "ext-1" } }), now, idof: () => "1", hashof: t => `h(${t})` });
    const reply = JSON.parse(created.replies[0] ?? "{}") as { body: { members: Array<{ role: string; id: string }>; capabilities: unknown } };
  ...
  it("exchanges each pairing code exactly once and refuses the second spend", () => {
    ...
    const firstjoin = relayserverframe({ state: created.state, connectionid: "conn-site", frame: relayframeof({ op: "sessionjoin", opid: "op-2", at: now + 1, body: { role: "site", pairingcode: "DT-PAIR" } }), now: now + 1, idof: () => "2", hashof: t => `h(${t})` });
    ...
    const secondspend = relayserverframe({ state: firstjoin.state, connectionid: "conn-attacker", frame: relayframeof({ op: "sessionjoin", opid: "op-3", at: now + 2, body: { role: "site", pairingcode: "DT-PAIR" } }), now: now + 2, idof: () => "3", hashof: t => `h(${t})` });
  ...
  it("refuses expired and wrong pairing codes", () => {
    ...
    const expired = relayserverframe({ state: created.state, connectionid: "conn-site", frame: relayframeof({ op: "sessionjoin", opid: "op-2", at: now + 300_001, body: { role: "site", pairingcode: "DT-EXP" } }), now: now + 300_001, idof: () => "2", hashof: t => `h(${t})` });
    expect(expired.replies[0]).toContain("matches no pending session");
    const wrong = relayserverframe({ state: created.state, connectionid: "conn-site", frame: relayframeof({ op: "sessionjoin", opid: "op-2", at: now + 1, body: { role: "site", pairingcode: "DT-OTHER" } }), now: now + 1, idof: () => "2", hashof: t => `h(${t})` });
    expect(wrong.replies[0]).toContain("matches no pending session");
    const whitespace = relayserverframe({ state: created.state, connectionid: "conn-site", frame: relayframeof({ op: "sessionjoin", opid: "op-2", at: now + 1, body: { role: "site", pairingcode: "   " } }), now: now + 1, idof: () => "2", hashof: t => `h(${t})` });
  ...
  it("refuses a second site member while the session holds one", () => {
    ...
    const first = relayserverframe({ state: created.state, connectionid: "conn-site", frame: relayframeof({ op: "sessionjoin", opid: "op-2", at: now + 1, body: { role: "site", pairingcode: "DT-SITE" } }), now: now + 1, idof: () => "2", hashof: t => `h(${t})` });
    expect(JSON.parse(first.replies[0] ?? "{}").body.token).toBe("token-2");
    const second = relayserverframe({ state: first.state, connectionid: "conn-evil", frame: relayframeof({ op: "sessionjoin", opid: "op-3", at: now + 2, body: { role: "site", pairingcode: "DT-SITE" } }), now: now + 2, idof: () => "3", hashof: t => `h(${t})` });
  ...
  it("refuses forged tokens on every operation without touching the session state", () => {
    ...
    const forged = relayserverframe({ state: created.state, connectionid: "conn-evil", frame: relayframeof({ op: "eventpost", opid: "op-e", sessionid: created.sessionid, token: "forged-token", at: now + 1, body: { kind: "chat", stream: "chat" } }), now: now + 1, idof: () => "2", hashof: t => `h(${t})` });
    ...
    const tokenless = relayserverframe({ state: created.state, connectionid: "conn-ext", frame: relayframeof({ op: "eventpost", opid: "op-e", sessionid: created.sessionid, at: now + 1, body: { kind: "chat" } }), now: now + 1, idof: () => "2", hashof: t => `h(${t})` });
    expect(tokenless.replies[0]).toContain("failed the token authentication");
    const wrongsession = relayserverframe({ state: created.state, connectionid: "conn-ext", frame: relayframeof({ op: "eventpost", opid: "op-e", sessionid: "session-other", token: created.token, at: now + 1, body: { kind: "chat" } }), now: now + 1, idof: () => "2", hashof: t => `h(${t})` });
  ...
  it("routes events to the other member with the member token and never echoes secrets", () => {
    ...
    const joined = relayserverframe({ state: created.state, connectionid: "conn-site", frame: relayframeof({ op: "sessionjoin", opid: "op-2", at: now + 1, body: { role: "site", pairingcode: "DT-ROUTE" } }), now: now + 1, idof: () => "2", hashof: t => `h(${t})` });
    const sitetoken = (JSON.parse(joined.replies[0] ?? "{}") as { body: { token: string } }).body.token;
    const posted = relayserverframe({ state: joined.state, connectionid: "conn-ext", frame: relayframeof({ op: "eventpost", opid: "op-3", sessionid: created.sessionid, token: created.token, at: now + 2, body: { kind: "chat", stream: "chat", payload: { text: "hello" } } }), now: now + 2, idof: () => "3", hashof: t => `h(${t})` });
    expect(posted.routed).toHaveLength(1);
    expect(posted.routed[0]?.connectionid).toBe("conn-site");
    const routedframe = JSON.parse(posted.routed[0]?.frame ?? "{}") as { token?: string; body: { payload: { text: string } } };
  ...
  it("acks the eventstream subscription and refuses the operations outside the contract", () => {
    ...
    const subscribed = relayserverframe({ state: created.state, connectionid: "conn-ext", frame: relayframeof({ op: "eventstream", opid: "op-2", sessionid: created.sessionid, token: created.token, at: now + 1, body: { streams: ["chat", "review"], subscribed: true } }), now: now + 1, idof: () => "2", hashof: t => `h(${t})` });
    ...
    const invalidstreams = relayserverframe({ state: created.state, connectionid: "conn-ext", frame: relayframeof({ op: "eventstream", opid: "op-3", sessionid: created.sessionid, token: created.token, at: now + 1, body: { streams: "chat" } }), now: now + 1, idof: () => "3", hashof: t => `h(${t})` });
    expect(JSON.parse(invalidstreams.replies[0] ?? "{}").body.streams).toEqual([]);
    const outside = relayserverframe({ state: created.state, connectionid: "conn-ext", frame: JSON.stringify({ version: 1, op: "eventack", opid: "op-4", at: now + 1 }), now: now + 1, idof: () => "4", hashof: t => `h(${t})` });
  ...
  it("rotates the token of a returning member and revokes the previous one", () => {
    ...
    const rotated = relayserverframe({ state: created.state, connectionid: "conn-ext", frame: relayframeof({ op: "sessionjoin", opid: "op-2", sessionid: created.sessionid, token: created.token, at: now + 1, body: { role: "extension" } }), now: now + 1, idof: () => "9", hashof: t => `h(${t})` });
    ...
    const replay = relayserverframe({ state: rotated.state, connectionid: "conn-ext", frame: relayframeof({ op: "eventpost", opid: "op-3", sessionid: created.sessionid, token: created.token, at: now + 2, body: { kind: "chat", stream: "chat" } }), now: now + 2, idof: () => "3", hashof: t => `h(${t})` });
    expect(replay.replies[0]).toContain("failed the token authentication");
    const fresh = relayserverframe({ state: rotated.state, connectionid: "conn-ext", frame: relayframeof({ op: "eventpost", opid: "op-4", sessionid: created.sessionid, token: "token-9", at: now + 3, body: { kind: "chat", stream: "chat" } }), now: now + 3, idof: () => "4", hashof: t => `h(${t})` });
  ...
  it("sweeps idle connections and lists pending pairings inside their window", () => {
    ...
    const joined = relayserverframe({ state: created.state, connectionid: "conn-site", frame: relayframeof({ op: "sessionjoin", opid: "op-2", at: now + 1, body: { role: "site", pairingcode: "DT-PEND" } }), now: now + 1, idof: () => "2", hashof: t => `h(${t})` });
    expect(relaypendingpairings(joined.state, now + 1)).toEqual([]);
    expect(relayframeof({ op: "eventpost", opid: "op-x", at: now, body: {} })).toBe(JSON.stringify({ version: 1, op: "eventpost", opid: "op-x", at: now, body: {} }));
  });
});
...
describe("torture: stdio framing", () => {
  ...
  it("pumps partial chunks into complete frames with the counters advancing", async () => {
    ...
    const pump = createlinepump({ write: line => { written.push(line); }, handle: async frame => ({ jsonrpc: "2.0", id: frame.id ?? null, result: { echo: frame.method } }), now });
    await pump.feed('{"jsonrpc":"2.0","id":1,');
    expect(written).toHaveLength(0);
    await pump.feed('"method":"ping"}\n{"jsonrpc":"2.0","id":2,"method":"ping"}\n');
    expect(written).toEqual([`${JSON.stringify({ jsonrpc: "2.0", id: 1, result: { echo: "ping" } })}\n`, `${JSON.stringify({ jsonrpc: "2.0", id: 2, result: { echo: "ping" } })}\n`]);
    await pump.feed("\n\n");
  ...
  it("answers notifications with nothing and decode failures with the parse error", async () => {
    const written: string[] = [];
    let handled = 0;
    const pump = createlinepump({ write: line => { written.push(line); }, handle: async frame => { handled += 1; return { jsonrpc: "2.0", id: frame.id ?? null, result: null }; }, now });
  ...
  it("keeps the transport alive when one handler throws", async () => {
    const written: string[] = [];
    let calls = 0;
    const pump = createlinepump({ write: line => { written.push(line); }, handle: async frame => { calls += 1; if (calls === 1) throw new Error("handler exploded"); return { jsonrpc: "2.0", id: frame.id ?? null, result: "ok" }; }, now });
    await pump.feed('{"jsonrpc":"2.0","id":"boom","method":"tools/call"}\n{"jsonrpc":"2.0","id":"fine","method":"tools/call"}\n');
    expect(written).toHaveLength(2);
    expect(JSON.parse(written[0] ?? "{}")).toMatchObject({ id: "boom", error: { code: "internal", message: "handler exploded" } });
