...
import { badgetextof, badgecolorof, countlabel, notifyattentionof, notifydoneof, notificationrespectsdnd, pluralize, recenttrayactions, recenttrayafter, recenttrayentryof, statusbadgeof, stetoasthistory, stetoastof, stetoaststackafter } from "../views.js";
...
describe("statusbadge, notifications, recenttray and stetoasts", () => {
  ...
  it("builds the done and attention payloads with deep links and gates the page content", () => {
    const done = notifydoneof({ runid: "run-1", origin: "https://example.com", summary: "The pricing table landed in 8 steps.", at: now });
    expect(done.kind).toBe("done");
    expect(done.deeplink).toBe("#run-run-1");
    expect(done.content).toBe(false);
    const attention = notifyattentionof({ runid: "run-1", stepid: "step-3", cause: "gatewait", reason: "The fillform step waits for its consent.", at: now });
    expect(attention.kind).toBe("attention");
    expect(attention.deeplink).toBe("#step-step-3");
    expect(attention.content).toBe(false);
    expect(() => notifyattentionof({ runid: "run-1", stepid: " ", cause: "gatewait", reason: "waits", at: now })).toThrow(/waiting step/);
    expect(notificationcontentgate({ content: false, consent: false }).allowed).toBe(true);
    expect(notificationcontentgate({ content: true, consent: false }).reason).toMatch(/never shows without its consent/);
    expect(notificationcontentgate({ content: true, consent: true }).allowed).toBe(true);
    expect(() => notifyattentionof({ runid: "run-1", stepid: "s", cause: "phishguard", reason: "r", content: true, consent: false, at: now })).toThrow(/consent/);
    expect(notifyattentionof({ runid: "run-1", stepid: "s", cause: "phishguard", reason: "r", content: true, consent: true, at: now }).content).toBe(true);
    ...
    const singular = notifydoneof({ runid: "run-2", origin: "https://example.com", summary: "", steps: 1, captures: 1, at: now });
    expect(singular.body).toContain("of the 1 step beside 1 capture");
    const plural = notifydoneof({ runid: "run-2", origin: "https://example.com", summary: "", steps: 8, captures: 4, at: now });
    ...
    const localized = notifydoneof({ runid: "run-2", origin: "https://example.com", summary: "", steps: 8, language: "pt", at: now });
    expect(localized.body).toContain("of the 8 etapas");
    const waits = notifyattentionof({ runid: "run-1", stepid: "step-3", cause: "gatewait", reason: "The fillform step waits for its consent.", at: now });
    expect(waits.body).toBe("The fillform step waits for its consent.");
    expect(notifyattentionof({ runid: "run-1", stepid: "step-3", cause: "gatewait", reason: "one gate holds.", waitinggates: 1, at: now }).body).toContain("(1 gate waiting)");
    expect(notifyattentionof({ runid: "run-1", stepid: "step-3", cause: "gatewait", reason: "three gates hold.", waitinggates: 3, at: now }).body).toContain("(3 gates waiting)");
  });
  ...
  it("lists the latest runs in the recenttray with resume and reopen offers", () => {
    const halted = recenttrayentryof({ runid: "run-1", origin: "https://example.com", outcome: "halted", title: "Pricing pass", at: now });
    const completed = recenttrayentryof({ runid: "run-2", origin: "https://example.com", outcome: "completed", title: "Invoice sweep", at: now + 1 });
    const running = recenttrayentryof({ runid: "run-3", origin: "https://example.com", outcome: "running", title: "Live pass", at: now + 2 });
    ...
    expect(recenttrayafter([halted], recenttrayentryof({ runid: "run-1", origin: "https://example.com", outcome: "completed", title: "again", at: now }), undefined)).toHaveLength(1);
    expect(() => recenttrayentryof({ runid: " ", origin: "https://example.com", outcome: "halted", title: "t", at: now })).toThrow(/run id/);
  });
