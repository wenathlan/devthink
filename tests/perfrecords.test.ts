...
import { abandonedparsetasks, captureplanschedule, dedupesnapshotrequests, perfbundle, perfrecordof, perfsummary, pruneperfrecords, queuedepthsampleof, queuedepthview, snapshotrequestkey, stepdurationchart, workerbackpressure } from "../perf.js";
...
describe("worker queue backpressure", () => {
  it("defers parse tasks past the user configured depth and never refuses them", () => {
    expect(workerbackpressure({ depth: 8, pending: 8, configured: 5 })).toEqual({ deferred: 3, admitted: 5, unbounded: false });
    expect(workerbackpressure({ depth: 2, pending: 4, configured: 5 })).toEqual({ deferred: 0, admitted: 4, unbounded: false });
...
describe("perf recording per step", () => {
  it("records one perf record per step with its provenance attached", () => {
    const record = perfrecordof({ runid: "run1", stepid: "s1", duration: 120, queries: 5, cachehits: 3, delta: true, now, provenance: { origin: "https://example.com", environment: "offscreenworker", task: "htmlsnapshot" } });
    expect(record.delta).toBe(true);
    expect(perfprovenancegate(record).allowed).toBe(true);
    expect(() => perfrecordof({ runid: " ", stepid: "s1", duration: 1, queries: 1, cachehits: 0, delta: false, now })).toThrow(/run id/i);
    expect(() => perfrecordof({ runid: "run1", stepid: " ", duration: 1, queries: 1, cachehits: 0, delta: false, now })).toThrow(/step id/i);
  ...
  it("sums the perf summary and charts the step durations over the run", () => {
    const records = [
      perfrecordof({ runid: "run1", stepid: "s1", duration: 100, queries: 4, cachehits: 2, delta: false, now: now + 100, provenance: { origin: "https://example.com" } }),
      perfrecordof({ runid: "run1", stepid: "s2", duration: 300, queries: 6, cachehits: 3, delta: true, now: now + 50, provenance: { environment: "offscreenworker" } }),
    ];
    const summary = perfsummary(records);
    expect(summary).toEqual({ steps: 2, duration: 400, average: 200, queries: 10, cachehits: 5, hitratio: 0.5, deltashare: 0.5 });
  ...
  it("deduplicates identical snapshot requests within one run", () => {
    const requests = [{ stepid: "s1", fingerprint: "f1" }, { stepid: "s1", fingerprint: "f1" }, { stepid: "s2", fingerprint: "f1" }, { stepid: "s2", fingerprint: "f2" }];
  ...
  it("prunes the perf records past the user configured retention window and exports the bundle", async () => {
    ...
    const fresh = perfrecordof({ runid: "run1", stepid: "s1", duration: 10, queries: 1, cachehits: 0, delta: false, now, provenance: { origin: "https://example.com" } });
    const old = perfrecordof({ runid: "run1", stepid: "s2", duration: 20, queries: 2, cachehits: 1, delta: false, now: now - 10_000, provenance: { environment: "pagecontext" } });
  ...
  it("builds the perf report envelope for the dashboardpage", () => {
    const report = perfreport({ runid: "run1", summary: { steps: 2, duration: 400, average: 200, queries: 10, cachehits: 5, hitratio: 0.5, deltashare: 0.5 }, chart: [{ stepid: "s1", duration: 100, delta: false }], selcache: { generation: 3, entries: 4 }, queue: { depth: 2, deferred: 1, peak: 6 }, chunk: { tableid: "table1", extracted: 10, total: 25, complete: false } });
    expect(report.runid).toBe("run1");
    expect(report.chart[0]?.stepid).toBe("s1");
    expect(report.chunk?.tableid).toBe("table1");
    const minimal = perfreport({ runid: "run2", summary: { steps: 0, duration: 0, average: 0, queries: 0, cachehits: 0, hitratio: 0, deltashare: 0 }, chart: [], selcache: { generation: 0, entries: 0 }, queue: { depth: 0, deferred: 0, peak: 0 } });
    expect(minimal.chunk).toBeUndefined();
    expect(() => perfreport({ runid: " ", summary: { steps: 0, duration: 0, average: 0, queries: 0, cachehits: 0, hitratio: 0, deltashare: 0 }, chart: [], selcache: { generation: 0, entries: 0 }, queue: { depth: 0, deferred: 0, peak: 0 } })).toThrow(/run id/i);
  });

  it("prunes the record list through the pure pruner beside the store", () => {
    const records = [
      perfrecordof({ runid: "run1", stepid: "s1", duration: 1, queries: 1, cachehits: 0, delta: false, now, provenance: {} }),
      perfrecordof({ runid: "run1", stepid: "s2", duration: 1, queries: 1, cachehits: 0, delta: false, now: now - 100, provenance: {} }),
    ];
