...
describe("torture: dataset records, checksums and artifact records", () => {
  ...
  it.skip("neutralizes spreadsheet formula payloads in the csv export", () => {
    const csv = exportcontent(
      datasetof([textcolumn("cell")], [{ cell: "=1+1" }, { cell: "+cmd|' /C calc'!A0" }, { cell: "@SUM(A1:A9)" }]),
      "csv",
    );
...
describe("torture: extraction sessions, provenance and retention", () => {
  ...
  it("stamps provenance with one based row ranges and applies retention as a head slice", () => {
    const provenance = provenancefor(
      { id: "art1", name: "Pricing.csv", rowcount: 42, checksum: "fnv1a-1" },
      "https://example.com",
      "s1",
      now,
    );
...
describe("torture: row interpolation and loop variables", () => {
  it("interpolates {{column}} tokens with trim, missing keys and repeated tokens", () => {
    expect(interpolate("{{name}} costs {{ price }}", { name: "Devthink", price: "1299.50" })).toBe(
      "Devthink costs 1299.50",
    );
...
describe("torture: grid preview, sorting and task rules", () => {
  ...
  it("merges task rules keeping the stored list when the new list is empty", () => {
    const stored = {
      taskid: "t1",
      transforms: [{ expression: "", sources: [], target: "a", field: "a", operation: "trim" as const }],
      dedupekeys: ["a"],
    };
...
describe("torture: streamdisk, resume and transform passes", () => {
  it("streams empty extracts without one write and survives a cursor pointing past the end", () => {
    ...
    const past = streamdisk({
      pipeline: pipeline(),
      rows: [row("r1", { a: "1" })],
      cursor: { pipelineid: "p1", offset: 100, chunk: 9, updatedat: now },
      chunk: 5,
      write: () => 1,
      now,
    });
  ...
  it("refuses invalid chunk sizes and propagates a sink error mid stream", () => {
    expect(() =>
      streamdisk({ pipeline: pipeline(), rows: [row("r1", { a: "1" })], chunk: 0, write: () => 1, now }),
    ).toThrow(/positive whole number/);
    expect(() =>
      streamdisk({ pipeline: pipeline(), rows: [row("r1", { a: "1" })], chunk: -2, write: () => 1, now }),
    ).toThrow(/positive whole number/);
    expect(() =>
      streamdisk({ pipeline: pipeline(), rows: [row("r1", { a: "1" })], chunk: 1.5, write: () => 1, now }),
    ).toThrow(/positive whole number/);
    expect(() => streamdisk({ pipeline: pipeline(), rows: [], write: () => 1, now })).not.toThrow();
    expect(() => streamdisk({ pipeline: pipeline({ id: " " }), rows: [], write: () => 1, now })).toThrow(
      /needs its pipeline/,
    );
    let wrote = 0;
    expect(() =>
      streamdisk({
        pipeline: pipeline(),
        rows: [row("r1", { a: "1" }), row("r2", { a: "2" })],
        chunk: 1,
        write: () => {
          wrote += 1;
          if (wrote === 2) throw new Error("sink exploded");
          return 1;
        },
        now,
      }),
  ...
  it("resumes with a cursor at the end, all keys persisted and refuses finished pipelines and foreign cursors", () => {
    ...
    const atend = resumeextract({
      pipeline: pipeline({ state: "paused" }),
      rows,
      cursor: { pipelineid: "p1", offset: 2, chunk: 2, updatedat: now },
      now,
    });
    expect(atend.rows).toEqual([]);
    expect(atend.skipped).toBe(2);
    const allpersisted = resumeextract({
      pipeline: pipeline({ state: "paused" }),
      rows,
      persistedkeys: ["r1", "r2"],
      now,
    ...
    expect(() =>
      resumeextract({
        pipeline: pipeline(),
        rows,
        cursor: { pipelineid: "p9", offset: 0, chunk: 0, updatedat: now },
        now,
      }),
    ).toThrow(/another pipeline/);
    expect(() => resumeextract({ pipeline: pipeline({ id: " " }), rows, now })).toThrow(/names its pipeline/);
    const partial = resumeextract({
      pipeline: pipeline({ state: "paused" }),
      rows,
      cursor: { pipelineid: "p1", offset: 1, chunk: 1, updatedat: now },
      persistedkeys: ["r2"],
      now,
    });
  ...
  it("keeps every row through number, case, trim and date transforms with refusals reported", () => {
    const rules: transformrule[] = [
      ...
      { expression: "", sources: [], target: "seen", field: "seen", operation: "date" },
      {
        ...
        field: "mystery",
        operation: "hex",
      } as unknown as transformrule,
    ];
    const rows = [
      row("r1", { price: "$ 1,299.50", name: "  dev think  ", seen: "March 3, 2026", mystery: "x" }),
      row("r2", { price: "free", name: "plain", seen: "no date", mystery: "y" }),
    ];
  ...
  it("falls back to the rule target when the field name is absent", () => {
    const result = transformvalues({
      rows: [row("r1", { sku: "  a  " })],
      rules: [{ expression: "", sources: [], target: "sku", operation: "trim" } as unknown as transformrule],
    });
...
describe("torture: dedupe normalization collisions and sampler bounds", () => {
  it("keeps distinct rows apart when a cell value embeds the join separator", () => {
    const rows = [row("r1", { a: "x|y", b: "1" }), row("r2", { a: "x", b: "y|1" }), row("r3", { a: "x", b: "1" })];
  ...
  it("refuses empty and whitespace only key columns while trimming the configured ones", () => {
    ...
    expect(deduperows({ rows: [], key: { columns: ["a"], normalization: "none" } })).toMatchObject({
      kept: [],
      dropped: 0,
    });
  });
...
describe("torture: source stamps and grid projection", () => {
  ...
  it("projects the grid with column kinds, sorting, filtering, marks and refusals for unknown sort columns", () => {
    ...
    expect(pipelinegridpreview({ rows, dedupekeys: ["r1", "r2"] }).rows.map((entry) => entry.deduplicated)).toEqual([
      true,
      true,
      false,
    ]);
    expect(() => pipelinegridpreview({ rows, sort: { column: "ghost", direction: "ascending" } })).toThrow(
      /knows no ghost/,
    );
    expect(
      pipelinegridpreview({ rows, sort: { column: "name", direction: "descending" } }).rows.map((entry) => entry.key),
    ).toEqual(["r2", "r3", "r1"]);
  });
});
...
describe("torture: provlog redaction, append only and queries", () => {
  it("redacts every secret shape from summaries before an entry lands", () => {
    const entry = provlog({
      ...
      rowkeys: ["r1"],
      summary:
        "streamed with token=abc123 and api-key: xyz and sk-abcdefghijklmnopqrstuv and ghp_AAAA0123456789aaaaaa and password:hunter2",
      now,
    }).entry;
  ...
  it("redacts pem private key blocks inside a summary", () => {
    ...
    const entry = provlog({
      ...
      operation: "transform",
      rowkeys: [],
      summary: `the pass carried ${pem} through`,
      now,
    }).entry;
  ...
  it("counts the pipeline states per run with empty inputs", () => {
    ...
    const state = pipelinestate({
      pipelines: [
        pipeline(),
        pipeline({ id: "p2", state: "paused", runid: "run2" }),
        pipeline({ id: "p3", state: "finished" }),
      ],
    });
    expect(state).toEqual({
      ...
      finished: 1,
      perrun: [
        { runid: "run1", running: 1, paused: 0, finished: 1 },
        { runid: "run2", running: 0, paused: 1, finished: 0 },
      ],
    });
    ...
    expect(() =>
      extractbatchof({
        ...
        pipeline: pipeline(),
        rows: [],
        cursor: { pipelineid: "p1", offset: 0, chunk: 0, updatedat: now },
        now,
      }),
    ).toThrow(/needs its id/);
  });
...
describe("torture: streamparse windows, chunks and validation", () => {
  it("splits byte ranges with zero, fractional and huge bounds while refusing non positive chunk bytes", () => {
    ...
    expect(streamparsewindows({ bytes: 10, chunkbytes: 4 }).windows).toEqual([
    ...
    expect(streamparsewindows({ bytes: 10, chunkbytes: 1_000_000 })).toEqual({
      chunks: 1,
  ...
  it("tokenizes chunks with element, attribute and text tokens while refusing over bound chunks", () => {
    const chunk = streamparsechunkof({
      index: 0,
      text: 'hello <div class="a">world</div> tail',
      chunkbytes: 100,
      last: true,
    });
    expect(chunk.tokens.map((token) => token.kind)).toEqual([
      "text",
      "element",
      "attribute",
      "text",
    ...
    expect(() => streamparsechunkof({ index: 0, text: "a".repeat(11), chunkbytes: 10, last: true })).toThrow(
      /over the 10 byte bound/,
    );
  });
  ...
  it("yields progressive observations and memory bounds", () => {
    ...
    const chunks = [
      streamparsechunkof({ index: 0, text: "one", chunkbytes: 10, last: false }),
      streamparsechunkof({ index: 1, text: "two <b>three</b>", chunkbytes: 20, last: true }),
    ];
  ...
  it("shapes worker tasks with positive bounds", () => {
    expect(streamparsetaskof({ runid: "r1", stepid: "s1", chunkbytes: 64, chunks: 3 })).toEqual({
      task: "streamparse",
      runid: "r1",
      stepid: "s1",
      chunkbytes: 64,
    ...
    expect(() => streamparsetaskof({ runid: "r1", stepid: "s1", chunkbytes: 0, chunks: 3 })).toThrow(
      /positive chunk bound/,
    );
    expect(() => streamparsetaskof({ runid: "r1", stepid: "s1", chunkbytes: -1, chunks: 3 })).toThrow(
      /positive chunk bound/,
    );
    expect(
      chunkextracttaskof({ runid: "r1", stepid: "s1", tableid: "t1", fingerprint: "f1", rowindex: 4, window: 2 }),
    ).toMatchObject({ task: "chunkextract", tableid: "t1" });
    expect(() =>
      chunkextracttaskof({ runid: "r1", stepid: "s1", tableid: "t1", fingerprint: "f1", rowindex: 4, window: 0 }),
    ).toThrow(/positive row window/);
    expect(() =>
      chunkextracttaskof({ runid: "r1", stepid: "s1", tableid: "t1", fingerprint: "f1", rowindex: 4, window: -3 }),
    ).toThrow(/positive row window/);
  });
});
...
describe("torture: chunkextract cursors, windows and merges", () => {
  it("opens cursors with zero row tables, defaults and refusals", () => {
    expect(openchunkextractcursor({ tableid: "t1", fingerprint: "f1", rows: 0 })).toMatchObject({
      rowindex: 0,
      complete: true,
    });
    expect(openchunkextractcursor({ tableid: "t1", fingerprint: "f1", rows: 10 })).toMatchObject({
      window: 10,
      complete: false,
    });
    expect(openchunkextractcursor({ tableid: "t1", fingerprint: "f1", rows: 10, window: 3 })).toMatchObject({
      window: 3,
      complete: false,
    });
    ...
    expect(() => openchunkextractcursor({ tableid: "t", fingerprint: "f", rows: 1, window: 0 })).toThrow(
      /positive row count/,
    );
    expect(() => openchunkextractcursor({ tableid: "t", fingerprint: "f", rows: 1, window: -2 })).toThrow(
      /positive row count/,
    );
  });
  ...
  it("slices windows at the end, past the end and larger than the table", () => {
    ...
    const first = chunkextractwindow({
      cursor: openchunkextractcursor({ tableid: "t1", fingerprint: "f1", rows: 5, window: 2 }),
      rows,
    });
    expect(first.rows).toEqual([{ a: "1" }, { a: "2" }]);
    expect(first.complete).toBe(false);
    const oversized = chunkextractwindow({
      cursor: { tableid: "t1", fingerprint: "f1", rowindex: 0, window: 100, complete: false },
      rows,
    });
    expect(oversized.rows).toHaveLength(5);
    expect(oversized.complete).toBe(true);
    const atend = chunkextractwindow({
      cursor: { tableid: "t1", fingerprint: "f1", rowindex: 5, window: 2, complete: true },
      rows,
    });
    expect(atend.rows).toEqual([]);
    ...
    expect(() =>
      chunkextractwindow({
        cursor: { tableid: "t1", fingerprint: "f1", rowindex: 6, window: 2, complete: false },
        rows,
      }),
    ).toThrow(/past the end/);
    expect(
      chunkextractwindow({ cursor: openchunkextractcursor({ tableid: "t", fingerprint: "f", rows: 0 }), rows: [] }),
    ).toMatchObject({ rows: [], complete: true });
  });
  ...
  it("resumes only the same table with the same fingerprint while a completed cursor never resumes", () => {
    ...
    expect(
      resumechunkextract({ cursor: { ...cursor, complete: true }, tableid: "t1", fingerprint: "f1" }).reason,
  ...
  it("merges windows without duplicating rows and reports the duplicates", () => {
    ...
    const windows = [
      ...
      { cursor: { tableid: "t", fingerprint: "f", rowindex: 2, window: 2, complete: true }, rows, complete: true },
      {
        cursor: { tableid: "t", fingerprint: "f", rowindex: 4, window: 2, complete: true },
        rows: [{ a: "3" }],
        complete: true,
      },
    ];
    ...
    const reordered = mergechunkwindows([
      {
        cursor: { tableid: "t", fingerprint: "f", rowindex: 1, window: 1, complete: true },
        rows: [{ b: "2", a: "1" }],
        complete: true,
      },
      {
        cursor: { tableid: "t", fingerprint: "f", rowindex: 0, window: 1, complete: true },
        rows: [{ a: "1", b: "2" }],
        complete: true,
      },
    ]);
  ...
  it("reads progress with zero totals and cursors past the end", () => {
    expect(
      chunkextractprogress({ tableid: "t", fingerprint: "f", rowindex: 3, window: 2, complete: false }, 5),
    ).toEqual({ extracted: 3, total: 5, share: 0.6, complete: false });
    expect(
      chunkextractprogress({ tableid: "t", fingerprint: "f", rowindex: 0, window: 2, complete: false }, 0).share,
    ).toBe(1);
    expect(
      chunkextractprogress({ tableid: "t", fingerprint: "f", rowindex: 9, window: 2, complete: false }, 5).share,
    ).toBe(1.8);
  });
});
