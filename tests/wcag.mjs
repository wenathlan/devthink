...
async function entry(spec, body) {
  try {
    ...
    executed.push({ id: spec.id, title: spec.title, criterion: spec.criterion, family: spec.family, module: spec.module, outcome: "pass", detail: detail ?? "" });
  } catch (error) {
    executed.push({ id: spec.id, title: spec.title, criterion: spec.criterion, family: spec.family, module: spec.module, outcome: "fail", detail: error instanceof Error ? error.message : String(error) });
  }
}
...
function withoutscripts(markup) {
  ...
  for (;;) {
    ...
    if (after !== "" && /[a-zA-Z0-9_]/.test(after)) { cursor = open + 7; continue; } /* a longer tag name that merely opens with the script prefix never strips */
...
const viewsources = ["views.ts", "sidepanel.ts", "popup.ts", "dashboardpage.ts", "optionspage.ts", "transparencypage.ts"];
...
export async function runwcagsuite() {
  if (!existsSync(designpath)) {
    ...
    return { release, checklist: "docs/wcag.md", entries: [{ id: 1, title: "the design file exists", criterion: "3.1.1", family: "surfaces", module: "web/extension/index.html", outcome: "fail", detail: "the design file is absent" }], summary: { total: 1, passed: 0, failed: 1 } };
  }
  ...
  const surfaces = [...design.matchAll(/<template data-surface="([^"]+)">([\s\S]*?)<\/template>/g)].map(match => ({ name: match[1], body: match[2] }));
  const required = ["site", "popup", "sidepanel", "dashboardpage", "optionspage", "transparencypage", "sandbox", "offscreen"];
  const styleclasses = new Set([...stylesheet.matchAll(/\.[a-z][a-z0-9-]*/g)].map(match => match[0].slice(1)));
  const contrastpairs = [];

  await entry({ id: 1, title: "every ui surface template the build splits is present and named", criterion: "2.4.3", family: "surfaces", module: "web/extension/index.html" }, async () => {
    ...
    check(names.length === required.length, `the design file carries ${names.length} surface templates instead of ${required.length}`);
    ...
    check(/createElement\("button"\)/.test(switcher) && /\.textContent\s*=\s*name/.test(switcher), "the surface switcher creates nav buttons without their text content");
  ...
  await entry({ id: 2, title: "every image carries its alt text and the decorative markers stay off the interactive elements", criterion: "1.1.1", family: "non-text content", module: "web/extension/index.html" }, async () => {
    let images = 0;
    for (const surface of surfaces) {
      for (const match of surface.body.matchAll(/<img\b([^>]*)>/g)) {
        images += 1;
        check(/alt\s*=\s*"[^"]*"/.test(match[1]), `the ${surface.name} surface carries an img element without its alt attribute`);
      }
    }
    let decorative = 0;
    for (const surface of surfaces) {
      for (const match of surface.body.matchAll(/<(div|span|iframe|img|svg)\b([^>]*)aria-hidden="true"([^>]*)>/g)) {
        ...
        check(attributes.tabindex === undefined, `the decorative ${match[1]} of the ${surface.name} surface carries a tabindex and stays inside the focus order`);
        check(!["button", "input", "select", "textarea", "a"].includes(match[1]), `the decorative marker of the ${surface.name} surface sits on the interactive element ${match[1]}`);
      }
      ...
      for (const match of surface.body.matchAll(/createElement\("(iframe|div|span)"\)/g)) {
        ...
        decorative += 1;
        check(!/tabindex/.test(region), `the decorative ${host} the ${surface.name} script hides carries a tabindex and stays inside the focus order`);
        check(/display\s*=\s*"none"|hidden/.test(region), `the decorative ${host} the ${surface.name} script marks stays visible`);
      }
    }
  ...
  await entry({ id: 3, title: "every button carries an accessible name and every field carries its label", criterion: "1.3.1", family: "names and labels", module: "web/extension/index.html" }, async () => {
    let buttons = 0;
    let fields = 0;
    for (const surface of surfaces) {
      for (const match of surface.body.matchAll(/<button\b([^>]*)>([\s\S]*?)<\/button>/g)) {
        ...
        check(attributes["aria-label"] !== undefined || attributes.title !== undefined || text !== "", `the button ${attributes.id ?? "unnamed"} of the ${surface.name} surface carries no accessible name`);
      }
      ...
      for (const match of surface.body.matchAll(/<(input|textarea|select)\b([^>]*?)\/?>/g)) {
        ...
        const labelled = attributes["aria-label"] !== undefined || attributes["aria-labelledby"] !== undefined || (attributes.id !== undefined && labelsof.has(attributes.id));
        check(labelled, `the ${match[1]} ${attributes.id ?? "unnamed"} of the ${surface.name} surface carries neither a label element, an aria-label nor an aria-labelledby`);
      }
    }
  ...
  await entry({ id: 4, title: "every surface heading hierarchy starts at the h1 with no skipped level", criterion: "1.3.1", family: "structure", module: "web/extension/index.html" }, async () => {
    const shapes = [];
    for (const surface of surfaces) {
      const levels = [...withoutscripts(surface.body).matchAll(/<h([1-6])\b/g)].map(match => Number(match[1]));
      if (levels.length === 0) { shapes.push(`${surface.name}: no headings`); continue; }
      check(levels[0] === 1, `the ${surface.name} surface starts its heading hierarchy at h${levels[0]} instead of the h1`);
      ...
      for (let index = 1; index < levels.length; index += 1) check(levels[index] - levels[index - 1] <= 1, `the ${surface.name} surface skips from h${levels[index - 1]} to h${levels[index]}`);
      shapes.push(`${surface.name}: ${levels.map(level => `h${level}`).join("")}`);
    }
    return `every surface heading hierarchy audited: ${shapes.join("; ")}.`;
  });

  await entry({ id: 5, title: "the default theme tokens meet the computed contrast ratios", criterion: "1.4.3", family: "contrast", module: "web/extension/index.html" }, async () => {
    const themeblock = /:root\s*\{([^}]*)\}/.exec(stylesheet)?.[1] ?? "";
    const tokens = {};
    for (const match of themeblock.matchAll(/--([a-z-]+):\s*(#[0-9a-fA-F]{6})/g)) tokens[match[1]] = match[2].toLowerCase();
    for (const name of ["theme-surface", "theme-elevated", "theme-text", "theme-muted", "theme-accent"]) check(tokens[name] !== undefined, `the stylesheet declares no --${name} token for the default theme`);
    const fallbacks = {};
    for (const match of (stylesheet.match(/:root\s*\{[^}]*--base:[^}]*\}/g) ?? []).join("").matchAll(/--([a-z-]+):\s*(#[0-9a-fA-F]{6})/g)) fallbacks[match[1]] = match[2].toLowerCase();
    for (const [fallback, theme] of [["base", "theme-surface"], ["text", "theme-text"], ["muted", "theme-muted"], ["accent", "theme-accent"]]) {
      check(fallbacks[fallback] === tokens[theme], `the fallback token --${fallback} (${fallbacks[fallback]}) disagrees with the theme token --${theme} (${tokens[theme]})`);
    }
    const pairs = [
      { name: "text on surface", foreground: tokens["theme-text"], background: tokens["theme-surface"], minimum: 4.5 },
      { name: "text on elevated", foreground: tokens["theme-text"], background: tokens["theme-elevated"], minimum: 4.5 },
      { name: "muted on surface", foreground: tokens["theme-muted"], background: tokens["theme-surface"], minimum: 4.5 },
      { name: "muted on elevated", foreground: tokens["theme-muted"], background: tokens["theme-elevated"], minimum: 4.5 },
      { name: "accent on surface", foreground: tokens["theme-accent"], background: tokens["theme-surface"], minimum: 3 },
      { name: "accent on elevated", foreground: tokens["theme-accent"], background: tokens["theme-elevated"], minimum: 3 },
    ];
    ...
    const inks = new Set();
    for (const match of stylesheet.matchAll(/button\s*\{[^}]*\}/g)) for (const ink of match[0].matchAll(/color:\s*(#[0-9a-fA-F]{6})/g)) inks.add(ink[1].toLowerCase());
    ...
    for (const ink of inks) {
      ...
      check(ratio >= 4.5, `the button ink ${ink} on the accent background answers ${ratio}:1 under the required 4.5:1`);
    }
  ...
  await entry({ id: 6, title: "the stylesheet carries the focus visible outlines", criterion: "2.4.7", family: "focus visible", module: "web/extension/index.html" }, async () => {
    ...
    for (const selector of ["input:focus-visible", "textarea:focus-visible"]) check(rule.includes(selector), `the focus visible rule misses the ${selector} selector`);
  ...
  await entry({ id: 7, title: "no positive tabindex reorders the focus order of any surface", criterion: "2.4.3", family: "focus order", module: "web/index.html and dist/extension" }, async () => {
    let scanned = 0;
    const offenders = [];
    for (const surface of surfaces) {
    ...
    check(built.filter(name => name.endsWith(".html")).length >= required.length - 1, `the built surface pages are absent; run node tests/build.mjs before the accessibility sweep`);
  ...
  await entry({ id: 8, title: "every built surface page stamps its html lang and no template overrides it", criterion: "3.1.1", family: "language", module: "web/index.html, tests/build.mjs and dist/extension" }, async () => {
    ...
    for (const surface of surfaces) {
      ...
      check(override === null, `the ${surface.name} template overrides the html element the build stamps: ${override?.[0]}`);
    }
    ...
    check(stamped.length >= required.length - 1, `the built surface pages are absent; run node tests/build.mjs before the accessibility sweep`);
  ...
  await entry({ id: 9, title: "the generated markup classes resolve to real stylesheet rules", criterion: "1.3.1", family: "generated markup", module: "views.ts, sidepanel.ts, popup.ts, dashboardpage.ts and optionspage.ts" }, async () => {
    const templateclasses = new Set();
    for (const surface of surfaces) for (const match of withoutscripts(surface.body).matchAll(/class="([^"]+)"/g)) for (const token of match[1].split(/\s+/)) if (token) templateclasses.add(token);
    const missingtemplate = [...templateclasses].filter(token => !styleclasses.has(token)).sort();
    check(missingtemplate.length === 0, `the surface templates reference classes the stylesheet never declares: ${missingtemplate.join(", ")}`);
    ...
    for (const source of viewsources) {
      ...
      for (const match of text.matchAll(/className = "([^"]+)"/g)) for (const token of match[1].split(/\s+/)) if (token) assigned.add(token);
    }
    const missingtoggled = [...toggled].filter(token => !styleclasses.has(token)).sort();
    check(missingtoggled.length === 0, `the view scripts toggle state classes the stylesheet never declares: ${missingtoggled.join(", ")}`);
    ...
    check(missingshared.length === 0, `the shared class vocabulary the view scripts assign never resolves: ${missingshared.join(", ")}`);
  ...
  await entry({ id: 10, title: "the sweep report records every check and the exit answers the failures", criterion: "2.4.3", family: "report", module: "wcag.mjs" }, async () => {
    ...
    for (const candidate of recorded) {
      ...
      check(/^[0-9]\.[0-9]+\.[0-9]+$/.test(candidate.criterion), `the check ${candidate.id} carries no wcag success criterion reference`);
    }
    ...
    check(summary.failed === 0, `${summary.failed} accessibility check${summary.failed === 1 ? "" : "s"} failed before the report landed`);
