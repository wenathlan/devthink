...
describe("agentstream event subscriptions", () => {
  it("registers subscriptions with their kinds and filters and refuses unknown kinds", () => {
    ...
    expect(subscriberegister({ clientid: "client1", kinds: ["bogus" as never], now }).reason).toMatch(
      /not a protocol event kind/i,
    );
  ...
  it("pushes protocol events to the matching subscribers only", () => {
    const one = subscriberegister({ clientid: "client1", kinds: ["callresult"], now, id: "sub1" })
      .subscription as NonNullable<ReturnType<typeof subscriberegister>["subscription"]>;
    const two = subscriberegister({
      clientid: "client2",
      kinds: ["callresult", "progress"],
      origin: "https://example.com",
      now,
    ...
    const cancelled = {
      ...(subscriberegister({ clientid: "client4", kinds: ["callresult"], now, id: "sub4" })
        .subscription as NonNullable<ReturnType<typeof subscriberegister>["subscription"]>),
      canceledat: now,
    };
    const delivered = notifyevent({
      subscriptions: [one, two, three, cancelled],
      kind: "callresult",
      origin: "https://example.com",
      tool: "browser.readtext",
      payload: { ok: true },
    ...
    expect(delivered.deliveries[0]?.frame.params?.kind).toBe("callresult");
...
describe("agentstream resource watchers", () => {
  ...
  it("pushes the page state deltas of changed keys and absorbs the baseline", () => {
    const watch = watchresource({
      ...
      now,
      id: "watch1",
    }).watch as NonNullable<ReturnType<typeof watchresource>["watch"]>;
    const unchanged = notifyresource({
      watches: [watch],
      resource: "page",
      state: { url: "https://example.com", title: "Example" },
      now: now + 1,
    ...
    const changed = notifyresource({
      watches: [watch],
      resource: "page",
      state: { url: "https://example.com/next", title: "Example" },
      now: now + 2,
    });
    ...
    const other = notifyresource({
      watches: changed.watches,
      resource: "other",
      state: { url: "https://elsewhere.example" },
      now: now + 3,
    });
...
describe("agentstream sampling callbacks", () => {
  it("sends sampling requests that respect the client capabilities and strip ungranted page content", () => {
    ...
    expect(
      requestsampling({ clientid: "client1", capabilities: caps(false), prompt: "Summarize.", now }).reason,
    ).toMatch(/declared no sampling capability/i);
    expect(requestsampling({ clientid: "client1", capabilities: caps(true), prompt: " ", now }).reason).toMatch(
      /needs its prompt/i,
    );
    expect(
      requestsampling({ clientid: "client1", capabilities: caps(true), prompt: "Summarize.", maxtokens: 0, now })
        .reason,
    ).toMatch(/positive user value/i);
  ...
  it("completes the sampling round trip with answers and refusals while closed requests stay untouched", () => {
    const request = requestsampling({ clientid: "client1", prompt: "Summarize.", now, id: "sample1" })
      .request as NonNullable<ReturnType<typeof requestsampling>["request"]>;
    ...
    expect(
      answersampling({ requests: answered.requests, id: "sample1", answer: "again", now: now + 20 }).reason,
    ).toMatch(/already closed/i);
    expect(answersampling({ requests: answered.requests, id: "missing", now }).reason).toMatch(
      /names no stored request/i,
    );
    const refused = requestsampling({ clientid: "client2", prompt: "Summarize.", now, id: "sample2" })
      .request as NonNullable<ReturnType<typeof requestsampling>["request"]>;
...
describe("agentstream prompt tools", () => {
  ...
  it("renders the templates and calls the prompts with defaults and required refusals", () => {
    ...
    const rendered = renderprompt(runreview as NonNullable<typeof runreview>, {
    ...
    expect(called.toolcall?.params.arguments).toMatchObject({
      objective: "fill the form",
      steps: "click, type",
      tone: "plain",
    });
...
describe("agentstream streaming, progress and cancellation", () => {
  it("emits ordered stream chunks with the done marker and reassembles them", () => {
    ...
    expect(chunkcontent({ callid: "call2", content: "", now })).toEqual([
      expect.objectContaining({ content: "", done: true }),
    ]);
  });
  ...
  it("aborts in flight calls on their cancel frame and preserves the partial result", () => {
    ...
    expect(canceltool({ contexts: aborted.contexts, callid: "call1", now: now + 200 }).reason).toMatch(
      /already left the in flight state/i,
    );
    expect(canceltool({ contexts: aborted.contexts, callid: "missing", now }).reason).toMatch(/names no call context/i);
    const finished: callcontext = {
      callid: "call2",
      clientid: "client1",
      tool: "memory.list",
      state: "done",
    ...
    expect(canceltool({ contexts: [finished], callid: "call2", now }).reason).toMatch(
      /already left the in flight state/i,
    );
  });
});
