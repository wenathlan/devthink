...
import { applycooldown, armrule, confirmmanualrun, cronnext, cronparse, defaulttriggercooldown, drainqueue, evaluatetrigger, eventrulematches, listdue, manualpreview, matchurl, observeevents, pauseall, queuefire, resumeall, ruleoriginsgranted, runurllist, schedulecron, scheduleinterval, timezonevalid, triggereventcatalog, triggerfamilyof, triggerkinds, triggersummary, updaterule, verifywebhook, visitmatch, webhooksecretok } from "../workflow.js";
import { actionrisk, canexecute, isworkflowkind, istriggeraction, triggergate, triggerorigins, validatestep } from "../policy.js";
...
import type { agentplan, agentsession, manualrun, toolstep, triggerfire, triggerule, workflowrecord } from "../types.js";
...
const plan: agentplan = { id: "run", objective: "Run the reviewed triggers", origin: "https://example.com", steps: [], createdat: now - 2000, expiresat: now + 600_000, state: "approved" };
...
function triggerstep(kind: string, payload: Record<string, unknown>, extra: Record<string, unknown> = {}): toolstep {
  return { id: "t1", kind: kind as toolstep["kind"], summary: `The reviewed ${kind} step`, risk: "sensitive", options: JSON.stringify({ workflowid: "wf1", reviewed: true, rule: payload, ...extra }) };
}
...
describe("trigger kinds, families and rule arming", () => {
  it("lists the ten trigger kinds and grades every automatic launcher sensitive", () => {
    expect(triggerkinds).toEqual(["visitrule", "urlrule", "menurule", "keyrule", "buttonrule", "cronrule", "intervalrule", "urllistrule", "webhookrule", "eventrule"]);
  ...
  it("arms every family behind its grammar and applies the documented webhook and event cooldown defaults", () => {
    ...
    expect(armedrule("urlrule", { pattern: "https://example.com/*/detail" }).pattern).toBe("https://example.com/*/detail");
    ...
    expect(armedrule("cronrule", { cron: "*/15 * * * *", timezone: "America/New_York" }).timezone).toBe("America/New_York");
    ...
    expect(armedrule("webhookrule", { secret: "devthink-secret-2026-abc123", schema: [{ name: "task", kind: "string", required: true }] }).state.cooldown).toBe(defaulttriggercooldown);
  ...
  it("refuses malformed family payloads and cooldown windows", () => {
    expect(armrule({ family: "visit", workflowid: "wf1", payload: { origins: ["http://insecure.example"] }, now })).toBeUndefined();
    ...
    expect(armrule({ family: "cron", workflowid: "wf1", payload: { cron: "* * * * *", timezone: "Not/AZone" }, now })).toBeUndefined();
    expect(armrule({ family: "interval", workflowid: "wf1", payload: { period: 0 }, now })).toBeUndefined();
    expect(armrule({ family: "interval", workflowid: "wf1", payload: { period: 1000, jitter: -1 }, now })).toBeUndefined();
    expect(armrule({ family: "urllist", workflowid: "wf1", payload: { urls: ["https://example.com/a", "nope"] }, now })).toBeUndefined();
    expect(armrule({ family: "webhook", workflowid: "wf1", payload: { secret: "short", schema: [{ name: "task", kind: "string" }] }, now })).toBeUndefined();
    expect(armrule({ family: "webhook", workflowid: "wf1", payload: { secret: "devthink-secret-2026-abc123", schema: [] }, now })).toBeUndefined();
    expect(armrule({ family: "event", workflowid: "wf1", payload: { events: ["nope"] }, now })).toBeUndefined();
    expect(armrule({ family: "event", workflowid: "wf1", payload: { events: ["mutate"] }, cooldown: 0, now })).toBeUndefined();
  ...
  it("summarizes every family for the review panel and checks the rule origins against the workflow grants", () => {
    ...
    expect(triggersummary(armedrule("webhookrule", { secret: "devthink-secret-2026-abc123", schema: [{ name: "task", kind: "string" }] })).fields).toBe(1);
...
describe("trigger url matching", () => {
  ...
  it("fires visit rules when a navigation lands on a reviewed origin", () => {
    ...
    const decision = evaluatetrigger({ rule, now, cause: "visit", url: "https://example.com/articles/one", title: "One", runactive: false, workflowreviewed: true });
...
describe("cron parsing and scheduling", () => {
  ...
  it("computes cron fire times inside the reviewed timezone through the runtime timezone database", () => {
    ...
    expect(schedulecron({ cron: "0 9 * * *", timezone: "America/New_York" }, utcfrom)).toBe(Date.UTC(2026, 0, 15, 14, 0));
  });

  it("spreads interval fires inside the reviewed jitter window with the seeded random source", () => {
    const samples = new Set<number>();
    for (let seed = 0; seed < 200; seed += 1) samples.add(scheduleinterval({ period: 60_000, jitter: 20_000 }, 1_000_000, 500_000, seed));
  ...
  it("returns the scheduled rules whose next fire time has passed", () => {
    ...
    const listed = listdue([{ ...due, state: { ...due.state, nextfireat: now - 5000 } }, { ...later, state: { ...later.state, nextfireat: now + 500_000 } }, { ...interval, state: { ...interval.state, nextfireat: now - 1 } }, { ...disabled, state: { ...disabled.state, nextfireat: now - 1 } }, { ...paused, state: { ...paused.state, nextfireat: now - 1 } }], now);
...
describe("trigger evaluation, cooldown, dedupe and the queue", () => {
  it("suppresses fires inside the cooldown window and reports the remaining window", () => {
    ...
    expect(evaluatetrigger({ rule: { ...cooled, state: { ...cooled.state, lastfireat: now - 60_000 } }, now, cause: "event", runactive: false, workflowreviewed: true }).fired).toBe(true);
  });
  ...
  it("never fires disabled, paused or unreviewed rules and dedupes while a run is active", () => {
    ...
    expect(evaluatetrigger({ rule: updaterule(rule, { state: { enabled: false } }), now, cause: "visit", runactive: false, workflowreviewed: true }).suppressed).toBe("disabled");
    expect(evaluatetrigger({ rule: updaterule(rule, { state: { pausedat: now } }), now, cause: "visit", runactive: false, workflowreviewed: true }).suppressed).toBe("paused");
    expect(evaluatetrigger({ rule, now, cause: "visit", runactive: false, workflowreviewed: false }).suppressed).toBe("unreviewed");
    expect(evaluatetrigger({ rule, now, cause: "visit", runactive: true, workflowreviewed: true }).suppressed).toBe("dedupe");
  });
  ...
  it("drains the queued fires in arrival order and keeps the rest when a launch fails", async () => {
    const fires: triggerfire[] = [{ id: "f1", ruleid: "r1", at: now, cause: "visit" }, { id: "f2", ruleid: "r2", at: now + 1, cause: "url" }];
    const launched: string[] = [];
    const drained = await drainqueue(fires, async fire => { launched.push(fire.id); });
    expect(launched).toEqual(["f1", "f2"]);
    expect(drained).toEqual({ launched: 2, remaining: [] });
    const failing = await drainqueue(fires, async fire => { if (fire.id === "f1") throw new Error("the run refused the launch"); launched.push(fire.id); });
  ...
  it("suspends every enabled rule on session pause and releases them on resume", () => {
    const rules = [armedrule("visitrule", { origins: ["https://example.com"] }), armedrule("cronrule", { cron: "* * * * *" }), updaterule(armedrule("eventrule", { events: ["mutate"] }), { state: { enabled: false } })];
  ...
  it("plans one run per url of a url list rule", () => {
    const rule = armedrule("urllistrule", { urls: ["https://example.com/a", "https://example.com/b", "https://example.com/c"] });
    const fires = runurllist(rule, now);
    expect(fires).toHaveLength(3);
    expect(fires.every(fire => fire.ruleid === rule.id && fire.cause === "urllist")).toBe(true);
    expect(fires.map(fire => fire.url)).toEqual(["https://example.com/a", "https://example.com/b", "https://example.com/c"]);
  });
});
...
describe("webhook verification and page event subscription", () => {
  it("verifies the shared secret and the payload schema before anything persists", () => {
    const rule = armedrule("webhookrule", { secret: "devthink-secret-2026-abc123", schema: [{ name: "task", kind: "string", required: true }, { name: "count", kind: "number" }, { name: "urgent", kind: "boolean" }] });
    expect(verifywebhook({ rule, secret: "devthink-secret-2026-abc123", payload: { task: "read", count: 3 } }).verified).toBe(true);
    expect(verifywebhook({ rule, secret: "wrong-secret-2026-abc1234", payload: { task: "read" } }).reason).toContain("secret");
    expect(verifywebhook({ rule, secret: "devthink-secret-2026-abc123", payload: { count: 3 } }).reason).toContain("task");
    expect(verifywebhook({ rule, secret: "devthink-secret-2026-abc123", payload: { task: 7 } }).reason).toContain("task");
    expect(verifywebhook({ rule, secret: "devthink-secret-2026-abc123", payload: { task: "read", urgent: "yes" } }).reason).toContain("urgent");
    expect(verifywebhook({ rule, secret: "devthink-secret-2026-abc123", payload: "not an object" }).reason).toContain("object");
    expect(verifywebhook({ rule: { ...rule, secret: undefined } as unknown as triggerule, secret: "", payload: {} }).verified).toBe(false);
  ...
  it("subscribes event rules to the observed event catalog only", () => {
    ...
    const decision = evaluatetrigger({ rule, now, cause: "event", payload: { event: "mutate" }, runactive: false, workflowreviewed: true });
...
describe("manual runs with the step preview", () => {
  it("renders the expanded step preview with control summaries before anything runs", () => {
    const record: workflowrecord = composeworkflow({ id: "wf1", name: "reader", version: 1, origins: ["https://example.com"], steps: [
      workflowstepof({ id: "s1", kind: "readtext", label: "Read the heading", target: "h1" }) as never,
      { id: "s2", kind: "branch", label: "Branch by title", options: JSON.stringify({ branch: { paths: [{ name: "short", when: { left: { ref: "title" }, right: { literal: "x" }, operator: "contains", result: "match", resultkind: "boolean" }, steps: [{ id: "s3", kind: "wait", label: "Wait a beat", value: "10" }] }], else: { name: "else", steps: [{ id: "s4", kind: "wait", label: "Wait longer", value: "20" }] } } }) } as never,
    ], now, kindallowed, riskof });
...
describe("trigger policy validation and sensitivity grading", () => {
  it("validates the grammar of every trigger kind", () => {
    expect(validatestep(triggerstep("visitrule", { origins: ["https://example.com"] }), "https://example.com").allowed).toBe(true);
    expect(validatestep(triggerstep("urlrule", { pattern: "https://example.com/*" }), "https://example.com").allowed).toBe(true);
    expect(validatestep(triggerstep("menurule", { title: "Run the reader" }), "https://example.com").allowed).toBe(true);
    ...
    expect(validatestep(triggerstep("intervalrule", { period: 60_000, jitter: 5_000 }), "https://example.com").allowed).toBe(true);
    expect(validatestep(triggerstep("urllistrule", { urls: ["https://example.com/a"] }), "https://example.com").allowed).toBe(true);
    expect(validatestep(triggerstep("webhookrule", { secret: "devthink-secret-2026-abc123", schema: [{ name: "task", kind: "string", required: true }] }), "https://example.com").allowed).toBe(true);
    expect(validatestep(triggerstep("eventrule", { events: ["mutate"] }), "https://example.com").allowed).toBe(true);
    expect(validatestep(triggerstep("eventrule", { events: ["mutate"] }, { label: "Panel rule" }), "https://example.com").allowed).toBe(true);
  });

  it("refuses rules without the arm review, weak secrets, unparseable crons and unknown events", () => {
    expect(validatestep(triggerstep("visitrule", { origins: ["https://example.com"] }, { reviewed: false }), "https://example.com").reason).toContain("arm review");
    expect(validatestep(triggerstep("visitrule", { origins: ["https://example.com"] }, { workflowid: "" }), "https://example.com").reason).toContain("workflow");
    expect(validatestep(triggerstep("cronrule", { cron: "99 * * * *" }), "https://example.com").reason).toContain("cron");
    expect(validatestep(triggerstep("webhookrule", { secret: "short", schema: [{ name: "task", kind: "string" }] }), "https://example.com").reason).toContain("secret");
    expect(validatestep(triggerstep("eventrule", { events: ["nope"] }), "https://example.com").reason).toContain("event");
    ...
    expect(validatestep(triggerstep("keyrule", { command: "Bad Command" }), "https://example.com").reason).toContain("keyboard");
    expect(validatestep(triggerstep("visitrule", { origins: ["http://insecure.example"] }), "https://example.com").reason).toContain("visit");
    expect(validatestep(triggerstep("urlrule", { pattern: "no-scheme/*" }), "https://example.com").reason).toContain("url rule");
    expect(validatestep(triggerstep("eventrule", { events: ["mutate"] }, { cooldown: 0 }), "https://example.com").reason).toContain("cooldown");
    expect(validatestep(triggerstep("eventrule", { events: ["mutate"] }, { label: "" }), "https://example.com").reason).toContain("label");
    expect(validatestep({ ...triggerstep("buttonrule", {}), kind: "nope" as toolstep["kind"] }, "https://example.com").reason).toContain("Unsupported");
  });
  ...
  it("gates every trigger kind behind the live session, the approved plan and the arm review", () => {
    ...
    expect(triggergate({ session: undefined, plan, step, tabid: 4, origin: "https://example.com", now }).allowed).toBe(false);
    expect(triggergate({ session: { ...session, stoppedat: now }, plan, step, tabid: 4, origin: "https://example.com", now }).allowed).toBe(false);
    expect(triggergate({ session, plan: { ...plan, state: "pending" }, step, tabid: 4, origin: "https://example.com", now }).reason).toContain("approved plan");
    expect(triggergate({ session, plan, step: triggerstep("visitrule", { origins: ["https://example.com"] }, { reviewed: false }), tabid: 4, origin: "https://example.com", now }).reason).toContain("arm review");
    expect(canexecute({ session: undefined, plan, step, tabid: 4, origin: "https://example.com", now }).allowed).toBe(false);
  ...
  it("extracts the match origins of a rule so proposals stay inside the grants", () => {
    expect(triggerorigins(triggerstep("visitrule", { origins: ["https://example.com", "https://example.com"] }))).toEqual(["https://example.com"]);
    expect(triggerorigins(triggerstep("urlrule", { pattern: "https://example.com/*" }))).toEqual(["https://example.com"]);
    expect(triggerorigins(triggerstep("urllistrule", { urls: ["https://example.com/a", "https://other.example/b"] }))).toEqual(["https://example.com", "https://other.example"]);
    expect(triggerorigins(triggerstep("buttonrule", {}))).toEqual([]);
    expect(triggerorigins(triggerstep("visitrule", { origins: ["https://example.com"] }, { reviewed: false }))).toEqual(["https://example.com"]);
  });
});

describe("trigger protocol envelopes", () => {
  it("builds the trigger list with workflow names, counters and the queue depth", () => {
    const cronrule = { ...armedrule("cronrule", { cron: "0 9 * * *" }), state: { ...armedrule("cronrule", { cron: "0 9 * * *" }).state, nextfireat: now + 3600_000 } };
    ...
    const list = triggerlist({ rules: [cronrule, unbound], workflows: [{ id: "wf1", name: "reader", version: 1, origins: ["https://example.com"], steps: [], blocks: [], risk: "read", createdat: now }], queue: [{ id: "f1", ruleid: "r1", at: now, cause: "visit" }] });
  ...
  it("builds the trigger fired notification with the run binding and returns the next fire time per rule", () => {
    const fired = triggerfired({ fire: { id: "f1", ruleid: "r1", at: now, cause: "webhook", url: "https://example.com", payload: { task: "read" } }, workflowid: "wf1", runid: "run1" });
    expect(fired.triggerfired).toEqual({ fireid: "f1", ruleid: "r1", workflowid: "wf1", at: now, cause: "webhook", url: "https://example.com", runid: "run1" });
    const body = JSON.parse(outcomeresponse({ outcome: { stepid: "t1", ok: true, summary: "Armed.", at: now }, plan, trigger: { ruleid: "r1", kind: "cron", enabled: true, nextfireat: now + 60_000 } }));
    expect(body.trigger).toEqual({ ruleid: "r1", kind: "cron", enabled: true, nextfireat: now + 60_000 });
    const without = JSON.parse(outcomeresponse({ outcome: { stepid: "t1", ok: true, summary: "Armed.", at: now }, plan }));
  ...
  it("refuses trigger proposals without the arm review or with origins outside the grants", () => {
    const proposal = parseproposal({ version: protocolversion, plan: { objective: "Arm triggers", steps: [
      { kind: "visitrule", options: JSON.stringify({ workflowid: "wf1", reviewed: true, rule: { origins: ["https://example.com"] } }), summary: "Arm the visit rule." },
    ] } }, "https://example.com");
    expect(proposal.plan.steps[0]?.kind).toBe("visitrule");
    expect(() => parseproposal({ version: protocolversion, plan: { objective: "Unreviewed arm", steps: [
      { kind: "visitrule", options: JSON.stringify({ workflowid: "wf1", rule: { origins: ["https://example.com"] } }), summary: "Arm without review." },
    ] } }, "https://example.com")).toThrow("arm review");
    expect(() => parseproposal({ version: protocolversion, plan: { objective: "Outside grants", steps: [
      { kind: "visitrule", options: JSON.stringify({ workflowid: "wf1", reviewed: true, rule: { origins: ["https://other.example"] } }), summary: "Arm outside the grants." },
    ] } }, "https://example.com")).toThrow("outside the grants");
    expect(() => parseproposal({ version: protocolversion, plan: { objective: "Url list outside grants", steps: [
      { kind: "urllistrule", options: JSON.stringify({ workflowid: "wf1", reviewed: true, rule: { urls: ["https://elsewhere.example/a"] } }), summary: "Arm the url list." },
    ] } }, "https://example.com")).toThrow("outside the grants");
  });
});
...
describe("trigger memory persistence", () => {
  it("stores rules with their workflow names, replaces state and removes disarmed rules", async () => {
    ...
    await store.addworkflowrecord({ id: "wf1", name: "reader", version: 1, origins: ["https://example.com"], steps: [], blocks: [], risk: "read", createdat: now });
  ...
  it("stores webhook payloads and manual run previews with their confirmation outcomes", async () => {
    ...
    const preview: manualrun = { id: "m1", workflowid: "wf1", preview: [{ stepid: "s1", kind: "readtext", label: "Read the heading" }], at: now };
  ...
  it("records trigger evidence with the fire, launch and suppression counts", () => {
    let progress = recordtrigger(undefined, plan.id, "t1", { family: "arm", detail: "Armed the visit rule", ruleid: "r1", nextfireat: now + 60_000 }, now);
    progress = recordtrigger(progress, plan.id, "t1", { family: "fire", detail: "The visit rule fired", ruleid: "r1", fires: 1, launches: 1 }, now + 1);
    progress = recordtrigger(progress, plan.id, "t1", { family: "suppress", detail: "The visit rule suppressed a fire", ruleid: "r1", suppressions: 1, queued: 0 }, now + 2);
