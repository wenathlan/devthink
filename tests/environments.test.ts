...
describe("execution environments", () => {
  it("routes every reviewed action kind to an allowed environment set with a default", () => {
    ...
    for (const requirement of requirements) {
      ...
      for (const environment of requirement.environments)
        expect(["pagecontext", "isolatedworld", "offscreenworker", "sandboxframe"]).toContain(environment);
    }
  });
  ...
  it("marks evaluate steps as isolatedworld only and untrusted markup renders as sandboxframe only", () => {
    ...
    const markupstep = step("setattribute", {
      options: JSON.stringify({ name: "data-summary", value: "inert", markup: "<p>untrusted</p>" }),
    });
    ...
    expect(
      markuprenderstep(step("setattribute", { options: JSON.stringify({ name: "data-summary", value: "inert" }) })),
    ).toBe(false);
  ...
  it("marks the parse heavy read kinds as offscreenworker eligible beside the live page", () => {
    ...
    expect(families.map((family) => family.task)).toEqual([
      "htmlsnapshot",
      "jsonpayload",
      "tablerows",
      "a11ytree",
  ...
  it("maps every environmentkind to its runtime adapter in the executor registry", () => {
    ...
    expect(registry.map((entry) => entry.environment)).toEqual([
      "pagecontext",
      "isolatedworld",
      "offscreenworker",
      "sandboxframe",
    ]);
    expect(registry.map((entry) => entry.adapter)).toEqual([
      "pagebridge",
      "scriptingapi",
      "offscreendocument",
      "sandboxpage",
  ...
  it("routes steps by the reviewed environment field, the offload toggle and the capability grant with an inline fallback", () => {
    ...
    expect(routeenvironment(step("readhtml"), { offload: true, granted: true })).toMatchObject({
    ...
    const named = routeenvironment(step("readhtml", { environment: "offscreenworker" }), {
      offload: false,
      granted: true,
    });
  ...
  it("builds worker requests with the parse family and the transferable keys and reads their answers", () => {
    ...
    const request = workerrequestof({
      ...
      stepid: "s1",
      kind: "readhtml",
      payload: "<html><body>heavy</body></html>",
      options: { snapshot: buffer, label: "front page" },
      sentat: now,
    });
    expect(request.task).toBe("htmlsnapshot");
    expect(request.transferables).toEqual(["snapshot"]);
    const partial = acceptworkerresponse(
      workerresponseof({
        id: "a1",
        requestid: request.id,
        ok: true,
        partial: 1,
        summary: "The first chunk streams back.",
        receivedat: now + 5,
      }),
    );
    expect(partial).toMatchObject({ done: false, partial: true });
    const final = acceptworkerresponse(
      workerresponseof({
        id: "a2",
        requestid: request.id,
        ok: true,
        result: "front page text",
        summary: "The parse finished.",
        receivedat: now + 20,
      }),
    );
    expect(final).toMatchObject({ done: true, partial: false, result: "front page text" });
    const refused = acceptworkerresponse(
      workerresponseof({
        id: "a3",
        requestid: request.id,
        ok: false,
        summary: "The payload was malformed.",
        receivedat: now + 25,
      }),
    );
    ...
    expect(() =>
      workerrequestof({ id: "w2", runid: "run1", stepid: "s2", kind: "click", payload: "x", sentat: now }),
    ).toThrow(/outside the offscreen worker pool families/i);
    expect(() =>
      workerrequestof({ id: "w3", runid: "run1", stepid: "s3", kind: "readjson", payload: "  ", sentat: now }),
    ).toThrow(/payload/i);
    expect(() => workerresponseof({ id: "w4", requestid: "r", ok: true, summary: " ", receivedat: now })).toThrow(
      /summary/i,
    );
  });
  ...
  it("spawns the offscreen document on first use, reuses it across the steps of one run and closes it at completion", () => {
    ...
    expect(() =>
      openoffscreen({ registry: [], document: " ", runid: "run1", reasons: ["DOM_PARSER"], justification: "j", now }),
    ).toThrow(/document/i);
    expect(() =>
      openoffscreen({ registry: [], document: "offscreen.html", runid: "run1", reasons: [], justification: "j", now }),
    ).toThrow(/reasons/i);
    expect(() =>
      openoffscreen({
        registry: [],
  ...
  it("builds the isolated world injection of evaluate steps with reviewed arguments only", () => {
    const injection = isolatedinjection(
      step("evaluate", { value: "document.title", options: JSON.stringify(["arg one", "arg two"]) }),
    );
