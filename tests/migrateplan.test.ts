...
import { importautoma, importselenium, importtabular, importuivision, importv1plan, migrateplanconversion, parsecsvrows, planlintfindings } from "../cli.js";
...
const refusals: Array<{ name: string; format: string; pattern: RegExp }> = [
  { name: "v1-unknown.json", format: "v1", pattern: /version one step snap[\s\S]*screenshotpage outside the reviewed catalog/ },
  { name: "automa-unknown.json", format: "automa", pattern: /automa block ping of type notification stays outside the conversion table/ },
  { name: "selenium-unknown.json", format: "selenium", pattern: /selenium command roll of mouseOver stays outside the conversion table/ },
  { name: "uivision-unknown.json", format: "uivision", pattern: /ui vision command xclick-2 of XClick stays outside the conversion table/ },
  { name: "tabular-unknown.csv", format: "tabular", pattern: /tabular row 2[\s\S]*dragdrop outside the reviewed catalog/ }
];
...
describe("migrateplan importer conversion", () => {
  ...
  it("converts the automa blocks through the documented table and lands the trigger block in the report as the paused trigger", async () => {
    ...
    const fields = JSON.parse(conversion.file.steps.find(step => step.id === "survey")?.options ?? "{}") as { fields: Array<Record<string, string>> };
    ...
    expect(JSON.parse(conversion.file.steps.find(step => step.id === "hold")?.options ?? "{}")).toEqual({ delay: 1500 });
  ...
  it("resolves the selenium locators, the open target and the pause milliseconds onto the reviewed grammar", async () => {
    ...
    expect(JSON.parse(conversion.file.steps.find(step => step.id === "hold")?.options ?? "{}")).toEqual({ delay: 2000 });
...
describe("migrateplan refusal semantics", () => {
  ...
  it("refuses the formats, the shapes and the locators it cannot map instead of guessing", async () => {
    expect(() => migrateplanconversion({ format: "playwright", text: "{}", now })).toThrow(/stays outside the v1, automa, selenium, uivision and tabular source formats/);
    expect(() => migrateplanconversion({ format: "v1", text: "not json", now })).toThrow(/v1 source parses no json/);
    expect(() => importv1plan({ source: { version: "1", goal: "g", origin: "https://example.org", steps: [{ id: "s", action: "observe", label: "l", legacy: true }] }, now })).toThrow(/unknown field legacy/);
    expect(() => importv1plan({ source: { version: "1", goal: "g", origin: "http://example.org", steps: [{ id: "s", action: "observe", label: "l" }] }, now })).toThrow(/HTTPS/);
    expect(() => importv1plan({ source: { version: "1", goal: "g", origin: "https://example.org", steps: [{ id: "s", action: "click", label: "l", milliseconds: 5 }] }, now })).toThrow(/only the delay action lifts/);
    expect(() => importautoma({ source: { name: "n", blocks: { tab: { type: "newtab", data: { url: "https://example.org" } }, away: { type: "newtab", data: { url: "https://other.example" } } } }, now })).toThrow(/automa block away of type newtab navigates to https:\/\/other\.example/);
    expect(() => importautoma({ source: { name: "n", blocks: { only: { type: "click-element", data: { selector: "button" } } } }, now })).toThrow(/carries no newtab block/);
    expect(() => importselenium({ source: { version: "2.0", name: "n", url: "https://example.org", tests: [] }, now })).toThrow(/carries 0 tests; one plan addresses exactly one test/);
    expect(() => importselenium({ source: { version: "2.0", name: "n", url: "https://example.org", tests: [{ name: "t", commands: [{ id: "x", command: "click", target: "xpath=//button", value: "" }] }] }, now })).toThrow(/outside the css=, id= and name= locator prefixes/);
    expect(() => importuivision({ source: { Name: "n", Commands: [{ Command: "click", Target: "css=button", Value: "" }] }, now })).toThrow(/carries no open command/);
    expect(() => importtabular({ text: "step,target,value\nobserve,,\n", now })).toThrow(/no `# goal:` preface line/);
    expect(() => importtabular({ text: "# goal: g\nstep,target,value\nobserve,,\n", now })).toThrow(/no `# origin:` preface line/);
    expect(() => importtabular({ text: "# goal: g\n# origin: http://example.org\nstep,target,value\nobserve,,\n", now })).toThrow(/HTTPS origin/);
    expect(() => importtabular({ text: "# goal: g\n# origin: https://example.org\nstep,target,extra\nobserve,,\n", now })).toThrow(/column extra outside the step, target and value columns/);
  });
});
...
describe("the csv parser of the tabular importer", () => {
  it("parses headers, quoted cells, escaped quotes, embedded newlines and crlf line endings while refusing an unterminated quote", () => {
    expect(parsecsvrows("step,target,value\nobserve,,\n")).toEqual([["step", "target", "value"], ["observe", "", ""]]);
    expect(parsecsvrows("a,b\n1,\"x,y\"\n")).toEqual([["a", "b"], ["1", "x,y"]]);
    expect(parsecsvrows("a,b\n1,\"say \"\"hi\"\"\"\n")).toEqual([["a", "b"], ["1", "say \"hi\""]]);
    expect(parsecsvrows("a,b,c\n1,\"two\nlines\",3")).toEqual([["a", "b", "c"], ["1", "two\nlines", "3"]]);
    expect(parsecsvrows("a,b\r\n1,2\r\n")).toEqual([["a", "b"], ["1", "2"]]);
    expect(parsecsvrows("a,b\n,\"\",\"x\"\n")).toEqual([["a", "b"], ["", "", "x"]]);
    expect(parsecsvrows("a,b\n,\"\"\"\",x\n")).toEqual([["a", "b"], ["", "\"", "x"]]);
...
describe("the migrateplan command surface", () => {
  ...
  it("keeps the conversion idempotent: converting the already converted output yields the same plan", async () => {
    ...
    const reimported = migrateplanconversion({ format: "tabular", text: await fixture("tabular-plan.csv"), now: now + 5_000 });
