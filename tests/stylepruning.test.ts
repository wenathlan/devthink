...
const surfacesources = ["views.ts", "web/extension/sidepanel.ts", "web/extension/dashboardpage.ts", "web/extension/popup.ts", "web/extension/optionspage.ts", "web/extension/transparencypage.ts"];
...
async function usedclassesof(text: string): Promise<{ used: Set<string>; composed: string[] }> {
  ...
  for (const match of text.matchAll(/class="([^"]*)"/g)) for (const token of (match[1] ?? "").split(/\s+/)) if (token) used.add(token);
  ...
  for (const match of text.matchAll(/className:\s*"([^"]*)"/g)) for (const token of (match[1] ?? "").split(/\s+/)) if (token) used.add(token);
  ...
  for (const match of text.matchAll(/setAttribute\("class",\s*"([^"]*)"\)/g)) for (const token of (match[1] ?? "").split(/\s+/)) if (token) used.add(token);
...
describe("the style pruning of the extension pages", () => {
  ...
  it("keeps the pruned selector families out of the stylesheet while the classes the surfaces render stay declared", async () => {
    ...
    for (const pruned of [".runrow", ".idemkey", ".memorymark", "span.wordbox", ".editorlibrary"]) expect(stylesheet.includes(pruned)).toBe(false);
    ...
    for (const source of surfacesources) {
      ...
      for (const match of text.matchAll(/classList\.(?:add|toggle|remove)\("([^"]+)"\)/g)) expect(styleclasses.has(match[1]!)).toBe(true);
    }
  });
});
