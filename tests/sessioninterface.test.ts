...
import {
  addhistoryentry, addrecallentry, cancelrunactionof, classifyfailure, consentadvisoryverdict, consentmemoryof, distillrunsummary,
  editedcorrectionof, errorsurfaceof, expirnotes, expirerecallindex, expirecorrections, historyqueryof, historysearch, highlightterms,
  matchingcorrections, notebodyof, notehistoryentry, prunescratchpad, rankrecall, recallentryof, rejectedcorrectionof, retryhintof,
  rollbackof, rollbacksplit, scratchentryof, scratchpadof, sessiongridrows, sessionbundleof, sitenoteof, summaryhistoryentry,
  tabsessionrefof, tabsessionkey, emptystatemessage, sealnotebody, opennotebody,
} from "../session.js";
import { cancelrungate, consentmemoryadvisorygate, memoryreadscopegate, retrydispatchgate, scratchpadscopegate, semanticrecallscopegate, sessionretentionvalid, sitenotesreadgate, sitenoteswritegate, summarywindowvalid } from "../policy.js";
...
const plan: agentplan = { id: "run1", objective: "Collect the pricing table", origin: "https://example.com", steps: [
...
describe("session interface", () => {
  it("builds site notes per origin with title, body, author provenance and timestamps", () => {
    const note = sitenoteof({ origin: "https://example.com", title: "Pricing flow", body: "The pricing table loads under #pricing.", author: "user", now });
  ...
  it("seals sensitive note bodies at rest and opens them for the reading surface", () => {
    const note = sitenoteof({ origin: "https://bank.example", title: "Login flow", body: "The login asks the card number first.", author: "user", sensitive: true, now });
  ...
  it("keeps the scratchpad append only per task with step provenance and prunes at the user window", () => {
    const entry = scratchentryof({ taskid: "run1", sessionid: "sess1", text: "The table needs a wait first.", stepid: "s1", author: "agent", now });
    expect(entry.stepid).toBe("s1");
    const usertext = scratchentryof({ taskid: "run1", sessionid: "sess1", text: "Remember the vat id.", author: "user", now: now - 10_000 });
    ...
    expect(() => scratchentryof({ taskid: "run1", sessionid: "sess1", text: " ", author: "agent", now })).toThrow(/text/);
  });
  ...
  it("distills a run summary from step outcomes inside the user configured window with no fixed cap", () => {
    ...
    const full = distillrunsummary({ plan, outcomes, origins: ["https://example.com"], sessionid: "sess1", provenance: "offscreenworker", now });
    ...
    const windowed = distillrunsummary({ plan, outcomes, origins: ["https://example.com"], sessionid: "sess1", window: 2, provenance: "inline", now });
  ...
  it("builds the runsummary request as one offscreen worker task", () => {
    ...
    const request = summaryrequestof({ id: "w1", runid: "run1", sessionid: "sess1", payload: JSON.stringify({ steps: 3 }), sentat: now });
    expect(request.task).toBe("runsummary");
    expect(request.transferables).toEqual([]);
    expect(() => summaryrequestof({ id: "w2", runid: "run1", sessionid: "sess1", payload: " ", sentat: now })).toThrow(/payload/);
  });

  it("indexes extraction records with fingerprint deduplication and ranks them by text similarity with provenance", () => {
    const first = recallentryof({ origin: "https://example.com", runid: "run1", stepid: "s1", text: "pricing table with monthly and yearly plans", at: now });
    const duplicate = recallentryof({ origin: "https://example.com", runid: "run2", stepid: "s9", text: "pricing  table with monthly and yearly plans", at: now + 1 });
    let index = addrecallentry([], first);
    index = addrecallentry(index, duplicate);
    expect(index).toHaveLength(1);
    const other = recallentryof({ origin: "https://shop.example", runid: "run1", stepid: "s2", text: "shipping options and delivery windows", at: now });
    index = addrecallentry(index, other);
    const ranked = rankrecall(index, { text: "pricing plans" }, { origins: ["https://example.com", "https://shop.example"] });
  ...
  it("captures corrections from plan review edits and rejections and feeds matching ones into proposals", () => {
    const edited = editedcorrectionof({ origin: "https://example.com", kind: "click", stepid: "s2", original: "click #next", corrected: "click #next-page", reason: "The target changed.", now: now - 10_000 });
    expect(edited.source).toBe("edited");
    expect(() => editedcorrectionof({ origin: "https://example.com", kind: "click", stepid: "s2", original: "same", corrected: "same", reason: "r", now })).toThrow(/changed/);
    const rejected = rejectedcorrectionof({ origin: "https://example.com", kind: "readtable", stepid: "s3", original: "readtable #t2", reason: "The user refused the second table.", now: now - 1000 });
  ...
  it("records consent memory decisions per origin with expiry and boundary fields and keeps them advisory", () => {
    const grant = consentmemoryof({ origin: "https://example.com", decision: "grant", boundary: "one hour the user chose", kinds: ["click"], expiresat: now + 3600_000, now });
    const denial = consentmemoryof({ origin: "https://example.com", decision: "deny", boundary: "the origin profile the user edits", kinds: ["fillcard"], now });
  ...
  it("derives session grid rows from the session stores with no new state", () => {
    const log: storedrunlog = { runid: "run0", sessionid: "sess0", entries: [
      { id: "e1", runid: "run0", kind: "step", summary: "The readtable step completed.", origin: "https://old.example", stepid: "s0", at: now - 1000, hash: { previous: "genesis", current: "h1", algorithm: "sha-256" } },
    ], seal: { runid: "run0", entries: 1, sealhash: { previous: "h1", current: "seal", algorithm: "sha-256" }, sealedat: now - 500 }, updatedat: now - 500 };
    const rows = sessiongridrows({ session: { id: "sess1", tabid: 7, origin: "https://example.com", pausedat: now - 10, grants: ["https://example.com"] }, plan, progress: { planid: "run1", completedsteps: ["s1"], updatedat: now }, logs: [log], summaries: [{ runid: "run0", sessionid: "sess0", origins: ["https://old.example"], kinds: ["readtable"], steps: [{ stepid: "s0", kind: "readtable", ok: true, summary: "done" }], task: "runsummary", provenance: "inline", distilledat: now - 400 }], locks: [{ runid: "run1" }], tabsessions: [{ tabid: 3, sessionid: "sess0", runid: "run0", origin: "https://old.example", updatedat: now - 1000 }] });
  ...
  it("searches sessions, notes and summaries from one box with filters and matched term highlighting", () => {
    let corpus = addhistoryentry([], { source: "session", id: "sess1", origin: "https://example.com", title: "Session of https://example.com", text: "session started for the pricing work", at: now - 3000 });
    corpus = addhistoryentry(corpus, notehistoryentry(sitenoteof({ origin: "https://example.com", title: "Pricing notes", body: "The yearly plan needs a confirmation dialog.", author: "user", now: now - 2000, id: "note1" })));
    corpus = addhistoryentry(corpus, summaryhistoryentry(distillrunsummary({ plan, outcomes: [{ stepid: "s1", ok: true, summary: "Read the pricing rows.", at: now }], origins: ["https://example.com"], sessionid: "sess1", provenance: "inline", now: now - 1000 })));
    ...
    expect(historysearch(corpus, { text: "pricing", outcome: "completed" }).map(hit => hit.source)).toEqual(["summary"]);
  ...
  it("classifies error causes as page, network, policy or gate and serves retry hints with the policy verdict", () => {
    expect(classifyfailure({ message: "The element never appeared.", policyrefused: false, gatewait: false })).toBe("page");
    expect(classifyfailure({ message: "The fetch failed with a network error.", policyrefused: false, gatewait: false })).toBe("network");
    expect(classifyfailure({ message: "The origin never granted the step.", policyrefused: true, gatewait: false })).toBe("policy");
    expect(classifyfailure({ message: "The payment waits for the confirm gate.", policyrefused: false, gatewait: true })).toBe("gate");
    const surface = errorsurfaceof({ stepid: "s3", runid: "run1", message: "The table never loaded.", cause: "page", retryallowed: true, retryreason: "A new reviewed dispatch may retry the read.", context: { origin: "https://example.com", kind: "readtable" }, now });
    expect(retryhintof(surface).reason).toMatch(/new reviewed dispatch/);
    expect(retryhintof({ ...surface, retry: { allowed: false, reason: "The step exhausted its retries." } }).allowed).toBe(false);
    expect(() => errorsurfaceof({ stepid: "s3", runid: "run1", message: " ", cause: "page", retryallowed: true, retryreason: "r", context: {}, now })).toThrow(/message/);
  });
  ...
  it("cancels a run with a rollback of the queued steps only while executed steps stay untouched", () => {
    ...
    const gate = cancelrungate({ queuedstepids: action.rollback.queuedstepids, executedstepids: split.executedstepids, rollbackscope: "queued" });
  ...
  it("exports notes, summaries and corrections as one audit bundle with sealed sensitive bodies", () => {
    const note = sitenoteof({ origin: "https://bank.example", title: "Login", body: "card number first", author: "user", sensitive: true, now });
...
describe("session interface gates", () => {
  it("gates sitenotes reads to granted origins and writes behind explicit consent", () => {
    ...
    expect(sitenotesreadgate({ origin: "https://other.example", grants: ["https://example.com"] }).reason).toMatch(/never granted/);
  ...
  it("scopes scratchpad access to the owning task session and memory reads to planning and prompting", () => {
    expect(scratchpadscopegate({ taskid: "run1", sessionid: "sess1", entrytaskid: "run1", entrysessionid: "sess1" }).allowed).toBe(true);
    expect(scratchpadscopegate({ taskid: "run2", sessionid: "sess1", entrytaskid: "run1", entrysessionid: "sess1" }).reason).toMatch(/never crosses/);
  ...
  it("refuses semantic recall across origins outside the run scope", () => {
    ...
    expect(semanticrecallscopegate({ origin: "https://example.com", scope: ["https://example.com"] }).allowed).toBe(true);
    expect(semanticrecallscopegate({ origin: "https://other.example", scope: ["https://example.com"] }).reason).toMatch(/outside the run scope/);
  });
  ...
  it("keeps consent memory advisory with denials carrying the same weight as grants", () => {
    expect(consentmemoryadvisorygate({ auto: true, latest: { decision: "grant", origin: "https://example.com" } }).allowed).toBe(false);
    expect(consentmemoryadvisorygate({ auto: false, latest: { decision: "deny", origin: "https://example.com" } }).reason).toMatch(/same weight/);
...
describe("session interface memory accessors", () => {
  it("stores site notes with read, write, list and expiry per origin", async () => {
    ...
    const note = sitenoteof({ origin: "https://example.com", title: "Pricing", body: "Table under #pricing.", author: "user", now });
    ...
    await store.writesitenote(sitenoteof({ origin: "https://old.example", title: "Old", body: "old", author: "user", now: now - 10_000 }));
  ...
  it("appends and reads the scratchpad per task and prunes at the user window", async () => {
    const store = new sessionmemory(new fakeadapter());
    await store.appendscratchentry(scratchentryof({ taskid: "run1", sessionid: "sess1", text: "first", author: "agent", now }));
    await store.appendscratchentry(scratchentryof({ taskid: "run1", sessionid: "sess1", text: "second", author: "agent", now: now + 1 }));
    await store.appendscratchentry(scratchentryof({ taskid: "run2", sessionid: "sess1", text: "other task", author: "agent", now }));
  ...
  it("stores run summaries per run and lists them per origin", async () => {
    const store = new sessionmemory(new fakeadapter());
    const summary = distillrunsummary({ plan, outcomes: [{ stepid: "s1", ok: true, summary: "Read 12 rows.", at: now }], origins: ["https://example.com"], sessionid: "sess1", provenance: "offscreenworker", now });
  ...
  it("answers semantic recall queries across the extraction store with deduplication and expiry", async () => {
    const store = new sessionmemory(new fakeadapter());
    await store.addrecallentry(recallentryof({ origin: "https://example.com", runid: "run1", stepid: "s1", text: "pricing table plans", at: now }));
    await store.addrecallentry(recallentryof({ origin: "https://example.com", runid: "run1", stepid: "s1", text: "pricing table plans", at: now + 1 }));
  ...
  it("records and queries correction memory and consent memory per origin", async () => {
    const store = new sessionmemory(new fakeadapter());
    await store.addcorrection(rejectedcorrectionof({ origin: "https://example.com", kind: "readtable", stepid: "s3", original: "readtable #t2", reason: "Refused.", now }));
    await store.addcorrection(editedcorrectionof({ origin: "https://shop.example", kind: "click", stepid: "s2", original: "click #a", corrected: "click #b", reason: "Target moved.", now }));
    ...
    await store.addconsentmemoryentry(consentmemoryof({ origin: "https://example.com", decision: "grant", boundary: "one hour", kinds: ["click"], expiresat: now + 3600_000, now }));
    await store.addconsentmemoryentry(consentmemoryof({ origin: "https://example.com", decision: "deny", boundary: "profile edit", kinds: ["fillcard"], now }));
  ...
  it("records error surfaces per failed step with their retry hints", async () => {
    const store = new sessionmemory(new fakeadapter());
    const surface = errorsurfaceof({ stepid: "s3", runid: "run1", message: "The table never loaded.", cause: "page", retryallowed: true, retryreason: "A new reviewed dispatch may retry.", context: { kind: "readtable" }, now });
  ...
  it("builds the history index incrementally and answers one search across the corpus", async () => {
    const store = new sessionmemory(new fakeadapter());
    await store.addhistoryentry({ source: "note", id: "n1", origin: "https://example.com", title: "Pricing notes", text: "yearly plan details", at: now });
    await store.addhistoryentry({ source: "note", id: "n1", origin: "https://example.com", title: "Pricing notes edited", text: "yearly plan details", at: now + 1 });
  ...
  it("isolates per tab session references and lists them for the session grid", async () => {
    const store = new sessionmemory(new fakeadapter());
    await store.settabsession(tabsessionrefof({ tabid: 1, sessionid: "sessA", runid: "runA", origin: "https://a.example", now }));
  ...
  it("exports notes, summaries and corrections as one audit bundle", async () => {
    const store = new sessionmemory(new fakeadapter());
    await store.writesitenote(sitenoteof({ origin: "https://example.com", title: "Pricing", body: "Table under #pricing.", author: "user", now }));
    await store.addcorrection(rejectedcorrectionof({ origin: "https://example.com", kind: "click", stepid: "s1", original: "click #a", reason: "Refused.", now }));
...
function editnoteof(note: ReturnType<typeof sitenoteof>, title: string, body: string, author: string, at: number): ReturnType<typeof sitenoteof> {
  return note.sensitive ? { ...note, title, sealedbody: sealnotebody(note.id, body), updatedat: at, author } : { ...note, title, body, updatedat: at, author };
}
