...
describe("buildauthheaders — every auth mode", () => {
  ...
  it("apikeyheader honors custom header names", () => {
    expect(buildauthheaders({ mode: "apikeyheader", headername: "x-goog-api-key" }, "k")["x-goog-api-key"]).toBe("k");
    expect(buildauthheaders({ mode: "apikeyheader", headername: "api-key" }, "k")["api-key"]).toBe("k");
  });
  ...
  it("extraheaders ride along every mode", () => {
    const h = buildauthheaders({ mode: "bearer", extraheaders: { "http-referer": "https://x", "x-title": "t" } }, "k");
...
describe("buildauthqueryparams — query auth", () => {
  ...
  it("extraparams ride along", () => {
    const p = buildauthqueryparams({ mode: "queryparam", extraparams: { "api-version": "2024-02-01" } }, "k");
