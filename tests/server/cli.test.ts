...
const clits = join(import.meta.dirname, "..", "..", "server.ts");
...
async function runcli(
  ...
  try {
    const { stdout, stderr } = await exec(tsxbin, [clits, ...args], {
      cwd: workdir,
      env: { ...process.env, DEVTHINK_DATABASE_URL: `file:${join(workdir, "cli.db")}` },
...
describe("cli help", () => {
  it("prints the usage banner with every command", async () => {
    ...
    for (const cmd of ["init", "add", "list", "show", "validate", "keys", "models", "serve", "export"]) {
...
describe("cli init", () => {
  ...
  it("the prisma config resolves the schema and datasource url", () => {
    ...
    expect(prismaconfig).toContain("DEVTHINK_DATABASE_URL");
  });
