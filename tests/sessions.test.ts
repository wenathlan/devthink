...
function record(id: string, overrides: Partial<sessionrecord> = {}): sessionrecord {
  return newsessionrecord({
    ...
    name: `session ${id}`,
    createdat: now,
    tabs: [tab(0, "https://example.com/a"), tab(1, "https://example.com/b")],
    captures: ["cap1"],
    storage: [{ origin: "https://example.com", keys: ["theme"], values: ["dark"] }],
    cookies: [{ origin: "https://example.com", names: ["sid"] }],
    ...overrides,
  });
}
...
function step(kind: toolstep["kind"], options?: Record<string, unknown>, risk: toolstep["risk"] = "read"): toolstep {
  return {
    id: "s1",
    kind,
    summary: "Reviewed session memory step",
    risk,
...
describe("task state persistence", () => {
  it("checkpoints the run with a checksum that detects corruption before any resume", () => {
    const state = taskstateof({
      runid: "run",
      stepcursor: 2,
      outputs: [{ stepid: "s1", ok: true, summary: "done", at: now }],
      checkpointat: now,
    });
    ...
    expect(
      taskstatevalid({
        ...state,
        outputs: [...state.outputs, { stepid: "s2", ok: true, summary: "injected", at: now }],
      }),
    ).toBe(false);
  ...
  it("marks runs interrupted by a browser restart and leaves complete runs untouched", () => {
    const interrupted = crashinterrupted(
      taskstateof({ runid: "run", stepcursor: 1, outputs: [], checkpointat: now }),
      4,
      now + 5000,
    );
...
describe("session grammars", () => {
  it("normalizes the snapshot plan, the restore plan, the auto interval, the search query and the folder", () => {
    const snapshot = snapshotplanof({
      scope: "run",
      sections: ["tabs", "scroll", "forms", "storage", "cookies"],
      captures: true,
      auto: { period: 60_000, maxsnapshots: 5, expiry: 3_600_000 },
    });
    ...
    expect(
      snapshotplanof({
        ...
        sections: ["tabs"],
        captures: true,
        auto: { period: 0, maxsnapshots: 1, expiry: 0 },
      }),
    ).toBeUndefined();
    expect(restoreplanof({ tabpolicy: "reopen", formpolicy: "restore", capturepolicy: "link" })).toEqual({
      tabpolicy: "reopen",
      formpolicy: "restore",
      capturepolicy: "link",
    });
  ...
  it("normalizes captured tabs and drops broken form entries", () => {
    expect(
      ...
      scrollx: 5,
      scrolly: 9,
      forms: [{ selector: "#q", value: "devthink" }],
    });
...
describe("session diff and search", () => {
  it("classifies tab, url, form and storage changes between two saved sessions", () => {
    const left = record("left", {
      tabs: [tab(0, "https://example.com/a", [{ selector: "#q", value: "old" }]), tab(1, "https://example.com/b")],
      storage: [{ origin: "https://example.com", keys: ["theme"], values: ["dark"] }],
    });
    const right = record("right", {
      tabs: [
        tab(0, "https://example.com/a2", [
          { selector: "#q", value: "new" },
          { selector: "#email", value: "a@b.c" },
        ]),
        tab(2, "https://example.com/c"),
      ],
      storage: [{ origin: "https://example.com", keys: ["theme"], values: ["light"] }],
    });
  ...
  it("matches urls, titles, names and captured text inside the reviewed time window", () => {
    const login = record("login", {
      name: "login session",
      createdat: now - 500,
      tabs: [tab(0, "https://example.com/signin", [{ selector: "#email", value: "user@example.com" }])],
    });
    const shop = record("shop", {
      name: "shopping session",
      createdat: now - 10_000,
      tabs: [tab(0, "https://example.com/cart")],
    });
...
describe("session consent gates", () => {
  it("requires review of every restore plan behind the session consent gate", () => {
    const restorestep = step(
      ...
      {
        sessionid: "one",
        restore: { tabpolicy: "reopen", formpolicy: "restore", capturepolicy: "link" },
        reviewed: true,
    ...
    expect(
      sessionrestoregate({ session, plan, step: restorestep, tabid: 4, origin: "https://example.com", now }).allowed,
    ).toBe(true);
    expect(
      sessionrestoregate({ session: undefined, plan, step: restorestep, tabid: 4, origin: "https://example.com", now })
        .allowed,
    ).toBe(false);
    const paused: agentsession = { ...session, pausedat: now };
    expect(
      sessionrestoregate({ session: paused, plan, step: restorestep, tabid: 4, origin: "https://example.com", now })
        .allowed,
    ).toBe(false);
    expect(
      sessionrestoregate({
    ...
    expect(
      sessionrestoregate({ session, plan, step: restorestep, tabid: 9, origin: "https://example.com", now }).allowed,
    ).toBe(false);
    expect(
      restorereviewgranted(
        step(
          "restoresession",
          { sessionid: "one", restore: { tabpolicy: "reopen", formpolicy: "restore", capturepolicy: "link" } },
          "sensitive",
        ),
      ).allowed,
    ).toBe(false);
  ...
  it("skips and reports the origins a restore reopens outside the grants", () => {
    const outcome = restoreoriginsgranted(
      ["https://example.com/a", "https://elsewhere.example/b", "https://other.example/c"],
      ["https://example.com"],
    );
  ...
  it("validates the options payload of every session kind", () => {
    ...
    expect(
      validatestep(
        step("capturesession", { snapshot: { scope: "run", sections: ["tabs"], captures: true } }),
        "https://example.com",
      ).allowed,
    ).toBe(true);
      ...
      validatestep(
        step("capturesession", {
          snapshot: {
            ...
            sections: ["tabs"],
            captures: true,
            auto: { period: -1, maxsnapshots: 2, expiry: 0 },
          },
        }),
        "https://example.com",
      ).allowed,
    ...
    expect(
      validateStepRefusal("restoresession", {
        sessionid: "one",
        restore: { tabpolicy: "reopen", formpolicy: "restore", capturepolicy: "link" },
      }),
    ).toContain("restore review");
    ...
    expect(
      validatestep(step("importsessions", { reviewed: true, file }, "sensitive"), "https://example.com").allowed,
    ).toBe(true);
  });
  ...
  it("refuses restores that reopen ungranted origins and import files of unknown versions in the proposal parser", () => {
            ...
            steps: [
              {
                id: "s1",
                kind: "restoresession",
                summary: "Restore the saved session",
      ...
      parseproposal(
        {
          version: protocolversion,
          plan: {
            objective: "Import a session file",
            steps: [
              {
                id: "s1",
                kind: "importsessions",
                summary: "Import the reviewed session file",
                options: JSON.stringify({ reviewed: true, file: { ...file, formatversion: 99 } }),
              },
            ],
          },
        },
    ...
    ).toThrow("unknown format versions");
...
describe("session envelopes and evidence", () => {
  it("carries the session block with the record id and section counts in the outcome envelope", () => {
    const envelope = JSON.parse(
      outcomeresponse({
        outcome: { stepid: "s1", ok: true, summary: "Captured 2 tabs.", at: now },
        plan,
        session: { recordid: "one", sections: 5, matches: 3, restored: 2, skipped: 1, cursor: 4, bytes: 900 },
      }),
    );
  ...
  it("builds the session report envelope with records, events, folders, diffs and the auto interval", () => {
    const report = sessionreport({
      records: [record("one")],
      events: [{ id: "e1", kind: "capture", at: now, detail: "Captured 2 tabs." }],
      folders: [{ name: "work", tags: [] }],
      diffs: [newsessiondiff({ id: "d1", left: record("one"), right: record("two"), at: now })],
      auto: { period: 60_000, maxsnapshots: 5, expiry: 3_600_000 },
      crashed: true,
    });
  ...
  it("records session evidence in the plan progress outcome log", () => {
    let progress = recordsession(
      undefined,
      "run",
      "s1",
      { family: "capture", detail: "Captured the session record one", recordid: "one", sections: 5 },
      now,
    );
    progress = recordsession(
      ...
      "run",
      "s2",
      { family: "restore", detail: "Restored the session record one", restored: 2, skipped: 1 },
      now + 1,
    );
...
describe("session memory store", () => {
  ...
  it("stores task state checkpoints per run, session events, records, folders, diffs, the auto interval and the crash marker", async () => {
    ...
    await store.setautosnapshot({
      interval: { period: 60_000, maxsnapshots: 5, expiry: 3_600_000 },
      lastat: now,
      count: 1,
    });
