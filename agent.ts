...
import type { agentbudget, agentrecord, agentscope, agentrole, budgetstate, runrecord, runstate, taskqueue, toolnamespace, agentidentity, agentmailbox, agentmessage, messagerouting, comparisonrecord, consensusrecord, consensusvote, escalationrecord, replayrecord, reviewrecord, reviewverdict, callcontext, cancelframe, capabilityset, eventkind, jsonrpcframe, progressnotice, promptdef, protocoleventsubscription, resourcewatch, samplingrequest, streamchunk, aggregatecell, aggregaterecord, arbitrationcase, arbitrationverdict, costentry, depthlimit, interleaveevent, lessonrecord, loadreport, spawnrecord, subagentspec, tasklane, agentevent, agenteventkind, agentusage, killswitch, spawnrequest, swarmstate, taskitem, workflowrun, actionrisk, blackboard, blackboardsection, conflictscan, conflictwriter, handoffrecord, mergeentry, mergerule, outputcomparison, resourcelock, resultreport, swarmaction, swarmcost } from "./types.js";
...
export function agentname(input: { records: agentrecord[]; id: string; proposed: string; role?: agentrole; origin: string; now: number }): agentrecord {
...
export function agentscopeof(input: { agentid: string; requested?: { origins?: string[]; toolnamespaces?: toolnamespace[]; actionkinds?: string[] }; grants?: string[]; allowedkinds?: string[] }): agentscope {
  ...
  const origins = input.grants === undefined ? requestedorigins : requestedorigins.filter(origin => input.grants!.includes(origin));
  if (input.grants !== undefined && requestedorigins.length > 0 && origins.length === 0) throw new Error(`The requested origins ${requestedorigins.join(", ")} sit outside the session grants ${input.grants.join(", ")}; the scope never widens past the grants.`);
  const requestedkinds = (requested.actionkinds ?? []).map(kind => kind.trim()).filter(kind => kind !== "");
  const actionkinds = input.allowedkinds === undefined ? requestedkinds : requestedkinds.filter(kind => input.allowedkinds!.includes(kind));
  if (input.allowedkinds !== undefined && requestedkinds.length > 0 && actionkinds.length === 0) throw new Error(`The requested action kinds ${requestedkinds.join(", ")} sit outside the allowed catalog; the scope narrows every step to the allowed kinds only.`);
...
export function agentbudgetof(input: { agentid: string; maxsteps?: number; maxtokens?: number; maxdurationms?: number; currency?: string; now: number }): { budget: agentbudget; state: budgetstate } {
  if (input.agentid.trim() === "") throw new Error("The budget needs its agent id.");
  const ceilings: Array<[string, number | undefined]> = [["maxsteps", input.maxsteps], ["maxtokens", input.maxtokens], ["maxdurationms", input.maxdurationms]];
  for (const [label, value] of ceilings) {
    if (value !== undefined && (!Number.isFinite(value) || value <= 0)) throw new Error(`The ${label} ceiling must stay a positive number; every ceiling stays the user's choice with no engine default.`);
  }
  const agentid = input.agentid.trim();
  return {
    budget: { agentid, ...(input.maxtokens !== undefined ? { maxtokens: input.maxtokens } : {}), ...(input.maxsteps !== undefined ? { maxsteps: input.maxsteps } : {}), ...(input.maxdurationms !== undefined ? { maxdurationms: input.maxdurationms } : {}), ...(input.currency !== undefined && input.currency.trim() !== "" ? { currency: input.currency.trim() } : {}), configuredat: input.now },
    state: { agentid, spentsteps: 0, spenttokens: 0, spentdurationms: 0, ...(input.maxsteps !== undefined ? { maxsteps: input.maxsteps } : {}), ...(input.maxtokens !== undefined ? { maxtokens: input.maxtokens } : {}), ...(input.maxdurationms !== undefined ? { maxdurationms: input.maxdurationms } : {}), updatedat: input.now }
  };
}
...
export function spendbudget(input: { state: budgetstate; steps?: number; tokens?: number; durationms?: number; now: number }): budgetstate {
  const steps = input.steps ?? 1;
  const tokens = input.tokens ?? 0;
  const durationms = input.durationms ?? 0;
  for (const [label, value] of [["steps", steps], ["tokens", tokens], ["durationms", durationms]] as Array<[string, number]>) {
  ...
  if (input.state.maxsteps !== undefined && nextsteps > input.state.maxsteps) throw new Error(`The agent ${input.state.agentid} already executed ${input.state.spentsteps} of its ${input.state.maxsteps} budgeted steps; the user raises the ceiling or stops the agent.`);
  if (input.state.maxtokens !== undefined && nexttokens > input.state.maxtokens) throw new Error(`The agent ${input.state.agentid} already spent ${input.state.spenttokens} of its ${input.state.maxtokens} budgeted tokens; the user raises the ceiling or stops the agent.`);
  if (input.state.maxdurationms !== undefined && nextduration > input.state.maxdurationms) throw new Error(`The agent ${input.state.agentid} already spent ${input.state.spentdurationms} of its ${input.state.maxdurationms} budgeted milliseconds; the user raises the ceiling or stops the agent.`);
...
export function budgetremaining(state: budgetstate): { steps?: number; tokens?: number; durationms?: number; reason: string } {
  ...
  const durationms = state.maxdurationms === undefined ? undefined : Math.max(0, state.maxdurationms - state.spentdurationms);
  const parts: string[] = [];
  ...
  parts.push(durationms === undefined ? "duration unbounded" : `${durationms} of ${state.maxdurationms} milliseconds left`);
  return { ...(steps !== undefined ? { steps } : {}), ...(tokens !== undefined ? { tokens } : {}), ...(durationms !== undefined ? { durationms } : {}), reason: `The agent ${state.agentid} spent ${state.spentsteps} steps, ${state.spenttokens} tokens and ${state.spentdurationms} milliseconds: ${parts.join(", ")}.` };
}
...
export function pauseagent(input: { records: agentrecord[]; agentid: string; now: number }): agentrecord[] {
  ...
  return input.records.map(record => record.id === input.agentid ? { ...record, state: "paused" as const, lastseenat: input.now } : record);
}
...
export function unpauseagent(input: { records: agentrecord[]; agentid: string; now: number }): agentrecord[] {
  ...
  return input.records.map(record => record.id === input.agentid ? { ...record, state: "active" as const, lastseenat: input.now } : record);
}
...
export function engagekillswitch(input: { records: agentrecord[]; runs: runrecord[]; queues?: taskqueue[]; reason?: string; now: number }): { records: agentrecord[]; runs: runrecord[]; queues: taskqueue[]; stopped: Array<{ agentid: string; name: string; runs: string[]; queued: number }>; engagedat: number; reason: string } {
  const reason = input.reason !== undefined && input.reason.trim() !== "" ? input.reason.trim() : "The user engaged the killswitch.";
  const terminal: runstate[] = ["completed", "failed", "cancelled", "rolledback", "done"];
  const runs = input.runs.map(run => run.agentid !== undefined && !terminal.includes(run.state) ? { ...run, state: "cancelled" as runstate, updatedat: input.now } : run);
  const cancelled = runs.filter(run => run.agentid !== undefined && run.state === "cancelled" && input.runs.find(original => original.runid === run.runid)?.state !== "cancelled");
  ...
  return { records: input.records.map(record => ({ ...record, state: "stopped" as const, lastseenat: input.now })), runs, queues, stopped, engagedat: input.now, reason };
}
...
export function fleetoverview(input: { records: agentrecord[]; budgets: budgetstate[]; openescalations: number; openreviews: number; killswitchat?: number }): { agents: number; active: number; paused: number; stopped: number; openescalations: number; openreviews: number; lastkillswitchat?: number } {
  return {
...
export function resolverecipients(input: { agents: agentidentity[]; senderid: string; recipient: string; routing: messagerouting }): string[] {
...
export function sendmessage(input: { mailboxes: agentmailbox[]; agents: agentidentity[]; id: string; senderid: string; recipient: string; routing: messagerouting; payload: string; now: number }): agentmailbox[] {
  ...
  const message: agentmessage = { id: input.id, senderid: input.senderid, recipient: input.recipient, routing: input.routing, payload: input.payload, sentat: input.now };
  ...
  const known = new Set([...input.mailboxes.map(mailbox => mailbox.agentid), ...recipients, ...(input.agents.some(agent => agent.id === input.senderid) ? [input.senderid] : [])]);
...
export function receivemessages(input: { mailboxes: agentmailbox[]; agentid: string; now: number }): { mailboxes: agentmailbox[]; messages: agentmessage[] } {
  const mailbox = mailboxof(input.mailboxes, input.agentid);
  const messages = [...mailbox.inbox].sort((one, two) => one.sentat - two.sentat).map(message => ({ ...message, ...(message.readat === undefined ? { readat: input.now } : {}) }));
...
export function escalationblock(input: { escalations: escalationrecord[]; agentid: string }): { blocked: boolean; open: number; reason: string } {
...
export function reviewrecordof(input: { id: string; fromagentid: string; toagentid: string; subject: string; output: string; now: number }): reviewrecord {
...
export function recordverdict(input: { records: reviewrecord[]; id: string; reviewerid: string; verdict: reviewverdict; issues?: string[]; now: number }): reviewrecord {
...
export function runreplay(input: { id: string; agentid: string; runid: string; steps: Array<{ stepid: string; kind: string; summary: string; state: string; at: number }>; now: number }): replayrecord {
...
export function reconstructreplay(input: { id: string; agentid: string; runid: string; events: Array<{ id: string; kind: string; summary: string; at: number; agentid?: string; stepid?: string; planid?: string }>; now: number }): replayrecord {
  const owned = input.events.filter(event => event.agentid === input.agentid && (event.planid === undefined || event.planid === input.runid));
  ...
  const steps = owned
    ...
    .map(event => ({ stepid: event.stepid as string, kind: event.kind, summary: event.summary, state: event.kind === "action" || event.kind === "complete" ? "done" : event.kind === "error" ? "failed" : "recorded", at: event.at }))
...
export function outputcompare(input: { id: string; subject: string; left: { agentid: string; fields: Record<string, string> }; right: { agentid: string; fields: Record<string, string> }; now: number }): comparisonrecord {
...
export function consensusrecordof(input: { id: string; proposal: string; votes: Array<{ agentid: string; vote: consensusvote; reason?: string; castat: number }>; quorum: number; voters?: number; now: number }): consensusrecord {
  ...
  for (const vote of input.votes) {
    ...
    votes.push({ agentid: vote.agentid.trim(), vote: vote.vote, ...(vote.reason !== undefined && vote.reason.trim() !== "" ? { reason: vote.reason.trim() } : {}), castat: vote.castat });
  }
...
export const protocoleventkinds: eventkind[] = ["callstarted", "callresult", "streamchunk", "progress", "resourcedelta", "sampling", "cancellation"];

/** The observation event kinds that stay read only: every kind except the callstarted mutation mirror. */
export const readonlyeventkinds: eventkind[] = ["callresult", "streamchunk", "progress", "resourcedelta", "sampling", "cancellation"];
...
export function subscriberegister(input: { clientid: string; kinds?: string[]; origin?: string; tool?: string; now: number; id?: string }): { subscription?: protocoleventsubscription; reason?: string } {
  if (input.clientid.trim() === "") return { reason: "The event subscription needs the paired client it belongs to." };
  const kinds = input.kinds === undefined || input.kinds.length === 0 ? [...readonlyeventkinds] : [...new Set(input.kinds)];
  for (const kind of kinds) {
    if (!protocoleventkinds.includes(kind as eventkind)) return { reason: `The event kind ${kind} is not a protocol event kind.` };
  }
  if (input.origin !== undefined && input.origin.trim() === "") return { reason: "The origin filter of an event subscription must name an origin or stay absent." };
  if (input.tool !== undefined && input.tool.trim() === "") return { reason: "The tool filter of an event subscription must name a tool or stay absent." };
  return { subscription: { id: input.id ?? randomid(), clientid: input.clientid, kinds: kinds as eventkind[], ...(input.origin !== undefined ? { origin: input.origin } : {}), ...(input.tool !== undefined ? { tool: input.tool } : {}), createdat: input.now } };
}

/** Cancels one client event subscription; the record stays for the audit trail with its cancel time. */
export function unsubscriberegister(subscriptions: protocoleventsubscription[], id: string, now: number): protocoleventsubscription[] {
  return subscriptions.map(subscription => subscription.id === id && subscription.canceledat === undefined ? { ...subscription, canceledat: now } : subscription);
}
...
export function notifyevent(input: { subscriptions: protocoleventsubscription[]; kind: eventkind; origin?: string; tool?: string; payload?: Record<string, unknown>; now: number }): { deliveries: Array<{ subscriptionid: string; clientid: string; frame: jsonrpcframe }>; subscriptions: protocoleventsubscription[] } {
  ...
  const subscriptions = input.subscriptions.map(subscription => {
    ...
    if (subscription.origin !== undefined && input.origin !== undefined && subscription.origin !== input.origin) return subscription;
    if (subscription.tool !== undefined && input.tool !== undefined && subscription.tool !== input.tool) return subscription;
    deliveries.push({ subscriptionid: subscription.id, clientid: subscription.clientid, frame: { jsonrpc: "2.0", method: "events/notify", params: { subscriptionid: subscription.id, kind: input.kind, ...(input.origin !== undefined ? { origin: input.origin } : {}), ...(input.tool !== undefined ? { tool: input.tool } : {}), ...(input.payload !== undefined ? { payload: input.payload } : {}), at: input.now } } });
...
export function watchresource(input: { clientid: string; resource: string; state?: Record<string, unknown>; now: number; id?: string }): { watch?: resourcewatch; reason?: string } {
...
export function unwatchresource(watches: resourcewatch[], id: string, now: number): resourcewatch[] {
  return watches.map(watch => watch.id === id && watch.canceledat === undefined ? { ...watch, canceledat: now } : watch);
}
...
export function notifyresource(input: { watches: resourcewatch[]; resource: string; state: Record<string, unknown>; now: number }): { deliveries: Array<{ watchid: string; clientid: string; delta: Record<string, unknown> }>; watches: resourcewatch[] } {
...
export function requestsampling(input: { clientid: string; capabilities?: capabilityset; prompt: string; system?: string; pagecontent?: string; pagegrant?: boolean; maxtokens?: number; now: number; id?: string }): { request?: samplingrequest; reason?: string } {
  ...
  if (input.maxtokens !== undefined && (!Number.isFinite(input.maxtokens) || input.maxtokens <= 0)) return { reason: "The granted maximum tokens of a sampling callback must stay a positive user value." };
  const granted = input.pagegrant === true;
  const pagecontent = granted ? input.pagecontent : undefined;
  const prompt = granted || input.pagecontent === undefined ? input.prompt : `${input.prompt}\nThe page content stays stripped because the user granted none.`;
  return { request: { id: input.id ?? randomid(), clientid: input.clientid, prompt, ...(input.system !== undefined ? { system: input.system } : {}), ...(pagecontent !== undefined ? { pagecontent } : {}), ...(input.maxtokens !== undefined ? { maxtokens: input.maxtokens } : {}), state: "pending", requestedat: input.now } };
}
...
export function answersampling(input: { requests: samplingrequest[]; id: string; answer?: string; refused?: boolean; now: number }): { requests: samplingrequest[]; request?: samplingrequest; reason?: string } {
  ...
  const request: samplingrequest = { ...match, state: input.refused === true ? "refused" : "answered", answeredat: input.now, ...(input.refused !== true && input.answer !== undefined ? { answer: input.answer } : {}) };
...
export function callprompt(input: { name: string; args?: Record<string, unknown>; now?: number }): { toolcall?: { name: string; params: Record<string, unknown> }; rendered?: string; reason?: string } {
...
export function streamchunkof(input: { callid: string; seq: number; content: string; done?: boolean; now: number }): streamchunk {
...
export function notifyprogress(input: { callid: string; percent?: number; message: string; cancellable?: boolean; now: number }): progressnotice {
  return { callid: input.callid, ...(input.percent !== undefined ? { percent: input.percent } : {}), message: input.message, cancellable: input.cancellable !== false, at: input.now };
}
...
export function canceltool(input: { contexts: callcontext[]; callid: string; reason?: string; partial?: callcontext["partial"]; now: number }): { contexts: callcontext[]; context?: callcontext; reason?: string } {
  const match = input.contexts.find(context => context.callid === input.callid);
  if (match === undefined) return { contexts: input.contexts, reason: `The cancellation frame names no call context ${input.callid}.` };
  if (match.state !== "inflight") return { contexts: input.contexts, reason: `The call ${input.callid} already left the in flight state.` };
  const context: callcontext = { ...match, state: "cancelled", endedat: input.now, ...(input.partial !== undefined ? { partial: input.partial } : {}) };
...
export function spawnsubagent(input: { records: agentrecord[]; spawns: spawnrecord[]; spec: subagentspec; parentscope?: agentscope; id: string; name?: string; now: number }): { record: agentrecord; scope: agentscope; spawn: spawnrecord } {
  ...
  const record = agentname({ records: input.records, id: input.id, proposed: input.name !== undefined && input.name.trim() !== "" ? input.name : `${parent.name}sub${input.spec.depth}`, ...(input.spec.role !== undefined ? { role: input.spec.role } : { role: "worker" as agentrole }), origin: parent.origin, now: input.now });
  const scope = childscopeof({ agentid: record.id, ...(input.parentscope !== undefined ? { parent: input.parentscope } : {}), ...(input.spec.narrowscope !== undefined ? { narrowed: input.spec.narrowscope } : {}) });
  const spawn: spawnrecord = { id: `${record.id}:spawn`, parentid: parent.id, childid: record.id, role: record.role, depth: input.spec.depth, objective: input.spec.objective.trim(), at: input.now };
...
export function childscopeof(input: { agentid: string; parent?: agentscope; narrowed?: { origins?: string[]; actionkinds?: string[] } }): agentscope {
...
export function depthoflineage(input: { spawns: spawnrecord[]; agentid: string }): number {
  ...
  for (;;) {
    ...
    if (seen.has(spawn.parentid)) throw new Error(`The spawn lineage of the agent ${input.agentid} carries a cycle at ${spawn.parentid}; the depth never loops.`);
...
export function depthlimitof(input: { spawns: spawnrecord[]; agentid: string; limit: depthlimit }): { depth: number; allowed: boolean; reason: string } {
  const depth = depthoflineage({ spawns: input.spawns, agentid: input.agentid });
  if (input.limit.maxdepth === undefined) return { depth, allowed: true, reason: `The lineage depth of the agent ${input.agentid} sits at ${depth}; the user configured no depth ceiling so the spawn recursion stays unbounded.` };
...
export function aggregatereport(input: { id: string; subject: string; cells: aggregatecell[]; conflictorder?: string[]; expected?: string[]; now: number }): aggregaterecord {
  ...
  for (const [key, writers] of [...sections.entries()].sort((one, two) => (one[0] < two[0] ? -1 : 1))) {
    ...
    const ordered = input.conflictorder !== undefined ? writers.slice().sort((one, two) => {
  ...
  const complete = input.expected === undefined || input.expected.every(agentid => cells.some(cell => cell.agentid === agentid));
  const state: aggregaterecord["state"] = unresolved.length > 0 || !complete ? "open" : "merged";
  return { id: input.id.trim(), subject: input.subject.trim(), cells, conflicts, ...(input.expected !== undefined ? { expected: input.expected } : {}), state, ...(state === "merged" ? { mergedat: input.now } : {}), createdat: input.now };
}
...
export function interleave(input: { events: Array<{ id?: string; agentid: string; kind: string; summary: string; at: number; lane?: string }>; lanes: tasklane[] }): interleaveevent[] {
  const lanemap = new Map<string, string>();
  for (const lane of input.lanes) for (const agentid of lane.agentids ?? []) lanemap.set(agentid, lane.name);
  const events: interleaveevent[] = input.events.map((event, index) => ({ id: event.id !== undefined && event.id.trim() !== "" ? event.id.trim() : `ie${index}`, agentid: event.agentid, kind: event.kind, summary: event.summary, lane: event.lane !== undefined && event.lane.trim() !== "" ? event.lane.trim() : lanemap.get(event.agentid) ?? "background", at: event.at }));
...
export function lessonrecordof(input: { id: string; agentid: string; finding: string; origin: string; now: number }): lessonrecord {
...
export function lessonmatches(input: { lessons: lessonrecord[]; task: string; origin?: string; limit?: number }): lessonrecord[] {
  ...
  const limit = input.limit !== undefined && Number.isFinite(input.limit) && input.limit > 0 ? Math.floor(input.limit) : scored.length;
...
export function lessondecay(input: { lessons: lessonrecord[]; now: number; stalewindow?: number }): lessonrecord[] {
  if (input.stalewindow === undefined || !Number.isFinite(input.stalewindow) || input.stalewindow <= 0) return input.lessons;
...
export function arbitrationcaseof(input: { id: string; resource: string; origin: string; requesters: string[]; lanes: tasklane[]; now: number }): arbitrationcase {
...
export function releasecase(input: { cases: arbitrationcase[]; id: string; holderfinished: boolean; now: number }): arbitrationcase {
...
export function prioritylaneof(input: { tasks: Array<{ id: string; lane: string; priority?: number; sensitive?: boolean; enqueuedat: number }>; lanes: tasklane[] }): Array<{ id: string; lane: string; priority?: number; sensitive?: boolean; enqueuedat: number }> {
...
export function scalesuggestion(input: { reports: loadreport[]; lowthreshold?: number; throttlethreshold?: number }): Array<{ origin: string; suggestion: "spawn" | "pause" | "hold"; reason: string }> {
  return input.reports.map(report => {
    if (input.throttlethreshold !== undefined && Number.isFinite(input.throttlethreshold) && input.throttlethreshold > 0 && report.latency >= input.throttlethreshold) return { origin: report.origin, suggestion: "pause" as const, reason: `The origin ${report.origin} throttles at ${report.latency} milliseconds of latency past the threshold ${input.throttlethreshold}; pausing one worker eases the site, and the pause needs the user.` };
    if (input.lowthreshold !== undefined && Number.isFinite(input.lowthreshold) && input.lowthreshold > 0 && report.concurrency <= input.lowthreshold && report.latency < input.lowthreshold) return { origin: report.origin, suggestion: "spawn" as const, reason: `The origin ${report.origin} carries its load of ${report.concurrency} worker${report.concurrency === 1 ? "" : "s"} with ${report.latency} milliseconds of latency; spawning one more worker shares the work, and the spawn needs the user consent.` };
    return { origin: report.origin, suggestion: "hold" as const, reason: `The origin ${report.origin} carries ${report.concurrency} worker${report.concurrency === 1 ? "" : "s"} at ${report.latency} milliseconds of latency; the fleet holds its size.` };
  });
}
...
export function costentryof(input: { agentid: string; runid?: string; units: number; description: string; now: number }): costentry {
  ...
  return { agentid: input.agentid.trim(), ...(input.runid !== undefined && input.runid.trim() !== "" ? { runid: input.runid.trim() } : {}), units: input.units, description: input.description.trim(), at: input.now };
}
...
export function sharedcostsplit(input: { entries: costentry[] }): Array<{ agentid: string; units: number; share: number; sharedwith?: string[] }> {
  ...
  return agentids.map(agentid => {
    ...
    return { agentid, units: soloamount + (sharedamount?.units ?? 0), share: soloamount, ...(sharedamount !== undefined ? { sharedwith: [...sharedamount.peers].sort() } : {}) };
  });
}
...
export function registeragent(input: { agents: agentidentity[]; id: string; name: string; tabid?: number; sessionid?: string; role?: agentrole; now: number }): agentidentity[] {
  if (input.name.trim() === "") throw new Error("The agent needs the user chosen name; agent naming stays a user choice.");
  if (input.tabid !== undefined && input.agents.some(agent => agent.tabid === input.tabid && agent.state !== "stopped")) throw new Error(`Tab ${input.tabid} already holds the agent ${input.agents.find(agent => agent.tabid === input.tabid)?.name ?? "another agent"}; one tab binds one agent.`);
  if (input.id !== "" && input.agents.some(agent => agent.id === input.id)) throw new Error(`The agent id ${input.id} is already registered.`);
  const agent: agentidentity = { id: input.id, name: input.name.trim(), role: input.role ?? "worker", depth: 0, state: "active", ...(input.tabid !== undefined ? { tabid: input.tabid } : {}), ...(input.sessionid !== undefined ? { sessionid: input.sessionid } : {}), registeredat: input.now, heartbeatat: input.now };
...
export function assignrole(input: { agents: agentidentity[]; agentid: string; role: agentrole; now: number }): agentidentity[] {
...
export function opentabagent(input: { agents: agentidentity[]; agentid: string; tabid: number; sessionid?: string; now: number }): agentidentity[] {
  const agent = input.agents.find(entry => entry.id === input.agentid);
  if (!agent) throw new Error(`The agent ${input.agentid} is not registered.`);
  ...
  return input.agents.map(entry => entry.id === input.agentid ? { ...entry, tabid: input.tabid, ...(input.sessionid !== undefined ? { sessionid: input.sessionid } : {}), heartbeatat: input.now } : entry);
}
...
export function spawn(input: { agents: agentidentity[]; request: spawnrequest; limit: depthlimit; id: string; name?: string; now: number }): agentidentity[] {
  ...
  if (input.limit.maxdepth !== undefined && input.request.depth > input.limit.maxdepth) throw new Error(`The spawn refuses recursion at depth ${input.request.depth}: the user configured depth limit stops at ${input.limit.maxdepth}.`);
  const child: agentidentity = { id: input.id, name: input.name?.trim() !== "" && input.name !== undefined ? input.name.trim() : `${parent.name} sub ${input.request.depth}`, role: input.request.role, parentid: parent.id, depth: input.request.depth, state: "active", registeredat: input.now, heartbeatat: input.now };
...
export function pauseone(input: { agents: agentidentity[]; agentid: string; now: number }): agentidentity[] {
  ...
  if (!agent) throw new Error(`The agent ${input.agentid} is not registered.`);
  if (agent.state === "stopped") throw new Error(`The agent ${agent.name} is stopped; a stopped agent needs no pause.`);
  return input.agents.map(entry => entry.id === input.agentid ? { ...entry, state: "paused" as const, heartbeatat: input.now } : entry);
}
...
export function resumeone(input: { agents: agentidentity[]; agentid: string; now: number }): agentidentity[] {
  ...
  if (!agent) throw new Error(`The agent ${input.agentid} is not registered.`);
  if (agent.state !== "paused") throw new Error(`The agent ${agent.name} is not paused.`);
  return input.agents.map(entry => entry.id === input.agentid ? { ...entry, state: "active" as const, heartbeatat: input.now } : entry);
}
...
export function stopone(input: { agents: agentidentity[]; agentid: string; now: number }): agentidentity[] {
  const agent = input.agents.find(entry => entry.id === input.agentid);
  if (!agent) throw new Error(`The agent ${input.agentid} is not registered.`);
  return input.agents.map(entry => entry.id === input.agentid ? { ...entry, state: "stopped" as const, heartbeatat: input.now } : entry);
}
...
export function killall(input: { agents: agentidentity[]; reason?: string; now: number }): { agents: agentidentity[]; killswitch: killswitch } {
  return {
    agents: input.agents.map(agent => ({ ...agent, state: "stopped" as const, heartbeatat: input.now })),
    killswitch: { engaged: true, engagedat: input.now, ...(input.reason !== undefined && input.reason.trim() !== "" ? { reason: input.reason } : {}) }
  };
}
...
export function agentbudgetcheck(input: { agent: agentidentity; usage: agentusage | undefined }): { halted: boolean; reason?: string } {
  const budget = input.agent.budget;
  const usage = input.usage;
  if (!budget || !usage) return { halted: false };
  if (budget.maxtokens !== undefined && usage.tokens > budget.maxtokens) return { halted: true, reason: `The agent ${input.agent.name} spent ${usage.tokens} tokens past its ceiling of ${budget.maxtokens}; the user raises the ceiling or stops the agent.` };
  if (budget.maxcost !== undefined && usage.cost > budget.maxcost) return { halted: true, reason: `The agent ${input.agent.name} spent ${usage.cost} past its cost ceiling of ${budget.maxcost}; the user raises the ceiling or stops the agent.` };
  if (budget.maxsteps !== undefined && usage.steps > budget.maxsteps) return { halted: true, reason: `The agent ${input.agent.name} executed ${usage.steps} steps past its ceiling of ${budget.maxsteps}; the user raises the ceiling or stops the agent.` };
  return { halted: false };
}
...
export function scopegate(input: { scope: agentscope | undefined; origin?: string; namespace?: toolnamespace }): { allowed: boolean; reason?: string } {
  if (!input.scope) return { allowed: true };
  if (input.origin !== undefined && input.scope.origins.length > 0 && !input.scope.origins.includes(input.origin)) return { allowed: false, reason: `The agent scope grants no access to the origin ${input.origin}.` };
  if (input.namespace !== undefined && input.scope.toolnamespaces.length > 0 && !input.scope.toolnamespaces.includes(input.namespace)) return { allowed: false, reason: `The agent scope grants no access to the ${input.namespace} tool namespace.` };
  return { allowed: true };
}
...
export function recordagentusage(input: { usage: agentusage | undefined; agentid: string; tokens?: number; cost?: number; steps?: number; now: number }): agentusage {
...
export function agentruncontext(input: { agent: agentidentity; task: taskitem; now: number }): { agentid: string; taskid: string; run: workflowrun } {
...
export function agenteventof(input: { id: string; kind: agenteventkind; summary: string; agentid?: string; taskid?: string; now: number }): agentevent {
  return { id: input.id, kind: input.kind, summary: input.summary, ...(input.agentid !== undefined ? { agentid: input.agentid } : {}), ...(input.taskid !== undefined ? { taskid: input.taskid } : {}), at: input.now };
}

/** Returns the swarm at a glance: the agents with their live states, the tasks by claim state and the message counters. */
export function swarmoverview(input: { agents: agentidentity[]; queue: taskqueue; mailboxes: agentmailbox[] }): { agents: number; active: number; paused: number; stopped: number; tasks: number; queued: number; claimed: number; done: number; cancelled: number; messages: number; unread: number } {
...
export function swarmstateof(input: { agents: agentidentity[]; queue: taskqueue; mailboxes: agentmailbox[]; killswitch: killswitch }): swarmstate {
...
export function preparehandoff(input: { agents: agentidentity[]; id: string; fromagentid: string; toagentid: string; taskstate: string; tabid?: number; reason?: string; now: number }): handoffrecord {
  ...
  if (tabid === undefined) throw new Error("The handoff needs its tab id; the transferring agent holds no tab to hand off.");
  return { id: input.id, fromagentid: input.fromagentid, toagentid: input.toagentid, tabid, taskstate: input.taskstate, state: "prepared", ...(input.reason !== undefined && input.reason.trim() !== "" ? { reason: input.reason } : {}), createdat: input.now };
}
...
export function transferhandoff(input: { agents: agentidentity[]; handoffs: handoffrecord[]; id: string; now: number }): { agents: agentidentity[]; handoffs: handoffrecord[] } {
  ...
  return { agents, handoffs: input.handoffs.map(entry => entry.id === input.id ? { ...entry, state: "transferred" as const, transferredat: input.now } : entry) };
}
...
export function acquirelock(input: { locks: resourcelock[]; holder: string; origin: string; selector: string; kind?: "exclusive" | "shared"; expiresat?: number; now: number }): { locks: resourcelock[]; acquired: boolean; reason: string } {
  ...
  if (held.some(lock => lock.holder === input.holder)) return { locks: input.locks, acquired: false, reason: `The agent ${input.holder} already holds the lock ${key}.` };
  if (held.length > 0) {
    if (held.some(lock => lock.kind === "exclusive") || kind === "exclusive") return { locks: input.locks, acquired: false, reason: `The lock ${key} is held ${held.some(lock => lock.kind === "exclusive") ? "exclusively" : "shared"}; the ${kind} request of ${input.holder} refuses.` };
  }
  const lock: resourcelock = { key, holder: input.holder, kind, origin: input.origin.trim(), selector: input.selector.trim(), acquiredat: input.now, ...(input.expiresat !== undefined ? { expiresat: input.expiresat } : {}) };
...
export function releaselock(input: { locks: resourcelock[]; key: string; holder: string; now: number }): { locks: resourcelock[]; released: boolean } {
...
export function expirelocks(input: { locks: resourcelock[]; now: number }): { locks: resourcelock[]; expired: string[] } {
...
export function mergeresults(input: { entries: mergeentry[]; rule: mergerule; preferagent?: string; now: number }): { entries: mergeentry[]; conflicts: string[]; refused: boolean } {
  ...
  for (const [key, entries] of keys) {
    ...
    const note = input.rule === "preferagent" && input.preferagent !== undefined && !ordered.some(entry => entry.agentid === input.preferagent)
...
export function swarmreport(input: { id: string; title: string; outputs: mergeentry[]; rule: mergerule; preferagent?: string; confidence?: string; now: number }): { report: resultreport; conflicts: string[]; refused: boolean } {
  if (input.title.trim() === "") throw new Error("The report needs its title.");
  const fold = mergeresults({ entries: input.outputs, rule: input.rule, ...(input.preferagent !== undefined ? { preferagent: input.preferagent } : {}), now: input.now });
  ...
  return {
    report: { id: input.id, title: input.title.trim(), sections, sources: [...new Set(input.outputs.map(output => output.agentid))], ...(input.confidence !== undefined && input.confidence.trim() !== "" ? { confidence: input.confidence } : {}), createdat: input.now },
...
export function compareoutputs(input: { id: string; subject: string; outputs: Array<{ agentid: string; value: string }>; now: number }): outputcomparison {
...
export function sharelesson(input: { board: blackboard; id: string; agentid: string; statement: string; verifiedby: string; section?: blackboardsection; consentclass?: actionrisk; now: number }): blackboard {
...
export function replayagentrun(input: { events: Array<{ id: string; kind: string; summary: string; at: number; agentid?: string }>; agentid: string }): swarmaction[] {
  return interleavetimeline(input.events.filter(event => event.agentid === input.agentid).map(event => ({ id: event.id, kind: event.kind, summary: event.summary, at: event.at, ...(event.agentid !== undefined ? { agentid: event.agentid } : {}) })));
}
