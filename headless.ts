...
import type { consentprovider, flowrungate, flowrunrequest, headlessevent, headlessfixture, headlessfixtureoutcome, headlesssession, immutablelogentry, jsonrpcframe, libraryrunhandle, planfile, planfilestep, planprogress, stepoutcome, storedrunlog, toolstep } from "./types.js";
...
export function headlessfixturestep(fixture: headlessfixture, step: planfilestep): headlessfixtureoutcome {
  const gate = fixtureconsentgate({ fixtureorigin: fixture.origin, grants: fixture.grants, kind: step.kind, riskof: kind => { try { return actionrisk(kind as never); } catch { return "sensitive" as const; } } });
  if (!gate.allowed) {
  ...
  const projections: Record<string, () => { summary: string; details?: Record<string, unknown> }> = {
    ...
    countelements: () => ({ summary: `The recorded page counts ${observation.interactive.length} interactive element${observation.interactive.length === 1 ? "" : "s"}${step.target !== undefined ? ` for the selector ${step.target}` : ""}.`, details: { count: observation.interactive.length } })
  };
  const projection = projections[step.kind];
  if (projection === undefined) return { stepid: step.id, kind: step.kind, state: "unsupported", summary: `The kind ${step.kind} is read only but a recorded page state carries no projection for it; the fixture cannot satisfy the step without a live tab.` };
  const projected = projection();
  return { stepid: step.id, kind: step.kind, state: "done", summary: projected.summary, ...(projected.details !== undefined ? { details: projected.details } : {}) };
}
...
export function headlessprogress(input: { progress: planprogress | undefined; planid: string; step: planfilestep; outcome: headlessfixtureoutcome; now: number }): planprogress {
  let progress = recordstep(input.progress, input.planid, input.step.id, input.now);
  const recorded: stepoutcome = { stepid: input.step.id, ok: input.outcome.state === "done", summary: input.outcome.summary, ...(input.outcome.details !== undefined ? { details: input.outcome.details } : {}), at: input.now };
...
export function openheadlesssession(input: { origin: string; provider?: consentprovider; now: number; hostoptin?: boolean }): headlesssession {
...
export async function headlessgatewait(input: { session: headlesssession; gate: flowrungate; provider?: consentprovider }): Promise<{ session: headlesssession; resolution: "approve" | "refuse"; reason: string }> {
  if (input.session.state !== "active") throw new Error("The headless session closed; a closed session resolves no gate.");
  const resolution = input.provider !== undefined ? await input.provider.resolvegate(input.gate) : undefined;
  const gate = headlessconsentgate({ providerpresent: input.provider !== undefined, ...(resolution !== undefined ? { resolution } : {}) });
  if (input.provider !== undefined && resolution === "approve") return { session: { ...input.session, gatesresolved: input.session.gatesresolved + 1 }, resolution: "approve", reason: gate.reason ?? "The consent provider approved the gate." };
...
export function remoteattachframes(input: { id: number | string; session: headlesssession; runtime: string }): { initialize: jsonrpcframe; ready: jsonrpcframe } {
  return {
...
export async function headlessauditread(log: storedrunlog): Promise<{ ok: boolean; entries: immutablelogentry[]; reason: string }> {
...
export function headlessplan(file: planfile, grants: string[]): { version: string; workflow: { name: string; origins: string[]; steps: Array<Record<string, unknown>> } } {
  if (file.origin.trim() === "" || !file.origin.startsWith("https://")) throw new Error("The headless plan needs the HTTPS origin it addresses.");
  if (!grants.includes(file.origin)) throw new Error(`The headless plan origin ${file.origin} stays outside the grants; a run without a granted origin never starts.`);
  return {
    version: protocolversion,
    workflow: {
      name: `headless ${file.goal}`.trim().slice(0, 80),
      origins: [file.origin],
      steps: file.steps.map(step => ({ id: step.id, kind: step.kind, label: step.label, ...(step.target !== undefined ? { target: step.target } : {}), ...(step.value !== undefined ? { value: step.value } : {}) }))
    }
  };
}
...
export function headlessrun(input: { request: flowrunrequest; file: planfile; provider?: consentprovider }): { request: flowrunrequest; file: planfile; provider?: consentprovider } {
  return { request: input.request, file: input.file, ...(input.provider !== undefined ? { provider: input.provider } : {}) };
}
...
export function headlesssessionview(session: headlesssession): { id: string; origin: string; state: string; gatesresolved: number; telemetry: boolean } {
  return { id: session.id, origin: session.origin, state: session.state, gatesresolved: session.gatesresolved, telemetry: session.telemetry };
}
...
export function headlesssubscribe(handler: (event: headlessevent) => void): { emit(event: headlessevent): void; events(): headlessevent[] } {
  const events: headlessevent[] = [];
  return {
    emit(event: headlessevent): void { events.push(event); handler(event); },
    events(): headlessevent[] { return [...events]; }
  };
}
...
export function openlibraryrun(input: { plan: planfile; statesource: (origin: string) => headlessfixture | undefined; policy?: { grants?: string[] }; now: number }): { handle(): libraryrunhandle; step(step: planfilestep, now: number): headlessfixtureoutcome; pause(now: number): void; resume(now: number): void; cancel(reason: string, now: number): void } {
  const fixture = input.statesource(input.plan.origin);
  if (fixture === undefined) throw new Error(`The state source carries no recorded state for the origin ${input.plan.origin}; the library run needs its state before it opens.`);
  let progress: planprogress | undefined;
  let paused = false;
  let cancelled = false;
  let reason: string | undefined;
  const handle = (): libraryrunhandle => ({ runid: `library:${input.plan.origin}:${input.now}`, progress: progress ?? { planid: input.plan.origin, completedsteps: [], updatedat: input.now }, paused, cancelled, ...(reason !== undefined ? { reason } : {}) });
  const state = {
    handle,
    step(step: planfilestep, now: number): headlessfixtureoutcome {
      ...
      const scopedgrants = input.policy?.grants === undefined ? fixture.grants : fixture.grants.filter(kind => input.policy?.grants?.includes(kind));
    ...
    pause(now: number): void { paused = true; if (progress !== undefined) progress = { ...progress, updatedat: now }; },
    resume(now: number): void { paused = false; if (progress !== undefined) progress = { ...progress, updatedat: now }; },
    cancel(cancelreason: string, now: number): void { cancelled = true; reason = cancelreason; if (progress !== undefined) progress = { ...progress, updatedat: now }; }
  };
  return state;
}
