...
export function markuprenderstep(step: Pick<toolstep, "kind" | "options">): boolean {
  ...
  try {
    ...
    return Boolean(parsed && typeof parsed === "object" && !Array.isArray(parsed) && typeof (parsed as Record<string, unknown>).markup === "string" && ((parsed as Record<string, unknown>).markup as string).trim() !== "");
  } catch { return false; }
}
...
export function routeenvironment(step: toolstep, input: { offload: boolean; granted: boolean }): { environment: environmentkind; fallback: boolean; reason: string } {
...
export function workerrequestof(input: { id: string; runid: string; stepid: string; kind: actionkind; payload: string; options?: Record<string, unknown>; sentat: number }): workerrequest {
...
export function workerresponseof(input: { id: string; requestid: string; ok: boolean; result?: string; partial?: number; summary: string; receivedat: number }): workerresponse {
  if (input.summary.trim() === "") throw new Error("The worker answer needs its summary in plain language.");
  return { id: input.id, requestid: input.requestid, ok: input.ok, ...(input.result !== undefined ? { result: input.result } : {}), ...(input.partial !== undefined ? { partial: input.partial } : {}), summary: input.summary, receivedat: input.receivedat };
}
...
export function acceptworkerresponse(response: workerresponse): { done: boolean; partial: boolean; result?: string; reason: string } {
  if (!response.ok) return { done: true, partial: false, reason: `The worker refused the request ${response.requestid}: ${response.summary}` };
  if (response.partial !== undefined) return { done: false, partial: true, reason: `The partial ${response.partial} of the request ${response.requestid} streams back to the executor.` };
  return { done: true, partial: false, ...(response.result !== undefined ? { result: response.result } : {}), reason: `The request ${response.requestid} completed inside the offscreen worker pool.` };
}
...
export function poolplan(input: { pending: number; current: number; size?: number }): { workers: number; added: number; retired: number; reason: string } {
...
export function openoffscreen(input: { registry: offscreenregistryentry[]; document: string; runid: string; reasons: string[]; justification: string; now: number }): { registry: offscreenregistryentry[]; entry: offscreenregistryentry; reused: boolean } {
  ...
  const entry: offscreenregistryentry = { document: input.document, runid: input.runid, reasons: [...input.reasons], justification: input.justification, createdat: input.now };
...
export function closeoffscreen(registry: offscreenregistryentry[], runid: string, now: number): { registry: offscreenregistryentry[]; closed: boolean } {
...
export function isolatedinjection(step: toolstep): { world: "ISOLATED"; code: string; args: string[] } {
  ...
  if (step.options) {
    ...
    } catch { /* an options object outside the reviewed argument list stays empty */ }
  }
...
export function summaryrequestof(input: { id: string; runid: string; sessionid: string; payload: string; sentat: number }): workerrequest {
...
export function stripscripts(markup: string): string {
  ...
  while (index < markup.length) {
    ...
    if (open === -1) { out += markup.slice(index); break; }
    out += markup.slice(index, open);
    const close = markup.indexOf(">", open);
    if (close === -1) { index = markup.length; break; } /* an unterminated tag fragment never renders */
    const body = markup.slice(open + 1, close);
    if (body.includes("<")) { index = close + 1; continue; } /* a reassembled payload drops as a whole */
    const namematch = /^\/?\s*([a-zA-Z][a-zA-Z0-9:-]*)/.exec(body);
    const name = (namematch?.[1] ?? "").toLowerCase();
    if (name === "script") {
      if (body.startsWith("/") || body.endsWith("/")) { index = close + 1; continue; } /* a closing or self closing script tag drops alone */
      const rest = markup.slice(close + 1).toLowerCase();
      const end = rest.indexOf("</script");
      if (end === -1) { index = markup.length; break; } /* an unterminated script element swallows the rest */
...
function rebuildtag(name: string, body: string): string {
  ...
  while (cursor < body.length) {
    ...
    if (charof(body, cursor) !== "=") { kept.push(attrname); continue; }
    cursor += 1;
...
export function sandboxrenderof(input: { id: string; markup: string; sourceorigin: string; stepid: string; now: number }): sandboxrender {
...
export function acceptrenderresult(input: { renders: sandboxrender[]; message: { channel?: string; type?: string; nonce?: string; ok?: boolean; text?: string; summary?: string }; now: number }): { accepted: boolean; result?: sandboxrenderresult; reason: string; renders: sandboxrender[] } {
  if (input.message.channel !== "devthinksandbox") return { accepted: false, reason: "The sandbox message travels the devthinksandbox channel only.", renders: input.renders };
  if (input.message.type !== "renderresult") return { accepted: false, reason: "The sandbox message answers with the renderresult type only.", renders: input.renders };
  const render = input.renders.find(entry => entry.nonce === input.message.nonce && entry.answeredat === undefined && entry.renderedat <= input.now);
  if (!render) return { accepted: false, reason: "The sandbox message carries no nonce of a known unanswered render; a stale, replayed or already answered message never passes.", renders: input.renders };
  const text = striptags(input.message.text ?? "");
  const result: sandboxrenderresult = { nonce: render.nonce, ok: input.message.ok !== false, text, summary: input.message.summary?.trim() || `The sandbox frame rendered the markup of the step ${render.stepid} and returned its inert text.`, at: input.now };
  const renders = input.renders.map(entry => entry.nonce === render.nonce ? { ...entry, answeredat: input.now } : entry);
  return { accepted: true, result, reason: `The render result of the step ${render.stepid} answers the nonce of its render; the text stays inside the frame.`, renders };
}
