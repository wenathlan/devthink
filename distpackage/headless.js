...
async function headlessgatewait(input) {
  if (input.session.state !== "active") throw new Error("The headless session closed; a closed session resolves no gate.");
  const resolution = input.provider !== void 0 ? await input.provider.resolvegate(input.gate) : void 0;
  const gate = headlessconsentgate({ providerpresent: input.provider !== void 0, ...resolution !== void 0 ? { resolution } : {} });
  if (input.provider !== void 0 && resolution === "approve") return { session: { ...input.session, gatesresolved: input.session.gatesresolved + 1 }, resolution: "approve", reason: gate.reason ?? "The consent provider approved the gate." };
  return { session: input.session, resolution: "refuse", reason: gate.reason ?? "The gate refused under denydefault." };
}
function closeheadlesssession(session) {
...
function headlesspolicyvalidate(step, origin) {
  return validatestep(step, origin);
}
async function headlessauditread(log) {
  return readverifiedlog(log);
}
function headlessmemory(adapter) {
  return new sessionmemory(adapter);
}
function headlessplan(file, grants) {
  ...
  return {
    ...
    workflow: {
      ...
      origins: [file.origin],
      steps: file.steps.map((step) => ({ id: step.id, kind: step.kind, label: step.label, ...step.target !== void 0 ? { target: step.target } : {}, ...step.value !== void 0 ? { value: step.value } : {} }))
    }
  };
}
function headlessrun(input) {
  return { request: input.request, file: input.file, ...input.provider !== void 0 ? { provider: input.provider } : {} };
}
function headlesssessionview(session) {
...
function headlesssubscribe(handler) {
  const events = [];
  return {
    emit(event) {
      events.push(event);
...
function headlesstelemetryposture(hostoptin) {
