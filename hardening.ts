...
export function masklogtext(input: { text: string; shapes: string[] }): { text: string; maskedkeys: string[]; reason: string } {
...
export function localscangate(input: { hook?: { scanner: string; localsignatures?: string[] }; digest?: string }): { verdict: scanverdict; reason: string } {
  ...
  if (input.digest === undefined || input.digest.trim() === "") return { verdict: "pending", reason: `The local signature check of ${input.hook?.scanner ?? "the scanning hook"} needs the file digest; a digest nobody computed flags nothing.` };
...
export function hatchbannerof(input: { halted: number; at: number }): { title: string; explanation: string; halted: number; at: number } {
...
export function newdomainpausecard(input: { origin: string; knownorigins: string[]; kind: string }): { pause: boolean; card: { title: string; explanation: string; origin: string }; reason: string } {
...
export function runtracekeys(input: { runid: string; keys: string[] }): { keys: string[]; kept: string[]; reason: string } {
...
export function stdioclientsignaturegate(input: { platformverifier?: (host: string, signature: string) => boolean; host: string; signature?: string }): { verified: boolean; allowed: boolean; reason: string } {
  if (input.signature === undefined || input.platformverifier === undefined) return { verified: false, allowed: true, reason: "The platform carries no signature verifier the bridge can call; the local stdio bridge stays unverified while the record marks it." };
...
export function transparencyexportmask(input: { report: Record<string, unknown>; shapes: string[]; regions: redactregion[] }): { payload: Record<string, unknown>; maskedfields: string[]; redactedregions: number; reason: string } {
  ...
  if (input.regions.length > 0 && Array.isArray(payload.captures)) payload.captures = (payload.captures as unknown[]).map(entry => Boolean(entry) && typeof entry === "object" && !Array.isArray(entry) ? { ...(entry as Record<string, unknown>), redacted: true, redactedregions: input.regions.length } : entry);
