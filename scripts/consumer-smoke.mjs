...
try {
  ...
  check(
    ...
    data.choices?.[0]?.message?.content === "mock reply from the wire",
  );
  ...
  check(
    "streaming body reads as text (bytes not strings)",
    streamtext.includes("streamed ") && donecount >= 1,
  );
  check("exactly one stream terminator", donecount === 1, `got ${donecount}`);
  check(
    "stream masking",
    streamtext.includes('"model":"meta"') && !streamtext.includes("hidden/model"),
  );
