...
export function recordoutcomewithclock(
...
export function recordoutcome(
  ...
  return {
    ...base,
    outcomes: [...(base.outcomes ?? []), { ...outcome, provenance: provenancestampof() }],
    updatedat: now,
  };
}
...
export function recordgatewait(
  progress: planprogress | undefined,
  planid: string,
  stepid: string,
  entry: gatewaitevidence,
  now: number,
): planprogress {
  const base = progress && progress.planid === planid ? progress : emptyprogress(planid, now);
...
export function gatewaitof(
  progress: planprogress | undefined,
  planid: string,
  stepid: string,
): gatewaitevidence | undefined {
  if (!progress || progress.planid !== planid) return undefined;
...
export function resetforplan(progress: planprogress | undefined, plan: agentplan, now: number): planprogress {
  ...
  const snapshot: planprogress = {
    ...
    ...(progress.outcomes ? { outcomes: progress.outcomes } : {}),
  ...
  return {
    ...
    completedsteps: [],
    outcomes: [],
    prior: [...(progress.prior ?? []), snapshot],
    provenance: provenancestampof(),
    updatedat: now,
  };
}
...
export function recordwatchcompletion(
  progress: planprogress | undefined,
  planid: string,
  stepid: string,
  startedat: number,
...
export function recordnaventry(
  progress: planprogress | undefined,
  planid: string,
  stepid: string,
  entry: naventry,
  ...
  const outcome: stepoutcome = {
    stepid,
    ok: entry.ok,
    summary: `Navigation list entry ${entry.index + 1} of ${entry.url} ${entry.ok ? "completed" : "failed"}.`,
    details: { naventry: entry },
    at: now,
  };
...
export function naventries(progress: planprogress | undefined, planid: string, stepid: string): naventry[] {
  ...
  return (progress.outcomes ?? [])
    .filter((outcome) => outcome.stepid === stepid && outcome.details?.naventry !== undefined)
    .map((outcome) => outcome.details?.naventry as naventry);
}

/** Assigns one tab to the running task so progress tracks work across its tabs. */
export function assigntasktab(
  progress: planprogress | undefined,
  planid: string,
  tabid: number,
  now: number,
...
export function releasetasktab(
  progress: planprogress | undefined,
  planid: string,
  tabid: number,
  now: number,
...
export function recordextraction(
  ...
  const outcome: stepoutcome = {
    stepid,
    ok: true,
    summary: `Extraction page ${entry.page} collected ${entry.rows} row${entry.rows === 1 ? "" : "s"} at cursor ${entry.cursor}.`,
    details: { extraction: entry },
    at: now,
  };
  return recordoutcome(base, planid, outcome, now);
}
...
export function extractionentries(
  ...
  if (!progress || progress.planid !== planid) return [];
  return (progress.outcomes ?? [])
    .filter((outcome) => outcome.stepid === stepid && outcome.details?.extraction !== undefined)
    .map((outcome) => outcome.details?.extraction as extractionentry);
}
...
export function recordwizardstep(
  progress: planprogress | undefined,
  planid: string,
  stepid: string,
  state: wizardstate,
  ...
  const outcome: stepoutcome = {
    stepid,
    ok: done,
    summary: `Wizard step ${executed} of ${state.steps} ${done ? "completed the wizard" : "executed"}.`,
    details: { wizard: { index: state.index, steps: state.steps, completed: [...state.completed] } },
    at: now,
  };
  return recordoutcome(base, planid, outcome, now);
}
...
export function recorddownload(
  ...
  const outcome: stepoutcome = {
    stepid,
    ok: entry.state === "complete",
    summary: `Download ${entry.index + 1} of ${entry.url} ended in the ${entry.state} state.`,
    details: { download: entry },
    at: now,
  };
  return recordoutcome(base, planid, outcome, now);
}
...
export function downloadentries(progress: planprogress | undefined, planid: string, stepid: string): downloadentry[] {
  if (!progress || progress.planid !== planid) return [];
  return (progress.outcomes ?? [])
    .filter((outcome) => outcome.stepid === stepid && outcome.details?.download !== undefined)
    .map((outcome) => outcome.details?.download as downloadentry);
}
...
export function recordcapture(
  ...
  const outcome: stepoutcome = {
    stepid,
    ok: true,
    summary: `Captured a ${capture.format} ${capture.kind} shot of ${capture.width} by ${capture.height} pixels with ${bytes} character${bytes === 1 ? "" : "s"} of image data.`,
    details: {
      capture: {
        id: capture.id,
        kind: capture.kind,
...
export function recordpair(
  ...
  const outcome: stepoutcome = {
    stepid,
    ok: true,
    summary: `Paired the before shot ${pair.beforeid} with the after shot ${pair.afterid} around the ${pair.actionkind} action.`,
    details: {
      shotpair: {
        id: pair.id,
        beforeid: pair.beforeid,
        afterid: pair.afterid,
        actionkind: pair.actionkind,
        ...(pair.target !== undefined ? { target: pair.target } : {}),
        ...(pair.domsnapshotid !== undefined ? { domsnapshotid: pair.domsnapshotid } : {}),
      },
    },
    at: now,
  };
...
export function captureentries(
  ...
  planid: string,
  stepid: string,
): Array<{ id: string; kind: string; format: string; width: number; height: number; bytes: number }> {
  if (!progress || progress.planid !== planid) return [];
  return (progress.outcomes ?? [])
    .filter((outcome) => outcome.stepid === stepid && outcome.details?.capture !== undefined)
    .map(
      (outcome) =>
        outcome.details?.capture as {
          id: string;
          kind: string;
          format: string;
          width: number;
...
export function pairentries(
  ...
  planid: string,
  stepid: string,
): Array<{ id: string; beforeid: string; afterid: string; actionkind: string }> {
  if (!progress || progress.planid !== planid) return [];
  return (progress.outcomes ?? [])
    .filter((outcome) => outcome.stepid === stepid && outcome.details?.shotpair !== undefined)
    .map(
      (outcome) => outcome.details?.shotpair as { id: string; beforeid: string; afterid: string; actionkind: string },
    );
}
...
export function recordmedia(
  ...
  planid: string,
  stepid: string,
  media: { id: string; kind: string; scope: string; bytes: number },
  now: number,
): planprogress {
  const base = progress && progress.planid === planid ? progress : emptyprogress(planid, now);
  const outcome: stepoutcome = {
    stepid,
    ok: true,
    summary: `Captured a ${media.kind} media record of ${media.scope} scope with ${media.bytes} character${media.bytes === 1 ? "" : "s"} of media data.`,
    details: { media },
    at: now,
  };
  return recordoutcome(base, planid, outcome, now);
...
export function mediaentries(
  ...
  planid: string,
  stepid: string,
): Array<{ id: string; kind: string; scope: string; bytes: number }> {
  if (!progress || progress.planid !== planid) return [];
  return (progress.outcomes ?? [])
    .filter((outcome) => outcome.stepid === stepid && outcome.details?.media !== undefined)
    .map((outcome) => outcome.details?.media as { id: string; kind: string; scope: string; bytes: number });
}
...
export function recordcall(
  ...
  const outcome: stepoutcome = {
    stepid,
    ok: entry.statusclass === "success",
    summary: `Outbound ${entry.method} ${entry.kind} call to ${entry.origin} ended in the ${entry.status} ${entry.statusclass} class after ${entry.retries} retr${entry.retries === 1 ? "y" : "ies"} and ${entry.bytes} byte${entry.bytes === 1 ? "" : "s"}.`,
    details: { call: entry },
    at: now,
  };
  return recordoutcome(base, planid, outcome, now);
...
export function callentries(progress: planprogress | undefined, planid: string, stepid: string): callentry[] {
  if (!progress || progress.planid !== planid) return [];
  return (progress.outcomes ?? [])
    .filter((outcome) => outcome.stepid === stepid && outcome.details?.call !== undefined)
    .map((outcome) => outcome.details?.call as callentry);
}
...
export function recordfetchretry(
  progress: planprogress | undefined,
  planid: string,
  stepid: string,
  retry: fetchretryentry,
  now: number,
): planprogress {
  const base = progress && progress.planid === planid ? progress : emptyprogress(planid, now);
  const outcome: stepoutcome = {
    stepid,
    ok: false,
    summary: `Fetch attempt ${retry.attempt} of ${retry.url} failed (${retry.reason}); retrying after a ${retry.wait} millisecond backoff.`,
    details: { fetchretry: retry },
    at: now,
  };
  return recordoutcome(base, planid, outcome, now);
}
...
export function fetchretryentries(
  progress: planprogress | undefined,
  planid: string,
  stepid: string,
): fetchretryentry[] {
  if (!progress || progress.planid !== planid) return [];
  return (progress.outcomes ?? [])
    .filter((outcome) => outcome.stepid === stepid && outcome.details?.fetchretry !== undefined)
    .map((outcome) => outcome.details?.fetchretry as fetchretryentry);
}
...
export function recordchannel(
  ...
  const outcome: stepoutcome = {
    ...
    summary: `The ${entry.kind} channel of ${entry.url} is ${entry.state} after ${entry.sent} sent and ${entry.received} received message${entry.received === 1 ? "" : "s"}.`,
    details: { channel: entry },
    at: now,
  };
  return recordoutcome(base, planid, outcome, now);
}
...
export function channelentries(progress: planprogress | undefined, planid: string, stepid: string): channelentry[] {
  if (!progress || progress.planid !== planid) return [];
  return (progress.outcomes ?? [])
    .filter((outcome) => outcome.stepid === stepid && outcome.details?.channel !== undefined)
    .map((outcome) => outcome.details?.channel as channelentry);
}
...
export function recordexchange(
  ...
  planid: string,
  stepid: string,
  entry: exchangeevidence,
  now: number,
): planprogress {
  const base = progress && progress.planid === planid ? progress : emptyprogress(planid, now);
  const outcome: stepoutcome = {
    stepid,
    ok: entry.errorclass === undefined && entry.statusclass === "success",
    summary: `Observed the ${entry.method} request of ${entry.url} as exchange ${entry.correlationid} in the ${entry.status} ${entry.statusclass} class over ${entry.duration} millisecond${entry.duration === 1 ? "" : "s"}${entry.errorclass !== undefined ? ` failing with the ${entry.errorclass} class` : ""}.`,
    details: { exchange: entry },
    at: now,
  };
  return recordoutcome(base, planid, outcome, now);
}
...
export function exchangeentries(
  progress: planprogress | undefined,
  planid: string,
  stepid: string,
): exchangeevidence[] {
  if (!progress || progress.planid !== planid) return [];
  return (progress.outcomes ?? [])
    .filter((outcome) => outcome.stepid === stepid && outcome.details?.exchange !== undefined)
    .map((outcome) => outcome.details?.exchange as exchangeevidence);
}
...
export function recordevent(
  ...
  const outcome: stepoutcome = {
    stepid,
    ok: true,
    summary: `The event stream of ${entry.url} observed ${entry.events} ${entry.name || "message"} event${entry.events === 1 ? "" : "s"}${entry.lasteventid !== undefined ? ` resuming from ${entry.lasteventid}` : ""}.`,
    details: { event: entry },
    at: now,
  };
  return recordoutcome(base, planid, outcome, now);
...
export function evententries(progress: planprogress | undefined, planid: string, stepid: string): evententry[] {
  if (!progress || progress.planid !== planid) return [];
  return (progress.outcomes ?? [])
    .filter((outcome) => outcome.stepid === stepid && outcome.details?.event !== undefined)
    .map((outcome) => outcome.details?.event as evententry);
}
...
export function recordpoll(
  ...
  const outcome: stepoutcome = {
    stepid,
    ok: true,
    summary: `Long poll ${entry.poll} returned the ${entry.status} status${entry.cursor !== undefined ? ` at cursor ${entry.cursor}` : ""} and ${entry.stopped ? `stopped: ${entry.reason}` : "continues"}.`,
    details: { poll: entry },
    at: now,
  };
  return recordoutcome(base, planid, outcome, now);
...
export function pollentries(progress: planprogress | undefined, planid: string, stepid: string): pollentry[] {
  if (!progress || progress.planid !== planid) return [];
  return (progress.outcomes ?? [])
    .filter((outcome) => outcome.stepid === stepid && outcome.details?.poll !== undefined)
    .map((outcome) => outcome.details?.poll as pollentry);
}
...
export function recordcontrol(
  ...
  const outcome: stepoutcome = {
    stepid,
    ok: true,
    summary: `${entry.reason}: ${entry.applied} applied rule${entry.applied === 1 ? "" : "s"}, ${entry.blocked} blocked request${entry.blocked === 1 ? "" : "s"}, ${entry.mocked} mocked response${entry.mocked === 1 ? "" : "s"} and ${entry.reverts} reverted rule${entry.reverts === 1 ? "" : "s"}.`,
    details: { control: entry },
    at: now,
  };
  return recordoutcome(base, planid, outcome, now);
}
...
export function controlentries(progress: planprogress | undefined, planid: string, stepid: string): controlevidence[] {
  if (!progress || progress.planid !== planid) return [];
  return (progress.outcomes ?? [])
    .filter((outcome) => outcome.stepid === stepid && outcome.details?.control !== undefined)
    .map((outcome) => outcome.details?.control as controlevidence);
}
...
export function recordupload(
  ...
  const outcome: stepoutcome = {
    stepid,
    ok: true,
    summary: `The multipart upload moved chunk ${entry.chunk} of ${entry.chunks} with ${entry.uploaded} of ${entry.bytes} bytes sent.`,
    details: { upload: entry },
    at: now,
  };
  return recordoutcome(base, planid, outcome, now);
}
...
export function uploadentries(progress: planprogress | undefined, planid: string, stepid: string): uploadentry[] {
  if (!progress || progress.planid !== planid) return [];
  return (progress.outcomes ?? [])
    .filter((outcome) => outcome.stepid === stepid && outcome.details?.upload !== undefined)
    .map((outcome) => outcome.details?.upload as uploadentry);
}
...
export function recordtimeline(
  ...
  const outcome: stepoutcome = {
    stepid,
    ok: true,
    summary: `Captured ${entry.entries} timeline entr${entry.entries === 1 ? "y" : "ies"} with ${entry.collapsed} collapsed repeat${entry.collapsed === 1 ? "" : "s"}, ${entry.errors} error${entry.errors === 1 ? "" : "s"}, ${entry.rejections} rejection${entry.rejections === 1 ? "" : "s"} and ${entry.longtasks} long task${entry.longtasks === 1 ? "" : "s"}.`,
...
export function timelineevidences(
  progress: planprogress | undefined,
  planid: string,
  stepid: string,
): timelineevidence[] {
  if (!progress || progress.planid !== planid) return [];
  return (progress.outcomes ?? [])
    .filter((outcome) => outcome.stepid === stepid && outcome.details?.timeline !== undefined)
    .map((outcome) => outcome.details?.timeline as timelineevidence);
}
...
export function cdpevidences(progress: planprogress | undefined, planid: string, stepid: string): cdpevidence[] {
  if (!progress || progress.planid !== planid) return [];
  return (progress.outcomes ?? [])
    .filter((outcome) => outcome.stepid === stepid && outcome.details?.cdp !== undefined)
    .map((outcome) => outcome.details?.cdp as cdpevidence);
}
...
export function recordprofile(
  ...
  const base = progress && progress.planid === planid ? progress : emptyprogress(planid, now);
  const counts =
    `${entry.metrics !== undefined ? `${entry.metrics} metric${entry.metrics === 1 ? "" : "s"}, ` : ""}${entry.samples !== undefined ? `${entry.samples} sample${entry.samples === 1 ? "" : "s"}, ` : ""}${entry.nodes !== undefined ? `${entry.nodes} node${entry.nodes === 1 ? "" : "s"}, ` : ""}${entry.events !== undefined ? `${entry.events} event${entry.events === 1 ? "" : "s"}, ` : ""}${entry.bytes !== undefined ? `${entry.bytes} byte${entry.bytes === 1 ? "" : "s"}, ` : ""}${entry.flagged !== undefined ? `${entry.flagged} flagged step${entry.flagged === 1 ? "" : "s"}, ` : ""}`.replace(
      /, $/,
      "",
    );
  const outcome: stepoutcome = {
    stepid,
    ok: true,
    summary: `The profiling ${entry.family} capture ran${counts.length > 0 ? ` with ${counts}` : ""}.`,
    details: { profile: entry },
    at: now,
  };
  return recordoutcome(base, planid, outcome, now);
}
...
export function profileevidences(
  progress: planprogress | undefined,
  planid: string,
  stepid: string,
): profileevidence[] {
  if (!progress || progress.planid !== planid) return [];
  return (progress.outcomes ?? [])
    .filter((outcome) => outcome.stepid === stepid && outcome.details?.profile !== undefined)
    .map((outcome) => outcome.details?.profile as profileevidence);
}
...
export function recordemulation(
  progress: planprogress | undefined,
  planid: string,
  stepid: string,
  entry: emulationevidence,
  now: number,
): planprogress {
  const base = progress && progress.planid === planid ? progress : emptyprogress(planid, now);
  const outcome: stepoutcome = {
    stepid,
    ok: true,
    summary: `${entry.reason}: ${entry.applied.length} applied layer${entry.applied.length === 1 ? "" : "s"}${entry.applied.length > 0 ? ` (${entry.applied.join(", ")})` : ""} and ${entry.reverted.length} reverted layer${entry.reverted.length === 1 ? "" : "s"}${entry.reverted.length > 0 ? ` (${entry.reverted.join(", ")})` : ""}.`,
    details: { emulation: entry },
    at: now,
  };
  return recordoutcome(base, planid, outcome, now);
}
...
export function emulationevidences(
  progress: planprogress | undefined,
  planid: string,
  stepid: string,
): emulationevidence[] {
  if (!progress || progress.planid !== planid) return [];
  return (progress.outcomes ?? [])
    .filter((outcome) => outcome.stepid === stepid && outcome.details?.emulation !== undefined)
    .map((outcome) => outcome.details?.emulation as emulationevidence);
}
...
export function recordsession(
  ...
  const outcome: stepoutcome = {
    stepid,
    ok: true,
    summary: `${entry.detail}${counts.length > 0 ? ` with ${counts}` : ""}.`,
    details: { session: entry },
    at: now,
  };
  return recordoutcome(base, planid, outcome, now);
}
...
export function sessionevidences(
  progress: planprogress | undefined,
  planid: string,
  stepid: string,
): sessionevidence[] {
  if (!progress || progress.planid !== planid) return [];
  return (progress.outcomes ?? [])
    .filter((outcome) => outcome.stepid === stepid && outcome.details?.session !== undefined)
    .map((outcome) => outcome.details?.session as sessionevidence);
}
...
export function recordworkflow(
  ...
  planid: string,
  stepid: string,
  entry: workflowevidence,
  now: number,
): planprogress {
  const base = progress && progress.planid === planid ? progress : emptyprogress(planid, now);
  const counts =
    `${entry.executed !== undefined ? `${entry.executed} executed step${entry.executed === 1 ? "" : "s"}, ` : ""}${entry.refused !== undefined ? `${entry.refused} refused step${entry.refused === 1 ? "" : "s"}, ` : ""}${entry.total !== undefined ? `${entry.total} total step${entry.total === 1 ? "" : "s"}, ` : ""}${entry.iterations !== undefined ? `${entry.iterations} iteration${entry.iterations === 1 ? "" : "s"}, ` : ""}${entry.denominator !== undefined ? `denominator ${entry.denominator}, ` : ""}`.replace(
      /, $/,
      "",
    );
  const outcome: stepoutcome = {
...
export function workflowevidences(
  progress: planprogress | undefined,
  planid: string,
  stepid: string,
): workflowevidence[] {
  if (!progress || progress.planid !== planid) return [];
  return (progress.outcomes ?? [])
    .filter((outcome) => outcome.stepid === stepid && outcome.details?.workflow !== undefined)
    .map((outcome) => outcome.details?.workflow as workflowevidence);
}
...
export function recordtrigger(
  ...
  planid: string,
  stepid: string,
  entry: triggerevidence,
  now: number,
): planprogress {
  const base = progress && progress.planid === planid ? progress : emptyprogress(planid, now);
  const counts =
    `${entry.fires !== undefined ? `${entry.fires} fire${entry.fires === 1 ? "" : "s"}, ` : ""}${entry.launches !== undefined ? `${entry.launches} launch${entry.launches === 1 ? "" : "es"}, ` : ""}${entry.suppressions !== undefined ? `${entry.suppressions} suppression${entry.suppressions === 1 ? "" : "s"}, ` : ""}${entry.queued !== undefined ? `${entry.queued} queued fire${entry.queued === 1 ? "" : "s"}, ` : ""}`.replace(
      /, $/,
      "",
    );
  const outcome: stepoutcome = {
...
export function triggerevidences(
  progress: planprogress | undefined,
  planid: string,
  stepid: string,
): triggerevidence[] {
  if (!progress || progress.planid !== planid) return [];
  return (progress.outcomes ?? [])
    .filter((outcome) => outcome.stepid === stepid && outcome.details?.trigger !== undefined)
    .map((outcome) => outcome.details?.trigger as triggerevidence);
}
...
export function recordtoolcall(
  ...
  const outcome: stepoutcome = {
    stepid,
    ok: entry.ok,
    summary: `The ${entry.tool} tool call of the client ${entry.clientid} ${entry.ok ? "ran behind the consent gates" : `was refused${entry.code !== undefined ? ` with the ${entry.code} error` : ""}`}.`,
    details: { tool: entry },
    at: now,
  };
  return recordoutcome(base, planid, outcome, now);
...
export function toolcallevidences(progress: planprogress | undefined, planid: string, stepid: string): toolevidence[] {
  if (!progress || progress.planid !== planid) return [];
  return (progress.outcomes ?? [])
    .filter((outcome) => outcome.stepid === stepid && outcome.details?.tool !== undefined)
    .map((outcome) => outcome.details?.tool as toolevidence);
}
...
export function recorddenied(
  ...
  const outcome: stepoutcome = {
    stepid,
    ok: false,
    summary: `The ${entry.kind} step on ${entry.origin} was denied: ${entry.reason}`,
    details: { denied: entry },
    at: now,
  };
  return recordoutcome(base, planid, outcome, now);
}
...
export function deniedevidences(
  progress: planprogress | undefined,
  planid: string,
  stepid: string,
): deniedstepevidence[] {
  if (!progress || progress.planid !== planid) return [];
  return (progress.outcomes ?? [])
    .filter((outcome) => outcome.stepid === stepid && outcome.details?.denied !== undefined)
    .map((outcome) => outcome.details?.denied as deniedstepevidence);
}
...
export function recordrevocation(
  ...
  const outcome: stepoutcome = {
    stepid,
    ok: false,
    summary: `The run halted at the step ${stepid}${entry.haltedstepids.length > 1 ? ` with ${entry.haltedstepids.length - 1} queued step${entry.haltedstepids.length === 2 ? "" : "s"} rolled back` : ""}: ${entry.reason}`,
    details: { revoked: entry },
    at: now,
  };
  return recordoutcome(base, planid, outcome, now);
}
...
export function revocationevidences(
  progress: planprogress | undefined,
  planid: string,
  stepid: string,
): revokedrunevidence[] {
  if (!progress || progress.planid !== planid) return [];
  return (progress.outcomes ?? [])
    .filter((outcome) => outcome.stepid === stepid && outcome.details?.revoked !== undefined)
    .map((outcome) => outcome.details?.revoked as revokedrunevidence);
}
...
export function replayprogress(
  progress: planprogress | undefined,
  planid: string,
): { planid: string; entries: progressreplayentry[]; provenance?: { release: string; protocolmajor: number } } {
  if (!progress || progress.planid !== planid) return { planid, entries: [] };
  const entries = (progress.outcomes ?? []).map((outcome, index) => ({
    index,
