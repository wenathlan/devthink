  ...
  return [
    {
      ...
      handler: "initialize",
      description:
        "Completes the mcp handshake, records the client metadata and returns the server info with the server contract version.",
    },
    { method: "ping", handler: "ping", description: "Answers keepalive frames with pong." },
    {
      ...
      handler: "listtools",
      description:
        "Returns every exposed tool the current grants cover with its version, json schema inputs and consent metadata.",
    },
    { method: "negotiate", handler: "negotiate", description: "Exchanges capability sets with the client." },
    {
      ...
      handler: "dispatch",
      description:
        "Invokes one tool behind the consent gates with the mock, dry run, idempotency and approval composition.",
    },
    {
      method: "prompts/list",
      handler: "listprompts",
      description: "Lists the walkthrough prompts, the user authored template library and the protocol prompt tools.",
    },
    {
      method: "prompts/call",
      handler: "callprompt",
      description: "Renders one served prompt with its declared arguments.",
    },
    {
      method: "resources/list",
      handler: "listresources",
      description: "Lists every served resource with its uri, description and mimetype.",
    },
    { method: "resources/read", handler: "readresource", description: "Reads one served resource into its wire text." },
    {
      method: "resources/subscribe",
      handler: "subscriberesource",
      description: "Opens one resource subscription with its change notifications.",
    },
    {
      method: "resources/unsubscribe",
      handler: "unsubscriberesource",
      description: "Closes one resource subscription.",
    },
    {
      method: "calls/cancel",
      handler: "cancel",
      description: "Aborts one in flight tool call and preserves its partial result.",
    },
    {
      method: "calls/batch",
      handler: "batch",
      description: "Runs an ordered batch of tool calls in one request with the stop on first error flag.",
    },
    {
      method: "health",
      handler: "health",
      description: "Returns the health resource with the version and the uptime.",
    },
  ];
}
...
export function servemetadata(input: { config: mcpserverconfig; catalog: toolcatalog; templates: prompttemplate[] }): {
  serverinfo: capabilityset & { servercontractversion: number; resourcecount: number; promptcount: number };
  ...
  return {
    serverinfo: {
      ...
      promptcount: listmcpprompts(input.templates).length,
    },
    protocolversion,
    servercontractversion,
...
export function recordclientmetadata(
  client: clientrecord,
  ...
  return {
    ...
    ...(metadata.version !== undefined ? { clientversion: metadata.version } : {}),
  };
}
...
export function serveinitialize(input: {
  params?: Record<string, unknown>;
  config: mcpserverconfig;
  catalog: toolcatalog;
  templates: prompttemplate[];
}): { handshake: ReturnType<typeof servemetadata>; client: (client: clientrecord) => clientrecord } {
  return {
    handshake: servemetadata({ config: input.config, catalog: input.catalog, templates: input.templates }),
    client: (client: clientrecord) => recordclientmetadata(client, clientmetadataof(input.params)),
  };
}
...
export function transportendpoints(input: { config: mcpserverconfig; now: number }): servetransport[] {
  ...
  if (input.config.transports.includes("stdio"))
    transports.push({ kind: "stdio", endpoint: "stdio://devthink", startedat: input.now });
  if (input.config.transports.includes("http")) {
    const bind =
      input.config.bind !== undefined && input.config.bind.trim() !== "" ? input.config.bind.trim() : "127.0.0.1";
    const path =
      input.config.httpstream?.endpoint !== undefined && input.config.httpstream.endpoint.trim() !== ""
        ? input.config.httpstream.endpoint.trim()
        : "/mcp";
...
export function startserve(input: { config: mcpserverconfig; now: number; grants?: string[]; origin?: string }): {
  state?: { state: "running"; transports: servetransport[]; startedat: number; degraded: boolean };
  reason?: string;
} {
  const gate = mcpmodegate(input.config);
  if (!gate.allowed) return { reason: gate.reason ?? "The serve mode failed its gate." };
  const degradation = degradationgate({
  ...
  return {
    state: {
      state: "running",
      transports: transportendpoints({ config: input.config, now: input.now }),
      startedat: input.now,
      degraded: !degradation.allowed,
    },
  };
}
...
export function shutdowndrain(input: { inflight: callcontext[]; drainstart?: number; now: number; window?: number }): {
  ...
  if (input.window !== undefined && input.drainstart !== undefined && input.now - input.drainstart >= input.window)
    return { phase: "stopped", waiting };
...
export function healthof(input: { startedat: number; now: number; state: string }): servehealth {
  return {
    ...
    protocolversion,
    servercontractversion,
    uptime: Math.max(0, input.now - input.startedat),
    state: input.state,
    at: input.now,
  };
}
...
export function bindclientsession(input: {
  clientid: string;
  sessionid: string;
  tokenid?: string;
  grants?: string[];
...
export function releasebinding(bindings: clientbinding[], clientid: string, now: number): clientbinding[] {
  return bindings.map((binding) =>
    binding.clientid === clientid && binding.releasedat === undefined ? { ...binding, releasedat: now } : binding,
  );
}
...
export function exposedtools(
  catalog: toolcatalog,
  options: { readonly?: boolean; scopes?: toolnamespace[] } = {},
): toolcatalog {
  const exposed = alltools(catalog).filter(
    (tool) =>
      (options.readonly !== true || tool.risk === "read") &&
      (options.scopes === undefined || options.scopes.includes(namespaceof(tool.name) as toolnamespace)),
  );
  return {
    ...
    domains: catalog.domains
      .map((domain) => ({
        ...domain,
        tools: domain.tools.filter((tool) => exposed.some((entry) => entry.name === tool.name)),
      }))
...
export function planprogressevent(input: {
  stepid: string;
  status: string;
  sessionid?: string;
  sequence: number;
  at: number;
}): { envelope: ReturnType<typeof composeenvelope>; wire: string } {
  const envelope = composeenvelope({
    op: "eventpost",
    opid: stableopid(`mcpmode-progress-${input.stepid}`, input.sequence),
    at: input.at,
    ...
    body: eventpostbody({
      event: {
        kind: "progress",
        stream: "mcpmode",
        payload: progresseventpayload({ stepid: input.stepid, status: input.status }),
      },
    }),
  });
...
export function samplingframe(input: {
  clientid: string;
  ...
  maxtokens?: number;
  now: number;
}): { request?: samplingrequest; frame?: jsonrpcframe; reason?: string } {
  const requested = requestsampling({
    ...
    ...(input.pagegrant !== undefined ? { pagegrant: input.pagegrant } : {}),
    ...(input.maxtokens !== undefined ? { maxtokens: input.maxtokens } : {}),
    now: input.now,
  });
  if (requested.request === undefined)
    return { ...(requested.reason !== undefined ? { reason: requested.reason } : {}) };
  return {
    ...
    frame: {
      ...
      params: {
        id: requested.request.id,
        prompt: requested.request.prompt,
        ...(requested.request.system !== undefined ? { system: requested.request.system } : {}),
        ...(requested.request.pagecontent !== undefined ? { pagecontent: requested.request.pagecontent } : {}),
        ...(requested.request.maxtokens !== undefined ? { maxtokens: requested.request.maxtokens } : {}),
      },
    },
  };
}
...
export function chunkframes(chunks: streamchunk[]): jsonrpcframe[] {
  return chunks.map((chunk) =>
    notificationframe("calls/streamchunk", {
      callid: chunk.callid,
      seq: chunk.seq,
...
export function progressframe(notice: progressnotice): jsonrpcframe {
  return notificationframe("calls/progress", {
...
export function chunkof(input: { callid: string; seq: number; content: string; done?: boolean; now: number }): {
  chunk: streamchunk;
  frame: jsonrpcframe;
} {
...
export function progressof(input: {
  ...
  cancellable?: boolean;
  now: number;
}): { notice: progressnotice; frame: jsonrpcframe } {
...
export function servecallevent(input: {
  client: clientrecord;
  tool: string;
  origin: string;
  ok: boolean;
...
export function auditline(record: toolcallrecord, client: clientrecord | undefined): string {
  const caller =
    client !== undefined
      ? `${client.name ?? client.id}${client.clientversion !== undefined ? ` (${client.clientversion})` : ""}`
      : record.clientid;
...
export function stepdetails(step: toolstep, result: toolresult): toolresult {
  return {
    ...result,
    payload: { ...(result.payload ?? {}), stepid: step.id, kind: step.kind, risk: step.risk, summary: step.summary },
  };
}
...
export async function dispatchcall(input: {
  ...
  const id = input.id;
  const answer = (result: unknown): calloutcome => ({
    response: respond({ ...(id !== undefined ? { id } : {}), result }),
  });
  const fail = (code: rpcerrorcode, message: string): calloutcome => ({
    response: respond({ ...(id !== undefined ? { id } : {}), error: rpcerrorof(code, message) }),
  });
  if (!params || typeof params !== "object" || Array.isArray(params))
    return fail("params", "The tool call needs its params object.");
  ...
  if (input.readonly === true && tool.risk !== "read")
    return fail("consentrefused", `The serve runs degraded to the read only tools and refuses ${name}.`);
  const namespace = namespaceof(tool.name);
  if (input.scopes !== undefined && namespace !== undefined && !input.scopes.includes(namespace))
    return fail("consentrefused", `The session token grants no ${namespace} tools.`);
  const key =
  ...
  if (params.dryrun === true) {
    const dryrun = dryruntool({
      ...
      origin: input.origin,
      ...(typeof params.stepid === "string" ? { stepid: params.stepid } : {}),
      now: input.now,
    });
    ...
    const record = servecallevent({
      ...
      ok,
      now: input.now,
      ...(ok ? {} : { code: "consentrefused" as rpcerrorcode }),
      dryrun: true,
    });
    return {
      response: respond({
        ...(id !== undefined ? { id } : {}),
        result: { content: JSON.stringify(dryrun), iserror: !ok },
      }),
      record,
    };
  }
  if (input.limits !== undefined && input.limits.length > 0) {
    ...
    if (!limited.allowed)
      return fail(
        "params",
        `The client exhausted its call budget of ${String(limited.budget ?? 0)} for the ${String(limited.limit?.windowms ?? 0)} millisecond window; retry after ${String(limited.retryafter ?? 0)} milliseconds.`,
      );
  }
  if (input.client.fingerprint !== undefined && input.allowlist !== undefined && input.allowlist.length > 0) {
    const admitted = checkallowlist({
      entries: input.allowlist,
      fingerprint: input.client.fingerprint,
      ...(namespace !== undefined ? { namespace } : {}),
    });
    if (!admitted.allowed)
      return fail("consentrefused", admitted.reason ?? "The client fingerprint stays outside the allowlist.");
  }
  ...
  if (tool.risk !== "read" && tool.consentmeta?.approvalrequired === true) {
    const approval = requireapproval({
      clientid: input.client.id,
      tool: tool.name,
      reason: tool.consentmeta.review,
      params,
      now: input.now,
      ...(input.approvaltimeout !== undefined ? { timeout: input.approvaltimeout } : {}),
    });
    return {
      blocked: true,
      approval,
      approvals: [...(input.approvals ?? []).filter((candidate) => candidate.id !== approval.id), approval],
    };
  }
  const step =
    tool.risk === "read"
      ? {
          id: `mcp-${input.client.id}-${input.now}`,
          kind: tool.kind,
          summary: tool.description.split(".")[0] ?? tool.description,
          risk: "read" as const,
          ...(typeof params.target === "string" ? { target: params.target } : {}),
          ...(typeof params.value === "string" ? { value: params.value } : {}),
          ...(params.options !== undefined && typeof params.options === "object" && !Array.isArray(params.options)
  ...
  try {
    ...
    return {
      response: respond({
        ...(id !== undefined ? { id } : {}),
        error: rpcerrorof("internal", error instanceof Error ? error.message : String(error)),
      }),
      contexts: closed.contexts,
      record: servecallevent({
        client: input.client,
        tool: tool.name,
        origin: input.origin,
        ok: false,
  ...
  const records =
    key !== undefined
      ? recordidempotency({
          ...
          result: detailed,
          now: input.now,
          ...(input.idempotencywindow !== undefined ? { window: input.idempotencywindow } : {}),
        })
      : input.idempotency;
...
export async function resumegatedcall(input: {
  approval: approvalrequest;
  client: clientrecord;
  catalog: toolcatalog;
  session?: agentsession;
  ...
  if (input.approval.state !== "approved") {
    return {
      response: respond({
        error: rpcerrorof(
          "consentrefused",
          `The approval gate ${input.approval.id} answered ${input.approval.state} and the call never executes.`,
        ),
      }),
      record: servecallevent({
        client: input.client,
  ...
  if (tool === undefined)
    return {
      response: respond({
        error: rpcerrorof("params", `The catalog holds no unambiguous tool named ${input.approval.tool}.`),
      }),
    };
  ...
  if (!gate.allowed) {
    return {
      response: respond({
        error: rpcerrorof("consentrefused", gate.reason ?? "The consent gates refused the tool call."),
      }),
      record: servecallevent({
        client: input.client,
        tool: tool.name,
  ...
  if (step === undefined)
    return {
      response: respond({
        error: rpcerrorof("consentrefused", "The approved gate names no step the approved plan carries."),
      }),
    };
  const result = await input.execute(step);
  return {
    response: respond({
      ...(input.approval.params.id !== undefined ? { id: input.approval.params.id as number | string } : {}),
      result: stepdetails(step, result),
    }),
    record: servecallevent({
      client: input.client,
      tool: tool.name,
...
export async function runcallsbatch(input: {
...
}): Promise<{ batch: batchcall; outcomes: batchoutcome[] }> {
  const batchid = `batch-${input.now}-${randomid().slice(0, 6)}`;
  const batch: batchcall = {
    id: batchid,
    clientid: input.client.id,
    calls: input.calls.map((call) => ({ id: call.id, name: call.name, params: call.params })),
...
export async function routeserveframe(input: {
  frame: jsonrpcframe;
  state: servesession;
  now: number;
  execute: (step: toolstep) => toolresult | Promise<toolresult>;
}): Promise<{ response?: jsonrpcframe; notifications?: jsonrpcframe[]; state: servesession }> {
  const frame = input.frame;
  const state = input.state;
  const id = frame.id;
  const params = frame.params;
  const answer = (result: unknown): { response: jsonrpcframe; state: servesession } => ({
    response: respond({ ...(id !== undefined ? { id } : {}), result }),
    state,
  });
  const fail = (error: rpcerror): { response: jsonrpcframe; state: servesession } => ({
    response: respond({ ...(id !== undefined ? { id } : {}), error }),
    state,
  });
  ...
  if (entry.handler === "initialize") {
    const handshake = serveinitialize({
    ...
    return {
      response: respond({
        ...(id !== undefined ? { id } : {}),
        result: {
          serverinfo: handshake.handshake.serverinfo,
          protocolversion: handshake.handshake.protocolversion,
          servercontractversion: handshake.handshake.servercontractversion,
          instructions: handshake.handshake.instructions,
        },
      }),
      state: { ...state, client: handshake.client(state.client) },
    };
  }
  if (entry.handler === "ping") return answer(ping({ now: input.now }));
  if (entry.handler === "health")
    return answer(healthof({ startedat: state.startedat, now: input.now, state: "running" }));
  if (entry.handler === "listtools") {
    const binding = state.bindings.find(
      (candidate) => candidate.clientid === state.client.id && candidate.releasedat === undefined,
    );
    const degraded = binding?.readonly === true;
    const scopes = state.client.capabilities?.namespaces;
    const exposed = exposedtools(state.catalog, {
      ...(degraded ? { readonly: true } : {}),
  ...
  if (entry.handler === "negotiate") {
    ...
    const clientcaps =
      params?.capabilities && typeof params.capabilities === "object" && !Array.isArray(params.capabilities)
        ? (params.capabilities as Partial<capabilityset>)
        : undefined;
    const outcome = negotiate({ ...(clientcaps !== undefined ? { client: clientcaps } : {}), server });
    return outcome.agreed
      ? answer(outcome.capabilities)
      : fail(rpcerrorof("params", outcome.mismatch ?? "The capability negotiation did not agree."));
  }
  if (entry.handler === "listresources") {
    const gate = resourceexposuregate({
      client: state.client,
      uri: "devthink://health",
      served: servedresources().map((resource) => resource.uri),
    });
  ...
  if (entry.handler === "readresource") {
    ...
    if (!gate.allowed) return fail(rpcerrorof("consentrefused", gate.reason ?? "The resource exposure was refused."));
    const read = readresource({
      uri,
      ...(state.pagestate !== undefined ? { pagestate: state.pagestate } : {}),
      ...(state.plan !== undefined ? { plan: state.plan } : {}),
      ...(state.audit !== undefined ? { audit: state.audit } : {}),
      ...(state.session !== undefined ? { session: state.session } : {}),
      health: healthof({ startedat: state.startedat, now: input.now, state: "running" }),
    });
    if (read.resource === undefined || read.text === undefined)
      return fail(rpcerrorof("params", read.reason ?? "The resource read failed."));
  ...
  if (entry.handler === "subscriberesource") {
    ...
    if (!gate.allowed) return fail(rpcerrorof("consentrefused", gate.reason ?? "The resource exposure was refused."));
    const subscribed = subscriberesource({
      clientid: state.client.id,
      uri,
      ...(state.pagestate !== undefined && uri === "devthink://pagestate" ? { state: state.pagestate } : {}),
      now: input.now,
    });
    if (subscribed.watch === undefined)
      return fail(rpcerrorof("params", subscribed.reason ?? "The resource subscription did not open."));
    return {
      response: respond({ ...(id !== undefined ? { id } : {}), result: { watchid: subscribed.watch.id, uri } }),
      state: { ...state, watches: [subscribed.watch, ...state.watches] },
    };
  }
  if (entry.handler === "unsubscriberesource") {
    ...
    return {
      response: respond({ ...(id !== undefined ? { id } : {}), result: { watchid, canceled: true } }),
      state: {
        ...state,
        watches: state.watches.map((watch) =>
          watch.id === watchid && watch.canceledat === undefined ? { ...watch, canceledat: input.now } : watch,
        ),
      },
    };
  }
  ...
  if (entry.handler === "callprompt") {
    ...
    if (rendered.rendered === undefined || rendered.prompt === undefined)
      return fail(rpcerrorof("params", rendered.reason ?? "The prompt call did not render."));
  ...
  if (entry.handler === "cancel") {
    ...
    if (aborted.context === undefined)
      return fail(rpcerrorof("params", aborted.reason ?? "The cancellation frame named no in flight tool call."));
    return {
      response: respond({
        ...(id !== undefined ? { id } : {}),
        result: {
          cancelled: true,
          callid,
          ...(aborted.context.partial !== undefined ? { partial: aborted.context.partial } : {}),
        },
      }),
      state: { ...state, contexts: aborted.contexts },
    };
  }
  if (entry.handler === "batch") {
    const rawcalls =
      params !== undefined && Array.isArray(params.calls) ? (params.calls as Array<Record<string, unknown>>) : [];
    const calls = rawcalls
      .filter((call) => call !== null && typeof call === "object" && typeof call.name === "string")
      .map((call, index) => ({
        id: typeof call.id === "string" ? call.id : `member-${index + 1}`,
        name: call.name as string,
        params:
          call.params !== undefined && typeof call.params === "object" && !Array.isArray(call.params)
    ...
    const run = await runcallsbatch({
      calls,
      client: state.client,
      now: input.now,
      stoponerror: params?.stoponerror !== false,
      execute: async (call) => {
        const outcome = await dispatchcall({
          params: { ...call.params, name: call.name },
          ...(id !== undefined ? { id } : {}),
          client: state.client,
          catalog: state.catalog,
          ...(state.session !== undefined ? { session: state.session } : {}),
          ...
          origin: state.origin,
          now: input.now,
          ...(state.mocks.length > 0 ? { mocks: state.mocks } : {}),
          ...(state.idempotency.length > 0 ? { idempotency: state.idempotency } : {}),
          ...(state.limits.length > 0 ? { limits: state.limits } : {}),
          ...(state.allowlist.length > 0 ? { allowlist: state.allowlist } : {}),
        ...
        if (outcome.response?.error !== undefined)
          return {
            ok: false,
            error: { code: outcome.response.error.code, message: outcome.response.error.message, retryhint: "none" },
          };
        const result = outcome.response?.result as toolresult | undefined;
        return result !== undefined
          ? { ok: !result.iserror, result }
          : {
              ok: false,
              error: { code: "internal", message: "The batch member produced no result.", retryhint: "none" },
            };
      },
    });
  ...
  const outcome = await dispatchcall({
    ...
    origin: state.origin,
    now: input.now,
    ...(state.bindings.find((binding) => binding.clientid === state.client.id && binding.releasedat === undefined)
      ?.readonly === true
      ? { readonly: true }
      : {}),
    ...(state.mocks.length > 0 ? { mocks: state.mocks } : {}),
    ...
    ...(state.allowlist.length > 0 ? { allowlist: state.allowlist } : {}),
    approvals: state.approvals,
    ...(state.config.remoteaccess?.approvaltimeout?.windowms !== undefined
      ? { approvaltimeout: state.config.remoteaccess.approvaltimeout.windowms }
      : {}),
    contexts: state.contexts,
    execute: input.execute,
  });
  return {
    ...
    state: {
      ...state,
      ...(outcome.approvals !== undefined ? { approvals: outcome.approvals } : {}),
      ...(outcome.contexts !== undefined ? { contexts: outcome.contexts } : {}),
      ...(outcome.records !== undefined ? { idempotency: outcome.records } : {}),
    },
  };
}
...
export function publishresourcechange(input: {
  state: servesession;
  uri: string;
  changed: Record<string, unknown>;
  now: number;
}): { notifications: jsonrpcframe[]; state: servesession } {
  const published = publishchange({
    watches: input.state.watches,
    uri: input.uri,
    state: input.changed,
    now: input.now,
  });
  return {
    notifications: published.deliveries.map((delivery) => delivery.frame),
    state: { ...input.state, watches: published.watches },
  };
}
...
export function createservesession(input: {
  config: mcpserverconfig;
  client: clientrecord;
  templates?: prompttemplate[];
  session?: agentsession;
  ...
  return {
    config: input.config,
    catalog: input.catalog ?? buildtoolcatalog(),
...
export const rpcerrornumbers: Record<rpcerrorcode, number> = {
  parse: -32700,
  method: -32601,
  params: -32602,
  internal: -32603,
  consentrefused: -32001,
};
...
export function rpcerrorcodeof(number: number): rpcerrorcode | undefined {
  const entry = (Object.entries(rpcerrornumbers) as Array<[rpcerrorcode, number]>).find(
    ([, value]) => value === number,
  );
  return entry?.[0];
}
...
export function parseframe(raw: string): jsonrpcframe {
  ...
  if (!parsed || typeof parsed !== "object" || Array.isArray(parsed))
    throw new Error("A json rpc frame must be an object.");
...
export function validateframe(
  ...
  if (frame.id !== undefined && typeof frame.id !== "number" && typeof frame.id !== "string" && frame.id !== null)
    return rpcerrorof("parse", "The frame id must be a number, a string or null.");
  if (frame.method === undefined || frame.method.trim() === "")
    return rpcerrorof("method", "The frame carries no method to route.");
  if (!methods.some((entry) => entry.method === frame.method))
    return rpcerrorof("method", `The server routes no method named ${frame.method}.`);
  if (frame.params !== undefined && (typeof frame.params !== "object" || Array.isArray(frame.params)))
    return rpcerrorof("params", "The frame params must be an object.");
  if (config?.framesize !== undefined && serializeframe(frame).length > config.framesize)
    return rpcerrorof(
      "params",
      `The serialized frame exceeds the user configured frame size of ${config.framesize} characters.`,
    );
  return undefined;
}
...
export function respond(input: { id?: number | string | null; result?: unknown; error?: rpcerror }): jsonrpcframe {
  return {
    jsonrpc: "2.0",
    ...(input.id === undefined ? (input.error !== undefined ? { id: null } : {}) : { id: input.id }),
  ...
  return [
    {
      method: "initialize",
      handler: "initialize",
      description: "Completes the mcp handshake and returns the server info.",
    },
    { method: "ping", handler: "ping", description: "Answers keepalive frames with pong." },
    {
      method: "tools/list",
      handler: "listtools",
      description: "Returns every tool with its version and json schema inputs.",
    },
    { method: "negotiate", handler: "negotiate", description: "Exchanges capability sets with the client." },
    { method: "tools/call", handler: "dispatch", description: "Invokes one tool behind the consent gates." },
    {
      method: "prompts/list",
      handler: "listprompts",
      description: "Lists the prompt defs the server exposes as callable tools.",
    },
    {
      method: "prompts/call",
      handler: "callprompt",
      description: "Renders one prompt and returns its arguments as a tool call.",
    },
    {
      method: "calls/cancel",
      handler: "cancel",
...
export function servercapabilities(input: { config: mcpserverconfig; catalog: toolcatalog }): capabilityset {
  return {
    ...
    toolversion: input.catalog.version,
    tools: alltools(input.catalog).length,
    namespaces: toolnamespaces,
    transports: input.config.transports,
  };
}
...
export function initialize(input: {
  ...
  config: mcpserverconfig;
  catalog: toolcatalog;
}): { serverinfo: capabilityset; protocolversion: string; protocolmajor: number; instructions: string } {
  void input.params;
  return {
    serverinfo: servercapabilities({ config: input.config, catalog: input.catalog }),
    protocolversion,
    protocolmajor: protocolmajorversion,
    instructions:
      "Devthink serves browser tools behind the human review gates: read only tools run once a session is approved while every tool with side effects executes exactly the approved plan step it names. No endpoint, provider or key is hardcoded; the user pairs every client.",
  };
}
...
export function listtools(catalog: toolcatalog): {
  tools: Array<{
    name: string;
    version: number;
    description: string;
    ...
    risk: string;
    consentmeta?: { review: string; riskclass: string; approvalrequired: boolean; originscope: string };
  }>;
} {
  return {
    tools: alltools(catalog).map((tool) => ({
      name: tool.name,
      ...
      ...(tool.consentmeta !== undefined
        ? {
            consentmeta: {
              review: tool.consentmeta.review,
              riskclass: tool.consentmeta.riskclass ?? tool.risk,
              approvalrequired: tool.consentmeta.approvalrequired ?? true,
              originscope: tool.consentmeta.originscope ?? "session",
            },
          }
        : {}),
    })),
...
export function unknownframefields(frame: jsonrpcframe): string[] {
  return Object.keys(frame as unknown as Record<string, unknown>).filter(
    (key) =>
      key !== "jsonrpc" && key !== "id" && key !== "method" && key !== "params" && key !== "result" && key !== "error",
  );
}
...
export function negotiate(input: { client?: Partial<capabilityset>; server: capabilityset }): {
  agreed: boolean;
  mismatch?: string;
  capabilities?: capabilityset;
  ...
  const negotiation = negotiateprotocol({
    ...(client?.protocolmajor !== undefined ? { client: client.protocolmajor } : {}),
  });
  if (!negotiation.agreed)
    return { agreed: false, ...(negotiation.reason !== undefined ? { mismatch: negotiation.reason } : {}) };
  if (client?.toolversion !== undefined && client.toolversion > input.server.toolversion)
    return {
      agreed: false,
      mismatch: `The client requires tool version ${String(client.toolversion)} while the server offers ${String(input.server.toolversion)}.`,
    };
  if (
    client?.transports !== undefined &&
    client.transports.some((transport) => !input.server.transports.includes(transport))
  )
    return { agreed: false, mismatch: "The client requires a transport the server configuration does not allow." };
  return {
    agreed: true,
    capabilities: input.server,
    ...(negotiation.major !== undefined ? { protocolmajor: negotiation.major } : {}),
    ...(negotiation.deprecation !== undefined ? { deprecation: negotiation.deprecation } : {}),
  };
}
...
export function pairclient(clients: clientrecord[], id: string, approved: boolean, now: number): clientrecord[] {
  return clients.map((client) =>
    client.id !== id || client.disconnectedat !== undefined
      ? client
      : approved
        ? { ...client, paired: true, pairedat: now }
        : { ...client, paired: false, disconnectedat: now },
  );
}
...
export function disconnectclient(clients: clientrecord[], id: string, now: number): clientrecord[] {
  return clients.map((client) =>
    client.id === id && client.disconnectedat === undefined ? { ...client, disconnectedat: now } : client,
  );
}
...
export function enqueuerequest(input: {
  queue: jsonrpcframe[];
  frame: jsonrpcframe;
  depth?: number;
...
export function negotiatetoolfloor(
  clientfloor: number | undefined,
  catalogversion: number,
): { floor: number } | { mismatch: string } {
  if (clientfloor === undefined) return { floor: catalogversion };
  if (clientfloor > catalogversion)
    return {
      mismatch: `The client requires the tool version floor ${clientfloor} while the catalog serves version ${catalogversion}.`,
    };
...
export async function dispatchtool(input: {
  ...
  scopes?: toolnamespace[];
  execute: (step: toolstep) => toolresult | Promise<toolresult>;
}): Promise<{ result?: toolresult; error?: rpcerror; step?: toolstep }> {
  const params = input.params;
  if (!params || typeof params !== "object" || Array.isArray(params))
    return { error: rpcerrorof("params", "The tool call needs its params object.") };
  if (typeof params.name !== "string" || !params.name.trim())
    return { error: rpcerrorof("params", "The tool call needs the namespaced name of the tool it invokes.") };
  const tool = resolvetool(input.catalog, params.name.trim());
  if (tool === undefined)
    return { error: rpcerrorof("params", `The catalog holds no unambiguous tool named ${params.name.trim()}.`) };
  const namespace = namespaceof(tool.name);
  if (namespace === undefined)
    return { error: rpcerrorof("params", `The tool ${tool.name} carries no reviewed namespace.`) };
  if (input.scopes !== undefined && !input.scopes.includes(namespace))
    return { error: rpcerrorof("consentrefused", `The session token grants no ${namespace} tools.`) };
  const floor = input.client.toolfloor ?? input.client.capabilities?.toolversion ?? input.catalog.version;
  if (tool.version < floor)
    return {
      error: rpcerrorof(
        "params",
        `The tool ${tool.name} of version ${tool.version} stays below the negotiated compatibility floor of ${floor}.`,
      ),
    };
  const stepid = typeof params.stepid === "string" ? params.stepid : undefined;
  const gate = tooldispatchgate({
  ...
  if (!gate.allowed)
    return { error: rpcerrorof("consentrefused", gate.reason ?? "The consent gates refused the tool call.") };
  const step =
    tool.risk === "read"
      ? {
          id: `mcp-${input.client.id}-${input.now}`,
      ...
      : input.plan?.steps.find((candidate) => candidate.id === stepid);
  if (step === undefined)
    return { error: rpcerrorof("consentrefused", "The tool call names a step the approved plan does not carry.") };
  try {
    const result = await input.execute(step);
    return { result, step };
  } catch (error) {
...
export async function handleframe(input: {
  raw?: string;
  frame?: jsonrpcframe;
  client: clientrecord;
  catalog: toolcatalog;
  ...
  if (input.raw !== undefined && input.config.framesize !== undefined && input.raw.length > input.config.framesize)
    return respond({
      id: null,
      error: rpcerrorof(
        "params",
        `The wire frame exceeds the user configured frame size of ${input.config.framesize} characters.`,
      ),
    });
  ...
  if (unknown.length > 0) {
    ...
    if (!strictness.allowed)
      return respond({
        ...(frame.id !== undefined ? { id: frame.id } : {}),
        error: rpcerrorof(
          "params",
          strictness.reason ?? "The frame carries unknown fields the frozen schema declares nowhere.",
        ),
      });
  }
  ...
  if (entry === undefined)
    return respond({
      ...(frame.id !== undefined ? { id: frame.id } : {}),
      error: rpcerrorof("method", `The server routes no method named ${String(frame.method)}.`),
    });
  const params = frame.params;
  if (entry.handler === "initialize")
    return respond({
      ...(frame.id !== undefined ? { id: frame.id } : {}),
      result: initialize({ ...(params !== undefined ? { params } : {}), config: input.config, catalog: input.catalog }),
    });
  if (entry.handler === "ping")
    return respond({ ...(frame.id !== undefined ? { id: frame.id } : {}), result: ping({ now: input.now }) });
  if (entry.handler === "listtools")
    return respond({ ...(frame.id !== undefined ? { id: frame.id } : {}), result: listtools(input.catalog) });
  if (entry.handler === "negotiate") {
    ...
    return respond({
      ...
      ...(outcome.agreed
        ...
        : { error: rpcerrorof("params", outcome.mismatch ?? "The capability negotiation did not agree.") }),
    });
  }
  if (entry.handler === "listprompts")
    return respond({ ...(frame.id !== undefined ? { id: frame.id } : {}), result: { prompts: listprompts() } });
  if (entry.handler === "callprompt") {
    const name = typeof params?.name === "string" ? params.name : "";
    const args =
      params?.arguments && typeof params.arguments === "object" && !Array.isArray(params.arguments)
    ...
    const called =
      name === ""
        ? { reason: "The prompt call needs the prompt name." }
        : callprompt({ name, ...(args !== undefined ? { args } : {}) });
    if (called.toolcall === undefined)
      return respond({
        ...(frame.id !== undefined ? { id: frame.id } : {}),
        error: rpcerrorof("params", called.reason ?? "The prompt call did not render."),
      });
    return respond({
      ...(frame.id !== undefined ? { id: frame.id } : {}),
      result: { toolcall: called.toolcall, rendered: called.rendered },
    });
  }
  if (entry.handler === "cancel") {
    const callid = typeof params?.callid === "string" ? params.callid : "";
    ...
    if (callid.trim() === "")
      return respond({
        ...(frame.id !== undefined ? { id: frame.id } : {}),
        error: rpcerrorof("params", "The cancellation frame needs the call id it aborts."),
      });
    const aborted = canceltool({
    ...
    if (aborted.context === undefined)
      return respond({
        ...(frame.id !== undefined ? { id: frame.id } : {}),
        error: rpcerrorof("params", aborted.reason ?? "The cancellation frame named no in flight tool call."),
      });
    return respond({
      ...(frame.id !== undefined ? { id: frame.id } : {}),
      result: {
  ...
  return respond({
    ...(frame.id !== undefined ? { id: frame.id } : {}),
    ...(dispatched.error !== undefined ? { error: dispatched.error } : { result: dispatched.result }),
  });
}
...
export function launchbridge(input: {
  host: string;
  pid?: number;
  now: number;
  id?: string;
  ...
  if (input.signature !== undefined && input.verifier !== undefined && !input.verifier(input.host, input.signature))
    throw new Error(
      `The client signature of the ${input.host} bridge failed the platform verification; the local stdio bridge refuses the client and no frame relays.`,
    );
  return {
    id: input.id ?? `bridge-${input.now}`,
    host: input.host,
    connected: true,
    ...
    received: 0,
    sent: 0,
    ...(input.signature !== undefined && input.verifier !== undefined
      ? { clientverified: input.verifier(input.host, input.signature) }
      : {}),
  };
}
...
export function relayframe(input: {
  bridge: stdiobridge;
  ...
  return {
    ...input.bridge,
    connected: true,
    received: input.bridge.received + (input.direction === "inbound" ? 1 : 0),
    sent: input.bridge.sent + (input.direction === "outbound" ? 1 : 0),
...
export function restartbridge(input: { bridge: stdiobridge; pid: number; now: number }): stdiobridge {
  return {
    ...
    connected: true,
    pid: input.pid,
    restarts: input.bridge.restarts + 1,
    startedat: input.now,
  };
}
