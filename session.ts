...
export const sessionkinds: string[] = [
  "persiststate",
  "capturesession",
  "restoresession",
  "namedsessions",
...
export function taskstatechecksum(runid: string, stepcursor: number, outputs: stepoutcome[]): string {
  return checksumtext(
    `${runid}:${stepcursor}:${outputs.length}:${outputs.map((output) => `${output.stepid}:${output.ok}:${output.summary.length}`).join("|")}`,
  );
}

/** Builds one persisted task state checkpoint of a run with its checksum. */
export function taskstateof(input: {
  runid: string;
  stepcursor: number;
  outputs: stepoutcome[];
  checkpointat: number;
  ...
  return {
    runid: input.runid,
    stepcursor: input.stepcursor,
    outputs: input.outputs,
    checkpointat: input.checkpointat,
    checksum: taskstatechecksum(input.runid, input.stepcursor, input.outputs),
  };
}
...
export function autointervalof(value: unknown): autointerval | undefined {
  if (!value || typeof value !== "object" || Array.isArray(value)) return undefined;
  const candidate = value as Record<string, unknown>;
  if (typeof candidate.period !== "number" || !Number.isFinite(candidate.period) || candidate.period <= 0)
    return undefined;
  if (
    typeof candidate.maxsnapshots !== "number" ||
    !Number.isInteger(candidate.maxsnapshots) ||
    candidate.maxsnapshots < 1
  )
    return undefined;
  if (typeof candidate.expiry !== "number" || !Number.isFinite(candidate.expiry) || candidate.expiry < 0)
    return undefined;
...
export function snapshotplanof(value: unknown): snapshotplan | undefined {
  ...
  const sections = Array.isArray(candidate.sections)
    ? candidate.sections.flatMap((section) =>
        typeof section === "string" && snapshotsections.includes(section as snapshotscope)
          ? [section as snapshotscope]
          : [],
      )
    : [];
...
export function searchqueryof(value: unknown): searchquery | undefined {
  ...
  const terms = Array.isArray(candidate.terms)
    ? candidate.terms.flatMap((term) => (typeof term === "string" && term.trim() ? [term.trim()] : []))
    : [];
  if (terms.length === 0) return undefined;
  const fields = Array.isArray(candidate.fields)
    ? candidate.fields.flatMap((field) =>
        typeof field === "string" && searchfields.includes(field as searchfield) ? [field as searchfield] : [],
      )
    : [...searchfields];
...
export function sessionfolderof(value: unknown): sessionfolder | undefined {
  ...
  const tags = Array.isArray(candidate.tags)
    ? candidate.tags.flatMap((tag) => (typeof tag === "string" && tag.trim() ? [tag] : []))
    : [];
...
export function diffsessionrecords(left: sessionrecord, right: sessionrecord): sessionchange[] {
  ...
  for (const tab of right.tabs) {
    ...
    if (prior.url !== tab.url)
      changes.push({
        class: "changed",
        subject: "url",
        detail: `Tab ${tab.index} moved from ${prior.url} to ${tab.url}`,
      });
    if (prior.title !== tab.title)
      changes.push({
        class: "changed",
        subject: "tab",
        detail: `Tab ${tab.index} title changed from "${prior.title}" to "${tab.title}"`,
      });
    ...
    for (const form of tab.forms) {
      ...
      if (before === undefined) {
        changes.push({
          class: "added",
          subject: "form",
          detail: `Form field ${form.selector} of tab ${tab.index} added with a value`,
        });
        continue;
      }
      if (before !== form.value)
        changes.push({
          class: "changed",
          subject: "form",
          detail: `Form field ${form.selector} of tab ${tab.index} changed its captured value`,
        });
    }
    for (const form of prior.forms)
      if (!tab.forms.some((entry) => entry.selector === form.selector))
        changes.push({
          class: "removed",
          subject: "form",
          detail: `Form field ${form.selector} of tab ${tab.index} removed`,
        });
  }
  for (const tab of left.tabs)
    if (!rightbyindex.has(tab.index))
      changes.push({ class: "removed", subject: "tab", detail: `Tab ${tab.index} removed: ${tab.url}` });
  ...
  for (const entry of right.storage) {
    ...
    if (!prior) {
      changes.push({
        class: "added",
        subject: "storage",
        detail: `Local storage of ${entry.origin} captured with ${entry.keys.length} keys`,
      });
      continue;
    }
    if (prior.keys.join("|") !== entry.keys.join("|") || prior.values.join("|") !== entry.values.join("|"))
      changes.push({
        class: "changed",
        subject: "storage",
        detail: `Local storage of ${entry.origin} changed its captured keys or values`,
      });
  }
  for (const entry of left.storage)
    if (!right.storage.some((candidate) => candidate.origin === entry.origin))
      changes.push({
        class: "removed",
        subject: "storage",
        detail: `Local storage of ${entry.origin} left the capture`,
      });
  return changes;
}

/** Builds one stored session diff result of two compared records. */
export function newsessiondiff(input: {
  id: string;
  left: sessionrecord;
  right: sessionrecord;
  at: number;
  ...
  return {
    ...
    changes: diffsessionrecords(input.left, input.right),
    at: input.at,
  };
}
  ...
  for (const record of records) {
    ...
    for (const haystack of haystacks) {
      ...
      for (const term of query.terms) {
        ...
        matches.push({
          sessionid: record.id,
          field: haystack.field,
          term,
          at: record.createdat,
          excerpt: haystack.text.slice(start, start + 80).trim(),
        });
      }
    }
  }
...
export function exportsessionfile(records: sessionrecord[], now: number): sessionfile {
  ...
  return {
    formatversion: sessionfileversion,
    records,
    recordids,
    bytesize: payload.length,
    checksum: checksumtext(`${sessionfileversion}:${recordids.join(",")}:${payload.length}`),
    exportedat: now,
  };
}
...
export function importsessionfile(value: unknown): sessionfile | undefined {
  ...
  const records = Array.isArray(candidate.records)
    ? candidate.records.flatMap((record) => (sessionrecordvalid(record) ? [record] : []))
    : [];
  if (records.length === 0) return undefined;
  if (
    !Array.isArray(candidate.recordids) ||
    candidate.recordids.length !== records.length ||
    !candidate.recordids.every((id, index) => id === records[index]?.id)
  )
    return undefined;
  const bytesize =
    typeof candidate.bytesize === "number" && Number.isFinite(candidate.bytesize) ? candidate.bytesize : -1;
  ...
  return {
    formatversion: sessionfileversion,
    records,
    recordids: candidate.recordids as string[],
    bytesize,
    checksum,
    exportedat:
      typeof candidate.exportedat === "number" && Number.isFinite(candidate.exportedat) ? candidate.exportedat : 0,
  };
}
...
export function expiresessions(records: sessionrecord[], retention: number | undefined, now: number): sessionrecord[] {
  ...
  return records.map((record) => {
    ...
    return {
      ...
      ...(record.restoredat !== undefined ? { restoredat: record.restoredat } : {}),
      sectionsexpired: true,
    };
  });
}
...
export function filteredsessions(
  ...
  filter: { name?: string; folder?: string; from?: number; to?: number },
): sessionrecord[] {
...
export function sealnotebody(id: string, body: string): string {
  const sealed = Array.from(body, (character, index) =>
    String.fromCharCode(character.charCodeAt(0) ^ keystreambyte(id, index)),
  ).join("");
...
export function opennotebody(id: string, sealedbody: string): string {
  ...
  try {
    ...
    return Array.from(sealed, (character, index) =>
      String.fromCharCode(character.charCodeAt(0) ^ keystreambyte(id, index)),
    ).join("");
  } catch {
    return "";
...
export function sitenoteof(input: {
  origin: string;
  title: string;
  body: string;
  author: string;
  ...
  if (input.sensitive === true)
    return {
      ...
      sealedbody: sealnotebody(id, input.body),
      author: input.author,
      sensitive: true,
      createdat: input.now,
      updatedat: input.now,
...
export function editnote(
  note: sitenote,
  input: { title: string; body: string; author: string; now: number },
): sitenote {
  ...
  if (note.sensitive)
    return {
      ...note,
      title: input.title.trim(),
      sealedbody: sealnotebody(note.id, input.body),
      updatedat: input.now,
      author: input.author,
    };
...
export function scratchentryof(input: {
  taskid: string;
  sessionid: string;
  text: string;
  stepid?: string;
  ...
  return {
    ...
    text: input.text,
    ...(input.stepid !== undefined && input.stepid.trim() !== "" ? { stepid: input.stepid } : {}),
    author: input.author,
    at: input.now,
  };
}
...
export function prunescratchpad(
...
export function distillrunsummary(input: {
  plan: agentplan;
  ...
  const windowed =
    input.window !== undefined && Number.isInteger(input.window) && input.window >= 0
      ? steps.slice(Math.max(0, steps.length - input.window))
      : steps;
...
export function summaryhistoryentry(summary: runsummary): historyindexentry {
  return {
    source: "summary",
    id: summary.runid,
    title: `Run summary of ${summary.runid}`,
    text: `${summary.origins.join(" ")} ${summary.kinds.join(" ")} ${summary.steps.map((step) => step.summary).join(" ")}`,
    outcome: summary.steps.every((step) => step.ok) ? "completed" : "failed",
    at: summary.distilledat,
  };
}
...
export function notehistoryentry(note: sitenote): historyindexentry {
  return {
    ...
    title: note.title,
    text: note.sensitive ? note.title : `${note.title} ${note.body ?? ""}`,
    at: note.updatedat,
  };
}
...
export function recallentryof(input: {
  origin: string;
  runid: string;
  stepid: string;
  text: string;
  ...
  if (input.stepid.trim() === "" || input.runid.trim() === "")
    throw new Error("The recall index entry needs its run and step provenance.");
  const normalized = input.text.trim().replace(/\s+/g, " ");
  return {
    fingerprint: textfingerprint(normalized),
    origin: input.origin,
    runid: input.runid,
    stepid: input.stepid,
    text: normalized,
...
export function rankrecall(index: recallindexentry[], query: recallquery, scope: { origins: string[] }): recallmatch[] {
  ...
  for (const entry of index) {
    ...
    matches.push({
      entry,
      score,
      reason: `The extraction of ${entry.origin} shares ${shared} term${shared === 1 ? "" : "s"} with the query at the score ${score.toFixed(3)}; the match carries the run ${entry.runid} and the step ${entry.stepid}.`,
    });
  }
  const ranked = matches.sort((one, two) => two.score - one.score);
  return query.limit !== undefined && Number.isInteger(query.limit) && query.limit >= 0
    ? ranked.slice(0, query.limit)
    : ranked;
}
...
export function expirerecallindex(
...
export function editedcorrectionof(input: {
  origin: string;
  kind: string;
  stepid: string;
  original: string;
  ...
  if (input.stepid.trim() === "" || input.kind.trim() === "")
    throw new Error("The correction needs its step and kind.");
...
export function rejectedcorrectionof(input: {
  origin: string;
  kind: string;
  stepid: string;
  original: string;
  ...
  if (input.stepid.trim() === "" || input.reason.trim() === "")
    throw new Error("The rejected correction needs its step and its rejection reason.");
  return {
    id: input.id ?? randomid(),
    origin: input.origin,
    kind: input.kind,
...
export function matchingcorrections(
  corrections: correctionentry[],
  proposal: { origin: string; kind: string },
): correctionentry[] {
...
export function consentmemoryof(input: {
  origin: string;
...
export function consentadvisoryverdict(
  entries: consentmemoryentry[],
  origin: string,
  kind: string,
): { advisory: boolean; reason: string } {
  const matching = entries.filter((entry) => entry.origin === origin && entry.kinds.includes(kind));
  const latest = matching[matching.length - 1];
  if (latest === undefined)
    return {
      advisory: false,
      reason: `No prior decision exists for the ${kind} kind on ${origin}; the prompt opens fresh.`,
    };
  if (latest.decision === "deny")
    return {
      advisory: true,
      reason: `The consent memory holds a prior denial of the ${kind} kind on ${origin} from ${new Date(latest.at).toISOString()}; the denial carries the same weight as a grant and the record stays advisory only.`,
    };
  return {
    advisory: true,
    reason: `The consent memory holds a prior ${latest.decision} of the ${kind} kind on ${origin} with the boundary ${latest.boundary}; the record advises the new prompt and never auto grants.`,
  };
}
...
export function rollbacksplit(
  plan: agentplan | undefined,
  progress: planprogress | undefined,
): { executedstepids: string[]; queuedstepids: string[] } {
...
export function rollbackof(
  ...
  if (preference === "none")
    return {
      scope: "none",
      label: `Stop the run ${plan?.id ?? ""} without a rollback; the ${split.queuedstepids.length} queued step${split.queuedstepids.length === 1 ? "" : "s"} stay as the run left them.`,
      queuedstepids: split.queuedstepids,
    };
  return {
    scope: "queued",
    label: `Cancel the run ${plan?.id ?? ""} and roll its ${split.queuedstepids.length} queued step${split.queuedstepids.length === 1 ? "" : "s"} back${split.queuedstepids.length > 0 ? ` (${split.queuedstepids.join(", ")})` : ""} while the ${split.executedstepids.length} executed step${split.executedstepids.length === 1 ? "" : "s"} stay untouched in the sealed log.`,
    queuedstepids: split.queuedstepids,
  };
}
...
export function cancelrunactionof(input: {
  runid: string;
  sessionid: string;
  plan: agentplan | undefined;
  progress: planprogress | undefined;
  ...
  return {
    runid: input.runid,
    sessionid: input.sessionid,
    rollback: rollbackof(input.plan, input.progress, input.preference),
  };
}
...
export function errorsurfaceof(input: {
  stepid: string;
  runid: string;
  message: string;
  cause: errorcause;
  ...
  return {
    ...
    retry: { allowed: input.retryallowed, reason: input.retryreason },
...
export function classifyfailure(input: { message: string; policyrefused: boolean; gatewait: boolean }): errorcause {
  ...
  if (/\b(network|offline|timeout|timed out|fetch failed|socket|dns|connection)\b/i.test(input.message))
    return "network";
  return "page";
}
...
export function retryhintof(surface: errorsurface): { allowed: boolean; reason: string } {
  if (!surface.retry.allowed)
    return {
      allowed: false,
      reason: `The ${surface.cause} failure of the step ${surface.stepid} refuses the retry: ${surface.retry.reason}`,
    };
  return {
    allowed: true,
    reason: `The ${surface.cause} failure of the step ${surface.stepid} may retry through a new reviewed dispatch; the retry rides the full consent gate chain and never bypasses the review.`,
  };
}
...
export function sessiongridrows(input: {
  session?: { id: string; tabid: number; origin: string; stoppedat?: number; pausedat?: number; grants?: string[] };
  plan?: agentplan;
  progress?: planprogress;
  logs: storedrunlog[];
  summaries: runsummary[];
  locks: Array<{ runid: string }>;
  ...
  if (input.session && input.plan && ["pending", "approved"].includes(input.plan.state)) {
    ...
    rows.push({
      ...
      origins,
      state: "live",
      outcome: `${split.executedstepids.length} of ${input.plan.steps.length} reviewed steps executed`,
      steps: input.plan.steps.length,
      completed: split.executedstepids.length,
      lock: held ? "held" : "free",
  ...
  for (const log of input.logs) {
    ...
    rows.push({
      ...
      runid: log.runid,
      origins: [...new Set(log.entries.map((entry) => entry.origin).filter((origin) => origin !== ""))],
      state: "saved",
      outcome:
        summary !== undefined
          ? summary.steps.every((step) => step.ok)
            ? "completed"
            : "failed"
          : log.seal !== undefined
            ? "sealed"
            : "open",
      steps: summary?.steps.length ?? log.entries.filter((entry) => entry.kind === "step").length,
      completed:
        summary?.steps.filter((step) => step.ok).length ?? log.entries.filter((entry) => entry.kind === "step").length,
      lock: held ? "held" : "free",
      ...(log.seal !== undefined ? { sealhash: log.seal.sealhash.current } : {}),
      ...(tabsession !== undefined ? { tabid: tabsession.tabid } : {}),
...
export function historyqueryof(value: unknown): historysearchquery | undefined {
  ...
  const origin =
    typeof candidate.origin === "string" && candidate.origin.trim() !== "" ? candidate.origin.trim() : undefined;
  const from = typeof candidate.from === "number" && Number.isFinite(candidate.from) ? candidate.from : undefined;
  const to = typeof candidate.to === "number" && Number.isFinite(candidate.to) ? candidate.to : undefined;
  if (from !== undefined && to !== undefined && from > to) return undefined;
  const outcome =
    typeof candidate.outcome === "string" && candidate.outcome.trim() !== "" ? candidate.outcome.trim() : undefined;
  return {
...
export function emptystatemessage(surface: emptystatesurface, origin?: string): string {
  if (surface === "historysearch")
    return "No history matches yet; start with a first query such as an origin, a note title or a kind the runs executed.";
  if (surface === "sitenotes")
    return `No site note exists${origin !== undefined ? ` for ${origin}` : ""} yet; write the first note with a title and a body and the note flow keeps it per origin with its author provenance.`;
  if (surface === "scratchpad")
    return "The scratchpad holds no entry yet; the agent appends its per task notes here while the reviewed steps run, and every entry carries its step provenance.";
...
export function tabsessionrefof(input: {
  tabid: number;
  sessionid: string;
  runid?: string;
  origin: string;
  ...
  if (!Number.isInteger(input.tabid) || input.tabid < 0)
    throw new Error("The per tab session reference needs its tab.");
...
export function sessionbundleof(input: {
  notes: sitenote[];
  summaries: runsummary[];
  corrections: correctionentry[];
  exportedat: number;
