...
import type { beforeafterpair, capturebundle, capturenamerule, consoletraceentry, diffbaserecord, diffresult, nettraceentry, ocrregion, outputcomparesession, provlogentry, runlogentry, timelapseconfig, timelapseframe, thumbnailrecord } from "./types.js";
...
export type pixeldiff = (baseline: string, capture: string, grid: { cols: number; rows: number }) => Promise<{ width: number; height: number; blocks: number[] }>;
...
export const forensickinds: string[] = ["beforeafter", "consoletimeline", "nettimeline", "diffbase", "diffshot", "thumbshot", "timelapse", "namecaptures", "exportcaptures"];
 ...
 */
export function beforeafter(input: { runid: string; stepid: string; stepkind: string; risk: string; pre?: { id: string; at: number }; post?: { id: string; at: number }; beat?: number; now: number; id?: string }): { pair: beforeafterpair; skipped?: "precapture" | "postcapture"; reason: string } {
  if (input.runid.trim() === "") throw new Error("The beforeafter pair names its run; an unnamespaced pair answers no run.");
  if (input.stepid.trim() === "") throw new Error("The beforeafter pair names its step; the captures link to the stepid they wrap.");
  ...
  if (readonly && input.pre === undefined && input.post !== undefined) return { pair, skipped: "precapture", reason: `The read only ${input.stepkind} step skips its pre capture because a page a read never changed needs no before state; the post capture ${input.post.id} still answers what the step saw.` };
  if (input.pre === undefined && input.post === undefined) return { pair, skipped: "postcapture", reason: `The ${input.stepkind} step captured neither state; the pair records the gap so the forensic view names it instead of inventing a capture.` };
  return { pair, reason: `The ${input.stepkind} step paired its ${pair.precaptureid !== undefined ? "pre and post" : "post"} capture${pair.precaptureid !== undefined && pair.postcaptureid !== undefined ? "s" : ""} under the step ${input.stepid} of the run ${input.runid}.` };
}
 ...
 */
export function consoletimeline(input: { runid: string; entries: Array<{ level?: string; text: string; source?: string; at: number; reload?: number }>; steps: Array<{ id: string; startedat: number; endedat?: number }>; beat?: number; startsequence?: number; now: number }): consoletraceentry[] {
  if (input.runid.trim() === "") throw new Error("The console timeline names its run; an unnamespaced timeline answers no run.");
  ...
  return ordered.map((entry, index) => {
    ...
    const active = input.steps.find(step => entry.at >= step.startedat && (step.endedat === undefined || entry.at <= step.endedat)) ?? input.steps.filter(step => step.startedat <= entry.at).sort((one, two) => two.startedat - one.startedat)[0];
    return {
      id: randomid(),
      runid: input.runid,
      stepid: active?.id ?? "",
 ...
 */
export function nettimeline(input: { runid: string; entries: Array<{ url: string; method: string; status?: number; at: number; correlationid?: string; requestid?: string }>; steps: Array<{ id: string; startedat: number; endedat?: number }>; correlations: Array<{ requestid: string; correlationid: string; responseid?: string; status?: number }>; beat?: number }): nettraceentry[] {
  if (input.runid.trim() === "") throw new Error("The net timeline names its run; an unnamespaced timeline answers no run.");
  const ordered = [...input.entries].sort((one, two) => one.at - two.at);
  return ordered.map(entry => {
    const joined = input.correlations.find(correlation => correlation.correlationid === entry.correlationid || (entry.requestid !== undefined && correlation.requestid === entry.requestid));
 ...
 */
export function diffbase(input: { runid: string; captureid: string; pagestate: string; threshold?: number; beat?: number; now: number; id?: string }): diffbaserecord {
  if (input.runid.trim() === "") throw new Error("The diff baseline names its run; an unnamespaced baseline answers no run.");
  if (input.captureid.trim() === "") throw new Error("The diff baseline names its stored capture; a baseline without pixels flags nothing honestly.");
  if (input.pagestate.trim() === "") throw new Error("The diff baseline names its page state; the label answers which page the baseline froze.");
  if (input.threshold !== undefined && !(input.threshold >= 0 && input.threshold <= 1)) throw new Error("The diff threshold stays a similarity score between zero and one when configured; the bound never defaults in code.");
  return { id: input.id ?? randomid(), runid: input.runid, captureid: input.captureid, pagestate: input.pagestate, ...(input.threshold !== undefined ? { threshold: input.threshold } : {}), ...(input.beat !== undefined ? { beat: input.beat } : {}), at: input.now };
}
 ...
 */
export async function diffshot(input: { baseline: diffbaserecord; capture: { id: string; bytes: string; width: number; height: number }; diff: pixeldiff; blocks?: { cols: number; rows: number }; stepid?: string; beat?: number; now: number; id?: string }): Promise<diffresult & { summary: string }> {
  if (input.capture.id.trim() === "") throw new Error("The diffshot comparison names its capture; a capture without pixels compares nothing.");
  const grid = input.blocks !== undefined ? input.blocks : { cols: Math.max(1, Math.min(12, Math.round(input.capture.width / 64) || 1)), rows: Math.max(1, Math.min(12, Math.round(input.capture.height / 64) || 1)) };
  if (!(grid.cols >= 1 && grid.rows >= 1)) throw new Error("The diff grid needs at least one block column and row; the comparison reads real blocks only.");
  const walked = await input.diff(input.baseline.captureid, input.capture.bytes, grid);
  if (walked.blocks.length !== grid.cols * grid.rows) throw new Error(`The pixel diff seam returned ${walked.blocks.length} block fractions for the reviewed ${grid.cols} by ${grid.rows} grid; the comparison reads exactly the reviewed blocks.`);
  const changedblocks = walked.blocks.map((fraction, index) => ({ index, fraction })).filter(block => block.fraction > 0);
  ...
  for (const block of changedblocks) {
    ...
    if (previous !== undefined && previous.y === region.y && previous.x + previous.width === region.x) previous.width += region.width;
  ...
  return { ...result, summary: `The capture ${input.capture.id} scores ${rounded} against the baseline ${input.baseline.id} of ${input.baseline.pagestate} with ${regions.length} changed region${regions.length === 1 ? "" : "s"}${threshold !== undefined ? ` under the user threshold ${threshold}` : " with no user threshold configured so no regression flags"}; ${regression ? "the visual regression flags" : "the comparison stays inside the reviewed bound"}.` };
}
 ...
 */
export function thumbshot(input: { runid: string; capture: { id: string; width: number; height: number }; edge?: number; now: number; id?: string }): { thumb: thumbnailrecord; width: number; height: number; scale: number; reason: string } {
  if (input.runid.trim() === "") throw new Error("The thumbnail names its run; an unnamespaced thumbnail answers no run.");
  if (input.capture.id.trim() === "") throw new Error("The thumbnail names its capture; a thumbnail without its source links to nothing.");
  if (input.edge !== undefined && !(input.edge >= 1)) throw new Error("The thumbnail edge stays a positive number of pixels when configured; the bound never defaults in code.");
  const source = { width: input.capture.width > 0 ? input.capture.width : 1, height: input.capture.height > 0 ? input.capture.height : 1 };
  const scale = input.edge === undefined ? 1 : Math.min(1, input.edge / Math.max(source.width, source.height));
  const width = Math.max(1, Math.round(source.width * scale));
  const height = Math.max(1, Math.round(source.height * scale));
  const thumb: thumbnailrecord = { id: input.id ?? randomid(), runid: input.runid, captureid: input.capture.id, width, height, at: input.now };
  return { thumb, width, height, scale, reason: input.edge === undefined ? `The thumbnail of ${input.capture.id} keeps its capture size ${source.width} by ${source.height} because no user edge is configured; the bound never defaults in code.` : `The thumbnail of ${input.capture.id} scales to ${width} by ${height} inside the user edge ${input.edge}; the aspect ratio holds.` };
}
 ...
 */
export function timelapse(input: { runid: string; interval: number; duration: number; startedat: number; stoppedat?: number }): { config: timelapseconfig; frames: number; stopsat: number; reason: string } {
  if (input.runid.trim() === "") throw new Error("The timelapse names its run; an unnamespaced lapse answers no run.");
  if (!(input.interval > 0)) throw new Error("The timelapse interval stays a positive number of milliseconds the user chose; the interval carries no code floor.");
  if (!(input.duration >= 0)) throw new Error("The timelapse duration stays zero or a positive number of milliseconds the user chose; the duration never defaults in code.");
  const stopsat = input.stoppedat !== undefined ? Math.min(input.startedat + input.duration, input.stoppedat) : input.startedat + input.duration;
  const span = Math.max(0, stopsat - input.startedat);
  const frames = Math.floor(span / input.interval) + 1;
  const config: timelapseconfig = { runid: input.runid, interval: input.interval, duration: input.duration, startedat: input.startedat, state: input.stoppedat !== undefined || span < input.duration ? "stopped" : "running", ...(input.stoppedat !== undefined ? { stoppedat: input.stoppedat } : {}) };
  return { config, frames, stopsat, reason: `The timelapse of the run ${input.runid} captures ${frames} frame${frames === 1 ? "" : "s"} every ${input.interval} milliseconds across ${span} milliseconds${input.stoppedat !== undefined ? " and stopped on the user stop" : ""}; the plan completion or the user stop closes the lapse.` };
}
 ...
 */
export function namecaptures(input: { rule: capturenamerule; parts: Array<{ plan: string; step: string; timestamp: number; sequence: number }>; taken?: string[] }): { names: string[]; taken: string[]; reason: string } {
  if (input.rule.pattern.trim() === "") throw new Error("The capture naming rule carries its lowercase pattern; an empty pattern names no file.");
  if (input.rule.parts.length === 0) throw new Error("The capture naming rule lists its parts; a pattern without parts stamps nothing.");
  ...
  return { names, taken: [...known], reason: `The naming rule stamped ${names.length} lowercase name${names.length === 1 ? "" : "s"} from the ${input.rule.parts.join(", ")} parts; a repeated name bumps its counter so the names stay unique per run.` };
}
 ...
 */
export function exportcaptures(input: { runid: string; captures: string[]; pairs: string[]; consoleentries: number; netentries: number; diffs: string[]; thumbnails: string[]; lapses: string[]; names: Array<{ captureid: string; name: string }>; provlog: provlogentry[]; masked: boolean; now: number; id?: string }): capturebundle {
  if (input.runid.trim() === "") throw new Error("The capture bundle names its run; an unnamespaced bundle answers no run.");
  ...
  if (input.captures.length > 0 && (input.provlog.length === 0 || missing.length > 0)) throw new Error(`The capture bundle of the run ${input.runid} carries ${input.captures.length} capture${input.captures.length === 1 ? "" : "s"} but the provlog provenance covers ${input.captures.length - missing.length} of them${missing.length > 0 ? ` (missing: ${missing.slice(0, 3).join(", ")})` : ""}; an export payload without provenance entries never leaves the device.`);
  if (!input.masked) throw new Error("The capture bundle assembles only after the redactshot masks ran over its captures; an unmasked capture never exports.");
  return { id: input.id ?? randomid(), runid: input.runid, captures: input.captures, pairs: input.pairs, consoleentries: input.consoleentries, netentries: input.netentries, diffs: input.diffs, thumbnails: input.thumbnails, lapses: input.lapses, names: input.names, provenance: input.provlog.map(entry => entry.id), masked: input.masked, at: input.now };
}
...
function islevel(value: unknown): value is consoletraceentry["level"] {
  return value === "error" || value === "warn" || value === "info" || value === "log" || value === "debug" || value === "trace";
}
...
function issource(value: unknown): value is consoletraceentry["source"] {
  return value === "console" || value === "error" || value === "rejection" || value === "resource" || value === "longtask" || value === "network" || value === "cdp";
}
...
export function joinruns(logsa: runlogentry[], logsb: runlogentry[]): Array<{ stepid: string; index: number; a?: runlogentry; b?: runlogentry }> {
...
export function stepcomparisonof(pair: { stepid: string; index: number; a?: runlogentry; b?: runlogentry }): { stepid: string; index: number; agreement: "agree" | "diverge" | "onlyone"; summarya: string; summaryb: string; durationdelta: number } {
  const a = pair.a;
  const b = pair.b;
  if (a === undefined || b === undefined) return { stepid: pair.stepid, index: pair.index, agreement: "onlyone", summarya: a?.summary ?? "", summaryb: b?.summary ?? "", durationdelta: 0 };
...
export function firstdivergenceof(steps: Array<{ stepid: string; index: number; agreement: "agree" | "diverge" | "onlyone" }>): number | undefined {
...
export function outputcomparesessionof(input: { runids: [string, string]; logsa: runlogentry[]; logsb: runlogentry[]; metrics?: string[]; now: number }): outputcomparesession {
  ...
  return { id: `compare:${input.runids[0]}:${input.runids[1]}:${input.now}`, runids: input.runids, metrics, steps, ...(firstdivergence !== undefined ? { firstdivergence } : {}), openedat: input.now };
}
...
export function comparesessionmetrics(session: outputcomparesession): { metrics: string[]; agree: number; diverge: number; onlyone: number; firstdivergence?: number; reason: string } {
  ...
  return { metrics: session.metrics, agree, diverge, onlyone, ...(session.firstdivergence !== undefined ? { firstdivergence: session.firstdivergence } : {}), reason: `The comparison of ${session.runids[0]} and ${session.runids[1]} graded ${agree} agreeing, ${diverge} divergent and ${onlyone} single run step${agree + diverge + onlyone === 1 ? "" : "s"} under the metric set ${session.metrics.join(", ")}${session.firstdivergence !== undefined ? ` with the first divergence at step index ${session.firstdivergence}` : " with agreement across the whole sequence"}.` };
}
...
export function outputcompareview(session: outputcomparesession): Array<{ stepid: string; index: number; agreement: string; summarya: string; summaryb: string; durationdelta: number; highlighted: boolean }> {
  return session.steps.map(step => ({ stepid: step.stepid, index: step.index, agreement: step.agreement, summarya: step.summarya, summaryb: step.summaryb, durationdelta: step.durationdelta, highlighted: session.firstdivergence !== undefined && step.index === session.firstdivergence }));
}
