import type { capabilityprobe, featuredowngrade, gatewaykind, platformtarget, portableruleset, runtimeadapterdeclaration, actionkind, agentbudget, agentidentity, agentrecord, agentmessage, agentplan, agentscope, agentsession, agentusage, allowlistentry, approvaltimeout, arbitrationrule, attachtarget, blackboardentry, callratelimit, captureexport, automationallowlistentry, budgetstate, checkpointrecord, classconsent, closedtabrecord, lockrecord, conflictwriter, connectallowentry, consentwindow, costbudget, criticreview, deeplinkpattern, depthlimit, draftstep, environmentkind, environmentrequirement, escalationrecord, handoffrecord, heartbeatrecord, incrsnapshotdelta, lazymoddescriptor, lockkind, mergerule, modeloutput, nativecallclass, ocrregion, origincheckverdict, originprofile, perfrecord, phishverdict, plandraft, preconnecttarget, providerconfig, replanrecord, resourcelock, resultreport, reviewrequest, revokerunevent, ratelimitbucket, ratelimitwindow, rollbackitem, runrecord, runsettings, safetyverdict, samplingrequest, selcacheentry, sensitiveclass, siteoverride, sourcemapconsent, spamrule, steptemplate, streamparsechunk, tasklane, toolcallrecord, toolcatalog, tooldef, toolmock, toolnamespace, toolstep, tooldryrun, transformrule, dedupekey, extractpipeline, extractrow, provlogentry, samplepolicy, streamcursor, verifiercheck, virtlistwindow, waitstep, watchdogconfig, workerassignment, workflowrecord, workflowstep, schemaerror, spawnrequest, swarmaction, taskqueue, captureformat, capturenaming, captureoptions, cdpallowlist, chunkextractcursor, cleanuprule, clientidentity, clientrecord, consoleconsentrecord, debuggergrant, delaystep, downloadspec, editormodel, endpointconfig, fieldkind, formprofile, locationconsent, loglevel, loglevelset, mimefilter, mcpserverconfig, navpause, navtrailentry, observationmode, permissionstate, policyevaluation, protocoleventsubscription, quarantineentry, regionrect, rotationrule, localrule, scanverdict, syncsettings } from "./types.js";
...
import { graphqlsubscriptionof, longpollrequestof } from "./gateway.js";
import { apireplayspecof, privatemime } from "./net.js";
import { blockruleof, cookiedomaingranted, cookierecordof, mockspecof, patternorigin, proxyrouteof, headeruleof } from "./net.js";
import { allowlistcovers, breakpointinputof, cdpallowlistof, cdpdomains, cdpeventruleof, methoddomain, overrideinputof, stepmodeof, teardownplanof, watchexpressionof } from "./debug.js";
import { annotationof, attachtargetof, flowspecof, tracecategories } from "./debug.js";
import { agentgrammarvalid, agentpresetof, blackboxruleof, browserpermissions, devicepresetof, familyofkind, locationconsentcovers, locationpresetof, locationrangevalid, networkpresetof, permissiongrantof, permissiongrade, permissionstates, revertplanof } from "./environments.js";
import { autointervalof, importsessionfile, restoreplanof, searchqueryof, sessionkinds, snapshotplanof } from "./session.js";
...
import { branchof, conditionof, controlsteps, foreachof, iscontrolflowkind, loopof, parallelof, repeatuntilof, tryof, whileof } from "./workflow.js";
import { armrule, cronparse, triggerfamilyof, triggerpayloadof, triggereventcatalog, webhooksecretok } from "./workflow.js";
...
function validatesocketgrammar(step: toolstep, options: Record<string, unknown>): policyevaluation {
  ...
  if (kind === "longpoll") {
    ...
    if (options.pollrequest !== undefined) {
      ...
      if (request.timeout !== undefined && (typeof request.timeout !== "number" || !Number.isFinite(request.timeout) || request.timeout <= 0)) return { allowed: false, reason: "The reviewed poll request timeout must be a positive number of milliseconds with no code default." };
    }
  }
  return { allowed: true };
}
...
function validatecontrolgrammar(step: toolstep, options: Record<string, unknown>): policyevaluation {
  ...
  if (kind === "postform") {
    ...
    const encoding = options.form && typeof options.form === "object" && !Array.isArray(options.form) ? (options.form as Record<string, unknown>).encoding : undefined;
    if (encoding !== undefined && !isnonempty(encoding)) return { allowed: false, reason: "The reviewed form payload encoding must be a non-empty name such as urlencoded." };
...
export function validatebreakpointcondition(condition: string): policyevaluation {
  ...
  for (const token of scan.tokens) {
    if (token.startsWith("\"") || token.startsWith("'")) continue; /* a string token left the scanner only as one structurally closed literal */
...
function conditioncallshape(expression: string): boolean {
  ...
  while (paren !== -1) {
    let end = paren - 1;
    while (end >= 0) { const char = expression[end] ?? ""; if (char !== " " && char !== "\t" && char !== "\n" && char !== "\r") break; end -= 1; }
    let start = end;
    while (start >= 0) { const code = expression.charCodeAt(start); const part = (code >= 97 && code <= 122) || (code >= 65 && code <= 90) || (code >= 48 && code <= 57) || code === 95 || code === 36; if (!part) break; start -= 1; }
...
function conditiontokens(expression: string): { tokens: string[]; complete: boolean } {
  ...
  while (index < expression.length) {
    ...
    if (identifierstart) {
      let end = index + 1;
      while (end < expression.length) { const c = expression.charCodeAt(end); const part = (c >= 97 && c <= 122) || (c >= 65 && c <= 90) || (c >= 48 && c <= 57) || c === 95 || c === 36; if (!part) break; end += 1; }
    ...
    if (char === "\"" || char === "'") {
      ...
      while (end < expression.length) {
        ...
        if (inner === "\\") { end += 2; continue; } /* one escape pair: a backslash consumes the next character whatever it is */
        if (inner === char) { closed = true; end += 1; break; }
        end += 1;
      }
    ...
    if (three === "===" || three === "!==") { tokens.push(three); index += 3; continue; }
    const two = expression.slice(index, index + 2);
    if (two === "==" || two === "!=" || two === ">=" || two === "<=" || two === "&&" || two === "||") { tokens.push(two); index += 2; continue; }
...
function nestedquantifiershape(pattern: string): boolean {
  ...
  while (open !== -1) {
    ...
    if (after === "+" || after === "*" || after === "{") {
      ...
      while (position > open && pattern[position] === "\\") { backslashes += 1; position -= 1; }
...
export function canexecute(input: { session: agentsession | undefined; plan: agentplan | undefined; step: toolstep; tabid: number; origin: string; now?: number; verdicts?: safetyverdict[]; settings?: runsettings; run?: runrecord; heartbeat?: heartbeatrecord }): policyevaluation {
  ...
  if (input.run !== undefined && input.run.state === "queued") return { allowed: false, reason: `The run ${input.run.runid} sits queued in the offlinequeue; a queued run executes no step before the replay or the executor moves it to running.` };
  if (input.run !== undefined && (input.run.state === "cancelled" || input.run.state === "rolledback")) return { allowed: false, reason: `The run ${input.run.runid} is ${input.run.state}; a stopped run executes no further step.` };
  if (input.run !== undefined && input.step.risk === "sensitive") {
    if (input.heartbeat === undefined) return { allowed: false, reason: `The sensitive step ${input.step.id} needs a fresh run heartbeat; no heartbeat stands on record for the run ${input.run.runid}.` };
...
export function mcpmodegate(config: mcpserverconfig): policyevaluation {
  ...
  if (config.drainwindow !== undefined && (typeof config.drainwindow !== "number" || !Number.isFinite(config.drainwindow) || config.drainwindow <= 0)) return { allowed: false, reason: "The shutdown drain window must stay a positive user value in milliseconds with no code ceiling." };
  return { allowed: true };
}
...
export function degradationgate(input: { grants?: string[]; origin?: string }): policyevaluation {
  if (input.origin === undefined || input.origin.trim() === "") return { allowed: false, reason: "The serve mode needs the session origin before it decides the degradation; without it the serve stays read only." };
...
export function resourceexposuregate(input: { client: clientrecord; uri: string; served?: string[] }): policyevaluation {
  if (input.client.disconnectedat !== undefined) return { allowed: false, reason: "A disconnected client reads no served resource." };
  ...
  if (input.served !== undefined && !input.served.includes(input.uri)) return { allowed: false, reason: `The serve mode publishes no resource named ${input.uri}.` };
  return { allowed: true };
}
...
export function promptexposuregate(client: clientrecord): policyevaluation {
  if (client.disconnectedat !== undefined) return { allowed: false, reason: "A disconnected client reads no served prompt." };
...
export function protocolnegotiationgate(input: { client?: string | number }): policyevaluation & { major?: number } {
  const declared = typeof input.client === "string" || typeof input.client === "number" ? String(input.client) : undefined;
  if (declared === undefined || declared.trim() === "") return { allowed: true, reason: `The client declared no protocol version, so the negotiation answers the frozen protocolv2 default of ${protocolmajor}.`, major: protocolmajor };
...
export function stepapprovegate(input: { stepids: string[]; resolution: "approve" | "reject" | "edit"; surface: "popup" | "sidepanel" | "dashboardpage" | "optionspage" | "onboarding" | "omnibox" | "page" | "background" }): policyevaluation {
...
export function quickactiongate(input: { action: { command: string; origin: string; permission?: string; session?: boolean }; granted: string[]; sessionactive: boolean; capabilities?: string[] }): policyevaluation {
  ...
  if (input.action.permission !== undefined && !(input.capabilities ?? []).includes(input.action.permission)) return { allowed: false, reason: `The ${input.action.command} quickaction needs the ${input.action.permission} capability granted before it registers; the context menu never offers an action the current capability set refuses.` };
...
export function shortcutkeygate(input: { command: string; palettecommands: string[]; granted: string[]; sessionactive: boolean; action?: { permission?: string; session?: boolean } }): policyevaluation {
  ...
  if (input.action?.permission !== undefined && !input.granted.includes(input.action.permission)) return { allowed: false, reason: `The ${input.command} shortcut needs the ${input.action.permission} capability granted before it dispatches; the shortcut never bypasses the palette action gate.` };
...
export function libraryquarantinegate(input: { verified: boolean; signaturepresent: boolean; signaturevalid: boolean }): policyevaluation {
...
export const portablerulefamilies: readonly string[] = ["origin", "consent", "capability", "schema", "gate", "selector", "control"];
...
export function headlessconsentgate(input: { providerpresent: boolean; resolution?: "approve" | "refuse" }): policyevaluation {
...
export function adaptermappinggate(input: { adapter: runtimeadapterdeclaration; headless?: boolean }): policyevaluation {
  const expected: Record<runtimeadapterdeclaration["runtime"], { storage: runtimeadapterdeclaration["storage"]; worker: runtimeadapterdeclaration["worker"]; dom: runtimeadapterdeclaration["dom"] }> = {
...
export function debouncewindowvalid(input: { kind: string; window?: number }): policyevaluation {
  if (input.window === undefined) return { allowed: true, reason: `The ${input.kind} events pass through uncoalesced; the user set no debouncedom window and the engine sets none.` };
...
export function incrsnapshotgate(input: { delta: incrsnapshotdelta; runid: string; baseexists: boolean }): policyevaluation {
...
export function chunkextractgate(input: { cursor: chunkextractcursor; tableid: string; fingerprint: string }): policyevaluation {
...
export function workerbackpressuregate(input: { depth?: number; pending: number }): policyevaluation {
  if (input.depth === undefined) return { allowed: true, reason: `The worker queue runs unbounded at ${input.pending} pending parse task${input.pending === 1 ? "" : "s"}; the user set no depth and the engine sets none.` };
...
export function virtlistwindowvalid(input: { surface: string; rows?: number }): policyevaluation {
  if (input.rows === undefined) return { allowed: true, reason: `The ${input.surface} list renders every row; the user set no virtlist window and the engine sets none.` };
...
export function perfprovenancegate(record: perfrecord): policyevaluation {
  ...
  if (record.provenance === undefined || Object.keys(record.provenance).length === 0) return { allowed: false, reason: "The perf record attaches its provenance; a record without provenance never enters the tuning view." };
...
export function lazybudgetvalid(input: { budget?: number; prewarmed: number }): policyevaluation {
  if (input.budget === undefined) return { allowed: true, reason: `The startup path carries ${input.prewarmed} prewarmed module${input.prewarmed === 1 ? "" : "s"}; the user set no budget and the engine sets none.` };
...
export function batchwindowvalid(input: { window?: number }): policyevaluation {
  if (input.window === undefined) return { allowed: true, reason: "The batch run queue never pauses; the user set no backpressure window and the engine sets none." };
...
export function domainlimitsvalid(input: { domain: string; slots?: number }): policyevaluation {
  if (input.slots === undefined) return { allowed: true, reason: `The domain ${input.domain} runs unbounded; the user set no concurrency limit and the engine sets none.` };
...
export function politedelayvalid(input: { domain: string; base?: number; floor?: number; jitter?: number }): policyevaluation {
  if (input.base === undefined && input.floor === undefined && input.jitter === undefined) return { allowed: true, reason: `The requests of the domain ${input.domain} run unspaced; the user set no politedelay and the engine sets none.` };
  if (input.base !== undefined && (!Number.isFinite(input.base) || input.base < 0)) return { allowed: false, reason: `The politedelay base of the domain ${input.domain} must stay a non negative millisecond count; the delay stays the user's choice.` };
  if (input.jitter !== undefined && (!Number.isFinite(input.jitter) || input.jitter < 0)) return { allowed: false, reason: `The politedelay jitter of the domain ${input.domain} must stay a non negative millisecond window; the jitter stays the user's choice.` };
  return { allowed: true, reason: `The requests of the domain ${input.domain} space themselves through the user politedelay${input.base !== undefined ? ` of ${input.base} milliseconds` : ""}${input.jitter !== undefined ? ` with a jitter window of ${input.jitter} milliseconds` : ""}${input.floor !== undefined ? ` above the per domain floor of ${input.floor} milliseconds` : ""}.` };
}
...
export function adaptivepollvalid(input: { window?: { floor: number; ceiling: number; growth: number } }): policyevaluation {
  if (input.window === undefined) return { allowed: true, reason: "The poll interval stays fixed; the user set no adaptivepoll window and the engine sets none." };
...
export function runbudgetvalid(input: { stepbudget?: number; memorybudget?: number }): policyevaluation {
  if (input.stepbudget === undefined && input.memorybudget === undefined) return { allowed: true, reason: "The runbudget tracker stays informational; the user set no budget and the engine sets none." };
  if (input.stepbudget !== undefined && (!Number.isFinite(input.stepbudget) || input.stepbudget <= 0)) return { allowed: false, reason: "The step budget must stay a positive step count; the budget stays the user's choice." };
  if (input.memorybudget !== undefined && (!Number.isFinite(input.memorybudget) || input.memorybudget <= 0 || input.memorybudget > 1)) return { allowed: false, reason: "The memory budget must stay a ratio between zero and one; the budget stays the user's choice." };
  return { allowed: true, reason: `The runbudget tracker reports against the user budget${input.stepbudget !== undefined ? ` of ${input.stepbudget} step${input.stepbudget === 1 ? "" : "s"}` : ""}${input.memorybudget !== undefined ? ` and the memory ratio of ${input.memorybudget}` : ""}; the alerts report and the critical threshold pauses pending a user choice.` };
}
...
export function budgetthresholdsvalid(input: { thresholds?: { warning?: number; critical?: number } }): policyevaluation {
  if (input.thresholds === undefined) return { allowed: true, reason: "The budgetalerts stay informational; the user set no thresholds and the engine sets none." };
  const { warning, critical } = input.thresholds;
  if (warning !== undefined && (!Number.isFinite(warning) || warning <= 0 || warning > 1)) return { allowed: false, reason: "The warning threshold must stay a ratio between zero and one; the threshold stays the user's choice." };
  if (critical !== undefined && (!Number.isFinite(critical) || critical <= 0 || critical > 1)) return { allowed: false, reason: "The critical threshold must stay a ratio between zero and one; the threshold stays the user's choice." };
  if (warning !== undefined && critical !== undefined && warning > critical) return { allowed: false, reason: "The warning threshold must stay at or under the critical threshold so the severity levels keep their order." };
  return { allowed: true, reason: `The budgetalerts fire at the user thresholds${warning !== undefined ? ` with the warning at ${warning}` : ""}${critical !== undefined ? ` and the critical pause at ${critical}` : ""}.` };
}
...
export function timeoutboundvalid(input: { bound?: number }): policyevaluation {
  if (input.bound === undefined) return { allowed: true, reason: "The steps never abort on time; the user set no timeout bound and the engine sets none." };
...
export function suspendwindowvalid(input: { window?: number }): policyevaluation {
  if (input.window === undefined) return { allowed: true, reason: "The tabs never suspend during a wait; the user set no suspend window and the engine sets none." };
...
export function sessionreusegate(input: { grant: { profile: string; promptid: string; consentedat: number; runid?: string } }): policyevaluation {
  ...
  return { allowed: true, reason: `The profile ${input.grant.profile} attaches to the run${input.grant.runid !== undefined ? ` ${input.grant.runid}` : ""} through its consented prompt ${input.grant.promptid} with its cookies isolated in its own container.` };
}
...
export function resumefingerprintgate(input: { checkpoint: { stepid: string; digest: string }; fingerprint: string }): policyevaluation {
...
export function logprunegate(input: { plan: { prune: string[]; refused: string[] }; sealed: Array<{ runid: string; sealed: boolean }> }): policyevaluation {
...
export function stepprefetchgate(input: { hint: { page?: string; selectors: string[] }; plannedpages: string[]; plannedselectors: string[] }): policyevaluation {
  if (input.hint.page !== undefined && !input.plannedpages.includes(input.hint.page)) return { allowed: false, reason: `The stepprefetch hint warms ${input.hint.page} which the reviewed plan never names; the prefetch never reaches an unreviewed page.` };
...
export function slowmofactorvalid(input: { factor?: number }): policyevaluation {
  if (input.factor === undefined) return { allowed: true, reason: "The replay keeps its recorded speed; the user set no slowmo factor and the engine sets none." };
...
export function checkpointgate(input: { checkpoint: checkpointrecord; runid: string; digest: string }): policyevaluation {
...
export function heartbeatwindowvalid(input: { window?: number }): policyevaluation {
  if (input.window === undefined) return { allowed: true, reason: "The heartbeat staleness window keeps the documented roadmap default; the user sets no window and the engine caps nothing." };
...
export function queuedtaskexpirygate(input: { task: { planid: string; expiresat: number }; now: number }): policyevaluation {
...
export function queuedepthvalid(input: { depth?: number }): policyevaluation {
  if (input.depth === undefined) return { allowed: true, reason: "The offline queue keeps no user depth; every waiting approved plan stays queued and none ever refuses." };
...
export function sessionlockgate(input: { lock: lockrecord | undefined; runid: string; now: number }): policyevaluation {
  if (input.lock === undefined) return { allowed: true, reason: "No sessionlock stands on the session; the run starts through the reviewed path." };
...
export function urlhistorygate(input: { visit: { url: string; runid: string }; origin: string; grants?: string[] }): policyevaluation {
  let visitorigin = "";
  try { visitorigin = new URL(input.visit.url).origin; } catch { visitorigin = ""; }
...
export function encryptmemorygate(input: { memoryclass?: string; enabled: boolean; encrypted: boolean }): policyevaluation {
...
export function quotacleanupgate(input: { approved: boolean; batch: Array<{ key: string }>; touchesaudit: boolean }): policyevaluation {
...
export function lockwindowvalid(input: { window?: number }): policyevaluation {
  if (input.window === undefined) return { allowed: true, reason: "The sessionlock expiry window keeps the documented roadmap default; the user sets no window and the engine caps nothing." };
...
export function exportprovenancegate(input: { items: Array<{ key: string; provenance: { origin: string; runid: string; stepid: string } }> }): policyevaluation {
...
export function agentscopegate(input: { scope: agentscope | undefined; kind: string; origin?: string; risk?: string }): policyevaluation {
  if (input.scope === undefined) return { allowed: true, reason: "The step runs under no agent scope; the session grants alone bound it." };
  if (input.scope.readonly === true && input.risk !== undefined && input.risk !== "read") return { allowed: false, reason: `The read only scope of the agent ${input.scope.agentid} refuses the ${input.kind} step; an observer never acts on the page.` };
  if (input.scope.actionkinds !== undefined && input.scope.actionkinds.length > 0 && !input.scope.actionkinds.includes(input.kind)) return { allowed: false, reason: `The agent scope of ${input.scope.agentid} grants no ${input.kind} step; the narrowed action kinds stay ${input.scope.actionkinds.slice(0, 5).join(", ")}${input.scope.actionkinds.length > 5 ? " and more" : ""}.` };
  if (input.origin !== undefined && input.scope.origins.length > 0 && !input.scope.origins.includes(input.origin)) return { allowed: false, reason: `The agent scope of ${input.scope.agentid} grants no access to the origin ${input.origin}.` };
...
export function budgetgate(input: { state: budgetstate | undefined; steps?: number; tokens?: number; durationms?: number }): policyevaluation {
  if (input.state === undefined) return { allowed: true, reason: "The step runs under no agent budget; the user set none and the engine sets none." };
  const steps = input.steps ?? 1;
  const tokens = input.tokens ?? 0;
  const durationms = input.durationms ?? 0;
  if (input.state.maxsteps !== undefined && input.state.spentsteps + steps > input.state.maxsteps) return { allowed: false, reason: `The agent ${input.state.agentid} already executed ${input.state.spentsteps} of its ${input.state.maxsteps} user configured steps; the user raises the ceiling or stops the agent.` };
  if (input.state.maxtokens !== undefined && input.state.spenttokens + tokens > input.state.maxtokens) return { allowed: false, reason: `The agent ${input.state.agentid} already spent ${input.state.spenttokens} of its ${input.state.maxtokens} user configured tokens; the user raises the ceiling or stops the agent.` };
  if (input.state.maxdurationms !== undefined && input.state.spentdurationms + durationms > input.state.maxdurationms) return { allowed: false, reason: `The agent ${input.state.agentid} already spent ${input.state.spentdurationms} of its ${input.state.maxdurationms} user configured milliseconds; the user raises the ceiling or stops the agent.` };
...
export function fleetoperationgrade(operation: string): policyevaluation {
  const readonly = new Set(["outputcompare", "consensusvote", "runreplay", "reconstructreplay", "reviewrequest", "recordverdict", "escalate", "escalationblock"]);
...
export function spawngate(input: { parentscope: agentscope | undefined; objectiveorigin: string; parentstate?: string }): policyevaluation {
  ...
  if (input.parentscope === undefined || input.parentscope.origins.length === 0) return { allowed: true, reason: "The parent scope stays unbounded inside the session grants; the child objective origin needs no extra coverage." };
...
export function depthgate(input: { depth: number; limit: depthlimit }): policyevaluation {
  if (input.limit.maxdepth === undefined) return { allowed: true, reason: `The lineage depth ${input.depth} sits unbounded; the user configured no depth ceiling.` };
...
export function arbitrationverdictgate(input: { holderagentid: string; origin: string; sessionlockholder?: string; origingrants?: string[] }): policyevaluation {
  if (input.sessionlockholder !== undefined && input.sessionlockholder !== input.holderagentid) return { allowed: false, reason: `The run lock of ${input.origin} stays held by ${input.sessionlockholder}; the verdict grants a queue position and never bypasses the sessionlock.` };
  if (input.origingrants !== undefined && input.origingrants.length > 0 && !input.origingrants.includes(input.origin)) return { allowed: false, reason: `The origin ${input.origin} sits outside the session grants; the verdict grants a queue position and never widens the origin grants.` };
...
export function lessonsecretgate(input: { text: string }): policyevaluation {
  const patterns: Array<[string, RegExp]> = [["token", /\b(?:api[- ]?key|token|secret|password|passwd|bearer|authorization)\b\s*[:=]\s*\S+/i], ["credential", /\b(?:sk-[a-z0-9]{16,}|gh[pousr]_[A-Za-z0-9]{20,}|AKIA[0-9A-Z]{16}|-----BEGIN [A-Z ]*PRIVATE KEY-----)/i]];
...
export function prefetchgate(input: { urls: string[]; grants: string[] }): policyevaluation {
  const outside = input.urls.filter(url => {
    try { return !input.grants.includes(new URL(url).origin); } catch { return true; }
  });
...
export function preconnectgate(input: { targets: preconnecttarget[]; grants?: string[] }): policyevaluation {
  ...
  const grants = input.grants;
  const outside = grants !== undefined ? input.targets.filter(target => target.origin.trim() !== "" && !grants.includes(target.origin)) : [];
...
export function reopentabgate(input: { record: closedtabrecord; grants: string[] }): policyevaluation {
  let origin = "";
  try { origin = new URL(input.record.url).origin; } catch { origin = ""; }
...
export function pausenavconsentgate(input: { pause?: navpause; kind: string }): policyevaluation {
  const navkinds = new Set(["openlink", "openprivate", "followlink", "spanav", "navlist", "openclipboard", "batchopen", "prefetch", "preconnect", "deeplink", "reopentab"]);
  const pause = input.pause;
  if (pause === undefined || pause.pausedat === undefined || !navkinds.has(input.kind)) return { allowed: true, reason: `The ${input.kind} step runs under no open consent freeze; the navigation pause holds the navigation kinds only.` };
  return { allowed: false, reason: `Navigation is paused while ${pause.reason}${pause.pendingurl !== undefined ? `; the pending navigation ${pause.pendingurl} queues until the answer` : ""}; resume navigation first.` };
}
...
export function batchsizelimitgate(input: { size: number; limit?: number }): policyevaluation {
  if (input.limit === undefined) return { allowed: true, reason: `The batch of ${input.size} url${input.size === 1 ? "" : "s"} runs under no user ceiling; the batch size bound stays a user choice with no code default.` };
...
export function trailorigingate(input: { entries: navtrailentry[]; origins: string[] }): policyevaluation {
  const outside = input.entries.filter(entry => {
    try { return !input.origins.includes(new URL(entry.url).origin); } catch { return true; }
  });
...
export function dedupeconfiggate(input: { key?: dedupekey; pipelineid: string }): policyevaluation {
  if (input.key === undefined || input.key.columns.filter(column => column.trim() !== "").length === 0) return { allowed: false, reason: `The pipeline ${input.pipelineid} carries no configured dedupe key; the deduplication stays a user choice per pipeline and an implicit key never drops rows.` };
...
export function resumegate(input: { pipeline: extractpipeline; planid?: string; origin: string }): policyevaluation {
  if (input.pipeline.planid !== undefined && input.planid !== undefined && input.pipeline.planid !== input.planid) return { allowed: false, reason: `The pipeline ${input.pipeline.name} belongs to the plan ${input.pipeline.planid} while the resume names ${input.planid}; a resume continues exactly the plan it came from.` };
...
export function pipelinetargetgate(input: { fields: string[]; target: string[]; pipelineid: string }): policyevaluation {
...
export function streamchunkgate(input: { chunk: number; limit?: number }): policyevaluation {
  if (input.limit === undefined) return { allowed: true, reason: `The stream chunk of ${input.chunk} row${input.chunk === 1 ? "" : "s"} runs under no user ceiling; the chunk size stays a user choice with no code default.` };
...
export function rowstampgate(input: { stored: Array<{ key: string; capturedat: number }>; row: { key: string; capturedat: number } }): policyevaluation {
  const existing = input.stored.find(candidate => candidate.key === input.row.key);
  if (existing !== undefined && existing.capturedat !== input.row.capturedat) return { allowed: false, reason: `The row ${input.row.key} already carries its stamp at ${existing.capturedat} while the restamp names ${input.row.capturedat}; a stamped timestamp never rewrites.` };
...
export function ratelimitrespectgate(input: { waitms: number; origin: string; budget?: number }): policyevaluation {
  ...
  if (input.budget !== undefined && input.waitms > input.budget) return { allowed: false, reason: `The rate limit wait of ${input.waitms} milliseconds on ${input.origin} exceeds the reviewed wait budget of ${input.budget} milliseconds; review a wider budget or let the window reset.` };
...
export function subscriptionboundgate(input: { count: number; limit?: number }): policyevaluation {
  if (input.limit === undefined) return { allowed: true, reason: "The open subscription count stays unbounded because no user ceiling is configured." };
...
export function pollwaitbudgetgate(input: { interval: number; timeout?: number; wait?: number }): policyevaluation {
  ...
  if (input.timeout !== undefined && !(input.timeout >= 0)) return { allowed: false, reason: "The reviewed long poll timeout must be zero or a positive number of milliseconds." };
  if (input.wait !== undefined && !(input.wait >= 0)) return { allowed: false, reason: "The reviewed wait budget must be zero or a positive number of milliseconds." };
  if (input.wait !== undefined && input.interval > input.wait) return { allowed: false, reason: `The long poll interval of ${input.interval} milliseconds exceeds the reviewed wait budget of ${input.wait} milliseconds; review a wider budget or a shorter interval.` };
  const total = input.timeout !== undefined ? input.interval + input.timeout : input.interval;
  if (input.wait !== undefined && total > input.wait) return { allowed: false, reason: `The long poll interval of ${input.interval} milliseconds with its ${input.timeout ?? 0} millisecond timeout exceeds the reviewed wait budget of ${input.wait} milliseconds; review a wider budget or a shorter loop.` };
  return { allowed: true, reason: `The long poll waits of ${total} milliseconds sit inside the reviewed wait budget${input.wait !== undefined ? ` of ${input.wait} milliseconds` : ""}.` };
}
...
export function regionboundsgate(input: { region: ocrregion; viewport: { width: number; height: number } }): policyevaluation {
...
export function framebudgetgate(input: { waitms: number; budget?: number }): policyevaluation {
  ...
  if (input.budget !== undefined && input.waitms > input.budget) return { allowed: false, reason: `The frameocr wait of ${input.waitms} milliseconds exceeds the reviewed wait budget of ${input.budget} milliseconds; review a wider budget or a nearer position.` };
  return { allowed: true, reason: `The frameocr wait of ${input.waitms} milliseconds sits inside the reviewed wait budget${input.budget !== undefined ? ` of ${input.budget} milliseconds` : ""}.` };
}
...
export function visioncacheexpirygate(input: { entryat: number; now: number; retention?: number }): policyevaluation {
  if (input.retention === undefined) return { allowed: true, reason: "The visioncache entry stays forever because no user retention window is configured." };
...
export function timelapsegate(input: { userstarted: boolean; interval?: number }): policyevaluation {
  ...
  if (input.interval !== undefined && !(input.interval > 0)) return { allowed: false, reason: "The timelapse interval stays a positive number of milliseconds the user chose; the interval carries no code floor." };
  return { allowed: true, reason: `The user started the timelapse${input.interval !== undefined ? ` on the ${input.interval} millisecond interval` : ""}; the lapse captures on the reviewed cadence only.` };
}
...
export function captureexportgate(input: { useraction: boolean; redacted: boolean; provenance: boolean; captures: number }): policyevaluation {
...
export function captureretentiongrade(input: { retention?: number; pruned: number }): policyevaluation {
  if (input.retention === undefined) return { allowed: true, reason: `The forensic retention stays unconfigured so every record survives; the pass pruned ${input.pruned} record${input.pruned === 1 ? "" : "s"} only because the user asked for the cleanup.` };
...
export function thumbnailsizereadonlygrade(input: { edge?: number }): policyevaluation {
  if (input.edge === undefined) return { allowed: true, reason: "The thumbnail edge stays unconfigured so every thumbnail keeps its capture size; the bound never defaults in code." };
...
export function diffthresholdgrade(input: { threshold?: number }): policyevaluation {
  if (input.threshold === undefined) return { allowed: true, reason: "The diff threshold stays unconfigured so no regression flags on its own; the bound never defaults in code." };
...
export function timelapseintervalgrade(input: { interval?: number }): policyevaluation {
  if (input.interval === undefined) return { allowed: true, reason: "The timelapse interval stays unconfigured so the lapse waits for the explicit user start; the interval carries no code floor." };
...
export function forensicchoices(settings: runsettings | undefined): { diffthreshold?: number; timelapseinterval?: number; thumbnailedge?: number; forensicretention?: number } {
...
export function encryptsyncgate(input: { encrypted: boolean; formattag?: string }): policyevaluation {
  ...
  if (input.formattag === undefined || input.formattag.trim() === "") return { allowed: false, reason: "The sync payload carries no format version tag; an envelope that cannot name its format answers no decoder." };
...
export function jargate(input: { jarid?: string; jar?: { jarid: string; runid: string; sealed: boolean }; runid: string; operation: "read" | "write" }): policyevaluation {
  if (input.jarid === undefined) return { allowed: false, reason: "The cookie access needs the session jar; a session without a jar never touches the cookie state of a run." };
  if (input.jar === undefined) return { allowed: false, reason: `No cookie jar matches the jar id ${input.jarid}; the cookie access stays bound to the jar the run owns.` };
...
export function scannerhookgate(input: { hook?: { scanner: string; endpoint: string; origin: string }; granted: boolean }): policyevaluation {
  if (input.hook === undefined) return { allowed: false, reason: "The quarantine scan needs the user configured scanner hook; an unconfigured scanner keeps the verdict pending and the file held." };
...
export function minimizationchoices(settings: runsettings | undefined): { synccadence?: number; cleanupdelay?: number; jarexpiry?: number } {
...
function compoundconsumes(token: string): boolean {
  ...
  else if (first >= 97 && first <= 122) { /* the type: one lowercase letter then lowercase alphanumerics and hyphens */
    index = 1;
    while (index < token.length) { const code = token.charCodeAt(index); if (!((code >= 97 && code <= 122) || (code >= 48 && code <= 57) || code === 45)) break; index += 1; }
  }
  while (index < token.length) {
    ...
    if (char === "#" || char === ".") {
      ...
      if (!((code >= 97 && code <= 122) || (code >= 65 && code <= 90) || code === 95)) return false; /* one id or class head character */
      let end = start + 1;
      while (end < token.length) { const c = token.charCodeAt(end); if (!((c >= 97 && c <= 122) || (c >= 65 && c <= 90) || (c >= 48 && c <= 57) || c === 95 || c === 45)) break; end += 1; }
      index = end;
      continue;
    }
    if (char === "[") {
      const close = token.indexOf("]", index + 1);
      if (close === -1 || close === index + 1) return false; /* the attribute needs its closing bracket and at least one inner character */
      index = close + 1;
      continue;
    }
    if (char === ":") {
      ...
      let letters = 0;
      while (cursor < token.length) { const code = token.charCodeAt(cursor); if (!((code >= 97 && code <= 122) || (code >= 65 && code <= 90) || code === 45)) break; letters += 1; cursor += 1; }
      if (letters === 0) return false;
      if (token[cursor] === "(") {
        const close = token.indexOf(")", cursor + 1);
        if (close === -1) return false;
        if (token.indexOf("(", cursor + 1) !== -1 && token.indexOf("(", cursor + 1) < close) return false; /* the pseudo argument stays one plain parenthesized run */
        index = close + 1;
        continue;
      }
      index = cursor;
...
export function fixtureconsentgate(input: { fixtureorigin: string; grants: string[]; kind: string; riskof: (kind: string) => "read" | "interaction" | "sensitive" }): policyevaluation {
  ...
  try { risk = input.riskof(input.kind); } catch { return { allowed: false, reason: `The kind ${input.kind} is not a reviewed action kind; the fixture replays only the reviewed vocabulary.` }; }
...
export function serverurlgate(url: string): policyevaluation {
  ...
  let parsed: URL;
  try { parsed = new URL(trimmed); } catch { return { allowed: false, reason: `The relay url ${trimmed} does not parse as a url; the serverurl setting refuses it before saving.` }; }
...
export function bridgecontentgate(input: { payload: Record<string, unknown>; pageconsent?: boolean }): policyevaluation {
...
export function bridgeframeorigingate(input: { framesession?: string; framesender?: string; sessionid: string; origin: string; paired: boolean }): policyevaluation {
  ...
  if (input.framesession !== undefined && input.framesession !== input.sessionid) return { allowed: false, reason: `The frame session ${input.framesession} sits outside the paired session ${input.sessionid}; the origin check refuses the frame.` };
  if (input.framesender !== undefined && input.framesender.trim() === "") return { allowed: false, reason: "The frame names its sender; an anonymous frame never reaches the extension." };
...
export function bridgechoices(settings: runsettings | undefined): { serverurl?: string; bridgeconsent?: boolean; bridgepageconsent?: boolean; bridgeidlewindow?: number; bridgeheartbeatinterval?: number; bridgeratelimit?: number; bridgeratewindow?: number; bridgepairinglifetime?: number } {
  const current = settings ?? {};
  return { ...(current.serverurl !== undefined ? { serverurl: current.serverurl } : {}), ...(current.bridgeconsent !== undefined ? { bridgeconsent: current.bridgeconsent } : {}), ...(current.bridgepageconsent !== undefined ? { bridgepageconsent: current.bridgepageconsent } : {}), ...(current.bridgeidlewindow !== undefined ? { bridgeidlewindow: current.bridgeidlewindow } : {}), ...(current.bridgeheartbeatinterval !== undefined ? { bridgeheartbeatinterval: current.bridgeheartbeatinterval } : {}), ...(current.bridgeratelimit !== undefined ? { bridgeratelimit: current.bridgeratelimit } : {}), ...(current.bridgeratewindow !== undefined ? { bridgeratewindow: current.bridgeratewindow } : {}), ...(current.bridgepairinglifetime !== undefined ? { bridgepairinglifetime: current.bridgepairinglifetime } : {}) };
}
...
export function gatewaybaseurlgate(input: { kind: gatewaykind; baseurl: string }): policyevaluation {
  ...
  let parsed: URL;
  try { parsed = new URL(url); } catch { return { allowed: false, reason: `The ${input.kind} base url ${url} does not parse as an absolute url.` }; }
...
export function gatewayconsentgate(input: { config: { kind: gatewaykind; baseurl: string; enabled: boolean; consented?: boolean }; local?: boolean }): policyevaluation {
...
export function nativeinstallconsentgate(input: { consent?: boolean; profiledir: string; hostname: string }): policyevaluation {
...
export function nativetransportconsentgate(input: { consent?: boolean; installed: boolean; callclass: nativecallclass; classconsents?: nativecallclass[]; killswitch?: boolean }): policyevaluation {
...
export function nativesensitiveapprovalgate(input: { callclass: nativecallclass; approval?: boolean }): policyevaluation {
