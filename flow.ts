...
import type { flowlibraryentry, flowlibrarymanifest, flowlibrarystep, dataexpectation, publishdescriptor, workflowrecord, schemaerror, actionkind } from "./types.js";
import { actionrisk, librarycapabilitygate, librarymanifestgate, libraryquarantinegate, librarysensitivegate, libraryimportgate } from "./policy.js";
...
function manifestbody(manifest: Omit<flowlibrarymanifest, "publish">): string {
  return JSON.stringify({ id: manifest.id, title: manifest.title, description: manifest.description, version: manifest.version, publisher: manifest.publisher, ...(manifest.registry !== undefined ? { registry: manifest.registry } : {}), steps: manifest.steps, kinds: manifest.kinds, requiredgrants: manifest.requiredgrants, dataexpectations: manifest.dataexpectations, sensitive: manifest.sensitive });
}
...
export async function validatemanifest(input: { manifest: unknown; capabilities: string[] }): Promise<{ ok: boolean; errors: schemaerror[]; reason: string; manifest?: flowlibrarymanifest }> {
  ...
  const manifest: flowlibrarymanifest = { id: candidate.id as string, title: candidate.title as string, description: candidate.description as string, version: candidate.version as string, publisher: candidate.publisher as string, ...(typeof candidate.registry === "string" && candidate.registry.trim() !== "" ? { registry: candidate.registry } : {}), steps: (candidate.steps as flowlibrarystep[]).map(step => ({ id: step.id, kind: step.kind, label: step.label, ...(step.target !== undefined ? { target: step.target } : {}), ...(step.value !== undefined ? { value: step.value } : {}), ...(step.namespace !== undefined ? { namespace: step.namespace } : {}) })), kinds: candidate.kinds as string[], requiredgrants: candidate.requiredgrants as string[], dataexpectations: candidate.dataexpectations as dataexpectation[], sensitive: candidate.sensitive as boolean, ...(isPublish(candidate.publish) ? { publish: candidate.publish } : {}) };
...
export async function verifypublishersignature(manifest: flowlibrarymanifest): Promise<{ verified: boolean; reason: string }> {
  if (manifest.publish === undefined) return { verified: false, reason: `The manifest ${manifest.id} of ${manifest.publisher} carries no publisher signature; the entry quarantines until the user verifies its publisher.` };
  ...
  const seal = await sha256(`${manifest.publish.publisher}\n${manifest.publish.digest}\n${manifest.publish.provenance}`);
...
export async function signmanifest(manifest: flowlibrarymanifest, provenance: string, publishedat: number): Promise<publishdescriptor> {
  const digest = await manifestdigest(manifest);
  return { publisher: manifest.publisher, signature: await sha256(`${manifest.publisher}\n${digest}\n${provenance}`), digest, provenance, publishedat };
}
...
export async function libraryentryof(input: { manifest: flowlibrarymanifest; provenance: string; now: number }): Promise<flowlibraryentry> {
  const digest = await manifestdigest(input.manifest);
  const verification = await verifypublishersignature(input.manifest);
  const quarantinegate = libraryquarantinegate({ verified: verification.verified, signaturepresent: input.manifest.publish !== undefined, signaturevalid: verification.verified });
...
export function grantdiffof(input: { manifest: flowlibrarymanifest; heldgrants: string[] }): { added: string[]; kept: string[]; originmappings: Array<{ origin: string; kinds: string[] }> } {
...
export function sensitiveconsentfor(manifest: flowlibrarymanifest, freshconsent: boolean): { required: boolean; reason: string } {
...
export function installlibrary(input: { entry: flowlibraryentry; selectornamespace?: string; now: number }): workflowrecord {
...
export function updatelibrary(input: { incoming: flowlibraryentry; existing: flowlibraryentry }): { changedsteps: string[]; addedgrants: string[]; versionfrom: string; versionto: string; replace: boolean; reason: string } {
  ...
  const changedsteps = [...new Set([...existingsteps, ...incomingsteps])].filter(id => {
    ...
    return before === undefined || after === undefined || before.kind !== after.kind || before.target !== after.target || before.value !== after.value;
  });
...
export function removelibrary(input: { entry: flowlibraryentry; forks: workflowrecord[] }): { removed: string; keptforks: string[]; reason: string } {
...
export function forklibrary(input: { entry: flowlibraryentry; now: number }): workflowrecord {
  const manifest = input.entry.manifest;
  const steps = manifest.steps.map(step => ({ id: `fork-${step.id}`, kind: step.kind as workflowrecord["steps"][number]["kind"], label: step.label, ...(step.target !== undefined ? { target: step.target } : {}), ...(step.value !== undefined ? { value: step.value } : {}) }));
...
export function librarysearch(input: { entries: flowlibraryentry[]; query?: string; filter?: { publisher?: string; sensitive?: boolean; state?: string } }): flowlibraryentry[] {
  ...
  return input.entries.filter(entry => {
    ...
    return [entry.manifest.title, entry.manifest.description, entry.manifest.publisher].some(text => text.toLowerCase().includes(query));
  });
}
...
export function librarybrowserow(entry: flowlibraryentry): { id: string; title: string; publisher: string; version: string; grants: string[]; sensitive: boolean; state: string; registry?: string } {
  return { id: entry.id, title: entry.manifest.title, publisher: entry.manifest.publisher, version: entry.manifest.version, grants: entry.manifest.requiredgrants, sensitive: entry.manifest.sensitive, state: entry.state, ...(entry.manifest.registry !== undefined ? { registry: entry.manifest.registry } : {}) };
}
...
export function librarystepsview(entry: flowlibraryentry): Array<{ id: string; kind: string; label: string; target?: string; namespace?: string; families: string[]; fields: string[] }> {
  return entry.manifest.steps.map(step => {
    ...
    return { id: step.id, kind: step.kind, label: step.label, ...(step.target !== undefined ? { target: step.target } : {}), ...(step.namespace !== undefined ? { namespace: step.namespace } : {}), families: expectations.map(expectation => expectation.family), fields: [...new Set(expectations.flatMap(expectation => expectation.fields))] };
  });
}
...
export function dataexpectationssummary(manifest: flowlibrarymanifest): Array<{ family: string; fields: string[]; steps: string[] }> {
...
export function libraryeventof(input: { kind: "install" | "update" | "remove"; entryid: string; title: string; version: string; detail: string; now: number }): { id: string; kind: "install" | "update" | "remove"; entryid: string; title: string; version: string; detail: string; at: number } {
...
export function exportlibrarymanifests(entries: flowlibraryentry[]): Array<{ id: string; title: string; publisher: string; version: string; digest: string; state: string; provenance: string; addedat: number }> {
...
import type { consentprovider, flowrunevent, flowrunoptions, flowrunoutcome, flowrunrequest, flowrungate, planfile, planfilestep, storedrunlog, toolstep } from "./types.js";
...
export function parseflowrunrequest(args: string[]): flowrunrequest {
  ...
  for (let index = 0; index < args.length; index += 1) {
    ...
    if (arg.startsWith("--")) {
      ...
      else if (next !== undefined && !next.startsWith("--")) { flags.set(name, next); index += 1; }
      else flags.set(name, "true");
    } else positional.push(arg);
  }
  const planpath = positional[0];
  if (planpath === undefined || planpath.trim() === "") throw new Error("The flowrun command needs the plan file path it executes.");
  ...
  if (grantspath === undefined && !interactive) throw new Error("The flowrun needs a grants file or the interactive flag; a run without an origin grant never starts.");
  const options: flowrunoptions = { format, outputdir, interactive, dryrun, ...(grantspath !== undefined ? { grantspath } : {}) };
...
export function flowproposalvalue(file: planfile): Record<string, unknown> {
  return {
    ...
    workflow: {
      ...
      version: 1,
      origins: [file.origin],
      steps: file.steps.map(step => ({ id: step.id, kind: step.kind, label: step.label, ...(step.target !== undefined ? { target: step.target } : {}), ...(step.value !== undefined ? { value: step.value } : {}), ...(step.options !== undefined ? { options: step.options } : {}) }))
    }
  };
}
...
export function flowstepsensitive(step: planfilestep): boolean {
  const classes = sensitiveclassesof({ kind: step.kind as never, ...(step.value !== undefined ? { value: step.value } : {}), ...(step.options !== undefined ? { options: step.options } : {}) });
  if (classes.sensitive) return true;
  try { return resolvedrisk({ id: step.id, kind: step.kind as never, label: step.label, summary: step.label, ...(step.target !== undefined ? { target: step.target } : {}), ...(step.value !== undefined ? { value: step.value } : {}), ...(step.options !== undefined ? { options: step.options } : {}), risk: "sensitive" } as toolstep) === "sensitive"; } catch { return true; }
}
...
export function renderflowevents(events: flowrunevent[], format: "human" | "json"): string {
  ...
  return events.map(event => `${new Date(event.at).toISOString()} ${event.kind.toUpperCase()}${event.stepid !== undefined ? ` ${event.stepid}` : ""}: ${event.summary}`).join("\n");
}
...
export function flowtimelinerowsof(events: flowrunevent[]): Array<{ stepid: string; status: "waiting" | "done" | "failed" | "revoked"; summary: string; at: number }> {
  const rows: Array<{ stepid: string; status: "waiting" | "done" | "failed" | "revoked"; summary: string; at: number }> = [];
...
export async function runflow(input: { request: flowrunrequest; file: planfile; grants: string[]; provider?: consentprovider; driver?: flowdriver; now: number; clock?: () => number; emit?: (event: flowrunevent) => void }): Promise<{ outcome: flowrunoutcome; events: flowrunevent[]; log: storedrunlog }> {
  const grantsource = input.request.options.grantspath !== undefined ? "file" : input.request.options.interactive ? "prompt" : "none";
  ...
  if (driver === undefined) throw new Error("The flowrun needs a driver: a remote endpoint the run attaches to or the dry run flag; the cli drives no local browser itself.");
  let executed = 0;
  for (const step of input.file.steps) {
    if (flowstepsensitive(step)) {
      ...
      const consentgate = headlessconsentgate({ providerpresent: input.provider !== undefined, ...(resolution !== undefined ? { resolution } : {}) });
      if (!consentgate.allowed) {
        ...
        return { outcome: { runid, state: "revoked", steps: executed, exitcode: flowrunexitcodeof("revoked"), sealhash: sealed.seal.sealhash.current }, events, log: sealed.log };
      }
    }
    ...
    if (result.state === "failed") {
      const sealed = await sealrunlog(current, clock());
      return { outcome: { runid, state: "failed", steps: executed, exitcode: flowrunexitcodeof("failed"), sealhash: sealed.seal.sealhash.current }, events, log: sealed.log };
    }
  }
...
export function flowrunreport(input: { outcome: flowrunoutcome; events: flowrunevent[] }): { version: string; outcome: flowrunoutcome; events: flowrunevent[]; timeline: string } {
...
export function syncbridgehookof(input: { provider: "file" | "web"; direction: "pull" | "push" | "both"; endpoint: string; now: number }): syncbridgehook {
...
export function syncbridgeproviders(): Array<{ provider: "file" | "web"; label: string; operations: string[]; note: string }> {
  return [
...
export async function syncbridgeexportpayload(manifests: flowlibrarymanifest[]): Promise<{ version: 1; kind: "syncbridge"; manifests: flowlibrarymanifest[]; digests: string[]; exclusions: string[] }> {
  const digests: string[] = [];
  for (const manifest of manifests) digests.push(await syncdigestof(manifest));
  return { version: 1 as const, kind: "syncbridge" as const, manifests, digests, exclusions: ["secretvault values", "logs"] };
}
...
export function syncbridgevalidate(payload: { kind?: string; manifests?: unknown; digests?: unknown; exclusions?: unknown; secrets?: unknown; logs?: unknown }): { ok: boolean; reason: string } {
  const carriessecrets = payload.secrets !== undefined || (Array.isArray(payload.manifests) && payload.manifests.some(manifest => secretcarrying(manifest)));
...
export async function syncbridgescan(input: { hook: syncbridgehook; local: Array<{ manifestid: string; version: string; digest: string }>; remote: Array<{ manifestid: string; version: string; digest: string }>; now: number }): Promise<{ conflicts: syncbridgeconflict[]; synced: string[]; reason: string }> {
...
export function resolveconflict(conflict: syncbridgeconflict, resolution: "local" | "remote" | "merge", now: number): syncbridgeconflict {
  if (conflict.resolution !== undefined) throw new Error(`The conflict ${conflict.id} already resolved at ${conflict.resolvedat}; one conflict resolves exactly once.`);
...
export function webproviderstub(hook: syncbridgehook): { hookid: string; endpoint: string; optin: boolean; operations: string[]; stub: true; reason: string } {
...
export function fileproviderof(hook: syncbridgehook): { hookid: string; operations: Array<{ kind: "pull" | "push" | "list"; label: string }> } {
