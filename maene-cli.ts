/**
 * @fileoverview cli.ts - production-ready CLI for maene (MERGED)
 * @module auth/cli
 * @description
 ...
 *  ./auth.js (v2.1.14: oauth.ts consolidated into it) ./accounts.js
...
import * as versionMod from "./maene-version.js";
import * as authMod from "./maene-auth.js";
import * as modelsMod from "./models.js";
import * as systemMod from "./maene-debug.js";
...
const authenticate = oauth.authenticate as typeof import("./maene-auth.js").authenticate;
...
// grand-merge rename: the maene saveConfig collided with devthink's root
// saveConfig in merged config.ts and was renamed agsaveConfig — reference the
// renamed export (the maene config writer), not devthink's saveConfig.
const saveConfig = (configMod.agsaveConfig as any) ?? (configPkg.agsaveConfig as any);
...
const ANTIGRAVITY_VER_FB: string =
  (constantsMod as any).ANTIGRAVITY_VERSION_FALLBACK ??
  (modelsMod as any).ANTIGRAVITY_VERSION_FALLBACK ??
  ANTIGRAVITY_VERSION_FALLBACK;
...
function printHeader(): void {
  console.log(
    C.bold(C.cyan("\n+---------------------------------------------------------+")) +
      `\n${C.bold(C.cyan("|"))}  ${C.bold("maene")} ${C.dim(`v${CLI_VERSION}`)} - unified CLI  ${C.bold(C.cyan("|"))}` +
...
async function cmdVersion(_args: string[]): Promise<void> {
  ...
  try {
    ...
    console.log(C.bold("\nVersions"));
    console.log(`  Plugin maene: ${C.bold(PLUGIN_VERSION_FALLBACK)}`);
    console.log(
...
const PLUGIN_NAME = "maene" as const;
 ...
 */
async function configureModelsFlow(opts: { global?: boolean | undefined; customPath?: string | undefined } = {}): Promise<void> {
  printHeader();
  ...
  //  - plugin: unique array merge; never duplicates maene when a variant
  //    (@wenathlan/maene, maene@latest, ...) is already listed
  ...
  // listed counts ("@wenathlan/maene", "maene@npm:@wenathlan/maene@latest"
  // — the scope-free npm alias —, "file:.../maene", ...) and is never
  // duplicated. When nothing exists we add the canonical npm name.
  // (A package literally named "maene" cannot exist on npmjs.org — the
  ...
  const hasMaeneVariant = [...pluginsSet].some((p) => /(^|[/@])maene([@/]|$)/.test(p));
  if (!hasMaeneVariant) {
    pluginsSet.add("@wenathlan/maene");
  }
...
function printHelp(): void {
  console.log(
    C.bold(`
maene CLI (unified merge) - production-ready
