...
# devthink 2.0.29 — THE ONE CONTAINER FILE (the saddle standard: a single
...
#   DEVTHINK_VERSION   baked into the OCI version label, default 2.0.29
...
#     -p 31080:8080 \
#     ghcr.io/wenathlan/devthink:2.0.29
#
...
# exactly the same on both legs. the guard below scales the timeouts and
# the linearity budgets unconditionally now: the push leg of the publish
# lane resolves the builder beside the four qemu-interpreted architecture
# legs of the five-arch index, and the native suite under that contention
# answers the same tests two to three times slower (the 2.0.25 push build
# measured the whole battery at twice the isolated scan build's time and
# the suite reddened on the five-second default timeouts and the one-second
# linearity budget alone) — the vitest config, the per-test overrides and
# the torture budgets read the same env, so every budget scales together
# whether the build node is native arm64 or the lane builds beside emulated
# legs. the build already ran above, so the frozen-contract tests that read
# dist/schemas answer.
...
# credential pair.
# the smoke-boot rides its own network namespace: the push leg of the
# publish lane resolves the five architecture legs in parallel on the
# one build node, and the default build network the runtime stages
# share let two legs bind 0.0.0.0:8080 in the same instant — the
# EADDRINUSE that killed the 2.0.26 push (the four-architecture
# publishes the family once shipped never raced the window; the fifth
# leg the 2.0.16 index added finally widened the parallelism into it).
# --network=none hands every leg its own loopback-only netns, so the
# runner binds and the probe connects inside the leg alone and no
# sibling leg can ever claim the port — the probe stays a pure
# localhost fetch, exactly the contract the healthcheck keeps.
