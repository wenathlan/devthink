...
export function artifactmanifestof(input: {
  version: string;
  artifacts: Array<{ name: string; size: number; checksum: string }>;
}): artifactmanifestdocument {
  if (!/^\d+\.\d+\.\d+(?:-[0-9A-Za-z.-]+)?$/.test(input.version))
    throw new Error("The artifact manifest carries the release version; a non semver version never names a manifest.");
  if (input.artifacts.length === 0)
    throw new Error(
      "The artifact manifest lists every artifact of the release; an empty artifact set never renders a manifest.",
    );
  ...
  for (const artifact of input.artifacts) {
    ...
    if (name === "")
      throw new Error("The artifact manifest names every artifact; an empty name never enters the manifest.");
    if (seen.has(name))
      throw new Error(
        `The artifact manifest lists the artifact ${name} once; a duplicate entry never enters the manifest.`,
      );
    seen.add(name);
    if (!/^[0-9a-f]{64}$/.test(artifact.checksum))
      throw new Error(
        `The artifact manifest carries the sha256 checksum of ${name}; a non sha256 digest never enters the manifest.`,
      );
    entries.push({
      name,
      size: artifact.size,
      checksum: artifact.checksum,
      channels: artifactchannelof(name, input.version),
    });
  }
  return {
    name: "devthink artifact manifest",
    version: input.version,
    banner: `devthink ${input.version} artifact manifest — GPL-3.0-only — the deterministic inventory of the publishing pipeline`,
...
export function artifactmanifestnameof(version: string): string {
  if (!/^\d+\.\d+\.\d+(?:-[0-9A-Za-z.-]+)?$/.test(version))
    throw new Error(
      "The artifact manifest name carries the release version; a non semver version never names an artifact.",
    );
...
export function artifactmanifestcheck(input: {
  manifest: artifactmanifestdocument;
  files: Array<{ name: string; checksum: string }>;
  ignored?: string[];
}): { ok: boolean; missing: string[]; unexpected: string[]; mismatched: string[] } {
...
export function rollbackpinnedset(input: { previousversion: string; manifest: artifactmanifestdocument }): {
  tag: string;
  version: string;
  artifacts: Array<{ name: string; checksum: string; channels: string[] }>;
  procedure: string[];
} {
  if (!/^\d+\.\d+\.\d+(?:-[0-9A-Za-z.-]+)?$/.test(input.previousversion))
    throw new Error("The rollback pin carries the previous release version; a non semver version never names a pin.");
  if (input.manifest.version !== input.previousversion)
    throw new Error(
      "The rollback pin freezes the previous release artifact set; the manifest version must match the pinned version.",
    );
  return {
    tag: `v${input.previousversion}`,
    version: input.previousversion,
    artifacts: input.manifest.artifacts.map((entry) => ({
      name: entry.name,
...
export function sbomcomponentof(artifact: { name: string; size: number; checksum: string; channels?: string[] }): {
  type: "file";
  name: string;
  ...
  if (!/^[0-9a-f]{64}$/.test(artifact.checksum))
    throw new Error(
      `The sbom component ${name} carries the sha256 checksum of its artifact; a non sha256 digest never enters the inventory.`,
    );
  return {
    type: "file",
    name,
...
export function sbominventory(input: sbominventoryinput): {
  bomformat: "CycloneDX";
  specversion: string;
  version: number;
  ...
  if (!/^\d+\.\d+\.\d+(?:-[0-9A-Za-z.-]+)?$/.test(input.version))
    throw new Error("The sbom inventory carries the release version; a non semver version never names an inventory.");
  if (input.artifacts.length === 0)
    throw new Error(
      "The sbom inventory covers every artifact of the release; an empty artifact set never renders an inventory.",
    );
  const seen = new Set<string>();
  for (const artifact of input.artifacts) {
    const name = artifact.name.trim();
    if (seen.has(name))
      throw new Error(
        `The sbom inventory lists the artifact ${name} once; a duplicate entry never enters the inventory.`,
      );
    seen.add(name);
  }
  return {
...
export function sbomnameof(version: string): string {
  if (!/^\d+\.\d+\.\d+(?:-[0-9A-Za-z.-]+)?$/.test(version))
    throw new Error("The sbom name carries the release version; a non semver version never names an artifact.");
...
export function sbomcoveragecheck(input: {
  inventory: { components: Array<{ name?: string; hashes?: Array<{ alg?: string; content?: string }> }> };
...
export function vsixnameof(version: string): string {
  if (!/^\d+\.\d+\.\d+(?:-[0-9A-Za-z.-]+)?$/.test(version))
    throw new Error("The vsix name carries the release version; a non semver version never names an artifact.");
...
export function vsixpackmanifestfields(version: string): vsixmanifestfields {
  if (!/^\d+\.\d+\.\d+(?:-[0-9A-Za-z.-]+)?$/.test(version))
    throw new Error("The vs code manifest carries the release version; a non semver version never names a vsix.");
  return {
    name: "devthink",
    ...
    version,
    description:
      "Consent-first browser agent bridge: the chatbridge surface and the library inside a vs code webview panel.",
    ...
    configuration: [
      {
        ...
        default: "",
        description:
          "The relay url of your own socket relay; an empty value keeps the webview offline and no url is ever assumed.",
      },
    ],
    telemetry: "off",
    relayurldefault: "",
      ...
      contributes: {
        ...
        configuration: {
          ...
          properties: Object.fromEntries(
            fields.configuration.map((entry) => [
              entry.key,
              { type: "string", default: entry.default, description: entry.description },
            ]),
          ),
        },
      },
...
export function vsixvsixmanifest(fields: vsixmanifestfields, entries: string[]): string {
  const files = entries
    .map(
      (entry) =>
        `    <Asset Type="${entry.endsWith(".js") ? "Microsoft.VisualStudio.Services.VSIXPackage" : "Microsoft.VisualStudio.Code.File"}" Path="${entry}" />`,
    )
    .join("\n");
...
export function vsixpackassemble(input: vsixpackinput): vsixpackoutput {
  ...
  if (!/^\d+\.\d+\.\d+(?:-[0-9A-Za-z.-]+)?$/.test(version))
    throw new Error("The vsixpack assemble carries the release version; a non semver version never names a vsix.");
  if (input.esmbundlebytes.length === 0)
    throw new Error(
      "The vsixpack assemble reuses the library esm build inside the webview; an empty bundle never ships a vsix.",
    );
  ...
  const manifest = vsixvsixmanifest(
    fields,
...
export function vsixmarketplacemetadatacheck(input: { fields: vsixmanifestfields; manifesttext: string }): {
  ok: boolean;
  reason?: string;
} {
  ...
  if (fields.name.trim() === "")
    return {
      ok: false,
      reason: "The vs code manifest carries the extension name; an empty name never registers an extension.",
    };
  if (fields.publisher.trim() === "")
    return {
      ok: false,
      reason: "The vs code manifest carries the publisher; an empty publisher never registers an extension.",
    };
  if (!/^\d+\.\d+\.\d+(?:-[0-9A-Za-z.-]+)?$/.test(fields.version))
    return {
      ok: false,
      reason: "The vs code manifest carries the release version; a non semver version never registers an extension.",
    };
  if (fields.engines.vscode === undefined || fields.engines.vscode.trim() === "")
    return {
      ok: false,
      reason: "The vs code manifest carries the engine floor; an engine-less extension never installs.",
    };
  if (fields.telemetry !== "off")
    return { ok: false, reason: "The vs code extension declares no telemetry; the manifest statement must read off." };
  if (fields.relayurldefault !== "")
    return {
      ok: false,
      reason: "The vs code extension declares no network default; the relay url default must stay empty.",
    };
  if (fields.commands.length === 0)
    return {
      ok: false,
      reason: "The vs code manifest declares the extension commands; an empty command list never registers a surface.",
    };
  const text = input.manifesttext;
  if (/marketplace\.visualstudio\.com|clients2\.google\.com|visualstudio\.gallery/i.test(text))
    return {
      ok: false,
      reason:
        "The vs code manifest names no vendor marketplace or store endpoint; the package installs from the release asset.",
    };
  const urlmatches = text.match(/https?:\/\/[^\s"]+/gi) ?? [];
  if (urlmatches.some((url) => url.includes("download") || url.includes("installer") || url.includes("update")))
    return {
      ok: false,
      reason: "The vs code manifest carries no download url of any kind; the release page provides the package.",
    };
  return { ok: true };
}
...
export function mavenattachedartifacts(version: string): mavenattachedartifact[] {
  if (!/^\d+\.\d+\.\d+(?:-[0-9A-Za-z.-]+)?$/.test(version))
    throw new Error(
      "The maven attached artifacts carry the release version; a non semver version never names an artifact.",
    );
  return [
...
export function mavenpacklayout(version: string): {
  version: string;
  resources: string[];
  ...
  if (!/^\d+\.\d+\.\d+(?:-[0-9A-Za-z.-]+)?$/.test(version))
    throw new Error("The maven layout carries the release version; a non semver version never names an artifact.");
  return {
    version,
    resources: mavenjarresources(),
    attached: mavenattachedartifacts(version),
    metadata: mavenpackinfo(),
  };
}
...
export function mavenpackpomtext(version: string): string {
  ...
  if (!/^\d+\.\d+\.\d+(?:-[0-9A-Za-z.-]+)?$/.test(version))
    throw new Error("The pom carries the release version; a non semver version never names an artifact.");
  ...
  const includes = mavenjarresources()
...
export function nugetcontententries(version: string): nugetcontententry[] {
  if (!/^\d+\.\d+\.\d+(?:-[0-9A-Za-z.-]+)?$/.test(version))
    throw new Error(
      "The nuget content entries carry the release version; a non semver version never names an artifact.",
    );
  return [
...
export function nugetdeclarationentries(): nugetcontententry[] {
  return ["index", "policy", "protocol", "memory", "progress", "cli", "headless", "mcp", "wsbridge", "nativehost"].map(
    (entry) => ({ source: `dist/${entry}.d.ts`, contentpath: `contentFiles/any/any/declarations/${entry}.d.ts` }),
  );
}
...
export function nugetfixtureentries(): nugetcontententry[] {
  return [
    {
      source: "dist/fixtures/example-org-pagestate.json",
      contentpath: "contentFiles/any/any/fixtures/example-org-pagestate.json",
    },
    {
      source: "dist/fixtures/plans/release-notes-plan.json",
      contentpath: "contentFiles/any/any/fixtures/plans/release-notes-plan.json",
    },
    {
      source: "dist/fixtures/plans/form-inventory-plan.json",
      contentpath: "contentFiles/any/any/fixtures/plans/form-inventory-plan.json",
    },
...
export function nugetpackinfo(): nugetpackmetadata {
  return {
    ...
    readme: "README.md",
    description:
      "Distribution package containing the Devthink library bundles, the cli, headless and mcp entries, the declaration files for ide integration, the browser extension ZIPs and the sample fixtures.",
...
export function nugetpacklayout(version: string): {
  ...
  if (!/^\d+\.\d+\.\d+(?:-[0-9A-Za-z.-]+)?$/.test(version))
    throw new Error("The nuget layout carries the release version; a non semver version never names an artifact.");
  return {
    version,
    content: nugetcontententries(version),
    declarations: nugetdeclarationentries(),
    fixtures: nugetfixtureentries(),
    metadata: nugetpackinfo(),
  };
}
  ...
  return [
    {
      name: "deps",
      purpose:
        "installs the pinned node base with the bun version the packageManager field of package.json pins, zip and unzip, and the frozen dependency tree as its own cacheable layer",
    ...
    },
    {
      name: "binary-builder",
      purpose:
        "compiles devthink.ts with bun build --compile aimed at the bun target resolved from TARGETARCH (bun-linux-x64 on amd64, bun-linux-arm64 on arm64)",
      checks: ["bun run build.ts --target <bun-target>"],
    },
    {
      name: "binary-runtime",
      purpose:
        "copies the single compiled binary onto the distroless non-root base (the explicit --target binary-runtime image)",
      checks: ["the compiled binary answers --version"],
    },
    {
      name: "runtime",
      purpose:
        "copies the lean output onto the plain node base, embeds the self hosting runner as a heredoc copy and runs the smoke boot with server death detection — the last stage of the file, so it is the default build target",
...
export function containerbuildchecks(): string[] {
  return [
    "node dist/cli.js manifest",
    "node dist/cli.js headless dist/fixtures/plans/release-notes-plan.json --fixtures dist/fixtures",
  ];
}
...
export function containerexposedsurfaces(): containerexposedsurface[] {
  return [
    { name: "static site", kind: "site", bindenv: "DEVTHINK_HTTP_BIND", portenv: "DEVTHINK_HTTP_PORT" },
    {
      name: "socket relay",
      kind: "relay",
      bindenv: "DEVTHINK_HTTP_BIND",
      portenv: "DEVTHINK_HTTP_PORT",
      path: "DEVTHINK_RELAY_PATH",
    },
    {
      name: "mcp server",
      kind: "mcp",
      bindenv: "DEVTHINK_MCP_BIND",
      portenv: "DEVTHINK_MCP_PORT",
      path: "DEVTHINK_MCP_PATH",
    },
  ];
}
...
export function containerimagetags(version: string): string[] {
  if (!/^\d+\.\d+\.\d+(?:-[0-9A-Za-z.-]+)?$/.test(version))
    throw new Error("The image tags carry the release version; a non semver version never tags an image.");
  return [version];
}
...
export function containerdigestfiles(version: string): Array<{ name: string; content: string }> {
  if (!/^\d+\.\d+\.\d+(?:-[0-9A-Za-z.-]+)?$/.test(version))
    throw new Error("The digest files carry the release version; a non semver version never names a digest.");
  return [
    ...
    {
      name: "devthink-container.json",
      content: '{"image":"ghcr.io/wenathlan/devthink:VERSION","digest":"sha256:IMAGE"}',
    },
  ];
}
