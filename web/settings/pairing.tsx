      ...
      <details className="pairing-dock__surface" open={invitationDetected || paired}>
        ...
        {paired ? (
          <div className="pairing-dock__active">
            ...
            <button type="button" onClick={() => onIdentityChange(requestedUserId)}>
              save public id
            </button>
            <div className="pairing-dock__preferences">
              <label>
                theme
                <select value={preferences.theme} onChange={(event) => onPreferenceChange("theme", event.target.value)}>
              ...
              <label>
                ...
                <select
                  ...
                  onChange={(event) => onPreferenceChange("railMode", event.target.value)}
                >
              ...
              <label>
                ...
                <select
                  ...
                  onChange={(event) => onPreferenceChange("interfaceZoom", event.target.value)}
                >
          ...
          <form onSubmit={onSubmit}>
            {invitationDetected ? (
              <small>CLI invitation loaded. The connection is attempted automatically and can be retried here.</small>
            ) : (
              <small>
                Create an invitation with <code>devthink pair create</code>, then open its generated link.
              </small>
            )}
            <button type="submit">{invitationDetected ? "retry local connection" : "connect local workspace"}</button>
            <details className="pairing-dock__advanced">
