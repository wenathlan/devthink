  ...
  return (
    <main className="terminal-entry-screen">
      ...
      <header className="entry-nav">
        <div className="entry-nav__state">
          <i aria-hidden="true" />
          {paired ? `paired as ${userId || "local user"}` : "pair local CLI to sync"}
        </div>
      </header>
      ...
      <form
        ...
        onSubmit={(event) => {
          ...
          if (canOpen) onCreate(label.trim());
        }}
      >
        <p className="entry-hero__eyebrow">a local place to think through the work</p>
        <h1>DEVTHINK</h1>
        <p className="entry-hero__copy">
          Begin with one intention. Pair the local CLI to use the same person, sessions, tabs and preferences in this
          browser.
        </p>
        {invitationDetected && (
          <p className="entry-hero__invitation">A one-time local pairing invitation is ready for this browser.</p>
        )}
        <label className="entry-command">
          <span className="entry-command__prompt" aria-hidden="true">
            ›_
          </span>
          <input
            value={label}
        ...
        <div className="entry-suggestions" aria-label="Sugestões locais">
          ...
          <button type="button" onClick={() => setLabel("Review provider readiness")}>
            review providers
          </button>
          <button type="button" onClick={() => setLabel("Continue the active session")}>
            continue session
          </button>
        </div>
        <button type="button" className="entry-hero__quiet" onClick={() => onCreate("")}>
          open an unpaired local preview <span aria-hidden="true">→</span>
        </button>
      </form>
      ...
      <footer className="entry-footer">
        ...
        <span>
          {paired ? "shared local identity" : "pairing required for shared state"} · tab to navigate · ⌘K commands
        </span>
      </footer>
    </main>
  );
