...
async function renderheartbeatstate(): Promise<void> {
  ...
  if (!node) return;
  try {
    const view = await request({ kind: "resilience", view: true }) as { runs: Array<{ runid: string; state: string; updatedat: number; heartbeat?: number; beats?: number }>; window: number };
    const running = view.runs.filter(run => run.state === "running");
    if (running.length === 0) { node.textContent = "Heartbeat: no running run heartbeats yet."; delete node.dataset.state; return; }
    ...
    if (stale) node.dataset.state = "stale"; else delete node.dataset.state;
  } catch { node.textContent = "Heartbeat: no run heartbeats yet."; }
}
...
async function renderencryptstate(): Promise<void> {
  ...
  if (!node) return;
  try {
    const view = await request({ kind: "state", view: true }) as { memory: { items: number; encrypted: number; provenancecomplete: boolean }; encryptrest: boolean };
  ...
  } catch { node.textContent = "Encryption: no memory workspace yet."; delete node.dataset.state; }
}
...
async function renderfleetstate(): Promise<void> {
  ...
  if (!node) return;
  try {
    const view = await request({ kind: "fleet", view: true }) as { overview: { agents: number; active: number; paused: number; stopped: number; openescalations: number; openreviews: number; lastkillswitchat?: number } };
  ...
  } catch { node.textContent = "Fleet: no agents."; delete node.dataset.state; }
}
...
async function renderworkstate(): Promise<void> {
  ...
  if (!node) return;
  try {
    const view = await request({ kind: "work", view: true }) as { workersperorigin: Array<{ origin: string; count: number }>; suggestions: Array<{ origin: string; suggestion: string; reason: string }> };
    if (view.workersperorigin.length === 0) { node.textContent = "Workers: none."; delete node.dataset.state; return; }
  ...
  } catch { node.textContent = "Workers: none."; delete node.dataset.state; }
}
...
async function renderpipelinestate(): Promise<void> {
  ...
  if (!node) return;
  try {
    const view = await request({ kind: "pipeline", view: true }) as { state: { running: number; paused: number; finished: number }; progress: Array<{ rows: number; bytes: number; state: string }> };
    const active = view.state.running + view.state.paused;
    if (active === 0 && view.state.finished === 0) { node.textContent = "Pipelines: none."; delete node.dataset.state; return; }
    node.textContent = `Pipelines: ${view.state.running} running · ${view.state.paused} paused · ${view.state.finished} finished${view.progress.some(entry => entry.state !== "finished") ? ` · ${view.progress.filter(entry => entry.state !== "finished").reduce((total, entry) => total + entry.rows, 0).toLocaleString()} row${view.progress.filter(entry => entry.state !== "finished").reduce((total, entry) => total + entry.rows, 0) === 1 ? "" : "s"} streamed` : ""}.`;
    if (active > 0) node.dataset.state = "active";
    else delete node.dataset.state;
  } catch { node.textContent = "Pipelines: none."; delete node.dataset.state; }
}
...
async function rendertransportstate(): Promise<void> {
  ...
  if (!node) return;
  try {
    const view = await request({ kind: "webapi", view: true }) as { subscriptions: Array<{ state: string }>; graphqlchannels: Array<{ completed: boolean }>; ratelimits: Array<{ origin: string }>; cache: Array<{ hits: number }> };
    const open = view.subscriptions.filter(record => record.state === "open").length + view.graphqlchannels.filter(channel => !channel.completed).length;
    const hits = view.cache.reduce((total, entry) => total + entry.hits, 0);
    if (open === 0 && view.ratelimits.length === 0 && hits === 0) { node.textContent = "Transports: none open."; delete node.dataset.state; return; }
  ...
  } catch { node.textContent = "Transports: none open."; delete node.dataset.state; }
}
...
async function rendervisionstate(): Promise<void> {
  ...
  if (!node) return;
  try {
    const view = await request({ kind: "vision", view: true }) as { ocrs: Array<{ words: number }>; visions: Array<{ regions: number }>; cachehits: number; cost: { modelcalls: number; calls: number }; settings: { visionmodel?: string; visionendpoint?: string } };
    const words = view.ocrs.reduce((total, record) => total + record.words, 0);
    if (view.ocrs.length === 0 && view.visions.length === 0) {
      node.textContent = view.settings.visionmodel !== undefined ? `Vision: model ${view.settings.visionmodel} configured, none read yet.` : "Vision: no model configured.";
  ...
  } catch { node.textContent = "Vision: no model configured."; delete node.dataset.state; }
}
...
async function renderforensicsstate(): Promise<void> {
  ...
  if (!node) return;
  try {
    const view = await request({ kind: "forensics", view: true }) as { pairs: Array<{ stepkind: string }>; consoleentries: Array<{ level: string }>; netentries: Array<{ status: number }>; diffs: Array<{ regression: boolean; score: number }>; lastdiffscore?: number };
    ...
    if (view.pairs.length === 0 && view.consoleentries.length === 0 && view.netentries.length === 0 && view.diffs.length === 0) { node.textContent = "Forensics: none."; delete node.dataset.state; return; }
  ...
  } catch { node.textContent = "Forensics: none."; delete node.dataset.state; }
}
...
async function renderbridgestate(): Promise<void> {
  ...
  if (!node) return;
  try {
    const view = await request({ kind: "bridge", view: true }) as { status: { status: string; paired: boolean; queued: number } };
    node.textContent = bridgestatuslabel(view.status);
    if (view.status.status === "connected") node.dataset.state = "active";
    else delete node.dataset.state;
  } catch { node.textContent = "Bridge: disabled (no relay url set)."; delete node.dataset.state; }
}
...
async function rendertelemetrystate(): Promise<void> {
  ...
  if (!node) return;
  try {
    const view = await request({ kind: "minimization", view: true }) as { telemetry: { enabled: boolean; counters: string }; inventory: Array<{ key: string; dataclass: string; size: number; records: number }> };
    ...
    delete node.dataset.state;
  } catch { node.textContent = "Telemetry: off by default and by construction."; delete node.dataset.state; }
}

async function renderperfstate(): Promise<void> {
  if (!perfstatenode) return;
  try {
    const view = await request({ kind: "perf", view: true }) as { queue: { depth: number; deferred: number; peak: number; configured?: number }; startup: { prewarmed: string[]; lazy: string[]; budget?: number; over: boolean } };
...
killswitchbutton?.addEventListener("click", async () => { try { await request({ kind: "fleet", killswitch: { reason: "The popup killswitch button." } }); status("The killswitch engaged: every fleet record stopped, every agent run cancelled and every queue cleared; the switch stays user triggered only."); await restore(); } catch (error) { status(error instanceof Error ? error.message : String(error), true); } });
...
type migrationpromptnotice = { promptedat: number; previousversion: string; command: string; migrationpromptdismissed?: boolean; dismissedat?: number };
...
export function v1sunsetbannercard(state: v1sunsetnotice | undefined): { title: string; body: string; guide: string; command: string; dismiss: string; visible: boolean } {
  return {
...
export function migrationpromptcard(state: migrationpromptnotice | undefined): { title: string; body: string; guide: string; command: string; migrate: string; dismiss: string; visible: boolean } {
  return {
...
export function renderv1sunsetbanner(state: v1sunsetnotice | undefined): void {
  ...
  if (!card.visible) { sunsetbannersection.remove(); return; }
  ...
  dismiss.addEventListener("click", () => { void request({ kind: "surface", v1sunset: { dismiss: true } }).then(() => renderv1sunsetbanner(undefined)).catch(error => status(error instanceof Error ? error.message : String(error), true)); });
...
export function rendermigrationpromptbanner(state: migrationpromptnotice | undefined): void {
  ...
  if (!card.visible) { migrationbannersection.remove(); return; }
  ...
  migrate.addEventListener("click", () => { void request({ kind: "surface", migrationprompt: { migrate: true } }).then(() => status(`The migration command: ${card.command}. The guide at ${card.guide} records the full path.`)).catch(error => status(error instanceof Error ? error.message : String(error), true)); });
  const dismiss = document.createElement("button");
  dismiss.type = "button";
  dismiss.className = "secondary";
  dismiss.textContent = card.dismiss;
  dismiss.addEventListener("click", () => { void request({ kind: "surface", migrationprompt: { dismiss: true } }).then(() => rendermigrationpromptbanner(undefined)).catch(error => status(error instanceof Error ? error.message : String(error), true)); });
...
async function renderrecenttray(): Promise<void> {
  ...
  try {
    const result = await request({ kind: "views", recent: { list: true } }) as { tray?: Array<{ runid: string; origin: string; outcome: string; title: string; at: number; resumable: boolean; reopenable: boolean }> };
    const tray = result.tray ?? [];
    recenttraynode.replaceChildren();
    if (tray.length === 0) recenttraynode.textContent = "No run yet; the tray lists the latest runs with resume and reopen.";
    else {
    ...
    for (const entry of tray) {
      ...
      if (entry.resumable) {
        ...
        resume.addEventListener("click", () => { void (async () => { await request({ kind: "sessions", grid: { resume: { runid: entry.runid } } }); status(`The run ${entry.runid} resumed through the sessions seam.`); })().catch(error => status(error instanceof Error ? error.message : String(error), true)); });
        item.append(" ", resume);
      }
      if (entry.reopenable) {
        ...
        reopen.addEventListener("click", () => { void (async () => { await request({ kind: "sessions", grid: { reopen: { runid: entry.runid } } }); status(`The run ${entry.runid} reopened through the sessions seam.`); })().catch(error => status(error instanceof Error ? error.message : String(error), true)); });
        item.append(" ", reopen);
      }
      list.append(item);
    }
  ...
  } catch (error) { recenttraynode.textContent = error instanceof Error ? error.message : String(error); }
  try {
    const background = await request({ kind: "ecosystem", background: { list: true } }) as { tray: Array<{ runid: string; title: string; outcome: string }> };
    if (background.tray.length > 0) {
      ...
      for (const entry of background.tray.slice(0, 3)) backgroundlist.append(Object.assign(document.createElement("li"), { textContent: `${entry.outcome} — ${entry.title}` }));
      recenttraynode.append(backgroundlist);
    }
  } catch { /* a failing background tray read keeps the foreground tray rendered */ }
}
...
async function renderquickactions(): Promise<void> {
  ...
  try {
    const result = await request({ kind: "views", quickaction: { list: {}, surface: true } }) as { actions?: Array<{ id: string; label: string; command: string }> };
    const actions = result.actions ?? [];
    quickactionsnode.replaceChildren();
    if (actions.length === 0) { quickactionsnode.textContent = "No quickaction registers on this tab; grant its origin and the permitted actions surface."; return; }
    const list = document.createElement("ul");
    list.className = "audit";
    for (const action of actions) {
      ...
      run.textContent = "Run";
      run.addEventListener("click", () => { void (async () => { if (action.command === "opendashboardpage") { await chrome.tabs.create({ url: chrome.runtime.getURL("dashboardpage.html") }); status("The dashboardpage opened in a new tab."); } else { await busaction("starttask"); status(`The ${action.id} quickaction routed through the command bus.`); } })().catch(error => status(error instanceof Error ? error.message : String(error), true)); });
      item.append(" ", run);
      list.append(item);
    }
    quickactionsnode.append(list);
  } catch (error) { quickactionsnode.textContent = error instanceof Error ? error.message : String(error); }
}
...
async function renderclosedtabs(): Promise<void> {
  ...
  try {
    const view = await request({ kind: "nav", view: true }) as { closedtabs?: Array<{ id: string; url: string; title: string; closedat: number; reopenedat?: number }> };
    const records = view.closedtabs ?? [];
    closedtabsnode.replaceChildren();
    if (records.length === 0) { closedtabsnode.textContent = "No closed tab yet; the records stay under the user retention window."; return; }
    const list = document.createElement("ul");
    list.className = "audit";
    for (const record of records.slice(0, 6)) {
      ...
      if (record.reopenedat === undefined) {
        ...
        reopen.className = "secondary";
        reopen.textContent = "Reopen";
        reopen.addEventListener("click", () => { void (async () => { await request({ kind: "nav", reopen: { id: record.id } }); status(`The closed tab ${record.url} reopened after the grant recheck.`); await renderclosedtabs(); })().catch(error => status(error instanceof Error ? error.message : String(error), true)); });
        item.append(" ", reopen);
      }
      list.append(item);
    }
    closedtabsnode.append(list);
  } catch (error) { closedtabsnode.textContent = error instanceof Error ? error.message : String(error); }
}
