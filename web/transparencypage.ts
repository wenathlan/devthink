...
async function render(): Promise<void> {
  ...
  if (permissionsroot) {
    ...
    if ((security.permissions ?? []).length === 0)
      permissionsroot.append(line("No permission was declared; the manifest requests nothing."));
    for (const permission of security.permissions ?? [])
      permissionsroot.append(
        line(
          `${permission.permission} — ${permission.state} — consumed by the ${permission.surface} surface through ${permission.messages} message${permission.messages === 1 ? "" : "s"} and ${permission.kinds} action kind${permission.kinds === 1 ? "" : "s"}`,
        ),
      );
    permissionsroot.append(
      document.createElement("hr"),
      button("Export transparency report", async () => {
        const answer = await request<{ maskedfields: string[]; redactedregions: number; reason: string }>({
          kind: "transparency",
  ...
  if (datakindsroot) {
    ...
    if ((minimization.inventory ?? []).length === 0)
      datakindsroot.append(line("No stored data kind reports inventory; nothing left the device."));
    for (const kind of minimization.inventory ?? []) {
      const row = line(
        `${kind.dataclass} — ${kind.records} record${kind.records === 1 ? "" : "s"} of ${kind.size} bytes at the local storage key ${kind.key}`,
      );
      const typed = field("type the purge confirmation");
      row.append(
        " ",
        button(`Purge ${kind.dataclass}`, async () => {
          await request({ kind: "minimization", purge: { scope: [kind.dataclass], typed: typed.value.trim() } }).catch(
            (error) => reportstatus(error instanceof Error ? error.message : String(error)),
          );
          typed.value = "";
        }),
        " ",
    ...
    datakindsroot.append(
      document.createElement("hr"),
      button("Export all stored data in one archive", async () => {
  ...
  if (integrityroot) {
    ...
    if ((security.chain ?? []).length === 0)
      integrityroot.append(line("No run log exists yet; the audit trail integrity check has nothing to verify."));
    for (const chain of security.chain ?? [])
      integrityroot.append(
        line(
          chain.valid
            ? `The run log ${chain.runid} verifies: ${chain.entries} chained entries${chain.sealhash !== undefined ? ` sealed at ${new Date(chain.sealedat ?? 0).toLocaleString()} with the seal hash ${chain.sealhash.slice(0, 16)}` : ""}.`
            : `The run log ${chain.runid} broke at entry ${chain.brokenat ?? 0}: ${chain.reason}`,
        ),
      );
  }
  ...
  if (grantsroot) {
    ...
    if (report.grants.length === 0)
      grantsroot.append(line("No active grant exists; the denydefault posture refuses every origin."));
    for (const grant of report.grants) {
      const row = line(
        `${grant.origin} — ${grant.scope} — boundary ${grant.boundary} — granted ${new Date(grant.grantedat).toLocaleString()}`,
      );
      row.append(
        " ",
        button("Revoke allowlist grant", async () => {
          await request({ kind: "security", allowlist: { remove: { origin: grant.origin } } });
        }),
      );
  ...
  if (windowsroot) {
    ...
    for (const window of report.windows)
      windowsroot.append(
        line(
          `${window.origin} — ${window.state} — boundary ${window.boundary} — started ${new Date(window.startedat).toLocaleString()} — expiry ${new Date(window.expiresat).toLocaleString()}`,
        ),
      );
  }
  ...
  if (sendersroot) {
    ...
    if ((security.connectallow ?? []).length === 0)
      sendersroot.append(
        line("The connectallow list ships empty by default; every unknown sender drops without handler execution."),
      );
    for (const entry of security.connectallow ?? []) {
      const row = line(
        `${entry.displayname} (${entry.senderid})${entry.origin !== undefined ? ` of ${entry.origin}` : ""} — allowed ${new Date(entry.addedat).toLocaleString()}`,
      );
      row.append(
        " ",
        button("Remove sender", async () => {
          await request({ kind: "security", connectallow: { remove: { senderid: entry.senderid } } });
        }),
      );
    ...
    sendersroot.append(
      ...
      displayname,
      origin,
      button("Allow external sender", async () => {
        await request({
          kind: "security",
          connectallow: {
            add: {
              ...
              ...(origin.value.trim() !== "" ? { origin: origin.value.trim() } : {}),
            },
          },
        });
      }),
  ...
  if (permdiffsroot) {
    ...
    for (const diff of report.permdiffs)
      permdiffsroot.append(
        line(
          `${diff.fromversion} → ${diff.toversion} — added ${diff.added.length > 0 ? diff.added.join(", ") : "nothing"} — removed ${diff.removed.length > 0 ? diff.removed.join(", ") : "nothing"} — ${new Date(diff.computedat).toLocaleString()}`,
        ),
      );
  }
  ...
  if (safedefaultsroot) {
    ...
    if (report.safedefaults.length === 0)
      safedefaultsroot.append(line("No unknown origin received the safedefaults profile yet."));
    for (const application of report.safedefaults)
      safedefaultsroot.append(
        line(
          `${application.origin} — first seen ${new Date(application.firstseenat).toLocaleString()} — reads only, every sensitive class denied`,
        ),
      );
  }
  ...
  if (vaultroot) {
    ...
    if (report.vault.length === 0)
      vaultroot.append(line("The secretvault holds no entry; values live behind the vault seam and never persist."));
    for (const entry of report.vault) {
      const row = line(
        `${entry.label} — ${entry.scope} — ${entry.provenance} — stored ${new Date(entry.createdat).toLocaleString()}${entry.lastusedat !== undefined ? ` — last used ${new Date(entry.lastusedat).toLocaleString()}` : ""} — the value never displays or exports`,
      );
      row.append(
        " ",
        button("Delete", async () => {
          await request({ kind: "security", vault: { delete: { vaultid: entry.vaultid } } });
        }),
      );
    ...
    vaultroot.append(
      ...
      scope,
      value,
      button("Store secret", async () => {
        await request({
          kind: "security",
          vault: { add: { label: label.value.trim(), scope: scope.value.trim(), value: value.value } },
        });
        value.value = "";
      }),
    );
  ...
  if (ratelimitroot) {
    ...
    if ((security.deferred ?? []).length > 0)
      for (const deferred of security.deferred ?? [])
        ratelimitroot.append(
          line(
            `Deferred: the ${deferred.kind} step ${deferred.stepid} on ${deferred.origin} waits for the bucket reset at ${new Date(deferred.resetsat).toLocaleTimeString()}.`,
          ),
        );
    ...
    ratelimitroot.append(
      ...
      limit,
      window,
      button("Configure bucket", async () => {
        await request({
          kind: "security",
          ratelimit: {
            set: {
              origin: origin.value.trim(),
              limit: Number(limit.value.trim()),
              window: Number(window.value.trim()),
            },
          },
        });
      }),
      " ",
      button("Remove bucket", async () => {
        await request({ kind: "security", ratelimit: { remove: { origin: origin.value.trim() } } });
      }),
    );
  }
  ...
  if (phishroot) {
    ...
    if ((security.phishverdicts ?? []).length === 0)
      phishroot.append(
        line("No phishguard verdict exists yet; a credential step on a new origin records the first one."),
      );
    for (const verdict of security.phishverdicts ?? [])
      phishroot.append(
        line(
          `${verdict.origin}${verdict.matchedorigin !== undefined ? ` sits ${verdict.distance} from ${verdict.matchedorigin}` : ` sits ${verdict.distance} from the closest granted origin`} under the threshold ${verdict.threshold}${verdict.blocked ? " — blocked" : ""}.`,
        ),
      );
  ...
  if (redactroot) {
    ...
    if ((security.redactregions ?? []).length === 0)
      redactroot.append(
        line(
          "No redactshot region exists yet; draw one and the capture seam masks it across viewport, element and stitched captures.",
        ),
      );
    for (const region of security.redactregions ?? []) {
      const row = line(
        `${region.origin}/${region.template} — ${region.x},${region.y} of ${region.width}x${region.height} — ${region.source} — ${region.reason}`,
      );
      row.append(
        " ",
        button("Remove region", async () => {
          await request({ kind: "security", redact: { remove: { id: region.id } } });
        }),
      );
    ...
    redactroot.append(
      line(
        "Draw one region with its origin, page template, geometry and reason; the capture seam masks the regions of that origin and template across viewport, element and stitched captures.",
      ),
    );
    const origin = field("origin");
        ...
        await request({
          ...
          redact: {
            add: {
              ...
              width: Number(width.value.trim()),
              height: Number(height.value.trim()),
...
function rendersessionoptions(view: sessionoptionsview): void {
  ...
  root.append(
    line(
      `Recall index window: ${view.recallwindow !== undefined ? `${view.recallwindow} milliseconds` : "the whole index stays live"}. Summary length window: ${view.summarywindow !== undefined ? `${view.summarywindow} steps` : "no cap"}. Historysearch index: ${view.historyindex !== false ? "on" : "off"}. Cancelrun rollback: ${view.cancelrollback === "none" ? "none" : "queued"}.`,
    ),
  );
  ...
  root.append(
    ...
    historylabel,
    cancelrollback,
    button("Save session preferences", async () => {
      await request({
        kind: "sessions",
        settings: {
          ...
          ...(scratchretention.value.trim() !== ""
            ? { scratchpadretention: Number(scratchretention.value.trim()) }
            : {}),
          ...(summaryretention.value.trim() !== "" ? { summaryretention: Number(summaryretention.value.trim()) } : {}),
          ...(correctionretention.value.trim() !== ""
            ? { correctionretention: Number(correctionretention.value.trim()) }
            : {}),
...
async function rendersessionoptionsview(): Promise<void> {
  const context = await request<{
    sessionview?: { historyindex: boolean; cancelrollback?: string };
    sessionpreferences?: sessionoptionsview;
  }>({ kind: "context" });
  rendersessionoptions({
    ...(context.sessionpreferences ?? {}),
    ...(context.sessionview !== undefined
      ? {
          historyindex: context.sessionview.historyindex,
          ...(context.sessionview.cancelrollback !== undefined
            ? { cancelrollback: context.sessionview.cancelrollback }
            : {}),
        }
      : {}),
  });
