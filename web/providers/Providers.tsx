...
export default function Providers() {
  ...
  return (
    ...
    >
      ...
      <div className="control-grid control-grid--providers">
        {providers.map((provider) => (
          <article
            className={`provider-card ${active === provider.id ? "provider-card--active" : ""}`}
            key={provider.id}
          >
            <span className="provider-card__tag">{provider.protocol}</span>
            <h2>{provider.id}</h2>
            <p>
      ...
      <aside className="control-note">
        <Terminal size={16} />
        <p>
          For the recommended device flow, use <code>devthink auth login &lt;provider&gt; --token &lt;key&gt;</code>. A
          CLI credential never enters this web page.
        </p>
      </aside>
      <section className="control-note">
        <KeyRound size={16} />
        <div>
          <strong>optional browser-local credential</strong>
          <p>
            This explicit opt-in stores a credential only in this browser's IndexedDB. It is not displayed after saving,
            never goes to the CLI gateway, and is excluded from every sync adapter.
          </p>
          <label>
            provider
