...
interface nativeframe { kind: string; correlationid: string; body?: Record<string, unknown>; }
...
function logline(line: string): void {
  ...
  try {
    if (Number.isFinite(logbound) && logbound > 0) {
      ...
      } catch { /* a missing log file starts fresh */ }
    }
    appendFileSync(logpath, text);
  } catch { /* an unwritable log path never kills the companion */ }
}
...
const bridge: { port: number; token: string; idlewindow: number | undefined; started: boolean; extension: Socket | null; clients: Set<Socket>; lastframeat: number } = { port: 0, token: "", idlewindow: undefined, started: false, extension: null, clients: new Set(), lastframeat: Date.now() };
...
function bridgerelay(text: string): void {
  for (const client of bridge.clients) {
    try { client.write(encodeframe(text)); } catch { /* a closed client leaves the set on its close event */ }
  }
}
  ...
  const server = createServer(socket => {
    ...
    socket.on("data", (chunk: Buffer) => {
      ...
      for (const frame of frames) {
        ...
        try { envelope = JSON.parse(frame.payload.toString("utf8")) as { token?: unknown; role?: unknown; correlationid?: unknown }; } catch { socket.destroy(); return; }
        if (!authenticated) {
          ...
          if (token === "" || createHash("sha256").update(token).digest("hex") !== createHash("sha256").update(bridge.token).digest("hex")) {
            try { socket.write(encodeframe(JSON.stringify({ error: "The wsbridge frame token failed the session authentication." }))); } catch { /* closed */ }
            socket.destroy();
            return;
          }
          if (wantedrole === "extension") {
            if (bridge.extension !== null && bridge.extension.writable) {
              try { socket.write(encodeframe(JSON.stringify({ error: "The wsbridge session holds one extension connection at a time." }))); } catch { /* closed */ }
              socket.destroy();
              return;
            }
...
async function handleframe(frame: unknown): Promise<void> {
  if (frame === null || typeof frame !== "object" || typeof (frame as nativeframe).kind !== "string" || typeof (frame as nativeframe).correlationid !== "string" || (frame as nativeframe).correlationid === "") {
  ...
  if (typed.kind === "handshake") {
    ...
    if (bridge.idlewindow === undefined && typeof body?.idlewindow === "number" && body.idlewindow > 0) bridge.idlewindow = body.idlewindow;
    startbridge(() => {
      writeframe({ kind: "advertisement", correlationid: typed.correlationid, body: { port: bridge.port, token: bridge.token, ...(bridge.idlewindow !== undefined ? { idlewindow: bridge.idlewindow } : {}) } });
    });
