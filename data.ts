...
export function builddataset(
  ...
  grid: {
    columns: columnspec[];
    rows: datasetrow[];
    children?: Array<{ parentrow: number; selector: string; columns: columnspec[]; rows: datasetrow[] }>;
  },
  at: number,
): dataset {
...
export function exportartifact(
  ...
  return {
    id,
    kind: format,
    name: `${datasetvalue.name || datasetvalue.id}.${extension}`,
    stepid,
    rowcount: datasetvalue.rows.length,
    content,
...
export function newextractsession(
  id: string,
  datasetid: string,
  name: string,
  target: string,
...
export function provenancefor(
  artifact: { id: string; name: string; rowcount: number; checksum: string },
  url: string,
  stepid: string,
  at: number,
): provenancerecord {
  return {
    ...
    url,
    stepid,
    rowstart: artifact.rowcount > 0 ? 1 : 0,
    rowend: artifact.rowcount,
...
export function gridpreview(
  datasetvalue: dataset,
  sample: number,
): { datasetid: string; columns: string[]; rows: number; sample: datasetrow[] } {
  return {
    datasetid: datasetvalue.id,
    columns: datasetvalue.columns.map((column) => column.key),
    rows: datasetvalue.rows.length,
    sample: datasetvalue.rows.slice(0, Math.max(0, Math.floor(sample))),
  };
}
...
export function sortrows(rows: datasetrow[], key: string, direction: "asc" | "desc"): datasetrow[] {
  ...
  return [...rows].sort((left, right) => {
    ...
    if (Number.isFinite(numeric) && Number.isFinite(numericb) && a.trim() !== "" && b.trim() !== "")
      return (numeric - numericb) * sign;
    return a.localeCompare(b) * sign;
  });
}

/** Builds the sheet push payload of one dataset for a reviewed sheet endpoint. */
export function sheetpayload(
  datasetvalue: dataset,
  sheet: string,
): { sheet: string; columns: string[]; rows: datasetrow[] } {
...
export function mergetaskrules(
  existing: { taskid: string; transforms: transformrule[]; dedupekeys: string[] } | undefined,
  taskid: string,
  transforms: transformrule[],
  dedupekeys: string[],
  at: number,
): { taskid: string; transforms: transformrule[]; dedupekeys: string[]; at: number } {
  return {
    taskid,
...
function normalizekeyvalue(value: string, normalization: dedupekey["normalization"]): string {
  ...
  if (normalization === "whitespace" || normalization === "whitespace and case")
...
export function streamdisk(input: {
  pipeline: extractpipeline;
  rows: extractrow[];
  write: (chunk: extractrow[]) => number;
  cursor?: streamcursor;
  chunk?: number;
  checkpoint?: (cursor: streamcursor) => void;
  now: number;
}): { cursor: streamcursor; rowswritten: number; byteswritten: number; chunks: number; reason: string } {
  if (input.pipeline.id.trim() === "")
    throw new Error("The streamdisk pass needs its pipeline; every streamed extract carries its identity.");
  const startoffset =
    input.cursor !== undefined && input.cursor.pipelineid === input.pipeline.id ? input.cursor.offset : 0;
  const startchunk =
    input.cursor !== undefined && input.cursor.pipelineid === input.pipeline.id ? input.cursor.chunk : 0;
  if (input.chunk !== undefined && (!Number.isInteger(input.chunk) || input.chunk <= 0))
    throw new Error(
      "The stream chunk size stays a positive whole number of rows the user chose; the chunk bound never defaults in code.",
    );
  ...
  for (let index = 0; index < remaining.length; index += size) {
    ...
    cursor = {
      pipelineid: input.pipeline.id,
      offset: startoffset + rowswritten,
      chunk: startchunk + chunks,
      updatedat: input.now,
    };
  ...
  return {
    ...
    byteswritten,
    chunks,
    reason: `Streamed ${rowswritten} row${rowswritten === 1 ? "" : "s"} of the pipeline ${input.pipeline.name} to the reviewed sink in ${chunks} chunk${chunks === 1 ? "" : "s"} of ${size} row${size === 1 ? "" : "s"} with ${byteswritten} byte${byteswritten === 1 ? "" : "s"} written; only the active chunk rode the write and the cursor checkpoints after every chunk.`,
  };
}
...
export function resumeextract(input: {
  pipeline: extractpipeline;
  rows: extractrow[];
  cursor?: streamcursor;
  ...
  logid?: string;
  now: number;
}): { pipeline: extractpipeline; rows: extractrow[]; skipped: number; cursor: streamcursor; entry: provlogentry } {
  if (input.pipeline.id.trim() === "")
    throw new Error("The resume names its pipeline; an interrupted extraction carries its identity.");
  if (input.pipeline.state === "finished")
    throw new Error(`The pipeline ${input.pipeline.name} already finished; a finished extraction resumes nothing.`);
  const cursor = input.cursor ?? { pipelineid: input.pipeline.id, offset: 0, chunk: 0, updatedat: input.now };
  if (cursor.pipelineid !== input.pipeline.id)
    throw new Error(
      `The streamcursor of ${cursor.pipelineid} belongs to another pipeline; the resume continues exactly the extraction it came from.`,
    );
  const persisted = new Set(input.persistedkeys ?? []);
  const remaining: extractrow[] = [];
  let skipped = 0;
  for (const [index, row] of input.rows.entries()) {
    if (index < cursor.offset || persisted.has(row.key)) {
      skipped += 1;
      continue;
    }
    remaining.push(row);
  ...
  const entry: provlogentry = {
    id: input.logid !== undefined && input.logid.trim() !== "" ? input.logid.trim() : `${input.pipeline.id}:resume`,
    pipelineid: input.pipeline.id,
    runid: input.pipeline.runid,
    operation: "resume",
    rowkeys: remaining.map((row) => row.key),
    summary: `The pipeline ${input.pipeline.name} resumed from the streamcursor at offset ${cursor.offset} and chunk ${cursor.chunk}: ${skipped} row${skipped === 1 ? "" : "s"} already persisted skipped by their row keys and ${remaining.length} row${remaining.length === 1 ? "" : "s"} continue to the reviewed sink.`,
    at: input.now,
  };
...
export function transformvalues(input: { rows: extractrow[]; rules: transformrule[] }): {
  rows: extractrow[];
  applied: number;
  refusals: string[];
} {
  ...
  const rows = input.rows.map((row) => {
    ...
    for (const rule of input.rules) {
      ...
      if (rule.operation !== undefined && !["trim", "case", "number", "date"].includes(rule.operation)) {
        refusals.push(
          `${field}: the operation ${rule.operation} is not a reviewed transform; the raw value stays and the pipeline refuses the rule.`,
        );
        continue;
      }
      ...
      if (rule.operation === "date") {
        ...
        if (Number.isNaN(parsed)) {
          refusals.push(
            `${field}: the value ${original} parses as no date; the raw value stays beside the refused transform.`,
          );
          continue;
        }
  ...
  if (rows.length !== input.rows.length)
    throw new Error(
      "A transform pass never drops a row; every row keeps its place with the raw value beside the transformed one.",
    );
...
export function deduperows(input: { rows: extractrow[]; key: dedupekey }): {
  kept: extractrow[];
  dropped: number;
  droppedkeys: string[];
  reason: string;
  ...
  if (columns.length === 0)
    throw new Error(
      "The dedupe key names its columns; an empty key would compare whole rows the user never configured.",
    );
  ...
  for (const row of input.rows) {
    const joined = columns
      .map((column) => JSON.stringify(normalizekeyvalue(row.values[column] ?? "", input.key.normalization)))
      .join("|");
    if (seen.has(joined)) {
  ...
  return {
    ...
    droppedkeys,
    reason: `Deduplicated ${input.rows.length} row${input.rows.length === 1 ? "" : "s"} by ${columns.join(", ")} under the ${input.key.normalization} normalization: the first occurrence of every key stays and ${droppedkeys.length} duplicate${droppedkeys.length === 1 ? "" : "s"} dropped with the count reported.`,
  };
}
...
export function samplerows(input: { rows: extractrow[]; policy: samplepolicy; seed?: number }): {
  rows: extractrow[];
  strategy: samplepolicy["strategy"];
  reason: string;
} {
  if (!Number.isInteger(input.policy.rows) || input.policy.rows <= 0)
    throw new Error(
      "The sample policy names its row count as a positive whole number the user chose; the preview bound never defaults in code.",
    );
  ...
  return {
    rows: picked.map((row) => ({ ...row, values: { ...row.values } })),
    strategy: input.policy.strategy,
    reason: `Sampled ${picked.length} of ${input.rows.length} row${input.rows.length === 1 ? "" : "s"} by the ${input.policy.strategy} strategy of the user policy; the full extract stays untouched and the preview reads a copy.`,
  };
}
...
export function sourcestamp(input: { rows: extractrow[]; stampid: string; url: string; stepid: string; now: number }): {
  rows: extractrow[];
  stamp: sourcestamprecord;
} {
  if (input.stampid.trim() === "")
    throw new Error("The source stamp needs its id; every stamp carries its identity for the cell links.");
  if (input.url.trim() === "")
    throw new Error("The source stamp names its url; every extracted row answers the page it came from.");
  if (input.stepid.trim() === "")
    throw new Error("The source stamp names its step; every extracted row answers the reviewed step that captured it.");
  const stamp: sourcestamprecord = {
    id: input.stampid.trim(),
    url: input.url.trim(),
...
export function pipelinegridpreview(input: {
  rows: extractrow[];
  ...
  let rows = input.rows.map((row) => ({
    key: row.key,
    values: { ...row.values },
    stamped: row.stamps.length > 0,
    transformed: Object.keys(row.rawvalues ?? {}),
    deduplicated: deduped.has(row.key),
  }));
  if (query !== "")
    rows = rows.filter((row) => Object.values(row.values).some((value) => value.toLowerCase().includes(query)));
  if (input.sort !== undefined) {
    const column = columns.find((candidate) => candidate.name === input.sort?.column);
    if (column === undefined)
      throw new Error(
        `The grid preview knows no ${input.sort.column} column to sort; the projection sorts by the columns it observed.`,
      );
...
export function provlog(input: {
  entries: provlogentry[];
  id: string;
  pipelineid: string;
  runid: string;
  operation: provlogentry["operation"];
  rowkeys: string[];
  summary: string;
  now: number;
}): { entries: provlogentry[]; entry: provlogentry } {
  if (input.id.trim() === "")
    throw new Error("The provlog entry needs its id; every logged operation carries its identity.");
  if (input.pipelineid.trim() === "")
    throw new Error("The provlog entry names its pipeline; the provenance log stays per pipeline.");
  if (input.summary.trim() === "")
    throw new Error(
      "The provlog entry needs its summary in plain language; the audit reads exactly what the operation did.",
    );
  if (input.entries.some((entry) => entry.id === input.id.trim()))
    throw new Error(
      `The provlog entry ${input.id.trim()} already sits in the log; the provenance log stays append only and an entry never rewrites.`,
    );
  const entry: provlogentry = {
    id: input.id.trim(),
    ...
    summary: redactprovenance(input.summary.trim()),
    at: input.now,
  };
...
export function provlogquery(entries: provlogentry[], rowkey: string): provlogentry[] {
  return entries
    .filter((entry) => entry.rowkeys.includes(rowkey))
    .sort((one, two) => one.at - two.at || (one.id < two.id ? -1 : 1));
}
...
export function pipelinestate(input: { pipelines: extractpipeline[] }): {
  running: number;
  paused: number;
  finished: number;
  perrun: Array<{ runid: string; running: number; paused: number; finished: number }>;
} {
  ...
  return {
    ...
    perrun: runs.map((runid) => ({
      runid,
      running: input.pipelines.filter((pipeline) => pipeline.runid === runid && pipeline.state === "running").length,
      paused: input.pipelines.filter((pipeline) => pipeline.runid === runid && pipeline.state === "paused").length,
      finished: input.pipelines.filter((pipeline) => pipeline.runid === runid && pipeline.state === "finished").length,
    })),
  };
}
...
export function extractbatchof(input: {
  id: string;
  pipeline: extractpipeline;
  rows: extractrow[];
  cursor: streamcursor;
  ...
  if (input.id.trim() === "")
    throw new Error("The extract batch needs its id; every previewed pass carries its identity.");
...
export function streamparsewindows(input: { bytes: number; chunkbytes: number }): {
  chunks: number;
  windows: Array<{ index: number; start: number; end: number }>;
} {
  if (input.chunkbytes <= 0)
    throw new Error(
      "The streamparse chunk bound must stay a positive number of bytes; the bound stays the user's choice and never defaults inside the engine.",
    );
  const chunks = Math.ceil(input.bytes / input.chunkbytes);
  const windows = Array.from({ length: chunks }, (_, index) => ({
    index,
    start: index * input.chunkbytes,
    end: Math.min((index + 1) * input.chunkbytes, input.bytes),
  }));
...
export function streamparsechunkof(input: {
  index: number;
  text: string;
  chunkbytes: number;
  last: boolean;
}): streamparsechunk {
  if (input.text.length > input.chunkbytes)
    throw new Error(
      `The streamparse chunk ${input.index} carries ${input.text.length} bytes over the ${input.chunkbytes} byte bound; the parser never holds more than one chunk.`,
    );
...
export function validatesetreamparsechunk(chunk: unknown, expectedindex: number): { valid: boolean; reason?: string } {
  if (!Boolean(chunk) || typeof chunk !== "object" || Array.isArray(chunk))
    return {
      valid: false,
      reason: "Every streamparse chunk travels as one plain object; schemastrict refuses the chunk carrier.",
    };
  const candidate = chunk as { index?: unknown; tokens?: unknown; bytes?: unknown; complete?: unknown };
  if (typeof candidate.index !== "number" || candidate.index !== expectedindex)
    return {
      valid: false,
      reason: `The streamparse chunk must carry its index ${expectedindex}; a chunk out of order never yields observations.`,
    };
  if (!Array.isArray(candidate.tokens))
    return { valid: false, reason: "The streamparse chunk carries its tokens as an array." };
  if (typeof candidate.bytes !== "number")
    return { valid: false, reason: "The streamparse chunk carries its byte count as a number." };
  if (typeof candidate.complete !== "boolean")
    return { valid: false, reason: "The streamparse chunk carries its completion flag as a boolean." };
  return { valid: true };
}
...
export function streamparsememorybound(input: { chunkbytes: number; chunks: number }): {
  peak: number;
  fullpage: number;
  saved: number;
} {
...
export function openchunkextractcursor(input: {
  tableid: string;
  fingerprint: string;
  rows: number;
  window?: number;
  ...
  if (input.fingerprint.trim() === "")
    throw new Error(
      "The chunkextract cursor needs its table fingerprint; a resume without its fingerprint never continues.",
    );
  if (input.window !== undefined && input.window <= 0)
    throw new Error("The chunkextract row window stays a positive row count and the user's choice.");
  return {
    tableid: input.tableid,
    fingerprint: input.fingerprint,
    rowindex: 0,
    window: input.window ?? input.rows,
    complete: input.rows === 0,
  };
}
...
export function chunkextractwindow(input: {
  cursor: chunkextractcursor;
  rows: Array<Record<string, string>>;
}): chunkextractwindowresult {
  if (input.cursor.rowindex > input.rows.length)
    throw new Error("The chunkextract cursor stands past the end of its table; open a fresh cursor for a new table.");
...
export function resumechunkextract(input: { cursor: chunkextractcursor; tableid: string; fingerprint: string }): {
  resumed: boolean;
  reason: string;
} {
  ...
  if (input.cursor.fingerprint !== input.fingerprint)
    return {
      resumed: false,
      reason: `The table fingerprint changed since the cursor stopped; the resume refuses so no window mixes two table states.`,
    };
  if (input.cursor.complete)
    return {
      resumed: false,
      reason: "The chunkextract cursor already completed its table; the merge holds every window.",
    };
...
export function mergechunkwindows(windows: chunkextractwindowresult[]): {
  rows: Array<Record<string, string>>;
  duplicates: number;
} {
...
export function chunkextractprogress(
  cursor: chunkextractcursor,
  totalrows: number,
): { extracted: number; total: number; share: number; complete: boolean } {
  return {
    extracted: cursor.rowindex,
    total: totalrows,
    share: totalrows === 0 ? 1 : cursor.rowindex / totalrows,
...
export function streamparsetaskof(input: { runid: string; stepid: string; chunkbytes: number; chunks: number }): {
  task: "streamparse";
  runid: string;
  stepid: string;
  chunkbytes: number;
...
export function chunkextracttaskof(input: {
  runid: string;
  stepid: string;
  tableid: string;
  fingerprint: string;
