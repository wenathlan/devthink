...
export function errorwithretry(input: { code: rpcerrorcode | string; message: string; retryhint: retryhint; retryafter?: number }): rpcerror {
  ...
  const structured = structurederrorof({ code: input.code, message: input.message, retryhint: input.retryhint, ...(input.retryafter !== undefined ? { retryafter: input.retryafter } : {}) });
...
export function errorresponse(input: { id?: number | string | null; code: rpcerrorcode | string; message: string; retryhint: retryhint; retryafter?: number }): jsonrpcframe {
  return respond({ ...(input.id === undefined ? {} : { id: input.id }), error: errorwithretry({ code: input.code, message: input.message, retryhint: input.retryhint, ...(input.retryafter !== undefined ? { retryafter: input.retryafter } : {}) }) });
}
...
export function toolresultof(result: unknown): toolresult | undefined {
  if (result !== undefined && result !== null && typeof result === "object" && !Array.isArray(result)) {
    ...
    if (typeof candidate.content === "string" && typeof candidate.iserror === "boolean") return { content: candidate.content, iserror: candidate.iserror, ...(candidate.payload !== undefined && candidate.payload !== null && typeof candidate.payload === "object" && !Array.isArray(candidate.payload) ? { payload: candidate.payload as Record<string, unknown> } : {}) };
  }
  return undefined;
}
...
export const resourceuris = ["devthink://pagestate", "devthink://plan", "devthink://audit", "devthink://session", "devthink://health"] as const;
...
export function readresource(input: { uri: string; pagestate?: Record<string, unknown>; plan?: agentplan; audit?: auditevent[]; session?: agentsession; health?: servehealth }): { resource?: servedresource; text?: string; reason?: string } {
  ...
  if (resource.uri === "devthink://session") {
    ...
    return { resource, text: JSON.stringify({ id: input.session.id, tabid: input.session.tabid, origin: input.session.origin, startedat: input.session.startedat, expiresat: input.session.expiresat, grants: input.session.grants, ...(input.session.stoppedat !== undefined ? { stoppedat: input.session.stoppedat } : {}), ...(input.session.pausedat !== undefined ? { pausedat: input.session.pausedat } : {}) }) };
  }
...
export function subscriberesource(input: { clientid: string; uri: string; state?: Record<string, unknown>; now: number; id?: string }): { watch?: resourcewatch; reason?: string } {
  if (input.clientid.trim() === "") return { reason: "The resource subscription needs the paired client it belongs to." };
  if (resourceof(input.uri) === undefined) return { reason: `The serve mode publishes no resource named ${input.uri}.` };
  const watched = watchresource({ clientid: input.clientid, resource: input.uri, ...(input.state !== undefined ? { state: input.state } : {}), now: input.now, id: input.id ?? randomid() });
  return { ...(watched.watch !== undefined ? { watch: watched.watch } : {}), ...(watched.reason !== undefined ? { reason: watched.reason } : {}) };
}
...
export function publishchange(input: { watches: resourcewatch[]; uri: string; state: Record<string, unknown>; now: number }): { deliveries: Array<{ watchid: string; clientid: string; delta: Record<string, unknown>; frame: jsonrpcframe }>; watches: resourcewatch[] } {
...
export function resourcechangeframe(watchid: string, uri: string, delta: Record<string, unknown>, now: number): jsonrpcframe {
...
export function rendermcpprompt(input: { name: string; args?: Record<string, unknown>; templates: prompttemplate[] }): { prompt?: promptdef; rendered?: string; reason?: string } {
