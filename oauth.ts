/* ── Merged: the grand merge section ── the correlated provider oauth logics of the merged repository interned here, one surface without duplicate variations ── */
/**
 * @file oauth.ts — the provider oauth family of the merged repository.
 * @description The grand merge consolidation: ONE file owns the ENTIRE
 *              provider authentication context (OAuth clients, PKCE, consent
 *              URL, callback server, token exchange, token refresh, userinfo,
 *              account file management, identity bundles) of the merged
 *              repository. Absorbed verbatim:
 *              (a) the CLI identity bundle of the antigravity engine lineage;
 *              (b) the Gemini CLI OAuth client + shared endpoint/model
 *                  constants of the provider family;
 *              (c) the full-featured OAuth primitives (PKCE, auth URL
 *                  builder, callback server, exchange, refresh, userinfo);
 *              (d) the minimal legacy flow + the v3 account manager +
 *                  project/quota helpers of the provider family;
 *              (e) the oauth orchestrators and helpers (authenticate,
 *                  startOAuthFlow, validation, model lists, header builders,
 *                  platform detection, the legacy default export object).
 *
 *              The family is server and cli surface — the neutral core
 *              barrel (index.ts) never carries it, so the browser bundles
 *              stay free of node imports: node:* builtins first, global
 *              fetch beside them.
/**
 * @file oauth.ts — the provider oauth family of the merged repository.
 ...
 *              full flow with researches 9router omniroute gemini-cli;
...
//
// Masquerade set of the provider plugin (all four Google client identities):
 ...
 * top-level definitions of auth.ts). The grand merge consolidation moved the
 * env-overridable owner into constants.ts (the layer-0 raw-value module) so
 * the client pair never rides an import cycle; this module keeps the
 * authentication-surface aliases. Env chain: ANTIGRAVITY_CLIENT_ID /
 * GOOGLE_CLIENT_ID / ANTIGRAVITY_CLIENT_SECRET. Google renders the consent
 * screen as "Google Antigravity" for this client.
 */
import {
  ANTIGRAVITY_CLI_OAUTH_CLIENT_ID as AGY_OAUTH_CLIENT_ID,
  ANTIGRAVITY_CLI_OAUTH_CLIENT_SECRET as AGY_OAUTH_CLIENT_SECRET,
} from "./constants.js";
export const CLIENT_ID: string = AGY_OAUTH_CLIENT_ID;
export const CLIENT_SECRET: string = AGY_OAUTH_CLIENT_SECRET;
 ...
 * remote page that the user must copy/paste back — the plugin does NOT use it.
 * the plugin uses a loopback redirect (accepted by Google for this client,
...
/** OAuth client identities the plugin can present. */
...
// - Ported uniques from oauth(2).ts (the 2.0.0 lineage): extra scopes (openid,
...
export { SUPPORTED_MODELS_2026 as OAUTH_MODELS_2026 };
export type { SupportedModel2026 };

/**
 * Updated User-Agent of the 2.0.0 lineage (GPN:GeminiCLI style).
  ...
  const server = http.createServer((req, res) => {
    try {
      ...
      if (!isCallback) {
        ...
        res.end("devthink provider OAuth callback server running. Awaiting " + callbackPath);
        return;
      }
...
export async function authenticate(
  ...
  const log =
    opts.onLog ??
    ((level, msg) => {
      const prefix = level === "error" ? "[m[devthink][ERR]" : level === "warn" ? "[m[devthink][WARN]" : "[m[devthink]";
...
export async function startOAuthFlow(options: StartOAuthFlowOptions = {}): Promise<OAuthResult> {
  ...
  console.log(
    `\n[m[devthink] Opening browser for authentication...\n` +
      `If it does not open automatically, copy and paste into the browser:\n\n${authUrl}\n`,
  );
  try {
    opener(authUrl);
  } catch (err) {
    console.log(`[m[devthink] openBrowserFn failed, manual fallback: ${authUrl}`);
  }
  ...
  console.log(
    `\n[m[devthink] Authenticated as ${userInfo.email} - ready models: ${SUPPORTED_MODELS_2026.join(", ")}\n`,
  );
