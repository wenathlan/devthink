...
export function cdpallowlistof(value: unknown): cdpallowlist | undefined {
  ...
  const domains = Array.isArray(entry.domains)
    ? entry.domains.filter((domain): domain is string => typeof domain === "string" && cdpdomains.includes(domain))
    : [];
  ...
  const methods = Array.isArray(entry.methods)
    ? entry.methods.filter(
        (method): method is string =>
          ...
          methoddomain(method) !== undefined &&
          domains.includes(methoddomain(method) as string),
      )
    : [];
...
export function attachcdpsession(input: {
  id: string;
  runid: string;
  stepid: string;
  tabid: number;
  ...
  return {
    ...
    debuggerversion: input.debuggerversion,
  };
}
...
export function sendcdpcommand(input: {
  id: string;
  sessionid: string;
  runid: string;
  stepid: string;
  ...
  if (domain === undefined)
    return {
      ...
      ...(input.errorclass !== undefined ? { errorclass: input.errorclass } : { errorclass: "malformedmethod" }),
      at: input.at,
    };
  return {
    ...input,
...
export function watchcdpevents(
  rules: cdpeventrule[],
  events: Array<{ domain: string; event: string; payload?: string }>,
): {
  matched: Array<{ ruleid: string; domain: string; event: string; payload?: string }>;
...
export function breakpointinputof(
  value: unknown,
): Pick<breakpointspec, "url" | "line" | "column" | "condition"> | undefined {
  if (!value || typeof value !== "object" || Array.isArray(value)) return undefined;
  const entry = value as Record<string, unknown>;
  const url = typeof entry.url === "string" && entry.url.trim() ? entry.url.trim() : undefined;
  const line =
    typeof entry.line === "number" && Number.isInteger(entry.line) && entry.line >= 0 ? entry.line : undefined;
  if (url === undefined || line === undefined) return undefined;
  const column =
    typeof entry.column === "number" && Number.isInteger(entry.column) && entry.column >= 0 ? entry.column : undefined;
...
export function capturepause(input: {
  id: string;
  runid: string;
  stepid: string;
  reason: string;
  ...
  return {
    ...
    ...(input.hitbreakpoint !== undefined ? { hitbreakpoint: input.hitbreakpoint } : {}),
...
export function watchexpressionof(value: unknown): Pick<watchexpression, "expression" | "scope"> | undefined {
  ...
  const entry = value as Record<string, unknown>;
  const expression =
    typeof entry.expression === "string" && entry.expression.trim() ? entry.expression.trim() : undefined;
...
export function recordwatchvalue(
...
export function overrideinputof(value: unknown): Pick<scriptoverride, "urlpattern" | "source"> | undefined {
  ...
  const entry = value as Record<string, unknown>;
  const urlpattern =
    typeof entry.urlpattern === "string" && entry.urlpattern.trim() ? entry.urlpattern.trim() : undefined;
...
export function teardownplanof(value: unknown): teardownplan | undefined {
  if (!value || typeof value !== "object" || Array.isArray(value)) return undefined;
  const entry = value as Record<string, unknown>;
  const revertsteps = Array.isArray(entry.revertsteps)
    ? entry.revertsteps.filter((step): step is string => typeof step === "string" && step.trim().length > 0)
    : [];
...
export function teardowncdpsession(input: {
  session: cdpsession;
  breakpoints: breakpointspec[];
  overrides: scriptoverride[];
  plan: teardownplan | undefined;
...
export const profilerkinds: string[] = [
  "measureflow",
  "heapshot",
  "trackmemory",
  "profilecpu",
...
export function attachtargetof(value: unknown): attachtarget | undefined {
  ...
  const kind =
    typeof entry.kind === "string" && kinds.includes(entry.kind as attachtarget["kind"])
      ? (entry.kind as attachtarget["kind"])
      : undefined;
  const url = typeof entry.url === "string" && entry.url.trim() ? entry.url.trim() : undefined;
...
export function flowspecof(value: unknown): flowspec | undefined {
  ...
  const steps = Array.isArray(entry.steps)
    ? entry.steps.filter((step): step is string => typeof step === "string" && step.trim().length > 0)
    : [];
  const metrics = Array.isArray(entry.metrics)
    ? entry.metrics.filter((metric): metric is string => typeof metric === "string" && flowmetricnames.includes(metric))
    : [];
...
export function measure(input: {
  ...
  const windows = stepwindows(
    input.spec,
    input.entries.filter((entry) => entry.type === "mark"),
  );
  const stepsof = (start: number, end: number): string[] =>
    windows.filter((window) => window.end >= start && window.start <= end).map((window) => window.stepid);
  const push = (name: string, start: number, end: number, steps: string[]): void => {
    if (!input.spec.metrics.includes(name)) return;
    metrics.push({
      id: `${input.runid}-${input.stepid}-${name}-${metrics.length}`,
      runid: input.runid,
      stepid: input.stepid,
      name,
      start,
  ...
  if (navigation !== undefined)
    push(
      "navigation",
      navigation.start,
      navigation.start + navigation.duration,
      stepsof(navigation.start, navigation.start + navigation.duration),
    );
  ...
  const lcp =
    lcps.length > 0 ? lcps.reduce((largest, entry) => (entry.start > largest.start ? entry : largest)) : undefined;
  ...
  if (firstinput !== undefined)
    push(
      "fid",
      firstinput.start,
      firstinput.start + firstinput.duration,
      stepsof(firstinput.start, firstinput.start + firstinput.duration),
    );
  ...
  if (interactions.length > 0) {
    ...
    const end = interactions.reduce(
      (latest, entry) => (entry.start + entry.duration > latest ? entry.start + entry.duration : latest),
      start,
    );
  ...
  for (const window of windows) {
    ...
    const blocking = input.entries
      .filter((entry) => entry.type === "longtask" && entry.start >= window.start && entry.start <= window.end)
      .reduce((total, entry) => total + Math.max(0, entry.duration - 50), 0);
    metrics.push({
      id: `${input.runid}-${input.stepid}-blocking-${window.stepid}`,
      runid: input.runid,
      stepid: input.stepid,
      name: "blocking",
      start: window.start,
...
export function heapintervalallowed(
...
export function heapsnap(input: {
  id: string;
  runid: string;
  stepid: string;
  origin: string;
...
export function growsampleof(input: {
  id: string;
  runid: string;
  stepid: string;
  usedbytes: number;
...
export function growthtrend(input: { runid: string; samples: growsample[]; slope: number; now: number }): memorytrend {
  ...
  const computed =
    first !== undefined && last !== undefined && last.at > first.at
      ? (last.usedbytes - first.usedbytes) / (last.at - first.at)
      : 0;
...
export function cpusnap(input: {
  ...
  const hotfunctions = [...ranked.entries()]
...
export function shiftentryof(value: unknown): shiftentry | undefined {
  ...
  const entry = value as Record<string, unknown>;
  const score =
    typeof entry.score === "number" && Number.isFinite(entry.score) && entry.score >= 0 ? entry.score : undefined;
  const starttime =
    typeof entry.starttime === "number" && Number.isFinite(entry.starttime) ? entry.starttime : undefined;
  if (score === undefined || starttime === undefined) return undefined;
  const selectors = Array.isArray(entry.selectors)
    ? entry.selectors.filter(
  ...
  return {
    ...
    starttime,
    selectors,
    at: typeof entry.at === "number" ? entry.at : starttime,
  };
}
...
export function tracestart(input: {
  id: string;
  runid: string;
  stepid: string;
  origin: string;
...
export function tracetofile(
  trace: tracerecord,
  events: traceevent[],
): { content: string; bytesize: number; events: number } {
  const payload = {
    devthinktrace: "1.1.47",
    runid: trace.runid,
    origin: trace.origin,
    ...
    traceevents: events.map((event) => ({ name: event.name, cat: event.category, offset: event.offset })),
  };
...
export function annotationof(value: unknown): traceannotation | undefined {
  ...
  const offset =
    typeof entry.offset === "number" && Number.isFinite(entry.offset) && entry.offset >= 0 ? entry.offset : 0;
...
export function annotatetrace(input: {
  trace: tracerecord;
  annotations: traceannotation[];
  timeline: Array<{ stepid: string; time: number }>;
  now: number;
}): tracerecord {
...
export function replaytrace(content: string): {
  traceid: string;
  runid: string;
  categories: Record<string, number>;
  events: Array<{ name: string; category: string; offset: number; stepid?: string }>;
  annotations: traceannotation[];
} {
  let parsed: unknown;
  try {
  ...
  if (!parsed || typeof parsed !== "object" || Array.isArray(parsed))
    throw new Error("The trace file is not a json object.");
    ...
    return [
      {
        ...
        category: entry.cat,
        offset,
        ...(annotation !== undefined ? { stepid: annotation.stepid } : {}),
      },
    ];
  });
...
export function capturesourcemaps(input: {
  runid: string;
  stepid: string;
  origin: string;
  scripts: Array<{ url: string; source: string }>;
  now: number;
}): sourcemapref[] {
...
export function rewritesourcelocation(
  input: { url: string; line: number; column?: number },
...
export function expireprofilerecords(input: {
  heaps: heaprecord[];
  profiles: cpuprofile[];
  traces: tracerecord[];
  retention: number | undefined;
  now: number;
}): { heaps: heaprecord[]; profiles: cpuprofile[]; traces: tracerecord[] } {
  ...
  return {
    heaps: input.heaps.map((heap) =>
      expired(heap.capturedat) && heap.bytesexpired !== true ? { ...heap, bytesexpired: true } : heap,
    ),
    profiles: input.profiles.map((profile) =>
      expired(profile.at) && profile.samplesexpired !== true ? { ...profile, samplesexpired: true } : profile,
    ),
    traces: input.traces.map((trace) =>
      expired(trace.endedat) && trace.bytesexpired !== true ? { ...trace, bytesexpired: true } : trace,
    ),
  };
}
