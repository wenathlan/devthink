...
import type { actionkind, actionrisk, toolcatalog, tooldef, toolnamespace, toolschema, toolschemaproperty, agentsession, agentplan, batchoutcome, callcontext, callratelimit, clientrecord, idempotencykey, idempotencyrecord, retryhint, structurederror, tooldryrun, toolmock, toolresult } from "./types.js";
...
export function openapilayout(schema: toolschema): string {
  const fields = Object.entries(schema.properties).map(([name, property]) => `${name}:${property.type}${property.required === true ? "*" : ""}${property.default !== undefined ? `=${typeof property.default === "object" ? JSON.stringify(property.default) : String(property.default)}` : ""}`);
...
function readtool(name: string, kind: actionkind, description: string, inputs: Record<string, toolschemaproperty> = {}): tooldef {
...
export function toolsbynamespace(catalog: toolcatalog): Array<{ namespace: toolnamespace; version: number; tools: tooldef[] }> {
...
export function applyratelimit(input: { limits: callratelimit[]; clientid: string; now: number }): { allowed: boolean; used: number; budget?: number; retryafter?: number; limit?: callratelimit; limits: callratelimit[] } {
  ...
  const limit: callratelimit = elapsed >= existing.windowms ? { ...existing, windowstartedat: input.now, used: 0 } : existing;
  ...
  return { allowed: true, used: counted.used, ...(counted.budget !== undefined ? { budget: counted.budget } : {}), limit: counted, limits: input.limits.map(candidate => candidate.clientid === input.clientid ? counted : candidate) };
}

/** Builds one structured error with its code, message, retry hint and optional retry after window. */
export function structurederrorof(input: { code: string; message: string; retryhint: retryhint; retryafter?: number }): structurederror {
  return { code: input.code, message: input.message, retryhint: input.retryhint, ...(input.retryafter !== undefined ? { retryafter: input.retryafter } : {}) };
}
...
export function maptoolerror(input: { failure: { code?: string; message: string } | Error; retryafter?: number }): structurederror {
  ...
  return structurederrorof({ code, message, retryhint: callretryhintof({ code, message }), ...(input.retryafter !== undefined ? { retryafter: input.retryafter } : {}) });
}
...
export function checkidempotency(input: { records: idempotencyrecord[]; key: idempotencykey; clientid: string; now: number }): { replay?: toolresult; record?: idempotencyrecord; reason?: string } {
...
export function recordidempotency(input: { records: idempotencyrecord[]; key: idempotencykey; clientid: string; tool: string; result: toolresult; now: number; window?: number }): idempotencyrecord[] {
  const record: idempotencyrecord = { key: input.key, clientid: input.clientid, tool: input.tool, result: input.result, createdat: input.now, expiresat: input.now + (input.window ?? defaultidempotencywindowms) };
...
export async function runbatch(input: { calls: Array<{ id: string; name: string; params: Record<string, unknown> }>; stoponerror: boolean; now: number; execute: (call: { id: string; name: string; params: Record<string, unknown> }, index: number) => Promise<{ ok: boolean; result?: toolresult; error?: structurederror }> }): Promise<{ outcomes: batchoutcome[]; stoppedat?: string }> {
  const outcomes: batchoutcome[] = [];
  for (let index = 0; index < input.calls.length; index += 1) {
    const call = input.calls[index];
    if (call === undefined) continue;
    const outcome = await input.execute(call, index);
    outcomes.push({ callid: call.id, tool: call.name, ok: outcome.ok, ...(outcome.result !== undefined ? { result: outcome.result } : {}), ...(outcome.error !== undefined ? { error: outcome.error } : {}), at: input.now + index });
...
export function dryruntool(input: { tool: tooldef; params: Record<string, unknown>; client: clientrecord; session?: agentsession; plan?: agentplan; origin: string; stepid?: string; now: number; callid?: string }): tooldryrun {
  const findings: string[] = [];
  for (const required of input.tool.inputschema.required) {
    const value = input.params[required];
    if (value === undefined || value === null || (typeof value === "string" && value.trim() === "")) findings.push(`The required argument ${required} of ${input.tool.name} stays empty.`);
  }
  ...
  const stepid = input.stepid !== undefined ? input.stepid : typeof input.params.stepid === "string" ? input.params.stepid : undefined;
  const gate = tooldispatchgate({ client: input.client, tool: input.tool, session: input.session, plan: input.plan, origin: input.origin, ...(stepid !== undefined ? { stepid } : {}), now: input.now });
...
export function begincall(input: { clientid: string; tool: string; callid?: string; idempotencykey?: idempotencykey; dryrun?: boolean; batchid?: string; now: number }): callcontext {
  return { callid: input.callid ?? randomid(), clientid: input.clientid, tool: input.tool, state: "inflight", startedat: input.now, ...(input.idempotencykey !== undefined && input.idempotencykey.trim() !== "" ? { idempotencykey: input.idempotencykey } : {}), ...(input.dryrun === true ? { dryrun: true } : {}), ...(input.batchid !== undefined ? { batchid: input.batchid } : {}), chunks: 0 };
}
...
export function endcall(input: { contexts: callcontext[]; callid: string; ok: boolean; errorcode?: string; partial?: toolresult; now: number }): { contexts: callcontext[]; context?: callcontext; reason?: string } {
  const match = input.contexts.find(context => context.callid === input.callid);
  if (match === undefined) return { contexts: input.contexts, reason: `The call context ${input.callid} never opened.` };
  if (match.state !== "inflight") return { contexts: input.contexts, context: match };
  const context: callcontext = { ...match, state: input.ok ? "done" : "failed", endedat: input.now, ...(input.errorcode !== undefined ? { errorcode: input.errorcode } : {}), ...(input.partial !== undefined ? { partial: input.partial } : {}) };
