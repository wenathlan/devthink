...
export function electleader(input: {
  ...
  if (live.length === 0)
    throw new Error("The swarm holds no live agent; the leader election waits for the user to register one.");
  const rule = input.rule ?? { kind: "first" as const };
  let leader: agentidentity | undefined;
  if (rule.kind === "named") {
    if (rule.agentid === undefined || rule.agentid.trim() === "")
      throw new Error("The named election rule needs the agent id the user named.");
    leader = live.find((agent) => agent.id === rule.agentid);
    if (!leader)
      throw new Error(
        `The named election rule names the agent ${rule.agentid} which is not a live agent of the swarm.`,
      );
  } else {
...
export function assignwork(input: { topology: leaderworker; tasks: taskitem[]; now: number }): leaderworker {
  if (input.topology.workerids.length === 0)
    throw new Error("The topology holds no worker; the user adds workers before the assignment.");
  ...
  tasks.forEach((task, index) => {
    ...
    assignments.push({
      workerid,
      taskid: task.id,
      slice: `${task.payload} (slice ${Math.floor(index / input.topology.workerids.length) + 1} of lane ${task.lane})`,
...
export function collectresults(input: {
  ...
  outputs: Array<{ workerid: string; taskid: string; state: string; summary: string }>;
}): { gathered: Array<{ workerid: string; taskid: string; state: string; summary: string }>; missing: string[] } {
  const gathered = input.topology.assignments.map((assignment) => {
    const output = input.outputs.find(
      (entry) => entry.taskid === assignment.taskid && entry.workerid === assignment.workerid,
    );
    return (
      output ?? {
        ...
        state: "pending",
        summary: `The worker ${assignment.workerid} has not returned its slice of the task ${assignment.taskid} yet.`,
      }
    );
  });
  return {
    gathered,
    missing: gathered.filter((entry) => entry.state === "pending").map((entry) => `${entry.workerid}:${entry.taskid}`),
  };
}
...
export function scaleworkers(input: {
  topology: leaderworker;
  agents: agentidentity[];
  pending: number;
  bound?: number;
  now: number;
}): { topology: leaderworker; added: string[]; retired: string[]; reason: string } {
  const live = input.agents.filter(
    (agent) => agent.state === "active" && agent.role === "worker" && agent.id !== input.topology.leaderid,
  );
  ...
  if (input.pending > current.length) {
    ...
    const addable =
      ceiling === undefined
        ? available.slice(0, wanted)
        : available.slice(0, Math.min(wanted, Math.max(ceiling - current.length, 0)));
    if (addable.length === 0)
      return {
        ...
        reason:
          ceiling === undefined
            ? `The load of ${input.pending} pending slices exceeds the ${current.length} workers but the swarm holds no further live worker role agent to add.`
            : `The load of ${input.pending} pending slices exceeds the ${current.length} workers but the user configured bound of ${ceiling} workers holds.`,
      };
    return {
      topology: {
        ...input.topology,
        workerids: [...current, ...addable],
        electedat: input.topology.electedat,
        assignments: input.topology.assignments,
      },
      added: addable,
      retired: [],
      reason: `The load of ${input.pending} pending slices added the workers ${addable.join(", ")}; ${ceiling === undefined ? "no bound is configured so the user scale stands alone" : `the user configured bound of ${ceiling} workers holds`}.`,
    };
  }
  ...
  if (current.length > keep) {
    ...
    return {
      topology: {
        ...input.topology,
        workerids: current.slice(0, keep),
        electedat: input.topology.electedat,
        assignments: input.topology.assignments.filter((assignment) => !retired.includes(assignment.workerid)),
      },
      added: [],
      retired,
      reason: `The load of ${input.pending} pending slices retired the idle workers ${retired.join(", ")}.`,
    };
  }
  return {
    topology: input.topology,
    added: [],
    retired: [],
    reason: `The load of ${input.pending} pending slices matches the ${current.length} workers; the scale stays unchanged.`,
  };
}
...
export function plannersplit(input: {
  id: string;
  planownerid: string;
  runownerid: string;
  taskid?: string;
  ...
  if (input.planownerid.trim() === "" || input.runownerid.trim() === "")
    throw new Error("The planner executor split needs its plan owner and run owner agent ids.");
  if (input.planownerid === input.runownerid)
    throw new Error(
      "The planner executor split keeps plan drafting and execution in different agents; one agent holds both sides never.",
    );
  return {
    id: input.id,
    planownerid: input.planownerid,
    runownerid: input.runownerid,
    ...(input.taskid !== undefined && input.taskid.trim() !== "" ? { taskid: input.taskid } : {}),
    stepreports: [],
    splitat: input.now,
  };
}
...
export function reportstep(input: {
  split: plannerexecutor;
  stepid: string;
  outcome: "done" | "failed";
  detail: string;
  now: number;
  ...
  return {
    ...input.split,
    stepreports: [...input.split.stepreports.filter((entry) => entry.stepid !== report.stepid), report],
  };
}
...
export function requestreview(input: {
  ...
  if (input.toagentid.trim() === "" || input.toagentid === input.fromagentid)
    throw new Error("The review request names another reviewing agent, never its own requester.");
  const request: reviewrequest = {
    ...
    ...(input.timeoutms !== undefined ? { timeoutat: input.now + input.timeoutms } : {}),
  };
...
export function ackreview(input: { requests: reviewrequest[]; id: string; now: number }): reviewrequest[] {
  ...
  if (request.state !== "open")
    throw new Error(`The review request ${input.id} is ${request.state}; only an open request receives its ack.`);
  return input.requests.map((entry) =>
    entry.id === input.id ? { ...entry, state: "acked" as const, ackedat: input.now } : entry,
  );
}
...
export function applyreview(input: {
  ...
  taskid?: string;
  now: number;
}): { review: criticreview; requests: reviewrequest[] } {
  const request = input.requests.find((entry) => entry.id === input.id);
  if (!request) throw new Error(`The review request ${input.id} does not exist.`);
  if (request.state === "answered" || request.state === "timeout")
    throw new Error(
      `The review request ${input.id} is ${request.state}; an answered or timed out request never reviews again.`,
    );
  if (request.toagentid !== input.reviewerid)
    throw new Error(
      `The review request ${input.id} routes to the agent ${request.toagentid}; the agent ${input.reviewerid} never answers in its place.`,
    );
  if (input.verdict === "changes" && input.requiredchanges.length === 0)
    throw new Error("A changes verdict needs its required changes in plain language.");
  const review: criticreview = {
    id: request.id,
    reviewerid: input.reviewerid,
    subjectagentid: request.fromagentid,
    ...(input.taskid !== undefined && input.taskid.trim() !== "" ? { taskid: input.taskid } : {}),
  ...
  return {
    review,
    requests: input.requests.map((entry) =>
      entry.id === input.id ? { ...entry, state: "answered" as const, answeredat: input.now } : entry,
    ),
  };
}
...
export function sweepreviews(input: { requests: reviewrequest[]; now: number }): {
  requests: reviewrequest[];
  timedout: string[];
} {
  const expired = input.requests
    .filter((request) => request.state === "open" || request.state === "acked")
    .filter((request) => request.timeoutat !== undefined && input.now > request.timeoutat);
  ...
  return {
    requests: input.requests.map((request) =>
      ids.has(request.id) ? { ...request, state: "timeout" as const } : request,
    ),
    timedout: [...ids],
  };
}
...
export function checkclaim(input: {
  ...
  return {
    ...
    method: input.method,
    outcome: input.outcome,
    ...(input.evidence !== undefined && input.evidence.trim() !== "" ? { evidence: input.evidence } : {}),
    checkedat: input.now,
  };
}
...
export function boardstate(input: {
  ...
  queue: taskqueue;
  topology?: leaderworker;
  milestones?: Record<string, boardmilestone[]>;
  now: number;
}): progressboard {
  const lanes: boardlane[] = input.agents
    ...
    .map((agent) => {
      ...
      const claim = input.queue.claims.find(
        (record) =>
          record.agentid === agent.id &&
          input.queue.items.some((item) => item.id === record.taskid && item.state === "claimed"),
      );
      const task = claim !== undefined ? input.queue.items.find((item) => item.id === claim.taskid) : undefined;
      const lane =
        task?.lane ??
        (agent.role === "critic" || agent.role === "verifier"
          ? agent.role
          : agent.role === "planner"
            ? "planning"
            : "idle");
      return {
        ...
        ...(task !== undefined
          ...
          : assignment !== undefined
            ? { currenttask: assignment.slice }
            : {}),
        milestones: input.milestones?.[agent.id] ?? [],
      };
    });
...
export function escalate(input: {
  ...
  if (input.context.trim() === "")
    throw new Error("The escalation needs its full context in plain language; the user decides on what the agent saw.");
  ...
  if (input.rule.strategy === "priority") {
    ...
    return [...input.claims]
      .sort((one, two) => {
        ...
        return (
          (oneindex === -1 ? order.length : oneindex) - (twoindex === -1 ? order.length : twoindex) ||
          one.claimedat - two.claimedat
        );
      })
      .map((claim) => claim.agentid);
  }
  if (input.rule.strategy === "age")
    return [...input.claims]
      .sort((one, two) => one.claimedat - two.claimedat || (one.agentid < two.agentid ? -1 : 1))
      .map((claim) => claim.agentid);
  if (input.leaderid === undefined)
    throw new Error("The leader arbitration strategy needs the elected leader of the topology.");
  return [...input.claims]
    .sort(
      (one, two) =>
        (one.agentid === input.leaderid ? -1 : 1) - (two.agentid === input.leaderid ? -1 : 1) ||
        one.claimedat - two.claimedat,
    )
    .map((claim) => claim.agentid);
}
...
export function openconsensus(input: { id: string; subject: string; quorum: number; now: number }): consensusround {
  ...
  if (!Number.isInteger(input.quorum) || input.quorum < 1)
    throw new Error("The consensus round needs its quorum as a positive whole number the user configured.");
...
export function castvote(input: {
  ...
  if (input.round.state !== "open")
    throw new Error(`The consensus round ${input.round.id} is ${input.round.state}; a closed round collects no vote.`);
  if (input.round.votes.some((entry) => entry.agentid === input.agentid))
    throw new Error(`The agent ${input.agentid} already voted in the round ${input.round.id}.`);
...
export function consensusstate(round: consensusround): {
  yes: number;
  no: number;
  abstain: number;
  quorum: number;
...
export function emptyqueue(
  input: { lanes?: string[]; priorities?: number[]; completionpolicy?: "all" | "any" } = {},
): taskqueue {
  return {
    lanes: input.lanes ?? [],
...
export function enqueue(input: {
  ...
  if (input.queue.lanes.length > 0 && !input.queue.lanes.includes(input.lane))
    throw new Error(`The lane ${input.lane} is not one of the configured lanes.`);
  if (input.queue.items.some((item) => item.id === input.id))
    throw new Error(`The task id ${input.id} already waits in the queue.`);
  const item: taskitem = {
    id: input.id,
    lane: input.lane,
...
export function claim(input: { queue: taskqueue; agentid: string; now: number }): {
  queue: taskqueue;
  task?: taskitem;
} {
  ...
  return {
    queue: {
      ...
      items: input.queue.items.map((item) =>
        item.id === candidate.id ? { ...item, state: "claimed" as const } : item,
      ),
      claims: [
        ...input.queue.claims.filter(
          (entry) =>
            entry.agentid !== input.agentid ||
            input.queue.items.find((item) => item.id === entry.taskid)?.state !== "claimed",
        ),
        record,
      ],
    },
...
export function steal(input: {
  ...
  now: number;
}): { queue: taskqueue; task?: taskitem } {
  const rule = (input.ownership ?? []).find((entry) => entry.lane === input.fromlane);
  if (rule && !rule.roles.includes(input.role))
    throw new Error(
      `The lane ${input.fromlane} only lets the roles ${rule.roles.join(", ")} steal its tasks; the ${input.role} agent stays out.`,
    );
  ...
  return {
    queue: {
      ...
      claims: [...input.queue.claims.filter((entry) => entry.agentid !== input.agentid), record],
    },
    task: { ...candidate, state: "claimed" },
  };
}
...
export function complete(input: { queue: taskqueue; taskid: string; now: number }): taskqueue {
  ...
  if (task.state !== "claimed")
    throw new Error(`The task ${input.taskid} is ${task.state}; only a claimed task completes.`);
  return {
    ...input.queue,
    items: input.queue.items.map((item) => (item.id === input.taskid ? { ...item, state: "done" as const } : item)),
    claims: input.queue.claims.filter((record) => record.taskid !== input.taskid),
  };
}
...
export function canceltask(input: { queue: taskqueue; taskid: string; now: number }): taskqueue {
  ...
  return {
    ...input.queue,
    items: input.queue.items.map((item) =>
      item.id === input.taskid ? { ...item, state: "cancelled" as const } : item,
    ),
    claims: input.queue.claims.filter((record) => record.taskid !== input.taskid),
  };
}
...
export function claimheartbeat(input: { queue: taskqueue; agentid: string; now: number }): taskqueue {
  return {
    ...input.queue,
    claims: input.queue.claims.map((record) =>
      record.agentid === input.agentid ? { ...record, heartbeatat: input.now } : record,
    ),
  };
}
...
export function requeue(input: { queue: taskqueue; now: number; window?: number }): {
  queue: taskqueue;
  requeued: string[];
} {
  ...
  return {
    queue: {
      ...input.queue,
      items: input.queue.items.map((item) =>
        expiredids.has(item.id) && item.state === "claimed" ? { ...item, state: "queued" as const } : item,
      ),
...
export function lanereport(
  ...
  cancelled: number;
  claims: Array<{ agentid: string; taskid: string; heartbeatat: number }>;
}> {
  const lanes = [...queue.lanes, ...queue.items.map((item) => item.lane)].filter(
  ...
  return lanes.map((lane) => ({
    ...
    claims: queue.claims
      .filter((record) =>
        queue.items.some((item) => item.id === record.taskid && item.lane === lane && item.state === "claimed"),
      )
      .map((record) => ({ agentid: record.agentid, taskid: record.taskid, heartbeatat: record.heartbeatat })),
  }));
}
...
export function queuecomplete(queue: taskqueue): boolean {
  ...
  return queue.completionpolicy === "any"
    ? open.some((item) => item.state === "done")
    : open.every((item) => item.state === "done");
}
...
export function postentry(input: {
  ...
  if (!blackboardsections.includes(input.section))
    throw new Error(`The section ${input.section} is not one of the shared blackboard sections.`);
  if (input.board.entries.some((entry) => entry.id === input.id))
    throw new Error(`The blackboard entry id ${input.id} already exists.`);
  ...
  return {
    ...input.board,
    sections: input.board.sections.includes(input.section)
      ? input.board.sections
      : [...input.board.sections, input.section],
    entries: [entry, ...input.board.entries],
  };
}
...
export function retireentries(input: { board: blackboard; now: number }): { board: blackboard; retired: string[] } {
  ...
  const stale = input.board.entries.filter(
    (entry) => entry.retiredat === undefined && input.now - entry.postedat > input.board.retirementwindow!,
  );
  ...
  return {
    board: {
      ...input.board,
      entries: input.board.entries.map((entry) =>
        staleids.has(entry.id) ? { ...entry, retiredat: input.now } : entry,
      ),
    },
    retired: [...staleids],
  };
...
export function retireentry(input: { board: blackboard; entryid: string; now: number }): blackboard {
  ...
  return {
    ...input.board,
    entries: input.board.entries.map((candidate) =>
      candidate.id === input.entryid ? { ...candidate, retiredat: input.now } : candidate,
    ),
  };
}
...
export function boardsummary(
  board: blackboard,
  now: number,
): Array<{ section: blackboardsection; entries: number; authors: string[]; freshestat?: number }> {
  return board.sections.map((section) => {
    const live = board.entries.filter((entry) => entry.section === section && entry.retiredat === undefined);
    return {
      section,
      entries: live.length,
      authors: [...new Set(live.map((entry) => entry.author))],
      ...(live.length > 0 ? { freshestat: Math.max(...live.map((entry) => entry.postedat)) } : {}),
    };
  });
}
