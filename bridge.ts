...
export const pagecontentkeys: string[] = [
  "html",
  "content",
  "body",
  "textpreview",
...
export function stableopid(seed: string, sequence: number): string {
  ...
  if (trimmed === "")
    throw new Error("The operation id names its sender seed; an empty seed never correlates a reply.");
  if (!Number.isInteger(sequence) || sequence < 1)
    throw new Error("The operation id sequence stays a positive whole number per sender.");
...
export function negotiatecapabilities(
  ours: servercontractcapabilities,
  theirs: servercontractcapabilities,
): {
  ok: boolean;
  version: number;
  ...
  if (!Number.isInteger(ours.version) || ours.version < 1 || !Number.isInteger(theirs.version) || theirs.version < 1)
    return {
      ok: false,
      version: 0,
      operations: [],
      ...
      heartbeat: false,
      multiplex: false,
      reason: "The servercontract version stays a positive whole number on both sides of the handshake.",
    };
  const version = Math.min(ours.version, theirs.version);
  const operations = serveroperations.filter(
    (operation) => ours.operations.includes(operation) && theirs.operations.includes(operation),
  );
  ...
  if (version < 1)
    return {
      ...
      heartbeat: false,
      multiplex: false,
      reason: "The two handshake sides share no servercontract version.",
    };
  if (operations.length === 0)
    return {
      ...
      heartbeat: false,
      multiplex: false,
      reason: "The two handshake sides share no wire operation.",
    };
  if (events.length === 0)
    return {
      ok: false,
      ...
      heartbeat: false,
      multiplex: false,
      reason: "The two handshake sides share no event type.",
    };
  return {
    ok: true,
    version,
    operations,
    events,
    heartbeat: ours.heartbeat === true && theirs.heartbeat === true,
    multiplex: ours.multiplex === true && theirs.multiplex === true,
  };
}
...
export function composeenvelope(input: {
  ...
  if (opid === "")
    throw new Error("The envelope carries its stable operation id; an empty id never correlates a reply.");
  if (!serveroperations.includes(input.op))
    throw new Error(`The envelope operation ${input.op} sits outside the servercontract operations.`);
  if (!Number.isFinite(input.at)) throw new Error("The envelope carries its timestamp.");
  return {
    version: input.version ?? servercontractversion,
    op: input.op,
    opid,
    at: input.at,
    ...(input.sessionid !== undefined && input.sessionid.trim() !== "" ? { sessionid: input.sessionid.trim() } : {}),
    ...(input.token !== undefined && input.token.trim() !== "" ? { token: input.token.trim() } : {}),
...
export function parseenvelope(value: unknown): serverenvelope {
  if (!value || typeof value !== "object" || Array.isArray(value))
    throw new Error("The servercontract envelope must be a json object.");
  const entry = value as Record<string, unknown>;
  if (!Number.isInteger(entry.version) || (entry.version as number) < 1)
    throw new Error("The envelope version must be a positive whole number.");
  if (typeof entry.op !== "string" || !serveroperations.includes(entry.op as serveroperation))
    throw new Error(`The envelope operation ${String(entry.op)} sits outside the servercontract operations.`);
  if (typeof entry.opid !== "string" || entry.opid.trim() === "")
    throw new Error("The envelope operation id must be a non-empty string.");
  if (entry.sessionid !== undefined && (typeof entry.sessionid !== "string" || entry.sessionid.trim() === ""))
    throw new Error("The envelope session id must be a non-empty string when present.");
  if (entry.token !== undefined && (typeof entry.token !== "string" || entry.token.trim() === ""))
    throw new Error("The envelope frame token must be a non-empty string when present.");
  if (!Number.isFinite(entry.at)) throw new Error("The envelope timestamp must be a finite number.");
  if (entry.body !== undefined && (!entry.body || typeof entry.body !== "object" || Array.isArray(entry.body)))
    throw new Error("The envelope body must be a json object when present.");
  return {
    version: entry.version as number,
    op: entry.op as serveroperation,
    opid: entry.opid.trim(),
    at: entry.at as number,
    ...(entry.sessionid !== undefined ? { sessionid: (entry.sessionid as string).trim() } : {}),
    ...(entry.token !== undefined ? { token: (entry.token as string).trim() } : {}),
    ...(entry.body !== undefined ? { body: entry.body as Record<string, unknown> } : {}),
  };
}
...
export function sessioncreatebody(input: {
  ...
  if (input.role !== "extension" && input.role !== "site")
    throw new Error("The sessioncreate role is the extension or the site side.");
  return {
    role: input.role,
    memberid,
    capabilities: input.capabilities,
    ...(input.pairingcode !== undefined && input.pairingcode.trim() !== ""
...
export function sessionjoinbody(input: {
  sessionid?: string;
  role: "extension" | "site";
  memberid: string;
  capabilities: servercontractcapabilities;
  ...
  if (sessionid === "" && pairingcode === "")
    throw new Error(
      "The sessionjoin body names its session id or rides its pairing code; one of the two identifies the join.",
    );
  const memberid = input.memberid.trim();
  if (memberid === "") throw new Error("The sessionjoin body names its member id.");
  if (input.role !== "extension" && input.role !== "site")
    throw new Error("The sessionjoin role is the extension or the site side.");
  return {
    ...(sessionid !== "" ? { sessionid } : {}),
    role: input.role,
    memberid,
...
export function eventpostbody(input: {
  event: { kind: servereventtype; stream: string; payload: Record<string, unknown> };
}): Record<string, unknown> {
  if (!servereventtypes.includes(input.event.kind))
    throw new Error(`The event type ${input.event.kind} sits outside the servercontract event types.`);
  const stream = input.event.stream.trim();
  if (stream === "") throw new Error("The eventpost body names its multiplexed stream.");
  if (!input.event.payload || typeof input.event.payload !== "object" || Array.isArray(input.event.payload))
    throw new Error("The eventpost payload must be a json object.");
...
export function eventstreambody(input: { streams: string[]; since?: number }): Record<string, unknown> {
  ...
  if (input.since !== undefined && !Number.isFinite(input.since))
    throw new Error("The eventstream resume timestamp stays a finite number.");
...
export function planproposaleventpayload(card: bridgereviewcard): Record<string, unknown> {
  ...
  return {
    proposalid,
    title,
    steps: card.steps.map((step) => ({ id: step.id, kind: step.kind, origin: step.origin, sensitive: step.sensitive })),
    ...(card.timeoutat !== undefined ? { timeoutat: card.timeoutat } : {}),
  };
}
...
export function planrevieweventpayload(decision: bridgereviewdecision): Record<string, unknown> {
  ...
  if (decision.decision !== "approved" && decision.decision !== "changes" && decision.decision !== "refused")
    throw new Error("The plan review decision is approved, changes or refused.");
  ...
  return {
    ...
    by,
    ...(decision.note !== undefined && decision.note.trim() !== "" ? { note: decision.note.trim() } : {}),
  };
}
...
export function heartbeatevent(now: number): {
  ...
  return {
    kind: "progress",
    stream: "heartbeat",
    payload: progresseventpayload({ stepid: "heartbeat", status: "alive" }),
  };
}
...
export function bridgepayload(
  ...
  payload: Record<string, unknown>,
  pageconsent?: boolean,
): { payload: Record<string, unknown>; held: string[] } {
...
export function minimalplandigest(
  plan: agentplan,
  statuses: Record<string, string> = {},
): { title: string; steps: Array<{ id: string; kind: string; status: string }> } {
  const objective = plan.objective.trim();
  if (objective === "") throw new Error("The plan digest names its objective.");
  return {
    title: objective,
    steps: plan.steps.map((step) => ({ id: step.id, kind: step.kind, status: statuses[step.id] ?? "pending" })),
  };
}
...
export function sitemanifestof(version: number = servercontractversion): {
  name: string;
...
export function newrelayclient(input: {
  url: string;
  memberid: string;
  role: "extension" | "site";
  capabilities: servercontractcapabilities;
  ...
  return {
    url,
    origin: relayorigin(url),
    sessionid: (input.sessionid ?? "").trim(),
    token: (input.token ?? "").trim(),
    memberid,
    role: input.role,
    state: url === "" ? "idle" : "connecting",
    attempts: 0,
    rotations: 0,
    sent: 0,
    received: 0,
    ...
    queue: [],
    capabilities: input.capabilities,
    rate: { used: 0, windowstartedat: input.now },
    sequence: 0,
  };
}
...
export function bridgeframerate(input: {
  rate: { used: number; windowstartedat: number };
  cap?: number;
  window?: number;
  now: number;
}): { allowed: boolean; used: number; windowstartedat: number; resetsat?: number } {
  const window =
    input.window !== undefined && Number.isFinite(input.window) && input.window > 0 ? input.window : undefined;
  if (window === undefined)
    return { allowed: true, used: input.rate.used, windowstartedat: input.rate.windowstartedat };
  if (input.now - input.rate.windowstartedat >= window)
    return {
      allowed: input.cap === undefined || input.cap >= 1,
      used: input.cap === undefined ? input.rate.used : 1,
      windowstartedat: input.now,
      resetsat: input.now + window,
    };
  const used = input.rate.used + 1;
  if (input.cap !== undefined && Number.isFinite(input.cap) && input.cap >= 1 && used > input.cap)
    return {
      allowed: false,
      used: input.rate.used,
      windowstartedat: input.rate.windowstartedat,
      resetsat: input.rate.windowstartedat + window,
    };
  return {
    allowed: true,
    used,
...
export function sendframe(
  ...
  now: number,
): { state: relayclientstate; sent: boolean; reason?: string } {
  if (state.state !== "connected")
    return { state, sent: false, reason: "The bridge socket stays down; the frame waits in the offline queue." };
...
export function receiveframe(
  ...
  frame: string,
  now: number,
): { state: relayclientstate; envelope?: serverenvelope; rejection?: string } {
  ...
  if (!verifyframeauth(envelope, state))
    return {
      state,
      rejection: `The frame ${envelope.opid} failed the frame authentication of the session ${state.sessionid}.`,
    };
  if (envelope.op === "eventpost" && envelope.body !== undefined) {
    ...
    if (stream !== "") {
      const record: bridgeeventrecord = {
        ...
        kind: kind as bridgeeventrecord["kind"],
        opid: envelope.opid,
        stream,
        payload: (envelope.body.payload as Record<string, unknown>) ?? {},
        at: envelope.at,
        delivered: true,
      };
      const next: relayclientstate = {
        ...state,
        received: state.received + 1,
        lastframeat: now,
        acks: [...state.acks, envelope],
        streams: { ...state.streams, [stream]: [...(state.streams[stream] ?? []), record] },
      };
      return { state: next, envelope };
    }
  }
  return {
    state: { ...state, received: state.received + 1, lastframeat: now, acks: [...state.acks, envelope] },
    envelope,
  };
}
...
export function enqueuebridge(
  state: relayclientstate,
  input: { op: serveroperation; opid: string; body: Record<string, unknown>; now: number },
): relayclientstate {
  if (state.queue.some((record) => record.opid === input.opid)) return state;
  const record: bridgequeuerecord = {
    opid: input.opid,
    op: input.op,
    sessionid: state.sessionid,
    body: input.body,
...
export function replaybridgequeue(
  state: relayclientstate,
  socket: relaysocket,
  now: number,
): { state: relayclientstate; replayed: number; deduped: number } {
  let replayed = 0;
  let deduped = 0;
  ...
  for (const record of state.queue) {
    ...
    if (record.sentat !== undefined || state.state !== "connected") {
      queue.push(record);
      continue;
    }
    socket.send(
      frameof(frameauth(composeenvelope({ op: record.op, opid: record.opid, at: now, body: record.body }), state)),
    );
...
export function postevent(
  ...
  event: { kind: bridgeeventrecord["kind"]; stream: string; payload: Record<string, unknown> },
  now: number,
  rate?: { cap?: number; window?: number },
): { state: relayclientstate; sent: boolean; queued: boolean; reason?: string } {
  const opid = `op-${state.memberid}-${state.sequence + 1}`;
  const body = eventpostbody({ event });
  if (state.state !== "connected" || socket === undefined) {
    return {
      state: enqueuebridge({ ...state, sequence: state.sequence + 1 }, { op: "eventpost", opid, body, now }),
      sent: false,
      queued: true,
      reason: "The bridge socket stays down; the event waits in the offline queue.",
    };
  }
  const window = bridgeframerate({
    rate: state.rate,
    ...(rate?.cap !== undefined ? { cap: rate.cap } : {}),
    ...(rate?.window !== undefined ? { window: rate.window } : {}),
    now,
  });
  if (!window.allowed) {
    return {
      state: enqueuebridge(
        {
          ...
          rate: { used: window.used, windowstartedat: window.windowstartedat },
        },
        { op: "eventpost", opid, body, now },
      ),
      sent: false,
      queued: true,
      reason: `The bridge rate window holds its ${rate?.cap} frame cap; the event waits for the window reset.`,
    };
  }
  const sent = sendframe(
    { ...state, sequence: state.sequence + 1, rate: { used: window.used, windowstartedat: window.windowstartedat } },
    socket,
    composeenvelope({ op: "eventpost", opid, at: now, body }),
    now,
  );
...
export function heartbeattick(
  ...
  now: number,
  interval?: number,
): { state: relayclientstate; beat: boolean } {
  ...
  const sent = sendframe(
    state,
    socket,
    composeenvelope({ op: "eventpost", opid, at: now, body: eventpostbody({ event }) }),
    now,
  );
...
export function bridgestatusview(state: relayclientstate): {
  ...
  if (state.url === "")
    return {
      status: "disabled",
      paired: false,
      queued: state.queue.filter((record) => record.sentat === undefined).length,
    };
  const paired = state.sessionid !== "" && state.token !== "";
  if (state.state === "connected")
    return { status: "connected", paired, queued: state.queue.filter((record) => record.sentat === undefined).length };
  if (paired)
    return {
      status: "paired",
      paired: true,
...
export async function connectrelay(input: {
  ...
  sleep?: relaysleep;
  now?: () => number;
}): Promise<{ state: relayclientstate; socket?: relaysocket; error?: string }> {
  const sleep: relaysleep =
    input.sleep ??
    ((milliseconds: number): Promise<void> => new Promise((resolve) => setTimeout(resolve, Math.max(0, milliseconds))));
  const now = input.now ?? Date.now;
  if (input.state.url.trim() === "")
    return {
      state: {
        ...input.state,
        state: "idle",
        lasterror:
          "The bridge stays disabled because no relay url is configured; the url is always the user's setting.",
      },
      error: "The bridge stays disabled because no relay url is configured; the url is always the user's setting.",
    };
  ...
  for (let attempt = 0; attempt < budget; attempt += 1) {
      ...
      if (attempt < budget - 1) {
        ...
        if (input.rotate !== undefined) {
          try {
            ...
            /* the rotation seam failure keeps the current token for the next attempt */
          }
        }
      }
      continue;
    ...
    const reply = await new Promise<{ frame?: string; error?: string }>((resolve) => {
      ...
      socket.onclose = (code) =>
        resolve({
          error: `The relay socket closed with code ${code} before the ${firstjoin ? "session create" : "join"} reply.`,
        });
      ...
      socket.send(
        frameof(
          frameauth(composeenvelope({ op: createsession ? "sessioncreate" : "sessionjoin", opid, at: now(), body }), {
            ...state,
            token: state.token,
          }),
        ),
    ...
    const rotatedtoken =
      envelope.body !== undefined && typeof envelope.body.token === "string" ? envelope.body.token.trim() : "";
    const sessionid =
      envelope.body !== undefined && typeof envelope.body.sessionid === "string"
        ? envelope.body.sessionid.trim()
        : state.sessionid;
    state = {
      ...state,
      state: "connected",
      sessionid: sessionid !== "" ? sessionid : state.sessionid,
      token: rotatedtoken !== "" ? rotatedtoken : state.token,
      rotations: rotatedtoken !== "" ? state.rotations + 1 : state.rotations,
      openedat: now(),
      lastframeat: now(),
      lastheartbeatat: now(),
      acks: [...state.acks, envelope],
  ...
  return {
    state: { ...state, state: "closed", attempts: state.attempts, closedat: now(), lasterror: lasterror },
    error: lasterror,
  };
}
...
export function chatrowsof(
  events: bridgeeventrecord[],
  memberid: string,
): Array<{ from: string; text: string; mine: boolean; at: number }> {
  return events
    .filter((event) => event.kind === "chat")
    .map((event) => ({
      from: typeof event.payload.from === "string" ? event.payload.from : "",
      text: typeof event.payload.text === "string" ? event.payload.text : "",
      mine: typeof event.payload.from === "string" && event.payload.from === memberid,
      at: event.at,
    }));
}
...
export function reviewcardsof(events: bridgeeventrecord[]): bridgereviewcard[] {
  return events
    .filter((event) => event.kind === "planproposal")
    .map((event) => {
      const steps = Array.isArray(event.payload.steps)
        ? (event.payload.steps as Array<Record<string, unknown>>)
            .filter((step) => typeof step.id === "string" && typeof step.kind === "string")
            .map((step) => ({
              id: String(step.id),
              kind: String(step.kind),
              origin: typeof step.origin === "string" ? step.origin : "",
              sensitive: step.sensitive === true,
            }))
        : [];
      return {
        proposalid: typeof event.payload.proposalid === "string" ? event.payload.proposalid : "",
        title: typeof event.payload.title === "string" ? event.payload.title : "",
        steps,
        ...(typeof event.payload.timeoutat === "number" ? { timeoutat: event.payload.timeoutat } : {}),
        at: event.at,
      };
    })
...
export function carddecision(
  card: bridgereviewcard,
  decision: bridgereviewdecision["decision"],
  by: string,
  now: number,
  note?: string,
): bridgereviewdecision {
  if (card.proposalid.trim() === "") throw new Error("The review decision names its proposal.");
  if (decision !== "approved" && decision !== "changes" && decision !== "refused")
...
export function reviewgateoutcome(decision: bridgereviewdecision): { state: "approved" | "refused"; reason: string } {
  if (decision.decision === "approved")
    return {
      state: "approved",
      reason: `The site review approved the plan proposal ${decision.proposalid}${decision.note !== undefined ? ` with the note ${decision.note}` : ""}; the extension review gate resolves as approved and the extension stays the only executor.`,
    };
  return {
    state: "refused",
    reason: `The site review returned ${decision.decision} for the plan proposal ${decision.proposalid}${decision.note !== undefined ? ` with the note ${decision.note}` : ""}; the proposal returns to the review surface and no step executes.`,
  };
}
...
export function chateventof(
  text: string,
  from: string,
): { kind: servereventtype; stream: string; payload: Record<string, unknown> } {
...
export function planproposaleventof(card: bridgereviewcard): {
  kind: servereventtype;
  stream: string;
  payload: Record<string, unknown>;
} {
...
export function widgetview(input: { status: string; events: bridgeeventrecord[]; memberid: string }): {
  status: string;
  rows: Array<{ from: string; text: string; mine: boolean; at: number }>;
  ...
  const answered = new Set(
    input.events
      .filter((event) => event.kind === "planreview")
      .map((event) => (typeof event.payload.proposalid === "string" ? event.payload.proposalid : ""))
      .filter((id) => id !== ""),
  );
  return {
    status: input.status,
    rows: chatrowsof(input.events, input.memberid),
    cards,
    pending: cards.filter((card) => !answered.has(card.proposalid)).length,
  };
}
...
export function pairingpanel(
  record: Parameters<typeof pairingcountdown>[0],
  now: number,
): { code: string; secondsleft: number; expired: boolean; label: string } {
...
export function bridgestatuslabel(view: { status: string; paired: boolean; queued: number }): string {
  if (view.status === "connected")
    return `Bridge: connected${view.paired ? " and paired" : ""}${view.queued > 0 ? ` · ${view.queued} queued frame${view.queued === 1 ? "" : "s"}` : ""}.`;
  if (view.status === "paired")
    return `Bridge: paired, socket offline${view.queued > 0 ? ` · ${view.queued} queued frame${view.queued === 1 ? "" : "s"}` : ""}.`;
  if (view.status === "offline")
    return `Bridge: offline${view.queued > 0 ? ` · ${view.queued} queued frame${view.queued === 1 ? "" : "s"}` : ""}.`;
...
export function wsbridgebindcheck(input: { bind?: string; port?: number }): {
  ok: boolean;
  bind: string;
  port: number;
  reason?: string;
  ...
  if (!wsbridgelocalhosts.includes(bind))
    return {
      ok: false,
      bind,
      port: 0,
      reason: `The wsbridge binds a localhost address only; the ${bind} address accepts connections from other machines and the bridge never leaves the machine.`,
    };
  ...
  if (!Number.isInteger(port) || port < 0 || port > 65535)
    return {
      ...
      bind,
      port: 0,
      reason: `The wsbridge port stays a whole number between zero and 65535; the zero port asks the socket for a random free port.`,
    };
...
export function wsbridgesessionstart(input: {
  port: number;
  now: number;
  token: string;
  ...
  if (!Number.isInteger(input.port) || input.port < 0 || input.port > 65535)
    throw new Error("The wsbridge session port stays a whole number between zero and 65535.");
  const token = input.token.trim();
  if (token === "")
    throw new Error("The wsbridge session carries its per session token; an empty token authenticates nothing.");
  if (input.idlewindow !== undefined && (!Number.isFinite(input.idlewindow) || input.idlewindow <= 0))
    throw new Error("The wsbridge idle window stays a positive millisecond count the user chose.");
  return {
    id: input.id ?? `wsbridge-${input.now}`,
    port: input.port,
    tokenhash: input.hashof(token),
    boundat: input.now,
    ...(input.idlewindow !== undefined
      ? { idlewindow: input.idlewindow, expiresat: input.now + input.idlewindow }
      : {}),
...
export function wsbridgeconnection(session: wsbridgesession, now: number): wsbridgesession {
  ...
  return {
    ...
    lastframeat: now,
    ...(idlewindow !== undefined ? { expiresat: now + idlewindow } : {}),
  };
}
...
export function wsbridgeextensionconnect(
  session: wsbridgesession,
  now: number,
): { session?: wsbridgesession; refused?: string } {
  if (session.extensionconnected)
    return {
      refused: `The wsbridge session holds one extension connection at a time; the extension side of the session ${session.id} stays taken.`,
    };
...
export function wsbridgeframeauth(
  session: wsbridgesession,
  token: string,
  hashof: (text: string) => string,
  ...
  if (trimmed === "")
    return {
      ok: false,
      reason: "The wsbridge frame carries its session token; an anonymous frame refuses at the door.",
    };
  if (hashof(trimmed) !== session.tokenhash)
    return {
      ok: false,
      reason: `The wsbridge frame token fails the session ${session.id} authentication; a wrong token never reaches the body.`,
    };
  return { ok: true };
}
...
export function wsbridgeidlesweep(
  session: wsbridgesession,
  now: number,
): { expired: boolean; session: wsbridgesession; reason?: string } {
  ...
  if (now >= session.expiresat)
    return {
      expired: true,
      session,
      reason: `The wsbridge session ${session.id} sat quiet past its ${session.idlewindow} millisecond idle window; the session expired and the token stopped verifying.`,
    };
...
export function wsbridgeenvelopeof(input: {
  opid: string;
  at: number;
  frame: nativeframe;
  sessionid?: string;
  ...
  return composeenvelope({
    ...
    body: { kind: "progress", stream: wsbridgestreamname, payload: { frame: input.frame } },
  });
}
...
export function wsbridgeframeof(envelope: { op: string; body?: Record<string, unknown> }): {
  frame?: nativeframe;
  reason?: string;
} {
  if (envelope.op !== "eventpost")
    return {
      reason: `The wsbridge envelope carries the eventpost operation; a ${envelope.op} envelope never reaches the native transport.`,
    };
  const body = envelope.body ?? {};
  if (body.stream !== wsbridgestreamname)
    return {
      reason: `The wsbridge envelope rides the ${wsbridgestreamname} stream; a ${String(body.stream)} envelope stays outside the native transport.`,
    };
  const frame = body.payload;
  if (frame === undefined || frame === null || typeof frame !== "object" || Array.isArray(frame))
    return { reason: "The wsbridge envelope payload carries the native frame object." };
  ...
  if (candidate === undefined || candidate === null || typeof candidate !== "object" || Array.isArray(candidate))
    return { reason: "The wsbridge envelope payload wraps its native frame under the frame key." };
  const typed = candidate as Record<string, unknown>;
  if (typeof typed.kind !== "string" || typeof typed.correlationid !== "string")
    return { reason: "The wsbridge native frame carries its kind and its correlation id." };
  return {
    frame: {
      kind: typed.kind as nativeframe["kind"],
      correlationid: typed.correlationid,
      ...(typeof typed.sessionid === "string" ? { sessionid: typed.sessionid } : {}),
      ...(typed.body !== undefined &&
      typed.body !== null &&
      typeof typed.body === "object" &&
      !Array.isArray(typed.body)
        ? { body: typed.body as Record<string, unknown> }
        : {}),
    },
  };
}
...
export function wsbridgeadvertiseframe(session: wsbridgesession, token: string, correlationid: string): nativeframe {
  const trimmed = token.trim();
  if (trimmed === "")
    throw new Error("The wsbridge advertisement carries the per session token; an empty token advertises nothing.");
  return {
    kind: "advertisement",
    correlationid,
    body: {
      port: session.port,
      token: trimmed,
      ...(session.idlewindow !== undefined ? { idlewindow: session.idlewindow } : {}),
    },
  };
}
...
export function wsbridgereport(
  session: wsbridgesession,
  now: number,
): {
  id: string;
...
export function wsbridgeframecounted(
  session: wsbridgesession,
  ...
  return {
    ...session,
    received: session.received + (direction === "received" ? 1 : 0),
    sent: session.sent + (direction === "sent" ? 1 : 0),
    lastframeat: now,
    ...(idlewindow !== undefined ? { expiresat: now + idlewindow } : {}),
  };
}
...
import {
  protocolversion,
  type companionhandshake,
  type nativecallclass,
  ...
  return [
    {
      ...
      callclass: "sensitive",
      description:
        "Opens one os dialog on the user desktop; the surface addresses the user directly, so the call class stays sensitive and the surface asks for its own consent grant.",
    },
    {
      surface: "notification",
      callclass: "interaction",
      description:
        "Posts one os notification on the user desktop; the surface writes outside the browser, so the call class stays interaction and the surface asks for its own consent grant.",
    },
  ];
}
...
export function nativehostcapabilities(): { protocol: number; surfaces: nativesurfacekind[]; heartbeat: boolean } {
  return {
    protocol: nativebridgeprotocolmajor,
    surfaces: nativesurfacecatalog().map((entry) => entry.surface),
    heartbeat: true,
  };
}
...
export function nativehostmanifesttemplate(input: { hostname: string; companionpath: string; extensionid?: string }): {
  manifest: { name: string; description: string; path: string; type: "stdio"; allowed_origins: string[] };
  text: string;
} {
  const hostname = input.hostname.trim();
  if (hostname === "") throw new Error("The native host manifest names its host; an empty host name never registers.");
  if (!/^[a-z0-9][a-z0-9._-]*$/.test(hostname))
    throw new Error(
      `The native host name ${hostname} stays a plain dns style token; a name with spaces or capitals never registers.`,
    );
  const companionpath = input.companionpath.trim();
  if (companionpath === "")
    throw new Error(
      "The native host manifest carries the companion path the user chose; an empty path never launches a host.",
    );
  ...
  const manifest = {
    name: hostname,
    description:
      "The devthink companion process: an optional native host that speaks length prefixed json over stdio and never runs until the user installs it.",
    path: companionpath,
    type: "stdio" as const,
...
export function hostmanifestdestination(input: {
  hostname: string;
  profiledir?: string;
  systemwide?: boolean;
  platform?: "linux" | "macos" | "windows";
}): { path: string; systemwide: boolean; refused?: string } {
  const hostname = input.hostname.trim();
  if (hostname === "")
    return {
      path: "",
      systemwide: false,
      refused: "The host manifest destination names its host; an empty host name never writes.",
    };
  const systemwide = input.systemwide === true;
  const profiledir = (input.profiledir ?? "").trim();
  if (!systemwide && profiledir === "")
    return {
      path: "",
      systemwide: false,
      refused:
        "The installer writes the host manifest into the user profile directory; a system wide install needs the explicit flag and the user never gets one silently.",
    };
  if (systemwide) {
...
export async function installnativehost(input: {
  profiledir?: string;
  hostname: string;
  extensionid: string;
  ...
  systemwide?: boolean;
  platform?: "linux" | "macos" | "windows";
  io: { writefile: (path: string, text: string) => Promise<void>; mkdir: (dir: string) => Promise<void> };
}): Promise<{ state: nativehoststate; manifestpath: string; refused?: string }> {
  if (!input.consent)
    return {
      state: nativedefaultstate(),
      manifestpath: "",
      refused:
        "The install consent gate explains the scope before any host registration; the installer writes no manifest without the recorded consent.",
    };
  const extensionid = input.extensionid.trim();
  if (extensionid === "")
    return {
      state: nativedefaultstate(),
      manifestpath: "",
      refused: "The host manifest allows the origins of one extension id; an empty id never registers a host.",
    };
  if (input.systemwide === true && (input.profiledir ?? "").trim() !== "")
    return {
      state: nativedefaultstate(),
      manifestpath: "",
      refused:
        "The install targets either the user profile directory or, with the explicit flag, the system directory; never both at once.",
    };
  const destination = hostmanifestdestination({
    hostname: input.hostname,
    ...(input.profiledir !== undefined ? { profiledir: input.profiledir } : {}),
    ...(input.systemwide !== undefined ? { systemwide: input.systemwide } : {}),
    ...(input.platform !== undefined ? { platform: input.platform } : {}),
  });
  if (destination.refused !== undefined)
    return { state: nativedefaultstate(), manifestpath: "", refused: destination.refused };
  const template = nativehostmanifesttemplate({
    hostname: input.hostname,
    companionpath: input.companionpath,
    extensionid,
  });
  ...
  const state: nativehoststate = {
    installed: true,
    hostname: template.manifest.name,
    extensionid,
...
export async function uninstallnativehost(input: {
  profiledir?: string;
  hostname: string;
  now: number;
  systemwide?: boolean;
  platform?: "linux" | "macos" | "windows";
  io: { removefile: (path: string) => Promise<void>; exists: (path: string) => Promise<boolean> };
}): Promise<{ state: nativehoststate; manifestpath: string; removed: boolean; refused?: string }> {
  const destination = hostmanifestdestination({
    hostname: input.hostname,
    ...(input.profiledir !== undefined ? { profiledir: input.profiledir } : {}),
    ...(input.systemwide !== undefined ? { systemwide: input.systemwide } : {}),
  ...
  if (destination.refused !== undefined)
    return { state: nativedefaultstate(), manifestpath: "", removed: false, refused: destination.refused };
...
export function nativechoices(settings: runsettings | undefined): {
  ...
  return {
    ...(current.nativeinstallconsent !== undefined ? { nativeinstallconsent: current.nativeinstallconsent } : {}),
    ...(current.nativetransportconsent !== undefined ? { nativetransportconsent: current.nativetransportconsent } : {}),
    ...(current.nativecallclassconsents !== undefined
      ? { nativecallclassconsents: current.nativecallclassconsents }
      : {}),
    ...(current.nativesurfaceconsents !== undefined ? { nativesurfaceconsents: current.nativesurfaceconsents } : {}),
    ...(current.nativeidlewindow !== undefined ? { nativeidlewindow: current.nativeidlewindow } : {}),
    ...(current.nativeheartbeatinterval !== undefined
      ? { nativeheartbeatinterval: current.nativeheartbeatinterval }
      : {}),
    ...(current.nativecallratelimit !== undefined ? { nativecallratelimit: current.nativecallratelimit } : {}),
    ...(current.nativecallratewindow !== undefined ? { nativecallratewindow: current.nativecallratewindow } : {}),
  };
}
...
export function nativecorrelationid(seed: string, sequence: number): string {
  const trimmed = seed.trim();
  if (trimmed === "")
    throw new Error("The correlation id names its sender seed; an empty seed never correlates a frame.");
  if (!Number.isInteger(sequence) || sequence < 1)
    throw new Error("The correlation id sequence stays a positive whole number per run.");
...
export function nativeframeof(
  kind: nativeframe["kind"],
  ...
  if (trimmed === "")
    throw new Error("The native frame carries its correlation id; an empty id never correlates a frame.");
  return {
    kind,
...
export function parsecompanionhandshake(
  frame: nativeframe,
  now: number,
): { handshake?: companionhandshake; error?: nativeerror } {
  if (frame.kind !== "handshake")
    return {
      error: nativeerrorof(
        "handshake",
        `The handshake answer carries the handshake kind; a ${frame.kind} frame never completes a handshake.`,
        "reconnect",
        now,
      ),
    };
  ...
  if (build === "")
    return {
      error: nativeerrorof(
        "handshake",
        "The handshake answer carries the companion build version; an empty version never attaches.",
        "reconnect",
        now,
      ),
    };
  ...
  if (!Number.isInteger(protocol) || protocol < 1)
    return {
      error: nativeerrorof(
        "handshake",
        "The handshake answer carries its native bridge protocol version as a positive whole number.",
        "reconnect",
        now,
      ),
    };
  const surfaces = Array.isArray(body.surfaces)
    ? body.surfaces.filter(
        (surface): surface is nativesurfacekind => surface === "osdialog" || surface === "notification",
      )
    : [];
  const wsbridgeport = typeof body.wsbridgeport === "number" ? body.wsbridgeport : 0;
  if (!Number.isInteger(wsbridgeport) || wsbridgeport < 0)
    return {
      error: nativeerrorof(
        "handshake",
        "The handshake answer carries the wsbridge port as a whole number; zero keeps the bridge down.",
        "reconnect",
        now,
      ),
    };
...
export function nativeprotocolcompatible(
  ours: string | number,
  theirs: string | number,
): { ok: boolean; ours: number; theirs: number } {
...
export function negotiatenativecapabilities(input: {
  ours?: { protocol: number; surfaces: nativesurfacekind[]; heartbeat: boolean };
  theirs: { protocol: string | number; surfaces: nativesurfacekind[]; heartbeat?: boolean };
}): { ok: boolean; protocol: number; surfaces: nativesurfacekind[]; heartbeat: boolean; reason?: string } {
  ...
  if (!compat.ok)
    return {
      ...
      surfaces: [],
      heartbeat: false,
      reason: `The companion speaks the native bridge protocol major version ${compat.theirs} while this build speaks ${compat.ours}; the upgrade path keeps compatibility for one major version and the run degrades with the transport off.`,
    };
...
export function attachnativehost(state: nativehoststate, handshake: companionhandshake, now: number): nativehoststate {
  return {
    ...
    installed: true,
    port: "attached",
    companionversion: handshake.build,
    companionprotocol: handshake.protocol,
    wsbridgeport: handshake.wsbridgeport,
    attachedat: now,
    updatedat: now,
  };
}
...
export function crashnativehost(state: nativehoststate, now: number): nativehoststate {
  return {
    ...
    lasterrors: [
      nativeerrorof(
        "port",
        "The companion process crashed under the native port; the run stays alive and the extension reattaches on the next host start.",
        "reconnect",
        now,
      ),
      ...(state.lasterrors ?? []),
...
export function reattachnativehost(
...
export function nativeportliveness(input: { lastheartbeatat?: number; now: number; interval?: number }): {
  alive: boolean;
  reason?: string;
} {
  if (input.interval === undefined)
    return {
      alive: true,
      reason:
        "The heartbeat interval stays unset; the liveness check never expires a host without the user configured cadence.",
    };
  if (input.lastheartbeatat === undefined)
    return {
      alive: false,
      reason: "The native port heard no heartbeat yet; the host stays silent under the configured interval.",
    };
  if (input.now - input.lastheartbeatat > input.interval)
    return {
      alive: false,
      reason: `The last heartbeat sits ${input.now - input.lastheartbeatat} milliseconds back, past the ${input.interval} millisecond interval; the host process stays silent and the extension reattaches on its restart.`,
    };
  return { alive: true };
}
...
export function nativeframecheck(frame: nativeframe, sessionid: string): { allowed: boolean; reason?: string } {
  if (frame.correlationid.trim() === "")
    return {
      allowed: false,
      reason: "The native frame carries its correlation id; an anonymous frame never reaches the engine.",
    };
  if (sessionid.trim() === "")
    return {
      allowed: false,
      reason:
        "The native transport pairs its frames with the session id; a transport without a session never accepts a frame.",
    };
  if (frame.sessionid !== undefined && frame.sessionid !== sessionid)
    return {
      allowed: false,
      reason: `The native frame session ${frame.sessionid} sits outside the paired session ${sessionid}; the origin and session checks refuse the frame.`,
    };
  return { allowed: true };
}
...
export const nativesecretkeys: string[] = [
  "apikey",
  "api_key",
  "key",
  "token",
...
export function nativesecretexclusion(frame: nativeframe): { ok: boolean; held: string[] } {
  const body = frame.body ?? {};
  const held = Object.keys(body).filter((key) =>
    nativesecretkeys.includes(key.toLowerCase().replaceAll(" ", "").replaceAll("-", "")),
  );
...
export function redactnativeframe(frame: nativeframe): nativeframe {
  ...
  for (const [key, value] of Object.entries(body))
    if (!nativesecretkeys.includes(key.toLowerCase().replaceAll(" ", "").replaceAll("-", ""))) redacted[key] = value;
...
export function nativeerrorof(
  family: nativeerror["family"],
  message: string,
  retry: nativeerror["retry"],
  now: number,
): nativeerror {
...
export function nativedegradationof(input: {
  state: nativehoststate | undefined;
  negotiated?: { ok: boolean; reason?: string };
}): { degraded: boolean; reason: string } {
  if ((input.state?.installed ?? false) === false)
    return {
      degraded: true,
      reason:
        "The native host stays absent: nothing installed, the transport disabled and the run alive with every step inside the browser.",
    };
  if (input.negotiated !== undefined && !input.negotiated.ok)
    return {
      degraded: true,
      reason:
        input.negotiated.reason ??
        "The native host speaks another native bridge protocol major version; the run stays alive with the transport off.",
    };
  if (input.state?.port === "crashed")
    return {
      degraded: true,
      reason:
        "The native host crashed under the port; the run stays alive and waits for the reattach on the host restart.",
    };
  return {
    degraded: false,
    reason:
      "The native host sits attached with a negotiated capability set; the transport carries the consented call classes.",
  };
}
...
export function nativeratecheck(input: { calls: nativecallrecord[]; cap?: number; window?: number; now: number }): {
  allowed: boolean;
  used: number;
  cap?: number;
  reason?: string;
  ...
  if (!Number.isFinite(input.cap) || input.cap < 1)
    return {
      allowed: false,
      used: input.calls.length,
      cap: input.cap,
      reason: `The native transport rate cap of ${String(input.cap)} stays a finite positive count.`,
    };
  const window = input.window ?? Number.POSITIVE_INFINITY;
  const used = input.calls.filter(
    (record) => window === Number.POSITIVE_INFINITY || input.now - record.at <= window,
  ).length;
  if (used >= input.cap)
    return {
      allowed: false,
      used,
      cap: input.cap,
      reason: `The native transport hit its ${input.cap} call cap with ${used} calls inside the window; the cap stays the user's choice and the session waits.`,
    };
...
export function nativecallrecordof(input: {
  id: string;
  correlationid: string;
  surface: string;
  callclass: nativecallclass;
...
export function nativecallevent(record: nativecallrecord): jsonrpcframe {
  return notificationframe("native/call", {
...
export function nativesurfacegrant(
  surface: string,
  ...
  if (known === undefined)
    return {
      allowed: false,
      reason: `The native surface ${surface} sits outside the surface catalog; an unknown surface never grants.`,
    };
  if ((settings?.nativesurfaceconsents ?? []).includes(known.surface))
    return {
      allowed: true,
      reason: `The user granted the ${known.surface} surface its own consent stamp; the surface runs behind the class gate too.`,
    };
  return {
    allowed: false,
    reason: `The native surface ${known.surface} holds no consent grant; a desktop surface never opens from a class grant alone.`,
  };
}
...
export function nativeclassgrant(
  callclass: nativecallclass,
  settings: runsettings | undefined,
): { allowed: boolean; reason: string } {
  if ((settings?.nativecallclassconsents ?? []).includes(callclass))
    return { allowed: true, reason: `The user consented to the ${callclass} call class over the native transport.` };
  return {
    allowed: false,
    reason: `The ${callclass} call class holds no consent grant over the native transport; the gate asks per class and never widens a read grant.`,
  };
}
...
export function nativesurfaceresult(input: {
  surface: string;
  callclass: nativecallclass;
  details: Record<string, unknown>;
  now: number;
}): { surface: string; callclass: nativecallclass; details: Record<string, unknown>; at: number } {
  ...
  const held = Object.keys(input.details).filter((key) =>
    nativesecretkeys.includes(key.toLowerCase().replaceAll(" ", "").replaceAll("-", "")),
  );
...
export function nativediagnostics(input: {
  state: nativehoststate | undefined;
  settings?: runsettings | undefined;
  sessions?: wsbridgesession[] | undefined;
  negotiated?: { ok: boolean; reason?: string } | undefined;
  now: number;
}): {
  installed: boolean;
  port: string;
  ...
  degraded: { degraded: boolean; reason: string };
  transportconsent: boolean;
  classconsents: nativecallclass[];
  surfaceconsents: nativesurfacekind[];
  sessions: Array<{ port: number; connections: number; expired: boolean }>;
  ...
  const sessions = (input.sessions ?? []).map((session) => ({
    port: session.port,
    connections: session.connections,
    expired: session.expiresat !== undefined && session.expiresat <= input.now,
  }));
  return {
    ...
    degraded: nativedegradationof({
      state,
      ...(input.negotiated !== undefined ? { negotiated: input.negotiated } : {}),
    }),
