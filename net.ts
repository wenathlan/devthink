...
export function newexchange(input: { id: string; runid: string; stepid: string; correlationid: string; fact: resourcefact; at: number }): exchangerecord {
  const verdict = failureclass(input.fact);
  let origin = "";
  try { origin = new URL(input.fact.url).origin; } catch { origin = ""; }
  ...
  return { id: input.id, runid: input.runid, stepid: input.stepid, correlationid: input.correlationid, url: input.fact.url, origin, method, status: verdict.status, statusclass, ...(verdict.errorclass !== undefined ? { errorclass: verdict.errorclass } : {}), source: "page", ...(input.fact.initiator ? { initiator: input.fact.initiator } : {}), timing: Math.round(input.fact.duration), bytes: input.fact.transfer, at: input.at };
}
...
export function filterexchanges(exchanges: exchangerecord[], filter: { runid?: string; origin?: string; status?: number | "failed" }): exchangerecord[] {
  return exchanges.filter(exchange => (filter.runid === undefined || exchange.runid === filter.runid) && (filter.origin === undefined || exchange.origin === filter.origin) && (filter.status === undefined || (filter.status === "failed" ? exchange.errorclass !== undefined : exchange.status === filter.status)));
}
...
export function headerfilterof(value: unknown): headerfilter {
  ...
  const names = (source: unknown): string[] => Array.isArray(source) ? source.filter((name): name is string => typeof name === "string" && name.trim().length > 0).map(name => name.trim().toLowerCase()) : [];
...
export function bodyfilterof(value: unknown): bodyfilter {
  ...
  if (Array.isArray(entry.mimes)) filter.mimes = entry.mimes.filter((mime): mime is string => typeof mime === "string" && mime.trim().length > 0).map(mime => mime.trim().toLowerCase());
...
const privatemimes = new Set(["text/html", "text/plain", "text/xml", "application/xml", "application/json", "text/json", "application/x-www-form-urlencoded", "application/graphql", "multipart/form-data"]);
...
export function capturebody(input: { ref: string; runid: string; exchange: exchangerecord; body: string; mime: string; filter: bodyfilter; at: number }): { record: bodyrecord; truncated: boolean } | { refused: string } {
...
export function payloadshapeof(body: string | undefined): string[] {
  ...
  try {
    ...
    const shape = (value: unknown): string[] => value && typeof value === "object" && !Array.isArray(value) ? Object.keys(value as Record<string, unknown>) : [];
...
function isapicandidate(exchange: exchangerecord): boolean {
  ...
  try { return /\/api\/|\/graphql|\.json($|\?)|\/v\d+\//i.test(new URL(exchange.url).pathname); } catch { return false; }
}
...
export function apientries(exchanges: exchangerecord[], bodies: bodyrecord[]): apimapentry[] {
  ...
  const groups = new Map<string, { endpoint: string; method: string; origin: string; mimes: Map<string, number>; frequency: number; json: number; captured: number; shapes: Map<string, number>; correlationids: string[] }>();
  for (const exchange of exchanges) {
    if (!isapicandidate(exchange)) continue;
    let endpoint = exchange.url;
    let origin = exchange.origin;
    try { const parsed = new URL(exchange.url); endpoint = `${parsed.origin}${parsed.pathname}`; origin = parsed.origin; } catch { /* an unparsable url keeps the raw endpoint */ }
    const key = `${exchange.method} ${endpoint}`;
    const group = groups.get(key) ?? { endpoint, method: exchange.method, origin, mimes: new Map<string, number>(), frequency: 0, json: 0, captured: 0, shapes: new Map<string, number>(), correlationids: [] };
    group.frequency += 1;
...
export function apireplayspecof(value: unknown): apireplayspec | undefined {
  ...
  if (entry.overrides !== undefined && entry.overrides !== null && typeof entry.overrides === "object" && !Array.isArray(entry.overrides)) {
  ...
  if (Array.isArray(entry.paths)) spec.paths = entry.paths.filter((path): path is string => typeof path === "string" && path.trim().length > 0);
  return spec;
}
...
export function extractvalues(body: string, paths: string[]): Array<{ path: string; value?: unknown; missing?: boolean }> {
  let parsed: unknown;
  try { parsed = JSON.parse(body); } catch { return paths.map(path => ({ path, missing: true })); }
  const fields = readpath(parsed, paths.map(path => ({ name: path, path, kind: "json" as const })));
  return fields.map(field => ({ path: field.path, ...(field.value !== undefined ? { value: field.value } : {}), ...(field.missing ? { missing: true } : {}) }));
}

/* ── Merged from socketbus.ts ── */

import type { channelrecord, channelstate, messageenvelope, messagefilter, pollcursor, socketoptions } from "./types.js";
...
export type socketconnect = (url: string, protocols: string[]) => Promise<{ open: boolean; code?: number; error?: string }>;
...
export function channeloptionsof(value: unknown): { url: string; options: socketoptions } | undefined {
  ...
  if (Array.isArray(entry.protocols)) options.protocols = entry.protocols.filter((item): item is string => typeof item === "string" && item.trim().length > 0);
...
export function newchannel(input: { id: string; runid: string; stepid: string; kind: "websocket" | "sse"; url: string; protocols?: string[]; at: number }): channelrecord {
  return { id: input.id, runid: input.runid, stepid: input.stepid, kind: input.kind, url: input.url, origin: channelorigin(input.url), state: "connecting", openedat: input.at, sent: 0, received: 0, reconnects: 0, ...(input.protocols !== undefined && input.protocols.length > 0 ? { protocols: [...input.protocols] } : {}) };
}
 ...
 */
export async function openchannel(input: { record: channelrecord; options: socketoptions; connect: socketconnect; sleep?: (milliseconds: number) => Promise<void>; now?: () => number }): Promise<channelrecord> {
...
function tagmessage(state: busstate, channelid: string, stream: string, payload: string, at: number): { state: busstate; envelope: messageenvelope } {
...
export function publishmessage(state: busstate, channelid: string, stream: string, payload: string, at: number): { state: busstate; envelope: messageenvelope } {
...
export function receivemessage(state: busstate, channelid: string, stream: string, payload: string, at: number): { state: busstate; envelope: messageenvelope } {
...
export function collectmessages(state: busstate, channelid: string, filter: messagefilter | undefined): { state: busstate; matched: messageenvelope[] } {
  const limit = filter?.limit !== undefined && Number.isFinite(filter.limit) && filter.limit >= 1 ? Math.floor(filter.limit) : Number.POSITIVE_INFINITY;
...
export function sequenceintegrity(envelopes: messageenvelope[]): { ok: boolean; gaps: Array<{ channelid: string; expected: number; found: number }> } {
...
export function sserequestheaders(record: { lasteventid?: string }): Record<string, string> {
  return { accept: "text/event-stream", ...(record.lasteventid !== undefined && record.lasteventid !== "" ? { "last-event-id": record.lasteventid } : {}) };
}
...
export function subscriptionoptionsof(value: unknown): { url: string; lifetime?: number; lasteventid?: string; cancel: { kind: "stop" | "lifetime"; value: string | number } } | undefined {
  if (!value || typeof value !== "object" || Array.isArray(value)) return undefined;
  const entry = value as Record<string, unknown>;
  if (typeof entry.url !== "string" || !entry.url.trim()) return undefined;
  ...
  const result: { url: string; lifetime?: number; lasteventid?: string; cancel: { kind: "stop" | "lifetime"; value: string | number } } = { url: entry.url.trim(), cancel: { kind: cancelrecord.kind, value: cancelrecord.value } };
...
export function pollcursorof(value: unknown): pollcursor | undefined {
  ...
  const cursor: pollcursor = { url: entry.url.trim(), cursorfield: entry.cursorfield.trim(), interval: entry.interval, stop: { field: stoprecord.field.trim(), equals: stoprecord.equals } };
...
export function pollurl(cursor: pollcursor, value: string | undefined): { url: string; body?: string } {
  if (cursor.param === undefined || value === undefined) {
    return { url: cursor.url, ...(value !== undefined ? { body: JSON.stringify({ [cursor.cursorfield]: value }) } : {}) };
  }
 ...
 */
export function polldecision(input: { cursor: pollcursor; polls: number; response: unknown; cancelled?: () => boolean; expiresat?: number; now: number }): { continue: boolean; reason: string; cursor?: string; next?: { url: string; body?: string; wait: number } } {
  if (input.cancelled?.() === true) return { continue: false, reason: "The long poll loop was cancelled." };
  if (input.expiresat !== undefined && input.now >= input.expiresat) return { continue: false, reason: "The long poll loop stopped at the reviewed plan expiry." };
  const stopvalue = cursorfrom(input.response, input.cursor.stop.field);
  if (stopvalue !== undefined && stopvalue === input.cursor.stop.equals) return { continue: false, reason: `The stop condition matched ${input.cursor.stop.field} ${stopvalue}.` };
  if (input.cursor.maxpolls !== undefined && input.polls + 1 >= input.cursor.maxpolls) return { continue: false, reason: `The long poll loop reached the reviewed poll ceiling of ${input.cursor.maxpolls}.` };
  const value = cursorfrom(input.response, input.cursor.cursorfield);
  const next = pollurl(input.cursor, value);
  return { continue: true, reason: "The long poll loop continues.", ...(value !== undefined ? { cursor: value } : {}), next: { ...next, wait: input.cursor.interval } };
}
...
export const controlkinds: string[] = ["blockrequest", "mockresponse", "rewriteheaders", "setcookies", "readcookies", "clearcookies", "authflow", "saveapikey", "routeproxy", "postform", "postfiles"];
...
export function blockruleof(value: unknown): { urlpattern: string; resourcetypes?: string[] } | undefined {
  ...
  if (Array.isArray(options.resourcetypes)) {
    const types = options.resourcetypes.filter((item): item is string => typeof item === "string" && item.trim().length > 0);
...
export function newblockrule(input: { id: string; runid: string; stepid: string; urlpattern: string; resourcetypes?: string[]; at: number }): blockrule {
  return { id: input.id, runid: input.runid, stepid: input.stepid, urlpattern: input.urlpattern, ...(input.resourcetypes !== undefined ? { resourcetypes: input.resourcetypes } : {}), hits: 0, registeredat: input.at };
}
...
export function mockspecof(value: unknown): { urlpattern: string; status: number; headers?: Record<string, string>; body?: string; bodyref?: string; reviewed?: boolean } | undefined {
  if (!value || typeof value !== "object" || Array.isArray(value)) return undefined;
  const options = value as Record<string, unknown>;
  if (typeof options.urlpattern !== "string" || !options.urlpattern.trim()) return undefined;
  if (typeof options.status !== "number" || !Number.isInteger(options.status) || options.status < 100 || options.status > 599) return undefined;
  ...
  const spec: { urlpattern: string; status: number; headers?: Record<string, string>; body?: string; bodyref?: string; reviewed?: boolean } = { urlpattern: options.urlpattern.trim(), status: options.status };
  if (hasbody) spec.body = options.body as string;
  if (bodyref !== "") spec.bodyref = bodyref;
  if (options.headers && typeof options.headers === "object" && !Array.isArray(options.headers)) spec.headers = options.headers as Record<string, string>;
...
export function newmockspec(input: { id: string; runid: string; stepid: string; urlpattern: string; status: number; headers?: Record<string, string>; body?: string; bodyref?: string; reviewed: boolean; at: number }): mockspec {
  return { id: input.id, runid: input.runid, stepid: input.stepid, urlpattern: input.urlpattern, status: input.status, ...(input.headers !== undefined ? { headers: input.headers } : {}), ...(input.body !== undefined ? { body: input.body } : {}), ...(input.bodyref !== undefined ? { bodyref: input.bodyref } : {}), reviewed: input.reviewed, hits: 0, registeredat: input.at };
}
...
export function headeruleof(value: unknown): { urlpattern: string; name: string; operation: "set" | "append" | "remove"; value?: string } | undefined {
  if (!value || typeof value !== "object" || Array.isArray(value)) return undefined;
  const options = value as Record<string, unknown>;
  if (typeof options.urlpattern !== "string" || !options.urlpattern.trim()) return undefined;
  ...
  const rule: { urlpattern: string; name: string; operation: "set" | "append" | "remove"; value?: string } = { urlpattern: options.urlpattern.trim(), name: options.name.trim(), operation: options.operation };
...
export function newheaderule(input: { id: string; runid: string; stepid: string; urlpattern: string; name: string; operation: "set" | "append" | "remove"; value?: string; at: number }): headerule {
  return { id: input.id, runid: input.runid, stepid: input.stepid, urlpattern: input.urlpattern, name: input.name, operation: input.operation, ...(input.value !== undefined ? { value: input.value } : {}), hits: 0, registeredat: input.at };
}
...
export function applyheaderules(url: string, headers: Record<string, string>, rules: headerule[]): { headers: Record<string, string>; applied: headerule[] } {
  ...
  for (const rule of rules) {
    ...
    if (rule.operation === "remove") { delete rewritten[name]; applied.push(rule); continue; }
...
export function cookierecordof(value: unknown): cookierecord | undefined {
  ...
  const record: cookierecord = { name: options.name.trim(), domain: options.domain.trim().toLowerCase(), path: options.path.trim(), value: options.value };
...
export function cookiedomaingranted(domain: string, grants: string[]): boolean {
  ...
  return grants.some(grant => {
    let granthost = "";
    try { granthost = new URL(grant).hostname.toLowerCase(); } catch { return false; }
...
export function redactedcookies(records: cookierecord[]): Array<{ name: string; domain: string; path: string; expiresat?: number }> {
  return records.map(record => ({ name: record.name, domain: record.domain, path: record.path, ...(record.expiresat !== undefined ? { expiresat: record.expiresat } : {}) }));
}
...
export function proxyrouteof(value: unknown): { scheme: "http" | "https" | "socks4" | "socks5"; host: string; port: number; bypass: string[] } | undefined {
  if (!value || typeof value !== "object" || Array.isArray(value)) return undefined;
  const options = value as Record<string, unknown>;
  if (options.scheme !== "http" && options.scheme !== "https" && options.scheme !== "socks4" && options.scheme !== "socks5") return undefined;
  if (typeof options.host !== "string" || !options.host.trim()) return undefined;
  if (typeof options.port !== "number" || !Number.isInteger(options.port) || options.port < 1 || options.port > 65535) return undefined;
  if (!Array.isArray(options.bypass) || options.bypass.length === 0 || !options.bypass.every((item): item is string => typeof item === "string" && item.trim().length > 0)) return undefined;
...
export function ratelimitreadof(headers: Record<string, string>, origin: string, now: number): ratelimitread | undefined {
  ...
  const read: ratelimitread = { origin, ...(remaining !== undefined ? { remaining } : {}), ...(limit !== undefined ? { limit } : {}), resetat: now, at: now };
