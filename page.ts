...
import type { capabilityreport, toolstep, keyholdstate, columnspec, datasetrow, sourceref, transformrule, consoleentry, errorrecord, loglevel, longtaskentry, rejectionrecord, timelineentry, timelinesource, bannerreport, listpattern, tableshape, dialogpolicy, fielderror, fieldkind, fieldmatch, formrecord, formentry, formreport, retryrule, navtarget, urlpattern, waitoverride, waitprofile, authrecord, curatedlink, redirectchain, redirecthop, ratelimit, ratelimitstate, safetyverdict, a11ynode, readerarticle, pointpath, speedprofile, clickablemap, mapentry, resolvedtarget, targetmode, diffentry, focusevent, jsonstate, mutationevent, mutationwatch, quietrule, selectorcandidate, wizardstate } from "./types.js";
...
export function runpagecontrol(step: toolstep, target: Element | null, root: Document = document): stepresult | Promise<stepresult> {
  ...
  switch (step.kind) {
    ...
    case "setvalue": {
      ...
      eventscontrols(field);
      return { ok: true, summary: `Field value set through the dom property with ${valueevents().join(" and ")} eventscontrols.` };
    }
    case "typeedit": {
...
export async function runpagedata(step: toolstep, target: Element | null, root: Document = document): Promise<stepresult> {
  ...
  if (step.kind === "paginateextract") {
    ...
    return {
      ...
      details: { grid: { columns: merged.columns, rows: merged.rows, children: grids.flatMap(grid => grid.children) }, pages: grids.length, rows: merged.rows.length, next: hasnext, rowcursor: merged.rows.length },
    };
  }
...
import { consolecapture, errorcapture, levelrank, loglevels, longtaskcapture, rejectioncapture, serializearg, stackframes } from "./run.js";
import { breakpointinputof, stepmodeof, watchexpressionof, overrideinputof } from "./debug.js";
import { blackboxmatches, blackboxruleof, devicepresetof, familyofkind, locationpresetof, networkpresetof, agentpresetof, permissiongrantof } from "./environments.js";
...
export function flushconsole(): Array<{ level: string; text: string; source: string; at: number; reload?: number }> {
  ...
  return flushed.map(entry => ({ level: entry.level, text: entry.text, source: "console", at: entry.at, ...(entry.reload !== undefined ? { reload: entry.reload } : {}) }));
}
...
export function fillcontrol(element: Element, entry: formentry): boolean {
  if (element instanceof HTMLInputElement && (entry.kind === "check" || element.type === "checkbox")) { element.checked = entry.value === "true" || entry.value === "on" || entry.value === "checked"; eventsforms(element); return true; }
  if (element instanceof HTMLInputElement && (entry.kind === "radio" || element.type === "radio")) { element.checked = true; eventsforms(element); return true; }
...
async function runwaiturl(step: toolstep): Promise<stepresult> {
  ...
  const started = Date.now();
  for (;;) {
    if (urlmatches(location.href, pattern)) return { ok: true, summary: `The url matched the reviewed ${pattern.mode} pattern after ${Date.now() - started} milliseconds.`, details: { url: location.href, finalurl: location.href, mode: pattern.mode, waited: Date.now() - started } };
...
export function capturepagestate(sections: string[]): { scrollx: number; scrolly: number; forms: Array<{ selector: string; value: string }>; storagekeys: string[]; storagevalues: string[]; cookienames: string[] } {
...
export function restorepagestate(state: { scrollx: number; scrolly: number; forms: Array<{ selector: string; value: string }> }): { restored: number; summary: string } {
...
export function runpagewizard(step: toolstep, target: Element | null, root: Document = document): stepresult | Promise<stepresult> {
  ...
  switch (step.kind) {
    ...
    case "selectchain": {
      ...
      return pollforwizards(() => dependentloaded(previouscount, child.options.length), "Dependent options of the child control", waitwindow);
    }
    case "picktypeahead": {
    ...
    case "fillcode": {
      ...
      return pollforwizards(() => codeready(source, code), "The reviewed one time code source", timeout).then(result => {
...
function wrapxpath(element: Element): xnode {
  return { tag: element.tagName.toLowerCase(), attributes: attributesof(element), text: owntextxpath(element), children: [...element.children].map(wrapxpath), element };
}
...
export function buildxtree(root: Document): xnode {
  return { tag: "#document", attributes: {}, text: "", children: root.documentElement ? [wrapxpath(root.documentElement)] : [] };
}
