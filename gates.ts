...
export function deletepayload(input: {
  target?: string;
  scope: string;
...
export function credspayload(label: string): Record<string, string> {
  if (label.trim() === "")
    throw new Error("The confirmcreds gate names its credential label; the value never appears.");
...
export function opengate(input: {
  ...
  if (input.stepid.trim() === "" || input.runid.trim() === "" || input.origin.trim() === "")
    throw new Error("The confirm gate needs its step, run and origin.");
  if (Object.keys(input.payload).length === 0)
    throw new Error("The confirm gate carries the payload the human reviews.");
  return {
    gateid: input.gateid ?? randomid(),
    kind: input.kind,
    stepid: input.stepid,
    runid: input.runid,
    origin: input.origin,
...
export function gatestateof(
  gates: confirmgate[],
  stepid: string,
): { state: "none" | "open" | "resolved" | "refused"; gate?: confirmgate } {
...
export function resolvegate(input: {
  ...
  actor: string;
  now: number;
}): { gates: confirmgate[]; resolution?: gateresolution } {
  if (input.actor.trim() === "")
    throw new Error("The gate resolution names its acting user; only a human resolves a gate.");
  ...
  return {
    gates: input.gates.map((candidate) =>
      candidate.gateid === input.gateid
        ? { ...candidate, state: input.decision, resolvedat: input.now, actor: input.actor }
        : candidate,
    ),
    resolution,
  };
...
export function nobatchresolution(gateids: string[]): { allowed: boolean; reason: string } {
  if (gateids.length > 1)
    return {
      allowed: false,
      reason: `One human action resolves exactly one gate; the batch of ${gateids.length} gates refuses in full because no batch approval exists.`,
    };
...
export function gateprompttext(gate: confirmgate): string {
  if (gate.kind === "confirmpay") {
    const amount =
      gate.payload.amount !== undefined ? `the amount ${gate.payload.amount}` : "an amount the step options name";
...
export function gateforstep(input: {
  ...
  if (kind === "confirmpay") {
    ...
    return opengate({
      ...
      payload: paypayload({
        ...
        payeeorigin: String(options.payeeorigin ?? input.origin),
        ...(input.step.target !== undefined && input.step.target !== "" ? { target: input.step.target } : {}),
      }),
      now: input.now,
    });
  }
  if (kind === "confirmdelete") {
    return opengate({
      ...
      payload: deletepayload({
        ...(input.step.target !== undefined && input.step.target !== "" ? { target: input.step.target } : {}),
        scope: String(options.scope ?? input.origin),
        irreversibility: String(
          options.irreversibility ?? "A destructive delete destroys state the page cannot restore.",
        ),
      }),
      now: input.now,
    });
  ...
  return opengate({
    ...
    runid: input.runid,
    origin: input.origin,
    payload: credspayload(input.credentiallabel),
    now: input.now,
  });
}
...
export function requireapproval(input: {
  ...
  return {
    ...
    reason: input.reason,
    params: input.params,
    ...(input.secretfields !== undefined && input.secretfields.length > 0 ? { secretfields: input.secretfields } : {}),
    state: "pending",
    raisedat: input.now,
    ...(input.timeout !== undefined ? { timeoutat: input.now + input.timeout } : {}),
  };
}
...
export function resolveapproval(input: {
  ...
  actor: string;
  now: number;
}): { requests: approvalrequest[]; exec?: approvalexec } {
  ...
  const requests = input.requests.map((request) =>
    request.id === input.id ? { ...request, state: decision, decidedat: input.now, actor: input.actor } : request,
  );
  return {
    requests,
    exec: { requestid: input.id, decision, actor: input.actor, at: input.now, latencyms: input.now - gate.raisedat },
  };
}
...
export function expireapprovals(requests: approvalrequest[], now: number): approvalrequest[] {
  return requests.map((request) =>
    request.state === "pending" && request.timeoutat !== undefined && now >= request.timeoutat
...
export function redactparams(params: Record<string, unknown>, secretfields: string[]): Record<string, unknown> {
  ...
  for (const [name, value] of Object.entries(params))
    redacted[name] = secretfields.includes(name) ? "[redacted]" : value;
  return redacted;
}
...
export function approvalprompt(request: approvalrequest, identity?: clientidentity): string {
  const who =
    identity !== undefined
      ? `the client ${identity.displayname} (${identity.fingerprint})`
      : `the client ${request.clientid}`;
