...
export const planfilestepfields: readonly string[] = [
  "id",
  "kind",
  "label",
  "target",
...
export function parseplanfile(value: unknown): planfile {
  if (typeof value !== "object" || value === null || Array.isArray(value))
    throw new Error("The plan file must be a json object.");
  ...
  if (unknownrootfields.length > 0)
    throw new Error(
      `The plan file carries the unknown field${unknownrootfields.length === 1 ? "" : "s"} ${unknownrootfields.join(", ")}; schemastrict refuses unknown plan fields.`,
    );
  ...
  root.steps.forEach((rawstep, index) => {
    ...
    if (unknownfields.length > 0)
      throw new Error(
        `${path} carries the unknown field${unknownfields.length === 1 ? "" : "s"} ${unknownfields.join(", ")}; schemastrict refuses unknown plan fields.`,
      );
    const parsed: planfilestep = {
      id: textfield(step, "id", path),
      kind: textfield(step, "kind", path),
      label: textfield(step, "label", path),
    };
    ...
    if (step.bound !== undefined) {
      if (typeof step.bound !== "number" || !Number.isInteger(step.bound) || step.bound < 1)
        throw new Error(`${path} carries a loop bound that is not a positive integer.`);
      parsed.bound = step.bound;
    }
    if (step.attempts !== undefined) {
      if (typeof step.attempts !== "number" || !Number.isInteger(step.attempts) || step.attempts < 1)
        throw new Error(`${path} carries retry attempts that are not a positive integer.`);
  ...
  if (root.grants !== undefined) {
    if (!Array.isArray(root.grants) || !root.grants.every((grant) => typeof grant === "string" && grant.trim() !== ""))
      throw new Error("The plan file grants must be a list of non-empty action kind names.");
  ...
  if (root.denials !== undefined) {
    if (
      ...
      throw new Error("The plan file denials must be a list of non-empty action kind names.");
...
export function planoriginprofile(file: planfile, now: number): originprofile {
  return originprofileof({
    origin: file.origin,
    grants: (file.grants ?? []) as actionkind[],
    denials: (file.denials ?? []) as actionkind[],
    now,
  });
}
...
export function lintplanfile(input: {
  file: planfile;
  ...
  const profile: originprofile =
    input.file.origin.trim() === ""
      ? {
          profileid: "missing",
          origin: input.file.origin,
          grants: (input.file.grants ?? []) as actionkind[],
          denials: (input.file.denials ?? []) as actionkind[],
          createdat: input.now,
          updatedat: input.now,
        }
      : planoriginprofile(input.file, input.now);
  input.file.steps.forEach((step, index) => {
    ...
    if (rules.has("originprofilegrade")) {
      ...
      if (!grade.allowed)
        diagnostics.push({
          code: "plan.origin.grade",
          path: `${path}.kind`,
          severity: "error",
          message: `The step ${step.id} of kind ${step.kind} fails the originprofile of ${input.file.origin}: ${grade.reason ?? "the originprofile refuses the kind."}`,
        });
    }
    if (rules.has("classconsent")) {
      ...
      const missing = missingclassconsents({
        ...
        bydefault: classes.bydefault,
        consents: input.consents ?? [],
        now: input.now,
      });
      if (missing.needed)
        diagnostics.push({
          code: "plan.consent.class",
          path: `${path}.kind`,
          severity: "error",
          message: `The step ${step.id} needs the sensitive class consent${missing.missing.length === 1 ? "" : "s"} ${missing.missing.join(", ")} for ${input.file.origin}; the extension demands the same fresh consent before the step runs.`,
        });
    }
    if (rules.has("portablecapability")) {
      ...
      if (!capability.allowed)
        diagnostics.push({
          code: "plan.capability.kind",
          path: `${path}.kind`,
          severity: "error",
          message: capability.reason ?? "The step kind exceeds the portable capability set.",
        });
    }
    if (rules.has("gatedeclaration")) {
      const sensitive =
        steprisk(step) === "sensitive" ||
        sensitiveclassesof({
          kind: step.kind as actionkind,
          ...(step.value !== undefined ? { value: step.value } : {}),
          ...(step.options !== undefined ? { options: step.options } : {}),
        }).sensitive;
      if (sensitive && step.gate !== true)
        diagnostics.push({
          code: "plan.gate.declaration",
          path: `${path}.gate`,
          severity: "error",
          message: `The step ${step.id} of kind ${step.kind} is sensitive and declares no gate; a sensitive step without an explicit gate declaration refuses before any run starts.`,
        });
    }
    if (rules.has("staticselector") && step.target !== undefined && /\{\{[^{}]*\}\}/.test(step.target)) {
      diagnostics.push({
        code: "plan.selector.static",
        path: `${path}.target`,
        severity: "info",
        message: `The target of the step ${step.id} carries the template ${step.target}; a templated selector resolves only against the live page of the run.`,
      });
    }
    if (rules.has("loopbound") && loopkinds.includes(step.kind) && step.bound === undefined) {
      diagnostics.push({
        code: "plan.control.loopbound",
        path: `${path}.bound`,
        severity: "warn",
        message: `The loop step ${step.id} of kind ${step.kind} carries no bound; the linter flags every unbounded loop so the author states the iteration ceiling they want.`,
      });
    }
    if (rules.has("retrybound") && retrykinds.includes(step.kind) && step.attempts === undefined) {
      diagnostics.push({
        code: "plan.control.retrybound",
        path: `${path}.attempts`,
        severity: "warn",
        message: `The retrying step ${step.id} of kind ${step.kind} carries no attempts; the linter flags every missing retry bound so the author states the attempts they reviewed.`,
      });
    }
  });
  if (input.file.goal.trim() === "")
    diagnostics.push({
      code: "plan.goal.empty",
      path: "goal",
      severity: "error",
      message: "The plan file carries no goal; a goalless plan reviews nothing.",
    });
  if (input.file.origin.trim() === "")
    diagnostics.push({
      code: "plan.origin.empty",
      path: "origin",
      severity: "error",
      message: "The plan file carries no origin; an originless plan matches no profile.",
    });
  return diagnostics;
}
...
export function planlintsummary(diagnostics: planlintdiagnostic[]): {
  info: number;
  warn: number;
  error: number;
  exitcode: number;
  ...
  return {
    ...
    warn,
    error,
    exitcode: planlintexitcode(diagnostics),
    reason: `The plan lint raised ${info} info, ${warn} warn and ${error} error diagnostic${info + warn + error === 1 ? "" : "s"}; ${error > 0 ? "the error diagnostics refuse the plan file before any run starts" : "no error diagnostic refuses the plan file"}.`,
  };
}
...
export function plansensitiveclasses(file: planfile): sensitiveclass[] {
  ...
  for (const step of file.steps)
    for (const sensitiveclass of sensitiveclassesof({
      kind: step.kind as actionkind,
      ...(step.value !== undefined ? { value: step.value } : {}),
      ...(step.options !== undefined ? { options: step.options } : {}),
    }).classes)
      classes.add(sensitiveclass);
  return [...classes];
}
...
export function plancardsof(input: { plan: agentplan; corrections: correctionentry[] }): plancard[] {
  return input.plan.steps.map((step: toolstep) => ({
    ...
    corrections: matchingcorrections(input.corrections, { origin: input.plan.origin, kind: step.kind }).map(
      (entry) => ({ id: entry.id, source: entry.source, reason: entry.reason }),
    ),
...
export function plancardgroups(cards: plancard[]): plancardgroup[] {
  ...
  return order
    .map((risk) => ({ risk, cards: cards.filter((card) => card.risk === risk), expanded: risk === "sensitive" }))
...
export function stepresolutionof(input: {
  stepid: string;
  planid: string;
  origin: string;
  ...
  if (input.resolution === "edit" && (input.edited ?? "").trim() === "")
    throw new Error("The edited resolution needs its corrected step shape.");
  return {
    stepid: input.stepid,
    planid: input.planid,
    origin: input.origin,
    ...
    ...(input.edited !== undefined && input.edited.trim() !== "" ? { edited: input.edited } : {}),
    at: input.at,
  };
}

/** Builds the immutable log event of one stepapprove resolution: the review kind entry the sealed chain keeps. */
export function resolutionlogeventof(resolution: stepapproveresolution): {
  kind: "review";
  summary: string;
  stepid: string;
} {
...
export function resolutionhistoryafter(
  history: stepapproveresolution[],
  resolution: stepapproveresolution,
): stepapproveresolution[] {
...
export function maskverdictsof(state: Record<string, string>, sensitivefields: string[]): Record<string, string> {
  ...
  for (const [field, value] of Object.entries(state)) {
    if (sensitivefields.includes(field))
      verdicts[field] = `The ${field} value stays masked (${value.length} characters) and never renders in the clear.`;
  }
  return verdicts;
}
...
export function diffpreviewof(input: {
  stepid: string;
  ...
  for (const field of fields) {
    ...
    if (hasbefore && !hasafter && beforevalue !== undefined)
      changes.push({ field, kind: "removed", before: beforevalue });
    else if (!hasbefore && hasafter && aftervalue !== undefined)
      changes.push({ field, kind: "added", after: aftervalue });
    else if (
      hasbefore &&
      hasafter &&
      ...
      changes.push({ field, kind: "changed", before: beforevalue, after: aftervalue });
  }
  return {
    stepid: input.stepid,
    before: input.before,
    after: input.after,
    changes,
    maskverdicts: input.maskverdicts ?? {},
...
export function stepstimelinenodes(input: {
  plan: agentplan;
  ...
  return input.plan.steps.map((step: toolstep) => {
    ...
    return {
      ...
      ...(environments?.[step.id] !== undefined
        ? { environment: environments[step.id] }
...
export async function logstreameventof(input: {
  level: loglevel;
  source: string;
  origin: string;
  summary: string;
  ...
  const id = randomid();
  const hash: loghash = await entryhashof({
...
export function filterlogstream(events: logstreamevent[], filter: logstreamfilter): logstreamevent[] {
  return events.filter(
    (event) =>
      ...
      (filter.origin === undefined || filter.origin === "" || event.origin === filter.origin) &&
      (filter.stepid === undefined || filter.stepid === "" || event.stepid === filter.stepid),
  );
}
...
export async function verifylogstream(
  ...
  for (let index = 0; index < events.length; index += 1) {
    ...
    if (event.hash.previous !== expectedprevious)
      return {
        valid: false,
        brokenat: index,
        reason: `The logstream chain breaks at the event ${event.id}: its previous hash does not link to its predecessor.`,
      };
    const recomputed = await entryhashof({
      previous: event.hash.previous,
      entry: {
        id: event.id,
        runid: "surfaces",
        kind: "step",
    ...
    if (recomputed.current !== event.hash.current)
      return {
        valid: false,
        brokenat: index,
        reason: `The logstream chain breaks at the event ${event.id}: its own hash does not reproduce.`,
      };
  }
  return {
    valid: true,
    reason: `The logstream chain of ${events.length} event${events.length === 1 ? "" : "s"} verifies link by link.`,
  };
}
...
export async function auditexcerptof(
  events: logstreamevent[],
  input: { from: number; to: number },
): Promise<{ ok: boolean; text: string; reason: string }> {
  if (input.from < 0 || input.to <= input.from || input.to > events.length)
    return {
      ok: false,
      text: "",
      reason: `The excerpt range ${input.from} to ${input.to} names no contiguous slice of the ${events.length} event${events.length === 1 ? "" : "s"}.`,
    };
  const range = events.slice(input.from, input.to);
  const verification = await verifylogstream(range);
  if (!verification.valid)
    return { ok: false, text: "", reason: `The excerpt refuses the copy: ${verification.reason}` };
  const text = range
    .map(
      (event) =>
        `[${event.at}] ${event.level} ${event.source}${event.stepid !== undefined ? ` step ${event.stepid}` : ""} ${event.origin} — ${event.summary}${event.masked ? ` (${event.maskverdict})` : ""}`,
    )
    .join("\n");
  return {
    ok: true,
    text,
    reason: `The excerpt copied the verified range ${input.from} to ${input.to} of the logstream.`,
  };
}
