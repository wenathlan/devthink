...
export interface multiagentview {
  overview: { topology: string; leader?: string; workers: number; critics: number; verifiers: number; killswitch: string; agents: number; active: number; paused: number; stopped: number; tasks: number; queued: number; claimed: number; done: number; unread: number };
  ...
  costview: { rows: costperagentrow[]; total: { tokens: number; cost: number; steps: number; currency?: string }; source: string };
...
export function dashboardviewerof(input: { viewer?: { role?: string; readonly?: boolean } }): { readonly: boolean; controls: boolean; reason: string } {
  const viewer = input.viewer;
  if (viewer === undefined) return { readonly: false, controls: true, reason: "The dashboard viewer is the operator session; the run role holds and every control stays available." };
  const readonly = viewer.readonly === true || viewer.role === "observer";
  return { readonly, controls: !readonly, reason: readonly
...
export function multiagentoverviewof(input: { agents: agentidentity[]; queue: taskqueue; topology?: leaderworker; killswitch: killswitch; unread: number }): multiagentview["overview"] {
...
export function agentstatuscardof(input: { agent: agentidentity; usage?: agentusage; claim?: { taskid: string; payload: string }; done: number; unread: number; reviewstatus?: string; handoffarrows?: string[] }): agentstatuscard {
...
export function queuelaneviewof(input: { queue: taskqueue; lane?: string }): multiagentview["queueview"] {
  ...
  const rows = input.queue.items
    ...
    .map(item => {
      ...
      return { taskid: item.id, lane: item.lane, priority: item.priority, state: item.state, payload: item.payload, ...(claim !== undefined ? { holder: claim.agentid, heartbeatat: claim.heartbeatat } : {}) };
    });
...
export function messageflowof(input: { mailboxes: agentmailbox[] }): multiagentview["messageflow"] {
  ...
  for (const mailbox of input.mailboxes) {
    for (const message of mailbox.inbox) rows.push({ messageid: message.id, senderid: message.senderid, recipient: mailbox.agentid, routing: `${message.routing} inbox`, payload: message.payload, sentat: message.sentat, read: message.readat !== undefined });
    for (const message of mailbox.outbox) rows.push({ messageid: message.id, senderid: mailbox.agentid, recipient: message.recipient, routing: `${message.routing} outbox`, payload: message.payload, sentat: message.sentat, read: message.readat !== undefined });
  }
...
export function conflictlogof(input: { conflicts: conflictscan[]; locks: resourcelock[] }): multiagentview["conflictlog"] {
  ...
  for (const lock of input.locks) rows.push({ source: "lock", key: lock.key, detail: `${lock.kind} lock held by ${lock.holder} since ${lock.acquiredat}${lock.expiresat !== undefined ? ` expiring at ${lock.expiresat}` : " with no expiry"}.`, writers: [lock.holder], at: lock.acquiredat });
...
export function costperagentof(input: { usage: agentusage[]; totals?: swarmcost }): multiagentview["costview"] {
  ...
  const rows = [...input.usage]
    ...
    .map(usage => ({ agentid: usage.agentid, tokens: usage.tokens, cost: usage.cost, steps: usage.steps, share: tokens > 0 ? Math.round((usage.tokens / tokens) * 100) / 100 : 0 }));
  return { rows, total: { tokens, cost, steps, ...(input.totals?.currency !== undefined ? { currency: input.totals.currency } : {}) }, source: "the per agent usage the costcert reconciliation of tests/artifacts/costcert.json recomputes" };
}
...
export function escalationinboxof(input: { escalations: escalationrecord[] }): multiagentview["escalationinbox"] {
  const rows = [...input.escalations]
    ...
    .map(escalation => ({ escalationid: escalation.id, agentid: escalation.agentid, subject: escalation.subject, state: escalation.state, ...(escalation.decision !== undefined ? { decision: escalation.decision } : {}), raisedat: escalation.raisedat }));
...
export function agentcontrolsof(input: { agent: agentidentity; viewer: { readonly: boolean }; killswitch: killswitch }): multiagentview["controls"] {
...
export function timelinescrubof(input: { actions: swarmaction[]; position?: number }): multiagentview["timeline"] {
  ...
  const marks = actions.map(action => ({ actionid: action.id, kind: action.kind, ...(action.agentid !== undefined ? { agentid: action.agentid } : {}), summary: action.summary, at: action.at, position: span > 0 ? Math.round(((action.at - first) / span) * 100) / 100 : 0 }));
  const position = marks.length === 0 ? 0 : Math.min(Math.max(input.position !== undefined && Number.isFinite(input.position) ? input.position : marks.length - 1, 0), marks.length - 1);
...
export function aggregatereportdownloadof(input: { report?: resultreport; now: number }): multiagentview["reportdownload"] {
  if (input.report === undefined) return { filename: "", payload: "", bytes: 0, reason: "No aggregate report exists yet; the swarm builds one when the parallel results merge after the review." };
  const payload = JSON.stringify({ id: input.report.id, title: input.report.title, sections: input.report.sections, sources: input.report.sources, ...(input.report.confidence !== undefined ? { confidence: input.report.confidence } : {}), createdat: input.report.createdat, exportedat: input.now }, null, 2);
...
export function topologyinventoryof(input: { agents: agentidentity[]; topology?: leaderworker; spawns?: spawnrecord[] }): multiagentview["inventory"] {
  const topology = input.topology;
  ...
  const entries = [...input.agents]
    ...
    .map(agent => {
      ...
      return { agentid: agent.id, name: agent.name, role: agent.role, state: agent.state, depth: agent.depth, ...(parentid !== undefined ? { parentid } : {}), lane, scenarios: scenariofamilies[lane] ?? scenariofamilies[String(agent.role)] ?? ["escalation and review"] };
    });
  const scenarios = 6;
  return { entries, scenarios, certified: input.agents.length > 0 && topology !== undefined, reason: entries.length === 0
...
export function multiagentviewof(input: { agents: agentidentity[]; queue: taskqueue; mailboxes: agentmailbox[]; topology?: leaderworker; killswitch: killswitch; usage: agentusage[]; costs: swarmcost[]; conflicts: conflictscan[]; locks: resourcelock[]; escalations: escalationrecord[]; timeline: swarmaction[]; report?: resultreport; spawns?: spawnrecord[]; viewer?: { role?: string; readonly?: boolean }; lane?: string; now: number }): multiagentview {
  ...
  const cards = input.agents.map(agent => {
    ...
    const claim = claimrecord !== undefined ? input.queue.items.find(item => item.id === claimrecord.taskid) : undefined;
  ...
  return {
    overview: multiagentoverviewof({ agents: input.agents, queue: input.queue, ...(input.topology !== undefined ? { topology: input.topology } : {}), killswitch: input.killswitch, unread }),
    cards,
    ...
    reportdownload: aggregatereportdownloadof({ ...(input.report !== undefined ? { report: input.report } : {}), now: input.now }),
    empty: dashboardemptyguidanceof({ agents: input.agents }),
    viewer,
    inventory: topologyinventoryof({ agents: input.agents, ...(input.topology !== undefined ? { topology: input.topology } : {}), ...(input.spawns !== undefined ? { spawns: input.spawns } : {}) }),
  };
}
