...
export function exportdescriptorof(format: string, scope: string): exportdescriptor {
  if (!exportformats.includes(format))
    throw new Error(`The export format ${format} stays outside csv, json, log, jsonl and markdown.`);
  if (!exportscopes.includes(scope))
    throw new Error(`The export scope ${scope} stays outside runs, extractions, notes, session, audit and extraction.`);
...
export function unmaskedfieldsof(records: Array<Record<string, unknown>>): string[] {
  ...
  for (const record of records) {
    for (const [name, value] of Object.entries(record)) {
      ...
      if (secretfieldshapes.some((shape) => name.toLowerCase().includes(shape)) && !value.includes(maskmarker))
        unmasked.add(name);
    }
  }
...
export function exportrecords(input: {
...
export async function exportchain(input: {
  ...
  shapes: string[];
  path?: string;
}): Promise<{ result: exportresult; content: string }> {
  const verification = await verifylogchain(input.log.entries);
  const chaingate = exportchaingate({
  ...
  if (!chaingate.allowed)
    return {
      result: {
        ...
        rows: 0,
        ...(input.path !== undefined ? { path: input.path } : {}),
        reason: chaingate.reason ?? "The export refused an unverified chain.",
      },
      content: "",
    };
  return exportrecords({
    descriptor: input.descriptor,
    records: chainrows(input.log.entries),
    shapes: input.shapes,
    ...(input.path !== undefined ? { path: input.path } : {}),
  });
}
...
export function noterow(note: sitenote): Record<string, string> {
  return {
    ...
    title: note.title,
    author: note.author,
    sensitive: String(note.sensitive),
    ...(note.body !== undefined ? { body: note.body } : {}),
    ...(note.sealedbody !== undefined ? { sealedbody: note.sealedbody } : {}),
...
export function exportnotes(input: {
  ...
  return exportrecords({
    descriptor: input.descriptor,
    records: input.notes.map(noterow),
    shapes: input.shapes,
    ...(input.path !== undefined ? { path: input.path } : {}),
  });
}
...
export function exportextractions(input: {
  descriptor: exportdescriptor;
...
export const minimizationkinds: string[] = [
  "localfirst",
  "notelemetry",
  "optinsync",
  "encryptsync",
 ...
 */
export function localfirst(input: {
  origin: string;
  rows: Array<Record<string, string>>;
  reviewedfields?: string[];
  rules: localrule[];
  now: number;
}): { rows: Array<Record<string, string>>; stripped: string[]; localheld: string[]; reason: string } {
  if (input.origin.trim() === "")
    throw new Error("The localfirst pass names its origin; an unnamespaced extraction answers no review.");
  const reviewed = new Set(
    (input.reviewedfields ?? []).map((field) => field.trim().toLowerCase()).filter((field) => field !== ""),
  );
  const localfields = new Set(
    input.rules
      .filter((rule) => rule.origin === input.origin || rule.origin === "*")
      .flatMap((rule) => rule.fields.map((field) => field.trim().toLowerCase()).filter((field) => field !== "")),
  );
  ...
  const rows = input.rows.map((row) => {
    ...
    for (const [field, value] of Object.entries(row)) {
      ...
      if (
        identityfields.some(
          (identity) => key === identity || key.endsWith(`_${identity}`) || key.endsWith(`-${identity}`),
        ) &&
        !reviewed.has(key)
      ) {
        stripped.add(field);
  ...
  return {
    ...
    stripped: [...stripped],
    localheld: [...localheld],
    reason: `The localfirst pass kept ${rows.length} row${rows.length === 1 ? "" : "s"} on the device${stripped.size > 0 ? `, stripped ${stripped.size} identity field${stripped.size === 1 ? "" : "s"} the review never listed` : ""}${localheld.size > 0 ? ` and held ${localheld.size} local rule field${localheld.size === 1 ? "" : "s"} that never leave the device` : ""}.`,
  };
}
 ...
 */
export function optinsync(input: { current?: syncsettings; offered: string[]; enable: string[]; now: number }): {
  settings: syncsettings;
  consent: Array<{ dataclass: string; at: number }>;
} {
  const offered = input.offered.map((item) => item.trim().toLowerCase()).filter((item) => item !== "");
  if (offered.length === 0)
    throw new Error("The sync opt in lists every data class before any enablement; an empty listing enables nothing.");
  ...
  if (unknown.length > 0)
    throw new Error(
      `The sync opt in never enables a class the listing did not offer: ${unknown.join(", ")} stay${unknown.length === 1 ? "s" : ""} off until the user lists them.`,
    );
  ...
  return {
    settings: {
      enabled: [...enabled],
      ...(input.current?.cadence !== undefined ? { cadence: input.current.cadence } : {}),
      consent: [...consented.entries()].map(([dataclass, at]) => ({ dataclass, at })),
      at: input.now,
    },
    consent: enable.map((dataclass) => ({ dataclass, at: input.now })),
  };
}
 ...
 */
export async function encryptsync(input: {
  payload: string;
  passphrase?: string;
  derive: keyderive;
  encrypt: payloadencrypt;
}): Promise<{ cipher: string; formattag: string; reason: string }> {
  const passphrase = input.passphrase ?? "";
  if (passphrase.trim() === "")
    throw new Error(
      "The encryptsync pass needs the user passphrase; a sync without the passphrase never transports because the key never defaults in code.",
    );
  if (input.payload.length === 0)
    throw new Error("The encryptsync pass encrypts a non-empty payload; an empty sync carries nothing to sync.");
  ...
  return {
    cipher,
    formattag,
    reason: `The encryptsync pass derived its key from the user passphrase, encrypted the ${input.payload.length} character payload and stamped the ${formattag} format version; the passphrase never rides the envelope.`,
  };
}
 ...
 */
export async function syncpass(input: {
  payload: string;
  classes: string[];
  settings: syncsettings;
  passphrase?: string;
  ...
  send: syncsend;
  now: number;
}): Promise<{ record: syncrecord; delivered: boolean; reason: string }> {
  ...
  if (unconsented.length > 0)
    throw new Error(
      `The sync pass carries the classes ${unconsented.join(", ")} the user never opted in; an unconsented class never reaches the transport.`,
    );
  ...
  const record: syncrecord = {
    ...
    payloadhash: await shaof(encrypted.cipher),
    formattag: encrypted.formattag,
    encrypted: true,
    syncedat: input.now,
  };
  return {
    ...
    reason: `The sync pass encrypted the ${input.classes.join(", ")} payload, delivered it${delivery.endpoint !== undefined ? ` through ${delivery.endpoint}` : ""} and recorded the payload hash; every class rode with its own consent stamp.`,
  };
}
 ...
 */
export function purgeonrequest(input: {
  policy: purgepolicy;
  typed: string;
  inventory: datainventory[];
  now: number;
}): { deleted: string[]; kept: string[]; audithashespreserved: boolean; reason: string } {
  const scope = input.policy.scope.map((item) => item.trim().toLowerCase()).filter((item) => item !== "");
  if (scope.length === 0)
    throw new Error("The purge request names its scope; a purge without a scope deletes nothing.");
  const full =
    scope.includes("all") ||
    (scope.includes("runs") &&
  ...
  if (full && input.typed.trim() !== input.policy.confirmation)
    throw new Error("The full scope purge needs the typed confirmation phrase; a purge nobody typed out never runs.");
  const deleted: string[] = [];
  const kept: string[] = [];
  for (const entry of input.inventory) {
    if (entry.dataclass === "audit") {
      kept.push(entry.key);
      continue;
    }
  ...
  return {
    ...
    reason: `The purge deleted ${deleted.length} stored key${deleted.length === 1 ? "" : "s"} of the ${scope.join(", ")} scope${full ? " after the typed confirmation" : ""} — ${deleted.join(", ") || "none"} — while the audit hashes survived${kept.length > 0 ? ` and ${kept.length} key${kept.length === 1 ? "" : "s"} stayed` : ""}; every deleted key entered the audit trail.`,
  };
}
 ...
 */
export function exportall(input: {
  runs: string[];
  memory: string[];
  captures: string[];
  settings: boolean;
  ...
  if (
    ...
    throw new Error("The exportall bundle carries the stored records; a device holding nothing exports nothing.");
  const records =
    input.runs.length +
    input.memory.length +
...
export function streambundle(input: { bundle: exportallbundle; chunksize: number }): string[] {
  if (!Number.isInteger(input.chunksize) || input.chunksize <= 0)
    throw new Error(
      "The bundle streaming names its chunk size as a positive whole number of characters; the chunk size never defaults in code.",
    );
  const payload = JSON.stringify(input.bundle);
  const chunks: string[] = [];
  for (let offset = 0; offset < payload.length; offset += input.chunksize)
    chunks.push(payload.slice(offset, offset + input.chunksize));
  return chunks;
}
 ...
 */
export function jarscope(input: {
  jar: cookiejarrecord;
  ...
  jar: cookiejarrecord;
  entries: Array<{ name: string; domain: string; value: string; path?: string; expiresat?: number }>;
  reason: string;
} {
  if (input.operation === "write") {
    if (input.jar.sealed)
      throw new Error(
        `The jar ${input.jar.jarid} of the run ${input.jar.runid} sealed at the run completion; a sealed jar refuses every write.`,
      );
    const cookies = input.cookies ?? [];
    if (cookies.length === 0)
      throw new Error("The jar write carries its cookie entries; an empty write changes nothing.");
    ...
    return {
      jar: { ...input.jar, cookies: merged, updatedat: input.now },
      entries: cookies,
      reason: `The jar write scoped ${cookies.length} cookie entr${cookies.length === 1 ? "y" : "ies"} inside the jar ${input.jar.jarid} of the run ${input.jar.runid}; no other run reads them.`,
    };
  }
  ...
  return {
    jar: input.jar,
    entries,
    reason: `The jar read answered ${entries.length} cookie entr${entries.length === 1 ? "y" : "ies"} of the jar ${input.jar.jarid}${input.domain !== undefined ? ` for the domain ${input.domain}` : ""}; the read stays inside the active jar.`,
  };
}
...
export function sealjar(input: { jar: cookiejarrecord; now: number; expiry?: number }): cookiejarrecord {
  ...
  if (input.expiry !== undefined && !(input.expiry > 0))
    throw new Error(
  ...
  return {
    ...
    sealed: true,
    sealedat: input.now,
    ...(input.expiry !== undefined ? { expiresat: input.now + input.expiry } : {}),
    updatedat: input.now,
  };
}
 ...
 */
export function cleanupafterrun(input: {
  runid: string;
  artifacts: Array<{ id: string; dataclass: string; retained?: boolean }>;
  classes: string[];
  now: number;
}): { cleared: string[]; kept: string[]; reason: string } {
  if (input.runid.trim() === "")
    throw new Error("The cleanup pass names its run; an unnamespaced cleanup clears nothing.");
  const classes = input.classes.map((item) => item.trim().toLowerCase()).filter((item) => item !== "");
  if (classes.includes("audit"))
    throw new Error(
      "The cleanup pass never deletes the audit history without the user consent; the immutable hashes survive every pass.",
    );
  const cleared: string[] = [];
  const kept: string[] = [];
  for (const artifact of input.artifacts) {
    if (artifact.retained === true) {
  ...
  return {
    cleared,
    kept,
    reason: `The cleanup pass cleared ${cleared.length} artifact${cleared.length === 1 ? "" : "s"} of the run ${input.runid} under the classes ${classes.join(", ") || "none"} while ${kept.length} stayed${kept.length > 0 ? " — the retained artifacts the user flagged never leave" : ""}.`,
  };
}
 ...
 */
export function enforcequarantine(input: {
  path: string;
  reason: string;
  scan?: scanverdict;
  digest?: string;
  now: number;
  id?: string;
}): { entry: quarantineentry; action: "hold" | "release" | "delete"; reason: string } {
  if (input.path.trim() === "")
    throw new Error("The quarantine names its file path; a download without a path quarantines nowhere.");
  ...
  const entry: quarantineentry = {
    ...
    reason: input.reason,
    scan,
    status: scan === "clean" ? "released" : scan === "flagged" ? "deleted" : "held",
    ...(input.digest !== undefined && /^[0-9a-f]{64}$/.test(input.digest.trim().toLowerCase())
  ...
  if (scan === "clean")
    return {
      entry,
      action: "release",
      reason: `The scanner verdict read clean so the quarantined file ${input.path} releases from the sandbox folder to the reviewed download flow.`,
    };
  if (scan === "flagged")
    return {
      entry,
      action: "delete",
      reason: `The scanner verdict read flagged so the quarantined file ${input.path} deletes from the sandbox folder; a flagged download never reaches the downloads folder.`,
    };
  return {
    entry,
    action: "hold",
    reason: `The scanner verdict reads ${scan} so the quarantined file ${input.path} stays held in the sandbox folder; a file without a clean verdict never opens.`,
  };
}
...
export function inventoryentry(input: { key: string; dataclass: string; value: unknown; now: number }): datainventory {
  if (input.key.trim() === "")
    throw new Error("The inventory entry names its storage key; an unnamespaced key answers no family.");
  const serialized = JSON.stringify(input.value ?? null) ?? "null";
  const records = Array.isArray(input.value)
    ? input.value.length
...
export function importexportpayloadof(input: {
  profile: string;
  ...
  const secrets =
    [...input.originprofiles, ...input.siteprofiles, ...input.notes, ...Object.values(input.preferences)].find(
      (record) => secretcarrying(record),
    ) !== undefined;
  ...
  return {
    ...
    exportedat: input.at,
    contents: {
      originprofiles: input.originprofiles,
      siteprofiles: input.siteprofiles,
      notes: input.notes,
      preferences: input.preferences,
    },
    exclusions: ["secretvault values", "unmasked logs"],
  };
}
...
export function importexportvalidate(payload: importexportpayload): { ok: boolean; reason: string } {
  const records = [
    ...payload.contents.originprofiles,
    ...payload.contents.siteprofiles,
    ...payload.contents.notes,
    ...Object.values(payload.contents.preferences),
  ];
  const preferencessecrets = Object.entries(payload.contents.preferences).some(([key, value]) =>
    secretcarrying({ [key]: value }),
  );
  const gate = importexportgate({
    containssecrets: records.some((record) => secretcarrying(record)) || preferencessecrets,
    unmaskedlogs: payload.contents.unmaskedlogs !== undefined,
  });
  ...
  return {
    ok: true,
    reason: `The importexport bundle of the profile ${payload.profile} validates with ${payload.contents.originprofiles.length} origin profile${payload.contents.originprofiles.length === 1 ? "" : "s"}, ${payload.contents.siteprofiles.length} site profile${payload.contents.siteprofiles.length === 1 ? "" : "s"} and ${payload.contents.notes.length} note${payload.contents.notes.length === 1 ? "" : "s"}; ${payload.exclusions.join(" and ")} never enter any bundle.`,
  };
}
...
): { preferences: Record<string, unknown>; applied: string[] } {
...
export function dropimportof(input: { filename: string; bytes: number; head: string; at: number }): dropimportsession {
  ...
  if (kind === undefined)
    throw new Error(
      `The dropimport detects no csv, json or workflow kind in ${input.filename}; the import path refuses the file.`,
    );
  return {
    id: `${input.filename}:${input.at}`,
