...
import type { environmentkind, environmentprovenance, runlock, runstaterecord, sealedrunstate, urlhistoryentry } from "./types.js";
...
export function openrun(input: { runid: string; sessionid: string; planid: string; profileid: string; interval: number; now: number }): runstaterecord {
  ...
  return {
    ...
    environments: {},
    turnarounds: {},
    keepalive: { runid: input.runid, sessionid: input.sessionid, state: "active", startedat: input.now, interval: input.interval, beats: 0, lastbeatat: input.now, portopen: true, events: [{ kind: "start" as const, at: input.now, detail: `The keepalive port opens for the run ${input.runid} and beats every ${input.interval} milliseconds.` }] },
    updatedat: input.now,
  };
}
...
export function beatrun(state: runstaterecord, now: number): runstaterecord {
  ...
  return {
    ...state,
    keepalive: { ...state.keepalive, beats: state.keepalive.beats + 1, lastbeatat: now, events: [...state.keepalive.events, { kind: "heartbeat" as const, at: now }].slice(-200) },
    updatedat: now,
  };
}
...
export function closerun(state: runstaterecord, now: number): runstaterecord {
  ...
  return {
    ...state,
    state: "completed",
    keepalive: { ...state.keepalive, state: "stopped", stoppedat: now, portopen: false, events: [...state.keepalive.events, { kind: "stop" as const, at: now, detail: "The run reached a terminal state and the keepalive port closed." }].slice(-200) },
    updatedat: now,
  };
}
...
export function reattachrun(state: runstaterecord, now: number): runstaterecord {
  ...
  return {
    ...state,
    state: "recovered",
    keepalive: { ...state.keepalive, portopen: true, events: [...state.keepalive.events, { kind: "reattach" as const, at: now, detail: "The service worker restarted and the keepalive port reattached from the persisted run state." }].slice(-200) },
    updatedat: now,
  };
}
...
export function recordurl(state: runstaterecord, entry: { url: string; stepid: string; now: number }): runstaterecord {
  ...
  return { ...state, urlhistory: [...state.urlhistory.filter(item => !(item.url === record.url && item.stepid === record.stepid)), record], updatedat: entry.now };
}
...
export function recordenvironment(state: runstaterecord, input: { stepid: string; environment: environmentkind; origin: string; now: number }): runstaterecord {
  if (input.stepid.trim() === "") throw new Error("The environment record needs its step id.");
  const provenance: environmentprovenance = { origin: input.origin, stepid: input.stepid, environment: input.environment };
...
export function recordturnaround(state: runstaterecord, input: { stepid: string; milliseconds: number; now: number }): runstaterecord {
...
export function markpending(state: runstaterecord, stepid: string | undefined, now: number): runstaterecord {
  return { ...state, ...(stepid !== undefined && stepid.trim() !== "" ? { pendingstepid: stepid } : {}), updatedat: now };
}
...
export function recoveryplan(state: runstaterecord): { runid: string; pendingstepid?: string; recoverable: boolean; reason: string } {
  ...
  if (state.pendingstepid === undefined || state.pendingstepid.trim() === "") return { runid: state.runid, recoverable: false, reason: `The run ${state.runid} carries no pending step; a fresh reviewed run starts over instead of guessing.` };
...
export function zombiesweep(input: { states: runstaterecord[]; now: number; interval: number; missedlimit: number }): { states: runstaterecord[]; reaped: string[] } {
  ...
  return { states: input.states.map(state => ids.has(state.runid) ? { ...state, state: "reaped" as const, keepalive: { ...state.keepalive, state: "stopped" as const, portopen: false, ...(state.keepalive.stoppedat === undefined ? { stoppedat: input.now } : {}), events: [...state.keepalive.events, { kind: "stop" as const, at: input.now, detail: "The zombie reaper closed the silent run." }] } } : state), reaped: [...ids] };
}
...
export function acquirerunlock(input: { locks: runlock[]; sessionid: string; runid: string; holder: string; expiresat?: number; now: number }): { locks: runlock[]; acquired: boolean; reason: string } {
  if (input.sessionid.trim() === "" || input.runid.trim() === "") throw new Error("The run lock needs its session and run ids.");
  const live = input.locks.filter(lock => lock.sessionid === input.sessionid && (lock.expiresat === undefined || lock.expiresat > input.now));
  ...
  const lock: runlock = { sessionid: input.sessionid, runid: input.runid, holder: input.holder, acquiredat: input.now, ...(input.expiresat !== undefined ? { expiresat: input.expiresat } : {}) };
...
export function releaserunlock(input: { locks: runlock[]; sessionid: string; runid: string; now: number }): { locks: runlock[]; released: boolean; reason: string } {
...
export function serializesteps(input: { branches: Array<{ branchid: string; steps: Array<{ stepid: string; tabid: number }> }> }): Array<{ branchid: string; stepid: string; tabid: number; order: number }> {
...
export function prunerunstates(input: { states: runstaterecord[]; used: number; ceiling: number }): { states: runstaterecord[]; pruned: string[]; reason: string } {
...
export function exportrunstate(states: runstaterecord[], now: number): { runs: number; urls: number; environments: number; offloaded: number; beats: number; exportedat: number } {
  return {
    runs: states.length,
...
import type { agentplan, auditevent, checkpointrecord, heartbeatrecord, planprogress, rollbackitem, runrecord, runsettings, runstate, runtransition, queuedtask, toolstep } from "./types.js";
...
export function advancerun(run: runrecord, to: runstate, now: number): runrecord {
  if (!transitionislegal({ from: run.state, to })) throw new Error(`The run ${run.runid} cannot move from ${run.state} to ${to}; the run lifecycle allows only its legal transitions.`);
...
export function initialrun(input: { planid: string; sessionid: string; now: number; runid?: string }): runrecord {
  ...
  return { runid: input.runid ?? crypto.randomUUID(), planid: input.planid, sessionid: input.sessionid, state: "queued", createdat: input.now, updatedat: input.now };
}
...
export function stepkey(input: { planid: string; stepid: string; kind: string }): string {
  if (input.planid.trim() === "" || input.stepid.trim() === "") throw new Error("The step key needs its plan and step ids.");
...
export function enqueueoffline(queue: queuedtask[], input: { plan: agentplan; sessionid: string; now: number }): queuedtask[] {
  if (input.plan.state !== "approved") throw new Error("Only an approved plan enters the offlinequeue; an unreviewed plan never queues.");
  const sequence = queue.reduce((highest, task) => Math.max(highest, task.sequence), 0) + 1;
  return [...queue, { sequence, planid: input.plan.id, sessionid: input.sessionid, plan: input.plan, enqueuedat: input.now, expiresat: input.plan.expiresat }];
}
...
export function makecheckpoint(input: { runid: string; stepid: string; completed: string[]; digest: string; now: number }): checkpointrecord {
  ...
  if (input.digest.trim() === "") throw new Error("The checkpoint needs its page digest; a digestless resume never revalidates the page.");
  return { runid: input.runid, stepid: input.stepid, completed: [...input.completed], digest: input.digest, createdat: input.now };
}
...
export function checkpointisvalid(input: { checkpoint: checkpointrecord; runid: string; digest: string }): boolean {
  return input.checkpoint.runid === input.runid && input.checkpoint.digest === input.digest && input.checkpoint.digest.trim() !== "";
}
...
export function heartbeatisstale(input: { heartbeat: heartbeatrecord | undefined; now: number; window: number }): boolean {
...
export function zombiecheck(input: { runs: runrecord[]; heartbeats: Record<string, heartbeatrecord>; now: number; window: number }): runrecord[] {
  return input.runs.filter(run => run.state === "running" && heartbeatisstale({ heartbeat: input.heartbeats[run.runid], now: input.now, window: input.window }));
}
...
export function reaprun(run: runrecord, now: number): { run: runrecord; event: auditevent } {
  if (run.state !== "running") throw new Error(`The run ${run.runid} is ${run.state}; only a running run with a silent heartbeat reaps.`);
  const reaped: runrecord = { ...run, state: "failed", updatedat: now };
  return { run: reaped, event: { id: crypto.randomUUID(), kind: "reap", at: now, summary: `The zombie reaper failed the run ${run.runid} whose heartbeat stayed silent past the user window; the user starts a fresh reviewed run instead of guessing.`, sessionid: run.sessionid, planid: run.planid } };
}
...
const compensationtable: Record<string, string> = {
  ...
  reloadcache: "the navigation history stays as the run left it; the compensation reports the route for the user review",
...
export function rollbackrun(input: { runid: string; steps: toolstep[]; executedstepids: string[]; origin: string; now: number }): rollbackitem[] {
  return sideeffects(input.steps, input.executedstepids).map(step => ({
    runid: input.runid,
    stepid: step.id,
    kind: step.kind,
    origin: input.origin,
    compensation: compensationtable[step.kind] ?? `the ${step.kind} step stays executed; the compensation reports its outcome for the user review`,
    at: input.now,
  }));
}
...
export function cancelrollback(input: { run: runrecord; steps: toolstep[]; executedstepids: string[]; origin: string; choice: "rollback" | "none"; now: number }): { run: runrecord; compensations: rollbackitem[]; reason: string } {
  const cancelled = cancelrunrecord(input.run, input.now);
  if (input.choice === "none") return { run: cancelled, compensations: [], reason: `The run ${input.run.runid} cancelled without a rollback; the executed steps stay applied and the run state freezes for the audit trail.` };
  const compensations = rollbackrun({ runid: input.run.runid, steps: input.steps, executedstepids: input.executedstepids, origin: input.origin, now: input.now });
  const rolledback = advancerun(cancelled, "rolledback", input.now);
  return { run: rolledback, compensations, reason: `The run ${input.run.runid} cancelled with its rollback: ${compensations.length} compensating step${compensations.length === 1 ? "" : "s"} run only after the explicit user choice.` };
}

/** Transitions one run to failed and freezes its progress snapshot so the failure keeps exactly the state it failed at. */
export function runfail(input: { run: runrecord; progress?: planprogress; now: number }): { run: runrecord; frozen?: planprogress } {
  const failed = advancerun(input.run, "failed", input.now);
  if (input.progress === undefined) return { run: failed };
  return { run: failed, frozen: { ...input.progress, ...(input.progress.runid !== undefined ? { runid: input.progress.runid } : { runid: input.run.runid }), updatedat: input.now } };
}

/** Transitions one run to completed only when iscomplete reports the plan finished; an unfinished run never completes. */
export function runcomplete(input: { run: runrecord; plan: agentplan; progress?: planprogress; now: number }): runrecord {
  if (!iscomplete(input.progress, input.plan)) throw new Error(`The run ${input.run.runid} did not finish every step of its plan; an unfinished run never completes.`);
...
export function runfromplan(plan: agentplan, sessionid: string, now: number): runrecord {
  if (plan.state !== "approved" || plan.expiresat <= now) return { runid: plan.id, planid: plan.id, sessionid, state: "failed", createdat: plan.createdat, updatedat: now };
...
export function runsummaryof(input: { run: runrecord; plan: agentplan; progress?: planprogress; checkpoint?: checkpointrecord }): { runid: string; state: runstate; steps: number; completed: number; checkpointstep?: string; updatedat: number } {
  const completed = input.progress?.completedsteps.length ?? 0;
  return { runid: input.run.runid, state: input.run.state, steps: input.plan.steps.length, completed, ...(input.checkpoint !== undefined ? { checkpointstep: input.checkpoint.stepid } : {}), updatedat: input.run.updatedat };
}
...
export function visitof(input: { url: string; title?: string; runid: string; now: number }): urlvisit {
  ...
  return { url: input.url, ...(input.title !== undefined && input.title.trim() !== "" ? { title: input.title } : {}), at: input.now, runid: input.runid };
}
...
export function stepeventof(input: { runid: string; stepid: string; summary: string; ok: boolean; now: number }): runtimelineevent {
  return { at: input.now, runid: input.runid, source: "step", summary: `${input.ok ? "completed" : "failed"}: ${input.summary}`, stepid: input.stepid };
}
...
export function auditeventof(event: auditevent): runtimelineevent {
  return { at: event.at, runid: "", source: "audit", summary: event.summary, ...(event.stepid !== undefined ? { stepid: event.stepid } : {}), kind: event.kind };
}
...
export function visiteventof(visit: urlvisit): runtimelineevent {
  return { at: visit.at, runid: visit.runid, source: "visit", summary: `visited ${visit.url}${visit.title !== undefined ? ` (${visit.title})` : ""}` };
}
...
export function runtimelineof(input: { steps: runtimelineevent[]; audits: runtimelineevent[]; visits: urlvisit[]; runid: string }): runtimelineevent[] {
  const scoped = input.audits.filter(event => event.runid === input.runid || event.runid === "");
  const merged = [...input.steps.filter(event => event.runid === input.runid), ...scoped, ...input.visits.filter(visit => visit.runid === input.runid).map(visiteventof)];
...
export function buckettimeline(events: runtimelineevent[]): Array<{ phase: string; events: runtimelineevent[] }> {
  ...
  for (const event of events) {
    const phase = event.source === "visit" ? "route" : event.source === "audit" ? "audit" : event.summary.startsWith("failed") ? "failures" : "steps";
...
export function tabisolate(input: { tabid: number; config: Record<string, unknown>; runid?: string; now: number }): tabstate {
  if (!Number.isFinite(input.tabid)) throw new Error("The tab isolation needs its tab id.");
  return { tabid: input.tabid, namespace: tabnamespace(input.tabid), config: { ...input.config }, ...(input.runid !== undefined && input.runid.trim() !== "" ? { runid: input.runid } : {}), at: input.now };
}
...
export function acquiresessionlock(input: { runid: string; sessionid: string; window: number; now: number }): lockrecord {
  ...
  return { holder: input.runid, runid: input.runid, sessionid: input.sessionid, acquiredat: input.now, expiresat: input.now + input.window };
}
...
export function lockblocks(lock: lockrecord | undefined, runid: string, now: number): { blocked: boolean; reason: string } {
  if (lock === undefined) return { blocked: false, reason: "No sessionlock stands on the session; the run starts through the reviewed path." };
  if (lock.runid === runid) return { blocked: false, reason: `The sessionlock already belongs to the run ${runid}; the run holds its own lock.` };
  if (lock.expiresat <= now) return { blocked: false, reason: `The sessionlock of the run ${lock.holder} expired past its window; the stale lock leaves before a new run starts.` };
  return { blocked: true, reason: `The sessionlock of the session ${lock.sessionid} stays held by the run ${lock.holder}; the concurrent run ${runid} waits because one session never runs two runs at once.` };
}
...
export function visitsummary(visits: urlvisit[]): { count: number; origins: string[]; first?: number; last?: number } {
  ...
  const origins = [...new Set(scoped.map(visit => { try { return new URL(visit.url).origin; } catch { return ""; } }).filter(origin => origin !== ""))];
  const first = scoped[0]?.at;
  const last = scoped.at(-1)?.at;
  return { count: scoped.length, origins, ...(first !== undefined ? { first } : {}), ...(last !== undefined ? { last } : {}) };
}
...
export function navigationsteps(steps: toolstep[]): toolstep[] {
  return steps.filter(step => ["navigate", "openlink", "followlink", "spanav", "navlist", "batchopen", "openprivate", "reload", "reloadcache", "back", "forward"].includes(step.kind));
}
...
export function runbudgetof(input: { runid: string; steps: number; stepbudget?: number; pressure: number; memorybudget?: number; now: number }): runbudgetrecord {
  ...
  return { runid: input.runid, steps: Math.max(input.steps, 0), ...(input.stepbudget !== undefined ? { stepbudget: input.stepbudget } : {}), pressure, ...(input.memorybudget !== undefined ? { memorybudget: input.memorybudget } : {}), at: input.now };
}
...
export function budgetalertstate(input: { budget: runbudgetrecord; thresholds?: { warning?: number; critical?: number } }): budgetalert[] {
  ...
  if (stepsbudget !== undefined && stepsbudget > 0) {
    ...
    const alert = budgetalertof({ runid: input.budget.runid, kind: "steps", ratio, ...(input.thresholds !== undefined ? { thresholds: input.thresholds } : {}), at: input.budget.at });
  ...
  if (memorybudget !== undefined && memorybudget > 0) {
    ...
    const alert = budgetalertof({ runid: input.budget.runid, kind: "memory", ratio, ...(input.thresholds !== undefined ? { thresholds: input.thresholds } : {}), at: input.budget.at });
    if (alert !== undefined) alerts.push(alert);
  }
  return alerts;
}
...
function budgetalertof(input: { runid: string; kind: "steps" | "memory"; ratio: number; thresholds?: { warning?: number; critical?: number }; at: number }): budgetalert | undefined {
  const critical = input.thresholds?.critical;
  const warning = input.thresholds?.warning;
  if (critical !== undefined && critical > 0 && input.ratio >= critical) return { runid: input.runid, level: "critical", kind: input.kind, ratio: input.ratio, paused: true, at: input.at };
  if (warning !== undefined && warning > 0 && input.ratio >= warning) return { runid: input.runid, level: "warning", kind: input.kind, ratio: input.ratio, paused: false, at: input.at };
  return undefined;
}
...
export function timeoutcanceldecision(input: { elapsed: number; bound?: number }): { abort: boolean; reason: string } {
  if (input.bound === undefined) return { abort: false, reason: "The user set no timeout bound and the engine sets none; the step runs to its end inside the reviewed plan." };
  if (!Number.isFinite(input.elapsed)) return { abort: false, reason: "The step reported no finite elapsed time; the executor never aborts on a telemetry gap." };
  if (input.elapsed <= input.bound) return { abort: false, reason: `The step ran ${input.elapsed} milliseconds inside the user timeout bound of ${input.bound} milliseconds.` };
  return { abort: true, reason: `The step ran ${input.elapsed} milliseconds past the user timeout bound of ${input.bound} milliseconds; the executor aborts it and records the cancel event beside the step outcome.` };
}
...
export function timeoutcanceleventof(input: { runid: string; stepid: string; bound: number; elapsed: number; now: number; logged: boolean }): timeoutcancelevent {
  ...
  return { id: randomid(), runid: input.runid, stepid: input.stepid, bound: Math.max(input.bound, 0), elapsed: Math.max(input.elapsed, 0), at: input.now, logged: input.logged };
}
...
export function resumepointof(input: { runid: string; stepid: string; cursor: number; digest: string; now: number }): resumepoint {
  ...
  if (input.digest.trim() === "") throw new Error("The resume checkpoint needs its page digest; a digestless resume never revalidates the page.");
  return { runid: input.runid, stepid: input.stepid, cursor: Math.max(input.cursor, 0), digest: input.digest, at: input.now };
}
...
export function skipcompletedsteps(input: { checkpoints: resumepoint[]; steps: string[] }): { skip: string[]; resumeat?: string } {
...
export function revalidatefingerprint(input: { checkpoint: resumepoint; fingerprint: string }): { ok: boolean; reason: string } {
  if (input.checkpoint.digest === input.fingerprint) return { ok: true, reason: `The page fingerprint matches the checkpoint digest of the step ${input.checkpoint.stepid}; the resume continues at cursor ${input.checkpoint.cursor}.` };
  return { ok: false, reason: `The page fingerprint changed since the checkpoint of the step ${input.checkpoint.stepid}; the resume refuses so the run never continues against a different page.` };
}
...
export function navdedupeverdict(input: { url: string; activeurl: string }): { skip: boolean; reason: string } {
  if (input.url.trim() === "") return { skip: false, reason: "The planned navigation carries no url; the navigation runs as reviewed." };
  if (input.url === input.activeurl) return { skip: true, reason: `The planned navigation to ${input.url} targets the already active url; the navdedupe skips the duplicate navigation while the step outcome records the skip.` };
  return { skip: false, reason: `The planned navigation to ${input.url} targets a new url; the navigation runs as reviewed.` };
}
...
export function foldplannedvisits(steps: Array<{ id: string; url?: string }>): navdedupeentry[] {
  ...
  for (const step of steps) {
    ...
    if (entry === undefined) { folded.set(step.url, { url: step.url, stepids: [step.id] }); continue; }
...
export function runcachehit(entries: runcacheentry[], input: { runid: string; resource: string }): runcacheentry | undefined {
...
export function runcachesweep(entries: runcacheentry[], input: { runid: string; pin: boolean }): { kept: runcacheentry[]; cleared: number } {
...
export function stepprefetchhints(plan: { steps: Array<{ id: string; kind: string; value?: string; target?: string }> }): stepprefetchhint[] {
  return plan.steps.map((step, index) => {
    const rest = plan.steps.slice(index + 1);
    const nav = rest.find(candidate => ["navigate", "openlink", "followlink"].includes(candidate.kind) && candidate.value !== undefined && candidate.value.trim() !== "");
    const selectors = rest.map(candidate => candidate.target).filter((target): target is string => target !== undefined && target.trim() !== "");
    return { stepid: step.id, ...(nav?.value !== undefined ? { page: nav.value } : {}), selectors: [...new Set(selectors)].slice(0, 5) };
  });
}
...
export function replaystepof(entry: immutablelogentry, index: number, gates: Array<{ gateid: string; kind: string; resolution: string; at: number }>): runreplaystep {
  const refs = restoredrefsof(entry);
  return { stepid: entry.stepid ?? entry.id, index, summary: entry.summary, ...(refs.observationversion !== undefined ? { observationversion: refs.observationversion } : {}), ...(refs.captureid !== undefined ? { captureid: refs.captureid } : {}), gateresolutions: entry.stepid === undefined ? [] : gates.filter(gate => gate.gateid === entry.stepid) };
}
...
export function runreplaysessionof(input: { runid: string; entries: immutablelogentry[]; gates?: Array<{ gateid: string; kind: string; resolution: string; at: number }>; now: number }): runreplaysession {
...
export function replaymove(session: runreplaysession, direction: "forward" | "backward", now: number): runreplaysession {
  ...
  return { ...session, cursor: next, playing: false, actions: [...session.actions, { kind: "step" as const, at: now, ...(step !== undefined ? { stepid: step.stepid } : {}) }] };
}
...
export function replayjump(session: runreplaysession, stepid: string, now: number): runreplaysession {
  ...
  return { ...session, cursor: index, playing: false, actions: [...session.actions, { kind: "jump" as const, stepid, at: now }] };
}
...
export function replayplay(session: runreplaysession, playing: boolean, now: number): runreplaysession {
  return { ...session, playing, actions: [...session.actions, { kind: playing ? "play" as const : "pause" as const, at: now }] };
}

/** Records one viewer action of the replay session for the audit trail: every play, pause, step and jump keeps its time. */
export function replayviewaction(session: runreplaysession, action: { kind: "play" | "pause" | "step" | "jump"; stepid?: string; at: number }): runreplaysession {
  return { ...session, actions: [...session.actions, { kind: action.kind, ...(action.stepid !== undefined ? { stepid: action.stepid } : {}), at: action.at }] };
}
...
export function replayrestoredview(step: runreplaystep): { stepid: string; index: number; summary: string; observationversion?: number; captureid?: string; gateresolutions: Array<{ gateid: string; kind: string; resolution: string; at: number }> } {
  return { stepid: step.stepid, index: step.index, summary: step.summary, ...(step.observationversion !== undefined ? { observationversion: step.observationversion } : {}), ...(step.captureid !== undefined ? { captureid: step.captureid } : {}), gateresolutions: step.gateresolutions };
}
...
export function readparallelgroupof(input: { runid: string; reads: Array<{ stepid: string; origin: string }>; lanes?: number }): readparallelgroup {
  ...
  return { runid: input.runid, lanes: lanes.map(read => ({ stepid: read.stepid, origin: read.origin })), joined: false };
}
...
export function carrywarmselector(input: { selector: string; resolution: string; fingerprint: string; from: string }): warmselectorrecord {
  if (input.selector.trim() === "") throw new Error("The warmselector record needs its selector.");
  if (input.fingerprint.trim() === "") throw new Error("The warmselector record needs its page fingerprint; a fingerprintless carry never revalidates.");
  return { selector: input.selector, resolution: input.resolution, fingerprint: input.fingerprint, carriedfrom: input.from };
}
...
export function warmselectorcarryvalid(input: { record: warmselectorrecord; fingerprint: string }): { carry: boolean; reason: string } {
  if (input.record.fingerprint === input.fingerprint) return { carry: true, reason: `The page fingerprint stayed unchanged since the step ${input.record.carriedfrom}; the carried resolution of ${input.record.selector} holds and needs no revalidation.` };
  return { carry: false, reason: `The page fingerprint changed since the step ${input.record.carriedfrom}; the carried resolution of ${input.record.selector} revalidates before the dispatch.` };
}
...
export function sessionreusegrantof(input: { profile: string; container: string; promptid: string; consentedat: number; runid?: string }): sessionreusegrant {
  if (input.profile.trim() === "") throw new Error("The sessionreuse grant needs its profile.");
  if (input.promptid.trim() === "") throw new Error("The sessionreuse grant needs its consent prompt id; a promptless grant attaches no profile.");
  return { profile: input.profile, container: input.container, promptid: input.promptid, consentedat: input.consentedat, ...(input.runid !== undefined ? { runid: input.runid } : {}) };
}
...
export function backgroundqueueentryof(input: { workflowid: string; summary: string; now: number }): backgroundqueueentry {
...
export function enqueuebackgroundrun(input: { queue: backgroundqueueentry[]; workflowid: string; summary: string; now: number }): backgroundqueueentry[] {
...
export function beginbackgroundrun(entry: backgroundqueueentry, now: number, reviewed: boolean): { entry: backgroundqueueentry; gate: { allowed: boolean; reason: string } } {
  const gate = backgroundrungate({ reviewed, keepaliveheld: true });
  return { entry: gate.allowed ? { ...entry, state: "running" as const, keepaliveheld: true, startedat: now } : entry, gate: { allowed: gate.allowed, reason: gate.reason ?? "" } };
}

/** Finishes one background run: the entry reaches its terminal state and releases the keepalive hold it carried. */
export function finishbackgroundrun(entry: backgroundqueueentry, state: "done" | "failed", now: number): backgroundqueueentry {
...
export function resumebackgroundqueue(queue: backgroundqueueentry[], now: number): { queue: backgroundqueueentry[]; requeued: string[] } {
...
export function backgroundrunattention(entry: backgroundqueueentry, origin: string): { cause: "failure" | "deferral"; runid: string; origin: string; summary: string } | undefined {
...
export function backgroundrunsview(queue: backgroundqueueentry[]): Array<{ id: string; workflowid: string; state: string; keepaliveheld: boolean; queuedat: number; startedat?: number; endedat?: number; summary: string; progress: string }> {
  return queue.map(entry => ({ id: entry.id, workflowid: entry.workflowid, state: entry.state, keepaliveheld: entry.keepaliveheld, queuedat: entry.queuedat, ...(entry.startedat !== undefined ? { startedat: entry.startedat } : {}), ...(entry.endedat !== undefined ? { endedat: entry.endedat } : {}), summary: entry.summary, progress: entry.state === "running" ? `Running with the keepalive signal held since ${entry.startedat ?? entry.queuedat}` : entry.state === "queued" ? "Queued for the next executor turn" : entry.state === "done" ? "Completed in the background" : "Failed; the attentionfeed carries the cause" }));
}
...
export function backgroundtrayrows(queue: backgroundqueueentry[], origin: string): Array<{ runid: string; origin: string; outcome: "running" | "completed" | "halted" | "failed"; title: string; at: number; resumable: boolean; reopenable: boolean }> {
...
export function cancelbackgroundentry(queue: backgroundqueueentry[], id: string): { queue: backgroundqueueentry[]; cancelled: string[] } {
